...
1. 가져오기를 사용하여 카탈로그 [를](#catalog-import) AEM의 특정 위치로 가져옵니다.
...
1. 세로 막대에서 **시스템**, **패싯 검색**, 패싯 검색 구성 **을 차례로 선택합니다**.
1. **Open Editor for the** Sample Solr Configuration for clothing ****.

1. [ **카탈로그] 버전** 에서 [카탈로그 버전 **** 추가]를 사용하여 `outdoors-Staged` 목록 `outdoors-Online` 과목록을 추가합니다.
1. **구성을 저장합니다.**
1. SOLR **항목 유형을 열어** SOLR **정렬**`ClothesVariantProduct`을 추가할 수있습니다.
   ...
   * price-desc(&quot;가격(내림차순)&quot;, priceValue)
   ...
   >[!NOTE]
   >
   >컨텍스트 메뉴(일반적으로 오른쪽 단추 클릭)를 사용하여 선택합니다 `Create Solr sort`.
   >
   >Hybris 5.0.0의 경우 `Indexed Types` 탭을 열고 두 번 클릭한 `ClothesVariantProduct`다음 탭을 클릭합니다 `SOLR Sort`.

   ![chlimage_1-36](assets/chlimage_1-36.png)

1. 인덱스 **유형** 탭에서 **작성 유형** 을 다음과 같이 설정합니다.

   `Product - Product`

1. 인덱스 유형 **탭에서** 인덱서 **쿼리를** 다음 항목에 대해 `full`조정합니다.

   ```shell
   SELECT {pk} FROM {Product} WHERE {pk} NOT IN ({{SELECT {baseProductpk} FROM {variantproduct}}})
   ```

1. 인덱스 유형 **탭에서** 인덱서 **쿼리를** 다음 항목에 대해 `incremental`조정합니다.

   ```shell
   SELECT {pk} FROM {Product} WHERE {pk} NOT IN ({{SELECT {baseProductpk} FROM {variantproduct}}}) AND {modifiedtime} <= ?lastIndexTime
   ```

1. 인덱스 유형 **탭에서 패싯을** `category` 조정합니다. 범주 목록의 마지막 항목을 두 번 클릭하여 인덱스 **속성** 탭을 엽니다.
...
1. **변경 사항을 저장합니다.**
1. SOLR **항목 유형에서도**&#x200B;다음 스크린샷에 따라 `price` 패싯을 조정합니다. 와 `category`같이, Indexed 속성 탭 `price` 을 두 번 클릭하여 **엽니다** .
...
1. **변경 사항을 저장합니다.**
1. Open **System**, **Facet 검색**, Indexer **작업 마법사**. cronjob 시작:
...
**CQ Commerce Hybris 구성**() `com.adobe.cq.commerce.hybris.common.DefaultHybrisConfigurationService`)

**카탈로그 버전** 은 일반적으로 `Online` 또는 `Staged` (기본값)으로 설정됩니다.

>[!NOTE]
>
>When working with AEM there are several methods of managing the configuration settings for such services; see [Configuring OSGi](/help/sites-deploying/configuring-osgi.md) for full details. 구성 가능한 매개 변수와 기본값을 전체 목록으로 보려면 콘솔을 참조하십시오.
...
이러한 구조는 인터페이스를 구현하는 OSGi 서비스 `DefaultImportHandler` 에 의해 `ImportHandler` 만들어집니다. 제품, 제품 변형, 카테고리, 자산 등을 만들기 위해 실제 가져오기에서 가져오기 핸들러를 호출합니다.

>[!NOTE]
>
>직접 가져오기 핸들러를 구현하여 이 프로세스를 [사용자 지정할 수 있습니다](#configure-the-import-structure).
...
1. OSGi 번들 구성:
   ...
   **CQ Commerce Hybris 기본 응답 파서**(`com.adobe.cq.commerce.hybris.impl.importer.DefaultResponseParser`)
...
>[!NOTE]
>
>hybris 구현(예: `geometrixx-outdoors/en_US`)은 제품 ID 및 기타 기본 정보만 아래에 `/etc/commerce`저장합니다.
>
...
1. 필요한 경우 CRXDE Lite을 사용하여 기존 제품 데이터를 모두 삭제합니다.
   ...
   1. 제품 데이터를 보관하는 노드를 삭제합니다. 예를 들면 다음과 같습니다 `outdoors`.
...
1. Clickbox **Express 업데이트를 선택합니다**.
1. 카탈로그 **가져오기를** 클릭하여 가져오기를 시작합니다.
...
* **기본 저장소**&#x200B;하이브리스에 구성된 기본 저장소의 식별자입니다.

* **카탈로그**&#x200B;가져올 카탈로그의 식별자입니다.

* **루트 경로**&#x200B;카탈로그를 가져올 경로입니다.

## 카탈로그에서 제품 제거 {#removing-a-product-from-the-catalog}

카탈로그에서 하나 이상의 제품을 제거하려면 다음을 수행합니다.

1. [OSGi 서비스](/help/sites-deploying/configuring-osgi.md) 요일 CQ 커머스 하이브리스 카탈로그 가져오기 ****&#x200B;에 대한 구성; 카탈로그 가져오기 [구성을 참조하십시오](#configure-the-catalog-importer).
...
1. [적절한 카탈로그를 롤아웃합니다](/help/sites-administering/generic.md#rolling-out-a-catalog) . 제품 및 제품 페이지는 AEM 내에서 제거됩니다.
   ...
   * 해당 `Cajamara` 제품은 카테고리에서 `Bike` 제거됩니다

1. 제품을 다시 설치하려면:

   1. 하이브리스에서 승인 상태를 다시 **승인됨으로 설정합니다.**
   1. AEM:
...
주문 내역을 [클라이언트 컨텍스트에 추가하려면](/help/sites-developing/client-context.md):
...
1. [Order **History** 구성](/help/sites-administering/client-context.md#adding-a-property-component) 요소를 **클라이언트 컨텍스트의**&#x200B;장바구니 구성요소에 추가합니다.
1. 클라이언트 컨텍스트가 주문 내역에 대한 세부 사항을 표시하고 있는지 확인할 수 있습니다. 예:
   ...
   >  캠페인은 하나의 경험으로 구성됩니다.
   >
   >* 세그먼트(http://localhost:4502/etc/segmentation/geometrixx-outdoors/returning-customer.html)을[클릭합니다](http://localhost:4502/etc/segmentation/geometrixx-outdoors/returning-customer.html).
      >
      >
