...
>[!NOTE]
>
>블루프린트 구성을 사용하면 컨텐츠 변경 사항을 Live Copy에 푸시할 수 있습니다. Live [Copy - 소스, Blueprint 및 Blueprint 구성을 참조하십시오](/help/sites-administering/msm.md#source-blueprints-and-blueprint-configurations).
...
블루프린트 구성을 사용하면 소스/블루프린트의 Live Copy 동기화 방법을 결정하는 롤아웃 구성에 블루프린트 구성을 연결할 수 있습니다. See [Specifying the Rollout Configurations To Use](/help/sites-administering/msm-sync.md#specifying-the-rollout-configurations-to-use).

### 블루프린트 구성 만들기 {#creating-a-blueprint-configuration}

블루프린트 구성을 만들려면

1. [도구](/help/sites-authoring/basic-handling.md#global-navigation) 메뉴 **로** 이동한 다음 **사이트** 메뉴를 선택합니다.
1. Blueprint **를** 선택하여 **Blueprint 구성 콘솔을** 엽니다.

   ![chlimage_1-209](assets/chlimage_1-209.png)

1. **만들기**&#x200B;를 선택합니다.
1. 블루프린트 템플릿을 선택한 다음 **다음을** 클릭하여 계속합니다.
1. 블루프린트로 사용할 소스 페이지를 선택합니다. 다음 **을** 계속 진행합니다.
1. 정의:
...
* 블루프린트 구성에서 참조되는 소스 페이지 **에서** Live Copy를 만드는 경우 소스/블루프린트 페이지에서 롤아웃 명령을 사용할 수 있습니다.
* 블루프린트 구성에서 참조되지 **않는** 소스 페이지에서 Live Copy를 만드는 경우 소스/블루프린트 페이지에서 롤아웃 명령을 사용할 수 없습니다.

Live Copy를 만들려면

1. 사이트 **콘솔** 에서 **만들기**, **Live Copy를**&#x200B;선택합니다.
...
1. Live Copy의 대상 경로(Live Copy의 상위 폴더/페이지 열기)를 지정하고 다음을 클릭하거나 **탭합니다**.
...
1. 하위 페이지 **제외 확인란을** 사용합니다.
   ...
   * 선택되지 않음: 선택한 페이지의 모든 하위 항목을 포함하는 Live Copy 만들기(딥 Live Copy)
...
### 블루프린트 구성에서 사이트 Live Copy 생성 {#creating-a-live-copy-of-a-site-from-a-blueprint-configuration}

블루프린트 구성을 사용하여 Live Copy를 만들어 블루프린트(소스) 컨텐츠를 기반으로 사이트를 만듭니다. 블루프린트 구성에서 Live Copy를 만들 때 블루프린트 소스의 언어 분기를 하나 이상 선택하고 언어 분기에서 복사할 장을 선택합니다. 블루프린트 [구성 만들기를 참조하십시오](/help/sites-administering/msm-livecopy.md#creating-a-blueprint-configuration).

Live Copy에서 일부 언어 분기 또는 장을 생략하면 나중에 추가할 수 있습니다. Live Copy [에서 Live Copy 만들기(블루프린트 구성)를 참조하십시오](#creating-a-live-copy-inside-a-live-copy-blueprint-configuration).
...
* **롤아웃 구성**: (선택 사항) Live Copy를 동기화하는 데 사용할 하나 이상의 롤아웃 구성을 선택합니다. 기본적으로 롤아웃 구성은 블루프린트에서 상속됩니다. 자세한 [내용은 롤아웃 구성 지정을](/help/sites-administering/msm-sync.md#specifying-the-rollout-configurations-to-use) 참조하십시오.

블루프린트 구성에서 사이트의 Live Copy를 만들려면:

1. 사이트 **콘솔** 에서 **만들기**, **** 드롭다운 선택기에서사이트를선택합니다.
1. Live Copy의 소스로 사용할 블루프린트 구성을 선택하고 **다음**&#x200B;작업을 진행합니다.

   ![chlimage_1-216](assets/chlimage_1-216.png)

1. [ **초기 언어** ] 선택기를 사용하여 Live Copy에 사용할 블루프린트 사이트의 언어를 지정합니다.

   사용 가능한 모든 언어는 기본적으로 선택됩니다. 언어를 제거하려면 언어 옆에 표시되는 **X** 를 클릭하거나 탭합니다.
...
1. 나머지 속성에 대한 값을 제공한 다음 만들기를 **선택합니다**. 확인 대화 상자에서 **완료를** 선택하여 **사이트** 콘솔로 돌아가거나 **** 사이트열기를 선택하여 사이트의 루트 페이지를 엽니다.
...
블루프린트 페이지(블루프린트 구성에서 참조됨)는 현재(블루프린트) 페이지를 소스로 사용하는 Live Copy 페이지 목록을 제공합니다. 이 목록을 사용하여 Live Copy를 추적할 수 있습니다. 목록은 **페이지 속성의** 블루프린트 [탭에](/help/sites-authoring/editing-page-properties.md)나타납니다.
...
콘텐츠 변경 사항을 Live Copy로 푸시할 블루프린트 페이지를 롤아웃합니다. 롤아웃 **작업** 은 롤아웃 시 트리거를 사용하는 [롤아웃 구성을](/help/sites-administering/msm-sync.md#rollout-triggers) 실행합니다.

>[!NOTE]
>
>블루프린트 분기와 종속 Live Copy 분기 모두에서 페이지 이름이 같은 새 페이지가 생성되면 충돌이 발생할 수 있습니다.
>
>이러한 [충돌은 롤아웃 시 처리 및 해결되어야 합니다](/help/sites-administering/msm-rollout-conflicts.md).


#### 페이지 속성에서 블루프린트 롤아웃 {#rolling-out-a-blueprint-from-page-properties}

1. 사이트 **** 콘솔에서 블루프린트의 페이지를 선택하고 속성을 엽니다.
...
>[!NOTE]
>
>동기화는 롤아웃 시 트리거를 사용하는 [롤아웃 구성을](/help/sites-administering/msm-sync.md#rollout-triggers) 실행합니다.
...
1. Click or tap **Synchronize**.
   ...
   확인이 요청됩니다. **동기화를** 사용하여 진행하십시오.

#### Live Copy 개요에서 Live Copy 동기화 {#synchronize-a-live-copy-from-the-live-copy-overview}

Live Copy 페이지 [를 선택하면 Live Copy 개요에서도 동기화 작업을](/help/sites-administering/msm-livecopy-overview.md#using-the-live-copy-overview)사용할 수 있습니다.
...
Live Copy 페이지가 소스 페이지와 동기화되면 추가된 구성 요소는 변경되지 않습니다. Live [Copy 페이지에서 구성 요소 순서 변경을 참조하십시오](#changing-the-order-of-components-on-a-live-copy-page).

>[!NOTE]
>
>컨테이너로 표시된 구성 요소에 로컬로 수행된 변경 사항은 롤아웃 시 블루프린트 컨텐츠로 덮어쓰지 않습니다. See [MSM Best Practices](/help/sites-administering/msm-best-practices.md#components-and-container-synchronization) for more information.
...
>[!NOTE]
>
>또한 Live Copy [를 블루프린트에서](#detaching-a-live-copy) 분리하여 모든 연결을 제거할 수도 있습니다. 분리 동작은 영구적이며 되돌릴 수 없습니다.

#### 페이지 속성에서 상속 일시 중단 {#suspending-inheritance-from-page-properties}

페이지의 상속을 일시 중단하려면

1. 사이트 콘솔의 속성 **보기** 명령을 사용하거나 **페이지 도구 모음에서** 페이지 정보 **를** 사용하여 Live Copy 페이지의 속성을엽니다.
...
일시 [중단 작업은 Live Copy 페이지가 선택되면 Live Copy 개요](/help/sites-administering/msm-livecopy-overview.md#using-the-live-copy-overview)에서 사용할 수도 있습니다.
...
페이지에 대한 Live Copy 상속 일시 중단은 임시 작업입니다. 일시 중단된 **재개** 조치를 사용할 수 있게 되면 라이브 관계를 복원할 수 있습니다.

상속을 다시 활성화하면 페이지가 소스와 자동으로 동기화되지 않습니다. 필요한 경우 동기화를 요청할 수 있습니다.

* 다시 **시작**/**되돌리기** 대화 상자 예를 들면 다음과 같습니다.
...
일시 [중단된](#suspending-inheritance-from-page-properties) 경우 **다시 시작** 작업은 페이지 속성의 도구 모음에 표시됩니다.
...
Live Copy 페이지 [를 선택하면 Live Copy 개요](/help/sites-administering/msm-livecopy-overview.md#using-the-live-copy-overview)에서 다시 시작 작업을 사용할 수도 있습니다.
...
* 약식 Live Copy로 전환:
   ...
   * 중첩된 `LiveRelationships` 경우에도 하위 항목이 제거됩니다 `LiveCopies`.
...
* 얕은 Live Copy로 전환한 다음 심도로 돌아옵니다.
   ...
   * (이전의) 약식 live copy의 모든 하위 항목은 수동으로 생성된 것처럼 취급되므로 를 사용하여 제거됩니다 `[oldname]_msm_moved name`.

깊이를 지정하거나 변경하려면

1. 사이트 콘솔의 속성 **보기** 명령을 사용하거나 **페이지 도구 모음에서** 페이지 정보 **를** 사용하여 Live Copy 페이지의 속성을엽니다.
1. Live Copy **탭을** 클릭하거나 탭합니다.
1. 구성 **섹션** 에서 하위 페이지 포함 여부에 따라 **Live Copy 상속** 옵션을 설정하거나 지웁니다.
   ...
   * clear - 약식 live copy(하위 페이지는 제외됨)
   ...
   >
   >자세한 내용은 [Live Copy - 컴포지션을](/help/sites-administering/msm.md#live-copies-composition) 참조하십시오.

1. 업데이트를 **유지하려면 저장을** 클릭하거나 탭하십시오.
...
>[!NOTE]
>
>단락 시스템에서 취소된 상속을 되돌리면 구성 요소의 순서가 설계에서 자동으로 복원되지 **않습니다** . 필요한 경우 동기화를 수동으로 요청할 수 있습니다.

단락 시스템의 상속을 취소하려면 다음 절차를 따르십시오.

1. Live Copy 페이지를 엽니다.
1. 기존 구성 요소를 페이지의 새 위치로 드래그합니다.
1. 상속 **취소** 대화 상자에서 작업을 **예**&#x200B;로 확인합니다.
...
1. 사이트 콘솔의 속성 **보기** 옵션 또는 **페이지 도구 모음의** 페이지 정보 **아이콘을 사용하여** Live Copy 페이지의 속성을엽니다.
...
1. Live Copy **재설정** 대화 상자에서 재설정으로 **확인합니다**.

#### Live Copy 개요에서 Live Copy 페이지 재설정 {#reset-a-live-copy-page-from-the-live-copy-overview}

Live Copy 페이지 [를 선택하면 Live Copy 개요](/help/sites-administering/msm-livecopy-overview.md#using-the-live-copy-overview)에서 재설정 작업을 사용할 수도 있습니다.
...
변경 내용을 추적하려면 References에서 블루프린트 페이지를 보고 **Live Copy** 페이지와 비교할 수 있습니다.
...
>
>나중에 다시 설치하는 옵션과의 라이브 관계를 제거하려면 페이지의 Live Copy 상속을 [취소할 수](#suspending-inheritance-for-a-page) 있습니다.

분리를 사용하는 트리 내의 위치에 대한 **의미가 있습니다**.

* **LiveCopy의 루트 페이지에서 분리**

   Live Copy의 루트 페이지에서 이 작업을 수행하면 블루프린트와 Live Copy의 모든 페이지 간의 라이브 관계가 제거됩니다.

   설계도(있는 그대로)의 페이지에 대한 추가 변경 사항은 Live Copy에 영향을 주지 **않습니다** .
...
* **LiveCopy의 하위 페이지에서 분리**
   ...
   *하지만*&#x200B;하위 페이지는 여전히 상위 분기의 라이브 관계를 따르므로 더 많은 블루프린트 페이지 롤아웃이 둘 다 됩니다.
   ...
   >[!NOTE]
   >
   >이러한 [상황에 대한 자세한 내용은 MSM 롤아웃 충돌을](/help/sites-administering/msm-rollout-conflicts.md) 참조하십시오.
...
1. Live Copy **분리** 대화 상자에서 **분리** 작업을확인합니다.
