---
title: Dynamic Tag Management와 통합 Adobe
seo-title: Dynamic Tag Management와 통합 Adobe
description: Dynamic Tag Management와의 통합에 대해 알아봅니다.
seo-description: Dynamic Tag Management와의 통합에 대해 알아봅니다.
...
Dynamic Tag Management 웹 속성을 사용하여 AEM 사이트를 추적할 수 있도록 [Adobe Dynamic Tag Management](https://www.adobe.com/solutions/digital-marketing/dynamic-tag-management.html)을 AEM과 통합합니다. Dynamic Tag Management를 통해 마케터는 데이터를 수집하기 위한 태그를 관리하고 디지털 마케팅 시스템에 데이터를 배포할 수 있습니다. 예를 들어, Dynamic Tag Management를 사용하여 AEM 웹 사이트에 대한 사용 데이터를 수집하고 Adobe Analytics 또는 Adobe Target에서 분석할 데이터를 배포합니다.

통합하기 전에 AEM 사이트의 도메인을 추적하는 Dynamic Tag Management [웹 속성](https://microsite.omniture.com/t2/help/en_US/dtm/#Web_Properties)을 만들어야 합니다. Dynamic Tag Management 라이브러리에 액세스하도록 AEM을 구성할 수 있도록 웹 속성의 [호스팅 옵션](https://microsite.omniture.com/t2/help/en_US/dtm/#Hosting__Embed_Tab)을 구성해야 합니다.

통합을 구성한 후 Dynamic Tag Management 배포 도구 및 규칙을 변경해도 AEM에서 Dynamic Tag Management 구성을 변경할 필요가 없습니다. 변경 사항은 AEM에서 자동으로 사용할 수 있습니다.
...
다음 배포 옵션은 Dynamic Tag Management와의 통합 구성에 영향을 줍니다.

### Dynamic Tag Management 호스팅 {#dynamic-tag-management-hosting}

AEM은 클라우드에서 호스팅되거나 AEM에서 호스팅되는 Dynamic Tag Management를 지원합니다.

* 클라우드 호스팅:Dynamic Tag Management Javascript 라이브러리는 클라우드에 저장되고 AEM 페이지가 직접 참조합니다.
* AEM 호스팅:Dynamic Tag Management는 Javascript 라이브러리를 생성합니다. AEM은 워크플로우 모델을 사용하여 라이브러리를 가져오고 설치합니다.
...
일반적으로 작성 인스턴스는 Dynamic Tag Management 스테이징 라이브러리를 사용하고 프로덕션 인스턴스는 프로덕션 라이브러리를 사용합니다. 이 시나리오를 사용하면 작성자 인스턴스를 사용하여 승인되지 않은 Dynamic Tag Management 구성을 테스트할 수 있습니다.
...
배포 후크를 사용하려면 Dynamic Tag Management가 라이브러리를 호스팅하는 AEM 인스턴스에 연결할 수 있어야 합니다. Dynamic Tag Management 서버에 대해 [AEM](/help/sites-administering/dtm.md#enabling-access-for-the-deployment-hook-service)에 대한 액세스를 활성화해야 합니다.
...
## Dynamic Tag Management 구성 만들기 {#creating-the-dynamic-tag-management-configuration}

AEM 인스턴스가 Dynamic Tag Management로 인증하고 웹 속성과 상호 작용할 수 있도록 클라우드 구성을 만듭니다.

>[!NOTE]
>
>DTM 웹 속성에 Adobe Analytics 도구가 포함되어 있고 [Content Insight](/help/sites-authoring/content-insights.md)도 사용하는 경우 페이지에 두 개의 Adobe Analytics 추적 코드가 포함되어 있지 않도록 합니다. [Adobe Analytics 클라우드 구성](/help/sites-administering/adobeanalytics-connect.md#configuring-the-connection-to-adobe-analytics)에서 추적 코드 포함하지 않음 옵션을 선택합니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>API 토큰</td> 
   <td>Dynamic Tag Management 사용자 계정의 API 토큰 속성 값입니다. AEM에서는 이 속성을 사용하여 Dynamic Tag Management로 인증합니다.</td> 
  </tr> 
  <tr> 
   <td>회사</td> 
...
Dynamic Tag Management 구성의 다음 속성을 사용하면 AEM에서 Dynamic Tag Management 라이브러리를 호스팅할 수 있습니다. 속성을 사용하면 AEM에서 라이브러리를 다운로드하고 설치할 수 있습니다. 선택적으로, 라이브러리를 자동으로 업데이트하여 Dynamic Tag Management 애플리케이션에서 수행한 변경 사항을 반영하도록 할 수 있습니다.

일부 속성은 Dynamic Tag Management 웹 속성에 대한 포함 탭의 라이브러리 다운로드 섹션에서 가져오는 값을 사용합니다. 자세한 내용은 다이내믹 태그 관리 도움말의 [라이브러리 다운로드](https://microsite.omniture.com/t2/help/en_US/dtm/#Library_Download)를 참조하십시오.

>[!NOTE]
>
>AEM에서 Dynamic Tag Management 번들을 호스팅하는 경우 구성을 만들려면 먼저 Dynamic Tag Management에서 라이브러리 다운로드를 활성화해야 합니다. 또한 Akamai가 다운로드용 라이브러리를 제공하므로 Akamai를 활성화해야 합니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>DTM 번들 URL</td> 
   <td>Dynamic Tag Management 라이브러리를 다운로드하는 데 사용할 URL입니다. Dynamic Tag Management의 라이브러리 다운로드 페이지의 URL 다운로드 섹션에서 이 값을 가져옵니다. 보안상의 이유로 이 값을 수동으로 구성해야 합니다.</td> 
  </tr> 
  <tr> 
   <td>다운로드 워크플로우</td> 
  ...
  <tr> 
   <td>도메인 힌트</td> 
   <td><p>(선택 사항) Dynamic Tag Management 라이브러리를 호스팅하는 AEM 서버의 도메인입니다. <a href="/help/sites-developing/externalizer.md">일 CQ Link Externalizer 서비스</a>에 대해 구성된 기본 도메인을 재정의할 값을 지정합니다.</p> <p>Dynamic Tag Management에 연결되면 AEM은 이 값을 사용하여 Dynamic Tag Management 웹 속성에 대한 라이브러리 다운로드 속성의 스테이징 HTTP 경로 또는 프로덕션 HTTP 경로를 구성합니다.</p> </td> 
  </tr> 
  <tr> 
   <td>보안 도메인 힌트</td> 
   <td><p>(선택 사항) HTTPS를 통해 Dynamic Tag Management 라이브러리를 호스팅하는 AEM 서버의 도메인입니다. <a href="/help/sites-developing/externalizer.md">일 CQ Link Externalizer 서비스</a>에 대해 구성된 기본 도메인을 재정의할 값을 지정합니다.</p> <p>Dynamic Tag Management에 연결된 경우 AEM은 이 값을 사용하여 Dynamic Tag Management 웹 속성에 대한 라이브러리 다운로드 속성의 스테이징 HTTPS 경로 또는 프로덕션 HTTPS 경로를 구성합니다.</p> </td> 
  </tr> 
  <tr> 
   <td>공유 비밀</td> 
   <td><p>(선택 사항) 다운로드 암호를 해독하는 데 사용할 공유 암호입니다. Dynamic Tag Management의 라이브러리 다운로드 페이지의 공유 암호 필드에서 이 값을 가져옵니다.</p> <p><strong>참고:</strong>  AEM에서 다운로드한  <a href="https://www.openssl.org/docs/apps/openssl.html"></a> 라이브러리를 해독할 수 있도록 AEM이 설치된 컴퓨터에 OpenSSL 라이브러리가 설치되어 있어야 합니다.</p> </td> 
  </tr> 
  <tr> 
   <td>폴링 가져오기 활성화</td> 
   <td><p>(선택 사항) 를 선택하여 Dynamic Tag Management 라이브러리를 주기적으로 다운로드 및 설치하여 업데이트된 버전을 사용 중인지 확인합니다. 선택한 경우, Dynamic Tag Management는 HTTP POST 요청을 배포 후크 URL에 보내지 않습니다.</p> <p>AEM은 Dynamic Tag Management 웹 속성에 대한 라이브러리 다운로드 속성의 배포 후크 URL 속성을 자동으로 구성합니다. 선택하면 속성이 값 없이 구성됩니다. 선택하지 않으면 속성은 Dynamic Tag Management 구성의 URL로 구성됩니다.</p> <p>Dynamic Tag Management 배포 후크가 AEM에 연결할 수 없는 경우(예: AEM이 방화벽 뒤에 있을 경우) 폴링 가져오기를 활성화합니다.</p> </td> 
  </tr> 
  <tr> 
   <td>예약 표현식</td> 
...
다음 절차에서는 터치에 적합한 UI를 사용하여 Dynamic Tag Management와의 통합을 구성합니다.

1. 레일에서 도구 > 작업 > 클라우드 > Cloud Services 를 클릭합니다.
1. Dynamic Tag Management 영역에서 구성을 추가하기 위해 다음 링크 중 하나가 나타납니다.
...
1. DTM에 연결 을 클릭합니다. AEM은 Dynamic Tag Management를 사용하여 인증하고 계정과 연결된 회사 목록을 검색합니다.
...
## Dynamic Tag Management 라이브러리 {#manually-downloading-the-dynamic-tag-management-library} 수동으로 다운로드
...
1. 레일에서 도구 > 작업 > 클라우드 > Cloud Services 를 클릭합니다.
1. Dynamic Tag Management 영역에서 구성 표시 를 클릭한 다음 구성을 클릭합니다.
...
>
>다음 항목은 [DTM 구성](#creating-the-dynamic-tag-management-configuration)에서 직접 가져옵니다.
>
>* `myconfig`
>* `companyID`
>* `propertyID`
...
## Dynamic Tag Management 구성을 사이트 {#associating-a-dynamic-tag-management-configuration-with-your-site}에 연결

Dynamic Tag Management 구성을 웹 사이트의 페이지에 연결하여 AEM이 페이지에 필요한 스크립트를 추가합니다. 사이트의 루트 페이지를 구성과 연결합니다. 해당 페이지의 모든 하위 항목은 연결을 상속합니다. 필요한 경우 하위 페이지에서 연결을 재정의할 수 있습니다.

다음 절차를 사용하여 페이지 및 하위 항목을 Dynamic Tag Management 구성과 연결합니다.

1. 클래식 UI에서 사이트의 루트 페이지를 엽니다.
1. 사이드킥을 사용하여 페이지 속성을 엽니다.
1. Cloud Services 탭에서 서비스 추가 를 클릭하고 다이내믹 태그 관리 를 선택한 다음 확인 을 클릭합니다.
