...
description: AEM과 Dynamic Media Classic을 통합하는 방법을 알아봅니다.
seo-description: AEM과 Dynamic Media Classic을 통합하는 방법을 알아봅니다.
...
AEM Dynamic Media는 AEM 플랫폼 내에서 바로 Dynamic Media Classic 기능이 통합되어 있습니다.
...
1. 사용 사례와 관련된 특정 지침을 따르십시오.
   ...
   <table> 
    ...
    <td>AEM에 자산을 업로드하고 AEM Dynamic Media 구성 요소를 사용하여 사이트 페이지에서 자산을 작성합니다</td> 
    ...
    <td>관리 및 전달을 위해 NON-product 자산을 AEM에 업로드합니다. PRODUCT 자산을 Dynamic Media Classic에 업로드하고 AEM 및 구성 요소의 Dynamic Media Classic 컨텐츠 브라우저를 사용하여 사이트에서 제품 세부 사항 페이지를 작성합니다.</td> 
    <td><p>사용</p> <p>(3단계 참조)</p> </td> 
    <td><a href="/help/assets/adding-dynamic-media-assets-to-pages.md">사용</a></td> 
    ...
    <td>AEM Assets에 자산 업로드 및 Dynamic Media의 게시된 URL/포함 코드 사용</td> 
    <td><p>사용</p> <p>(3단계 참조)</p> </td> 
    <td>끔</td> 
    <td>끔</td> 
    <td>끔</td> 
    ...
    <td>DAM에 자산을 업로드하고 Dynamic Media Classic에 자동으로 게시하여 전달할 수 있습니다. AEM Dynamic Media Classic 컨텐츠 브라우저를 사용하여 사이트 페이지에서 자산을 검색하고 작성합니다.</td> 
    <td>끔</td> 
    <td>끔</td> 
    <td><a href="/help/assets/scene7.md#scene-content-browser">사용</a></td> 
    ...
    <td><p>AEM에 자산을 업로드하고 Dynamic Media를 사용하여 다운로드/공유를 위한 변환을 생성합니다. 전달을 위해 AEM 자산을 Dynamic Media Classic에 자동으로 게시합니다.</p> <p><strong>중요:</strong> AEM에서 생성된 중복 처리 및 변환은 Dynamic Media Classic에 동기화되지 않습니다.</p> </td> 
    <td><p>사용</p> <p>(3단계 참조)</p> </td> 
    <td>끔</td> 
    <td>끔</td> 
    <td><p><a href="#configuringautouploadingfromaemassets">사용</a></p> <p>(4단계 참조)</p> </td> 
...
Dynamic Media Classic 및 Dynamic Media를 모두 활성화한 경우 다음과 같은 제한 사항이 있습니다.
...
>[!CAUTION]
>
>기존 Dynamic Media Classic 회사 계정에서 자산을 가져오는 데 시간이 오래 걸릴 수 있습니다. 에셋이 너무 많지 않은 폴더를 Dynamic Media Classic에서 지정하는지 확인하십시오(예: 루트 폴더에 에셋이 너무 많아서 시스템이 충돌할 수 있음).

### AEM Assets과 Dynamic Media Classic에서 자산 업로드 {#uploading-assets-from-aem-assets-versus-from-scene}
...
자산이 이미 Dynamic Media Classic에 있고 일반 저장소에서 해당 자산을 관리하는 것이 중요하지 않은 경우 해당 자산을 AEM Assets으로 내보내 Dynamic Media Classic으로 다시 동기화하여 전달하면 불필요한 양방향 이동이 발생합니다. 그렇지 않으면 단일 저장소에 에셋을 유지하고 배달을 위해 Dynamic Media Classic에만 동기화하는 것이 더 좋을 수 있습니다.

## Dynamic Media Classic 통합 구성 {#configuring-scene-integration}

Dynamic Media Classic에 자산을 업로드하도록 AEM을 구성할 수 있습니다. CQ 대상 폴더의 에셋은 AEM에서 Dynamic Media Classic 회사 계정으로 업로드(자동 또는 수동으로)할 수 있습니다.

>[!NOTE]
>
>Adobe에서는 Dynamic Media Classic 자산을 가져오는 데 지정된 대상 폴더만 사용하는 것이 좋습니다. 대상 폴더 외부에 있는 디지털 자산은 Dynamic Media Classic 구성이 활성화된 페이지의 Dynamic Media Classic 구성 요소에서만 사용할 수 있습니다. 또한 Dynamic Media Classic의 임시 폴더에 배치됩니다. 임시 폴더는 AEM과 동기화되지 않지만 Dynamic Media Classic 컨텐츠 브라우저에서 자산을 검색할 수 있습니다.
...
1. AEM 에셋을 Dynamic Media Classic에 자동으로 업로드하려면 해당 옵션을 활성화하고 DAM 에셋 업데이트 워크플로우에 Dynamic Media Classic을 추가해야 합니다. 자산을 수동으로 업로드할 수도 있습니다.
...
>[!NOTE]
>
>AEM은 모든 메타데이터를 Dynamic Media Classic에 업로드하기 전에 XMP으로 포함하므로 메타데이터 노드의 모든 속성을 XMP으로 Dynamic Media Classic에서 사용할 수 있습니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>AEM에서 Dynamic Media Classic 업로드를 위해 지정된 폴더만 가질 수 있습니다. Dynamic Media Classic에서 두 개 이상의 회사 계정에 액세스해야 하는 경우 여러 구성을 만들 수 있습니다.</td> 
  </tr> 
  <tr> 
   <td>폴더 구조</td> 
  ...
  <tr> 
   <td>임시 폴더</td> 
   <td>WCM에서 Dynamic Media Classic으로 수동으로 업로드된 대상 폴더 외부에 있는 자산은 Dynamic Media Classic의 별도의 임시 폴더에 자동으로 배치됩니다. AEM의 클라우드 구성에서 구성합니다.</td> 
  </tr> 
  <tr> 
   <td>혼합 미디어</td> 
  ...
  <tr> 
   <td>Target 하위 폴더</td> 
   <td><p>대상 폴더 내의 하위 폴더를 사용하는 경우 위치에 관계없이 각 자산에 대해 고유한 이름을 사용하거나 설정 영역에서 Dynamic Media Classic을 구성하여 위치에 관계없이 에셋을 덮어쓰지 않도록 하십시오.</p> <p>그렇지 않으면 Dynamic Media Classic 대상 하위 폴더에 업로드되는 동일한 이름의 자산이 업로드되지만 대상 폴더에 있는 동일한 이름의 자산이 삭제됩니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
>[!CAUTION]
>
>기존 Dynamic Media Classic 회사 계정에서 자산을 가져오는 데 시간이 오래 걸릴 수 있습니다. 자산이 너무 많지 않은 폴더를 Dynamic Media Classic에서 지정하는지 확인합니다(예: 루트 폴더에 자산이 너무 많은 경우가 많음).
>
...
1. Dynamic Media Classic 계정의 이메일 주소, 암호 및 영역을 입력하고 **[!UICONTROL Connect를 눌러 Dynamic Media Classic을 누릅니다]**. Dynamic Media Classic 서버에 연결되고 대화 상자가 더 많은 옵션으로 확장됩니다.

1. 회사 **[!UICONTROL 이름]** 및 **[!UICONTROL 루트 경로]** (지정할 경로와 함께 게시된 서버 이름입니다. 게시된 서버 이름을 모를 경우 Dynamic Media Classic에서 [ **[!UICONTROL 설정] > [응용 프로그램 설정]으로 이동합니다]**.
...
>다시 연결하는 경우:
>
>* 게시 시 Dynamic Media Classic에 다시 연결할 때 게시 시 암호를 재설정해야 할 수도 있고 다시 연결할 수 없습니다. 이 문제는 작성 인스턴스에 문제가 아닙니다.
>* 지역, 회사 이름과 같은 값을 수정하는 경우 Dynamic Media Classic에 다시 연결해야 합니다. 구성 옵션이 수정되었지만 저장되지 않은 경우 AEM은 구성이 유효함을 잘못 나타냅니다. 다시 연결해야 합니다.
...
기본적으로 비활성화된 Adobe CQ Dynamic Media Classic DAM 수신기를 활성화해야 합니다.
...
자산이 AEM Assets에 추가되면 자동으로 업로드되고 Dynamic Media Classic에 게시됩니다.

>[!NOTE]
>
>AEM Assets에서 Dynamic Media Classic으로 자동 업로드하기 위한 최대 파일 크기는 500MB입니다.
...
1. 고급 **[!UICONTROL 탭을]** 누르고 **[!UICONTROL 자동 업로드]** 활성화 **[!UICONTROL 확인란을 선택한 다음]**&#x200B;확인을누릅니다. 이제 Dynamic Media Classic에 업로드하는 것을 포함하도록 DAM 자산 워크플로우를 구성해야 합니다.
...
라이브하기 전에 스테이징 환경에서 자산을 테스트하려는 경우 Dynamic Media Classic에 자산을 즉시 게시하지 않을 수도 있습니다. Dynamic Media Classic의 보안 테스트 환경에서 AEM을 사용하여 게시 취소된 상태에서 자산에서 Dynamic Media Classic으로 바로 자산을 푸시할 수 있습니다.

Dynamic Media Classic 에셋은 보안 미리 보기를 통해 계속 사용할 수 있습니다. AEM에 자산이 게시되는 경우에만 Dynamic Media Classic 자산도 프로덕션에서 라이브됩니다.

자산을 Dynamic Media Classic으로 푸시할 때 즉시 게시하려는 경우 옵션을 구성할 필요가 없습니다. 기본 동작입니다.

그러나 Dynamic Media Classic으로 푸시된 에셋이 자동으로 게시되지 않도록 하려면 이 섹션에서는 AEM 및 Dynamic Media Classic을 구성하는 방법에 대해 설명합니다.

#### 자산을 Dynamic Media Classic에 푸시하기 위한 사전 요구 사항 게시 취소 {#prerequisites-to-push-assets-to-scene-unpublished}
...
* 버전 제어를 지원하지 않습니다.
* 자산이 이미 AEM에 게시되고 후속 버전이 만들어지면 해당 새 버전이 즉시 프로덕션에 실시간으로 게시됩니다. 활성화 시 게시는 자산의 초기 게시에서만 작동합니다.
...
>[!NOTE]
>
>사용자가 AEM에 자산을 게시하면 S7 자산이 프로덕션/라이브 자산에 자동으로 트리거됩니다(자산은 더 이상 안전한 미리 보기/게시 취소되지 않음).
...
AEM의 페이지에서 Dynamic Media Classic 구성 요소로 이동하고 편집을 탭하여 **[!UICONTROL 확인할 수 있습니다]**. 자산에 URL에 보안 미리 보기 서버가 나열됩니다. AEM에서 게시하면 파일 참조의 서버 도메인이 미리 보기 URL에서 프로덕션 URL로 업데이트됩니다.
...
AEM과 Dynamic Media Classic을 통합하는 데 문제가 있는 경우 다음 해결 방법 시나리오를 참조하십시오.
...
* 자동 업로드를 활성화하도록 Cloud Services 구성을 구성했는지, Dynamic Media Classic 업로드를 포함하도록 DAM 자산 워크플로우를 업데이트 및 저장했는지 확인하십시오.
* Dynamic Media Classic 대상 폴더의 하위 폴더에 이미지를 업로드할 때 다음 중 하나를 수행해야 합니다.
   ...
   * Dynamic Media Classic 계정의 설정 영역에서 Dynamic Media Classic이 자산을 덮어쓰는 방식을 변경합니다. 하위 폴더에서 동일한 이름의 자산을 사용하는 경우 위치에 상관없이 자산을 덮어쓰도록 Dynamic Media Classic을 설정하지 마십시오.

**삭제된 자산 또는 폴더가 Dynamic Media Classic과 AEM 간에 동기화되지 않은 경우:**

* AEM Assets에서 삭제된 자산 및 폴더는 Dynamic Media Classic의 동기화된 폴더에 여전히 표시됩니다. 수동으로 삭제해야 합니다.
