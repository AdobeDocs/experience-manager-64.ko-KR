...
seo-description: AEM의 사용자 동기화에 대해 알아봅니다.
...
배포가 [게시 팜인](/help/sites-deploying/recommended-deploys.md#tarmk-farm)경우 멤버는 로그인하여 게시 노드에서 해당 데이터를 볼 수 있어야 합니다.
...
AEM 6.1의 경우 사용자 동기화가 활성화되면 사용자 데이터가 팜의 게시 인스턴스 간에 자동으로 동기화되고 작성자에 만들어지지 않습니다.

## Sling Distribution {#sling-distribution}

사용자 데이터는 [ACL과](/help/sites-administering/security.md)함께 [Oak Core](/help/sites-deploying/platform.md), Oak JCR [아래 레이어에 저장되며](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/org/apache/jackrabbit/oak/api/package-tree.html)Oak API를 사용하여 액세스됩니다. 자주 업데이트되지 않는 경우에는 사용자 데이터가 Sling Content Distribution(Sling [Distribution](https://github.com/apache/sling/blob/trunk/contrib/extensions/distribution/README.md) )을 사용하여 다른 게시 인스턴스와 동기화되는 것이 좋습니다.

기존 복제 기능과 비교하여 Sling 배포를 사용한 사용자 동기화 시 얻을 수 있는 이점은 다음과 같습니다.

* *사용자*, *사용자 프로필* 및 ** 사용자 그룹이 작성자에 만들어지지 않음

* Sling 배포는 jcr 이벤트에 속성을 설정하므로 무한 복제 루프에 대한 걱정 없이 게시 측 이벤트 수신기 내에서 작동할 수 있습니다
* Sling 배포는 사용자 데이터를 원래 게시 인스턴스가 아닌 인스턴스로 전송하므로 불필요한 트래픽이 제거됨
* [사용자 노드에](/help/sites-administering/security.md) 설정된 ACL은 동기화에 포함됩니다.

>[!NOTE]
>
>세션이 필요한 경우 SSO 솔루션을 사용하거나 고정 세션을 사용하고 고객이 다른 게시자로 전환되면 로그인 하도록 하는 것이 좋습니다.

>[!CAUTION]
>
>사용자 동기화가 활성화되어 있더라도 ***관리자** *그룹의 동기화는 지원되지 않습니다. 대신 &#39;비교 가져오기&#39; 실패 오류가 오류 로그에 기록됩니다.
>
>따라서 배포가 게시 팜인 경우 사용자가 ***관리자** *그룹에서 추가 또는 제거되면 각 게시 인스턴스에서 수동으로 수정해야 합니다.

## 사용자 동기화 사용 {#enable-user-sync}

>[!NOTE]
>
>기본적으로 사용자 동기화는 입니다 `disabled`.
>
>사용자 동기화 활성화에는 *기존* OSGi 구성을 수정하는 작업이 포함됩니다.
>
>사용자 동기화를 활성화한 결과로 새 구성을 추가할 필요가 없습니다.

사용자 동기화는 작성 환경에 의존하여 사용자 데이터가 작성자에 만들어지지 않더라도 사용자 데이터 배포를 관리합니다. 작성 환경에서 전체 구성 중에 많은 것이 발생하고 각 단계에서는 작성 환경에서 수행할지 또는 게시에서 수행될지를 명확하게 식별합니다.

다음은 사용자 동기화를 활성화하는 데 필요한 단계이며 [문제 해결](#troubleshooting) 섹션입니다.

### 전제 조건 {#prerequisites}

1. 사용자 및 사용자 그룹이 한 게시자에서 이미 만들어진 경우 사용자 동기화를 구성하고 활성화하기 전에 사용자 데이터를 모든 게시자와 [수동으로](#manually-syncing-users-and-user-groups) 동기화하는 것이 좋습니다.
...
**권한 구성**&#x200B;이 권한이 있는 사용자는 작성자에 대한 Sling 배포를 구성하는 데 3단계에서 사용됩니다.
...
* select `/home` node
* 오른쪽 창에서 탭을 `Access Control` 선택합니다
* ACL 항목을 추가할 `+` 단추 선택

   * **주체**: *사용자 동기화를 위해 만든 사용자 검색*
   * **유형**: `Allow`
   * **권한**: `jcr:all`
   * **제한 사항** 담당자:글로벌: `*/activities/*`
   * 확인 **선택**
...
### 3. Adobe 화강암 분배 - 암호화된 암호 전송 비밀 공급자 {#adobegraniteencpasswrd}

**권한 구성**

권한이 있는 사용자 **`administrators`**사용자 그룹의 구성원이 모든 게시 인스턴스에 만들어지면 작성자로부터 게시로 사용자 데이터를 동기화할 수 있는 권한이 있는 권한이 있는 권한이 있는 권한이 있는 권한이 작성자로부터 해당 권한이 있는 사용자가 식별되어야 합니다.
...
* **작성자**
   ...
   * 2단계에서 게시 시 생성된 [권한이 있는 사용자](#createauthuser) 에게 사용자 이름과 암호 설정
...
### 5. Adobe [화강암 분포] - Diff Observer Factory {#diffobserver}
...
* **작성자**
   ...
   * locate `Apache Sling Distribution Trigger - Scheduled Triggers Factory`
      ...
      * 원하는 간격 `Interval in Seconds` 으로 설정
      * select `Save`
...
기본 구성은 단일 게시 인스턴스에 대한 것입니다. 사용자 동기화를 활성화하는 이유는 게시 팜과 같은 여러 게시 인스턴스를 동기화하는 것이므로 추가 게시 인스턴스를 동기화 에이전트 팩터리에 추가해야 합니다.
...
여러 게시 인스턴스에 걸쳐 동기화해야 하는 사용자 지정 데이터가 있는 경우 다음을 수행합니다.
...
* **무시 가능한 속성**
   ...
   변경이 감지되면 무시되는 속성 목록입니다. 이러한 속성에 대한 변경 사항은 다른 변경 사항의 부작용으로 동기화될 수 있지만(동기화는 항상 노드 수준이기 때문에) 이러한 속성에 대한 변경 사항은 자체 동기화를 트리거하지 않습니다.
...
* **Ignorable Nodes**
   ...
   동기화 중에 완전히 무시되는 하위 경로 이러한 하위 경로 아래의 어떤 것도 동기화되지 않습니다.
...
>[!CAUTION]
>
>둘 이상의 게시 인스턴스 간에 Sling ID가 일치하는 경우 사용자 그룹 동기화가 실패합니다.
...
* 찾기 `Apache Sling Distribution Packaging - Vault Package Builder Factor`
...
제작 환경에서 만들어진 사용자 및 프로필은 작성 환경에 표시되지 않습니다(자가 등록).

토폴로지가 [게시 팜](/help/sites-deploying/recommended-deploys.md#tarmk-farm) 및 사용자 동기화가 올바르게 구성된 경우 *사용자 *와 *사용자 프로필* 이 Sling 배포를 사용하여 게시 팜에서 동기화됩니다.
...
### 오프라인으로 사용자 동기화 방법 {#how-to-take-user-sync-offline}

사용자 동기화를 오프라인으로 전환하려면 게시자를 [제거하거나 데이터를](#how-to-remove-a-publisher) [수동으로 동기화하려면](#manually-syncing-users-and-user-groups)배포 큐가 비어 있고 조용해야 합니다.
...
* 작성자:
   ...
   * CRXDE Lite 사용 [](/help/sites-developing/developing-with-crxde-lite.md)
      ...
      * 항목을 찾습니다. `/var/sling/distribution/packages`
...
* 작성자
   ...
   * *Apache Sling `Enabled` [Distribution Agent - Sync Agent Factory의 확인란의 선택을 취소합니다.](#apache-sling-distribution-agent-sync-agents-factory)

작업이 완료되면 사용자 동기화를 다시 활성화합니다.

* 작성자

   * Apache Sling `Enabled` [Distribution Agent - Sync Agent Factory 확인란 선택](#apache-sling-distribution-agent-sync-agents-factory)

### 사용자 동기화 진단 {#user-sync-diagnostics}

사용자 동기화 진단은 구성을 확인하고 문제를 식별하려고 시도하는 도구입니다.

작성자의 경우 기본 콘솔에서 **도구, 작업, 진단, 사용자 동기화 진단을 검색하면 됩니다.**

User Sync Diagnostics 콘솔을 입력하면 결과가 표시됩니다.

사용자 동기화가 활성화되지 않았을 때 표시되는 부분입니다.

![chlimage_1-397](assets/chlimage_1-397.png)

#### 게시자를 위한 진단 실행 방법 {#how-to-run-diagnostics-for-publishers}

작성자 환경에서 진단이 실행되면 합격/불합격 결과에는 확인을 위해 구성된 게시 인스턴스의 목록을 표시하는 [INFO] 섹션이 포함됩니다.

목록에 포함된 URL은 해당 인스턴스에 대한 진단을 실행하는 각 게시 인스턴스의 URL입니다. url 매개 변수 `syncUser` 는 진단 URL에 추가되며 이 값은 2단계에서 만든 *승인된 동기화 사용자* 로 [설정됩니다](/help/sites-administering/sync.md#createauthuser).

**참고** : URL을 실행하기 전에 *인가된 동기화 사용자* 가 이미 해당 게시 인스턴스에 로그인해야 합니다.

![chlimage_1-398](assets/chlimage_1-398.png)

### 구성이 부적절하게 추가됨 {#improperconfig}

사용자 동기화가 작동하지 않을 때 가장 일반적인 문제는 추가 구성이 *추가되었다는 것입니다*. 대신, 기존 *기본 구성은 *편집되어야*&#x200B;합니다.

다음은 편집한 기본 구성이 웹 콘솔에 어떻게 표시되어야 하는지에 대한 보기입니다. 두 개 이상의 인스턴스가 나타나면 추가된 구성을 제거해야 합니다.

#### (작성자) 하나의 Apache Sling Distribution Agent - Sync Agents Factory {#author-one-apache-sling-distribution-agent-sync-agents-factory}

![chlimage_1-399](assets/chlimage_1-399.png)

#### (작성자) 하나의 Adobe [화강암 배포] - 암호화된 암호 전송 암호 공급자 {#author-one-adobe-granite-distribution-encrypted-password-transport-secret-provider}
...
#### (게시) 하나의 Adobe [화강암 배포] - Diff Observer Factory {#publish-one-adobe-granite-distribution-diff-observer-factory}

![chlimage_1-402](assets/chlimage_1-402.png)

#### (작성자) 하나의 Apache Sling 배포 트리거 - 예약된 트리거 공장 {#author-one-apache-sling-distribution-trigger-scheduled-triggers-factory}
...
그런 다음 섹션 [2를 확인합니다. 승인된 사용자](#createauthuser) 만들기가 올바르게 수행되었습니다.

이 섹션에서는 모든 게시 인스턴스에 존재하는 인증된 사용자를 만들고 작성자의 &#39;비밀 공급자&#39; OSGi 구성에서 식별하는 방법에 대해 설명합니다. 기본적으로 사용자는 이(가) 됩니다 `admin`.

권한이 있는 사용자는 사용자 그룹의 구성원이 되어야 하며 해당 그룹에 대한 권한은 **`administrators`** 변경하면 안 됩니다.

권한이 있는 사용자는 명시적으로 모든 게시 인스턴스에 대해 다음 권한 및 제한을 가져야 합니다.
...
### 게시자를 사용할 수 없는 경우 {#when-a-publisher-becomes-unavailable}

게시 인스턴스를 사용할 수 없게 되면 나중에 다시 온라인 상태가 될 경우 해당 인스턴스를 제거하지 마십시오. 변경 내용이 게시자에 대해 대기열에 추가되고, 온라인으로 돌아오면 변경 사항이 처리됩니다.

게시 인스턴스가 온라인상에 다시 표시되지 않는 경우, 오프라인 상태인 경우 큐 증대로 인해 작성 환경에서 눈에 띄는 디스크 공간 사용이 발생하므로 해당 인스턴스를 제거해야 합니다.
...
Apache Sling [Distribution Agent - Sync Agents Factory에서](#apache-sling-distribution-agent-sync-agents-factory)게시자를 제거하려면 배포 큐가 비어 있고 조용해야 합니다.
