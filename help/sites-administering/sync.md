...
>[!NOTE]
>
>세션이 필요한 경우 SSO 솔루션을 사용하거나 고정 세션을 사용하는 것이 좋습니다. 고객이 다른 게시자로 전환되면 로그인하도록 하십시오.

>[!CAUTION]
>
>사용자 동기화가 활성화되어 있더라도 ***관리자** *그룹의 동기화는 지원되지 않습니다. 대신 &#39;비교 가져오기&#39; 오류가 오류 로그에 기록됩니다.
>
...
* **무시 가능한 속성**
   ...
   변경 사항이 감지되면 무시되는 속성 목록입니다. 이러한 속성에 대한 변경 사항은 다른 변경 사항의 부작용으로 동기화될 수 있지만(동기화는 항상 노드 수준에 있으므로) 이러한 속성에 대한 변경 사항은 동기화를 트리거하지 않습니다.
...
* locate `Apache Sling Distribution Packaging - Vault Package Builder Factor`
...
사용자 동기화가 활성화되지 않았을 때 표시되는 내용입니다.

![chlimage_1-397](assets/chlimage_1-397.png)

#### 게시자에 대한 진단 실행 방법 {#how-to-run-diagnostics-for-publishers}

작성자 환경에서 진단을 실행하면 통과/실패 결과에 확인을 위해 구성된 게시 [인스턴스] 목록을 표시하는 INFO 섹션이 포함됩니다.

목록에 포함된 URL은 해당 인스턴스에 대한 진단을 실행하는 각 게시 인스턴스의 URL입니다. URL 매개 변수는 진단 URL에 `syncUser` 추가되며 이 값은 2단계에서 만든 *인증된 동기화 사용자에게* 설정됩니다 [](/help/sites-administering/sync.md#createauthuser).
