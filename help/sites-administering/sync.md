...
배포가 [게시 팜](/help/sites-deploying/recommended-deploys.md#tarmk-farm)인 경우 구성원은 로그인하여 모든 게시 노드에서 해당 데이터를 볼 수 있어야 합니다.
...
사용자 데이터는 [ACL](/help/sites-administering/security.md)과 함께 Oak JCR 아래의 [Oak Core](/help/sites-deploying/platform.md)에 저장되고 [Oak API](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/org/apache/jackrabbit/oak/api/package-tree.html)를 사용하여 액세스합니다. 자주 업데이트되는 경우 [Sling 컨텐츠 배포](https://github.com/apache/sling/blob/trunk/contrib/extensions/distribution/README.md)(Sling 배포)를 사용하여 사용자 데이터를 다른 게시 인스턴스와 동기화하는 것이 좋습니다.

기존 복제와 비교하여 Sling 배포를 사용한 사용자 동기화를 통해 얻을 수 있는 이점은 다음과 같습니다.

* *사용자*,  *사용자* 프로필 및  *게시 시* 만든 사용자 그룹은 작성자에 만들어지지 않습니다
...
* [](/help/sites-administering/security.md) 사용자 노드의 ACLsset은 동기화에 포함됩니다
...
>[!CAUTION]
>
>사용자 동기화가 활성화되더라도 ***administrators** *그룹의 동기화가 지원되지 않습니다. 대신 &#39;diff 가져오기&#39;에 대한 오류가 오류 로그에 기록됩니다.
>
>따라서 배포가 게시 팜인 경우 ***administrators** *그룹에서 사용자가 추가되거나 제거되면 각 게시 인스턴스에서 수동으로 수정해야 합니다.
...
>
>사용자 동기화를 활성화하려면 *기존* OSGi 구성을 수정해야 합니다.
>
...
1. 사용자 및 사용자 그룹이 이미 하나의 게시자에서 생성된 경우 사용자 동기화를 구성 및 활성화하기 전에 [사용자 데이터를 모든 게시자에게 수동으로 동기화하는 것이 좋습니다](#manually-syncing-users-and-user-groups).
...
**권한**
구성 이 승인된 사용자는 3단계에서 작성자에 대한 Sling 배포를 구성하는 데 사용됩니다.
...
* `+` 단추를 선택하여 ACL 항목을 추가합니다
   ...
   * **유형**: `Allow`
   * **권한**:  `jcr:all`
   * **** Restrictionrep:glob:  `*/activities/*`
   * **확인** 선택
...
모든 게시 인스턴스에 **`administrators`**사용자 그룹의 구성원인 권한이 있는 사용자가 만들어지면 해당 권한이 있는 사용자는 작성자에서 게시로 사용자 데이터를 동기화할 수 있는 권한이 있는 작성자에서 식별되어야 합니다.
...
* **작성자**
   ...
   * 사용자 이름과 암호를 2단계에서 게시할 때 만든 [인증된 사용자](#createauthuser)에 설정합니다.
...
* **작성자**
   ...
   * `Apache Sling Distribution Trigger - Scheduled Triggers Factory` 찾기
      ...
      * `Interval in Seconds` 을 원하는 간격으로 설정합니다.
      * `Save` 선택
...
## 여러 게시 인스턴스 {#configure-for-multiple-publish-instances}에 대한 구성
...
* **Importer**
끝점각 게시자에 대한 Importer 끝점이 있어야 합니다. 예를 들어 두 개의 게시자인 localhost:4503 및 4504가 있는 경우 두 개의 항목이 있어야 합니다.
...
### 9. 고유한 Sling ID {#unique-sling-id}
...
1. `http://<host>:<port>/system/console/status-slingsettings`
1. **Sling ID** 값을 확인합니다.
...
### 사용자가 게시 {#user-self-registers-or-edits-profile-on-publish}에 프로필을 직접 등록하거나 편집합니다.

디자인적으로 게시 환경(자체 등록)에서 만든 사용자 및 프로필은 작성 환경에 표시되지 않습니다.

토폴로지가 [게시 팜](/help/sites-deploying/recommended-deploys.md#tarmk-farm)이고 사용자 동기화가 올바르게 구성되면 *사용자 *와 *사용자 프로필*&#x200B;이 Sling 배포를 사용하여 게시 팜 간에 동기화됩니다.
...
사용자 동기화를 오프라인으로 전환하려면 게시자](#how-to-remove-a-publisher) 또는 [수동으로 데이터 동기화](#manually-syncing-users-and-user-groups)를 제거하려면 배포 큐가 비어 있고 조용해야 합니다.[
...
* 작성자:
   ...
   * [패키지 관리자 사용](/help/sites-administering/package-manager.md)
      ...
      * 보류 중인 패키지(아직 설치되지 않음)를 찾습니다.
         ...
         * 이름이 `socialpubsync-vlt*` 패턴으로 지정됨
         * `communities-user-admin`에 의해 생성됨
...
* 작성자
   ...
   * *[Apache Sling Distribution Agent - Sync Agent Factory](#apache-sling-distribution-agent-sync-agents-factory)의 `Enabled` 확인란을 선택 취소합니다.
...
작성자의 경우 **도구, 작업, 진단, 사용자 동기화 진단을 통해 기본 콘솔에서 탐색하면 됩니다.**
...
작성자 환경에서 진단을 실행하면 통과/실패 결과에 [INFO] 섹션이 포함된 것으로 확인용으로 구성된 게시 인스턴스 목록을 표시합니다.

목록에 해당 인스턴스에 대한 진단을 실행하는 각 게시 인스턴스의 URL이 포함되어 있습니다. URL 매개 변수 `syncUser`은(는) [2단계에서 만든 *인증된 동기화 사용자*&#x200B;로 설정된 값이 있는 진단 URL에 추가됩니다.](/help/sites-administering/sync.md#createauthuser)

**참고** :url을 시작하기 전에  *인증된 동기화* 사용자가 이미 해당 게시 인스턴스에 로그인해야 합니다.

![chlimage_1-398](assets/chlimage_1-398.png)

### 구성이 잘못 추가됨 {#improperconfig}

사용자 동기화가 작동하지 않을 때 가장 일반적인 문제는 추가 구성이 *added*&#x200B;였다는 것입니다. 대신 *기존 *기본 구성은 *편집됨*&#x200B;이어야 합니다.

다음은 편집된 기본 구성이 웹 콘솔에 표시되는 방법에 대한 보기입니다. 두 개 이상의 인스턴스가 나타나면 추가된 구성을 제거해야 합니다.

#### (작성자) 하나의 Apache Sling Distribution Agent - Sync Agent Factory {#author-one-apache-sling-distribution-agent-sync-agents-factory}
...
#### (작성자) 하나의 Apache Sling 배포 트리거 - 예약된 트리거 팩토리 {#author-one-apache-sling-distribution-trigger-scheduled-triggers-factory}
...
그런 다음 섹션 [2를 확인합니다. 승인된 사용자 만들기](#createauthuser)가 올바르게 수행되었습니다.

이 섹션에서는 모든 게시 인스턴스에 있는 인증된 사용자를 만들고 이를 작성자의 &#39;비밀 공급자&#39; OSGi 구성에서 식별하는 방법에 대해 설명합니다. 기본적으로 사용자는 `admin`입니다.

허가된 사용자는 **`administrators`** 사용자 그룹의 구성원으로 만들어져야 하며 해당 그룹에 대한 권한은 변경하지 않아야 합니다.

승인된 사용자는 모든 게시 인스턴스에 대해 다음 권한 및 제한을 명시적으로 가져야 합니다.

| **경로** | **jcr:all** | **rep:glob** |
|---|---|---|
| /home | X | amp;ast;/activities/&amp;ast; |
| /home/users | X | amp;ast;/activities/&amp;ast; |
| /home/groups | X | amp;ast;/activities/&amp;ast; |
...
[9 섹션을 참조하십시오. 고유한 Sling ID](#unique-sling-id)
...
[Apache Sling Distribution Agent - Sync Agent Factory](#apache-sling-distribution-agent-sync-agents-factory)에서 게시자를 제거하려면 배포 큐가 비어 있고 조용해야 합니다.

* 작성자:

   * [사용자 동기화를 오프라인으로 전환](#how-to-take-user-sync-offline)
   * 두 서버 목록에서 게시자를 제거하려면 [7단계](#apache-sling-distribution-agent-sync-agents-factory)를 따르십시오.
