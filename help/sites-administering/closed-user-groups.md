...
* 보호된 JCR 컨텐츠를 수정하는 데 필요한 쓰기 권한이나 권한(액세스 제어, 노드 유형 정보, 버전 관리, 잠금 또는 사용자 관리)은 처리되지 않습니다.이러한 권한은 CUG 정책에 영향을 받지 않으며 관련 인증 모델로 평가되지 않습니다. 이러한 권한이 부여되는지 여부는 보안 설정에 구성된 다른 모델에 따라 달라집니다.
...
* 그러나 액세스 제어 노드의 형제 또는 상위 노드에는 영향을 주지 않습니다.
...
구성된 지원되는 경로 내에 `granite:AuthenticationRequired` mixin 형식을 추가하면 해당 `sling.auth.requirements` 속성이 포함된 새로운 추가 항목이 포함된 책임 처리기의 OSGi 등록이 업데이트됩니다. 지정된 인증 요구 사항이 선택적 `granite:loginPath` 속성을 지정하는 경우 이 값은 인증 요구 사항에서 제외되기 위해 인증자에게 &#39;-&#39; 접두사로 추가로 등록됩니다.
...
* 중첩된 하위 트리를 다시 제외하지 않고도 인증 요구 사항이 전체 트리에 적용되도록 하는 모델 리포지토리 컨텐츠
...
기존 CUG 정책을 편집하려면 다음 단계가 필요합니다. 수정된 정책은 다시 작성해야 하며 변경 사항은 계속 사용해야 `javax.jcr.Session.save()`합니다.
...
>[!NOTE]
>
>지정된 경로가 이미 기존 CUG의 일부인지 확인하려면 계층 구조를 `getEffectivePolicies` 따라 표시되는 및 이후의 코드 예제 간의 차이점을 참조하십시오.
...
>[!NOTE]
>
>위에 언급된 특정 대상 노드에 대한 수정 사항은 Apache Sling Authenticator에 반영됩니다. 단, 구성 `RequirementHandler` 및 대상이 지원되는 경로에 의해 정의된 트리에만 포함됩니다(구성 옵션 섹션 참조).
>
...
기존 인증 요구 사항을 제거하는 절차. 대상 노드를 포함하는 트리에 대해 Apache Sling Authenticator `RequirementHandler` 가 구성된 경우에만 요구 사항이 등록되지 않습니다.
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    ...
    </ul> <p>또한 아래 구성 <a href="/help/sites-administering/closed-user-groups.md#configuration-options" target="_blank">옵션을</a> 참조하십시오.</p> </td> 
  </tr> 
  <tr> 
   <td>구성 정책</td> 
...
>[!NOTE]
>
>그러나 구성을 제거해도 노드 유형에 적용할 수 있었던 혼합 유형은 등록 취소되지 않습니다.
...
권한 부여 관점의 기본 차이는 아래 목록에 요약되어 있습니다.
...
새로운 구현을 통해 기본 인증 모델의 액세스 제어 설정은 작성, 수정 또는 제거되는 CUG로 인해 영향을 받지 않습니다. 대신 호출된 새로운 유형의 정책은 대상 노드에 대한 추가 액세스 제어 컨텐츠로 `PrincipalSetPolicy` 적용됩니다. 이 추가 정책은 대상 노드의 1차 하위 구성요소(있는 경우)로 위치하며 기본 정책 노드의 동위가 됩니다.
...
CUG 정책을 지정된 페이지 아래에 있는 jcr:content 노드에만 배치하면 지정된 페이지의 컨텐츠에 대한 액세스 권한만 제한되지만 형제 페이지 또는 하위 페이지에는 적용되지 않습니다. 이것은 유효한 사용 사례일 수 있으며, 세분화된 액세스 컨텐츠 컨텐츠를 적용할 수 있는 저장소 편집기를 사용하여 수행할 수 있습니다. 그러나, 이 비교는 jcr:content 노드에 cq:cugEnabled 속성을 삽입하는 이전 구현이 페이지 노드에 내부적으로 다시 매핑되는 것을 대비합니다. 이 매핑은 더 이상 수행되지 않습니다.

**CUG 정책을 사용한 권한 평가**

이전 CUG 지원에서 추가 인증 모델로 이동하면 효과적인 읽기 권한 평가 방식이 변경됩니다. Jackrabbit 설명서에 [](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html)설명된 바와 같이, Oak 저장소에서 구성한 모든 모델의 권한 평가가 읽기 액세스 권한을 부여하는 경우에만 Jackrabbit을 볼 수 `CUGcontent` 있는 지정된 주체가 읽기 액세스 권한을 부여하게 됩니다.
