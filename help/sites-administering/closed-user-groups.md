...
* PrincipalSetPolicy는 주도자의 수정 집합에 권한을 부여합니다.
...
* 또한 쓰기 권한이나 보호된 JCR 컨텐츠를 수정하는 데 필요한 어떠한 종류의 권한도 처리하지 않습니다(액세스 제어, 노드 유형 정보, 버전 관리, 잠금 또는 사용자 관리).이러한 권한은 CUG 정책에 영향을 받지 않으며 관련 인증 모델로 평가되지 않습니다. 이러한 권한이 부여되는지 여부는 보안 설정에 구성된 다른 모델에 따라 다릅니다.
...
기존 CUG 정책을 편집하려면 다음 단계가 필요합니다. 수정된 정책은 다시 작성해야 하며 변경 사항은 `javax.jcr.Session.save()`을(를) 사용하여 지속되어야 합니다.
...
>[!NOTE]
>
>지정된 경로가 이미 기존 CUG의 일부인지 확인하기 위해 계층 구조를 단계별로 안내하는 `getEffectivePolicies` 및 다음 코드 예제 간의 차이를 참고하십시오.
...
>[!NOTE]
>
>위에 언급된 특정 대상 노드의 수정 사항은 `RequirementHandler`이(가) 구성되어 있고 대상이 지원되는 경로로 정의된 트리에 포함된 경우에만 Apache Sling Authenticator에 반영됩니다(구성 옵션 섹션 참조).
>
...
로그인 경로를 포함하여 새 인증 요구 사항을 만드는 절차. 대상 노드를 포함하는 트리에 대해 `RequirementHandler`이(가) 구성된 경우 로그인 경로에 대한 요구 사항 및 제외는 Apache Sling Authenticator에만 등록됩니다.
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    ...
    </ul> <p>또한 아래 <a href="/help/sites-administering/closed-user-groups.md#configuration-options" target="_blank">구성 옵션</a>을 참조하십시오.</p> </td> 
  </tr> 
  <tr> 
   <td>구성 정책</td> 
...
>[!NOTE]
>
>그러나 구성을 제거해도 노드 유형에 적용할 수 있는 혼합 유형이 등록 취소되지 않습니다. 이 유형은 영향을 미치지 않습니다.
...
### CUG 설정 및 구성 {#diferences-in-cug-setup-and-configuration}의 차이점
...
### 인증 {#diferences-with-regards-to-authorization}과(와) 관련된 차이점

인증 관점의 기본 차이는 아래 목록에 요약됩니다.

**CUG용 전용 액세스 제어 컨텐츠**

이전 구현에서는 기본 인증 모델이 게시 시 액세스 제어 목록 정책을 조작하는 데 사용되었으며 CUG에서 지정한 설정으로 기존 ACE를 대체합니다. 이는 게시에 해석된 영구 잔존 JCR 속성을 작성하여 트리거되었습니다.

새로운 구현을 통해 기본 인증 모델의 액세스 제어 설정은 작성, 수정 또는 제거되는 CUG로 인해 영향을 받지 않습니다. 대신 `PrincipalSetPolicy`이라는 새 유형의 정책이 대상 노드에 대한 추가 액세스 제어 컨텐츠로 적용됩니다. 이 추가 정책은 대상 노드의 자식으로 위치하며, 있을 경우 기본 정책 노드의 동위가 됩니다.
...
CUG 정책을 지정된 페이지 아래에 있는 jcr:content 노드에만 배치하면 지정된 페이지의 컨텐츠에 대한 액세스 권한만 제한되지만, 형제 페이지 또는 하위 페이지에는 적용되지 않습니다. 이것은 유효한 사용 사례이며, 세부적으로 분류된 액세스 컨텐츠 컨텐츠를 적용할 수 있는 저장소 편집기를 사용할 수 있습니다. 하지만, 이것은 jcr:content 노드에 cq:cugEnabled 속성을 배치하는 이전 구현이 내부적으로 페이지 노드에 다시 매핑되는 것과 대비가 됩니다. 이 매핑은 더 이상 수행되지 않습니다.

**CUG 정책을 사용한 권한 평가**

이전 CUG 지원에서 추가 인증 모델로 이동하면 효과적인 읽기 권한이 평가되는 방식이 변경됩니다. [Jackrabbit 설명서](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html)에 설명된 대로, Oak 저장소에서 모든 모델의 권한 평가가 읽기 액세스 권한을 부여하는 경우에만 `CUGcontent`을(를) 볼 수 있는 지정된 주체에 읽기 액세스 권한이 부여됩니다.
...
1. CUG를 확인할 루트 경로를 입력하고 **연습 실행 수행** 단추를 누릅니다. 이렇게 하면 선택한 위치에서 변환하기 위해 CUG가 검색됩니다.
