...
>[!NOTE]
>
>클라이언트 컨텍스트를 ContextHub로 대체했습니다. 자세한 내용은 관련 [구성](/help/sites-administering/contexthub-config.md) 및 [developer](/help/sites-developing/contexthub.md) 설명서를 참조하십시오.
...
* **편집**
프로필 속성을  [편집, 추가 또는 제거할 수 있는 새 페이지가 열립니다](#editing-property-details).
...
JSONP 표준은 동일한 원본 정책을 피함으로써(웹 앱이 다른 도메인에 있는 서버와 통신하는 것을 불가능하게 만들기) JSON을 보완하는 것입니다. 다른 도메인에서 JSON 개체를 로드하기 위해 함수 호출에서 JSON 개체를 래핑하는 방식으로 이루어집니다(동일한 원본 정책에 대한 예외 사항).
...
클라이언트 컨텍스트를 편집하여 특정 속성의 값을 설정(또는 재설정)할 수 있습니다. 이렇게 하면 특정 시나리오를 테스트할 수 있습니다(특히 [세그먼테이션](/help/sites-administering/campaign-segmentation.md) 및 [캠페인](/help/sites-authoring/personalization.md)에 유용).
