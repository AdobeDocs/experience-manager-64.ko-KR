...
>[!NOTE]
>
>다음 예에서는 AEM이 포트에서 실행되고 있으며 암호 `localhost` 가 있는 사용자 `4502` 를 사용한다고 가정합니다 `admin` `admin`. 추가 명령 자리 표시자는 꺾쇠 괄호로 설정됩니다.
...
자세한 [내용은 AEM에서](/help/sites-administering/enabling-crxde-lite.md) CRXDE Lite 활성화를 참조하십시오.
...
자세한 [내용은 Adobe Analytics 및 Adobe Target](/help/sites-administering/opt-in.md#configuring-the-setup-and-provisioning-via-script) 에 선택 을 참조하십시오.
