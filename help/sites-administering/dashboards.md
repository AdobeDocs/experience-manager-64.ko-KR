...
>
>
>그러나 [대시보드 템플릿](#creating-a-dashboard-template)을 사용하여 공통 구성 및 대시보드 레이아웃을 공유할 수 있습니다.
...
대시보드 구성 요소는 일반적인 [AEM 구성 요소](/help/sites-developing/developing-components-samples.md)입니다. 이 섹션에서는 AEM과 함께 제공되는 보고 구성 요소에 대해 설명합니다.

### 웹 분석 보고 구성 요소  {#web-analytics-reporting-components}

AEM은 [SiteCatalyst](/help/sites-administering/adobeanalytics.md) 데이터의 여러 지표를 렌더링하는 구성 요소 세트와 함께 제공합니다. 이러한 구성 요소들은 **대시보드** 섹션 아래의 사이드킥에 나열되어 있습니다.
...
**데이터** 를 가져오는 날짜 ToAbsolute 또는 상대적 날짜입니다.
...
**아니오. 최상위 항목 중** 보고서에 의해 표시되는 항목 수입니다.
...
**아니오. 최상위 항목 중** 보고서에 표시되는 섹션의 수입니다.
...
템플릿에서 기본적으로 활성화된 Analytics 보고 구성 요소가 들어 있는 기본 구성 요소 그룹 `Dashboard`이 있습니다.

### 대시보드 템플릿 만들기 {#creating-a-dashboard-template}

템플릿은 새로운 대시보드의 기본 컨텐트를 정의합니다. 다양한 유형의 대시보드를 만드는 데 여러 개의 템플릿을 사용할 수 있습니다.

대시보드 템플릿은 `/libs/cq/dashboards/templates/` 아래에 저장된다는 점을 제외하면 다른 페이지 템플릿과 유사하게 만들어집니다. [컨텐트 페이지 템플릿 만들기](/help/sites-developing/website.md#creating-the-contentpage-template) 섹션을 참조하십시오.
...
상위 작성자 구성 요소는 `/apps/geometrixx-outdoors/components/reporting` 리포지토리에 저장되고 로 구성됩니다.
...
[QueryBuilder](/help/sites-developing/querybuilder-api.md)는 리포지토리를 쿼리하여 `cq:AuditEvent` 노드를 읽는 데 사용됩니다. 쿼리 결과는 작성한 기고물이 추출되는 JSON 개체입니다.
