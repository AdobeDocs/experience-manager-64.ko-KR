...
Adobe Analytics으로 전송할 데이터를 수집하는 프레임워크에 구성 요소를 추가합니다. 분석 데이터를 수집하도록 설계된 구성 요소는 데이터를 적절한 **CQ 변수에 저장합니다**. 이러한 구성 요소를 프레임워크에 추가하면 프레임워크에 CQ 변수 목록이 표시되므로 각 구성 요소를 적절한 **Analytics 변수에 추가할 수 있습니다**.

![aa-11](assets/aa-11.png)

AEM **보기가** 열리면 Analytics 변수가 컨텐츠 파인더에 나타납니다.

![aa-12](assets/aa-12.png)

동일한 **CQ 변수에 여러 Analytics 변수를 매핑할 수 있습니다**.
...
1. Adobe Analytics 변수를 구성 요소에 정의된 변수와 매핑하려면 컨텐츠 파인더의 **Analytics 변수를** 추적 구성 요소의 필드로 드래그합니다. 예를 들어 드래그 `Page Name (pageName)` 를 클릭합니다 `pagedata.title`.
   ...
   >[!NOTE]
   >
   >프레임워크에 대해 선택된 보고서 세트 ID(RSID)가 컨텐츠 파인더에 표시되는 Adobe Analytics 변수를 결정합니다.

1. 다른 구성 요소 및 변수에 대해 이전 두 단계를 반복합니다.

   >[!NOTE]
   >
   >여러 Analytics 변수(예: `props`, `eVars``events`)를 동일한 CQ 변수에 연결합니다(예: `pagedata.title`)

   >[!CAUTION]
   >
   >다음과 같은 것이 좋습니다.
   >
   >* `eVars` 및 `props` `pagedata.X` CQ 변수에 매핑됩니다. `eventdata.X`
   >* 반면에 이벤트는 `eventdata.events.X`


1. 사이트의 게시 인스턴스에서 프레임워크를 사용할 수 있게 하려면 사이드 킥의 **Page **Tab을 열고 프레임워크 **활성화를 클릭합니다.**
...
| `product.quantity` | `product.quantity` (전환 변수) | 구매한 제품 수. |
...
1. 보고서 **** (페이지 왼쪽)에서 **사용자 지정 전환**&#x200B;을 선택한 다음 **사용자 지정 전환 1-10을** 선택하고 `eVar7`

1. 사용 중인 Adobe Analytics 버전에 따라 보고서가 사용되는 검색어로 업데이트될 때까지 평균 45분이 기다려야 합니다. 예: 예제의 가지
...
1. Adobe Analytics 보기에서 컨텐츠 파인더는 CQ 보기에서 드래그한 추적 구성 요소에 속하는 모든 CQ 변수를 표시합니다.
   ...
   * 이 변수를 3개 변수 섹션 중 하나(**트래픽**, 전환 및 **이벤트** )에 속하는 모든 Adobe Analytics 변수로 드래그하여 놓을 수 **있습니다**.
...
1. **AEM 보기**:
   ...
   1. 프레임워크에 상위 구성 요소에서 상속되는 구성 요소가 있는 경우 구성 요소에 매핑된 변수와 함께 여기에 표시됩니다.

      1. 상속된 구성 요소가 잠겨 있습니다.
      1. 상속된 구성 요소의 잠금을 해제하려면 구성 요소의 이름 옆에 있는 자물쇠를 두 번 클릭하면 됩니다
      1. 상속을 되돌리려면 잠금 해제된 구성 요소를 삭제해야 합니다. 이 후 잠겨진 상태를 되찾을 것입니다.
   ...
   1. 분석 프레임워크에 현재 포함된 모든 구성 요소를 찾을 수 있습니다.
      ...
      1. 구성 요소 및 모든 해당 매핑을 삭제하려면 구성 요소의 컨텍스트 메뉴에서 삭제를 선택한 다음 확인 대화 상자에서 삭제를 수락합니다.
...
1. 세 테이블(트래픽, 전환, 이벤트)에는 사용 가능한 모든 Adobe Analytics 변수가 나열됩니다. 선택한 RSID에 속합니다. 여기에 표시된 매핑은 AEM 보기에서와 동일해야 합니다.
   ...
   * **이벤트**:
      ...
      * CQ 이벤트()에 매핑된 이벤트 변수( `event1`) `eventdata.events.pageView`
