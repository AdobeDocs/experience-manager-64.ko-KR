...
[이 문서에는 운영 체제별](/help/sites-administering/webdav-access.md#connecting-via-webdav) 세부 지침이 포함되어 있지만 WebDAV 프로토콜을 사용하여 저장소에 연결하는 것은 기본적으로 WebDAV 클라이언트를 다음 위치로 보냅니다.
...
이 URL은 운영 체제 수준에서 연결되면 기본 작업 영역()에 대한 WebDAV 액세스 권한을 제공합니다 `crx.default`. 사용자는 작업 영역 이름을 지정하는 작업이 훨씬 수월하지만 추가 [WebDAV URL을 사용하여 수행할 수 있는 작업 영역 이름을 지정하는 데 더 많은 유연성을 제공하지 않습니다](/help/sites-administering/webdav-access.md#webdav-urls).

AEM은 다음과 같이 저장소 컨텐츠를 표시합니다.

* 유형의 노드 `nt:folder` 가 폴더로 표시됩니다. 노드 아래의 `nt:folder` 노드는 폴더 컨텐츠로 표시됩니다.

* 형식의 노드가 파일로 `nt:file` 표시됩니다. 노드 아래의 `nt:file` 노드는 표시되지 않지만 파일의 내용을 형성합니다.

WebDAV를 사용하여 폴더와 파일을 만들고 편집하면 AEM에서 필요한 노드 `nt:folder` 와 `nt:file` 노드를 만들고 편집합니다. WebDAV를 사용하여 컨텐츠를 가져오고 내보낼 계획인 경우, 가능한 한 `nt:file` 및 `nt:folder` 노드 유형으로 작업해 보십시오.
...
경로의 작업 영역 요소를 변경하여 기본() 이외의 작업 영역을 매핑할 수 `crx.default`있습니다. 예를 들어 이름이 지정된 작업 영역을 매핑하려면 다음 URL `staging`을 사용하십시오.
...
이 [레지스트리 변경에 대한 자세한 내용은 Microsoft 지원 KB 841215를](https://support.microsoft.com/default.aspx/kb/841215) 참조하십시오.

Windows [에서 WebDav 클라이언트의 응답 향상에 대한 자세한 내용은 Microsoft 지원 KB 2445570을](https://support.microsoft.com/kb/2445570) 참조하십시오.
...
Windows 8의 경우 Windows 7 이상 [에 대해 설명된 대로 레지스트리 항목을 변경해야 합니다](/help/sites-administering/webdav-access.md#windows-and-greater-configuration). 그러나 레지스트리 항목을 보려면 먼저 데스크탑 환경을 활성화해야 합니다.

데스크탑 경험을 활성화하려면 **서버 관리자**&#x200B;를 열고 **기능**&#x200B;을 **추가한 다음**&#x200B;기능 **을 추가한 다음**&#x200B;데스크탑 경험을 활성화합니다.
...
1. Windows **탐색기** 또는 **** 파일 탐색기를 **열고** 컴퓨터 **를**&#x200B;클릭하거나 이 PC을클릭합니다.

   ![chlimage_1-112](assets/chlimage_1-112.png)

1. 네트워크 **드라이브** 맵을 클릭하여 마법사를 시작합니다.
1. 매핑 세부 정보를 입력합니다.
   ...
   * 다른 자격 **증명을 사용하여 Connect 확인**
   ...
   >[!NOTE]
   >
   >AEM이 다른 포트에 있는 경우 4502 대신 해당 포트 번호를 사용하십시오. 또한 로컬 시스템에서 컨텐츠 저장소를 실행하고 있지 않은 경우 해당 서버 이름 또는 IP 주소 `localhost` 로 대체합니다.

1. 사용자 이름 `admin` 및 암호를 입력합니다 `admin`. Adobe은 테스트를 위해 미리 구성된 관리자 계정을 사용하는 것이 좋습니다.
...
1. Finder **창으로** 이동하여 **서버** 에 ****&#x200B;연결 **을**&#x200B;클릭하거나Command+k를 누릅니다.
...
1. 인증 메시지가 표시되면 사용자 이름 `admin` 및 암호를 입력합니다 `admin`. Adobe은 테스트를 위해 미리 구성된 관리자 계정을 사용하는 것이 좋습니다.
...
1. Nautilus(파일 탐색기)에서 **위치** 를 선택하고 **서버에 연결을 선택합니다**.
1. 서버에 **연결** 창의 서비스 유형에서 HTTP(WebDAV)를 선택합니다.

1. 서버 **에서**`http://localhost:4502/crx/repository/crx.default`
...
1. 폴더 **에서**`/dav`
1. 사용자 이름을 입력합니다 `admin`. Adobe은 테스트를 위해 미리 구성된 관리자 계정을 사용하는 것이 좋습니다.
1. 포트를 비워 두고 연결에 사용할 이름을 입력합니다.
1. [ **연결]을 클릭합니다**. AEM에서 암호를 입력하라는 메시지를 표시합니다.
1. 암호를 입력하고 `admin` Connect **를 클릭합니다**.
...
1. 이름 **에**&#x200B;연결 이름을 입력합니다.
1. 사용자 **에서**&#x200B;사전 구성된 관리자 계정을 사용하는 `admin.` 것이 좋습니다.
1. 서버 **에서**`http://localhost:4502/crx/repository/crx.default`

   >[!NOTE]
   >
   >AEM이 다른 포트에 있는 경우 4502 대신 해당 포트 번호를 사용하십시오. 또한 로컬 시스템에서 컨텐츠 저장소를 실행하고 있지 않은 경우 해당 서버 이름 또는 IP 주소 `localhost` 로 바꾸십시오

1. 폴더 **에서**`dav`

1. Click **Save and Connect**.
1. 암호를 묻는 메시지가 나타나면 암호를 입력하고 `admin` Connect **를 클릭합니다**.
