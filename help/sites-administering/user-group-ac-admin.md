...
**사용자 계정** CRX는 사용자 계정에 저장된 세부 사항에 따라 사용자(해당 사용자 또는 다른 애플리케이션)를 식별 및 확인하여 액세스 권한을 인증합니다.
...
**그룹 계정** 그룹 계정은 사용자 및/또는 다른 그룹의 컬렉션입니다. 그룹에 할당된 액세스 권한이 해당 그룹의 모든 사용자에게 자동으로 적용되므로 관리를 간소화하는 데 사용됩니다. 사용자는 그룹에 속하지 않아도 되지만 종종 여러 그룹에 속합니다.
...
>[!NOTE]
>
>CRX 구현 [JSR-283에 의해 정의된 액세스 제어](https://www.adobe.io/experience-manager/reference-materials/spec/jcr/2.0/16_Access_Control_Management.html).
>
...
* A **주체** 는 액세스 권한을 가지고 있는 엔티티입니다. 주도자는 다음과 같습니다.
...
그래서 린다가 만약 `/features` 노드 아래에 있는 노드:
...
에 표준 대화 상자가 사용됩니다 **사용자 관리**.

적절한 작업 공간에 로그인해야 다음 두 작업 모두에서 대화 상자에 액세스할 수 있습니다.

* a **사용자 관리** CRX의 기본 콘솔에서 연결
* a **보안** CRX Explorer 메뉴
...
GroupID(사용 가능한 경우)를 클릭하면 [그룹 관리](#group-administration) 해당 그룹에 대해 속해 있어야 합니다.

**가장자** 가장 기능을 사용하면 사용자는 다른 사용자를 대신하여 작업할 수 있습니다.
...
1. 그런 다음 등록 정보를 입력할 수 있습니다.
   ...
   * **UserID** 계정 이름으로 사용됩니다.
   * **암호** 로그인할 때 필요합니다.
   * **주체 이름** 전체 텍스트 이름을 입력합니다.
...
1. 사용 **사용자 관리** 대화 상자에서 모든 계정의 목록 보기를 엽니다.

1. 트리 구조를 탐색합니다.
1. 편집하려면 필요한 계정을 클릭합니다.
1. 변경한 다음 해당 항목에 대해 저장(녹색 눈금 기호)을 클릭합니다.
1. 클릭 **닫기** 완료하거나 **목록...** 를 눌러 모든 사용자 계정 목록으로 돌아갑니다.
...
1. 트리 구조를 탐색합니다.
1. 필요한 계정을 선택하고 을(를) 클릭합니다 **사용자 제거**; 계정이 즉시 삭제됩니다.
...
다음을 정의할 수 있습니다 **속성** 신규 또는 기존 계정에 대해:

1. 를 엽니다. **사용자 관리** 대화 상자를 엽니다 .
1. 정의 **속성** 이름.
1. 을(를) 선택합니다 **유형** 드롭다운 목록에서 을 선택합니다.
1. 을(를) 정의합니다 **값**.
...
다음 **암호** 는 을(를) 클릭하여 변경할 수 있는 특수 속성입니다 **암호 변경** 링크를 클릭합니다.

또한 암호를 **보안** 메뉴 아래의 제품에서 사용할 수 있습니다.
...
에 표준 대화 상자가 사용됩니다 **그룹 관리**.

적절한 작업 공간에 로그인해야 다음 두 작업 모두에서 대화 상자에 액세스할 수 있습니다.

* a **그룹 관리** CRX의 기본 콘솔에서 연결
* a **보안** CRX Explorer 메뉴
...
**그룹 멤버십** 현재 그룹 계정이 속한 모든 그룹이 표시됩니다. 상속됨 열은 다른 그룹의 멤버십으로 인해 상속된 멤버십을 나타냅니다.

GroupID를 클릭하면 해당 그룹에 대한 대화 상자가 열립니다.

**멤버** 현재 그룹의 구성원인 모든 계정(사용자 및/또는 그룹)을 나열합니다.

다음 **상속됨** 열은 다른 그룹의 멤버십으로 인해 상속된 멤버십을 나타냅니다.

>[!NOTE]
>
>소유자, 편집기 또는 뷰어 역할이 자산 폴더의 사용자에게 할당되면 새 그룹이 만들어집니다. 그룹 이름은 형식입니다 `mac-default-<foldername>` 역할이 정의된 각 폴더에 대해
...
1. 사용 **그룹 관리** 대화 상자에서 모든 계정의 목록 보기를 엽니다.

1. 트리 구조를 탐색합니다.
1. 편집하려면 필요한 계정을 클릭합니다.
1. 변경한 다음 해당 항목에 대해 저장(녹색 눈금 기호)을 클릭합니다.
1. 클릭 **닫기** 완료하거나 **목록...** 모든 그룹 계정 목록으로 돌아갑니다.
...
1. 트리 구조를 탐색합니다.
1. 필요한 계정을 선택하고 을(를) 클릭합니다 **그룹 제거**; 계정이 즉시 삭제됩니다.
...
1. 다음 중 하나를 선택합니다.
   ...
   * 또는 **찾아보기...** 추가할 주도자(사용자 또는 그룹 계정)를 검색하고 선택합니다.
...
사용 **액세스 제어** CRXDE Lite 탭에서 액세스 제어 정책을 정의하고 관련 권한을 지정할 수 있습니다.

예, 예 **현재 경로** 왼쪽 창에서 필요한 리소스를 선택하고 오른쪽 아래 창의 액세스 제어 탭을 선택합니다.
...
* **저장소**
저장소 수준 액세스 제어를 선택합니다. 예를 들어 
`jcr:namespaceManagement` 권한 - 노드가 아니라 저장소에만 해당됩니다.

* **주체**
리포지토리에 등록된 주체.

   를 입력하거나 **주체** 이름을 지정하거나 필드 오른쪽의 아이콘을 클릭하여 **주도자 선택** 대화 상자.

   이를 통해 다음을 수행할 수 있습니다 **검색** 대상 **사용자** 또는 **그룹**. 결과 목록에서 필요한 주도자를 선택한 다음 **확인** 를 이전 대화 상자로 다시 전달하는 데 사용됩니다.
...
액세스 제어 항목을 추가할 때 다음 권한을 선택할 수 있습니다( [보안 API](https://docs.adobe.com/docs/en/spec/javax.jcr/javadocs/jcr-2.0/javax/jcr/security/Privilege.html) 자세한 내용:
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>mixin 노드 유형을 추가 및 제거하고 노드의 기본 노드 유형을 변경합니다. 또한 새 노드의 mixin 또는 기본 유형이 명시적으로 지정된 Node.addNode 및 XML 가져오기 메서드에 대한 모든 호출도 포함됩니다.</td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>다음을 포함하는 집계 권한:<br /> - jcr:modifyProperties<br /> - jcr:addChildNodes<br /> - jcr:removeNode<br /> - jcr:removeChildNodes</td> 
  </tr> 
  <tr> 
...
1. 를 사용하십시오 **권한 등록** 아이콘 (**+**) 대화 상자를 열고 새 권한을 정의합니다.

   ![ac_privilegeregister](assets/ac_privilegeregister.png)

1. 클릭 **확인** 저장 이제 이 권한을 선택할 수 있습니다.

### 액세스 제어 항목 추가 {#adding-an-access-control-entry}

1. 리소스를 선택하고 을(를) 엽니다. **액세스 제어** 탭.

1. 새 **로컬 액세스 제어 정책**&#x200B;를 클릭하고 **+** 아이콘 을 클릭하여 제품에서 사용할 수 있습니다. **적용 가능한 액세스 제어 정책** 목록:

   ![crx_accesscontrol_acceptable](assets/crx_accesscontrol_applicable.png)

1. 새 항목이 아래에 나타납니다 **로컬 액세스 제어 정책:**

   ![crx_accesscontrol_newlocal](assets/crx_accesscontrol_newlocal.png)

1. 을(를) 클릭합니다. **+** 아이콘을 사용하여 새 항목을 추가합니다.
...
1. 액세스 제어 정책을 정의하고 **확인** 저장 새 정책은 다음과 같습니다.

   * 아래에 나열된 항목은 다음과 같습니다. **로컬 액세스 제어 정책**
   * 변경 내용은 **효과적인 액세스 제어 정책**.
...
1. 표 **로컬 액세스 제어 정책** 필요한 항목을 선택하고 테이블의 새 위치로 드래그합니다.

   ![crx_accesscontrol_reorder](assets/crx_accesscontrol_reorder.png)

1. 변경 내용은 **로컬** 그리고 **효과적인 액세스 제어 정책**.

### 액세스 제어 정책 제거 {#removing-an-access-control-policy}

1. 표 **로컬 액세스 제어 정책** 항목 오른쪽에 있는 빨간색 아이콘(-)을 클릭합니다.

1. 항목이 두 테이블의 **로컬** 그리고 **효과적인 액세스 제어 정책**.

### 액세스 제어 정책 테스트 {#testing-an-access-control-policy}

1. CRXDE Lite 도구 모음에서 를 선택합니다 **도구**, 그런 다음 **액세스 제어 테스트...**.

1. 오른쪽 상단 창에 새 대화 상자가 열립니다. 을(를) 선택합니다 **경로** 및/또는 **주체** 테스트해 보십시오.

1. 클릭 **테스트** 선택 사항에 대한 결과를 보려면 다음을 수행하십시오.
