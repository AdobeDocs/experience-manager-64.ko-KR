...
>[!NOTE]
>
>AEM-LF 패키지의 일부 기능은 SCF(소셜 구성 요소 프레임워크)에 따라 다릅니다. Livefyre 기능 팩을 비커뮤니티 사이트의 일부로 사용하는 경우 웹 사이트의 작성자 클라이언트 *에 cq.social.scf* 종속성이 있음을 선언해야 합니다. LF 기능 팩을 커뮤니티 웹 사이트의 일부로 사용하는 경우 이 종속성은 이미 선언되어야 합니다.

1. AEM 홈페이지에서 왼쪽 레일에 있는 **도구** 아이콘을 클릭합니다.
1. 배포 **> 패키지로 이동합니다**.
...
페이지에 대해 Livefyre가 활성화되면 Livefyre 구성 요소를 허용하도록 컨테이너를 구성해야 합니다. 다른 구성 [요소를 활성화하는 방법에 대한 지침은 디자인 모드에서](https://helpx.adobe.com/experience-manager/6-3/sites/authoring/using/default-components-designmode.html) 구성 요소 구성을 참조하십시오.

>[!NOTE]
>
>게시할 인증이 필요한 앱은 단일 사인온 통합 사용자 지정에서 인증이 구성될 때까지 작동하지 않습니다.

1. 디자인 모드의 **구성 요소** 사이드 패널에서 **Livefyre를** 선택하여 목록을 사용 가능한 Livefyre 구성 요소로 제한합니다.
...
1. 새 Livefyre 앱을 만들지 또는 기존 앱을 포함할지 여부를 선택합니다.
   ...
   기존 앱을 내장하는 경우 AEM에서 앱을 선택하라는 메시지가 표시됩니다. 새 앱을 만드는 경우 콘텐츠가 표시되기 전에 앱을 채워야 합니다. 페이지에 대해 Livefyre 클라우드 구성이 활성화되면 선택한 네트워크 및 Livefyre 사이트에서 앱이 만들어집니다.
...
## AEM Assets과 함께 Livefyre 사용 {#use-livefyre-with-aem-assets}

### 권한 요청 및 AEM Assets으로 UGC 가져오기 {#request-rights-and-import-ugc-into-aem-assets}

UGC 가져오기를 사용하여 Twitter 및 Instagram 사용자 생성 콘텐츠(UGC)를 Livefyre 스튜디오에서 AEM Assets으로 가져올 수 있습니다. 가져올 컨텐츠를 선택한 후 가져오기를 완료하기 전에 콘텐츠에 대한 권한을 요청해야 합니다.
...
1. 만들기 **를**&#x200B;클릭한 다음 **UGC 가져오기를 클릭합니다.**

   ![livefyre-aem-import-ugc](assets/livefyre-aem-import-ugc.png)

1. 컨텐츠 찾기:

   * Livefyre에서 UGC 라이브러리 탭을 클릭합니다. 필터와 검색을 사용하여 UGC 라이브러리에서 컨텐츠를 찾습니다.
...
1. 권한 요청에 대해 각 자산에 대해 다음 옵션 중 하나를 선택합니다.
   ...
   * **수동으로 권한** 요청을 참조하십시오.
   * **콘텐츠 권한** 수동 속성을 참조하십시오.

   >[!NOTE]
   >
   >비비즈니스 사용자 계정의 컨텐츠 집계에 영향을 주는 업데이트로 인해 더 이상 사용자를 대신하여 댓글을 게시하거나 작성자의 답글을 자동으로 확인할 수 없습니다. [자세한 내용을 보려면 여기를 클릭하십시오](https://developers.facebook.com/blog/post/2018/04/04/facebook-api-platform-product-changes/).

   ![livefyre-aem-import-ugc3-6-4](assets/livefyre-aem-import-ugc3-6-4.png)

   Twitter:

   * **메시지 작성자** - 자산에 대한 권한을 요청하는 콘텐츠 소유자에게 메시지를 보냅니다.
   * **콘텐츠 권한** 수동 속성을 참조하십시오.
...
1. **가져오기**&#x200B;를 클릭합니다. 
   ...
   >[!NOTE]
   >
   >Twitter는 동일한 계정에서 들어오는 동일한 요청에 대해 제한을 두고 있습니다. 두 개 이상의 자산을 가져올 때 플래그가 지정되지 않도록 메시지를 개별적으로 수정합니다.

1. 오른쪽 **위** 모서리에서 완료를 클릭하여 권한 요청 워크플로우를 완료합니다.

   Livefyre Studio에서 자산에 대한 보류 중인 권한 요청의 상태를 볼 수 있습니다. 콘텐츠가 권한 요청을 보류 중인 경우 권한이 부여될 때까지 자산이 AEM Assets에 표시되지 않습니다. 권한 요청이 부여되면 자산이 AEM Assets에 자동으로 표시됩니다.
...
1. 이 **컬렉션을 AEM의 특정 페이지에 연결하려면 Livefyre 페이지 접두사를** 입력합니다.
