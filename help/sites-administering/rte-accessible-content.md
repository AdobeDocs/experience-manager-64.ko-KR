...
seo-description: 액세스 가능한 사이트를 제작하도록 AEM 리치 텍스트 편집기를 구성하는 방법을 알아봅니다.
...
AEM은 다음 두 가지 모두를 지원합니다.

* 표준 액세서빌러티 기능, 이미지의 대체 텍스트 포함
* 뿐만 아니라 리치 텍스트 편집기(RTE)를 사용하는 구성 요소를 사용하여 컨텐츠를 만들 때 액세스할 수 있는 추가 기능
...
컨텐츠 작성자는 페이지에 컨텐츠를 추가하는 동안 RTE의 기능을 사용하여 액세서빌러티 정보를 제공할 수 있습니다. 여기에는 머리글 및 단락 요소를 통해 구조적 정보를 추가할 수 있습니다.

구성 요소에 대한 RTE 플러그인을 구성하여 이러한 기능을 [구성하고 사용자 지정할](#configuring-the-plugin-features) 수 있습니다. 예를 들어, 이 `paraformat` 플러그인을 사용하면 기본 수준을 넘어 지원되는 머리글 수준 수를 확장하거나 기본적으로 `H1`제공되는 것을 포함하여 추가적인 블록 수준 의미 요소 `H2` `H3` 를 추가할 수 있습니다.

RTE는 터치 활성화 UI와 클래식 UI 모두에서 다양한 구성 요소에서 사용할 수 있습니다. 그러나 RTE를 사용하기 위한 기본 구성 요소는 **텍스트** 구성 요소입니다.

AEM의 **텍스트** 구성 요소는 터치 지원 및 클래식 UI 모두에 사용할 수 있습니다. 다음 이미지는 다음을 포함하여 다양한 플러그인이 활성화된 리치 텍스트 편집기를 보여줍니다. `paraformat`:

* 터치 **활성화** UI의 텍스트 구성 요소:
...
>[!NOTE]
>
>클래식 UI에서 사용할 수 있는 RTE 기능과 터치 지원 UI는 차이가 있습니다. 자세한 내용은
>
...
RTE 구성에 대한 전체 지침은 리치 텍스트 편집기 구성 [페이지에서 확인할 수 있습니다](/help/sites-administering/rich-text-editor.md) . 여기에는 주요 단계를 비롯한 모든 문제가 포함됩니다.
...
CRXDE Lite의 해당 하위 `rtePlugins` 분기 내에 플러그인을 구성하면(다음 이미지 참조) 해당 플러그인에 대한 모든 또는 특정 기능을 활성화할 수 있습니다.
...
1. 그런 다음 RTE의 선택 필드에서 컨텐츠 작성자가 단락 포맷을 사용할 수 있습니다. 액세스 가능한 항목:
   ...
   * 터치 지원 UI에서 단락([플루트](https://en.wikipedia.org/wiki/Pilcrow)) 아이콘 사용:

   ![단락(펄로우) 아이콘](do-not-localize/chlimage_1-7.png)

   * 클래식 UI에서 **형식** 필드(드롭다운 선택기) 사용


RTE에서 단락 형식 옵션을 통해 구조적 요소를 사용할 수 있으므로 AEM은 액세스 가능한 컨텐츠를 개발할 수 있는 적절한 기반을 제공합니다. 컨텐츠 작성자는 RTE를 사용하여 글꼴 크기 또는 색상 또는 기타 관련 속성의 서식을 지정할 수 없으므로 인라인 서식을 만들 수 없습니다. 대신 머리글과 같은 적절한 구조 요소를 선택하고 스타일 옵션에서 선택한 전역 스타일을 사용해야 합니다. 이렇게 하면 자신만의 스타일 시트와 올바르게 구조화된 컨텐츠로 찾아보는 사용자에게 보다 향상된 마크업 기능과 깔끔한 마크업이 보장됩니다.
...
AEM의 액세스 가능성 기능을 추가로 확장하기 위해 추가 요소 및 특성을 사용하여 RTE( **텍스트** 및 **표** 구성 요소 등)를 기반으로 기존 구성 요소를 확장할 수 있습니다.
...
표 **에서** DOM 요소의 속성을 명시적으로 설정하거나 제거해야 합니다. 이 값은 `config` 개체의 대화 상자로 전달됩니다. DOM 속성은 해당 `CQ.form.rte.Common` 방법(바로 가기)을 사용하여 설정/제거해야 브라우저 구현의 일반적인 문제 `com` 를 방지할 수 `CQ.form.rte.Common`있습니다.
...
1. 모두 저장을 사용하여 변경 **내용 저장**

>[!NOTE]
>
>일반 텍스트 필드는 캡션 요소의 값에 대해 허용되는 입력 유형만 아닙니다. 이 메서드를 통해 캡션의 값을 제공하는 ExtJS 위젯을 사용할 수 `getValue()` 있습니다.
>
