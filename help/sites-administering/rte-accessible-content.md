...
* 리치 텍스트 편집기(RTE)를 사용하는 구성 요소를 사용하여 컨텐츠를 만들 때 액세스할 수 있는 추가 기능
...
컨텐츠 작성자는 페이지에 컨텐츠를 추가하는 동안 RTE의 기능을 사용하여 액세서빌러티 정보를 제공할 수 있습니다. 여기에는 머리글 및 단락 요소를 통한 구조 정보를 추가할 수 있습니다.

구성 요소에 대한 RTE 플러그인[을 구성하여 이러한 기능을 구성하고 사용자 지정할 수 있습니다. ](#configuring-the-plugin-features) 예를 들어 `paraformat` 플러그인을 사용하면 기본적으로 제공되는 기본 `H1`, `H2` 및 `H3` 외에 지원되는 머리글 수준 수를 확장하는 등 블록 수준 의미 요소를 추가로 추가할 수 있습니다.

RTE는 터치 활성화 UI와 클래식 UI 모두에서 다양한 구성 요소에서 사용할 수 있습니다. 그러나 RTE를 사용하기 위한 기본 구성 요소는 **Text** 구성 요소입니다.

AEM의 **텍스트** 구성 요소는 터치 지원 UI와 클래식 UI 모두에서 사용할 수 있습니다. 다음 이미지는 `paraformat`을(를) 포함하여 플러그인 범위가 활성화된 리치 텍스트 편집기를 보여줍니다.

* 터치가 활성화된 UI의 **텍스트** 구성 요소:
...
RTE 구성에 대한 전체 지침은 [리치 텍스트 편집기 구성](/help/sites-administering/rich-text-editor.md) 페이지에서 확인할 수 있습니다. 여기에는 주요 단계를 비롯한 모든 문제가 포함됩니다.
...
CRXDE Lite의 해당 `rtePlugins` 하위 분기 내에 플러그인을 구성함으로써(다음 이미지 참조) 해당 플러그인에 대한 모든 또는 특정 기능을 활성화할 수 있습니다.
...
1. 그런 다음 RTE의 선택 필드에서 컨텐츠 작성자가 단락 형식을 사용할 수 있습니다. 액세스 가능한 항목:
   ...
   * 터치가 활성화된 UI에서 단락([플루트](https://en.wikipedia.org/wiki/Pilcrow)) 아이콘 사용:

   ![단락(모음) 아이콘](do-not-localize/chlimage_1-7.png)
...
## 추가 HTML 요소 및 특성 지원 추가 {#adding-support-for-additional-html-elements-and-attributes}
...
### 예 - {#example-adding-the-caption-to-the-table-properties-dialog} 표 속성 대화 상자에 캡션 추가

`TablePropertiesDialog` 생성자에서 캡션을 편집하는 데 사용되는 추가 텍스트 입력 필드를 추가합니다. 내용을 자동으로 처리하려면 `itemId`을 `caption`(즉, DOM 속성의 이름)으로 설정해야 합니다.

**표**&#x200B;에서는 DOM 요소의 속성을 명시적으로 설정하거나 제거해야 합니다. 이 값은 `config` 객체의 대화 상자에 의해 전달됩니다. 브라우저 구현의 일반적인 실수를 방지하기 위해 해당 `CQ.form.rte.Common` 메서드( `com`은(는) `CQ.form.rte.Common` 단축키임)를 사용하여 DOM 속성을 설정/제거해야 합니다.
