...
* 수동:
   ...
   * 일괄 처리를 위한 워크플로우 패키지 [사용](#workflow-packages-for-batch-processing).

* 자동:

   * 노드 변경에 대한 응답으로; [실행 프로그램](#workflows-launchers)사용.

>[!NOTE]
>
>다른 방법은 작성자도 사용할 수 있습니다. 자세한 내용은 다음을 참조하십시오.
>
...
워크플로우 모델 콘솔에 나열된 모델 [중 하나를](/help/sites-administering/workflows.md#workflow-models-and-instances) 기반으로 워크플로우를 시작할 수 있습니다. 필수 정보는 페이로드이지만 제목 및/또는 댓글도 추가할 수 있습니다.

## 워크플로우 발사체 {#workflows-launchers}

Workflow Launcher는 변경된 노드의 위치 및 리소스 유형에 따라 워크플로우를 시작하기 위해 컨텐츠 저장소의 변경 사항을 모니터링합니다.

론처 **를 사용하여 다음을 수행할** 수 있습니다.

* 특정 노드에 대해 이미 시작된 워크플로우를 참조하십시오.
* 특정 노드/노드 유형이 생성/수정/제거되면 시작할 워크플로우를 선택합니다.
* 기존 워크플로우-노드 관계를 제거합니다.

모든 노드에 대해 론쳐를 만들 수 있습니다. 하지만 특정 노드에 대한 변경 사항은 워크플로우를 실행하지 않습니다. 다음 경로 아래의 노드를 변경해도 워크플로우가 실행되지 않습니다.

* `/var/workflow/instances`
* 분기의 어느 곳에나 있는 모든 Workflow-inbox `/home/users` 노드
* `/tmp`
* `/var/audit`
* `/var/classes`
* `/var/eventing`
...
* 작성 날짜, 패키지를 생성한 사용자 및 간단한 설명과 같은 패키지 정보를 저장합니다.
...
1. 도구, 워크플로우 **,** 모델 **을 사용하여**&#x200B;모델 **콘솔**&#x200B;로 이동한 다음 ****&#x200B;모델로 이동합니다.
1. 콘솔 보기에 따라 워크플로우를 선택합니다. 필요한 경우 검색(왼쪽 상단)을 사용할 수도 있습니다.
   ...
   >[!NOTE]
   >
   >임시 **[표시기는 워크플로우](/help/sites-developing/workflows.md#transient-workflows)**내역이 지속되지 않는 워크플로우를 보여줍니다.
...
1. 도구, **워크플로우** , ****&#x200B;방사형 **도구**&#x200B;를 사용하여 Workflow **Lautter 콘솔**&#x200B;으로이동합니다.
1. 만들기 **를**&#x200B;선택한 다음 **실행** 추가를 선택하여 대화 상자를 엽니다.
   ...
   * **조건**
      ...
      평가 시 워크플로우가 실행되는지 여부를 결정하는 노드 값에 대한 조건 목록입니다. 예를 들어 다음 조건에서는 노드에 사용자 값이 있는 속성 이름이 있을 때 워크플로우가 시작됩니다.
   ...
   * **비활성화된 기능**
   ...
   비활성화할 기능 목록. 드롭다운 선택기를 사용하여 필요한 기능을 선택합니다.
   ...
   * **설명**
      ...
      실행 프로그램 구성을 설명하고 식별하는 자체 텍스트입니다.
   ...
   * **활성화**
      ...
      * 워크플로가 **실행되지** 않을 경우(구성 속성이 만족되어도 사용 안 함)을 선택합니다.
   * **제외 목록**

      워크플로우 트리거 여부를 결정할 때 제외할 JCR 이벤트를 지정합니다(즉, 무시).

      이 론처 속성은 쉼표로 구분된 항목 목록입니다. &quot;

      * `property-name` 지정된 속성 이름에 트리거된 모든 `jcr` 이벤트를 무시합니다. &quot;
      * `event-user-data:<*someValue*>` API `*<someValue*`(https://docs.adobe.com/content/docs/en/spec/jsr170/javadocs/jcr-2.0/javax/jcr/observation/ObservationManager.html#setUserData(java.lang.String)을 통해 `user-data` > [ 설정된 이벤트를 `ObservationManager` 무시합니다].
...
론처 구성을 만든 후 동일한 콘솔을 사용하여 인스턴스를 선택한 다음 속성 **보기** (및 편집) 또는 **삭제를**&#x200B;선택할 수 있습니다.
