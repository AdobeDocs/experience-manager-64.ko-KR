...
>[!NOTE]
>
>사전 구성된 보고서는 성능이 높지 않지만 운영 환경에서 일별 스냅샷을 사용하는 것이 좋습니다. 웹 사이트에 많은 작업이 없는 시간에 가능한 경우 이러한 일일 스냅샷을 실행할 수 있습니다. 다음을 사용하여 정의할 수 있습니다. `Daily snapshots (repconf.hourofday)` 매개 변수 **일 CQ 보고 구성**; 참조 [OSGI 구성](/help/sites-deploying/configuring-osgi.md) 를 구성하는 방법에 대한 자세한 내용을 살펴보십시오.
...
>title="디스크 사용량 보고서"
>abstract="CTT Calculator에 대한 입력 값으로 사용자는 “http://HOSTPORT/etc/reports/diskusage.html”에서 디스크 사용량 보고서를 실행하여 AEM 저장소 크기를 예측할 수 있습니다. 사용자는 “http://HOST:PORT/etc/reports/diskusage.html?path=/content/dam”과 같은 “경로” 매개변수를 사용하여 특정 저장소 경로 크기를 예측할 수도 있습니다."
