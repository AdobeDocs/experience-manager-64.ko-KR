...
클라우드 구성 및 프레임워크를 찾으려면 로 이동합니다. **Cloud Services** via **도구** > **배포** > **클라우드**. ([http://localhost:4502/libs/cq/core/content/tools/cloudservices.html](http://localhost:4502/libs/cq/core/content/tools/cloudservices.html))\
Adobe Target 아래에서 을 클릭하거나 탭합니다 **구성 표시**.
...
1. 대화 상자에서 이러한 속성에 대한 값을 제공합니다.
   ...
   * **암호**: Target 계정 암호입니다.
   * **API 유형**: REST 또는 XML
   * **A4T Analytics 클라우드 구성**: 타겟 활동 목표 및 지표에 사용되는 Analytics 클라우드 구성을 선택합니다. 이는 콘텐츠를 타겟팅할 때 보고 소스로서의 Adobe Analytics를 사용하는 경우 필요합니다. 클라우드 구성이 표시되지 않으면 [A4T Analytics Cloud 구성 구성](#configuring-a-t-analytics-cloud-configuration).
   ...
   * **DTM을 사용하여 클라이언트 라이브러리 제공** - DTM이나 다른 태그 관리 시스템에서 AT.js 또는 mbox.js를 사용하려면 이 옵션을 선택합니다. 다음을 수행해야 합니다. [dtm 통합 구성](/help/sites-administering/dtm.md) 이 옵션을 사용하려면 다음을 수행하십시오. Adobe은 AEM보다 DTM을 사용하여 라이브러리를 전달하는 것을 권장합니다.
...
1. Target에 연결하려면 **Target에 연결**&#x200B;을 클릭하십시오. 연결에 성공하면** 연결 성공** 메시지가 표시됩니다. 메시지에서 **확인**&#x200B;을 클릭한 다음 대화 상자에서 **확인**&#x200B;을 클릭합니다.
...
Target 클라우드 구성을 구성한 다음에는 Target 프레임워크를 추가할 수 있습니다. 프레임워크는 사용 가능한 페이지에서 Adobe Target으로 전송되는 기본 매개 변수를 식별합니다 [Client Context](/help/sites-administering/client-context.md) 또는 [ContextHub](/help/sites-administering/contexthub-config.md) 구성 요소. Target은 매개변수를 사용하여 현재 컨텍스트에 적용되는 세그먼트를 결정합니다.
...
1. 프레임워크 만들기 대화 상자에서 **제목**&#x200B;을 지정하고 **Adobe Target 프레임워크**&#x200B;를 선택한 다음 **만들기**&#x200B;를 클릭합니다.
   ...
   프레임워크 페이지가 열립니다. 사이드 킥은 의 정보를 나타내는 구성 요소를 제공합니다 [Client Context](/help/sites-administering/client-context.md) 또는 [ContextHub](/help/sites-administering/contexthub-config.md) 매핑할 수 있습니다.
