...
* AEM 기능의 경우 AEM 작성자 인터페이스에서 json 호출이 요청된다는 것을 알 수 있습니다. 이로 인해 HTTP-500 오류가 발생하지 않습니다. HTTP-500 오류가 표시되는 경우 에 `error.log` 대한 자세한 내용을 확인하십시오.
* AEM에서 캠페인 클래스에 대한 디버그 수준을 높이면 문제를 해결하는 데 도움이 됩니다.

## 연결이 실패한 경우 {#if-the-connection-fails}

Adobe Campaign에서 aemserver **연산자를** 구성했는지 확인하십시오.

## Adobe Campaign 콘솔에 이미지가 나타나지 않는 경우 {#if-images-do-not-appear-in-the-adobe-campaign-console}
...
## 설정 로케일에 대한 경고가 나타나면 {#if-you-get-a-warning-about-your-setlocale}

Apache HTTPD 서비스를 시작하고 있는 경우 오류가 `"Warning: setlocale: LC_CTYPE cannot change locale"` 표시되면 시스템에 **en_CA.ISO-8859-15 로케일이** 설치되어 있는지 확인합니다.

을 사용하여 설치되어 있는지 확인할 수 있습니다 `local -a`. 설치되어 있지 않은 경우 /usr/local/neolane/nl6/env.sh **** 스크립트를 패치하고 로케일을 설치된 스크립트로 변경할 수 있습니다.
...
1. &quot;amcGetSeedMetaData&quot; 메서드의 467줄을 수정합니다.
...
Adobe Campaign Classic에서 **동기화** 버튼을 클릭하면 다음 오류가 표시됩니다.
...
동기화를 클릭하면 페이지의 스크립트가 발생했다는 오류가 표시됩니다. XTK Date+Time &#39;undefined&#39;을 구문 분석할 수 없습니다. 올바른 XTK 값이 아닙니다.
...
Adobe Campaign 지원팀과 함께 표를 구해주세요
...
* AEM 작성자 통신 시 https를 사용하는 호스팅 Adobe Campaign
* SSL을 종료하는 역방향 프록시
* 온-프레미스 AEM 작성자 인스턴스

AEM은 Adobe Campaign 배달에서 컨텐츠를 동기화하려고 할 때 뉴스레터 목록을 반환합니다. 하지만 목록의 뉴스레터에 대한 URL은 http 주소입니다. 목록의 항목 중 하나를 선택하면 오류가 발생합니다.

이 문제를 해결하려면 다음을 수행하십시오.

* 원래 프로토콜을 헤더로 전달하려면 디스패처 또는 역방향 프록시를 구성해야 합니다.
* OSGi 구성( *https://&lt;host>:&lt;port>/system/console/configMgr* )의 Apache Felix Http Service SSL 필터를[](http://localhost:4502/system/console/configMgr)각 헤더 설정으로 구성해야 합니다. https://felix.apache.org/documentation/subprojects/apache-felix-http-service.html#using-the-ssl-filter을 [참조하십시오.](https://felix.apache.org/documentation/subprojects/apache-felix-http-service.html#using-the-ssl-filter)

## 내가 만든 사용자 지정 템플릿을 페이지 속성에서 선택할 수 없는 경우 {#if-the-custom-template-i-created-cannot-be-selected-in-page-properties}
...
사용자 지정 템플릿을 사용하는 경우 로그에 &quot;com.day.cq.mcm.campaign.servlets.util.ParameterMapper&quot; 오류가 표시됩니다. 이 경우 [패키지 공유에서 Feature Pack 6576을 설치해야 합니다](/help/sites-administering/package-manager.md#package-share). 이 문제는 acMapping 속성이 recipient.firstName 이외의 값으로 설정된 경우 Adobe Campaign 관리자 측에 빈 값이 만들어집니다.
