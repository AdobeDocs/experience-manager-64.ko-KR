---
title: Adobe Analytics 및 프레임워크 연결
seo-title: Connecting to Adobe Analytics and Creating Frameworks
description: AEM과 SiteCatalyst을 연결하고 프레임워크를 만드는 방법에 대해 알아보십시오.
seo-description: AEM과 SiteCatalyst을 연결하고 프레임워크를 만드는 방법에 대해 알아보십시오.
...
# Adobe Analytics 및 프레임워크 연결{#connecting-to-adobe-analytics-and-creating-frameworks}

Adobe Analytics의 AEM 페이지에서 웹 데이터를 추적하려면 Adobe Analytics Cloud 서비스 구성 및 Adobe Analytics 프레임워크을 만듭니다.

* **Adobe Analytics 구성:** Adobe Analytics 계정에 대한 정보입니다. Adobe Analytics 구성을 통해 AEM은 Adobe Analytics에 연결할 수 있습니다. 사용하는 각 계정에 대한 Adobe Analytics 구성을 만듭니다.
...
웹 페이지를 프레임워크와 연결할 때 프레임워크는 해당 페이지와 해당 페이지의 하위 항목에 대한 추적을 수행합니다. 그런 다음 Adobe Analytics에서 페이지 보기를 검색하여 사이트 콘솔에 표시할 수 있습니다.
...
>[!CAUTION]
>
>Adobe Analytics 내 **에서 관리자** 권한을 제공한다고 해서 사용자가 AEM에서 Adobe Analytics으로 연결할 수 있는 것은 아닙니다. 계정에는 **웹 서비스 액세스** 권한도 있어야 합니다.

![chlimage_1-316](assets/chlimage_1-316.png)

계속하기 전에 다음 방법 중 하나를 사용하여 자격 증명을 통해 Adobe Analytics에 로그인할 수 있도록 해야 합니다.
...
>
>예를 들어, 기본 소매 ** 보고서 세트를 사용하도록 프레임워크가 구성되고 작성자가 선택한 서버 인스턴스입니다. 페이지가 프레임워크와 함께 게시되더라도 Adobe Analytics에 대한 호출은 여전히 진행되지만, 이러한 호출에는 RSID가 포함되지 않습니다. 작성자 인스턴스의 호출에만 RSID가 포함됩니다.

1. 탐색 **을 사용하여**&#x200B;도구 **,** Cloud Services **, 레거시 Cloud Services**&#x200B;을 ****&#x200B;선택합니다.
2. [사용 가능한 구성] 옆에 있는 **Adobe Analytics** 로 스크롤하고 **[다음]** 을 **클릭합니다**.
3. Adobe Analytics 구성 옆에 있는 **[+]** 링크를 클릭합니다.

4. 프레임워크 **만들기** 대화 상자에서 다음을 수행합니다.
...
5. 사이드 창의 **보고서 세트** 섹션(기본 패널의 오른쪽)에서 [항목 **추가]를 클릭합니다**. 그런 다음 드롭다운을 사용하여 프레임워크와 상호 작용할 보고서 세트 ID( `geometrixxauth`예:)를 선택합니다.

   >[!NOTE]
   >
   >왼쪽의 컨텐츠 파인더는 보고서 세트 ID를 선택하면 Adobe Analytics 변수(SiteCatalyst 변수)로 채워집니다.

6. 그런 다음 **실행 모드** 드롭다운(보고서 세트 ID 옆)을 사용하여 정보를 보고서 세트로 전송할 서버 인스턴스를 선택합니다.

   ![a-framework-01](assets/aa-framework-01.png)

7. 사이트의 게시 인스턴스에서 프레임워크를 사용할 수 있게 하려면 사이드 킥의 **페이지** 탭에서 **프레임워크 활성화를 클릭합니다.**

### Adobe Analytics에 대한 서버 설정 구성 {#configuring-server-settings-for-adobe-analytics}

프레임워크 시스템을 사용하면 각 Adobe Analytics 프레임워크 내의 서버 설정을 변경할 수 있습니다.

>[!CAUTION]
>
>이러한 설정은 데이터의 전송 위치와 방법을 결정하므로 이러한 설정을 *변경하지* 않고 Adobe Analytics 담당자가 직접 설정할 수 있도록 해야 합니다.
...
* **방문자 네임스페이스**
   ...
   * 예: 네임스페이스를 **CNAME으로** 변경하면 Adobe Analytics에 대한 호출은 기본값이 아닌 **CNAME.d1.omtrdc.net** 과같이 됩니다.
...
1. 구성 **추가** 드롭다운을 사용하여 사용 가능한 옵션 **에서 Adobe Analytics** 를 선택합니다. 상속이 있는 경우 선택기를 사용할 수 있게 되기 전에 상속을 비활성화해야 합니다.
...
1. 그런 다음 적절한 도구를 사용하여 Adobe Analytics에 대한 호출을 확인할 수 있습니다. 예: [Adobe Experience Cloud 디버거](https://docs.adobe.com/content/help/en/debugger/using/experience-cloud-debugger.html).
...
자세한 [내용은 페이지 분석 데이터](/help/sites-authoring/pa-using.md) 보기를 참조하십시오.
...
Adobe Analytics 구성 또는 프레임워크를 만들 때처럼 (기존) **Cloud Services** 화면으로 이동합니다. 구성 **표시를**&#x200B;선택한 다음 업데이트할 특정 구성에 대한 링크를 클릭합니다.

Adobe Analytics 구성을 편집할 때 구성 페이지 자체에서 구성 요소 **편집** 대화 상자를 열려면 **편집** 단추를 눌러야 합니다.
