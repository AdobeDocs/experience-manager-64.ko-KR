...
* 페이지 이벤트(예: 수정 또는 복제)를 구독했습니다. [ [알림 받은 편지함](/help/sites-classic-ui-authoring/author-env-inbox.md#subscribing-to-notifications) ] 섹션에서는 이러한 이벤트를 구독하는 방법을 설명합니다.
...
AEM에서 이메일을 전송하려면 **Day CQ Mail Service** 를 올바르게 구성해야 합니다. 웹 콘솔에서 구성을 볼 수 있습니다. When working with AEM there are several methods of managing the configuration settings for such services; see [Configuring OSGi](/help/sites-deploying/configuring-osgi.md) for more details and the recommended practices.
...
1. 사용자 `jcr_root/apps/*app_name*/config folder`에서 `com.day.cq.wcm.notification.email.impl.EmailChannel.xml`

1. 노드를 나타내는 다음 XML을 추가합니다.

   `<?xml version="1.0" encoding="UTF-8"?> <jcr:root xmlns:sling="https://sling.apache.org/jcr/sling/1.0" xmlns:jcr="https://www.jcp.org/jcr/1.0" jcr:primaryType="sling:OsgiConfig" email.from="name@server.com"/>`
1. 속성()의 값 `email.from` 을 이메일 `name@server.com`주소로 바꿉니다.
...
1. 메일 서비스 구성에 설명된 대로 이메일 서비스 [를 구성합니다](/help/sites-administering/notification.md#configuring-the-mail-service).
1. AEM에 관리자로 로그인합니다. 도구 **> 작업** **** > **웹 콘솔** 을 클릭하여 웹 콘솔 구성을 엽니다.
1. 일 **CQ DAM 리소스 수집 서블릿을 편집합니다**. 이메일 **전송을 선택합니다**. **저장**&#x200B;을 클릭합니다.
