...
seo-title: Adobe Campaign Standard와 통합
description: Adobe Campaign Standard와 통합
seo-description: Adobe Campaign Standard와 통합
...
# Adobe Campaign Standard와 통합{#integrating-with-adobe-campaign-standard}

>[!NOTE]
>
>이 설명서에서는 구독 기반 솔루션인 Adobe Campaign Standard와 AEM을 통합하는 방법을 설명합니다. Adobe Campaign 6.1을 사용하는 경우 [이러한 지침은 Adobe Campaign 6.1과](/help/sites-administering/campaignonpremise.md) 통합을 참조하십시오.
...
AEM에서 Adobe Campaign을 사용한 작업에는 Adobe Campaign을 통해 이메일 및 양식을 보내는 기능이 포함되어 있으며 Adobe Campaign [작업에 설명되어 있습니다](/help/sites-authoring/campaign.md).

또한 AEM을 Adobe Campaign과 통합할 때 다음 주제에 관심이 [있을 수 있습니다](https://docs.campaign.adobe.com/doc/standard/en/home.html).
...
[관리] > **[사용자** 및 보안 **] >** [사용자 **]**&#x200B;로 **이동하여 Aemserver** 사용자를선택합니다. 사용자 설정을 열려면 클릭합니다.

* 이 사용자의 암호를 설정해야 합니다. 이 작업은 UI를 통해 수행할 수 없습니다. 이 구성은 기술 관리자가 REST에서 수행해야 합니다.
* 사용자가 배달을 만들고 편집할 수 있는 deliveryPrepare **와**&#x200B;같은 특정 역할을 이 사용자에게 할당할 수 있습니다.
...
1. [관리] > **응용 프로그램** 설정 **>** 외부 계정으로 **이동합니다**.

   ![chlimage_1-124](assets/chlimage_1-124.png)

1. 기본 **aemInstance** 외부 계정을 선택하거나 만들기 **단추를 클릭하여 새 계정을** 만듭니다.
1. 유형 **필드에서** Adobe Experience Manager를 **선택하고** AEM 작성 인스턴스에 사용되는 액세스 매개 변수를 입력합니다.서버 주소, 계정 이름 및 암호

   >[!NOTE]
   >
   >URL 끝에 종료 **/** 슬래시를 추가하지 않으면 연결이 작동하지 않습니다.

1. 활성화 확인란을 **선택한** 다음 저장을 클릭하여 **수정 내용을** 저장합니다.
...
이 옵션은 미리 구성되어 있습니다.그러나 이 옵션을 변경하면 기능하지 않는 통합이 발생할 수 있습니다.

AEMResourceTypeFilter **옵션이** 구성되었는지 확인하려면 다음을 수행하십시오.

1. 관리 > **애플리케이션** 설정 **>** 옵션으로 **이동합니다**.
...
1. **작업 표시줄에서 확인 표시를 클릭하고 기존 표준** 이메일(메일) **기본 템플릿을 선택하여 선택한 다음 복사** 아이콘을 클릭하고 **확인을** 클릭하여 **해당 템플릿을**&#x200B;복제할 수있습니다.
1. x **를** 클릭하여 선택 모드를 비활성화하고 새로 만든 표준 **전자 메일(메일)** 템플릿을 연 다음 템플릿 **대시보드의 작업 표시줄에서 속성** 편집을 선택합니다.

   템플릿의 레이블을 수정할 수 **있습니다**.

1. 속성 콘텐츠 **섹션에서** 콘텐츠 **소스를** Adobe **Experience Manager로**&#x200B;변경합니다. 그런 다음 이전에 만든 외부 계정을 선택하고 확인을 **클릭합니다**.

   확인을 클릭하고 **저장을** 클릭하여 수정 내용을 **저장합니다.**

   이 템플릿에서 만든 이메일 배달에는 AEM 콘텐츠 기능이 활성화됩니다.
...
* 인스턴스 간 복제를 구성합니다.
* AEM을 Adobe Campaign에 연결합니다.
* Externalizer를 구성합니다.

### AEM 인스턴스 간 복제 구성 {#configuring-replication-between-aem-instances}

AEM 작성 인스턴스에서 만든 컨텐츠는 먼저 게시 인스턴스로 전송됩니다. 그런 다음 이 게시 인스턴스는 컨텐츠를 Adobe Campaign으로 전송합니다. 따라서 복제 에이전트를 AEM 작성 인스턴스에서 AEM 게시 인스턴스로 복제하도록 구성해야 합니다.

>[!NOTE]
>
>복제 URL을 사용하지 않고 대신 공개 URL을 사용하는 경우, OSGi의 다음 구성 설정에서 공개 URL을 **설정할 수 있습니다(** 도구&#x200B;**> 웹 콘솔** > **OSGi 구성** > AEM Campaign **통합 - 구성**).
...
1. 제작 인스턴스에서 AEM **로고**> ** **도구 **아이콘 > 배포** > **복제** > **작성자 에이전트, 기본 에이전트**&#x200B;클릭 ****&#x200B;에이전트를 선택합니다.
...
1. 도구 **> 작업** **** > **클라우드** > **클라우드 서비스**, **** 그런 다음 Adobe Campaign 섹션에서 구성을 선택합니다.
...
>[!NOTE]
연결에 실패하면 다음을 확인하십시오.
* Adobe Campaign 인스턴스(https)에 대한 보안 연결을 사용할 때 인증서 문제가 발생할 수 있습니다. JDK의 **cacerts **파일에 Adobe Campaign 인스턴스 인증서를 추가해야 합니다.
...
일반 [지침은](/help/sites-developing/externalizer.md) externalizer 구성을 참조하십시오. Adobe Campaign 통합의 경우 게시 서버를 Adobe Campaign 콘솔에서 연결할 수 `https://<host>:<port>/system/console/configMgr/com.day.cq.commons.impl.ExternalizerImpl` 있는 `localhost:4503` 서버가 아닌 서버에 구성해야 합니다.
