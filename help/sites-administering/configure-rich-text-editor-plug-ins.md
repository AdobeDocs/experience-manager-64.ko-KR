...
>[!NOTE]
>
>CRXDE Lite으로 작업할 때는 모두 저장을 사용하여 변경 사항을 정기적으로 저장하는 것이 좋습니다.
...
>[!NOTE]
>
>이 문서에서 중복을 방지하기 위해 해당 rtePlugins 노드를 &lt;*rtePlugins-node*>라고 합니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>문자열</td> 
   <td>문자열[](다중 문자열; [유형]을 [문자열]으로 설정하고 [다중 in CRXDE Lite]을 클릭합니다.</td> 
   <td>문자열</td> 
  </tr> 
  <tr> 
   <td><strong>값</strong></td> 
   <td>* (별표)<br /> </td> 
   <td>하나 이상의 피쳐 값으로 설정</td> 
   <td><strong>-</strong></td> 
  </tr> 
 </tbody> 
</table>
...
* **일반 텍스트 모드**: 클립보드 내용을 일반 텍스트로 붙여넣습니다. 복사한 콘텐트에서 스타일 및 서식 요소의 모든 요소를 제거하고 AEM 구성 요소에 삽입합니다.
...
* 클립보드 내용을 일반 텍스트로 붙여넣습니다. 복사한 콘텐트에서 스타일 및 서식 요소의 모든 요소를 제거하고 AEM 구성 요소에 삽입합니다. 아래를 사용하여 `plaintext` 구성합니다.
...
다른 프로그램에서 AEM에 텍스트를 붙여넣을 때 허용되는 형식을 구성하려면 다음을 수행하십시오.
...
>[!NOTE]
표 또는 [표 셀에도 스타일을 정의할 수 있습니다](/help/sites-administering/configure-rich-text-editor-plug-ins.md#tablestyles). 이러한 구성에는 별도의 절차가 필요합니다.
...
>[!CAUTION]
RTE의 단락 형식을 구성할 때는 단락 태그 &lt;p>를 서식 옵션으로 제거하지 마십시오. &lt;p> 태그가 제거된 경우, 컨텐츠 작성자는 추가 형식이 구성된 경우에도 **단락 서식** 옵션을 선택할 수 없습니다.
...
속성이 저장되면 표현된 문자가 CRXDE에 표시됩니다. 아래 절반의 예를 참조하십시오. 작성자가 사용할 수 있는 특수 문자를 만들려면 위 단계를 반복합니다.

![CRXDE에서 RTE 도구 모음에서 사용할 수 있는 단일 문자를 추가합니다.](assets/chlimage_1-412.png)

CRXDE에서 RTE 도구 모음에서 사용할 수 있는 단일 문자를 추가합니다.

### 문자 범위 정의 {#definerangechar}

1. 단일 문자 [정의에서 1~3단계를 사용합니다](#definesinglechar).
...
1. 이 노드 아래에서(특수 문자 범위에 따라 이름 지정) 다음 두 속성을 추가합니다.
   ...
   * **이름** `rangeStart`
      ...
      **유형** `Long`
      **범위** 에서 [첫 번째 문자](https://unicode.org/) 의 유니코드 표현(십진수) 값
...
1. 변경 사항을 저장합니다.
   ...
   ![RTE에서 사용할 수 있는 특수 문자가 팝업 창의 작성자에게 표시됩니다](assets/rtepencil.png)
...
>[!NOTE]
설치되지 않은 언어를 확인하려고 하면 메시지가 `Spell checking failed` 표시됩니다. 표준 사전은 해당 Readme 파일과 `/libs/cq/spellchecker/dictionaries`함께 에 있습니다. 파일을 수정하지 마십시오.

표준 AEM 설치에는 미국 영어(`en_us`) 및 영국 영어(`en_gb`)용 사전이 포함되어 있습니다. 사전을 더 추가하려면 다음 단계를 따르십시오.

1. https://extensions.openoffice.org/페이지로 [이동합니다](https://extensions.openoffice.org/).

1. 다음 중 하나를 수행하여 원하는 언어의 사전을 찾습니다.

   * 원하는 언어의 사전을 찾아보십시오. 사전 페이지에서 원본 소스 또는 저자의 웹 페이지에 대한 링크를 찾습니다. 해당 페이지에서 v2.x용 사전 파일을 찾습니다.
...
맞춤법 검사기에서 제안하는 변경 내용을 포함할 경우 텍스트 상태가 변경되고 맞춤법 오류가 있는 단어가 더 이상 강조 표시되지 않습니다. 맞춤법 검사기를 실행하려면 맞춤법 검사기 단추를 다시 탭/클릭합니다.
...
1. 노드에서 `undo` 속성을 만듭니다.
   ...
   * **기록에 저장할 실행 취소 단계 수의 값입니다** .
      ...
      * 기본값은 50입니다.
      * 실행 취소/재실행을 완전히 비활성화하려면 0을 사용합니다.

1. 변경 사항을 저장합니다.

## 탭 크기 구성 {#tabsize}

텍스트 내에서 탭 문자를 누르면 사전 정의된 공백 수가 삽입됩니다. 기본적으로 3개의 비분리 공백과 1개의 공백이 있습니다. 탭 크기를 정의하려면
