...
description: 개별 기능을 활성화하기 위해 AEM 리치 텍스트 편집기 플러그인을 구성하는 방법을 알아봅니다.
contentOwner: AG
...
>[!NOTE]
>
>CRXDE Lite 작업 시 모두 저장을 사용하여 정기적으로 변경 사항을 저장하는 것이 좋습니다.
...
>[!NOTE]
>
>각 rtePlugins 노드를 &lt;*rtePlugins-node*>라고 하여 이 문서에서 중복을 방지합니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>문자열</td> 
   <td>String[](다중 문자열)type 을 String으로 설정하고 Multi in CRXDE Lite 클릭)</td> 
   <td>문자열</td> 
  </tr> 
  <tr> 
   <td><strong>값</strong></td> 
   <td>* (별표)<br /> </td> 
   <td>하나 이상의 피쳐 값으로 설정합니다.</td> 
   <td><strong>-</strong></td> 
  </tr> 
 </tbody> 
</table>
...
* **일반 텍스트 모드**:클립보드 내용을 일반 텍스트로 붙여넣습니다. AEM 구성 요소에 삽입하기 전에 복사한 컨텐츠에서 스타일 및 서식의 모든 요소를 제거합니다.
...
* 클립보드 내용을 일반 텍스트로 붙여넣습니다. AEM 구성 요소에 삽입하기 전에 복사한 컨텐츠에서 스타일 및 서식의 모든 요소를 제거합니다. 아래의 `plaintext`을 사용하여 구성했습니다.
...
1. `edit` 노드에서 다음 세부 정보를 사용하여 속성을 만듭니다.
   ...
   * **이름** `defaultPasteMode`
   * **유형** `String`
   * **** 값필요한 붙여넣기 모드  `browser`,  `plaintext`또는  `wordhtml` 중 하나입니다.

### 컨텐츠를 붙여넣을 때 허용되는 형식을 구성합니다 {#pasteformats}

Microsoft Word와 같은 다른 프로그램에서 AEM에 붙여넣을 때 허용되는 스타일을 명시적으로 정의할 수 있도록 붙여넣기 as-Microsoft-Word(`paste-wordhtml`) 모드를 추가로 구성할 수 있습니다.
...
>[!NOTE]
대화 상자(클래식 UI)에서 RTE를 사용할 때 리치 텍스트 편집에 최적화된 스타일 시트를 지정할 수 있습니다. 기술 제한 사항으로 인해 CSS 컨텍스트는 편집기에서 손실되므로 이 컨텍스트를 에뮬레이션하여 WYSIWYG 경험을 개선할 수 있습니다.
리치 텍스트 편집기는 ID가 `CQrte`인 컨테이너 DOM 요소를 사용하여 보고 편집하는 데 다른 스타일을 제공할 수 있습니다.
`#CQ td {`
...
1. 노드 `styles` 아래에 새 노드(`styles`라고도 함)를 만들어 사용 가능한 목록을 유지합니다.

   * **이름** `styles`
   * **유형** `cq:WidgetCollection`

1. `styles` 노드 아래에 새 노드를 만들어 개별 스타일을 나타냅니다.
...
>[!CAUTION]
RTE의 단락 형식을 구성할 때는 단락 태그 &lt;p>를 서식 옵션으로 제거하지 마십시오. &lt;p> 태그가 제거되면 컨텐츠 작성자는 추가 형식이 구성되어 있어도 **단락 형식** 옵션을 선택할 수 없습니다.
...
1. `paraformat` 노드 아래에 새 노드를 만들어 형식 목록을 보관합니다.

   * **이름** `formats`
   * **유형** `cq:WidgetCollection`

1. `formats` 노드 아래에 새 노드를 만듭니다. 이 노드 아래에 개별 형식에 대한 세부 정보가 있습니다.
...
표준 AEM 설치에서 특수 문자(`specialchars`)에 대해 `misctools` 플러그인이 활성화되면 즉시 기본 선택 항목을 사용할 수 있습니다.예를 들어, 저작권 및 상표 기호
...
1. `chars` 아래에서 개별 문자 정의를 보유할 새 노드를 추가합니다.
...
1. [단일 문자 정의](#definesinglechar)에서 단계 1~3을 사용하십시오.
1. `chars` 아래에 새 노드를 추가하여 문자 범위의 정의를 유지하십시오.
...
1. 변경 사항을 저장합니다.
   ...
   ![CRXDE에서 RTE에서 사용할 수 있는 문자 범위를 정의합니다](assets/chlimage_1-413.png)

         *CRXDE에서 RTE*&#x200B;에 사용할 수 있는 문자 범위를 정의합니다.
...
1. `table` 노드 아래에 다음 새 노드를 만듭니다(필요에 따라).

   * 전체 테이블의 스타일을 정의하려면( **표 속성**&#x200B;에서 사용 가능):
...
표준 AEM 설치에는 미국 영어(`en_us`)와 영국 영어(`en_gb`)의 사전이 포함되어 있습니다. 사전을 더 추가하려면 다음 단계를 따르십시오.

1. 페이지 [https://extensions.openoffice.org/](https://extensions.openoffice.org/)로 이동합니다.

1. 원하는 언어 사전을 찾으려면 다음 중 하나를 수행합니다.
...
1. 맞춤법 정의가 있는 아카이브를 다운로드합니다. 파일 시스템에서 아카이브 컨텐츠를 추출합니다.
...
>[!NOTE]
RTE 맞춤법 검사기는 온디맨드로 사용할 수 있습니다. 텍스트 입력을 시작할 때 자동으로 실행되지 않습니다. 맞춤법 검사기를 실행하려면 도구 모음에서 [!UICONTROL 맞춤법 검사] 를 클릭합니다. RTE는 단어의 철자를 확인하고 철자가 잘못된 단어를 강조 표시합니다.
