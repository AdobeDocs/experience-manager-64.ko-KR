...
운영을 최적화하기 위해 AEM과 e커머스 엔진은 자신의 전문 분야에 집중합니다. 두 사람 사이에 실시간으로 정보가 전달된다. 예를 들면 다음과 같습니다.
...
* 전자 상거래 통합 프레임워크는 eCommerce 엔진을 AEM과 쉽게 통합할 수 있도록 구축되었습니다. 내장된 e커머스 엔진은 제품 데이터, 장바구니, 체크아웃 및 주문 이행을 제어하는 한편, AEM은 데이터 표시 및 마케팅 캠페인을 제어합니다.
...
>
>* API의 사용을 설명하기 위한 독립형 AEM 기본 e커머스 예입니다. 기존 데이터 표시 및 마케팅 캠페인과 함께 제품 데이터, 장바구니 및 결제를 제어하는 데 사용할 수 있습니다. 이 경우 제품 데이터베이스가 AEM(Adobe의 JCR 구현)이 구현되는 저장소에 [저장됩니다](https://docs.adobe.com/content/docs/en/spec/jcr/2.0/index.html).\
...
상거래 엔진에서 AEM e커머스 사이트로 데이터를 가져올 때 상거래 제공업체는 수입자에게 데이터를 제공하는 데 사용됩니다. 한 전자상거래 업체가 여러 수입업체를 지원할 수 있습니다.
...
ProductServicesManager [](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/cq/commerce/pim/api/ProductServicesManager.html) 는 (OSGi [를 통해)](/help/sites-deploying/configuring.md#osgi-configuration-settings)ProductImporter 및 [CatalogBlueprintImporterImporter 인터페이스의 구현](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/cq/commerce/pim/api/ProductImporter.html) 목록을 유지 관리합니다 [](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/cq/commerce/pim/api/CatalogBlueprintImporter.html) . 이러한 항목은 가져오기 마법사의 **가져오기/상거래 공급자** 드롭다운 필드에 나열되어 있습니다( `commerceProvider` 속성을 이름으로 사용).
...
다음 두 카테고리를 구별할 수 있는 경우, 의미 있는 구조(노드 트리)를 사용하여 URL을 명확히 할 수 있으므로 클래식 AEM 컨텐츠 관리에 매우 가까울 수 있습니다. `cq:Page`
...
각 제품에 대한 개별 속성은 사용되는 eCommerce 엔진 및 AEM 구현에 따라 달라질 수 있습니다. 제품 페이지 및/또는 제품 정보를 편집할 때 사용할 수 있으며 다음이 포함될 수 있습니다.
...
성능 테스트는 AEM eCommerce 구현을 고려해야 합니다.
...
제품 **구성** 요소는 필요한 메타데이터(예: 및 경로)를 전달하는 상위 페이지가 있는 모든 페이지에 추가할 `cartPage` 수 `cartObject`있습니다. 데모 사이트 Geometrixx Outdoors에서 이 항목은 `UserInfo.jsp`
...
하지만, 교역이 있다. AEM 내에서 제품 정보를 사용자 지정할 수 없으며 사이트에 대해 정의된 표준 템플릿이 사용됩니다.
...
>[!NOTE]
>
>AEM은 **바우처**&#x200B;라는 용어를 사용하며 쿠폰이라는 용어와 **비슷합니다**.
...
1. 사용자 계정은 AEM에만 있습니다.
   ...
   1. 사용자는 동일한 계정 ID와 무작위 암호로 e커머스 엔진에서 만들어지며 AEM에 저장됩니다.
   1. AEM이 첫 번째 통화 시 e커머스 엔진에 로그인하려고 할 때(예: 제품 페이지가 요청되고 e커머스 엔진이 가격에 대해 참조되는 경우) 무작위 암호가 필요합니다. 이 문제는 AEM 로그인 후 발생하므로 암호를 사용할 수 없습니다.
...
>[!NOTE]
>
>eCommerce 엔진을 사용하는 경우 AEM 인스턴스에 로그인하는 사용자에 대해 만들어진 계정이 해당 엔진과 통신하는 다른 AEM 인스턴스에 복제되도록(예: 워크플로우를 통해) 해야 합니다.
>
>그렇지 않으면 이러한 다른 AEM 인스턴스는 엔진에서 동일한 사용자에 대한 계정을 만들려고 합니다. 이러한 작업은 엔진에서 `DuplicateUidException` 나오는 경우 실패합니다.
...
SSO(Single Sign-On)가 제공되므로 두 번 로그인하지 않아도 작성자가 AEM 및 eCommerce 시스템에서 모두 알 수 있습니다.
...
새 주소 **추가..** .를 클릭하여 주소록에 새 주소를 추가할 수 있습니다. 양식에 내용을 입력한 다음 주소 **추가를 클릭합니다**.
...
주문에 대한 구현 정보에 따라 eCommerce 엔진이나 AEM에서 이러한 정보가 AEM에 의해 렌더링됩니다.
...
AEM 내에서 주문이 유지되면 주문 콘솔은 각 주문에 대해 다음을 표시합니다.
...
그런 다음 전자 상거래에 의해 AEM 양식의 입력과 함께 체크아웃 프로세스를 관리합니다.

### 지불 보안 {#payment-security}

신용카드 정보를 포함한 결제 세부 사항은 종종 eCommerce 엔진에서 관리합니다. AEM은 이러한 트랜잭션 정보를 엔진으로 전송(그런 다음 결제 처리 서비스로 전달)
