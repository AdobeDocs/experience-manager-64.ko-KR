...
AEM에서 JSR 286 호환 포틀릿을 실행할 수 있습니다. 포틀릿 구성 요소를 사용하여 페이지에 포틀릿을 포함할 수 있습니다. AEM [컨텐츠 포틀릿 관리를 참조하십시오](#administering-the-aem-content-portlet).
...
포털에 나타나도록 AEM 콘텐츠를 구성하려면 포틀릿의 [AEM 설치, 구성 및 사용을 참조하십시오](#installing-configuring-and-using-aem-in-a-portlet).
...
포털 서버가 호스트 localhost에서 실행된다고 가정할 때 포트 8080 및 AEM 포틀릿 웹 애플리케이션이 웹 애플리케이션 컨텍스트 *포틀릿에*&#x200B;마운트되면 웹 콘솔에 대한 URL이 `http://localhost:8080/cqportlet/cqbridge/system/console`됩니다. 기본 사용자 및 암호는 **관리자입니다**.
...
1. **편집**&#x200B;을 클릭합니다.
1. 설정 **탭에서** 활성화 **확인란을** 선택하고 일련화 유형으로 **발송자 플러시 **를 선택한 다음 재시도 시간 초과(예: 6000)를 입력합니다.
...
1. 기본적으로 http://localhost:4502/system/console/configMgr에 있는 OSGi 구성 콘솔의 드롭다운 메뉴에서 **CQ** PIN 인증 핸들러를 선택합니다.
...
게시 보기에는 도구 모음을 관리 보기로 전환하는 단추가 하나만 있습니다. 게시 보기는 [이전 번들의](/help/sites-deploying/configuring-osgi.md)publish.html 파일로 표현됩니다. HTML에서는 렌더링할 때 포틀릿으로 대체되는 다음 자리 표시자를 사용할 수 있습니다.
...
관리 보기에는 다음 네 개의 단추가 있습니다.편집, 웹 사이트 탭, 새로 고침 및 뒤로 관리 보기는 [이전 번들의](/help/sites-deploying/configuring-osgi.md)manage.html 파일로 표현됩니다. HTML에서는 렌더링할 때 포틀릿으로 대체되는 다음 자리 표시자를 사용할 수 있습니다.

#### 보기 자리 표시자 관리 {#manage-view-placeholders}

| 자리 표시자 문자열 | 설명 |
|---|---|
| {buttonEdit} | 자리 표시자는 AEM의 편집 모드에서 현재 페이지가 있는 새 창을 여는** 편집* 버튼으로 바뀝니다. |
...
1. Day Portal **Director CQ Server 구성** 구성을 선택하고 다음 값을 편집합니다.
   ...
   * **작성자는 게시로**&#x200B;사용됩니다.작성 인스턴스가 개발을 위한 게시 인스턴스로 사용됩니까?
...
AEM에서 컨텐츠가 요청되면 포틀릿은 현재 표시 모드(게시 또는 작성자)와 현재 경로를 사용하여 전체 URL을 어셈블합니다. 기본값이 있으면 첫 번째 url이 `http://localhost:4503/content/geometrixx/en.portlet.html`됩니다. 의 값은 확장 전에 URL에 자동으로 `htmlSelector` 추가됩니다.
