...
AEM에서 JSR 286 호환 포틀릿을 실행할 수 있습니다. 포틀릿 구성 요소를 사용하여 페이지에 포틀릿을 포함할 수 있습니다. 자세한 내용은 [AEM Content Portlet 관리](#administering-the-aem-content-portlet).
...
포털에 나타나도록 AEM 컨텐츠를 구성하려면 다음을 참조하십시오 [포틀릿에서 AEM 설치, 구성 및 사용](#installing-configuring-and-using-aem-in-a-portlet).
...
포털 서버가 호스트 localhost에서 실행된다고 가정하면 포트 8080과 AEM 포틀릿 웹 응용 프로그램이 웹 응용 프로그램 컨텍스트에 마운트됩니다 *cqportlet*, 웹 콘솔에 대한 url은 `http://localhost:8080/cqportlet/cqbridge/system/console`. 기본 사용자와 암호는 다음과 같습니다. **관리**.
...
1. 포털 서버가 호스트에서 실행된다고 가정함 **localhost****port 8080 **및 AEM 포틀릿 웹 애플리케이션이 컨텍스트에 마운트됩니다 **cqportlet**, 캐시를 플러시할 url은 `http://localhost:8080/cqportlet/cqbridge/cqpcache?Path=$(path)`. GET을 메서드로 사용합니다.
...
1. 클릭 **편집.**
1. 에서 **설정** 탭에서 을 선택합니다 **활성화됨** 확인란을 선택하고 **Dispatcher 플러시 **를 직렬화 유형으로 선택하고 다시 시도 시간 제한(예: 60000)을 입력합니다.
...
1. 기본적으로 http://localhost:4502/system/console/configMgr에 있는 OSGi 구성 콘솔에서 를 선택합니다. **CQ PIN 인증 핸들러** 를 클릭합니다.
...
게시 보기에는 도구 모음을 관리 보기로 전환하는 단추가 한 개만 있습니다. 게시 보기는 의 publish.html 파일로 표시됩니다. [이전 번들](/help/sites-deploying/configuring-osgi.md). HTML에서 다음 자리 표시자를 사용할 수 있습니다. 이 자리 표시자는 렌더링될 때 포틀릿에 해당 컨텐츠로 대체됩니다.
...
관리 보기에는 네 개의 버튼이 있습니다. 편집, 웹 사이트 탭, 새로 고침 및 뒤로. 관리 보기는 의 manage.html 파일로 표시됩니다 [이전 번들](/help/sites-deploying/configuring-osgi.md). HTML에서 다음 자리 표시자를 사용할 수 있습니다. 이 자리 표시자는 렌더링될 때 포틀릿에 해당 컨텐츠로 대체됩니다.
...
사용자 정의 레이아웃을 설치하려면 포틀릿의 OSGI 웹 콘솔에 액세스합니다 **번들** 섹션을 열고 번들을 업로드합니다.
...
1. 을(를) 선택하거나 선택을 취소합니다 **제목 표시줄 숨기기** 포틀릿의 제목 표시줄을 표시할지 여부에 따라 확인란을 선택합니다.
...
AEM에서 컨텐츠가 요청되면 포틀릿은 현재 표시 모드(게시 또는 작성자)와 현재 경로를 사용하여 전체 URL을 어셈블합니다. 기본값을 사용하여 첫 번째 url은 `http://localhost:4503/content/geometrixx/en.portlet.html`. 의 값 `htmlSelector` 는 확장 전 URL에 자동으로 추가됩니다.

포틀릿이 도움말 모드로 전환되고 `appendHelpViewModeAsSelector` 이 선택되면 `help` 선택기도 추가됩니다(예: ). `http://localhost:4503/content/geometrixx/en.portlet.html.help`. 포틀릿 창이 최대화되고 `appendMaxWindowStateAsSelector` 이 선택되면 선택기도 추가되며, 예를 들면 다음과 같습니다. `http://localhost:4503/content/geometrixx/en.portlet.max.help`.
