---
title: 일반 eCommerce 관리
seo-title: Administering Generic eCommerce
description: AEM 일반 솔루션은 리포지토리 내에 있는 상거래 정보를 관리하는 방법을 제공합니다.
seo-description: The AEM generic solution provides methods of managing the commerce information held within the repository.
uuid: 8af6933a-2dee-4b73-bc15-71b8394d082f
contentOwner: Guillaume Carlino
products: SG_EXPERIENCEMANAGER/6.4/SITES
topic-tags: e-commerce
content-type: reference
discoiquuid: ad80505f-116e-43f1-8d93-ffe6e8b1ac46
feature: Commerce Integration Framework
exl-id: 614815ef-6fe3-4b06-9c56-bc9fee127825
source-git-commit: bbc13d64a33d9033e04fb4f37d60bcfe223be337
workflow-type: tm+mt
source-wordcount: '2912'
ht-degree: 5%

---

# 일반 eCommerce 관리{#administering-generic-ecommerce}

AEM 일반 솔루션은 외부 전자 상거래 엔진을 사용하는 것과 대조적으로 저장소 내에 있는 상거래 정보를 관리하는 방법을 제공합니다. 여기에는 다음이 포함됩니다.

* [제품](/help/sites-administering/concepts.md#products)
* [제품 변형](/help/sites-administering/concepts.md#product-variants)

* [카탈로그](/help/sites-administering/concepts.md#catalogs)
* [프로모션](/help/sites-administering/concepts.md#promotions)
* [바우처](/help/sites-administering/concepts.md#vouchers)
* [주문](/help/sites-administering/concepts.md#shopping-cart-and-orders)
* [프록시 페이지](/help/sites-administering/concepts.md#proxy-pages)

>[!NOTE]
>
>표준 AEM 설치에는 일반 AEM(JCR) eCommerce 구현이 포함되어 있습니다.
>
>현재 데모 목적으로 사용하거나 요구 사항에 따라 사용자 지정 구현을 위한 기본 기반으로 사용됩니다.

## 제품 및 제품 변형 {#products-and-product-variations}

>[!NOTE]
>
>다음 절차는 제품 및 제품 변형 모두에 적용됩니다.

제품을 만들려면 먼저 [scaffold](/help/sites-authoring/scaffolding.md). 제품 및 제품 편집 방법을 정의하는 데 필요한 필드를 지정합니다.

개별 제품 유형에 대해 스캐폴드가 필요합니다. 다음 방법 중 하나로 해당 스캐폴드가 제품과 연결됩니다.

* 경로
* 제품이 스캐폴드를 참조할 수 있음

>[!NOTE]
>
>Geometrixx-Outdoors 스토어에는 단일 제품 유형(따라서 단일 스캐폴드)이 있습니다.
>
>`/etc/scaffolding/geometrixx-outdoors`
>
>Geometrixx-Outdoors 제품 유형이 활성화되어 있습니다.
>
>`/etc/commerce/products/geometrixx-outdoors`
>
>추가 설정 없이 해당 아래의 어디에서든 새 제품 정의를 만들 수 있습니다.

### 제품 가져오기 {#importing-products}

#### 제품 가져오기 - 터치에 적합한 UI {#importing-products-touch-optimized-ui}

1. 로 이동합니다 **제품** 콘솔, **상거래**.
1. 사용 **제품** 콘솔이 필요한 위치로 이동합니다.
1. 를 사용하십시오 **제품 가져오기** 아이콘을 클릭하여 마법사를 엽니다.

   ![](do-not-localize/chlimage_1-13.png)

1. 지정:

   * **가져오기**

      특정 세그먼트의 가져오기 [전자 상거래 업체](/help/sites-administering/concepts.md#commerce-providers)기본적으로 `Geometrixx`.

   * **소스**

      가져올 파일 브라우저를 사용하여 파일을 선택할 수 있습니다.

   * **증분 가져오기**

      증분 가져오기인지 여부를 나타냅니다(전체 가져오기와 반대).
   >[!NOTE]
   >
   >증분 가져오기(샘플 geometrixx-outdoors 가져오기)는 제품 수준에서 작동합니다.
   >
   >필요에 따라 사용자 지정 가져오기를 정의할 수 있습니다.

1. 선택 **다음** 제품을 가져오려면 수행한 작업 로그가 표시됩니다.

   >[!NOTE]
   >
   >제품을 현재 위치에 대해 또는 를 기준으로 가져옵니다.

   >[!NOTE]
   >
   >반복적으로 사용 **다음** 및 **뒤로** 은 제품 정의를 반복적으로 가져옵니다. 그러나 동일한 SKU가 있으므로 리포지토리에 있는 정보를 덮어씁니다.

1. 선택 **완료** 마법사를 닫으려면 다음을 수행하십시오.

#### 제품 가져오기 - 클래식 UI {#importing-products-classic-ui}

1. 사용 **도구** 콘솔 열기 **상거래** 폴더를 입력합니다.
1. 두 번 클릭하여 **제품 가져오기**:

   ![chlimage_1-54](assets/chlimage_1-54.jpeg)

1. 지정:

   * **저장소 이름**

      제품을 다음 위치로 가져옵니다.

      `/etc/commerce/products/<*store name*>/`

   * **상거래 공급자**

      사용자의 가져오기 [전자 상거래 업체](/help/sites-administering/concepts.md#commerce-providers); 기본 Geometrixx.

   * **소스 파일**

      가져올 파일의 저장소의 위치입니다.

   * **증분 가져오기**

      증분 가져오기인지 여부를 나타냅니다(전체 가져오기와 반대).

1. 클릭 **제품 가져오기**.

### 제품 정보 만들기 {#creating-product-information}

>[!NOTE]
>
>Geometrixx-Outdoors 제품 세트는 기본 상태로 유지되므로 표준 제품 관리는 기본입니다. 복잡성은 제품을 기반으로 합니다 [스캐폴딩](/help/sites-authoring/scaffolding.md)따라서 자체 제품 스캐폴딩을 사용하면 보다 정교한 편집을 수행할 수 있습니다.

#### 제품 정보 만들기 - 터치에 적합한 UI {#creating-product-information-touch-optimized-ui}

1. 사용 **제품** 콘솔(를 통해) **상거래**) 필요한 위치로 이동합니다.
1. 를 사용하십시오 **만들기** 아이콘 을 선택하여 구조 및 위치에 따라 다음 중 하나를 선택합니다.

   * **제품 만들기**
   * **제품 변형 만들기**

   ![](do-not-localize/chlimage_1-14.png)

1. 마법사가 열립니다. 를 사용하십시오 **기본** 및 **제품 탭** 을 입력합니다. [제품 속성](/help/sites-administering/concepts.md#product-attributes) 새 제품 또는 제품 변형에 대해 설명합니다.

   >[!NOTE]
   >
   >**제목** 및 **SKU** 는 제품 또는 변형을 만드는 데 필요한 최소값입니다.

1. 선택 **만들기** 정보를 저장합니다.

>[!NOTE]
>
>많은 상품들이 다양한 색상 및/또는 크기로 제공된다. 기본 제품 및 관련 제품 변형에 대한 정보는 모두 **제품** 콘솔.
>
>제품 및 그 변형은 트리 구조로 저장되고, 제품 정보는 맨 위에 있고, 변형은 아래에 있습니다(이 구조는 UI에 의해 적용됨).

### 제품 정보 편집 {#editing-product-information}

>[!NOTE]
>
>geometrixx-outdoors의 제품 이미지는 다음 제품에서 제공됩니다.
>
>`/etc/commerce/products/...`
>
>즉, 기본적으로 [dispatcher](https://helpx.adobe.com/kr/experience-manager/dispatcher/using/dispatcher-configuration.html), 필요에 따라 구성합니다.

#### 제품 정보 편집 - 터치에 적합한 UI {#editing-product-information-touch-optimized-ui}

1. 사용 **제품** 콘솔(를 통해) **상거래**) 제품 정보로 이동합니다.
1. 다음 중 하나 사용:

   * [빠른 작업](/help/sites-authoring/basic-handling.md#quick-actions)
   * [선택 모드](/help/sites-authoring/basic-handling.md#viewing-and-selecting-resources)

   을(를) 선택합니다 **제품 데이터 보기** 아이콘:

   ![](do-not-localize/chlimage_1-15.png)

1. 다음 [제품 속성](/help/sites-administering/concepts.md#product-attributes) 이 표시됩니다. 사용 **편집** 및 **완료** 을 눌러 변경합니다.

### 제품 참조 표시 {#showing-product-references}

#### 제품 참조 표시 - 터치에 적합한 UI {#showing-product-references-touch-optimized-ui}

1. 사용 **제품** 콘솔(를 통해) **상거래**) 제품 정보로 이동합니다.
1. 아이콘을 사용하여 참조에 대한 보조 레일을 엽니다.

   ![](do-not-localize/chlimage_1-16.png)

1. 필요한 제품을 선택합니다. 보조 레일이 업데이트되어 사용 가능한 참조 유형이 표시됩니다.

   ![chlimage_1-326](assets/chlimage_1-326.png)

1. 참조 유형(예: 제품 페이지)을 클릭/탭하여 목록을 확장합니다.
1. 특정 참조를 선택하여 옵션을 표시합니다.

   * 제품 페이지로 이동
   * 제품 페이지 편집

   ![chlimage_1-327](assets/chlimage_1-327.png)

### 제품 검색 {#search-for-products}

1. 로 이동합니다 **제품** 콘솔, **상거래**.
1. 아이콘을 사용하여 검색에 대한 보조 레일을 엽니다.

   ![](do-not-localize/chlimage_1-17.png)

1. 제품을 검색할 수 있는 몇 가지 패싯을 사용할 수 있습니다. 검색에 하나 또는 여러 패싯만 사용할 수 있습니다. 찾은 제품이 나타납니다.

   ![chlimage_1-328](assets/chlimage_1-328.png)

1. 제품을 클릭/탭하면 해당 제품이 열립니다. 게시하거나 제품 데이터를 볼 수도 있습니다.

#### 검색 확장 {#extending-search}

CRXDE Lite을 사용하여 기존 패싯을 수정하거나 새 패싯을 추가할 수 있습니다.

1. 다음으로 이동합니다.

   `http://localhost:4502/crx/de/index.jsp#/libs/commerce/gui/content/products/aside/items/search/items/searchpanel/facets`

1. 제품 검색 페이지에 표시되는 크기 등을 수정할 수 있습니다. 을(를) 클릭합니다. `sizegroup` 노드 아래에 있어야 합니다.
1. 클릭 `items` node 를 클릭한 다음 `propertypredicate` 노드 아래에 있어야 합니다.
1. 을 수정할 수 있습니다 `propertyValues`. 예를 들어 XS 또는 XXL을 추가하거나 크기를 제거할 수 있습니다.
1. 클릭 **모두 저장** 제품 검색 페이지로 이동합니다. 변경 사항이 나타납니다.

### 여러 자산 {#multiple-assets}

제품 구성 요소에 여러 자산을 추가한 다음 제품 페이지에 표시할 자산을 지정할 수 있습니다.

>[!NOTE]
>
>여러 자산과 관련된 모든 작업은 터치에 적합한 UI로 수행됩니다.

#### 여러 자산 추가 {#adding-multiple-assets}

1. 로 이동합니다 **제품** 콘솔, **상거래**.
1. 사용 **제품** 콘솔에서 필요한 제품으로 이동합니다.

   >[!NOTE]
   >
   >변형 수준이 아니라 제품 수준이어야 합니다.

1. 탭/클릭 **제품 데이터 보기** 아이콘(선택 모드 또는 빠른 작업 포함)
1. 편집 아이콘을 탭/클릭합니다.
1. 로 스크롤합니다. **추가**.

   ![chlimage_1-329](assets/chlimage_1-329.png)

1. 탭/클릭 **추가**. 새 자산 자리 표시자가 나타납니다.
1. **변경 **을 탭/클릭하면 자산을 선택할 수 있는 대화 상자가 열립니다.
1. 추가할 자산을 선택합니다.

   >[!NOTE]
   >
   >선택할 수 있는 자산은 다음과 같습니다 [자산](https://helpx.adobe.com/experience-manager/aem-previous-versions.html#assets).

1. 완료 아이콘을 탭/클릭합니다.

이제 두 개의 자산이 제품 구성 요소에 저장됩니다. 제품 페이지에 표시할 항목을 구성할 수 있습니다. 카테고리 시스템에서 작동합니다. 먼저 개별 자산에 카테고리를 추가해야 합니다.

1. 탭/클릭 **제품 데이터 보기**.
1. 을(를) 입력합니다 **자산 카테고리** 예를 들어, 자산 아래에 `cat1` 및 `cat2`.

   >[!NOTE]
   >
   >카테고리에 태그를 사용할 수도 있습니다.

1. 완료 아이콘을 탭/클릭합니다. 이제 다음을 수행해야 합니다 [롤아웃](#rolling-out-a-catalog) 변경 사항

이제 제품 구성 요소의 자산에 카테고리가 있습니다. 다음 세 가지 서로 다른 수준에서 표시될 카테고리를 구성할 수 있습니다.

* [제품 페이지](#product-page)
* [카탈로그](#catalog)
* [제품 콘솔](#products-console)

>[!NOTE]
>
>카테고리를 설정하지 않으면 첫 번째 자산이 제품 페이지에 표시됩니다.

표시할 이미지를 선택하는 메커니즘은 다음과 같습니다.

1. 제품 페이지에 대해 카테고리가 설정되어 있는지 확인합니다.
1. 없는 경우, 카탈로그에 대해 카테고리가 설정되어 있는지 확인합니다.
1. 그렇지 않은 경우 제품 콘솔에 대해 카테고리가 설정되어 있는지 확인합니다.

>[!NOTE]
>
>카탈로그 수준과 제품 콘솔 수준 모두의 경우 변경 사항을 롤아웃하여 수정 사항을 적용하고 제품 페이지에서 차이점을 확인해야 합니다.

#### 제품 페이지 {#product-page}

1. 제품 페이지로 이동합니다.
1. **편집** 제품 구성 요소입니다.
1. 을(를) 입력합니다 **이미지 카테고리** 선택한 ( `cat1` 예).
1. 탭/클릭 **완료**. 페이지가 새로 고쳐지고 올바른 자산이 표시됩니다.

#### 카탈로그  {#catalog}

1. 카탈로그로 이동합니다.
1. 탭/클릭 **속성 보기**.
1. **편집**&#x200B;을 탭/클릭합니다.
1. 을 탭/클릭합니다. **자산** 탭.
1. 필요한 을(를) 입력합니다 **제품 자산 카테고리**.
1. 탭/클릭 **완료**.
1. [롤아웃](#rolling-out-a-catalog) 변경 사항

#### 제품 콘솔 {#products-console}

1. 사용 **제품** 콘솔에서 필요한 제품으로 이동합니다.
1. 탭/클릭 **제품 데이터 보기**.
1. **편집**&#x200B;을 탭/클릭합니다.
1. 입력 **기본 자산 카테고리**.
1. 탭/클릭 **완료**.
1. [롤아웃](#rolling-out-a-catalog) 변경 사항

### 제품 정보 게시/게시 취소 {#publishing-unpublishing-product-information}

#### 제품 정보 게시/게시 취소 - 터치에 적합한 UI {#publishing-unpublishing-product-information-touch-optimized-ui}

>[!NOTE]
>
>제품 정보는 종종 참조하는 페이지를 통해 게시됩니다. 예를 들어 제품 Y를 참조하는 페이지 X를 게시하면 AEM에서 제품 Y를 게시할지 묻는 메시지를 표시합니다.
>  
>AEM에서는 제품 데이터에서 직접 게시가 지원됩니다.

1. 사용 **제품** 콘솔(를 통해) **상거래**) 제품 정보로 이동합니다.
1. 다음 중 하나 사용:

   * [빠른 작업](/help/sites-authoring/basic-handling.md#quick-actions)
   * [선택 모드](/help/sites-authoring/basic-handling.md#viewing-and-selecting-resources)

   을(를) 선택합니다 **게시** 또는 **게시 취소** 필요한 경우

   ![](do-not-localize/chlimage_1-18.png) ![](do-not-localize/chlimage_1-19.png)

   제품 정보가 적절히 게시되거나 게시 취소됩니다.

<!-- Search&Promote was end of life September 1, 2022. ### Product Feed {#product-feed} -->

<!-- Search&Promote was end of life September 1, 2022. The Search&Promote integration allows you to: -->

<!-- Search&Promote was end of life September 1, 2022. * use the eCommerce API, independently of the underlying repository structure and commerce platform. -->
<!-- Search&Promote was end of life September 1, 2022. * leverage the Index Connector feature of Search&Promote to provide a product feed in XML format. -->
<!-- Search&Promote was end of life September 1, 2022. * leverage the Remote Control feature of Search&Promote to perform on-demand or scheduled requests of the product feed -->
<!-- Search&Promote was end of life September 1, 2022. * feed generation for different Search&Promote accounts, configured as cloud services configurations. -->

<!-- Search&Promote was end of life September 1, 2022. For more information, read [Product Feed](/help/sites-administering/product-feed.md). -->

### 제품 업데이트에 대한 이벤트 처리기 {#event-handler-for-product-updates}

제품 추가, 수정 또는 삭제 시 및 제품 페이지가 추가, 수정 또는 삭제될 때 이벤트를 기록하는 이벤트 처리기가 있습니다. 다음과 같은 OSGi 이벤트가 있습니다.

* `com/adobe/cq/commerce/pim/PRODUCT_ADDED`
* `com/adobe/cq/commerce/pim/PRODUCT_MODIFIED`
* `com/adobe/cq/commerce/pim/PRODUCT_DELETED`
* `com/adobe/cq/commerce/pim/PRODUCT_PAGE_ADDED`
* `com/adobe/cq/commerce/pim/PRODUCT_PAGE_MODIFIED`
* `com/adobe/cq/commerce/pim/PRODUCT_PAGE_DELETED`

대상 `PRODUCT_*` events에서 경로는 기본 제품을 가리킵니다. `/etc/commerce/products`. 대상 `PRODUCT_PAGE_*` events에서 경로는 `cq:Page` 노드 아래에 있어야 합니다.

OSGI 이벤트( `/system/console/events`) 내의 예를 들면 다음과 같습니다.

![](do-not-localize/chlimage_1-20.png)

>[!NOTE]
>
>읽기 [AEM의 이벤트 처리](https://blogs.adobe.com/experiencedelivers/experience-management/event_handling_incq/). [](https://blogs.adobe.com/experiencedelivers/experience-management/event_handling_incq/)

### 장바구니에 추가 링크가 있는 이미지 {#image-with-add-to-cart-links}

장바구니에 추가 링크 구성 요소를 사용하는 이미지 를 사용하면 이미지에 연결된 핫스팟을 만들어 장바구니에 제품을 빠르게 추가할 수 있습니다.

핫스팟을 클릭하면 제품의 크기와 수량을 선택할 수 있는 대화 상자가 열립니다.

1. 구성 요소를 추가할 페이지로 이동합니다.
1. 페이지에서 구성 요소를 드래그하여 놓습니다.
1. 구성 요소에서 이미지를 끌어서 놓습니다 [자산 브라우저](/help/sites-authoring/author-environment-tools.md#assets-browser).
1. 다음과 같은 작업을 수행할 수 있습니다.

   * 구성 요소를 클릭한 다음 편집 아이콘을 클릭합니다
   * 느리게 두 번 클릭

1. 전체 화면 아이콘을 클릭합니다.

   ![chlimage_1-330](assets/chlimage_1-330.png)

1. 맵 시작 아이콘을 클릭합니다.

   ![chlimage_1-331](assets/chlimage_1-331.png)

1. 모양 아이콘 중 하나를 클릭합니다.

   ![](do-not-localize/chlimage_1-21.png)

1. 필요에 따라 셰이프를 수정하고 이동합니다.
1. 셰이프를 클릭합니다.
1. 찾아보기 아이콘을 클릭하면 [자산 선택기](/help/assets/asset-selector.md#using-the-asset-selector).

   >[!NOTE]
   >
   >또는 변형 수준이 아니라 제품 수준에 있어야 하는 제품 경로를 직접 입력할 수 있습니다.

   ![chlimage_1-332](assets/chlimage_1-332.png)

1. 확인 아이콘을 두 번 클릭한 다음 전체 화면 종료를 클릭합니다.
1. 구성 요소 옆에 있는 페이지의 아무 곳이나 클릭합니다. 페이지가 새로 고쳐지고 이미지에 다음 기호가 표시됩니다.

   ![](do-not-localize/chlimage_1-22.png)

1. 다음으로 전환 [미리 보기](/help/sites-authoring/editing-content.md#previewing-pages) 모드.
1. + 핫스팟을 클릭합니다. 입력한 제품의 크기와 수량을 선택할 수 있는 대화 상자가 열립니다 **경로**.

   ![chlimage_1-333](assets/chlimage_1-333.png)

1. 크기와 수량을 입력합니다.
1. 장바구니에 추가 단추를 클릭합니다. 대화 상자가 닫힙니다.
1. 장바구니로 이동합니다. 제품이 여기에 있어야 합니다.

#### 구성 옵션 {#configuration-options}

핫스팟을 클릭하면 대화 상자가 표시되는 방식을 구성할 수 있습니다.

1. 구성 요소를 클릭하고 구성 아이콘을 클릭합니다.

   ![chlimage_1-334](assets/chlimage_1-334.png)

1. 아래로 스크롤. 다음 항목이 있습니다 **장바구니에 추가** 탭.

   ![chlimage_1-335](assets/chlimage_1-335.png)

1. 클릭 **장바구니에 추가**. 사용할 수 있는 구성 옵션은 3가지가 있습니다.

   ![chlimage_1-336](assets/chlimage_1-336.png)

1. 완료 아이콘을 클릭합니다.

## 카탈로그 {#catalogs}

### 카탈로그 생성 {#generating-a-catalog}

#### 카탈로그 생성 - 터치에 적합한 UI {#generating-a-catalog-touch-optimized-ui}

>[!NOTE]
>
>카탈로그가 제품 데이터를 참조합니다.

카탈로그를 생성하려면

1. [사이트] 콘솔을 엽니다(예: [http://localhost:4502/sites.html/content](http://localhost:4502/sites.html/content)).
1. 새 페이지를 만들 위치로 이동합니다.
1. 옵션 목록을 열려면 **만들기** 아이콘을 사용합니다.

   ![](do-not-localize/chlimage_1-23.png)

1. 목록에서 을(를) 선택합니다 **카탈로그 만들기**&#x200B;을 입력하면 카탈로그 만들기 마법사가 열립니다.

   ![chlimage_1-337](assets/chlimage_1-337.png)

1. 필요한 카탈로그 블루프린트로 이동합니다.
1. 탭/클릭 **선택** 필요한 카탈로그 블루프린트를 버튼 및 탭/클릭합니다.
1. 탭/클릭 **다음**.

   ![chlimage_1-338](assets/chlimage_1-338.png)

1. 입력 **제목** 그리고 **이름**.
1. 을 탭/클릭합니다. **만들기** 버튼을 클릭합니다. 카탈로그가 생성되고 대화 상자가 열립니다.

   ![chlimage_1-339](assets/chlimage_1-339.png)

1. 탭/클릭 **완료** 단추를 클릭하면 카탈로그를 볼 수 있는 사이트 콘솔로 돌아갑니다.

   탭/클릭 **카탈로그 열기** 버튼이 카탈로그를 엽니다(예: `http://localhost:4502/editor.html/content/test-catalog.html`).

#### 카탈로그 생성 - 클래식 UI {#generating-a-catalog-classic-ui}

>[!NOTE]
>
>카탈로그에서 [제품 데이터](#editing-product-information).

1. 사용 **웹 사이트** 콘솔, **카탈로그 블루프린트**, 기본 카탈로그 순으로 선택합니다.

   예:

   `http://localhost:4502/siteadmin#/content/catalogs/geometrixx-outdoors/base-catalog`

1. 를 사용하여 새 페이지 만들기 **섹션 블루프린트** 템플릿.

   (예: `Swimwear`)

1. 새 `Swimwear` 페이지를 클릭한 다음 **블루프린트 편집** 열다 **속성** 대화 상자에서 다음을 설정할 수 있습니다. **제품** 선택.

   예를 들어 **태그/키워드** 필드에서 활동을 선택한 다음 Geometrixx-Outdoors 섹션에서 수영을 선택합니다.

1. 클릭 **확인** 속성을 저장하려면 제품 예는 **제품 선택 기준** 블루프린트 페이지에서 을 참조하십시오.
1. 클릭 **변경 내용 롤아웃..**, 선택 **페이지 및 모든 하위 페이지 롤아웃**&#x200B;를 클릭한 다음 **다음** 그런 다음 **롤아웃**. 롤아웃이 성공적으로 완료되면 **상태** 표시기가 녹색으로 표시됩니다.
1. 이제 을(를) 클릭합니다. **닫기** 새 카탈로그 섹션을 확인합니다. 예: on 및 under:

   `http://localhost:4502/cf#/content/geometrixx-outdoors/en/swimwear.html`

1. 블루프린트 페이지에서 다시 클릭하여 **블루프린트 편집** 그리고 **속성** 대화 상자 열기 **생성된 페이지** 탭. 배너 목록 필드에서 표시할 이미지를 선택합니다. 예 `summer.jpg`
1. 클릭 **확인** 속성을 저장하려면 배너 정보는 **제품 선택 기준** 블루프린트 페이지에서 을 참조하십시오.
1. 이러한 새 변경 사항을 롤아웃합니다.

### 카탈로그 롤아웃 {#rolling-out-a-catalog}

#### 카탈로그 롤아웃 - 터치에 적합한 UI {#rolling-out-a-catalog-touch-optimized-ui}

카탈로그를 롤아웃하려면

1. 로 이동합니다 **카탈로그** 콘솔, **상거래**.
1. 롤아웃할 카탈로그로 이동합니다.
1. 다음 중 하나 사용:

   * [빠른 작업](/help/sites-authoring/basic-handling.md#quick-actions)
   * [선택 모드](/help/sites-authoring/basic-handling.md#viewing-and-selecting-resources)

   을(를) 선택합니다 **롤아웃 변경 사항** 아이콘:

   ![](do-not-localize/chlimage_1-24.png)

1. 마법사에서 필요에 따라 롤아웃을 설정한 다음 탭/클릭합니다 **롤아웃 변경 사항**.
1. 대화 상자가 열립니다. 프로세스가 완료되면 **완료 **를 탭/클릭합니다.

#### 카탈로그 롤아웃 - 클래식 UI {#rolling-out-a-catalog-classic-ui}

카탈로그를 롤아웃하려면

1. 롤아웃할 카탈로그로 이동합니다. 예:

   `http://localhost:4502/cf#/content/catalogs/geometrixx-outdoors/base-catalog.html`

1. 클릭 **변경 내용 롤아웃..**
1. 필요에 따라 롤아웃을 설정합니다.
1. 클릭 **롤아웃**.

### 블루프린트 가져오기 {#blueprint-importer}

#### 블루프린트 가져오기 - 터치에 적합한 UI {#blueprint-importer-touch-optimized-ui}

1. 로 이동합니다 **카탈로그** 콘솔, **상거래**.
1. 카탈로그 블루프린트를 가져올 위치로 이동합니다.
1. 을 탭/클릭합니다. **블루프린트 가져오기** 아이콘.

   ![](do-not-localize/chlimage_1-25.png)

1. 마법사에서 필요에 따라 소스를 선택하고 탭/클릭합니다 **다음**.

   ![chlimage_1-340](assets/chlimage_1-340.png)

1. 탭/클릭 **완료** 가져오기가 완료되면

#### 블루프린트 가져오기 - 클래식 UI {#blueprint-importer-classic-ui}

1. 사용 **도구** 콘솔, 탐색 **상거래**.

   예:

   `http://localhost:4502/miscadmin#/etc/commerce`

1. 를 엽니다. **카탈로그 블루프린트 가져오기**.
1. 필요에 따라 가져오기를 설정합니다.
1. 클릭 **카탈로그 블루프린트 가져오기**.

## 프로모션 {#promotions}

### 프로모션 만들기 {#creating-a-promotion}

#### 프로모션 만들기 - 클래식 UI {#creating-a-promotion-classic-ui}

>[!NOTE]
>
>다음 예에서는 [campaign](/help/sites-authoring/personalization.md): 바우처에 사용됩니다.
>
>프로모션은 [경험](/help/sites-authoring/personalization.md) 캠페인 내.
>
>자세한 내용은 [프로모션 및 바우처](#promotions).

1. 를 엽니다. **웹 사이트** 작성자 인스턴스의 콘솔.
1. 왼쪽 창에서 필요한 을(를) 선택합니다 **캠페인**.
1. 클릭 **새로 만들기**&#x200B;에서 을(를) 선택합니다. **프로모션** 템플릿을 만든 다음 **제목** (및 **이름** 필요한 경우) 새 바우처에 사용할 수 있습니다.
1. **만들기**&#x200B;를 클릭합니다. 오른쪽 창에 새 프로모션 페이지가 표시됩니다.

1. 편집 **속성** 다음 중 하나를 수행합니다.

   * 페이지를 열고 편집 단추를 클릭하여 속성 대화 상자를 엽니다.
   * 웹 사이트 콘솔에서 페이지를 선택한 다음 컨텍스트 메뉴(일반적으로 오른쪽 마우스 단추)를 사용하여 선택합니다 **속성...** 속성 대화 상자를 엽니다.

   을(를) 지정합니다. **프로모션 유형**, **할인 유형**, **할인 값** 및 기타 모든 필드를 필요에 따라 구성합니다.

1. **확인**&#x200B;을 클릭하여 저장합니다.

1. 이제 프로모션을 활성화할 수 있으므로, 구매자가 게시 인스턴스에서 판촉을 볼 수 있습니다.

## 바우처 {#vouchers}

### 바우처 만들기 {#creating-a-voucher}

#### 바우처 만들기 - 클래식 UI {#creating-a-voucher-classic-ui}

1. 를 엽니다. **웹 사이트** 작성자 인스턴스의 콘솔.
1. 왼쪽 창에서 필요한 을(를) 선택합니다 **캠페인**.
1. 클릭 **새로 만들기**&#x200B;에서 을(를) 선택합니다. **바우처** 템플릿을 만든 다음 **제목** (및 **이름** 필요한 경우) 새 바우처에 사용할 수 있습니다.
1. **만들기**&#x200B;를 클릭합니다. 새 바우처 페이지가 오른쪽 창에 표시됩니다.

1. 두 번 클릭하여 새 바우처 페이지를 열고 을 클릭합니다 **편집** 필요에 따라 정보를 구성합니다.
1. **확인**&#x200B;을 클릭하여 저장합니다.

1. 이제 바우처를 활성화할 수 있으므로 쇼핑객이 게시 인스턴스의 장바구니에서 사용할 수 있습니다.

### 바우처 제거 {#removing-vouchers}

#### 바우처 제거 - 클래식 UI {#removing-vouchers-classic-ui}

고객이 바우처를 사용할 수 없도록 하려면 다음 중 하나를 수행합니다.

* 바우처 비활성화 - 나중에 다시 활성화할 수 있도록 작성 환경에서 사용 가능한 상태로 유지됩니다.
* 완전히 삭제하십시오.

두 작업은 모두 **웹 사이트** 콘솔.

### 바우처 수정 {#modifying-vouchers}

#### 바우처 수정 - 클래식 UI {#modifying-vouchers-classic-ui}

바우처 또는 프로모션의 속성을 변경하려면 **웹 사이트** 콘솔을 클릭하고 **편집**. 저장한 후 변경 사항이 게시 인스턴스에 푸시되도록 활성화해야 합니다.

### 장바구니에 바우처 추가 {#adding-vouchers-to-a-cart}

사용자가 장바구니에 바우처를 추가할 수 있도록 하기 위해 내장된 을 사용할 수 있습니다 **바우처** 구성 요소(상거래 카테고리). 장바구니가 표시되는 페이지와 동일한 페이지에 추가해야 합니다(하지만 필수는 아님). 바우처 구성 요소는 사용자가 바우처 코드를 입력할 수 있는 양식일 뿐입니다. 이는 실제로 적용된 바우처의 목록과 해당 할인을 보여주는 장바구니 구성 요소입니다.

데모 사이트(Geometrixx Outdoors - 영어)에서는 실제 장바구니 아래의 장바구니 페이지에서 바우처 양식을 볼 수 있습니다.

## 주문 {#orders}

>[!NOTE]
>
>즉시 사용 가능한 AEM에는 반품, 주문 상태 업데이트, 이행 수행, 포장 명세서 생성과 같은 주문과 관련된 표준 기능에 필요한 작업이 없습니다. 주로 기술 미리 보기로 사용됩니다.
>
>AEM의 일반 주문 관리는 기본 상태를 유지합니다. 마법사에서 사용할 수 있는 필드는 스캐폴드에 따라 다릅니다.\
>`/etc/scaffolding/geometrixx-outdoors/order/jcr:content/cq:dialog`
>
>사용자 지정된 스캐폴드를 생성하는 경우 주문 정보를 더 저장할 수 있습니다.

>[!NOTE]
>
>주문 콘솔은 게시되지 않은 공급업체 주문 정보를 표시합니다.
>  
>고객 주문 정보는 해당 홈 디렉토리에 저장되며 해당 계정의 주문 내역에 의해 노출됩니다. 이 정보는 홈 디렉토리의 나머지 디렉토리와 함께 게시됩니다.

### 주문 정보 생성 {#creating-order-information}

#### 주문 정보 만들기 - 터치에 적합한 UI {#creating-order-information-touch-optimized-ui}

1. 사용 **주문** 콘솔이 필요한 위치로 이동합니다.
1. 를 사용하십시오 **만들기** 아이콘을 선택합니다. **주문 만들기**.

   ![](do-not-localize/chlimage_1-26.png)

1. 마법사가 열립니다. 를 사용하십시오 **기본**, **컨텐츠**, **결제** 및 **이행** 탭을 입력하여 [새 주문에 대한 정보](/help/sites-administering/concepts.md#order-information).

1. 선택 **만들기** 정보를 저장합니다.

### 주문 정보 편집 {#editing-order-information}

#### 주문 정보 편집 - 터치에 적합한 UI {#editing-order-information-touch-optimized-ui}

1. 사용 **주문** 콘솔이 순서대로 이동합니다.
1. 다음 중 하나 사용:

   * [빠른 작업](/help/sites-authoring/basic-handling.md#quick-actions)
   * [선택 모드](/help/sites-authoring/basic-handling.md#viewing-and-selecting-resources)

   을(를) 선택합니다 **주문 데이터 보기** 아이콘:

   ![](do-not-localize/chlimage_1-27.png)

1. 다음 [주문 정보](/help/sites-administering/concepts.md#order-information) 이 표시됩니다. 사용 **편집** 및 **완료** 을 눌러 변경합니다.
