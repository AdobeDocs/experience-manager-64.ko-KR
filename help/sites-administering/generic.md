---
title: 일반 전자 상거래 관리
seo-title: 일반 전자 상거래 관리
description: AEM 일반 솔루션은 저장소 내에 있는 상거래 정보를 관리하는 방법을 제공합니다.
seo-description: AEM 일반 솔루션은 저장소 내에 있는 상거래 정보를 관리하는 방법을 제공합니다.
...
AEM 일반 솔루션은 외부 전자 상거래 엔진을 사용하는 대신 저장소 내에 있는 상거래 정보를 관리하는 방법을 제공합니다. 여기에는 다음이 포함됩니다.
...
제품을 만들기 전에 [스캐폴드를](/help/sites-authoring/scaffolding.md)정의해야 합니다. 이렇게 하면 제품을 정의하는 데 필요한 필드와 제품 편집 방법이 지정됩니다.

Scaffold는 각 개별 제품 유형에 필요합니다. 적절한 스캐폴드는 다음 방법 중 하나로 제품과 연결됩니다.

* 경로
* Scaffold를 참조할 수 있는 제품

>[!NOTE]
>
>Geometrixx-Outdoors 스토어에는 단일 제품 유형이 있으므로 단일 스캐폴드가 있습니다.
>
...
1. 지정:
   ...
   * **가져오기**
      ...
      기본적으로 특정 [상거래 공급자에](/help/sites-administering/concepts.md#commerce-providers)대한 가져오기 도구입니다 `Geometrixx`.
   ...
   * **증분 가져오기**
      ...
      전체 가져오기가 아닌 증분 가져오기인지 여부를 나타냅니다.
   >[!NOTE]
   >
   >증분 가져오기(샘플 geometrixx-outdoor 가져오기 도구)는 제품 수준에서 작동합니다.
   >
   >필요에 따라 작동하도록 사용자 지정된 가져오기를 정의할 수 있습니다.

1. 다음을 **선택하여** 제품을 가져오면 수행한 작업의 로그가 표시됩니다.

   >[!NOTE]
   >
   >제품은 현재 위치로 또는 현재 위치에 상대적인 것으로 가져옵니다.

   >[!NOTE]
   >
   >다음 **및** 뒤로를 반복적으로 **** 사용하면 제품 정의를 반복적으로 가져올 수 있습니다. 그러나 동일한 SKU가 있기 때문에 저장소에 존재하는 정보는 덮어쓰여집니다.
...
1. 지정:
   ...
   * **상거래 공급자**
      ...
      상거래 [제공업체를](/help/sites-administering/concepts.md#commerce-providers)위한 가져오기 도구기본 Geometrixx로 설정되어 있습니다.
...
>[!NOTE]
>
>Geometrixx-Outdoors 제품 세트는 기본적으로 유지되므로 표준 제품 관리는 기본적입니다. 제품 Scaffolding을 기반으로 [](/help/sites-authoring/scaffolding.md)하기 때문에 고유한 제품 Scaffolding을 사용하면 보다 정교한 편집 작업을 수행할 수 있습니다.

#### 제품 정보 만들기 - 터치에 적합한 UI {#creating-product-information-touch-optimized-ui}

1. 제품 **콘솔** (상거래를 통해 **)을 사용하여 필요한**&#x200B;위치로 이동합니다.
...
1. 마법사가 열립니다. 기본 **및** 제품 **탭을** 사용하여 [새 제품 또는 제품 변형에 대한](/help/sites-administering/concepts.md#product-attributes) 제품 속성을입력합니다.

   >[!NOTE]
   >
   >**제목** 및 **SKU는** 제품 또는 변형을 만드는 데 필요한 최소 요구 사항입니다.

1. 만들기를 **선택하여** 정보를 저장합니다.

>[!NOTE]
>
>다양한 색상 및/또는 크기로 제공되는 제품이 많습니다. 기본 제품 및 관련 제품 변형에 대한 정보는 모두 제품 **콘솔에서 관리할 수** 있습니다.
>
>제품 및 해당 변형은 트리 구조로 저장되며 제품 정보는 맨 위에 있고 변형은 아래에 있습니다(이 구조는 UI에 의해 적용됨).
...
>
>즉, 기본적으로 [디스패처에](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html)의해 차단되므로 필요에 따라 구성합니다.

#### 제품 정보 편집 - 터치에 적합한 UI {#editing-product-information-touch-optimized-ui}

1. 제품 **콘솔** (상거래를 통해 **)을 사용하여 제품**&#x200B;정보로 이동합니다.
1. 다음 중 하나 사용:
...
1. 제품 속성이 [](/help/sites-administering/concepts.md#product-attributes) 표시됩니다. 편집 **및** 완료를 **사용하여** 변경할 수 있습니다.
...
1. 여러 가지 패싯을 사용하여 제품을 검색할 수 있습니다. 검색에 하나 또는 여러 패싯만 사용할 수 있습니다. 찾은 제품이 나타납니다.

   ![chlimage_1-328](assets/chlimage_1-328.png)

1. 제품을 클릭/탭하면 열립니다. 게시하거나 제품 데이터를 볼 수도 있습니다.

#### 검색 확장 {#extending-search}

CRXDE Lite를 사용하여 기존 패싯을 수정하거나 새 패싯을 추가할 수 있습니다.
...
1. 를 수정할 수 `propertyValues`있습니다. 예를 들어 XS 또는 XXL을 추가하거나 크기를 제거할 수 있습니다.
...
1. 추가할 자산을 선택합니다.
   ...
   >[!NOTE]
   >
   >선택할 수 있는 자산은 자산에서 [가져옵니다](https://helpx.adobe.com/experience-manager/aem-previous-versions.html#assets).

1. 완료 아이콘을 탭/클릭합니다.

이제 두 개의 에셋이 제품 구성 요소에 저장됩니다. 제품 페이지에 표시할 항목을 구성할 수 있습니다. 이는 카테고리 시스템에서 작동합니다. 먼저 개별 자산에 카테고리를 추가해야 합니다.

1. 제품 데이터 보기를 탭/ **클릭합니다**.
1. 자산 **아래에 자산 카테고리를** 입력합니다(예: `cat1` 및 `cat2`).
...
이제 제품 구성 요소의 자산에 카테고리가 있습니다. 다음 세 가지 수준으로 표시할 카테고리를 구성할 수 있습니다.
...
>[!NOTE]
>
>카탈로그 수준 및 제품 콘솔 수준의 경우 변경 내용을 적용하여 수정 내용을 적용하고 제품 페이지의 차이점을 확인해야 합니다.

#### 제품 페이지 {#product-page}

1. 제품 페이지로 이동합니다.
1. **제품 구성 요소를 편집합니다** .
1. 선택한 **이미지 카테고리를** 입력합니다( `cat1` 예).
...
1. 완료를 탭/ **클릭합니다**.
1. [변경 내용을 롤아웃합니다](#rolling-out-a-catalog) .
...
>[!NOTE]
>
>제품 정보는 종종 참조하는 페이지를 통해 게시됩니다. 예를 들어 제품 Y를 참조하는 페이지 X를 게시하면 AEM에서 제품 Y를 게시할지 여부를 묻습니다.
>  
>특별한 경우 AEM은 제품 데이터에서 직접 게시를 지원합니다.
...
제품이 추가, 수정 또는 삭제되거나 제품 페이지가 추가, 수정 또는 삭제될 때 이벤트를 기록하는 이벤트 처리기가 있습니다. 다음과 같은 OSGi 이벤트가 있습니다.
...
이벤트의 경우 `PRODUCT_*` 경로는 기본 제품을 가리킵니다 `/etc/commerce/products`. 이벤트의 경우 `PRODUCT_PAGE_*` 경로는 `cq:Page` 노드를 가리킵니다.
...
장바구니에 추가 링크 구성 요소를 사용하면 이미지에서 제품과 연결된 핫스팟을 만들어 장바구니에 제품을 빠르게 추가할 수 있습니다.

핫스팟을 클릭하면 제품의 크기와 수량을 선택할 수 있는 대화 상자가 열립니다.

1. 구성 요소를 추가할 페이지로 이동합니다.
1. 페이지에서 구성 요소를 드래그하여 놓습니다.
1. 구성 요소의 이미지를 [자산 브라우저에서](/help/sites-authoring/author-environment-tools.md#assets-browser)드래그하여 놓습니다.
1. 다음을 수행할 수 있습니다.
...
1. 모양을 클릭합니다.
1. 찾아보기 아이콘을 클릭하면 자산 [선택기가](/help/assets/asset-selector.md#using-the-asset-selector)열립니다.
...
1. + 핫스팟을 클릭합니다. 경로에서 입력한 제품의 크기와 수량을 선택할 수 있는 대화 상자가 **열립니다**.
...
>[!NOTE]
>
>카탈로그는 제품 데이터를 [참조합니다](#editing-product-information).

1. 웹 사이트 **콘솔을** 사용하여 카탈로그 **블루프린트로 이동한**&#x200B;다음 기본 카탈로그로 이동합니다.
...
1. 새 `Swimwear` 페이지를 연 다음 **블루프린트 편집을** 클릭하여 제품 **선택을 설정할 수 있는 속성** 대화 상자를 **엽니다** .

   예를 들어 태그/ **키워드** 필드를 열어 활동, Geometrixx-Outdoors 섹션에서 수영 순으로 선택합니다.

1. 확인을 **클릭하여** 속성을 저장합니다.예제 제품은 블루프린트 페이지의 제품 **선택** 기준 아래에 표시됩니다.
1. 롤아웃 변경 **사항..**. **을 클릭하고**&#x200B;롤아웃 페이지 및 모든 하위 페이지를 **선택한 다음** 다음 **을 클릭하고 롤아웃을**&#x200B;클릭합니다. 롤아웃이 성공적으로 완료되면 **상태** 표시기가 녹색으로 표시됩니다.
1. 이제 닫기를 클릭하고 **새** 카탈로그 섹션을 확인할 수 있습니다.예를 들면 다음과 같습니다.

   `http://localhost:4502/cf#/content/geometrixx-outdoors/en/swimwear.html`

1. 다시 Blueprint 페이지에서 Blueprint **편집을** 클릭하고 **속성** 대화 상자에서 **생성된 페이지** 탭을 엽니다. 배너 목록 필드에서 표시할 이미지를 선택합니다.예를 들면 `summer.jpg`
1. 확인을 **클릭하여** 속성을 저장합니다.배너 정보는 블루프린트 페이지의 제품 **선택** 기준 아래에 표시됩니다.
1. 이러한 새로운 변경 사항을 롤아웃합니다.
...
1. 마법사에서 필요에 따라 롤아웃을 설정한 다음 롤아웃 변경 내용을 탭/ **클릭합니다**.
...
>[!NOTE]
>
>다음 예에서는 [캠페인에서](/help/sites-authoring/personalization.md)직접 이루어지는 프로모션을 다루며, 이는 바우처에 사용됩니다.
>
>프로모션은 캠페인 내에서 [경험이](/help/sites-authoring/personalization.md) 될 수도 있습니다.
>
>자세한 내용은 프로모션 [및 바우처를 참조하십시오](#promotions).

1. 작성자 **인스턴스의 웹** 사이트 콘솔을 엽니다.
1. 왼쪽 창에서 필요한 캠페인을 **선택합니다**.
1. 새로 만들기를 **클릭하고**&#x200B;판촉 **템플릿을** 선택한 다음 **새 바우처에 대해** 제목 및 **** 이름(필요한 경우)을 지정합니다.
1. **만들기**&#x200B;를 클릭합니다. 오른쪽 창에 새 판촉 페이지가 표시됩니다.

1. 다음 방법 **중** 하나로 속성을 편집합니다.

   * 페이지를 연 다음 편집 단추를 클릭하여 속성 대화 상자를 엽니다
   * **웹 사이트 콘솔에서 페이지를 선택한 다음 컨텍스트 메뉴(일반적으로 마우스 오른쪽 단추)를 사용하여 속성...을**&#x200B;선택합니다.속성 대화 상자 열기
   판촉 **유형**, 할인 **유형**, 할인 **가격** 및 기타 필수 필드를지정합니다.

1. **확인**&#x200B;을 클릭하여 저장합니다.

1. 이제 판촉을 활성화할 수 있으므로 구매자가 게시 인스턴스에서 판촉 행사를 볼 수 있습니다.
...
1. 작성자 **인스턴스의 웹** 사이트 콘솔을 엽니다.
1. 왼쪽 창에서 필요한 캠페인을 **선택합니다**.
1. [새로 만들기] **를**&#x200B;클릭하고 [바우처 **템플릿]** 을 **선택한 다음** 새 바우처에 대해 **제목 및** 이름(필요한 경우)을 지정합니다.
1. **만들기**&#x200B;를 클릭합니다. 오른쪽 창에 새 바우처 페이지가 표시됩니다.

1. 두 번 클릭하여 새 바우처 페이지를 연 다음 편집을 클릭하여 **필요한** 정보를 구성합니다.
1. **확인**&#x200B;을 클릭하여 저장합니다.

1. 이제 구매자가 게시 인스턴스의 장바구니에서 사용할 수 있도록 바우처를 활성화할 수 있습니다.
...
* 바우처를 비활성화하십시오. 나중에 다시 활성화할 수 있도록 작성 환경에서 사용할 수 있게 됩니다.
* 완전히 삭제합니다.
...
바우처 또는 프로모션의 속성을 변경하려면 웹 사이트 콘솔에서 바우처를 두 번 클릭하고 **편집을** 클릭하면 **됩니다**. 저장한 후에는 변경 사항이 게시 인스턴스로 푸시되도록 활성화해야 합니다.

### 장바구니에 바우처 추가 {#adding-vouchers-to-a-cart}

사용자가 장바구니에 바우처를 추가할 수 있도록 기본 제공 바우처 구성 요소( **커머스** 카테고리)를 사용할 수 있습니다. 장바구니가 표시되지만 필수는 아닙니다. 바우처 구성 요소는 사용자가 바우처 코드를 입력할 수 있는 양식일 뿐입니다. 이 구성 요소는 실제로 적용된 바우처의 목록과 할인율을 보여주는 장바구니 구성 요소입니다.

데모 사이트(Geometrixx Outdoors - English)에서는 장바구니 페이지의 실제 장바구니 아래에 있는 바우처 양식을 볼 수 있습니다.

## 주문 {#orders}

>[!NOTE]
>
>즉시 사용 가능한 AEM에는 반품, 주문 상태 업데이트, 주문 처리 수행, 포장 명세서 생성 등 주문과 관련된 표준 기능에 필요한 작업이 없습니다. 주로 기술 미리 보기용입니다.
>
>AEM의 일반 주문 관리는 기본적으로 유지되었습니다.마법사에서 사용할 수 있는 필드는 스캐폴드에 따라 다릅니다.\
...
>[!NOTE]
>
>주문 콘솔에는 게시되지 않은 공급업체 주문 정보가 표시됩니다.
>  
>고객 주문 정보는 해당 홈 디렉토리에 저장되며 해당 계정의 주문 내역에 노출됩니다. 이 정보는 나머지 홈 디렉토리와 함께 게시됩니다.
...
1. 마법사가 열립니다. 기본 **, 컨텐츠**, **지불****및 이행** 탭을 사용하여 **새 주문에 대한** [](/help/sites-administering/concepts.md#order-information)정보를 입력합니다.
...
1. 주문 [정보가](/help/sites-administering/concepts.md#order-information) 표시됩니다. 편집 **및** 완료를 **사용하여** 변경할 수 있습니다.
