---
title: 'AEM Managed Services에 대한 Adobe IMS 인증 및 Admin Console 지원 '
seo-title: 'AEM Managed Services에 대한 Adobe IMS 인증 및 Admin Console 지원 '
...
# AEM Managed Services에 대한 Adobe IMS 인증 및 Admin Console 지원 {#adobe-ims-authentication-and-admin-console-support-for-aem-managed-services}
...
* AEM Managed Services은 고객 토폴로지를 Admin Console과 동기화합니다. Admin Console에 인스턴스당 AEM Managed Services 제품 컨텍스트의 인스턴스가 하나씩 있습니다.
...
* Adobe Admin Console의 사용자 관리는 고객 관리자가 계속 소유합니다.
...
Admin Console에 대한 고객 입사 과정은 AEM 인증에 Adobe IMS를 사용하기 위한 사전 요구 사항입니다.

첫 번째 단계로, 고객은 Adobe IMS에서 조직이 프로비저닝되어야 합니다. Adobe Enterprise 고객은 [Adobe Admin Console](https://helpx.adobe.com/kr/enterprise/using/admin-console.html)에서 IMS 조직으로 표시됩니다.

AEM Managed Services 고객은 이미 프로비저닝된 조직을 보유하고 있어야 하며 IMS 프로비저닝의 일부로 고객 인스턴스는 사용자 권한 및 액세스 권한을 관리하기 위해 Admin Console에서 사용할 수 있게 됩니다.
...
>
>Enterprise 관리 및 Admin Console에 대한 자세한 내용은 [이 페이지](https://helpx.adobe.com/kr/enterprise/managing/user-guide.html) 문서를 참조하십시오.

### Admin Console {#onboarding-users-to-the-admin-console} 온보딩 사용자
...
#### Admin Console UI {#file-upload-in-the-admin-console-ui}에 파일 업로드
