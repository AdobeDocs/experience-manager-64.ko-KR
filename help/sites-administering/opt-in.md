...
description: Adobe Analytics 및 Adobe Target을 선택하는 방법을 살펴보십시오.
seo-description: Adobe Analytics 및 Adobe Target을 선택하는 방법을 살펴보십시오.
...
AEM은 Adobe Analytics 및 Adobe Target와의 통합을 지원하는 옵트인 절차를 제공합니다. 이 작업은 관리자 사용자 그룹에 미리 로드된 작업으로 즉시 사용할 수 있습니다.

관리자로 로그인하면 받은 편지함에서 이 작업(**분석 및 타깃팅**&#x200B;구성)을 사용할 수 [있습니다](/help/sites-authoring/inbox.md#out-of-the-box-administrative-tasks). 제공하는 자격 증명을 기반으로 이러한 서비스를 구성하고 통합하는 데 도움이 됩니다.

통합을 구성하기 위한 다음 옵션이 있습니다.

* 작업을 통해 통합을 구성합니다.

   이 작업은 즉시 또는 나중에 수행할 수 있으며 일부 작업이 수행될 때까지 받은 편지함에 남아 있습니다. 두 경우 모두 UI에서 직접 구성하거나 미리 정의된 `.properties` 파일을 사용하여 구성할 수 있습니다.

* 통합 거부

   통합을 [수동으로 구성하려면 이 옵션을 고려하십시오](/help/sites-administering/marketing-cloud.md). DTM을 [사용하여 AEM과 Adobe Target 및 Adobe Analytics 통합을 참조하십시오](https://helpx.adobe.com/experience-manager/using/integrate-digital-marketing-solutions.html).
...
두 가지 옵션 중 하나에 대해 사용자 계정 정보를 제공하고 추적할 페이지를 지정해야 합니다.

>[!NOTE]
>
>서버 시작 시 읽는 속성 파일을 사용하여 Analytics 및 Target 계정 정보를 선택적으로 제공할 수 있습니다. 속성 파일을 [사용하여 계정 정보 제공을 참조하십시오](/help/sites-administering/opt-in.md#providing-account-information-using-a-properties-file).
...
>[!NOTE]
>
>AT.js는 기본 클라이언트 라이브러리입니다. 이는 [대상 클라우드 서비스 구성 아래에 구성됩니다](/help/sites-administering/target-configuring.md#creating-a-target-cloud-configuration).
>
>Adobe에서는 AT.js를 클라이언트 라이브러리로 사용하는 것이 좋습니다.

미리 로드된 기본 작업에서 옵트인하려면

1. 받은 [편지함에서 분석 및 **타깃팅** 구성](/help/sites-authoring/inbox.md#taking-action-on-an-item) 작업을선택하고 엽니다.
...
1. Analytics의 경우:
   ...
   1. 해당 자격 증명이 인증됩니다.
   1. Analytics 계정이 인증되면 사용할 Analytics 보고서 세트를 선택합니다. AEM은 이러한 Analytics 보고서 세트를 검색합니다. 상태가 [추가됨]으로 **업데이트됩니다**.

1. Target:

   1. Target에 대한 사용자 계정 정보를 입력한 다음 해당 **추가** 단추를 클릭합니다.
   1. 해당 자격 증명이 인증됩니다. 상태가 [추가됨]으로 **업데이트됩니다**.
...
1. 완료를 **선택합니다** .
   ...
   >[!CAUTION]
   >
   >구성을 옵트인한 후 영향을 받는 사이트/페이지를 게시하여 이러한 변경 사항을 게시 인스턴스에 복제해야 합니다.

## 통합 선택 해제 {#opting-out-of-the-integration}

다음 중 한 가지를 수행할 때 Analytics 및 Target와의 통합을 옵트아웃합니다.

* 이러한 제품과 통합하지 마십시오.
* 통합을 수동으로 구성하는 것이 좋습니다.

   수동 통합 구성에 대한 자세한 내용은 Adobe Analytics과 [통합](/help/sites-administering/adobeanalytics.md) 및 Adobe Target과 [통합을 참조하십시오](/help/sites-administering/target.md).

수신을 거부하려면 사전 로드된 작업을 완료해야 합니다.

* 받은 [편지함에서 분석 및 **타깃팅** 구성](/help/sites-authoring/inbox.md#taking-action-on-an-item) 작업을 선택하고완료합니다.

## 속성 파일을 사용하여 계정 정보 제공 {#providing-account-information-using-a-properties-file}

AEM이 서버 시작 시 읽는 속성 파일을 설치하여 Analytics 및 Target와의 통합을 위한 계정 속성을 구성합니다. 속성 파일을 사용하면 옵트인 마법사가 파일의 속성을 자동으로 사용하고 그에 따라 클라우드 구성이 생성됩니다.
...
* analytics.secret: Analytics 사용자 이름과 연관된 비밀입니다.
...
속성과 값은 등호(=)로 구분됩니다. Analytics 속성 접두사가 `analytics`있고 Target 속성 접두어가 붙습니다 `target`. 서비스를 구성하려면 해당 서비스의 모든 속성에 대한 값을 제공합니다. 서비스를 구성하지 않으려면 해당 서비스에 대한 값을 제공하지 마십시오.
...
1. 통합 구성에 설명된 대로 분석 및 타깃팅 구성 [작업을 엽니다](/help/sites-administering/opt-in.md#configuring-the-integration). 계정 정보를 요청하는 대신 `.properties` 파일의 값을 사용합니다.
...
클라우드 구성을 변경할 필요가 없습니다. 하지만 필요에 따라 프레임워크를 구성할 수 있습니다. (Adobe Analytics [속성을 사용하여 구성 요소 데이터 매핑](/help/sites-administering/adobeanalytics-mapping.md) 및 Target 프레임워크 [추가를 참조하십시오](/help/sites-administering/target.md).)
...
>
>작성 인스턴스에도 추적이 필요한 경우 값을 로 변경해야 합니다 `all`.
...
전송하는 매개 변수는 다음과 같이 달라집니다.

* 필요한 모든 자격 증명으로 채워진 marketing **cloud.properties** 파일을 사용하려면 다음 매개 변수를 전송해야 합니다.

   * `automaticProvisioning`= `true`
   * `servicename`= `analytics|target`
   * `path`경로 = 생성된 클라우드 서비스 구성 첨부

   예를 들어 Analytics 및 Target 구성을 모두 만들고 이 구성을 we.retail 페이지에 연결하는 말림 요청은 다음과 같습니다.

   ```shell
   curl -v -u admin:admin -X POST -d"automaticProvisioning=true&servicename=target&servicename=analytics&path=/content/we-retail" http://localhost:4502/libs/cq/cloudservicesprovisioning/content/autoprovisioning.json
   ```

* marketingCloud.properties **** 파일을 사용하지 않으려면 매개 변수와 함께 자격 증명을 보내야 합니다. 예를 들면 다음과 같습니다.
