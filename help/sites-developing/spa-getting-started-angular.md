---
title: AEM에서 SPA 시작 - Angular
seo-title: AEM에서 SPA 시작 - Angular
description: 이 문서에서는 샘플 SPA 애플리케이션을 소개하고 이를 통합하는 방법을 설명하고 Angular 프레임워크를 사용하여 신속하게 자체 SPA를 사용할 수 있도록 합니다.
seo-description: 이 문서에서는 샘플 SPA 애플리케이션을 소개하고 이를 통합하는 방법을 설명하고 Angular 프레임워크를 사용하여 신속하게 자체 SPA를 사용할 수 있도록 합니다.
...
# AEM에서 SPA 시작 - Angular {#getting-started-with-spas-in-aem-angular}
...
SPA 제작 기능은 AEM 내의 SPA를 지원하는 포괄적인 솔루션을 제공합니다. 이 문서는 Angular 프레임워크에 간소화된 SPA 애플리케이션을 제공하여 SPA를 신속하게 설치하여 사용하는 방법을 설명합니다.

>[!NOTE]
>
>이 아티클은 Angular 프레임워크를 기반으로 합니다. React 프레임워크에 해당하는 문서는 AEM의 [SPA 시작하기 - 반응을 참조하십시오](/help/sites-developing/spa-getting-started-react.md).
...
본 문서는 간소화된 SPA의 구조를 자세히 설명하여 SPA에 이러한 이해를 적용할 수 있도록 지원합니다.

## 종속성, 구성 및 빌드 {#dependencies-configuration-and-building}

예상 Angular 종속성 외에도 샘플 SPA는 추가 라이브러리를 활용하여 SPA를 보다 효율적으로 만들 수 있습니다.

### 종속성 {#dependencies}

이 `package.json` 파일은 전체 SPA 패키지의 요구 사항을 정의합니다. 필요한 최소 AEM 종속성이 여기에 나열됩니다.
...
이 문서의 다음 섹션에서는 AEM의 SPA가 구조화된 방식, 애플리케이션을 구동하는 중요한 파일 및 SPA가 어떻게 연동되는지 살펴봅니다.

간소화된 이미지 구성 요소가 예로 사용되지만 애플리케이션의 모든 구성 요소는 동일한 개념을 기반으로 합니다.

### app.module.ts {#app-module-ts}

SPA의 엔트리 포인트는 중요한 컨텐츠에 초점을 맞추기 위해 간소화된 `app.module.ts` 파일입니다.
...
이 `app.module.ts` 파일은 앱의 시작점이며 초기 프로젝트 구성이 포함되어 있으며 앱을 부트스트랩하는 `AppComponent` 데 사용됩니다.
...
부트스트랩을 `app.module.ts` 실행하면, `AppComponent`여기에서 간단한 버전으로 표시되는 앱을 초기화하여 중요한 콘텐츠에 집중할 수 있습니다.
...
페이지를 처리하면 `app.component.ts` 여기에 나열된 간단한 버전으로 통화가 `main-content.component.ts` 이루어집니다.
...
인제스트는 페이지 모델의 JSON 표현을 `MainComponent` 설정하고 컨텐츠가 페이지의 각 요소를 래핑/장식하도록 처리합니다. 자세한 내용은 `Page` SPA Blueprint 문서를 [참조하십시오](/help/sites-developing/spa-blueprint.md).

### image.component.ts {#image-component-ts}

구성 `Page` 요소로 구성됩니다. 인제스트된 JSON을 사용하면 여기에 표시된 것과 같은 구성 요소를 처리할 `Page` 수 `image.component.ts` 있습니다.
...
마지막으로 이미지를 렌더링할 수 `image.component.html`있습니다.
...
* **** 옵션 1:util 클래스를 순수한 객체 지향 솔루션으로 사용하여 로직과 브로드캐스트를 필요한 구성 요소로 중앙에서 관리할 수 있습니다.
