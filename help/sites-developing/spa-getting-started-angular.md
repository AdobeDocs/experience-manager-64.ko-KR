...
페이지를 처리하면 `app.component.ts`이(가) 여기에 나열된 `main-content.component.ts`을(를) 간소화된 버전으로 호출합니다.
...
`MainComponent` 은 페이지 모델의 JSON 표현을 수집하고 컨텐츠를 처리하여 페이지의 각 요소를 래핑/장식합니다. `Page`에 대한 자세한 내용은 [SPA Blueprint](/help/sites-developing/spa-blueprint.md) 문서에서 확인할 수 있습니다.
