...
The [HTML Templating Language (HTL)](https://helpx.adobe.com/experience-manager/htl/user-guide.html), introduced with AEM 6.0, takes the place of JSP (JavaServer Pages) as the preferred and recommended server-side template system for HTML. 강력한 엔터프라이즈 웹 사이트를 구축해야 하는 웹 개발자를 위해 HTL을 사용하면 보안 및 개발 효율성을 향상시킬 수 있습니다.
...
WCM [모드](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) 쿠키는 페이지를 새로 고치지 않아도 미리 보기 **모드로** 전환할 때 설정됩니다.

WCM 모드에 민감한 렌더링을 사용하는 구성 요소의 경우 특별히 새로 고친 다음 쿠키의 값을 사용해야 합니다.

>[!NOTE]
>
>터치에 적합한 UI에서는 값만 `EDIT` 사용하고 WCM 모드 `PREVIEW` 쿠키에 [사용됩니다](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) .
...
[[MOCK] Granite UI provides a large range of the basic components (widgets)](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/index.html) needed to create your dialog on the authoring environment. 필요한 경우 이 선택 항목을 확장하고 자신만의 위젯을 만들 수 있습니다.
...
>[!NOTE]
>
>Granite UI에 대한 자세한 내용은 Granite UI [설명서를](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/index.html)참조하십시오.
...
특히 [화강암 UI]는 대화 상자(또는 보다 일반적으로 [양식에서](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/foundation/form/index.html))에 사용할 수 있는 다양한 필드 구성 요소를 제공합니다.
...
구성 요소 대화 상자용으로만 클라이언트 라이브러리를 불러오려면(즉, 다른 구성 요소용으로 로드되지 않음) 대화 상자의 속성을 방금 만든 클라이언트 라이브러리의 카테고리 이름으로 설정해야 합니다. `extraClientlibs` 클라이언트 라이브러리가 매우 크고/또는 필드가 해당 대화 상자에 고유하므로 다른 대화 상자에서는 필요하지 않을 경우 권장됩니다.
...
이제 대화 상자 필드에서 이벤트를 처리하는 방법이 사용자 정의 클라이언트 라이브러리의 [리스너와](#listeners-in-a-custom-client-library)함께 수행됩니다. 이는 컨텐츠 구조에 [리스너를](#listeners-in-the-content-structure)두는 이전 방식에서 변경된 것입니다.
...
이를 위해서는 상호 작용할 기본 위젯 라이브러리에 대해 알아야 합니다. 반응할 [이벤트를 확인하려면 Coral UI 설명서를](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/coral-ui/coralui3/index.html) 참조하십시오. 이는 이전에 ExtJS를 사용하여 수행해야 했던 프로세스와 매우 유사합니다.해당 위젯의 설명서 페이지를 찾은 다음 해당 이벤트 API의 세부 사항을 확인합니다.
...
* 템플릿의 단락 시스템에서 구성 요소 [속성을 정의합니다](/help/sites-developing/components-basics.md#adding-your-component-to-the-paragraph-system).
...
AEM에서는 사용자가 (적절한) 자산을 페이지의 [](/help/sites-authoring/editing-content.md) 인스턴스로 끌면(항상 빈 구성 요소를 페이지로 드래그하지 않아도) 새 구성 요소의 인스턴스가 자동으로 생성되도록 페이지에 단락 시스템을 구성할 수 있는 가능성을 제공합니다.
...
* 구성 요소 [개발을](https://helpx.adobe.com/experience-manager/htl/user-guide.html) 단순화하고 보안을 강화하기 위해 고안된 템플릿 언어인 HTL 지원을 제공합니다.
...
* HTL
   ...
   * HTL [을](https://helpx.adobe.com/experience-manager/htl/user-guide.html) 사용하는 것은 강제적이지 않지만 구성 요소를 업데이트해야 하는 경우 JSP에서 HTL로 [마이그레이션하는 것을 고려하는 것이 이상적인 시기입니다](/help/sites-developing/components-basics.md#htl-vs-jsp).
...
* 대화 상자
   ...
   * Granite UI [유효성 검사로 유형 마이그레이션](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/foundation/clientlibs/foundation/js/validation/index.html)
