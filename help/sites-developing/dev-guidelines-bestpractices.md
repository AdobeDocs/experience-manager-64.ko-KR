...
seo-description: AEM 개발 지침 및 우수 사례
...
기존 정의를 복사하고 오버레이하여 수행할 수 있습니다. 즉, 정의를 `/libs`에서 `/apps/<your-project>`로 복사합니다. `/apps`에서 이 새 정의는 요구 사항에 따라 업데이트할 수 있습니다.
...
>구성 및 기타 변경 사항에 대해서는
>
>1. `/libs`의 항목을 `/apps`에 복사합니다.
>1. `/apps` 내에서 변경


## JCR 쿼리를 사용해야 하는 경우 및 사용하지 않을 때는 {#when-to-use-jcr-queries-and-when-not-to-use-them}
...
>[!NOTE]
>
>[Query Builder](/help/sites-developing/querybuilder-api.md)를 사용하는 경우 Query Builder가 후드 아래에 JCR 쿼리를 생성하므로 JCR 쿼리를 사용합니다.
...
또한 Apache](https://modsecurity.org)용 [mod_security와 같은 웹 애플리케이션 방화벽은 배포 환경의 보안을 안정적이고 중앙 집중식으로 제어하며 이전에 감지되지 않은 교차 사이트 스크립팅 공격으로부터 보호할 수 있습니다.
...
## 고유한 개발 작업 {#distinct-development-tasks}

### 오류 페이지 사용자 지정 {#customizing-error-pages}

AEM에 대해 오류 페이지를 사용자 지정할 수 있습니다. 인스턴스가 내부 서버 오류에 대한 슬링 추적을 표시하지 않도록 하는 것이 좋습니다.

자세한 내용은 오류 핸들러로 표시된 [오류 페이지 사용자 지정](/help/sites-developing/customizing-errorhandler-pages.md)을 참조하십시오.

### Java 프로세스에서 파일 열기 {#open-files-in-the-java-process}

AEM은 많은 수의 파일에 액세스할 수 있으므로 Java 프로세스에 대해 [열린 파일 수를 AEM에 대해 명시적으로 구성하는 것이 좋습니다.](/help/sites-deploying/configuring.md#open-files-in-the-java-process)
