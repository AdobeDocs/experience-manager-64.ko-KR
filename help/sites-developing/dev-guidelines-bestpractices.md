...
기존 정의를 복사하고 오버레이하여 수행할 수 있습니다. 즉, 정의를 에서 로 복사하는 `/libs` 것입니다 `/apps/<your-project>`. 이 새로운 정의 `/apps`는 요구 사항에 따라 업데이트할 수 있습니다.
...
>[!NOTE]
>
>쿼리 [빌더를 사용하는](/help/sites-developing/querybuilder-api.md)경우 쿼리 빌더가 백그라운드에서 JCR 쿼리를 생성하기 때문에 JCR 쿼리를 사용합니다.
...
>[!NOTE]
>
>또한 [보안 체크리스트를 참조할 수 있습니다](/help/sites-administering/security-checklist.md).
...
또한 Apache용 [mod_security와](https://modsecurity.org)같은 웹 애플리케이션 방화벽은 배포 환경의 보안을 중앙 집중식으로 안전하게 제어하고 이전에 감지되지 않은 교차 사이트 스크립팅 공격으로부터 보호할 수 있습니다.
...
자세한 [내용은 오류 처리기에 표시되는 오류 페이지 사용자](/help/sites-developing/customizing-errorhandler-pages.md) 지정을 참조하십시오.

### Java 프로세스에서 파일 열기 {#open-files-in-the-java-process}

AEM은 많은 수의 파일에 액세스할 수 있으므로 AEM용으로 Java 프로세스에 [대해](/help/sites-deploying/configuring.md#open-files-in-the-java-process) 열려 있는 파일의 수를 명시적으로 구성하는 것이 좋습니다.
