...
>[!NOTE]
>
>[편집 ](/help/sites-developing/page-templates-editable.md) 가능한 템플릿을 사용할 수 있으며 가장 유연하고 최신 기능을 위해 권장되는 템플릿 유형입니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td> String[]</td> 
   <td>이 템플릿의 하위 항목으로 허용되는 템플릿 경로입니다.<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>템플릿의 콘텐트가 들어 있는 노드입니다.<br /> </td> 
  </tr> 
  <tr> 
   <td> thumbnail.png</td> 
   <td> nt:파일</td> 
...
페이지를 만들려면 템플릿을 사이트 트리의 해당 위치에 복사해야 합니다(node-tree `/apps/<myapp>/template/<mytemplate>`).이것은 **웹 사이트** 탭을 사용하여 페이지를 만드는 경우에 발생하는 작업입니다.

이 복사 작업은 페이지의 초기 컨텐츠(일반적으로 최상위 컨텐츠 전용)와 페이지를 렌더링하는 데 사용되는 페이지 구성 요소의 경로(하위 노드 jcr:content)인 sling:resourceType 속성도 제공합니다.

## 템플릿이 구조화된 방식 {#how-templates-are-structured}
...
이 노드에는 결과 페이지의 컨텐츠 노드의 기초로 사용되는 jcr:content(cq:PageContent) 노드가 포함되어 있습니다.새 페이지의 실제 컨텐츠를 렌더링하는 데 사용할 구성 요소인 sling:resourceType을 사용하여 참조합니다.
...
템플릿은 `cq:Page` 유형의 페이지를 만드는 데 사용됩니다(앞에서 언급했듯이 페이지는 구성 요소의 특별한 유형임). 각 AEM 페이지에는 구조화된 노드 `jcr:content`이(가) 있습니다. 이 기능은

* 의 유형이 cq:PageContent임
* 은 정의된 컨텐츠 정의를 포함하는 구조화된 노드 유형입니다.
* 내용 렌더링에 사용되는 sling 스크립트를 포함하는 구성 요소를 참조하는 속성 `sling:resourceType`이 있습니다.
...
예를 들어 AEM에는 컨텐트 페이지 및 홈 페이지를 비롯한 여러 템플릿이 포함되어 있습니다.
...
웹 사이트에 대한 기존 템플릿을 활성화하고 **웹 사이트** 콘솔에서 **웹 사이트** 아래에 페이지를 만들 때 **페이지 만들기** 대화 상자에 표시하려면 템플릿 노드의 allowedPaths 속성을 다음으로 설정합니다.**/content(/)&amp;ast;)?**

## 템플릿 디자인이 적용되는 방식 {#how-template-designs-are-applied}

[디자인 모드](/help/sites-authoring/default-components-designmode.md)를 사용하여 UI에 스타일이 정의되면 디자인이 스타일이 정의되는 컨텐츠 노드의 정확한 경로로 유지됩니다.
...
디자인이 디자인 모드에서만 적용되는 경우 다음 섹션인 [디자인 경로 해상도](/help/sites-developing/page-templates-static.md#design-path-resolution), [결정 트리](/help/sites-developing/page-templates-static.md#decision-tree) 및 [예제](/help/sites-developing/page-templates-static.md#example)를 적용할 수 없습니다.

>[!NOTE]
>
>이 섹션에서는 AEM 6.4.2.0의 설계 경로 해결 행동에 대해 설명합니다.

### 디자인 경로 해상도 {#design-path-resolution}

정적 템플릿을 기반으로 컨텐츠를 렌더링할 때 AEM은 컨텐츠 계층 구조의 탐색을 기반으로 컨텐츠에 가장 연관성이 높은 디자인 및 스타일을 적용하려고 합니다.

AEM은 다음 순서로 컨텐트 노드에 대해 가장 관련성이 높은 스타일을 결정합니다.

* 디자인 모드에서 디자인이 정의된 경우와 같이 컨텐츠 노드의 전체 경로 및 정확한 경로에 대한 디자인이 있는 경우 해당 디자인을 사용합니다.
* 상위의 컨텐츠 노드에 대한 디자인이 있으면 해당 디자인을 사용합니다.
* 컨텐트 노드의 경로에 노드에 대한 디자인이 있으면 해당 디자인을 사용합니다.

마지막 두 경우, 적용 가능한 디자인이 두 개 이상 있을 경우 컨텐츠 노드에 가장 가까운 디자인을 사용합니다.
...
<table> 
 <tbody> 
  <tr> 
   ...
   <td><strong>다음에 대한 디자인이 있습니다.<br /> </strong></td> 
  ...
  <tr> 
   ...
   <td>나무 아래에서 가장 가까운 시합으로 돌아가라.</td> 
  </tr> 
  <tr> 
   <td><code>leaf</code></td> 
   <td><code>root</code></td> 
   <td><code>root</code></td> 
   <td>다른 모든 것이 실패할 경우 남은 항목을 사용합니다.<br /> </td> 
  </tr> 
  <tr> 
   <td><code>branch</code></td> 
   <td><code>branch</code></td> 
  ...
  <tr> 
   ...
   <td><code>root</code></td> 
   <td><p>정확히 일치하는 것이 없으면 나무 아래에 있는 것을 가져가세요.</p> <p>이 경우 항상 적용할 수 있지만 트리 위쪽에서는 너무 구체적일 수 있습니다.<br /> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
AEM 페이지 템플릿은 새 페이지를 만드는 데 사용되는 모델입니다. 필요한 속성(주로 sling:resourceType)을 사용하여 편집 및 렌더링을 허용하는 상태로 설정하고, 필요에 따라 초기 컨텐츠를 거의 포함하거나 많이 포함할 수 있으므로 올바른 초기 노드 구조를 만드는 역할을 합니다.

### 기존 템플릿을 기반으로 새 템플릿 만들기 {#creating-a-new-template-based-on-an-existing-template}

새 템플릿을 처음부터 완전히 만들 수는 있지만 기존 템플릿을 복사 및 업데이트하여 시간과 노력을 절약할 수 있습니다. 예를 들어 Geometrixx 내의 템플릿을 사용하여 시작할 수 있습니다.

기존 템플릿을 기반으로 새 템플릿을 만들려면 다음을 수행하십시오.

1. 기존 템플릿(가능한 한 가까이 정의하면 원하는 대로 가능)을 새 노드에 복사합니다.

   템플릿은 일반적으로 **/apps/&lt;website-name>/templates/&lt;template-name>**&#x200B;에 저장됩니다.

   >[!NOTE]
   >
   >사용 가능한 템플릿 목록은 새 페이지의 위치 및 각 템플릿에 지정된 배치 제한에 따라 달라집니다. [템플릿 가용성](/help/sites-developing/templates.md#template-availability)을 참조하십시오.
...
1. 템플릿 기반이 되는 구성 요소를 복사하여 새 인스턴스를 만듭니다(템플릿 내의 **jcr:content** 노드의 **sling:resourceType** 속성으로 표시됨).
...
## 기존 템플릿을 사용 가능하게 만들기 {#making-an-existing-template-available}

이 예에서는 특정 컨텐츠 경로에 템플릿을 사용할 수 있도록 허용하는 방법을 보여 줍니다. 새 페이지를 만들 때 페이지 작성자가 사용할 수 있는 템플릿은 [템플릿 가용성](/help/sites-developing/templates.md#template-availability)에 정의된 논리에 의해 결정됩니다.

1. CRXDE Lite에서 뉴스레터 템플릿과 같이 페이지에 사용할 템플릿으로 이동합니다.
1. `allowedPaths` 속성 및 [템플릿 사용 가능](/help/sites-developing/templates.md#template-availability)에 사용되는 기타 속성을 변경합니다. 예: `allowedPaths`:`/content/geometrixx-outdoors/[^/]+(/.*)?`은 이 템플릿이 `/content/geometrixx-outdoors` 아래의 모든 경로에서 허용됨을 의미합니다.
