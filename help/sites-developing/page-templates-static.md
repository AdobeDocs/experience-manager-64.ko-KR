...
웹 사이트에 대한 기존 템플릿을 활성화하고 웹 사이트 **콘솔에서 웹 사이트** 바로 아래에 페이지를 만들 때 페이지 만들기 **대화** 상자에 **표시하려면** 템플릿 노드의allowedPaths 속성을 다음으로 설정합니다. **/content(/)&amp;ast;)?**
...
1. 기존 템플릿(가능한 한 원하는 것과 가깝게 정의되는 것)을 새 노드에 복사합니다.
   ...
   >[!NOTE]
   >
   >사용 가능한 템플릿 목록은 새 페이지의 위치 및 각 템플릿에 지정된 배치 제한에 따라 달라집니다. 템플릿 [가용성을 참조하십시오](/help/sites-developing/templates.md#template-availability).
