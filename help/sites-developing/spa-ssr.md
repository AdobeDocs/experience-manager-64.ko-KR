...
단일 페이지 애플리케이션(SPA)을 사용하면 기본 애플리케이션과 마찬가지로 익숙한 방식으로 반응하고 동작하는 풍부하고 다이내믹한 경험을 제공할 수 있습니다. [이를 위해서는 고객에게 콘텐츠를 먼저 로드한 다음 사용자 인터랙션을](/help/sites-developing/spa-walkthrough.md#how-does-a-spa-work) 처리하는 데 많은 시간을 할애하여 클라이언트와 서버 사이에 필요한 통신 양을 최소화하여 앱을 보다 재활성화할 수 있어야 합니다.
...
이 두 질문 중 하나 이상에 대해 프로젝트에 대해 명확한 &quot;예&quot;가 있는 경우에만 Adobe에서 SSR을 구현하는 것이 좋습니다. 다음 섹션에서는 Adobe I/O 런타임을 사용하는 방법에 대해 설명합니다.

## Adobe I/O 런타임 {#adobe-io-runtime}

프로젝트에서 SSR을 구현해야 [한다고 확신하는](#when-to-use-ssr)경우 Adobe의 권장 솔루션은 Adobe I/O 런타임을 사용하는 것입니다.
...
AEM 파섹 SSR용으로 구현하려는 모델에 [관계없이](#adobe-io-runtime)이 원격 렌더링 서비스에 액세스하는 방법을 AEM에 지정해야 합니다.
...
>[!NOTE]
>
>AEM 기반 통신 흐름 [또는 Adobe I/O 런타임](#aem-driven-communication-flow) 기반 흐름을 [](#adobe-io-driven-communication-flow)구현하도록 선택하더라도 원격 컨텐츠 렌더러 구성을 정의해야 합니다.
>
>사용자 지정 Node.js 서버를 [사용하려는](#using-node-js)경우에도 이 구성을 정의해야 합니다.

>[!NOTE]
>
>이 구성은 추가 확장 [및 사용자](#remote-content-renderer)지정 옵션이 있는 원격 컨텐츠 렌더러를 활용합니다.

## AEM 기반 커뮤니케이션 흐름 {#aem-driven-communication-flow}

SSR을 사용할 때 AEM에서 SPA의 [구성 요소 상호 작용 워크플로우는](/help/sites-developing/spa-overview.md#workflow) Adobe I/O 런타임에서 앱의 초기 컨텐츠를 생성하는 단계를 포함합니다.
...
AEM [기반 커뮤니케이션](#aem-driven-communication-flow) 흐름 섹션에서는 AEM에서 SPA와 관련하여 서버측 렌더링의 표준 및 권장 구현에 대해 설명합니다. 여기서 AEM은 콘텐츠 부트스트랩 및 제공을 수행합니다.
...
| 부트스트랩 | 장점 | 단점 |
|---|---|---|
| via AEM | AEM에서 라이브러리 삽입이<br>필요한 경우 관리리소스는 AEM에서만 유지 관리되어야 합니다. | SPA 개발자에 익숙하지 않은 경우 |
| adobe I/O 런타임 사용 | SPA 개발자에게 더 친숙해 | CSS 및 JavaScript와 같은 응용 프로그램에 필요한 Clientlib 리소스는 AEM 개발자가 [`allowProxy` 속성을 통해 사용할 수 있어야](/help/sites-developing/clientlibs.md#locating-a-client-library-folder-and-using-the-proxy-client-libraries-servlet)<br>합니다.리소스는 AEM과 Adobe I/O 런타임 사이에 동기화되어야<br>합니다.SPA를 제작하려면 Adobe I/O 런타임용 프록시 서버가 필요할 수 있습니다 |

## SSR 계획 {#planning-for-ssr}

일반적으로 응용 프로그램의 일부만 서버측에서 렌더링해야 합니다. 일반적인 예는 페이지의 초기 로드 시 접힘 위에 표시될 컨텐츠를 서버측에서 렌더링해야 하는 것입니다. 이렇게 하면 이미 렌더링된 컨텐츠를 클라이언트에 전달하여 시간을 절약할 수 있습니다. 사용자가 SPA와 상호 작용하면 추가 컨텐츠가 클라이언트에 의해 렌더링됩니다.
