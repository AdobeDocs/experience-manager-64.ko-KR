...
또한 프로그램 및 스크립트에서 워크플로우를 [이용할 수도 있습니다](/help/sites-developing/workflows-program-interaction.md).
...
모든 구성 요소 [와](/help/sites-developing/components.md)마찬가지로 워크플로우 단계 구성 요소는 `sling:resourceSuperType` 속성에 지정된 구성 요소에서 상속됩니다. 다음 다이어그램은 모든 워크플로우 단계 구성 요소의 기초가 되는 `cq:component` 노드의 계층을 보여줍니다. 사용자 지정 단계 구성 요소 개발을 위한 가장 일반적인(및 기본) 시작점이므로 **프로세스 단계****,**&#x200B;참가자 단계 **및** 동적 참가자 단계구성 요소도 다이어그램에 포함되어 있습니다.
...
>
>1. 필요한 항목(예: `/libs` `/apps`
>2. Make any changes within `/apps`


구성 요소 `/libs/cq/workflow/components/model/step` 는 **프로세스 단계**, **참가자 단계**&#x200B;및 **동적 참가자 단계**&#x200B;중 가장 일반적인 상위 항목으로서, 모두 다음 항목을 상속합니다.

* `step.jsp`

   모델에 추가될 때 `step.jsp` 스크립트는 단계 구성 요소의 제목을 렌더링합니다.
...
* [워크플로데이터](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/workflow/exec/WorkflowData.html) 워크플로데이터
...
세 가지 유형의 MetaDataMap 개체( `Workflow`for, `WorkflowData` and `WorkItem` objects)가 있습니다. 메타데이터가 저장되어 있는 목적에 모두 동일합니다.

WorkItem에는 해당 작업 항목(예: 단계)이 실행되는 동안에만 사용할 수 있는 고유한 MetaDataMap이 있습니다.

전체 워크플로우 `Workflow` 에서 `WorkflowData` 메타데이터와 메타데이터를 공유할 수 있습니다. 이러한 경우 `WorkflowData` 메타데이터 맵만 사용하는 것이 좋습니다.

## 사용자 지정 워크플로우 단계 구성 요소 만들기 {#creating-custom-workflow-step-components}

워크플로우 단계 구성 요소는 다른 구성 요소와 [동일한 방식으로 만들 수 있습니다](/help/sites-developing/components.md).
...
다음 절차를 사용하여 공통 **** 탭의 제목 **및** 설명 **필드** 에 대한 기본값을지정합니다.
...
1. 다음 `String` 이름의 속성을 `cq:formParameters` 노드에 추가합니다.

   * `jcr:title`: 이 값은 [ **일반** ] **탭의 제목** 필드를채웁니다.
...
항목의 이름 속성은 `cq:Widget` 위젯의 값을 저장하는 JCR 노드를 지정합니다. 워크플로우 단계 구성 요소의 대화 상자에 있는 위젯이 노드 아래에 값을 저장할 때 `./metaData` 해당 값이 워크플로우에 추가됩니다 `MetaDataMap`.

예를 들어 대화 상자의 텍스트 필드는 다음과 같은 속성을 가진 `cq:Widget` 노드입니다.
...
>[!NOTE]
>
>키가 `PROCESS_ARGS`되면 변수를 통해 ECMA 스크립트 구현에서 값을 쉽게 사용할 수 `args` 있습니다. 이 경우 name 속성의 값은 `./metaData/PROCESS_ARGS.`
...
1. 노드에 `String` 속성을 `cq:formParameters` 추가합니다. 구성 요소 슈퍼 유형은 속성의 이름을 결정합니다.
...
새 구성 요소에 대해 다음 절차를 수행하십시오(사용자 [지정 워크플로우 단계 구성 요소 만들기 참조](#creating-custom-workflow-step-components)).
...
노드 `cq:Component` 아래에 `cq:EditConfig` 노드를 추가합니다. Below that add a `nt:unstructured` `cq:formParameters`node (must be named) and that node add the following properties:
...
* 이름: `PROCESS_AUTO_ADVANCE`
   ...
   * 값:
      ...
      * 워크플로우로 설정하면 해당 단계 `true` 가 실행되고 계속 됩니다. 이 단계는 기본값이며 권장됩니다
      * when `false`, workflow will run and stop; 추가 처리가 필요하므로 `true` 권장
...
워크플로우 메타데이터는 [`MetaDataMap`](#metadatamaps) 객체에 저장됩니다. Java API는 적절한 개체를 제공하는 [`Workflow.getWorkflowData`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/granite/workflow/exec/Workflow.html) 개체를 반환하는 방법을 [`WorkflowData`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/granite/workflow/exec/WorkflowData.html) `MetaDataMap` 제공합니다. 이 `WorkflowData` `MetaDataMap` 개체는 단계 구성 요소의 OSGi 서비스 또는 ECMA 스크립트에서 사용할 수 있습니다.

#### Java {#java}

구현의 `WorkflowProcess` 실행 방법이 `WorkItem` 객체에 전달됩니다. 이 개체를 사용하여 현재 워크플로 인스턴스의 `WorkflowData` 개체를 가져옵니다. 다음 예에서는 워크플로우 객체에 항목을 추가한 다음 각 항목을 `MetaDataMap` 기록합니다. 워크플로우의 후속 단계에서 (&quot;mykey&quot;, &quot;My Step Value&quot;) 항목을 사용할 수 있습니다.
...
변수 `graniteWorkItem` 는 현재 Java 객체의 ECMA 스크립트 `WorkItem` 표현입니다. 따라서 이 변수를 사용하여 `graniteWorkItem` 워크플로우 메타데이터를 가져올 수 있습니다. 다음 ECMA 스크립트를 사용하여 **프로세스** 단계를 구현하여 워크플로우 `MetaDataMap` 개체에 항목을 추가한 다음 각 항목을 기록할 수 있습니다. 그런 다음 워크플로우의 후속 단계에서 이러한 항목을 사용할 수 있습니다.
...
### 런타임에 대화 상자 속성 값 액세스 {#accessing-dialog-property-values-at-runtime}

워크플로우 인스턴스의 `MetaDataMap` 개체는 워크플로우 수명 동안 데이터를 저장하고 검색하는 데 유용합니다. 워크플로우 단계 구성 요소 구현의 경우, 이 `MetaDataMap` 는 런타임 시 구성 요소 속성 값을 검색하는 데 특히 유용합니다.
...
이 워크플로우 `MetaDataMap` 는 Java 및 ECMA 스크립트 프로세스 구현에서 사용할 수 있습니다.

* WorkflowProcess 인터페이스의 Java 구현에서 매개 `args` 변수는 워크플로우의 `MetaDataMap` 개체입니다.

* ECMA 스크립트 구현에서 값은 `args` 및 변수를 사용하여 사용할 수 `metadata` 있습니다.

### 예: 프로세스 단계 구성 요소의 인수 검색 {#example-retrieving-the-arguments-of-the-process-step-component}

프로세스 단계 구성 요소의 편집 대화 상자에는 **인수** 속성이 **포함되어** 있습니다. Arguments **** 속성 값은 워크플로 메타데이터에 저장되고 `PROCESS_ARGS` 키와 연결됩니다.
...
다음 Java 코드는 구현에 `execute` 대한 `WorkflowProcess` 방법입니다. 이 메서드는 키와 연결된 값 `args``MetaDataMap` 을 `PROCESS_ARGS` 기록합니다.
...
프로세스 단계 구성 요소에 대한 스크립트 **에서** 인수를 `args` 개체를 통해 사용할 수 있습니다.

사용자 지정 단계 구성 요소를 만들 때 객체 `metaData` 를 스크립트로 사용할 수 있습니다. 이 개체는 단일 문자열 인수로 제한됩니다.
...
>[!NOTE]
>
>프로세스 단계 구성 요소를 서비스 또는 스크립트와 연결하는 방법에 대한 자세한 내용은 [프로세스 단계](/help/sites-developing/workflows-step-ref.md#process-step) 또는 단계 [구현](#overriding-the-step-implementation)중재를참조하십시오.
...
1. 번들을 만들고 OSGI 컨테이너에 배포합니다. CRXDE Lite [또는](/help/sites-developing/developing-with-crxde-lite.md) Eclipse를 사용하여 번들을 만드는 방법에 대한 설명서를 [참조하십시오](/help/sites-developing/howto-projects-eclipse.md).
   ...
   >[!NOTE]
   >
   >OSGI 구성 요소는 해당 메서드와 `WorkflowProcess` 인터페이스를 구현해야 `execute()` 합니다. 아래 예제 코드를 참조하십시오.

   >[!NOTE]
   >
   >패키지 이름을 구성 `<*Private-Package*>` 섹션에 추가해야 `maven-bundle-plugin` 합니다.

1. SCR 속성 `process.label` 을 추가하고 필요한 대로 값을 설정합니다. 일반 프로세스 단계 구성 요소를 사용할 때 프로세스 단계가 나열되는 **이름입니다** . 아래 예를 참조하십시오.
1. 모델 **** 편집기에서 일반 프로세스 단계 구성 요소를 사용하여 프로세스 단계를 워크플로우에 **추가합니다** .
1. 프로세스 단계 **편집 대화 상자(**&#x200B;프로세스 단계 **)에서** 프로세스탭으로 이동하여 프로세스 구현을 선택합니다.
...
페이로드가 페이지일 때 다음 OSGI 구성 요소 `approved` 는 페이지 컨텐츠 노드에 속성을 추가합니다.
...
다음 예제 스크립트는 워크플로우 페이로드를 나타내는 JCR 노드에 액세스하는 방법을 보여 줍니다. 이 `graniteWorkflowSession` 변수는 페이로드 경로에서 노드를 가져오는 데 사용되는 JCR 세션 변수에 적용됩니다.
...
동적 참가자 단계 구성 요소에 대한 참가자 선택기 **를 개발할 수** 있습니다.

워크플로우 중에 **동적 참가자 단계** 구성 요소를 시작할 때 생성된 작업 항목을 할당할 수 있는 참가자를 결정해야 합니다. 이 작업을 수행하려면 다음 중 하나를 수행합니다.
...
>[!NOTE]
>
>동적 참가자 단계 구성 요소를 **서비스 또는 스크립트와** 연결하는 방법에 대한 자세한 내용은 [동적 참가자 단계](/help/sites-developing/workflows-step-ref.md#dynamic-participant-step) 또는 단계 구현 [무효화](#persisting-and-accessing-data)를참조하십시오.

### Java 클래스를 사용하여 참가자 선택기 개발 {#developing-a-participant-chooser-using-a-java-class}

참가자 단계를 OSGI 서비스 구성 요소로 정의하려면(Java 클래스):

1. OSGI 구성 요소는 해당 메서드와 `ParticipantStepChooser` 인터페이스를 구현해야 `getParticipant()` 합니다. 아래 예제 코드를 참조하십시오.

   번들을 만들고 OSGI 컨테이너에 배포합니다.

1. SCR 속성을 추가하고 `chooser.label` 필요에 따라 값을 설정합니다. [ **동적 참가자 단계] 구성 요소를 사용하여 참가자 선택자가 나열되는 이름입니다** . 다음 예를 참조하십시오.
...
1. 모델 **** 편집기에서 일반 **동적 참가자 단계** 구성 요소를 사용하여 동적 참가자 단계를 워크플로우에추가합니다.
1. 편집 대화 상자에서 **참가자 선택기** 탭을 선택하고 선택 구현을 선택합니다.
1. 코드에서 인수를 사용하는 경우 **프로세스 인수를 설정합니다**. 이 예제의 경우: `/content/we-retail/de`.
1. 단계 및 워크플로우 모델 모두에 대한 변경 사항을 저장합니다.

### ECMA 스크립트를 사용한 참가자 선택기 개발 {#developing-a-participant-chooser-using-an-ecma-script}

참가자 단계에서 생성하는 작업 항목이 지정된 사용자를 선택하는 ECMA **스크립트를** 생성할 수 있습니다. 스크립트에는 인수가 필요 없는 함수 `getParticipant` 가 포함되어야 하며 사용자 또는 그룹의 ID가 `String` 포함된 함수를 반환합니다.
...
[워크플로우 패키지를](/help/sites-authoring/workflows-applying.md#specifying-workflow-details-in-the-create-workflow-wizard) 처리를 위한 워크플로우로 전달할 수 있습니다. 워크플로우 패키지에는 페이지 및 자산과 같은 리소스에 대한 참조가 포함되어 있습니다.
...
1. 그런 다음 복사한 단계를 /apps 폴더에 넣습니다. 예를 들면 다음과 같습니다.
   ...
   >[!CAUTION]
   >
   >표준 UI에서는, 클래식 UI 편집기에서처럼 제목 및 세부 사항은 카드에 표시되지 `details.jsp` 않으므로,
...
1. 노드에 다음 속성을 적용합니다.
   ...
   * `sling:resourceSuperType`
      ...
      이 예에서는 의 기본 단계에서 상속하지만, 다른 슈퍼 유형 `cq/workflow/components/model/step`을 사용할 수 `participant``process`있습니다.
   ...
   * `cq:icon`
      ...
      단계의 [Coral 아이콘을](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/coral-ui/coralui3/Coral.Icon.html) 지정하는 데 사용됩니다.
...
기본 단계 [를 만든](#creating-the-basic-step)후 다음과 같이 단계 **구성** 대화 상자를 정의합니다.

1. 다음과 같이 노드에서 속성을 `cq:editConfig` 구성합니다.

   **관심 속성:**

   * `cq:inherit`

      로 설정하면 단계 구성 요소 `true`가 지정된 단계에서 속성을 상속합니다 `sling:resourceSuperType`.
...
1. 다음과 같이 노드에서 속성을 `cq:formsParameter` 구성합니다.
...
1. 노드에서 속성을 구성합니다 `cq:listeners`.
   ...
   노드 및 `cq:listener` 해당 속성을 사용하면 터치 지원 UI 모델 편집기에서 이벤트에 반응하는 이벤트 핸들러를 설정할 수 있습니다. 예를 들어 모델 페이지로 단계를 드래그하거나 단계 속성을 편집할 수 있습니다.
   ...
   * `afterinsert: CQ.workflow.flow.Step.afterInsert`
   ...
   이 예에서 상속이 활성화되었으므로 노드를 제거할 수 있으며 `cq:listeners` 단계는 여전히 제대로 작동합니다.
...
사용자 지정 단계의 마크업은 구성 요소 루트 노드 `.content.xml` 에 표시됩니다. 이 예제에 `.content.xml` 사용되는 샘플:
