...
* Clientlibs
   ...
   Clientlibs를 사용하면 표준 함수, 개체 및 메서드를 재사용하면서 새로운 기능을 구현하도록 기본 구현을 확장할 수 있습니다. 사용자 지정할 때 사용자 지정 구성 요소에 필요한 코드를 저장할 수 있는 등 `/apps.` InDesign에서 사용자 고유의 clientlib을 만들 수 있습니다.

* 오버레이

   Overlays는 노드 정의를 기반으로 하며, 사용자 지정 기능(in `/libs`)과 표준 기능(in)을 오버레이할 수 `/apps`있도록 해줍니다. 오버레이를 만들 때는 리소스 병합에서 상속을 허용하므로 원본의 1:1 사본이 필요하지 않습니다.

이러한 기능은 여러 가지 방법으로 AEM 콘솔을 확장하는 데 사용할 수 있습니다. 작은 선택 항목은 아래에서 높은 수준으로 다룹니다.
...
>
>
이 항목에서는 [AEM Gems](https://docs.adobe.com/content/ddc/en/gems.html) 세션 - AEM 6.0용 [사용자 인터페이스 사용자](https://docs.adobe.com/content/ddc/en/gems/user-interface-customization-for-aem-6.html)인터페이스에서도 다룹니다.
...
>[!NOTE]
>
>추가 팁과 도구에 대해서는 기술 자료 문서, [AEM TouchUI 문제](https://helpx.adobe.com/experience-manager/kb/troubleshooting-aem-touchui-issues.html)해결을 참조하십시오.
...
1. 사용자 정의 작업을 위해 고유한 구성 요소를 만들고 해당 클라이언트 라이브러리를 포함할 수 있습니다. 예를 들어 다음 위치에서 **Twitter로** 홍보 작업을 수행합니다.
...
## 특정 그룹에 대한 도구 모음 작업 제한 {#restrict-a-toolbar-action-to-a-specific-group}
...
>[!NOTE]
>
>이 기능은 텍스트 필드 열에 최적화되어 있습니다. 다른 데이터 유형의 경우 오버레이할 수 `cq/gui/components/siteadmin/admin/listview/columns/analyticscolumnrenderer` 있습니다 `/apps`.
...
목록을 적절한 크기로 유지하고 사용 사례와 관련되도록 하기 위해 사용자 지정 술어의 형태로 필터를 구현할 수 있습니다. 자세한 내용은 [이 문서를](/help/sites-developing/customizing-page-authoring-touch.md#filtering-resources) 참조하십시오.
