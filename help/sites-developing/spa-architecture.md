...
AEM의 모든 SPA 프로젝트는 SPA Starter Kit [용 Maven Tranype을 활용해야 합니다](https://github.com/adobe/aem-spa-project-archetype).
...
* **[휴대성](/help/sites-developing/spa-architecture.md#portability)-**모든 구성 요소와 마찬가지로 가능한 한 휴대성을 갖추도록 구축해야 합니다. SPA는 안전하고 재사용 가능한 구성 요소로 구축되어 컨텐츠 구조를 참조하는 정적 경로를 방지해야 합니다.
...
* **[동적 라우팅](#dynamic-routing)-**SPA는 라우팅을 담당하고 AEM은 이를 수신하고 이를 기반으로 구성 요소 데이터를 가져옵니다. 모든 라우팅은 동적이여야 합니다.
...
개발자는 컨텐츠 작성자가 언제든지 경로를 수정할 수 있으므로 컨텐츠 구조를 참조하는 정적 경로를 사용하지 않아야 합니다. 이렇게 하면 라이브러리의 재사용성도 제한되며 AEM 템플릿 편집기의 구조가 컨텐츠가 아닌 다른 위치에 있으므로 사용되지 않습니다.
...
SPA는 동적 컨텐츠 렌더링에만 의존해야 합니다. 이는 AEM이 컨텐츠 구조의 모든 하위 항목을 가져오고 렌더링하는 기본 예측입니다.
...
AEM에서 SPA 개발 [원칙을](/help/sites-developing/spa-architecture.md#spa-development-principles-for-aem) 따르는 경우 지원되는 모든 AEM 컨텐츠 작성 기능과 함께 SPA가 작동합니다.
