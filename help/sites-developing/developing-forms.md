...
이러한 모든 구성 요소는 일련의 기본 [양식 구성](/help/sites-authoring/default-components.md)요소로 구현되며 표준 AEM 설치에서 사용할 수 있습니다.
...
1. 드롭다운 항목 목록을 포함할 다중 값 문자열( `myList``String[]`) 유형의 새 속성(예:)을 추가합니다. 셸 스크립트의 JSP 스크립트 또는 cURL과 같은 스크립트를 사용하여 컨텐츠를 가져올 수도 있습니다.
...
1. 폴더에서 다음 중 하나를 만듭니다.
   ...
   1. 게시물 스크립트.
      ...
      양식을 처리하기 위해 양식이 전송될 때 게시물 스크립트가 호출되며, 양식으로부터 오는 데이터를 처리하는 코드가 포함됩니다 `POST`.
...
스크립트를 작성할 때 사용할 수 있는 API 요소에 대한 자세한 내용은 양식과 관련된 [javadocs를 참조하십시오](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/foundation/forms/package-summary.html).
