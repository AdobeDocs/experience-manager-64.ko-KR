...
이러한 모든 구성 요소는 일련의 기본 [양식 구성 요소를](/help/sites-authoring/default-components.md)사용하여 구현되며 표준 AEM 설치에서 사용할 수 있습니다.
...
1. 폴더에서 다음 중 하나를 만듭니다.
   ...
   1. 게시물 스크립트입니다.
      ...
      양식을 처리하기 위해 양식이 전송될 때 게시물 스크립트가 호출되며 양식에서 `POST`오는 데이터를 처리하는 코드가 포함됩니다.
...
1. 폴더에 다시 추가합니다.
   ...
   1. 필드 추가를 위한 스크립트입니다.
      ...
      양식 시작에 대한 HTML이 작성되면 바로 addfields 스크립트가 호출됩니다. 이를 통해 동작에서 양식 내에 사용자 정의 입력 필드 또는 기타 HTML을 추가할 수 있습니다.
...
1. 이 폴더 내에서 다음 스크립트가 필요할 수 있습니다.
   ...
   * 클라이언트 유효성 검사 스크립트:
      ...
      스크립트 이름은 `clientvalidation.<extension>`예: `clientvalidation.jsp`

      양식 필드가 렌더링될 때 호출됩니다. 클라이언트 javascript를 만들어 클라이언트의 필드를 확인하는 데 사용할 수 있습니다.

   * 서버 유효성 검사 스크립트:

      스크립트 이름은 `servervalidation.<extension>`예: `servervalidation.jsp`

      이 메서드는 양식이 제출될 때 호출됩니다. 이 필드를 제출한 후 서버의 필드를 확인하는 데 사용할 수 있습니다.
...
스크립트를 작성할 때 사용할 수 있는 API 요소에 대한 자세한 내용은 양식과 [관련된](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/foundation/forms/package-summary.html)javadocs를 참조하십시오.
