...
## 프레임워크용 생성된 javascript 정보 {#about-the-generated-javascript-for-frameworks}

페이지가 Adobe Analytics 프레임워크과 연결되어 있고 페이지에 Analytics 모듈에 대한 [참조가 포함되어 있으면](/help/sites-administering/adobeanalytics.md)해당 페이지에 대해 analytics.sitecatalyst.js 파일이 자동으로 생성됩니다.

페이지의 javascript는 s_code.js Adobe Analytics 라이브러리가 정의하는 `s_gi`개체를 만들고 해당 속성에 값을 할당합니다. 객체 인스턴스의 이름은 입니다 `s`. 이 섹션에 표시되는 코드 예제에서는 이 변수에 대한 여러 참조를 `s` 만듭니다.
...
Adobe Analytics에는 프레임워크에서 구성할 수 있는 다양한 사전 정의된 변수가 있습니다*. ** **세트**, **쿠키** Lifetime **,** currencyCode **및** **** trackInlineStatsVariables는 기본적으로 CharacterGeneral Analytics기본 설정 목록에 포함되어 있습니다.

![aa-22](assets/aa-22.png)

목록에 변수 이름과 값을 추가할 수 있습니다. 이러한 사전 정의된 변수와 추가하는 모든 변수는 analytics.sitecatalyst.js 파일에서 `s` 개체의 속성을 구성하는 데 사용됩니다. 다음 예에서는 값의 추가된 `prop10` 속성이 javascript 코드 `CONSTANT` 에서 어떻게 표시되는지 보여줍니다.
...
1. 예를 들어 왼쪽 셀에서 변수 이름을 입력합니다 `prop10`.

1. 예를 들어 오른쪽 열에 변수 값을 입력합니다 `CONSTANT`.

1. 변수를 제거하려면 변수 옆에 있는 (-) 단추를 클릭합니다.

>[!NOTE]
>
>변수 및 값을 입력할 때 올바른 형식 및 맞춤법이 적용되었는지 또는 올바른 값/변수 쌍으로 **호출이 전송되지** 않도록 하십시오. 맞춤법이 틀린 변수와 값은 호출 발생을 방지할 수 있습니다.
>
>이러한 변수가 올바르게 설정되었는지 확인하려면 Adobe Analytics 담당자에게 문의하십시오.

>[!CAUTION]
>
>Adobe Analytics 호출이 올바르게 작동하도록 하기 위해 이 목록의 일부 변수 **는** 필수입니다(예: **currencyCode**, **charSet**).
>
...
[ **일반 분석 설정] 영역의 javascript** 상자에서 무료로 사용할 수 있으므로 Adobe Analytics 프레임워크에 사용자 지정 코드를 추가할 수 있습니다.

![aa-21](assets/aa-21.png)

추가한 코드가 analytics.sitecatalyst.js 파일에 추가됩니다. 따라서 에 정의된 javascript 개체의 인스턴스인 `s` 변수에 액세스할 수 `s_gi` `s_code.js`있습니다. 예를 들어 다음 코드를 추가하는 것은 이전 섹션의 예 `prop10` 인 값 `CONSTANT`이라는 변수를 추가하는 것과 같습니다.

`s.prop10= 'CONSTANT';`

Adobe Analytics [파일의 컨텐츠를 포함하는](/help/sites-developing/extending-analytics-components.md) analytics.sitecatalyst.js `s-code.js` 파일의 코드에는 다음 코드가 포함되어 있습니다.

`if (s.usePlugins) s.doPlugins(s)`

다음 절차에서는 javascript 상자를 사용하여 Adobe Analytics 추적을 사용자 지정하는 방법을 보여 줍니다. Javascript에서 Adobe Analytics 플러그인을 사용해야 하는 경우 AEM에 [통합하십시오](/help/sites-administering/adobeanalytics.md) .

1. 실행될 수 있도록 다음 javascript 코드를 상자에 `s.doPlugins` 추가합니다.
...
1. s_doPlugins 함수에 javascript 코드 **를** 추가합니다.

다음 예에서는 &quot;|&quot;의 일반적인 구분 기호를 사용하여 페이지에서 캡처된 데이터를 계층적 순서로 연결합니다.

Adobe Analytics 프레임워크에는 다음과 같은 구성이 있습니다.

* Adobe Analytics `prop2` 변수는 `pagedata.sitesection` 사이트 속성에 매핑됩니다.

* Adobe Analytics `prop3` 변수는 `pagedata.subsection` 사이트 속성에 매핑됩니다.
...
모든 Adobe Analytics 프레임워크에 통합된 사용자 정의 javascript 코드를 제공합니다. 페이지의 Adobe Analytics 프레임워크에 사용자 지정 [자유 형식 javascript](/help/sites-administering/adobeanalytics.md)가 없으면 /libs/cq/analytics/components/sitecatalyst/config.js.jsp 스크립트에서 생성하는 javascript가 [analytics.sitecatalyst.js](/help/sites-administering/adobeanalytics.md) 파일에 추가됩니다. 기본적으로 스크립트는 주석으로 처리되므로 아무런 효과가 없습니다. 이 코드는 다음 `s.usePlugins` 으로 설정됩니다 `false`.
...
따라서 javascript `s.usePlugins` 를 설정하여 함수의 모든 코드 `true` 가 `s_doPlugins` 실행되도록 해야 합니다. 코드를 사용자 정의하려면 자체 javascript를 사용하는 파일과 함께 config.js.jsp 파일을 오버레이합니다. Javascript에서 Adobe Analytics 플러그인을 사용해야 하는 경우 AEM에 [통합하십시오](/help/sites-administering/adobeanalytics.md) .
...
1. sitecatalyst 구성 요소를 마우스 오른쪽 단추로 클릭하고 만들기 > 파일 만들기를 클릭합니다. 이름을 지정한 `config.js.jsp` 다음 확인을 클릭합니다.
...
1. 함수에서 실행할 javascript 코드를 `s_doPlugins` 추가한 다음 모두를 클릭합니다.
...
Adobe Analytics 플러그인에 대한 javascript 코드를 입수하고 이를 AEM의 Adobe Analytics 프레임워크에 통합합니다. 사용자 지정 javascript 코드에서 사용할 수 있도록 해당 범주의 클라이언트 라이브러리 폴더에 코드를 `sitecatalyst.plugins` 추가합니다.

예를 들어 플러그인을 통합하면 사용자 지정 javascript의 `getQueryParams` `s_doPlugins` 함수에서 플러그인을 호출할 수 있습니다. 다음 예제 코드는 Adobe Analytics 호출이 트리거될 때 레퍼러의 URL에서 **&quot;pid&quot;****로 쿼리 문자열을** eVar1으로 전송합니다.
...
>[!NOTE]
>
>플러그인에 사용할 새 클라이언트 라이브러리 폴더를 만듭니다. 폴더에 플러그인을 추가하지 `/libs/cq/analytics/clientlibs/sitecatalyst/plugins` 마십시오. 이렇게 하면 AEM을 다시 설치하거나 업그레이드 작업을 하는 동안 카테고리에 대한 기여도를 `sitecatalyst.plugins` 덮어쓰지 않습니다.
