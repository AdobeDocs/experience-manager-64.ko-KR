...
* 주제: 작업 주제입니다.
* 이름: (선택 사항)
* 속성 맵: 입력 페이로드 경로 및 출력 페이로드 경로 등 많은 속성을 포함하는 `Map<String, Object>` 객체입니다. 이 맵 개체는 작업을 실행하는 JobConsumer 개체에서 사용할 수 있습니다.
...
항목 및 페이로드에 대해 JobGeneratorImpl.createJob이 호출될 때 로그에 다음 메시지가 `com/adobe/example/offloading` `/content/geometrixx/de/services` 포함됩니다.
...
작업을 소비하려면 `org.apache.sling.event.jobs.consumer.JobConsumer` 인터페이스를 구현하는 OSGi 서비스를 개발하십시오. 속성을 사용하여 사용할 항목을 `JobConsumer.PROPERTY_TOPICS` 확인합니다.

다음 예인 JobConsumer 구현이 `com/adobe/example/offloading` 주제에 등록합니다. 소비자는 페이로드 컨텐츠 노드의 소비됨 속성을 true로 설정하기만 하면 됩니다.
