...
description: 페이지 정보를 얻으려면 PageInfo 서블릿으로 요청을 보내 JSON 형식의 페이지 메타데이터를 얻으십시오
seo-description: 페이지 정보를 얻으려면 PageInfo 서블릿으로 요청을 보내 JSON 형식의 페이지 메타데이터를 얻으십시오
...
페이지 정보를 얻으려면 PageInfo 서블릿으로 요청을 전송하여 페이지 메타데이터를 JSON 형식으로 가져옵니다.

PageInfo 서블릿은 응답의 리소스에 대한 정보를 반환합니다. 서블릿은 URL에 바인딩되며 `https://<server>:<port>/libs/wcm/core/content/pageinfo.json` 이 `path` 매개 변수를 사용하여 리소스를 식별합니다. 다음 URL은 `/content/we-retail/us/en` 노드에 대한 정보를 반환합니다.
...
>[!NOTE]
>
>PageInfoProviders와 마찬가지로 ListInfoProviders를 사용하여 JSON 형식의 정보 목록을 업데이트합니다. (웹 [사이트 관리 콘솔 사용자 지정을 참조하십시오](/help/sites-developing/customizing-siteadmin.md).)

## 기본 페이지 정보 공급자 {#default-page-information-providers}

이 `/libs/foundation/components/page` 구성 요소는 다음 PageInfoProvider 서비스와 연결됩니다.

* **** 기본 페이지 상태 공급자:페이지 상태(예: 페이지 잠김 여부, 페이지가 활성 워크플로의 페이로드인지 여부, 페이지에 사용할 수 있는 워크플로우) 정보.
* **** 라이브 관계 정보 공급자:페이지가 파란색 인쇄에 속하는지 여부, Live Copy인지 여부 등 다중 사이트 관리(MSM)에 대한 정보입니다.
...
원하는 워크플로 패키지에 대한 정보만 반환하도록 CQ WCM Workflow Package Info Provider 서비스를 구성합니다. 기본적으로 워크플로우 패키지 정보 제공자 서비스는 저장소의 모든 워크플로우 패키지에 대한 정보를 반환합니다. 워크플로우 패키지의 하위 집합을 반복하면 서버 리소스가 줄어듭니다.

>[!NOTE]
>
>사이드 킥의 워크플로우 탭에서는 PageInfo 서블릿을 사용하여 워크플로우 패키지 목록을 가져옵니다. 목록에서 현재 페이지를 추가할 패키지를 선택할 수 있습니다. 만든 필터는 이 목록에 영향을 줍니다.


서비스의 ID가 `com.day.cq.wcm.workflow.impl.WorkflowPackageInfoProvider`있습니다. 필터를 만들려면 `workflowpackageinfoprovider.filter` 속성 값을 지정합니다.
...
>[!NOTE]
>
>AEM을 사용하여 작업할 때 이러한 서비스에 대한 구성 설정을 관리하는 몇 가지 방법이 있습니다. 자세한 [내용은 OSGi](/help/sites-deploying/configuring-osgi.md) 구성을 참조하십시오.

예를 들어 CRXDE Lite를 사용하여 서비스를 구성하려면:
...
1. 프로젝트 소스에서 AEM 응용 프로그램의 구성 폴더를 찾거나 만듭니다.
   ...
   예를 들어 Content Package Maven 플러그인의 다중 모듈 전형 형식을 사용하여 프로젝트를 만든 경우 폴더 경로는 `<projectroot>/content/src/ for example content/src/main/content/jcr_root/apps/<appname>/config`입니다.
...
1. 속성을 둘러싸는 대괄호(`[]`)에 `workflowpackageinfoprovider.filter` 다음 예와 유사한 필터 값의 쉼표로 구분된 목록을 입력합니다.
...
1. 애플리케이션에서 페이지 구성 요소를 만듭니다. 속성 값으로 `foundation/components/page` 사용합니다 `sling:resourceSuperType` .

1. 이름이 `cq:infoProviders`지정된 구성 요소 노드 아래에 노드를 추가합니다.
1. 노드 아래에 PageInfoProvider 서비스에 대한 노드를 추가합니다. `cq:infoProviders` 노드의 이름을 지정할 수 있습니다.
...
응용 프로그램 페이지 구성 요소를 응용 프로그램 페이지 구성 요소로 사용하는 리소스의 경우 PageInfo 서블릿은 기본 PageInfoProvider 메타데이터와 `sling:resourceType`더불어 사용자 지정 PageInfoProvider 메타데이터를 반환합니다.

### PageInfoProvider 구현 예 {#example-pageinfoprovider-implementation}

다음 Java 클래스는 PageInfoProvider [를](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/index.html) 구현하고 현재 페이지 리소스의 게시된 URL을 반환합니다.
...
다음 예제는 CRXDE Lite에서 PageUrlInfoProvider 서비스를 사용하도록 구성된 페이지 구성 요소를 보여줍니다.
