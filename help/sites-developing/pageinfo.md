...
description: 페이지 정보를 얻으려면 페이지 메타데이터를 JSON 형식으로 얻기 위해 PageInfo 서블릿으로 요청을 보냅니다
seo-description: 페이지 정보를 얻으려면 페이지 메타데이터를 JSON 형식으로 얻기 위해 PageInfo 서블릿으로 요청을 보냅니다
...
페이지 정보를 얻으려면 페이지 메타데이터를 JSON 형식으로 얻기 위해 PageInfo 서블릿으로 요청을 보냅니다.

PageInfo 서블릿은 저장소의 리소스에 대한 정보를 반환합니다. 서블릿은 URL에 바인딩되며 매개 변수 `https://<server>:<port>/libs/wcm/core/content/pageinfo.json` `path` 를 사용하여 리소스를 식별합니다. 다음 예제 URL은 노드에 대한 정보를 `/content/we-retail/us/en` 반환합니다.
...
>[!NOTE]
>
>일반적인 AEM 웹 페이지가 아닌 채널에 컨텐츠를 전달하기 위해 JSON 형식의 페이지 정보가 필요한 경우:
>
...
>
>
콘텐츠 서비스용 [JSON 내보내기 문서를 참조하십시오](/help/sites-developing/json-exporter.md).

## 페이지 정보 제공업체 {#page-information-providers}
...
1. 서블릿은 각 PageInfoProvider가 반환하는 메타데이터를 집계하고 JSON 개체의 HTTP 응답에 메타데이터를 추가합니다.

![chlimage_1-2](assets/chlimage_1-2.png)

>[!NOTE]
>
>PageInfoProviders와 유사하게, ListInfoProviders를 사용하여 JSON 형식의 정보 목록을 업데이트합니다. 웹 사이트 관리 콘솔 [사용자 정의를 참조하십시오](/help/sites-developing/customizing-siteadmin.md).

## 기본 페이지 정보 공급자 {#default-page-information-providers}

구성 요소 `/libs/foundation/components/page` 는 다음 PageInfoProvider 서비스와 연결됩니다.

* **기본 페이지 상태 공급자:** 페이지 상태(예: 페이지 잠김 여부, 페이지가 활성 워크플로의 페이로드인지 여부, 페이지에 사용할 수 있는 워크플로우) 정보
* **라이브 관계 정보 공급자:** 페이지가 파란색 인쇄물에 속하는지 여부 및 Live Copy인지 여부 등 다중 사이트 관리(MSM)에 대한 정보입니다.
* **컨텐츠 언어 서블릿:** 현재 페이지의 언어 및 페이지를 사용할 수 있는 각 언어에 대한 정보입니다.
* **워크플로 상태 공급자:** 페이지가 페이로드로 있는 실행 중인 워크플로우에 대한 상태 정보입니다.
* **워크플로 패키지 정보 공급자:** 저장소에 저장된 각 워크플로우 패키지 및 각 패키지에 현재 리소스가 포함되어 있는지 여부에 대한 정보입니다.
...
예를 들어 PageInfo 서블릿은 노드에 대해 다음 JSON 응답을 `/content/we-retail/us/en` 반환합니다.
...
## 워크플로우 패키지 정보 필터링 {#filtering-workflow-package-information}

원하는 워크플로우 패키지에 대한 정보만 반환하도록 CQ WCM Workflow 패키지 정보 제공자 서비스를 구성합니다. 기본적으로 Workflow Package Info Provider 서비스는 저장소의 모든 워크플로우 패키지에 대한 정보를 반환합니다. 워크플로우 패키지 하위 집합을 반복하여 더 적은 서버 리소스를 사용합니다.
...
서비스의 ID입니다 `com.day.cq.wcm.workflow.impl.WorkflowPackageInfoProvider`. 필터를 만들려면 속성 값을 `workflowpackageinfoprovider.filter` 지정합니다.

속성 값 앞에 + 또는 - 문자가 추가되고 패키지 경로가 추가됩니다.

* 경로는 워크플로우 패키지의 루트 노드의 경로입니다. 경로는 FileVault 구문을 사용합니다.
* 패키지를 포함하려면 + 접두사를 사용하십시오.
* 패키지를 제외하려면 - 접두사를 사용하십시오.

서비스는 모든 필터의 누적 결과를 적용합니다. 예를 들어 다음 필터 값은 Editions 폴더에 있는 패키지를 제외한 모든 워크플로우 패키지를 제외합니다.
...
>[!NOTE]
>
>AEM에서 작업할 때는 이러한 서비스에 대한 구성 설정을 관리하는 방법이 몇 가지 있습니다. 자세한 [내용은 OSGi](/help/sites-deploying/configuring-osgi.md) 구성을 참조하십시오.

예를 들어 CRXDE Lite을 사용하여 서비스를 구성하려면 다음을 수행하십시오.
...
1. 프로젝트 소스에서 AEM 응용 프로그램의 구성 폴더를 찾거나 만듭니다.
   ...
   예를 들어 Content Package Maven Plugin의 다중 모듈 원형 유형을 사용하여 프로젝트를 만든 경우 폴더 경로가 됩니다 `<projectroot>/content/src/ for example content/src/main/content/jcr_root/apps/<appname>/config`.
...
1. 속성을 둘러싸는 대괄호(`[]`)에 다음 `workflowpackageinfoprovider.filter` 예와 유사한 필터 값의 쉼표로 구분된 목록을 입력합니다.
...
1. 인터페이스를 `com.day.cq.wcm.api.PageInfoProvider` 구현합니다.
1. 클래스를 번들로 묶어서 OSGi 서비스로 배포합니다.
1. 애플리케이션에서 페이지 구성 요소를 만듭니다. 속성 `foundation/components/page` 의 값으로 `sling:resourceSuperType` 사용합니다.

1. 이름이 지정된 구성 요소 노드 아래에 노드를 추가합니다 `cq:infoProviders`.
1. 노드 아래에 `cq:infoProviders` PageInfoProvider 서비스의 노드를 추가합니다. 노드의 이름을 지정할 수 있습니다.
...
애플리케이션 페이지 구성 요소를 기본 PageInfo 메타데이터와 함께 PageInfo 서블릿은 사용자 지정 PageInfoProvider 메타데이터 `sling:resourceType`를 반환합니다.

### PageInfoProvider 구현 예 {#example-pageinfoprovider-implementation}

다음 Java 클래스는 [PageInfoProvider를 구현하고](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/index.html) 현재 페이지 리소스의 게시된 URL을 반환합니다.
...
다음 예는 CRXDE Lite에서 PageUrlInfoProvider 서비스를 사용하도록 구성된 페이지 구성 요소를 보여줍니다.

![chlimage_1-3](assets/chlimage_1-3.png)

PageUrlInfoProvider 서비스는 노드에 대해 다음 데이터를 `/content/we-retail/us/en` 반환합니다.
