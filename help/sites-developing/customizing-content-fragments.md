...
* [컨텐츠 조각 만들기 및 관리](/help/assets/content-fragments.md) 및 컨텐츠 조각으로 [페이지](/help/sites-authoring/content-fragments.md) 작성및 관리를 참조하십시오.

* [자산 관리](/help/assets/managing-assets-touch-ui.md) 및 [자산 사용자](/help/assets/extending-assets.md) 지정 및 확장을 참조하십시오.

## 아키텍처 {#architecture}

컨텐츠 조각의 기본 [구성](/help/assets/content-fragments.md#constituent-parts-of-a-content-fragment) 부분은 다음과 같습니다.
...
>[!CAUTION]
>
>[이제 컨텐츠 조각 모델을](/help/assets/content-fragments-models.md) 사용하여 모든 조각을 만드는 것이 좋습니다.
>
...
* 모든 컨텐츠는 자산의 노드 아래에 `jcr:content/data` 저장됩니다.
   ...
   * 각 요소의 데이터는 해당 하위 노드에 요소 이름이 있는 속성으로 저장됩니다.
      ...
      예: 요소의 컨텐츠 `text` `text` 가 `jcr:content/data/master`
...
OSGi 번들 [DAM 컨텐츠 조각 구성에 대해](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console)웹 콘솔에서 **매개 변수를 구성할 수 있습니다**.

* **리소스 유형**

   컨텐츠 조각을 렌더링하는 데 사용되는 구성 요소와 배경 처리를 적용할 위치를 정의하는 데 사용할 수 있는 목록을 제공할 `sling:resourceTypes` 수 있습니다.
...
![osgi 구성](assets/osgi-config.png)
...
* `ContentFragment` 을(를) 다음과 같이 적용할 수 있습니다.
   ...
   * `Asset` - 컨텐츠 조각을 나타내는 DAM `Asset` 추상화 를 `Asset` 직접 업데이트하려면 개체를 다시 `ContentFragment` 구성해야 합니다.
...
* 추가 작업이 필요할 수 있는 작업:
   ...
   * 새 요소를 만들거나 제거해도 간단한 조각(조각 템플릿 기반)의 데이터 구조가 업데이트되지 않습니다.
...
* 페이지 입력
   ...
   * 컨텐츠 조각이 페이지에서 이미 참조되었는지 확인하고, 참조한 경우 적절한 정보를 표시합니다.
