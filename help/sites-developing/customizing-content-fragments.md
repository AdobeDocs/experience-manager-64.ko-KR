...
* [컨텐츠 조각 만들기 및 관리](/help/assets/content-fragments.md) 및 [컨텐츠 조각으로 페이지 작성](/help/sites-authoring/content-fragments.md) 컨텐츠 조각에 대한 자세한 정보.

* [자산 관리](/help/assets/managing-assets-touch-ui.md) 및 [자산 사용자 지정 및 확장](/help/assets/extending-assets.md) 표준 자산에 대한 추가 정보.

## 아키텍처 {#architecture}

기본 [구성 부분](/help/assets/content-fragments.md#constituent-parts-of-a-content-fragment) 컨텐츠 조각의 유형은 다음과 같습니다.
...
* 모든 컨텐츠는 `jcr:content/data` 자산의 노드:
   ...
   * 각 요소의 데이터는 각 하위 노드에 요소 이름을 사용하는 속성으로 저장됩니다.
      ...
      예: 요소의 컨텐츠 `text` 는 속성으로 저장됩니다. `text` on `jcr:content/data/master`
...
자세한 내용은 [컨텐츠 조각 - 삭제 고려 사항](/help/assets/content-fragments-delete.md).
...
이에 대한 매개 변수는 [웹 콘솔](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console), OSGi 번들 **DAM 컨텐츠 조각 구성**.

* **리소스 유형**

   목록 `sling:resourceTypes` 컨텐츠 조각 렌더링에 사용되는 구성 요소와 배경 처리를 적용해야 하는 위치를 정의하는 데 제공될 수 있습니다.
