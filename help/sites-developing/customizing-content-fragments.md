...
* 모든 컨텐츠는 자산의 `jcr:content/data` 노드 아래에 저장됩니다.
   ...
   * 각 요소의 데이터는 해당 하위 노드에 요소 이름이 있는 속성으로 저장됩니다.
      ...
      예를 들어 요소의 컨텐츠는 `text` 다음 `text` 위치에 속성으로 저장됩니다. `jcr:content/data/master`
...
OSGi 번들 DAM 컨텐츠 조각 [구성에](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console)대해 웹 콘솔에서 매개 변수를 구성할 **수 있습니다**.

* **리소스 유형**

   컨텐츠 조각을 렌더링하는 데 사용되는 구성 요소와 백그라운드 처리가 적용되는 위치를 정의하는 데 목록을 제공할 `sling:resourceTypes` 수 있습니다.
