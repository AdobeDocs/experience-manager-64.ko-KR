...
We.Retail 미디어 샘플에서는 이 전략을 사용하여 사이트 디자인에서 스타일을 정의합니다. We.Retail에서 사용되는 CSS 파일은 에 있습니다 `*/apps/weretail/clientlibs/clientlib-site/less/grid.less`.
...
반응형 디자인의 동작을 테스트하려면 다양한 뷰포트 크기로 페이지 미리 보기를 참조하십시오. 미리 **[!UICONTROL 보기]****** 모드에서 **[!UICONTROL 사이드]** 킥에는 장치를선택하는 데 사용하는장치드롭다운 메뉴가 포함되어 있습니다. 장치를 선택하면 페이지가 뷰포트 크기에 맞게 변경됩니다.

![chlimage_1-5](assets/chlimage_1-5.png)

사이드 킥에서 장치 미리 보기를 **[!UICONTROL 활성화하려면]**&#x200B;페이지와 **[!UICONTROL MobileEmulatorProvider]** 서비스를 구성해야 합니다. 다른 페이지 구성은 장치 목록에 나타나는 장치 목록을 **[!UICONTROL 제어합니다]** .

### 장치 목록 추가 {#adding-the-devices-list}

[ **[!UICONTROL 장치]** ] 목록 **[!UICONTROL 은]** 페이지에 [장치] **[!UICONTROL 목록을 렌더링하는 JSP 스크립트가 포함된 경우]** 사이드 킥에나타납니다. 사이드 킥에 **[!UICONTROL 장치]** 목록 **[!UICONTROL 을]**&#x200B;추가하려면 `/libs/wcm/mobile/components/simulator/simulator.jsp` 페이지 `head` 섹션에 스크립트를 포함시킵니다.

섹션을 정의하는 JSP에 다음 코드를 `head` 포함합니다.

`<cq:include script="/libs/wcm/mobile/components/simulator/simulator.jsp"/>`

예를 보려면 CRXDE Lite에서 `/apps/weretail/components/page/head.jsp` 파일을 엽니다.

### 시뮬레이션을 위해 페이지 구성 요소 등록 {#registering-page-components-for-simulation}

장치 시뮬레이터가 페이지를 지원할 수 있도록 하려면 MobileEmulatorProvider 팩토리 서비스에 페이지 구성 요소를 등록하고 속성을 `mobile.resourceTypes` 정의하십시오.
...
* 이름: `com.day.cq.wcm.mobile.core.impl.MobileEmulatorProvider-*alias*`
   ...
   MobileEmulatorProvider 서비스는 팩터리 서비스이므로 - 접미사가 필요합니다 `*alias*` . 이 팩터리에 대해 고유한 별칭을 사용하십시오.
...
장치 목록에 나타나는 장치 그룹을 지정하려면 사이트의 루트 페이지 `cq:deviceGroups` 의 `jcr:content` 노드에 속성을 추가합니다. 속성 값은 장치 그룹 노드에 대한 경로 배열입니다.

장치 그룹 노드는 `/etc/mobile/groups` 폴더에 있습니다.

예를 들어 Geometrixx Media 사이트의 루트 페이지는 다음과 같습니다 `/content/geometrixx-media`. 노드에는 `/content/geometrixx-media/jcr:content` 다음 속성이 포함됩니다.
...
W3C에서 정의하는 [MediaQueryList 인터페이스](https://dev.w3.org/csswg/cssom-view/#the-mediaquerylist-interface) 구현으로 javascript를 사용하여 미디어 쿼리를 평가할 수 있습니다. 미디어 쿼리 결과에 로직을 적용하고 현재 창에 대해 타깃팅된 스크립트를 실행할 수 있습니다.

* MediaQueryList 인터페이스를 구현하는 브라우저는 함수를 `window.matchMedia()` 지원합니다. 이 함수는 지정된 문자열에 대해 미디어 쿼리를 테스트합니다. 함수는 쿼리 결과에 대한 액세스를 제공하는 `MediaQueryList` 객체를 반환합니다.

* 인터페이스를 구현하지 않는 브라우저의 경우 자유롭게 사용할 수 있는 javascript 라이브러리인 `matchMedia()` matchMedia.js [와 같은](https://github.com/paulirish/matchMedia.js)polyfill을 사용할 수 있습니다.

#### 미디어별 리소스 선택 {#selecting-media-specific-resources}

W3C에서 제안한 [그림 요소는](https://picture.responsiveimages.org/) 미디어 쿼리를 사용하여 이미지 요소에 사용할 소스를 결정합니다. 그림 요소는 요소 속성을 사용하여 미디어 쿼리를 이미지 경로와 연결합니다.
...
페이지가 렌더링되면 pictureflow.js는 요소를 요소의 마지막 `img` 자식으로 `<div data-picture>` 삽입합니다.
...
* [jquery.devounceedsize.js](https://github.com/louisremi/jquery-smartresize) (창 크기를 조정한 후 발생하는 jquery 이벤트)

**팁:** 여러 클라이언트 라이브러리 폴더를 임베드하여 자동으로 연결할 수 [있습니다](/help/sites-developing/clientlibs.md#embedding-code-from-other-libraries).
...
이 `path` 변수에는 현재 리소스(응용 이미지 구성 요소 노드)의 경로가 포함됩니다. 코드는 다음과 같은 구조를 가진 일련의 `div` 요소를 생성합니다.

`<div data-scr = "*path-to-parent-node*.adaptive-image.adapt.*width*.*quality*.jpg" data-media="*media query*"></div>`

속성의 값 `data-scr` 은 Sling이 이미지를 렌더링하는 응용 이미지 구성 요소 서블릿으로 확인되는 URL입니다. data-media 속성에는 클라이언트 속성에 대해 평가되는 미디어 쿼리가 포함됩니다.
...
예를 들어 Sling은 URL이 포함된 HTTP 요청을 응용 이미지 구성 요소 서블릿으로 `http://localhost:4502/content/geometrixx/adaptiveImage.img.jpg` 전달합니다.
...
이 `com.day.cq.wcm.foundation.impl.AdaptiveImageComponentServlet` 클래스는 [AbstractImageServlet 클래스를](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/commons/AbstractImageServlet.html) 확장합니다. AdaptiveImageComponentServlet 소스 코드는 `/libs/foundation/src/impl/src/com/day/cq/wcm/foundation/impl` 폴더에 있습니다.
...
이 `AbstractImageServlet` 클래스는 HTTP 요청을 처리하는 `doGet` 메서드를 제공합니다. 이 메서드는 요청과 연결된 리소스를 결정하고 저장소에서 리소스 속성을 검색하고 [ImageContext](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/commons/AbstractImageServlet.ImageContext.html) 개체에서 반환합니다.

>[!NOTE]
>
>com.day.cq.com [mons.DownloadResource](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/commons/DownloadResource.html) 클래스는 리소스 `getFileReference method`속성의 값을 검색하는 `fileReference` 변수를 제공합니다.

이 `AdaptiveImageComponentServlet` 클래스는 메서드를 `createLayer` 무시합니다. 이 메서드는 `ImageContext` 개체에서 이미지 리소스의 경로와 요청된 이미지 너비를 가져옵니다. 그런 다음 실제 이미지 크기 조정을 수행하는 `info.geometrixx.commons.impl.AdaptiveImageHelper` 클래스의 메서드를 호출합니다.
...
서블릿은 리소스에 바인딩되며 .jpg 파일 확장명을 `cq:page` 지원합니다. 서블릿 선택기가 입니다 `image`. 따라서 Sling은 다음 형식의 HTTP 요청 URL을 이 서블릿으로 확인합니다.
...
* 상위 노드는 리소스 `jcr:content` 의 `cq:page` 노드입니다.

* 이미지 경로는 이름이 지정된 속성의 값으로 저장됩니다 `fileReference`.

페이지를 작성할 때 **사이드** 킥을 사용하여 이미지를 지정하고 페이지 속성에 `image` 노드를 추가합니다.
...
이 `AbstractImageServlet` 클래스는 HTTP 요청을 처리하는 `doGet` 메서드를 제공합니다. 이 메서드는 호출과 연결된 리소스를 결정하고 저장소에서 리소스 속성을 검색하고 [ImageContext](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/commons/AbstractImageServlet.ImageContext.html) 개체에 저장합니다.

이 `ImageReferenceModificationServlet` 클래스는 `createLayer` 메서드를 재정의하고 렌더링할 이미지 리소스를 결정하는 논리를 구현합니다. 이 메서드는 이름이 지정된 페이지 노드의 하위 노드를 `jcr:content` 검색합니다 `image`. 이 [노드에서](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/foundation/Image.html) 이미지 `image` 객체가 생성되며 이 `getFileReference` 메서드는 이미지 노드의 `fileReference` 속성에서 이미지 파일 경로를 반환합니다.

>[!NOTE]
>
>com.day.cq.com [mons.DownloadResource](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/commons/DownloadResource.html) 클래스는 getFileReference 메서드를 제공합니다.
...
유동 격자에 익숙하지 않은 경우 이 페이지 하단의 [유동 격자](/help/sites-developing/responsive.md#developing-a-fluid-grid) 소개 섹션을 참조하십시오. 이 소개에서는 유동 격자에 대한 개요와 이러한 격자 디자인을 위한 지침을 제공합니다.
...
* 첫 번째 행에는 클래스의 `div` 요소 `span12` (컨텐츠는 열 12개)가 포함됩니다. 요소에는 parsys 구성 요소가 `div` 포함되어 있습니다.

* 두 번째 행에는 두 개의 `div` 요소, 하나는 클래스 `span8` 와 다른 하나는 클래스 `span4`. 각 `div` 요소에는 parsys 구성 요소가 포함됩니다.
...
geometrixx-media 페이지 구성 요소(`/etc/designs/geometrixx-media`)와 연결된 디자인에는 ClientLibraryFolder가 `clientlibs` 포함되어 있습니다. 이 ClientLibraryFolder는 클래스의 자식인 클래스, 클래스 `row-fluid` 및 `span*` 클래스에 대한 CSS 스타일을 `span*` `row-fluid` 정의합니다. 미디어 쿼리를 사용하면 다양한 뷰포트 크기에 맞게 스타일을 재정의할 수 있습니다.

다음 예제 CSS는 이러한 스타일의 하위 집합입니다. 이 하위 세트는 두 개의 뷰어 크기 `span12`에 대한 미디어 쿼리, `span8`및 클래스 `span4` 에 중점을 둡니다. CSS의 다음 특성을 확인합니다.
...
* 작은 뷰포트에 대한 미디어 쿼리는 동일한 클래스에 대한 `float:none` 스타일을 할당합니다.
...
본문 내 섹션, 섹션 등 페이지의 다양한 부분 `head` 을 생성하는 스크립트 `body` 를 제공하는 기본 페이지 구성 요소 `header`를 `content`만듭니다 `footer` .

기본 페이지 구성 요소를 사용하여 다른 페이지 구성 요소를 만듭니다 `cq:resourceSuperType`. 이러한 구성 요소에는 필요에 따라 기본 페이지의 스크립트를 무시하는 스크립트가 포함되어 있습니다.

예를 들어, gometrixx-media 응용 프로그램에는 페이지 구성 요소(기본 페이지 구성 요소임) `sling:resourceSuperType` 가 포함됩니다. 아티클, 카테고리 및 media-home과 같은 여러 하위 구성 요소는 이 페이지 구성 요소를 로 사용합니다 `sling:resourceSuperType`. 각 하위 구성 요소에는 페이지 구성 요소의 content.jsp 파일을 무시하는 content.jsp 파일이 포함되어 있습니다.

**스크립트 재사용**

여러 페이지 구성 요소에 공통인 행 및 열 조합을 생성하는 여러 JSP 스크립트를 만듭니다. 예를 들어, 아티클 및 media-home 구성 요소의 `content.jsp` 스크립트는 모두 스크립트를 `8x4col.jsp` 참조합니다.
...
필요한 경우 구성 요소는 생성하는 HTML 코드에 필요한 CSS 또는 javascript 라이브러리를 제공해야 합니다. 구성 요소 내의 클라이언트 라이브러리 폴더를 사용하여 CSS 및 JS 파일을 생성합니다. 파일을 노출하려면 종속성을 [만들거나 /etc 폴더 아래의 다른 클라이언트 라이브러리 폴더에 라이브러리를](/help/sites-developing/clientlibs.md#creating-client-library-folders) 포함합니다.
...
예를 들어 구성 요소는 두 개의 컨텐츠 열을 `/apps/geometrixx-media/components/2-col-article-summary` 생성합니다. 생성되는 HTML에는 다음 구조가 있습니다.
...
페이지 `.row-fluid .span6` CSS의 선택기는 이 HTML에서 동일한 클래스 및 구조의 `div` 요소에 적용됩니다. 그러나 구성 요소에는 /apps/geometrixx-media/components/2-col-article-summary/clientlibs 클라이언트 라이브러리 폴더도 포함되어 있습니다.

* CSS는 페이지 구성 요소와 동일한 미디어 쿼리를 사용하여 동일한 개별 페이지 너비로 레이아웃 변경을 설정합니다.
* 선택기 `multi-col-article-summary` 는 행 `div` 요소의 클래스를 사용하여 페이지 `row-fluid` 클래스의 동작을 덮어씁니다.
...
* HTML `div` 요소에는 특정 개수의 열을 확장한 컨텐츠 블록이 포함됩니다.
...
#### 격자에 컨텐츠 위치 변경 {#repositioning-content-in-the-grid}
...
* 서로 다른 너비의 컨텐츠 블록을 나타내는 요소에 대한 `div` 클래스입니다. 폭은 상위(행)의 백분율로 표시됩니다.
...
1. 와 같은 행을 나타내는 div 요소에 대한 CSS 클래스를 정의합니다 `row-fluid`.
1. 콘텐츠 블록 요소의 클래스 이름을 정의합니다. 열 범위에서 가능한 모든 너비에 대해 클래스가 필요합니다. 예를 들어 3개의 열에 걸쳐 `span3` 있는 `div` 요소에 대해서는 클래스를 사용하고, 열 `span4` 의 범위는 4개여야 합니다. 그리드에 열 수만큼 클래스를 정의합니다.

1. 타깃팅하는 각 뷰포트 크기에 대해 해당 미디어 쿼리를 CSS 파일에 추가합니다. 각 미디어 쿼리에 다음 항목을 추가합니다.

   * 예를 들어 `content` 클래스에 대한 선택기입니다 `.content{}`.
   * 예를 들어 각 span 클래스에 대한 선택기 `.span3{ }`.
   * 예: `row-fluid` 클래스의 선택기 `.row-fluid{ }`
   * 예를 들어 행-유동 클래스 내에 있는 범위 클래스에 대한 선택기 `.row-fluid span3 { }`.
...
1. 각 선택기에 너비 스타일 추가:
   ...
   1. 선택기 너비를 `.row-fluid .span` 전체 너비의 비율로 설정합니다. 공식을 사용하여 이 너비를 `(absolute span width)/(absolute page width)*100` 계산합니다.

#### 행에 컨텐츠 블록 배치 {#positioning-content-blocks-in-rows}

클래스의 부동 스타일을 사용하여 행의 콘텐츠 블록이 가로로 또는 세로로 배열되는지 여부를 `.row-fluid` 제어할 수 있습니다.

* 이 `float:left` 또는 `float:right` 스타일로 인해 하위 요소(콘텐츠 블록)가 가로 분포됩니다.
