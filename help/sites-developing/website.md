...
>[!NOTE]
>
>다운로드하려면 다음 링크를 클릭하십시오 ``mywebsite.zip``. 아카이브는 디자인에 사용할 static.css 및 이미지 파일을 포함합니다.

정적.css 파일 및 이미지 샘플

[파일 가져오기](assets/mywebsite.zip)

1. AEM 시작 페이지에서 **[!UICONTROL 도구를 클릭합니다]**. ([http://localhost:4502/libs/cq/core/content/welcome.html](http://localhost:4502/libs/cq/core/content/welcome.html))

   ![chlimage_1-102](assets/chlimage_1-102.png)

1. 폴더 트리에서 **[!UICONTROL 디자인]** 폴더를 선택한 다음 **[!UICONTROL 새로 만들기 > 새 페이지를 클릭합니다]**. 제목으로 입력하고 만들기 `mywebsite` 를 클릭합니다 ****.

1. mywebsite 항목이 표에 나타나지 않으면 트리 또는 테이블을 새로 고칩니다.

1. [http://localhost:4502의 URL에 대한 WebDAV](/help/sites-administering/webdav-access.md) 액세스를 사용하여 다운로드한 mywebsite.zip 파일의 샘플 `static.css` 파일과 `images` 폴더를 `/etc/designs/mywebsite` 폴더로 복사합니다.
...
1. 템플릿 만들기 대화 상자에서 다음 값을 입력한 다음 **[!UICONTROL 다음을 클릭합니다]**.
   ...
   * **[!UICONTROL 리소스 유형]**: mywebsite/components/contentpage
   ...
   리소스 유형은 페이지를 렌더링하는 구성 요소를 식별합니다. 이 경우 콘텐츠 페이지 템플릿을 사용하여 만든 모든 페이지는 구성 요소에 의해 `mywebsite/components/contentpage` 렌더링됩니다.

1. 이 템플릿을 사용할 수 있는 페이지의 경로를 지정하려면 더하기 단추를 클릭하고 나타나는 텍스트 상자 `/content(/.*)?` 에 입력합니다. 그런 다음 **[!UICONTROL 다음을 클릭합니다]**.

   ![chlimage_1-105](assets/chlimage_1-105.png)

   허용되는 경로 속성의 값은 *정규식입니다.* 표현식과 일치하는 경로가 있는 페이지는 템플릿을 사용할 수 있습니다. 이 경우 정규 표현식은 `/content` 폴더 경로 및 모든 하위 페이지와 일치합니다.

   작성자가 아래 페이지 `/content`를 만들면 **[!UICONTROL 컨텐츠 페이지]** 템플릿이 사용할 수 있는 템플릿 목록에 나타납니다.

1. [허용되는 부모 **[!UICONTROL 및]** 허용되는 하위 **[!UICONTROL 항목]** ] 패널에서 [다음 **을 클릭하고 []** 확인]을 ****&#x200B;클릭합니다. CRXDE Lite에서 모두 **[!UICONTROL 저장을 클릭합니다]**.
...
1. 구성 요소 **[!UICONTROL 만들기]** 대화 상자에서 다음 속성 값을 입력합니다.
   ...
   * **[!UICONTROL 설명]**: 내 웹 사이트 컨텐츠 페이지 구성 요소입니다.
   ...
   새 구성 요소의 위치는 입니다 `/apps/mywebsite/components/contentpage`. 이 경로는 컨텐트 페이지 템플릿의 리소스 유형(경로의 초기 `/apps/` 부분 제외)과 일치합니다.

   이러한 통신은 템플릿을 구성 요소에 연결하며 웹 사이트의 올바른 기능에 매우 중요합니다.

1. 대화 상자 **[!UICONTROL 의 []** 허용되는 하위 ** 패널이 나타날 때까지 [다음]을 클릭한 다음]** 확인을 클릭합니다 ****. CRXDE Lite에서 모두 **[!UICONTROL 저장을 클릭합니다]**.
...
1. CRXDE Lite에서 파일을 `contentpage.jsp` 엽니다 `/apps/mywebsite/components/contentpage`. 이 파일에는 기본적으로 다음 코드가 포함되어 있습니다.
...
1. 모두 **[!UICONTROL 저장을]** 클릭하여 변경 사항을 저장합니다.
...
1. 폴더 트리에서 **[!UICONTROL 웹 사이트]** 폴더를 선택한 다음 **[!UICONTROL 새로 만들기 > 새 페이지를 클릭합니다]**.
...
1. 새 웹 브라우저 탭 또는 창에서 http://localhost:4502/content/mywebsite/en/products.html을 [열어](http://localhost:4502/content/mywebsite/en/products.html) 제품 페이지를 확인합니다.
...
1. CRXDE Lite에서 노드에 속성을 `/apps/mywebsite/components/contentpage` 추가합니다.
...
1. 아래 `contentpage.jsp` 파일을 열고 기존 코드를 `/apps/mywebsite/components/contentpage` 다음 코드로 바꿉니다.
...
1. 파일을 편집하여 기존 컨텐츠 `left.jsp` 를 제거하고 다음 코드로 바꿉니다.
...
1. 파일을 편집하여 기존 컨텐츠 `center.jsp` 를 제거하고 다음 코드로 바꿉니다.
...
1. 파일을 편집하여 기존 컨텐츠 `right.jsp` 를 제거하고 다음 코드로 바꿉니다.
...
1. 마지막 **[!UICONTROL 창으로 오려면 [다음]** ]을 **[!UICONTROL 클릭합니다]**. 변경 내용을 저장합니다.
...
#### 콘텐츠 페이지 구성 요소에 위쪽 탐색 포함 {#including-top-navigation-in-the-contentpage-component}
...
#### 이미지 링크를 사용하여 위쪽 내비게이션 향상 {#enhance-top-navigation-by-using-image-links}

탐색 컨트롤에 하이퍼텍스트 대신 이미지 링크를 사용하도록 topnav 구성 요소의 렌더링 스크립트를 개선합니다. 이미지에는 링크 대상의 제목과 부제가 포함됩니다.

이 연습은 Sling [요청 처리를 보여줍니다](/help/sites-developing/the-basics.md#sling-request-processing). topnav.jsp 스크립트는 페이지 탐색 링크에 사용할 이미지를 동적으로 생성하는 스크립트를 호출하도록 수정되었습니다. 이 실습에서는 이미지 소스 파일의 URL을 구문 분석하여 이미지를 렌더링하는 데 사용할 스크립트를 결정합니다.
...
1. Sling은 이 경로와 `/content/mywebsite/en/products` 노드
1. Sling은 이 노드 `sling:resourceType` 를 결정합니다 `mywebsite/components/contentpage`.

1. Sling은 URL 선택기() 및 파일 이름 확장명()과 가장 잘 일치하는 스크립트를 이 구성 요소에서 `navimage``png`찾습니다.
...
1. 변경 사항을 저장합니다.
1. 노드를 마우스 오른쪽 단추로 `/apps/mywebsite/components/contentpage` 클릭하고 **[!UICONTROL 만들기 > 파일 만들기를 클릭합니다]**.
1. 파일 **[!UICONTROL 만들기]** 창 **[!UICONTROL 에 이름]**&#x200B;을 `navimage.png.java`입력합니다.

   .java 파일 이름 확장자는 Sling에 Apache Sling 스크립팅 Java 지원을 사용하여 스크립트를 컴파일하고 서블릿을 만들어야 함을 나타냅니다.

1. 다음 코드를 The code extends `navimage.png.java.`the AbstractImageServlet 클래스를 확장합니다.
...
제품 페이지 아래에 있는 두 **[!UICONTROL 페이지를]** 만듭니다. 각 페이지에 대해 두 개의 특정 제품을 설명하는 제목, 설명 및 날짜를 설정합니다.
...
1. listchildren 구성 요소 아래에 대화 상자 노드를 만듭니다.
   ...
   1. CRXDE Lite에서 `/apps/mywebsite/components/listchildren`노드를 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL 만들기 > 대화 상자 만들기를 클릭합니다]**.
...
1. [ **[!UICONTROL 속성]** ] 탭에서 **[!UICONTROL title]** 속성 값을 `List Children`
...
1. 목록 루트 노드에 대한 속성을 추가하여 텍스트 필드로 구성합니다. 다음 표의 각 행은 속성을 나타냅니다. 완료되면 **[!UICONTROL 모두 저장을 클릭합니다]**.
...
1. CRXDE Lite에서 /apps/mywebsite/components를 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL 만들기]**&#x200B;를 선택한 다음 구성 요소 **[!UICONTROL 만들기를 선택합니다]**.
...
1. 대화 상자의 최종 패널에 도달할 때까지 다음을 클릭한 다음 **[!UICONTROL 확인을 클릭합니다]**.
...
디자인 모드에서 로고 구성 요소를 구성하기 위한 대화 상자를 만듭니다. 디자인 모드 대화 상자 노드의 이름은 지정해야 합니다 `design_dialog`.

1. 로고 구성 요소 아래에 대화 상자 노드를 만듭니다.

   1. 노드를 마우스 오른쪽 단추로 `/apps/mywebsite/components/logo` 클릭하고 **[!UICONTROL 만들기 > 대화 상자 만들기를 클릭합니다]**.
   1. 다음 속성 값을 입력한 다음 **[!UICONTROL 확인을 클릭합니다]**.

      * **[!UICONTROL 레이블]** `design_dialog`
      * **[!UICONTROL 제목]** `Logo (Design)`

1. design_dialog 분기의 tab1 노드를 마우스 오른쪽 단추로 클릭하고 삭제를 클릭합니다. 모두 **[!UICONTROL 저장을 클릭합니다]**.
1. 노드 아래에 `design_dialog/items/items`Type이라는 새 노드 `img` 를 만듭니다 `cq:Widget`. 다음 속성을 추가한 다음 모두 **[!UICONTROL 저장을 클릭합니다]**.
...
1. 로고 구성 요소 노드를 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL 만들기 > 파일]** 만들기를 클릭하여 img.GET.java라는 스크립트 파일을 만듭니다.
1. 파일을 열고 다음 코드를 파일에 복사한 다음 **[!UICONTROL 모두 저장을 클릭합니다]**.
...
1. 변경 사항을 저장합니다.
1. 브라우저에서 **[!UICONTROL 제품]** 페이지를 다시 로드합니다. 로고는 다음과 같이 보이지만 현재 기본 링크만 표시됩니다.
...
1. 브라우저에서 **[!UICONTROL 제품]** 페이지가 열려 있는 상태에서 사이드 킥의 아래쪽에 있는 **[!UICONTROL 디자인]** **** 단추를 클릭하여디자인모드로 전환합니다.

   ![](do-not-localize/chlimage_1-10.png)

1. 로고 디자인 막대에서 **[!UICONTROL 편집을]** 클릭하여 대화 상자를 사용하여 로고 구성 요소의 설정을 편집합니다.
1. 대화 상자에서 **[!UICONTROL 이미지]** 탭 패널을 클릭하고 파일에서 추출한 `logo.png` 이미지를 `mywebsite.zip` 찾은 다음 **[!UICONTROL 확인을 클릭합니다]**.
...
1. 변경 사항을 저장합니다.
1. 브라우저에서 **[!UICONTROL 제품 1]** 페이지를 다시 로드합니다. 트레일 구성 요소는 다음과 같습니다.
...
1. 변경 사항을 저장합니다.
1. 브라우저에서 **[!UICONTROL 제품]** 페이지를 다시 로드합니다. 제목 구성 요소는 다음과 같습니다.
...
1. 브라우저에서 제품 **[!UICONTROL 페이지를 새로]** 고칩니다. 이제 parsys 구성 요소가 있으며 다음과 같습니다.
...
1. 노드를 마우스 오른쪽 단추로 `/apps/mywebsite/components/logo` 클릭하고 **[!UICONTROL 복사를 클릭합니다]**.
1. 노드를 마우스 오른쪽 단추로 `/apps/mywebsite/components` 클릭하고 **[!UICONTROL 붙여넣기를 클릭합니다]**.
1. 노드를 마우스 오른쪽 단추로 `Copy of logo` 클릭하고 이름 **[!UICONTROL 변경]**, 기존 텍스트를 삭제하고 `image`입력합니다.
...
노드 `cq:editConfig` 유형을 사용하면 속성을 편집할 때 구성 요소의 특정 동작을 구성할 수 있습니다.
...
1. CRXDE Lite에서 파일을 마우스 오른쪽 버튼으로 클릭하고 복사 `/libs/foundation/components/image/icon.png` 를 **[!UICONTROL 선택합니다]**.
1. 노드를 마우스 오른쪽 단추로 `/apps/mywebsite/components/image` 클릭하고 **[!UICONTROL 붙여넣기를]**&#x200B;클릭한 다음 **[!UICONTROL 모두 저장을 클릭합니다]**.
...
1. 페이지 목록에서 도구 **[!UICONTROL 모음]** 페이지를 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL 속성을 클릭합니다]**. [탐색 **[!UICONTROL 에서 숨기기]**]를 선택하고 [ **[!UICONTROL 확인]을 클릭합니다]**.
   ...
   탐색 **[!UICONTROL 에서 숨기기]** 옵션을 사용하면 탐색 구성 요소에 상위 및 목록 하위 페이지와 같은 페이지가 표시되지 않습니다.

1. 도구 **[!UICONTROL 모음]**&#x200B;아래에서 다음 페이지를 만듭니다.
...
1. 다음 코드를 다음 위치에 복사합니다 `search.jsp`.
...
1. 사이드 킥에서 **[!UICONTROL 디자인]** 모드 아이콘을 클릭합니다.
1. 단락 블록 디자인(검색 제목 아래)에서 **[!UICONTROL 편집을 클릭합니다]**.
1. 대화 상자에서 **[!UICONTROL 내 웹 사이트]** 그룹으로 스크롤 다운한 후 **[!UICONTROL 내 검색 구성]** 요소 **[!UICONTROL 를 선택한 다음 확인을]**&#x200B;클릭합니다.
