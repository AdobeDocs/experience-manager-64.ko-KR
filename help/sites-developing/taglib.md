...
글로벌 역시 Sling [라이브러리를 선언합니다](/help/sites-developing/taglib.md#sling-tag-library).
...
태그 `<ui:includeClientLib>` 에는 js, css 또는 테마 라이브러리가 될 수 있는 AEM html 클라이언트 라이브러리가 포함되어 있습니다. js 및 css와 같은 다양한 유형의 여러 포함을 위해 이 태그를 jsp에서 여러 번 사용해야 합니다. 이 태그는 서비스 인터페이스 주위의 편리한 ` [com.adobe.granite.ui.clientlibs.HtmlLibraryManager](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/granite/ui/clientlibs/HtmlLibraryManager.html)` 래퍼입니다.

여기에는 다음 속성이 있습니다.

**categories** - 쉼표로 구분된 클라이언트 lib 카테고리 목록입니다. 지정된 범주에 대한 모든 Javascript 및 CSS 라이브러리가 포함됩니다. 요청에서 테마 이름이 추출됩니다.

해당: `com.adobe.granite.ui.clientlibs.HtmlLibraryManager#writeIncludes`

**theme** - 쉼표로 구분된 클라이언트 lib 카테고리 목록입니다. 지정된 범주에 대한 모든 테마 관련 라이브러리(CSS 및 JS 모두)가 포함됩니다. 요청에서 테마 이름이 추출됩니다.

해당: `com.adobe.granite.ui.clientlibs.HtmlLibraryManager#writeThemeInclude`

**js** - 쉼표로 구분된 클라이언트 lib 카테고리 목록입니다. 지정된 범주에 대한 모든 Javascript 라이브러리가 포함됩니다.

해당: `com.adobe.granite.ui.clientlibs.HtmlLibraryManager#writeJsInclude`

**css** - 쉼표로 구분된 클라이언트 lib 카테고리 목록입니다. 지정된 범주에 대한 모든 CSS 라이브러리가 포함됩니다.
...
jsp에서 `<ui:includeClientLib>` 태그는 다음과 같이 사용할 수 있습니다.
...
>[!NOTE]
>
>스크립트에 `/libs/foundation/global.jsp` 파일이 포함되면 taglib가 자동으로 선언됩니다.
...
sling, CQ 및 jstl taglibs를 선언하며 [ `<cq:defineObjects />`](#amp-lt-cq-defineobjects) 태그로 정의된 자주 사용되는 스크립팅 개체를 노출합니다. 구성 요소의 jsp 코드를 단축하고 단순화합니다.

### <cq:text> {#cq-text}

태그는 `<cq:text>` JSP에 구성 요소 텍스트를 출력하는 편리성 태그입니다.
...
**escapeXml** - 결과 문자열에 있는 &lt;, >, &amp;, &#39; 및 &quot; 문자를 해당 문자 엔티티 코드로 변환할지 여부를 정의합니다. 기본값은 false입니다. escape는 선택적 서식 다음에 적용됩니다.

**형식** - 텍스트 서식 지정에 사용할 선택적 java.text.Format.

**noDiff** - 비교 정보가 있어도 비교 출력을 계산하지 않습니다.
...
**기본값** - null 또는 빈 텍스트에 사용할 기본값 기본 확인은 선택적 형식 지정 및 이스케이프 지정(예: 출력 그대로 표시됨) 후에 수행됩니다.

JSP에서 태그를 `<cq:text>` 사용하는 몇 가지 예:
...
이 `<cq:setContentBundle>` 태그는 i18n 현지화 컨텍스트를 만들어 구성 변수에 `javax.servlet.jsp.jstl.fmt.localizationContext` 저장합니다.

여기에는 다음 속성이 있습니다.

**언어** - 리소스 번들을 검색할 로케일의 언어입니다.

**source** - 로케일을 가져올 소스. 다음 값 중 하나로 설정할 수 있습니다.

* **static** - 로케일은 사용 가능한 경우 `language` 속성에서, 그렇지 않은 경우 서버 기본 로케일에서 가져옵니다.

* **page** - 로케일은 현재 페이지 또는 리소스의 언어로 이동되며, 사용 가능한 경우 속성에서, 그렇지 않으면 서버 기본 로케일에서 `language` 가져옵니다.

* **요청** - 로케일은 요청 로케일()에서 `request.getLocale()`가져옵니다.

* **auto** - 로케일은 가능한 경우 `language` 속성에서, 그렇지 않은 경우 현재 페이지 또는 리소스의 언어로, 그렇지 않으면 요청에서 가져옵니다.

속성이 `source` 설정되지 않은 경우:

* 속성이 `language` 설정된 경우 `source` 속성 기본값은 &quot;입니다 `static`.

* 속성이 `language` 설정되지 않으면 `source` 속성 기본값이 로 설정됩니다 `auto`.
...
1. 노드에 키와 정확히 동일한 이름의 속성이 포함되어 있지 않으면, 스링 요청에서 리소스 번들을 로드하는 것이 대비됩니다( `SlingHttpServletRequest.getResourceBundle(Locale)`). 이 번들의 언어 또는 로케일은 태그의 언어 및 소스 속성에 의해 `<cq:setContentBundle>` 정의됩니다.

태그를 jsp에서 다음과 같이 사용할 수 `<cq:setContentBundle>` 있습니다.
...
태그에는 `<cq:include>` 현재 페이지에 리소스가 포함됩니다.
...
스크립트 `<%@ include file="myScript.jsp" %>` 를 사용해야 `<cq:include script="myScript.jsp" %>` 합니까?

* 이 `<%@ include file="myScript.jsp" %>` 지시문은 JSP 컴파일러가 현재 파일에 전체 파일을 포함하도록 지시합니다. 포함된 파일의 내용을 원본 파일에 직접 붙여넣은 것과 같습니다.
* 태그가 있는 `<cq:include script="myScript.jsp">` 파일은 런타임에 포함됩니다.

사용하시겠습니까 `<cq:include>` 아니면 `<sling:include>`?
...
>[!CAUTION]
>
>`<cq:includeClientLib>` AEM 5.6부터 사용되지 않습니다. 대신 [ 사용해야 `<ui:includeClientLib>`](/help/sites-developing/taglib.md#ui-includeclientlib) 합니다.

태그 `<cq:includeClientLib>` 에는 js, css 또는 테마 라이브러리가 될 수 있는 AEM html 클라이언트 라이브러리가 포함되어 있습니다. js 및 css와 같은 다양한 유형의 여러 포함을 위해 이 태그를 jsp에서 여러 번 사용해야 합니다. 이 태그는 서비스 인터페이스 주위의 편리한 `com.day.cq.widget.HtmlLibraryManager` 래퍼입니다.
...
해당: `com.day.cq.widget.HtmlLibraryManager#writeJsInclude`
...
jsp에서 `<cq:includeClientLib>` 태그는 다음과 같이 사용할 수 있습니다.
...
이 `<cq:defineObjects>` 태그에는 개발자가 참조할 수 있는 정기적으로 사용되는 스크립팅 개체가 표시됩니다. 또한 태그로 정의된 개체를 [`<sling:defineObjects>`](#amp-lt-sling-defineobjects) 표시합니다.
...
* AEM 구성 요소의 edit 컨텍스트 개체(com.day.cq.wcm.api.components.EditContext 인터페이스)입니다.
...
태그는 현재 요청 URL을 JspWriter에 씁니다. `<cq:requestURL>` 두 개의 태그 [ 와 `<cq:addParam>`](#amp-lt-cq-addparam) [ `<cq:removeParam>`](#amp-lt-cq-removeparam) 이 태그의 본문 내에 있는 두 개의 태그를 사용하여 작성되기 전에 현재 요청 URL을 수정할 수 있습니다.
...
를 사용하거나 `addParam` 는 해당 매개 변수의 발생을 `removeParam` 변경하기만 하면 다른 모든 매개 변수는 영향을 받지 않습니다.
...
태그는 `<cq:addParam>` 지정된 이름과 값이 있는 요청 매개 변수를 바깥쪽 [`<cq:requestURL>`](#amp-lt-cq-requesturl) 태그에 추가합니다.
...
태그는 `<cq:removeParam>` 지정된 이름과 값이 있는 요청 매개 변수를 바깥쪽 [ 태그에서 `<cq:requestURL>`](#amp-lt-cq-requesturl) 제거합니다. 값을 제공하지 않으면 지정된 이름의 모든 매개 변수가 제거됩니다.
...
>[!NOTE]
>
>스크립트에 `/libs/foundation/global.jsp` 파일이 포함되면 sling taglib가 자동으로 선언됩니다.

### <sling:include> {#sling-include}

태그에는 `<sling:include>` 현재 페이지에 리소스가 포함됩니다.
...
* 전달하면 접미사가 이 속성의 값으로 바뀝니다.

>[!NOTE]
>
>태그에 포함된 리소스 및 스크립트의 해상도는 일반적인 슬링 URL `<sling:include>` 해상도와 동일합니다. 기본적으로 선택기, 확장 등 현재 요청에서 포함된 스크립트도 사용됩니다. 태그 속성을 통해 수정할 수 있습니다. 예를 들어 선택기 `replaceSelectors="foo.bar"` 를 덮어쓸 수 있습니다.
...
이 `<sling:defineObjects>` 태그에는 개발자가 참조할 수 있는 정기적으로 사용되는 스크립팅 개체가 표시됩니다.
...
* 이 응답 내에 있는 다른 리소스의 응답을 포함하기 위해 주로 sling.include(&#39;/some/other/resource&#39;)와 스크립트에 대한 편리성 메서드를 포함하는 SlingScriptHelper 개체 sling에서 사용 가능한 OSGi 서비스(스크립팅 언어에 따라 클래스 표기법)를 검색하기 위해 header html 조각)과 sling.getService(foo.bar.Service.class)를 포함합니다.
...
JavaServer [Pages Standard Tag Library에는](https://www.oracle.com/technetwork/java/index-jsp-135995.html) 다양하고 유용한 표준 태그가 포함되어 있습니다. 코어, 서식 및 함수 태그릿은 다음 코드 조각에 표시된 대로 `/libs/foundation/global.jsp` 로 정의됩니다.
...
앞에서 설명한 대로 `/libs/foundation/global.jsp` 파일을 가져온 후 `c`및 `fmt` `fn` 접두사를 사용하여 해당 태그릿에 액세스할 수 있습니다. JSTL의 공식 문서는 Java EE 5 자습서 [- JavaServer 페이지 표준 태그 라이브러리에서 사용할 수 있습니다](https://docs.oracle.com/javaee/5/tutorial/doc/bnakc.html).
