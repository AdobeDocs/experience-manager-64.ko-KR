...
위의 &quot;블로그 게시물&quot; 노드 `lastModified` 에서 Date 속성을 사용하는 예제는 특수 유형이 필요하다는 의미는 아닙니다. 최소한 처음에는 블로그 게시물 노드 `nt:unstructured` 에 사용할 수 있습니다. 블로그애플리케이션에 마지막 수정 날짜(주문했을 수도 있음)를 표시하려고 하는데, 날짜인지 전혀 신경 쓰지 않습니다. 나는 내 블로그 작성 응용 프로그램이 어쨌든 &quot;날짜&quot;를 거기에 두도록 암시적으로 신뢰하기 때문에, 정말로 `lastModified` 날짜의 존재를 Nodetype 형식으로 선언할 필요가 없다.
...
>
>CRX에서 처리할 수 있는 데이터 [양은 얼마입니까?](https://helpx.adobe.com/experience-manager/kb/CrxLimitation.html) 를 참조하십시오.
...
애플리케이션에서 `clone()`또는 방법을 사용하지 않는 경우 단일 작업 영역 `merge()` 을 사용하는 `update()` 것이 가장 좋습니다.
...
액세스 제어에는 작업 영역을 사용할 수 없습니다. 특정 사용자 그룹에 대한 컨텐츠의 가시성은 다른 작업 영역으로 항목을 구분하기 위한 좋은 주장이 아닙니다. JCR은 이에 대해 제공하기 위해 컨텐츠 저장소에 &quot;액세스 제어&quot;를 제공합니다.
...
개인적으로 저는 매끄러운 참조를 다룰 수 없고 다른 노드를 참조하기 위해 경로, 이름 또는 문자열 UUID를 사용할 수 없을 때만 참조를 사용하는 것을 확실히 합니다.
...
컨텐츠 모델에서 사용하려고 하는 파일 또는 폴더 *와* 같은 원격 `nt:file`냄새가 `nt:folder` 나는 것을 노출시키는 경우, `nt:resource`및를 확장할 수 있습니다.
...
내 생각에, 경험상 좋은 법칙은 다음과 같다. 파일 이름을 저장해야 하는 경우, `nt:file`//는 매우 `nt:resource` 일치합니다. 여러 개의 &quot;파일&quot;을 가질 수 있는 경우 nt:folder는 이러한 파일을 저장하기에 좋습니다.

리소스에 대한 메타 정보를 추가해야 하는 경우, &quot;author&quot; 또는 &quot;description&quot; 속성을 확장하지 `nt:resource` 않고 extend `nt:file`. 파일 확장이나 파일 확장도 거의 없습니다 `nt:resource`.
...
더 중요한 것은 **mix**:reference입니다. 즉, 노드를 실제로 참조해야 하는 시점에 노드에 적용할 수 있습니다.
