...
Touch Day 2는 AEM 인스턴스의 한계를 강조하게 하는 애플리케이션입니다. 기본 테스트 세트와 함께 실행되거나 테스트 요구 사항에 맞게 구성할 수 있습니다. 응용 프로그램 프레젠테이션을 위해 [이 레코딩을](https://docs.adobe.com/ddc/en/gems/Toughday2---A-new-and-improved-stress-testing-and-benchmarking-tool.html) 볼 수 있습니다.

## 어려운 날을 실행하는 방법 2 {#how-to-run-tough-day}

Adobe 저장소에서 최신 버전의 Touch Day 2를 [다운로드합니다](https://repo.adobe.com/nexus/content/repositories/releases/com/adobe/qe/toughday2/). 애플리케이션을 다운로드한 후 매개 변수를 제공하여 즉시 실행할 수 `host` 있습니다. 다음 예에서 AEM 인스턴스는 로컬로 실행되므로 값이 `localhost` 사용됩니다.

```xml
java -jar toughday2.jar --host=localhost
```

매개 변수를 추가한 후 실행되는 기본 세트의 이름이 지정됩니다 `toughday`. 여기에는 다음과 같은 사용 사례가 포함됩니다.
...
패키지 테스트를 실행하기 위해 Touch Day 2는 기본 컨텐츠 패키지를 설치합니다. 매개 변수를 다음으로 설정하여 이러한 `installsamplecontent` 작업을 방지할 수 `false`있지만 실행하려는 테스트의 기본 경로를 변경해야 합니다. jar가 매개 변수 없이 실행되는 경우 Touch Day 2에 [도움말 정보가 표시됩니다](/help/sites-developing/tough-day.md#getting-help).
...
| `--duration=<Val>` | 테스트의 지속 시간입니다. 은 (**s**)초, (**m**)분, (**h**)ours 및 (**dOurs)**&#x200B;로 표현될 수 있습니다. | 1d |  |
...
테스트 구성을 저장하는 유일한 방법은 YAML 형식으로 복사하는 것입니다. 자세한 내용은 아래 섹션의 이 [height.yaml](https://repo.adobe.com/nexus/service/local/repositories/releases/content/com/adobe/qe/toughday2/0.2.1/toughday2-0.2.1.yaml) 구성 및 YAML 구성 예제를 참조하십시오.

### 새 테스트 추가 {#adding-a-new-test}

기본 `toughday` 세트를 사용하지 않으려면 매개 변수를 사용하여 선택한 테스트를 추가할 수 `add` 있습니다. 아래 예는 명령줄 매개 변수 또는 YAML 구성 파일을 사용하여 `CreateAssetTreeTest` 테스트를 추가하는 방법을 보여줍니다.
...
하나 이상의 테스트 속성을 변경해야 하는 경우 해당 속성을 명령줄 또는 YAML 구성 파일에 추가할 수 있습니다. 사용 가능한 모든 테스트 속성을 보려면 다음 `--help <TestClass/PublisherClass>` 과 같이 명령줄에 매개 변수를 추가합니다.
...
아래 예제는 명령줄 매개 변수 또는 YAML 구성 파일을 사용하여 `template` `CreatePageTreeTest` 테스트의 속성을 변경하는 방법을 보여줍니다.
...
매개 변수를 사용하고 타깃팅된 사전 정의된 세트를 지정하여 사전 정의된 세트에 새 테스트를 추가할 수 `add` 있습니다.
...
지정된 세트의 기존 테스트는 `config`* *매개 변수를 사용하여 재구성할 수도 있습니다. 테스트 클래스 이름이 아니라 테스트 세트 이름과 실제 이름을 지정해야 합니다. 테스트 클래스 속성에서 테스트 이름을 찾을 수 `name` 있습니다. 테스트 속성을 찾는 방법에 대한 자세한 내용은 테스트 속성 [변경 섹션을](/help/sites-developing/tough-day.md#changing-the-test-properties) 참조하십시오.

아래 예에서 (명명된)의 기본 자산 제목 `CreatePageTreeTest` 이 &quot;NewAsset&quot; `UploadAsset`로 변경되었습니다.
...
또한 매개 변수를 사용하여 사전 정의된 세트 또는 게시자에서 테스트를 기본 구성에서 제거할 수도 `exclude` 있습니다. 테스트 C `lass` 이름이 아닌 테스트의 세트 이름과 실제 이름을 지정해야 합니다. 테스트 클래스 속성에서 테스트 이름을 찾을 수 `name` 있습니다. 아래 예에서, `CreatePageTreeTest` (명명된 `UploadAsset`) 테스트는 Started suite에서 제거됩니다.
...
터프한 2일은 다음 모드 중 하나를 실행할 수 있습니다. **정상** 및 **상수 로드**.
...
The **constant load** run mode different from the normal run mode by generating an constant number of started test execution, thread of threads. 동일한 이름의 실행 모드 매개 변수를 사용하여 로드를 설정할 수 있습니다.
...
또한, 테스트에 `count` 속성이 있을 수 있으므로 주어진 수로 실행 횟수를 제한합니다. 이 번호가 전달된 후에는 더 이상 테스트가 실행되지 않습니다. 이미 실행 중인 모든 테스트 인스턴스는 구성된 대로 실행을 마칩니다. 다음 예에서는 이러한 매개 변수를 명령줄에 추가하거나 YAML 구성 파일을 사용하여 추가하는 방법을 보여 줍니다.
...
>[!NOTE]
>
>병렬 실행으로 인해 실제 테스트 실행 수는 매개 변수에 구성된 정확한 양이 아닙니다 `count` . 실행 중인 스레드의 수에 편차가 `concurrency parameter`예상됩니다.
...
오늘 2일은 현재 사용자가 평가할 수 있는 9개의 테스트 지표를 보고합니다. 지표(&amp; **P);ast;** 기호는 성공적인 실행 후에만 보고됩니다.
...
| 전달됨 | 성공한 실행 수입니다. |
| 실패 | 실패한 실행 수입니다. |
| 최소/최소;ast; | 테스트 실행의 가장 낮은 기간입니다. |
| 최대 길이(&amp;M);ast; | 테스트 실행의 최고 기간입니다. |
| 중간값;ast; | 모든 테스트 실행의 평균 계산 기간입니다. |
| 평균 길이(&amp;M);ast; | 모든 테스트 실행의 계산된 평균 기간입니다. |
| StdDev&amp;ast; | 표준 편차 |
| 90p&amp;ast; | 90 백분위수 |
| 99p&amp;ast; | 99 백분위수 |
| 99.9p&amp;ast; | 99.9 백분위수. |
| Real Throughput &amp;ast; | 경과된 실행 시간으로 나눈 실행 수입니다. |

이러한 지표는 테스트 추가와 마찬가지로 매개 변수와 함께 추가할 수 있는 게시자의 `add` 도움으로 작성됩니다. 현재 두 가지 옵션이 있습니다.
...
* 게시 **모드** - 지정된 기간 내에 결과를 보고합니다. 시간 프레임을 **간격** 게시 모드 매개 변수로 설정할 수 있습니다.

다음 예에서는 명령줄 또는 YAML 구성 파일을 사용하여 `intervals` 매개 변수를 구성하는 방법을 보여 줍니다.
...
터프한 요일 2를 실행한 동일한 디렉토리에 로그 폴더가 생성됩니다. 이 폴더에는 두 가지 유형의 로그가 있습니다.

* **powerday.log**: 에는 응용 프로그램 상태, 디버깅 정보 및 전역 메시지와 관련된 메시지가 포함되어 있습니다.
* **powerday_&lt;testname>.log**: 지정된 테스트와 관련된 메시지.

로그가 덮어쓰이지 않으며, 이후 실행은 기존 로그에 메시지를 추가합니다. 로그에는 여러 수준이 있으며 자세한 내용은 을 참조하십시오 ` [loglevel parameter](/help/sites-developing/tough-day.md#global-parameters)`.