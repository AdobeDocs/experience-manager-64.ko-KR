...
Tough Day 2는 AEM 인스턴스의 한계를 테스트할 수 있는 애플리케이션입니다. 기본 테스트 세트로 즉시 실행하거나 테스트 요구 사항에 맞게 구성할 수 있습니다. 응용 프로그램 프레젠테이션을 보려면 [이 녹음](https://docs.adobe.com/ddc/en/gems/Toughday2---A-new-and-improved-stress-testing-and-benchmarking-tool.html)을 시청하십시오.

## Tough Day 2를 실행하는 방법 {#how-to-run-tough-day}

[Adobe 저장소](https://repo1.maven.org/maven2/com/adobe/qe/toughday2/)에서 최신 버전의 Tough Day 2를 다운로드하십시오. 애플리케이션을 다운로드한 후에는 `host` 매개 변수를 제공하여 즉시 실행할 수 있습니다. 다음 예에서는 AEM 인스턴스가 로컬로 실행되어 `localhost` 값이 사용됩니다.
...
세트 테스트를 실행하려면 Tough Day 2가 기본 컨텐츠 패키지를 설치합니다. 이 기능은 `installsamplecontent` 매개 변수를 `false`로 설정하여 방지할 수 있지만, 실행할 테스트에 대한 기본 경로도 변경해야 합니다. jar가 매개 변수 없이 실행되는 경우 Tough Day 2에는 [도움말 정보](/help/sites-developing/tough-day.md#getting-help)가 표시됩니다.
...
Tough Day 2 는 현재 사용자가 평가할 수 있는 9개의 테스트 지표를 보고합니다. **&amp;ast;** 기호가 있는 지표는 성공적인 실행 후에만 보고됩니다.
...
| 전달됨 | 성공한 실행 수입니다. |
| 실패 | 실패한 실행 수입니다. |
| 최소&amp;ast; | 테스트 실행 시간이 가장 짧습니다. |
| Max&amp;ast; | 테스트 실행 최대 기간입니다. |
| 중간값(&amp;M);ast; | 모든 테스트 실행의 평균 기간을 계산했습니다. |
| Average&amp;ast; | 모든 테스트 실행의 계산된 평균 기간입니다. |
| StdDev&amp;ast; | 표준 편차. |
| 90p&amp;ast; | 90번째 백분위수. |
| 99p&amp;ast; | 99번째 백분위수. |
| 99.9p&amp;ast; | 99.9백분위수. |
| 실제 처리량;amp;ast; | 경과된 실행 시간으로 나눈 실행 수입니다. |
