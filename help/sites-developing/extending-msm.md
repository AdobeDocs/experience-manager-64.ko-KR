...
* **`Blueprint`**
Live Copy `Blueprint` 가 컨텐츠를 상속할 수 있는 페이지를 [블루프린트 구성에서와 같이](/help/sites-administering/msm.md#source-blueprints-and-blueprint-configurations)지정합니다.

   ![chlimage_1-36](assets/chlimage_1-36.png)

   * 블루프린트 구성( `Blueprint`)의 사용은 선택 사항이지만, 다음과 같습니다.

      * 작성자가 소스(명시적으로)에서 **롤아웃** 옵션을 사용하여 이 소스에서 상속되는 Live Copy에 수정 내용을 푸시할 수 있습니다.
      * 작성자가 사이트 **만들기를 사용할 수 있습니다**. 따라서 사용자는 손쉽게 언어를 선택하고 live copy의 구조를 구성할 수 있습니다.
      * 결과 Live Copy에 대한 기본 롤아웃 구성을 정의합니다.

* **`LiveRelationship`** Live Copy 분기의 리소스와 해당 소스/블루프린트 리소스 간의 연결(관계)을 지정합니다 `LiveRelationship` .

   * 관계는 상속과 롤아웃을 구현할 때 사용됩니다.
   * `LiveRelationship` 개체는 롤아웃 구성(), 관계 `RolloutConfig`및 `LiveCopy``LiveStatus` 관련 개체에 대한 액세스(참조)를 제공합니다.
   * 예를 들어, Live Copy는 의 소스/블루프린트 `/content/copy/us` 에서 만들어집니다 `/content/we-retail/language-masters`. 리소스 `/content/we.retail/language-masters/en/jcr:content` 와 `/content/copy/us/en/jcr:content` 관계 형성

* **`LiveCopy`** live copy 리소스와 소스/블루프린트 리소스 간의 관계( `LiveRelationship`)에 대한 구성 세부 사항을 포함합니다.

   * 이 `LiveCopy` 클래스를 사용하여 페이지의 경로, 소스/블루프린트 페이지의 경로, 롤아웃 구성 및 하위 페이지가 페이지에 포함되는지 여부를 확인할 수 `LiveCopy`있습니다.
   * 사이트 `LiveCopy` 만들기 **또는 Live Copy** 만들기가 사용될 때마다 **노드가** 만들어집니다.
...
* **`LiveActionFactory`** 구성을 `LiveAction` 통해 개체를 `LiveAction` 만듭니다. 구성은 저장소에 리소스로 저장됩니다.

* **`RolloutConfig`** 트리거할 때 사용할 목록 `LiveActions`을 보유합니다. 는 `LiveCopy` 상속을 `RolloutConfig` 상속하고 결과는 에 있습니다 `LiveRelationship`.

   * Live Copy를 처음으로 설정하면 RolloutConfig(LiveActions를 트리거함)도 사용됩니다.

## 새 동기화 작업 만들기 {#creating-a-new-synchronization-action}

롤아웃 구성에 사용할 사용자 정의 동기화 작업을 만듭니다. 설치된 작업이 [특정 응용 프로그램 요구 사항을](/help/sites-administering/msm-sync.md#installed-synchronization-actions) 충족하지 않을 때 동기화 작업을 만듭니다. 이렇게 하려면 두 개의 클래스를 만듭니다.
...
* 인터페이스를 구현하고 클래스의 인스턴스를 만드는 OSGI 구성 [`com.day.cq.wcm.msm.api.LiveActionFactory`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.html) `LiveAction` 요소입니다.
...
* `LiveActionFactory` 클래스에는 다음 멤버가 포함됩니다.
   ...
   * `LIVE_ACTION_NAME`: 연결된 이름을 포함하는 필드입니다 `LiveAction`. 이 이름은 `getName` `LiveAction` 클래스의 메서드에서 반환되는 값과 일치해야 합니다.
   * `createAction`: 의 인스턴스를 만듭니다 `LiveAction`. 선택적 매개 `Resource` 변수를 사용하여 구성 정보를 제공할 수 있습니다.
   * `createsAction`: 연결된 이름을 반환합니다 `LiveAction`.

### LiveAction 구성 노드 액세스 {#accessing-the-liveaction-configuration-node}

저장소의 `LiveAction` 구성 노드를 사용하여 인스턴스의 런타임 동작에 영향을 주는 정보를 `LiveAction` 저장합니다. 구성을 저장하는 저장소의 노드는 런타임 시 개체에 `LiveAction` 사용할 수 `LiveActionFactory` 있습니다. 따라서 구성 노드에 속성을 추가하여 필요에 따라 구현에서 사용할 수 `LiveActionFactory` 있습니다.

예를 들어 블루프린트 작성자의 이름을 저장해야 `LiveAction` 합니다. 구성 노드의 속성에는 정보를 저장하는 블루프린트 페이지의 속성 이름이 포함됩니다. 런타임 시 구성 `LiveAction` 에서 속성 이름을 검색한 다음 속성 값을 가져옵니다.

메서드의 매개 변수 [`LiveActionFactory`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.html)`.createAction` 는 `Resource` 개체입니다. 이 `Resource` 개체는 롤아웃 구성의 이 라이브 작업에 대한 `cq:LiveSyncAction` 노드를 나타냅니다. 롤아웃 [구성 만들기를 참조하십시오](/help/sites-administering/msm-sync.md#creating-a-rollout-configuration). 구성 노드를 사용할 때와 같이, 개체에 맞게 `ValueMap` 조정해야 합니다.
...
다음 개체는 개체의 `execute` 메서드 매개 변수로 `LiveAction` 제공됩니다.

* Live Copy 소스를 나타내는 [`Resource`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/Resource.html) 개체.
* Live Copy의 대상을 나타내는 `Resource` 개체입니다.
* Live Copy [`LiveRelationship`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveRelationship.html) 의 개체입니다.
* 이 `autoSave` 값은 저장소에 `LiveAction` 수행된 변경 사항을 저장할지 여부를 나타냅니다.

* 재설정 값은 롤아웃 재설정 모드를 나타냅니다.

이러한 객체에서 에 대한 모든 정보를 얻을 수 있습니다 `LiveCopy`. 또한 객체를 사용하여 객체 `Resource` , `ResourceResolver`및 `Session``Node` 객체를 가져올 수도 있습니다. 이러한 개체는 저장소 컨텐츠를 조작하는 데 유용합니다.
...
>[!NOTE]
>
>인수 `Resource` 는 개체 `null` 와 같은 개체 `Resources` 에 적응하지 않는 개체일 수 `Node` [`NonExistingResource`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/NonExistingResource.html) 있습니다.
...
1. Under this **Create** a node with the following properties:
   ...
   * **이름**: 롤아웃 구성의 노드 이름입니다. md#installed-synchronization-actions)(예: `contentCopy` 또는 `workflow`)입니다.
   * **유형**: `cq:RolloutConfig`
...
1. 이 노드에 다음 속성을 추가합니다.
   ...
   * **이름**: `cq:trigger`
      ...
      **유형**: `String`
      **값**: 롤아웃 [트리거를](/help/sites-administering/msm-sync.md#rollout-triggers) 사용합니다. 다음 중에서 선택합니다.
      * `rollout`
      * `modification`
      * `publish`
      * `deactivate`
...
롤아웃 구성은 노드 아래에 만든 [롤아웃 구성 노드](#create-the-rollout-configuration) 아래에 `/apps/msm/<your-project>/rolloutconfigs` 저장됩니다.

롤아웃 구성에 동기화 작업 `cq:LiveSyncAction` 을 추가하려면 유형의 하위 노드를 추가합니다. 동기화 작업 노드의 순서는 작업이 발생하는 순서를 결정합니다.

1. 여전히 CRXDE Lite에서 롤아웃 구성 [노드를](#create-the-rollout-configuration) 선택합니다.
...
1. **다음 노드 속성을 사용하여 노드를 만듭니다** .
   ...
   * **이름**: 동기화 작업의 노드 이름입니다.
동기화 작업 아래의 테이블에 있는 **작업** 이름 [(예:](/help/sites-administering/msm-sync.md#installed-synchronization-actions)또는)과 `contentCopy` 이름이 같아야 `workflow`합니다.
...
이 섹션의 절차를 따라 개발 `LiveActionFactory` 을 수행하고 롤아웃 구성에 사용합니다. 이 절차에서는 Maven과 Eclipse를 사용하여 다음을 개발하고 배포합니다 `LiveActionFactory`.
...
* 프로젝트를 ZIP 파일 [로 다운로드](https://github.com/Adobe-Marketing-Cloud/experiencemanager-java-msmrollout/archive/master.zip)

### 마스터 프로젝트 만들기 {#create-the-maven-project}

다음 절차를 수행하려면 Maven 설정 파일에 adobe-public 프로필을 추가해야 합니다.

* adobe-public 프로필에 대한 자세한 내용은 Content Package [Maven 플러그인 입수를 참조하십시오.](/help/sites-developing/vlt-mavenplugin.md#obtaining-the-content-package-maven-plugin)
...
Eclipse 컴파일러가 코드에서 사용되는 클래스를 참조할 수 있도록 종속성을 `LiveActionFactory` 추가합니다.

1. Eclipse 프로젝트 탐색기에서 파일을 엽니다.

   `MyLiveActionFactory/pom.xml`

1. 편집기에서 탭을 클릭하고 `pom.xml` 섹션을 `project/dependencyManagement/dependencies` 찾습니다.
...
1. Project Explorer에서 번들의 **POM 파일을** 엽니다 `MyLiveActionFactory-bundle/pom.xml`.
1. 편집기에서 탭을 `pom.xml` 클릭하고 프로젝트/종속성 섹션을 찾습니다. 종속성 요소 내에 다음 XML을 추가한 다음 파일을 저장합니다.
...
다음 `LiveActionFactory` 클래스는 소스 및 대상 페이지에 대한 메시지를 기록하고 소스 노드에서 대상 노드로 `LiveAction` `cq:lastModifiedBy` 속성을 복사하는 것을 구현합니다. 라이브 작업의 이름입니다 `exampleLiveAction`.

1. Eclipse 프로젝트 탐색기에서 패키지를 마우스 오른쪽 단추로 클릭하고 `MyLiveActionFactory-bundle/src/main/java/com.adobe.example.msm` 새로 만들기 **>** 클래스 **를 클릭합니다**. 이름 **에**&#x200B;를 입력한 `ExampleLiveActionFactory` 다음 **마침을 클릭합니다**.
1. 파일을 열고 `ExampleLiveActionFactory.java` 다음 코드로 콘텐트를 바꾼 다음 파일을 저장합니다.
...
사용자가 만든 MSM 롤아웃 구성을 `LiveActionFactory` 만듭니다.

1. 표준 절차를 사용하여 [롤아웃 구성을 만들고 구성하고](/help/sites-administering/msm-sync.md#creating-a-rollout-configuration) 속성을 사용합니다.
...
1. 노드를 `exampleLiveAction` 선택하고 다음 속성을 추가합니다.
   ...
   * **값**: `true`
   ...
   이 속성은 `ExampleLiveAction` 클래스에서 해당 `cq:LastModifiedBy` 속성을 소스에서 대상 노드로 복제해야 함을 나타냅니다.

1. 모두 **저장을 클릭합니다**.

### Live Copy 만들기 {#create-the-live-copy}

[롤아웃](/help/sites-administering/msm-livecopy.md#creating-a-live-copy-of-a-page) 구성을 사용하여 We.Retail 참조 사이트의 영어/제품 분기의 Live Copy를 만듭니다.
...
* 언어 제목
* 국가 이름
* 언어 기본 국가(코드(예: `en`, 다른 `de`국가)

언어 목록은 노드 아래에 `/libs/wcm/core/resources/languages` 저장됩니다. 각 하위 노드는 언어 또는 언어 국가를 나타냅니다.

* 노드의 이름은 언어 코드( `en` 또는 `de`같은) 또는 language_country 코드(예: `en_us` 또는 `de_ch`)입니다.

* 노드의 `language` 속성은 코드의 전체 언어 이름을 저장합니다.
* 노드의 `country` 속성은 코드의 국가 전체 이름을 저장합니다.
* 노드 이름이 언어 코드(예: `en`)로만 구성되면 country 속성이 `*`되고 추가 `defaultCountry` 속성은 사용할 국가를 나타내는 언어 국가 코드를 저장합니다.
...
1. 이전 단계를 반복하여 `/apps/wcm/core` 폴더 트리를 만듭니다. Create a node of type `sling:Folder` in core called `resources`. <!-- ![chlimage_1-39](assets/chlimage_1-39.png) -->

1. 노드를 마우스 오른쪽 단추로 `/libs/wcm/core/resources/languages` 클릭하고 **복사를 클릭합니다**.
1. 폴더를 마우스 오른쪽 단추로 `/apps/wcm/core/resources` 클릭하고 **붙여넣기를 클릭합니다**. 필요에 따라 하위 노드를 수정합니다.
1. 모두 **저장을 클릭합니다**.
1. 도구 **,**&#x200B;작업 **,** 웹 콘솔 **순으로**&#x200B;클릭합니다. 이 콘솔에서 **OSGi**, 구성을 차례로 **클릭합니다**.
1. Day CQ **WCM Language Manager를**&#x200B;찾아 클릭하고 **언어 목록** 값을 다음으로 변경한 다음 `/apps/wcm/core/resources/languages`저장을 **클릭합니다**.
...
* 의 값 `cq-msm-lockable` 은 다음과 같습니다.
   ...
   * **절대** (예: `/image`)
      ...
      * 체인을 끊으면 혼합을 에 추가하고 로 설정하여 상속이 `cq:LiveSyncCancelled` 취소됩니다 `./image` `cq:isCancelledForChildren` `true`.
      * 체인을 닫으면 상속이 취소됩니다.


>[!NOTE]
>
>cq-msm-lockable은 편집할 리소스의 첫 번째 하위 수준에 적용되며, 값이 절대이나 상대 값으로 정의되었는지에 관계없이 더 높은 수준 상위 항목에서 작동하지 않습니다.
