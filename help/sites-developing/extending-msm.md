...
* **`Blueprint`** Live Copy `Blueprint` 가 컨텐츠를 상속할 수 있는 페이지를 [블루프린트 구성에서와](/help/sites-administering/msm.md#source-blueprints-and-blueprint-configurations)같이 지정합니다.
...
* **`LiveActionFactory`** 에서는 `LiveAction` 구성이 지정된 `LiveAction` 개체를 만듭니다. 구성은 저장소에 리소스로 저장됩니다.

* **`RolloutConfig`** 가 트리거될 때 사용할 목록 `LiveActions`을 보유합니다. The `LiveCopy` inherits `RolloutConfig` and the result is present in the `LiveRelationship`.
...
* 인터페이스를 구현하고 [`com.day.cq.wcm.msm.api.LiveActionFactory`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.html) `LiveAction` 클래스의 인스턴스를 만드는 OSGI 구성 요소입니다.
...
메서드의 매개 변수는 [`LiveActionFactory`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.html)`.createAction` 개체입니다 `Resource` . 이 `Resource` `cq:LiveSyncAction` 개체는 롤아웃 구성의 이 라이브 작업에 대한 노드를 나타냅니다.롤아웃 [구성 만들기를 참조하십시오](/help/sites-administering/msm-sync.md#creating-a-rollout-configuration). 구성 노드를 사용할 때와 같이, `ValueMap` 개체에 맞게 수정해야 합니다.
...
* Live Copy의 [`LiveRelationship`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveRelationship.html) 객체입니다.
...
>[!NOTE]
>
>이 `Resource` 인수는 `null` 개체와 같은 개체에 적용되지 않는 개체이거나 `Resources` 개체일 수 `Node` [`NonExistingResource`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/NonExistingResource.html) 있습니다.
...
1. 클래식 UI **에서** 도구 콘솔을 엽니다.예: [http://localhost:4502/miscadmin#/etc](http://localhost:4502/miscadmin#/etc)
...
1. 터미널 또는 명령 세션을 사용하여 디렉토리를 `MyLiveActionFactory` 디렉토리(Maven 프로젝트 디렉토리)로 변경합니다. 그런 다음 다음 명령을 입력합니다.
   ...
   예: http://localhost:4502/system/console/status-slinglogs [를 참조하십시오](http://localhost:4502/system/console/status-slinglogs).
...
>[!NOTE]
>
>cq-msm-lockable은 편집할 리소스의 첫 번째 하위 수준에 적용되며, 값이 절대 또는 상대 값으로 정의되었는지에 관계없이 더 깊은 상위 수준 상위 항목에서 작동하지 않습니다.
