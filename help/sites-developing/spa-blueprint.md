...
위의 구현에서 프로젝트 구성 요소는 구성 요소 매핑 저장소에 실제로 등록되기 전에 비어 있는 [기능으로 확장됩니다](/help/sites-developing/spa-blueprint.md#componentmapping) . 이 작업은 ComponentMapping 라이브러리를 캡슐화하고 확장하여 [구성](#componentmapping) `EditConfig` 개체에 대한 지원을 도입하여 수행됩니다.
...
기본 [`PageModelManager`](/help/sites-developing/spa-blueprint.md#pagemodelmanager) 라이브러리와 `[`해당 ModelRouter&#39;](/help/sites-developing/spa-routing.md) 모듈(기본적으로 활성화됨)은 지정된 리소스 경로와 연결된 모델에 대한 사전 페치 및 액세스 권한을 제공합니다.

두 개체는 라우팅 개념과 관련되지만 [`ModelRouter`](/help/sites-developing/spa-routing.md) PageModelManager가 [현재 응용 프로그램 상태와](/help/sites-developing/spa-blueprint.md#pagemodelmanager) 동기화된 데이터 모델을 불러와야 합니다.
