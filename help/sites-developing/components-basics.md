...
* 새로운 구성 요소를 사용하려면 정확히 무엇을 해야 합니까?
   ...
   * 명확한 사양은 개발, 테스트 및 인계 모든 단계에서 도움이 됩니다.

      세부 사항은 시간이 지남에 따라 변경될 수 있지만, 세부 항목을 업데이트할 수 있습니다(하지만 변경 사항도 문서로 기록해야 함).
...
이 철학은 [HTL에서](https://helpx.adobe.com/experience-manager/htl/user-guide.html)지원되며, 기본 비즈니스 로직을 정의하는 데 실제 프로그래밍 언어를 사용하도록 특별히 제한된 템플릿 언어입니다. 이(선택 사항) 로직은 특정 명령을 사용하여 HTL에서 호출됩니다. 이 메커니즘은 지정된 보기에 대해 호출되는 코드를 강조 표시하고 필요한 경우 동일한 구성 요소의 서로 다른 보기에 대한 특정 논리를 허용합니다.
...
* **컨테이너 계층**
   ...
   구성 설정(편집 기능과 관련) `cq:editConfig` 이 전달되고 `cq:childEditConfig` 전파됩니다.
...
( `/libs/foundation/components/download/download.jsp`여기서 `file` 는 다운로드 구성 요소의 편집 구성에 있는 드롭 대상의 노드 이름입니다.
...
노드( `cq:listeners` `cq:EditListenersConfig`노드 유형)는 구성 요소에 대한 작업 전후에 발생하는 작업을 정의합니다. 다음 표에서는 가능한 속성을 정의합니다.
