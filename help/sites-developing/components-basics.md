...
* 새 구성 요소를 사용하려면 정확히 무엇을 해야 합니까?
   ...
   * 명확한 사양은 개발, 테스트 및 인계 과정의 모든 단계에서 도움이 됩니다.

      세부 사항은 시간이 지남에 따라 변경될 수 있지만, 사양을 업데이트할 수 있습니다(변경 사항도 문서화해야 하지만).
...
* **터치 활성화 UI**
   [AEM 5.6.0](/help/sites-developing/touch-ui-concepts.md) 에서 미리 보기로 소개되고 6.x에서 확장되는 표준 사용자 인터페이스입니다.Coral  [UI 및 ](/help/sites-developing/touch-ui-concepts.md#coral-ui) Granite UI [ ](/help/sites-developing/touch-ui-concepts.md#granite-ui)의 기본 기술을 사용하여 Adobe Marketing Cloud의 통합 사용자 경험을 기반으로 합니다.

* **CQ**
5.1에서 소개된 ExtJS 기술을 기반으로 하는 클래식 UUIser 인터페이스
...
이 철학은 기본 비즈니스 로직을 정의하는 데 실제 프로그래밍 언어가 사용되도록 의도적으로 제한된 템플릿 언어인 [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html)에서 지원합니다. 이(선택 사항) 로직은 특정 명령을 사용하여 HTL에서 호출됩니다. 이 메커니즘은 지정된 보기에 대해 호출되는 코드를 강조 표시하고 필요한 경우 동일한 구성 요소의 다른 보기에 대한 특정 논리를 허용합니다.

### HTL과 JSP {#htl-vs-jsp} 비교

HTL은 AEM 6.0에서 도입된 HTML 템플릿 언어입니다.

사용자 고유의 구성 요소를 개발할 때 [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html) 또는 JSP(Java Server Pages)를 사용할지 여부에 대한 논의는 이제 HTL이 AEM에서 권장되는 스크립팅 언어이므로 간단해야 합니다.
