...
seo-description: 새 구성 요소를 개발하기 시작할 때 구조 및 구성의 기본 사항을 이해해야 합니다
...
* 구성 요소를 처음부터 새로 만들어야 합니까, 아니면 기존 구성 요소에서 기본 사항을 상속할 수 있습니까?
   ...
   * 바퀴를 다시 만들 필요가 없다.
   * 무시, 오버레이 및 [Sling Resource Merger](/help/sites-developing/sling-resource-merger.md)를 포함하여 다른 구성 요소 정의에서 세부 사항을 상속하고 확장할 수 있도록 AEM에서 제공하는 몇 가지 메커니즘이 있습니다.
...
### 터치 활성화와 클래식 UI {#touch-enabled-vs-classic-ui}

구성 요소 개발에 대한 진지한 논의가 시작되기 전에 작성자가 사용할 UI를 알고 있어야 합니다.

* **터치 활성화 UI**
   [AEM 5.](/help/sites-developing/touch-ui-concepts.md) 6.0에서 미리 보기로 도입되고 6.x에서 확장된 표준 사용자 인터페이스입니다.Coral  [UI 및 ](/help/sites-developing/touch-ui-concepts.md#coral-ui) Granite UI [ ](/help/sites-developing/touch-ui-concepts.md#granite-ui)의 기본 기술을 사용하여 Adobe Marketing Cloud에 대한 통합 사용자 경험을 기반으로 합니다.

* **CQ**
5.1에서 도입된 ExtJS 기술을 기반으로 한 클래식 UIer 인터페이스.

자세한 내용은 [UI Interface Recommendations for Customers](/help/sites-deploying/ui-recommendations.md) 를 참조하십시오.
...
>[!NOTE]
>
>Adobe은 터치 지원 UI를 활용하여 최신 기술을 활용할 것을 권장합니다. [AEM 현대화 도구](modernization-tools.md) 를 사용하면 마이그레이션이 쉬워집니다.
...
이 철학은 기본 비즈니스 논리를 정의하는 데 실제 프로그래밍 언어가 사용되도록 의도적으로 제한되는 템플릿 언어인 [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html)에서 지원됩니다. 이(선택 사항) 로직은 특정 명령을 사용하여 HTL에서 호출됩니다. 이 메커니즘은 지정된 보기에 대해 호출되는 코드를 강조 표시하며, 필요한 경우 동일한 구성 요소의 다른 보기에 대해 특정 논리를 허용합니다.

### HTL과 JSP {#htl-vs-jsp}

HTL은 AEM 6.0에 도입된 HTML 템플릿 언어입니다.

구성 요소를 개발할 때 [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html) 또는 JSP(Java Server Page)를 사용할지 여부에 대한 논의는 이제 AEM에서 권장되는 스크립팅 언어이므로 간단해야 합니다.
...
* [구성 요소 개발](/help/sites-developing/developing-components-samples.md) (터치 지원 UI에 주력)
...
* [패키지 관리자](/help/sites-administering/package-manager.md) 를 사용하여 구성 요소를 패키지에 추가하고 다른 AEM 인스턴스로 이동합니다.
...
* 단락 시스템:
   ...
   * 표준 인스턴스(예: `parsys`, ` [responsivegrid](/help/sites-authoring/responsive-layout.md)`)에는 다양한 단락 시스템이 있습니다.
...
* AEM 표준 구성 요소는 `cq:Component`으로 정의되며 주요 요소가 있습니다.
   ...
   * jcr 속성:
      ...
      jcr 속성 목록이러한 변수와 일부 노드는 구성 요소 노드의 기본 구조를 통해 선택 사항일 수 있으며, 속성 및 하위 노드는 `cq:Component` 정의에 의해 정의됩니다
...
* **중요한 속성**:
   ...
   * `jcr:description` - 구성 요소에 대한 설명구성 요소 브라우저 또는 사이드 킥에서 마우스 오버 힌트로 사용할 수 있습니다.
   * 클래식 UI:
...
1. `cq:icon` - 구성 요소 브라우저에 표시할  [Coral UI ](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/coral-ui/coralui3/Coral.Icon.html) 라이브러리의 표준 아이콘을 가리키는 문자열 속성
...
[구성 요소 콘솔](/help/sites-authoring/default-components-console.md#component-details)에는 특정 구성 요소에 대한 아이콘이 정의된 방식이 표시됩니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>현재 구성 요소. 구성 요소는 노드 유형 <code>cq:Component</code>.<br /> </td> 
  </tr> 
  <tr> 
   <td><code>componentGroup</code></td> 
   <td><code>String</code></td> 
   <td>구성 요소 브라우저(터치 지원 UI) 또는 사이드 킥(클래식 UI)에서 구성 요소를 선택할 수 있는 그룹입니다.<br /> 의 값 <code>.hidden</code> 은 실제 단락 시스템과 같은 UI에서 선택할 수 없는 구성 요소에 사용됩니다.</td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><code>String</code></td> 
   <td>설정된 경우 이 속성을 셀 ID로 가져옵니다. 자세한 내용은 기술 자료 문서 <a href="https://helpx.adobe.com/experience-manager/kb/DesigneCellId.html">디자인 셀 ID가 빌드된 방법</a>.<br /> 을 참조하십시오. </td> 
  </tr> 
  <tr> 
   <td><code>cq:childEditConfig</code></td> 
   <td><code>cq:EditConfig</code></td> 
   <td>구성 요소가 컨테이너(예: 단락 시스템)이면 하위 노드의 편집 구성을 구동합니다.<br /> </td> 
  </tr> 
  <tr> 
   <td><code>cq:editConfig</code></td> 
   <td><code>cq:EditConfig</code></td> 
...
* `jcr:title` - 구성 요소의 제목구성 요소를 식별하는 데 사용할 수 있습니다. 예를 들어 구성 요소 브라우저 또는 사이드 킥의 구성 요소 목록에 표시됩니다
* `jcr:description` - 구성 요소에 대한 설명사이드 킥의 구성 요소 목록에서 마우스 오버 힌트로 사용할 수 있습니다
...
* 터치 활성화 UI:
   ...
   * `cq:dialog` (  `nt:unstructured`) - 이 구성 요소의 컨텐츠를 편집할 수 있는 대화 상자를 정의합니다
...
>* [AEM 현대화 도구](/help/sites-developing/modernization-tools.md)도 클래식 UI에 대해 정의된 대화 상자만 있는 구성 요소를 확장/변환하는 데 도움이 됩니다.
...
* 터치 활성화 UI
   ...
   * `cq:dialog` (  `nt:unstructured`) 노드:
      ...
      * 표준 Sling 컨텐츠 구조로서 속성 `sling:resourceType`이 있어야 합니다.
      * 도움말 아이콘( ? `helpPath` 아이콘)을 선택합니다.
...
* 클래식 UI
   ...
   * `dialog` (  `cq:Dialog`노드)
      ...
      * **도움말** 단추를 선택할 때 액세스하는 상황에 맞는 도움말 리소스(절대 또는 상대 경로)를 정의하는 속성 `helpPath`이 있을 수 있습니다.
   ...
   * 대화 상자를 `cq:Dialog` 만들 수 있습니다. 이 대화 상자는 텍스트 구성 요소와 같이 단일 탭을 제공하거나, 텍스트 이미지 구성 요소와 같이 여러 탭이 필요한 경우 대화 상자를 `cq:TabPanel`로 정의할 수 있습니다.
   * `cq:WidgetCollection`( `items`)은 입력 필드( `cq:Widget`) 또는 추가 탭( `cq:Widget`)의 베이스를 제공하는 데 사용됩니다. 이 계층은 확장할 수 있습니다.
...
[디자인 대화 상자는 제목 및 이미지](/help/sites-authoring/default-components-designmode.md)와 같은 모든 구성 요소에 필요한 것은 아니지만 디자인 모드에서 사용할 수  **** 있지만  **** 모두 디자인 대화  **** 상자는 사용할 수 없습니다.
...
1. 아래의 템플릿 정의의 `components` 속성에 필요한 구성 요소를 추가합니다.
...
* 컨텐츠 내에서 작성자 컨텐츠를 포함하는 속성 `jcr:title`을 생성합니다.
...
* **리소스 유형 계층**
   ...
   이 확장은 `sling:resourceSuperType` 속성을 사용하여 구성 요소를 확장하는 데 사용됩니다. 이렇게 하면 구성 요소가 상속될 수 있습니다. 예를 들어, 텍스트 구성 요소는 표준 구성 요소의 다양한 속성을 상속합니다.
...
* **컨테이너 계층**
   ...
   `cq:editConfig` 및 `cq:childEditConfig`의 구성 설정(편집 기능과 관련된)이 전파됩니다.
...
* `cq:editConfig`의 하위 노드를 찾기 위해 예를 들어 `cq:DropTargetConfig` 유형의 `cq:dropTargets`을 검색할 수 있습니다.** CRXDE Lite**에서 쿼리 도구를 사용하고 다음 XPath 쿼리 문자열로 검색할 수 있습니다.
...
### cq:EditConfig 속성 {#configuring-with-cq-editconfig-properties}으로 구성

### cq:actions {#cq-actions}

`cq:actions` 속성( `String array`)은 구성 요소에서 수행할 수 있는 하나 또는 여러 작업을 정의합니다. 구성에 다음 값을 사용할 수 있습니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>정적 텍스트 값 &lt;일부 텍스트&gt;<br /> 클래식 UI에서만 표시됩니다. 터치 활성화 UI는 상황별 메뉴에 작업을 표시하지 않으므로 적용할 수 없습니다.</td> 
  </tr> 
  <tr> 
   <td>-</td> 
  ...
  <tr> 
   ...
   <td>구성 요소를 편집할 단추를 추가합니다.</td> 
    ...
    <td>단추를 추가하여 구성 요소를 편집할 수 있을 뿐만 아니라 <a href="/help/sites-authoring/annotations.md">annotations</a>도 사용할 수 있습니다.</td>
   </tr>
  <tr> 
...
구성 요소를 편집 대화 상자에 연결할 수 있습니다. `cq:dialogMode` 속성( `String`)은 구성 요소 대화 상자가 클래식 UI에서 열리는 방식을 정의합니다. 다음 값을 사용할 수 있습니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><code>auto</code></td> 
   <td>구성 요소 너비가 클라이언트 측 <code>CQ.themes.wcm.EditBase.INLINE_MINIMUM_WIDTH</code> 값보다 작은 경우 대화 상자가 플로팅되고, 그렇지 않으면 인라인으로 표시됩니다.</td> 
  </tr> 
 </tbody> 
</table>
...
* `fullscreen` 값을 사용하면 대화 상자가 전체 화면에서 열립니다.
...
`cq:dropTargets` 노드(노드 유형 `nt:unstructured`)는 컨텐츠 파인더에서 드래그한 자산에서 드롭을 허용할 수 있는 드롭 대상 목록을 정의합니다. 이 클래스는 `cq:DropTargetConfig` 유형의 노드 모음으로 사용됩니다.
...
`cq:DropTargetConfig` 유형의 각 하위 노드는 구성 요소에 드롭 대상을 정의합니다. 노드 이름은 다음과 같이 JSP에서 사용해야 효과적인 드롭 대상인 DOM 요소에 할당된 CSS 클래스 이름을 생성할 수 있으므로 중요합니다.
...
다음 구성은 다운로드 구성 요소에서 가져옵니다. 이를 통해 `media` 그룹의 모든 자산(mime 유형은 임의의 문자열일 수 있음)을 컨텐츠 파인더에서 구성 요소로 끌어 놓을 수 있습니다. 드롭 후 구성 요소 속성 `fileReference`이(가) 업데이트됩니다.
...
`cq:actionConfigs` 노드(노드 유형 `nt:unstructured`)는 `cq:actions` 속성으로 정의된 목록에 추가되는 새 작업 목록을 정의합니다. `cq:actionConfigs`의 각 하위 노드는 위젯을 정의하여 새 작업을 정의합니다.
...
* **주석 관리** 단추를 사용하여 `CQ_collab_forum_openCollabAdmin()` 처리기 함수를 실행합니다.
...
>[!NOTE]
>
>터치 지원 UI의 예로서 [구성 요소 도구 모음에 새 작업 추가](/help/sites-developing/customizing-page-authoring-touch.md#add-new-action-to-a-component-toolbar) 를 참조하십시오.

### cq:formParameters {#cq-formparameters}

`cq:formParameters` 노드(노드 유형 `nt:unstructured`)는 대화 상자 양식에 추가되는 추가 매개 변수를 정의합니다. 각 속성은 양식 매개 변수에 매핑됩니다.

다음 구성은 `photos/primary` 값으로 설정된 `name` 매개 변수를 대화 상자 양식에 추가합니다.
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>(<code>String</code>) 편집기 유형입니다. 사용 가능한 유형은 다음과 같습니다.</p> 
    <ul> 
     <li>일반 텍스트:비 HTML 컨텐츠에 사용할 수 있습니다.<br /> </li> 
     <li>제목:편집이 시작되기 전에 그래픽 제목을 일반 텍스트로 변환하는 향상된 일반 텍스트 편집기입니다. Geometrixx 제목 구성 요소에서 사용됩니다.<br /> </li> 
     <li>텍스트:HTML 콘텐츠에 사용할 대상(리치 텍스트 편집기 사용).<br /> </li> 
    </ul> </td> 
  </tr> 
 </tbody> 
</table>

다음 구성은 구성 요소를 즉석 편집할 수 있도록 하고 `plaintext` 을 편집기 유형으로 정의합니다.
...
>[!NOTE]
>
>중첩된 구성 요소의 경우 `cq:listeners` 노드에서 속성으로 정의된 작업에 특정 제한이 있습니다.
>
>* 중첩된 구성 요소의 경우 다음 속성 *의 값은*&#x200B;이 `REFRESH_PAGE`여야 합니다.
   >
   >  
* `aftermove`
>  * `aftercopy`
