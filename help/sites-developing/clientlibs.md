...
## AEM에서 클라이언트 측 라이브러리 작동 방식 {#how-client-side-libraries-work-in-aem}

페이지의 HTML에 클라이언트측 라이브러리(즉, JS 또는 CSS 파일)를 포함하는 표준 방법은 간단히 를 포함하는 것입니다. `<script>` 또는 `<link>` 태그에 다음 코드를 배치하십시오. 예를 들어
...
이 접근 방식은 AEM에서 작동하지만 페이지 및 구성 구성 요소가 복잡해질 때 문제가 발생할 수 있습니다. 이러한 경우 동일한 JS 라이브러리의 여러 복사본이 최종 HTML 출력에 포함될 수 있는 위험이 있습니다. 이를 방지하고 AEM에서 사용하는 클라이언트 측 라이브러리의 논리 구성을 허용하려면 다음을 수행하십시오 **클라이언트 측 라이브러리 폴더**.

클라이언트측 라이브러리 폴더는 유형의 저장소 노드입니다 `cq:ClientLibraryFolder`. 정의 [CND 표기법](https://jackrabbit.apache.org/node-type-notation.html) is
...
기본적으로 `cq:ClientLibraryFolder` 노드는 내부의 어느 곳에든 배치할 수 있습니다 `/apps`, `/libs` 및 `/etc` 저장소의 하위 트리(이러한 기본값 및 기타 설정은 **Adobe Granite HTML 라이브러리 관리자** 패널의 [시스템 콘솔](http://localhost:4502/system/console/configMgr)).
...
* `allowProxy`: 클라이언트 라이브러리가 아래에 있는 경우 `/apps`로 지정하는 경우 이 속성을 사용하면 프록시 서블릿을 통해 액세스할 수 있습니다. 자세한 내용은 [클라이언트 라이브러리 폴더 찾기 및 프록시 클라이언트 라이브러리 서블릿 사용](/help/sites-developing/clientlibs.md#locating-a-client-library-folder-and-using-the-proxy-client-libraries-servlet) 아래의 제품에서 사용할 수 있습니다.
...
HTL에서 클라이언트 라이브러리는 AEM에서 제공하는 도우미 템플릿을 통해 로드되며, 이 템플릿은 를 통해 액세스할 수 있습니다 [ `data-sly-use`](https://helpx.adobe.com/experience-manager/htl/using/block-statements.html#use). 이 파일에서는 세 개의 템플릿을 사용할 수 있으며, 이 템플릿은 를 통해 호출할 수 있습니다 [ `data-sly-call`](https://helpx.adobe.com/experience-manager/htl/using/block-statements.html#template-call):
...
추가 `ui:includeClientLib` 태그에 태그를 추가하여 생성된 HTML 페이지에서 클라이언트 라이브러리에 링크를 추가합니다. 라이브러리를 참조하려면 `categories` 속성 `ui:includeClientLib` 노드 아래에 있어야 합니다.
...
예: `/etc/clientlibs/foundation/jquery` node는 type입니다. `cq:ClientLibraryFolder` 값의 카테고리 속성 사용 `cq.jquery`. JSP 파일의 다음 코드는 라이브러리를 참조합니다.
...
JS, CSS 또는 테마 라이브러리 필터링을 위한 특성을 포함한 전체 정보는 를 참조하십시오 [ui:includeClientLib](/help/sites-developing/taglib.md#amp-lt-ui-includeclientlib).

>[!CAUTION]
>
>`<cq:includeClientLib>`이전에는 클라이언트 라이브러리를 포함하는 데 일반적으로 사용되던 가 AEM 5.6 이후로 더 이상 사용되지 않습니다. [ `<ui:includeClientLib>`](/help/sites-developing/taglib.md#amp-lt-ui-includeclientlib) 는 위에 자세히 설명된 대로 대신 사용해야 합니다.

## 클라이언트 라이브러리 폴더 만들기 {#creating-client-library-folders}

만들기 `cq:ClientLibraryFolder` 노드 를 사용하여 JavaScript 및 계단식 스타일 시트 라이브러리를 정의하고 HTML 페이지에서 사용할 수 있도록 합니다. 를 사용하십시오 `categories` 노드의 속성을 사용하여 해당 노드가 속한 라이브러리 카테고리를 식별합니다.

이 노드에는 런타임 시 단일 JS 및/또는 CSS 파일에 병합되는 하나 이상의 소스 파일이 포함되어 있습니다. 생성된 파일의 이름은 `.js` 또는 `.css` 파일 이름 확장명을 사용합니다. 예를 들어, 이름이 인 라이브러리 노드 `cq.jquery` 생성된 파일의 결과 `cq.jquery.js` 또는 `cq.jquery.css`.
...
위젯용 클라이언트 라이브러리에만 해당하는 요구 사항에 대한 자세한 내용은 [위젯 사용 및 확장](/help/sites-developing/widgets.md).

웹 클라이언트에는 `cq:ClientLibraryFolder` 노드 아래에 있어야 합니다. 저장소의 보안 영역에서 라이브러리를 노출할 수도 있습니다(아래 다른 라이브러리의 코드 포함 참조).

### /lib에서 라이브러리 재정의 {#overriding-libraries-in-lib}

아래에 있는 클라이언트 라이브러리 폴더 `/apps` 비슷한 위치에 있는 동일한 이름의 폴더보다 우선합니다. `/libs`. 예, `/apps/cq/ui/widgets` 우선 순위 `/libs/cq/ui/widgets`. 이러한 라이브러리가 동일한 카테고리에 속하면 아래 라이브러리가 제공됩니다 `/apps` 이 사용됩니다.

### 클라이언트 라이브러리 폴더 찾기 및 프록시 클라이언트 라이브러리 서블릿 사용 {#locating-a-client-library-folder-and-using-the-proxy-client-libraries-servlet}

이전 버전에서는 클라이언트 라이브러리 폴더가 아래에 있었습니다 `/etc/clientlibs` 로그인합니다. 이 기능은 계속 지원되지만 클라이언트 라이브러리는 이제 다음에 있는 것이 좋습니다 `/apps`. 일반적으로 아래에 있는 다른 스크립트 근처에서 클라이언트 라이브러리를 찾기 위한 것입니다 `/apps` 및 `/libs`.

>[!NOTE]
>
>클라이언트 라이브러리 폴더 아래의 정적 리소스는 *리소스*. 폴더 아래에 이미지와 같은 정적 리소스가 없는 경우 *리소스*&#x200B;게시 인스턴스에서 참조할 수 없습니다. 다음은 한 예입니다. http://localhost:4503/etc.clientlibs/geometrixx/components/clientlibs/resources/example.gif

>[!NOTE]
>
>코드와 컨텐츠를 분리하고 구성을 더 잘 구분하려면 아래에서 클라이언트 라이브러리를 찾는 것이 좋습니다 `/apps` 그들을 `/etc.clientlibs` 다음을 활용하여 `allowProxy` 속성을 사용합니다.
...
그런 다음 `allowProxy` 속성 `foo` true로 설정합니다.

* 그런 다음 요청할 수 있습니다 `/etc.clientlibs/myprojects/clientlibs/foo.js`
...
>[!CAUTION]
>
>Adobe에서 클라이언트 라이브러리를 찾을 것을 권장합니다. `/apps` 프록시 서블릿을 사용하여 사용할 수 있도록 합니다. 그러나 우수 사례를 따르려면 공용 사이트에 를 통해 직접 제공되는 항목이 포함되어 있지 않아야 합니다 `/apps` 또는 `/libs` 경로.
...
1. 라이브러리 파일의 이름을 입력하고 유형 목록에서 을 선택합니다 `cq:ClientLibraryFolder`. 클릭 **확인** 을 클릭한 다음 **모두 저장**.
...
클라이언트 라이브러리 폴더의 코드가 다른 라이브러리를 참조하면 다른 라이브러리를 종속성으로 식별합니다. JSP에서 `ui:includeClientLib` 클라이언트 라이브러리 폴더를 참조하는 태그로 인해 HTML 코드가 생성된 라이브러리 파일과 종속성에 대한 링크를 포함합니다.
...
예: / `etc/clientlibs/myclientlibs/publicmain` 는 `cq.jquery` 라이브러리. 기본 클라이언트 라이브러리를 참조하는 JSP는 다음 코드를 포함하는 HTML을 생성합니다.
...
모든 응용 프로그램 관련 파일을 아래의 응용 프로그램 폴더에 보관하는 것이 가장 좋습니다 `/app`. 또한 웹 사이트 방문자가 `/app` 폴더를 입력합니다. 두 가지 우수 사례를 모두 만족하려면 아래에 클라이언트 라이브러리 폴더를 만드십시오. `/etc` 아래에 있는 클라이언트 라이브러리를 포함하는 폴더 `/app`.
...
경우에 따라 게시 인스턴스가 일반적인 페이지에 대해 생성한 최종 HTML에 상대적으로 많은 수가 포함되어 있을 수 있습니다 `<script>` 요소, 특히 사이트에서 analytics 또는 타깃팅에 클라이언트 컨텍스트 정보를 사용하는 경우. 예를 들어 최적화되지 않은 프로젝트에서 다음 일련의 `<script>` 페이지에 대한 HTML의 요소:
...
다음 클라이언트 라이브러리 카테고리는 AEM에 포함되어 있습니다. 특정 사이트의 기능에 필요한 만 포함해야 합니다. 하지만, **여기에 나열된 순서를 유지해야 합니다.**:
...
를 사용하십시오 `channels` 라이브러리를 사용하는 모바일 그룹을 식별하는 클라이언트 라이브러리 폴더의 속성입니다. 다음 `channels` 속성은 동일한 카테고리의 라이브러리가 다른 장치 기능용으로 디자인될 때 유용합니다.

클라이언트 라이브러리 폴더를 장치 그룹에 연결하려면 속성을 `cq:ClientLibraryFolder` 노드(다음 특성 포함):
...
1. 의 Apache Felix 구성 관리자로 이동합니다. [http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr)
...
### 포함된 파일 을 참조하십시오. {#see-embedded-files}
...
앞의 예에서 [다른 라이브러리의 코드 포함](/help/sites-developing/clientlibs.md#embedding-code-from-other-libraries) 섹션, `/etc/client/libraries/myclientlibs/publicmain` 클라이언트 라이브러리 폴더에 포함 `/apps/myapp/clientlib` 클라이언트 라이브러리 폴더. 웹 페이지에 매개 변수를 추가하면 웹 페이지의 소스 코드에 다음 링크가 생성됩니다.
...
HTML 라이브러리 관리자 서비스 프로세스 `cq:ClientLibraryFolder` 태그를 지정하고 런타임에 라이브러리를 생성합니다. 환경, 개발 또는 프로덕션의 유형에 따라 서비스를 구성하는 방법이 결정됩니다.

* 보안 강화: 디버깅 비활성화
* 성능 향상: 공백을 제거하고 라이브러리를 압축합니다.
* 가독성 개선: 공백을 포함하고 압축하지 않습니다.

서비스 구성에 대한 자세한 내용은 [AEM HTML 라이브러리 관리자](/help/sites-deploying/osgi-configuration-settings.md).
