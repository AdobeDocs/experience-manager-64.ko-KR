...
* 흐름 시작 및 **[!UICONTROL 흐름]** 종료 **[!UICONTROL 단계입니다]**.
...
1. 워크플로우 **[!UICONTROL 모델]** 콘솔을 엽니다.도구, **[!UICONTROL 워크플로우]**, **[!UICONTROL 모델]**&#x200B;또는 **[!UICONTROL 예를]** 들면다음과 같습니다.

   [http://localhost:4502/aem/workflow](http://localhost:4502/aem/workflow)

1. Select **[!UICONTROL Create]**, then **[!UICONTROL Create Model]**.
1. 워크플로우 **[!UICONTROL 모델 추가]** 대화 상자가 나타납니다. 완료를 선택하기 **[!UICONTROL 전에]** 제목 및 **[!UICONTROL 이름]** (선택 사항)을 **[!UICONTROL 입력합니다]**.
1. 새 모델은 워크플로우 모델 **[!UICONTROL 콘솔에]** 나열됩니다.
1. 새 워크플로우를 선택한 다음 편집을 사용하여 [**[!UICONTROL 구성을&#x200B;]**위해 엽니다](#editing-a-workflow).
...
* 작업 과정이 일시적인지 [여부](#configuring-workflow-stages-that-show-workflow-progress)및/또는 여러 리소스를 [사용할지 여부를](#creating-a-transient-workflow) 포함한 작업 흐름 속성 [구성](#configuring-a-workflow-for-multi-resource-support)

기본 [**또는 이전&#x200B;**(즉시 사용) 워크플로우를](#editing-a-default-or-legacy-workflow-for-the-first-time)편집하는 경우 변경[전에](/help/sites-developing/workflows-best-practices.md#locations-workflow-models)안전한 복사본이생성되는지 확인하는 추가 단계가 있습니다.

워크플로우에 대한 업데이트가 완료되면 [동기화]를 사용하여 **[!UICONTROL 런타임]** 모델 **[!UICONTROL 생성을 사용해야 합니다]**. 자세한 [내용은 워크플로우](#sync-your-workflow-generate-a-runtime-model) 동기화를 참조하십시오.

### 워크플로우 동기화 - 런타임 모델 생성 {#sync-your-workflow-generate-a-runtime-model}

**동기화** (편집기 도구 모음에서 바로)는 [런타임 모델을](/help/sites-developing/workflows.md#runtime-model)생성합니다. 런타임 모델은 사용자가 워크플로우를 시작할 때 실제로 사용되는 모델입니다. 변경 내용을 **[!UICONTROL 동기화하지]** 않으면 변경 내용을 런타임에 사용할 수 없습니다.

사용자(또는 다른 사용자)가 작업 과정을 변경하면 **[!UICONTROL 개별 대화 상자]** (예: 단계)에 자체 저장 옵션이 있는 경우에도 Sync를 사용하여 런타임 모델을 생성해야 합니다.

변경 사항이 런타임(저장된) 모델과 동기화되면 **[!UICONTROL 동기화된]** 것이 대신 표시됩니다.

일부 단계에는 필수 필드 및/또는 유효성 검사가 내장되어 있습니다. 이러한 조건이 충족되지 않으면 모델 동기화를 시도하면 오류가 **[!UICONTROL 표시됩니다]** . 예를 들어 참가자 단계에 대해 정의된 참가자가 없는 **[!UICONTROL 경우]** :
...
* 단계 **[!UICONTROL 브라우저는]** 사용할 수 없습니다(왼쪽).
* 도구 모음( **[!UICONTROL 오른쪽]** )에서 사용 가능한 편집 작업이 있습니다.
...
* 워크플로우의 사본을 `/conf`
* 단계 **[!UICONTROL 브라우저를]** 사용할 수 있도록 설정
* 변경을 수행할 수 있도록 설정
...
모델을 편집할 때 단계 브라우저의 다양한 그룹에 사용 가능한 단계가 **[!UICONTROL 나타납니다]** . 예:
...
1. 편집할 기존 워크플로우 모델을 엽니다. 워크플로우 모델 **[!UICONTROL 콘솔에서]** 필요한 모델을 선택한 다음 편집을 **[!UICONTROL 선택합니다]**.
1. 단계 **[!UICONTROL 브라우저를]** 엽니다.상단 **[!UICONTROL 도구 모음의 맨]**&#x200B;왼쪽에 있는 [사이드 패널 전환]을 사용합니다. 여기에서 다음을 수행할 수 있습니다.
...
1. 동기화(편집기 도구 **[!UICONTROL 모음]** )를 사용하여 변경 사항을 확인하여 런타임 모델을 생성합니다.
...
1. 단계 속성 **[!UICONTROL 대화 상자를]** 열려면 다음 중 하나를 수행합니다.

   * 워크플로우 모델의 단계를 누르고 구성 요소 도구 **[!UICONTROL 모음에서 구성을]** 선택합니다.
   * 단계를 두 번 클릭합니다.
   >[!NOTE]
   >
   >AEM과 함께 설치되는 기본 단계 구성 요소에 대한 자세한 내용은 워크플로우 [단계 참조를 참조하십시오](/help/sites-developing/workflows-step-ref.md).

1. 필요에 **[!UICONTROL 따라 단계]** 속성을 구성합니다.사용 가능한 속성은 단계 유형에 따라 다르며, 사용 가능한 여러 탭이 있을 수도 있습니다. 예를 들어, 기본 참가자 **[!UICONTROL 단계가]**&#x200B;새 워크플로우에 있는 `Step 1`경우
...
1. 워크플로우 모델을 열어 [편집합니다](#editing-a-workflow).
1. 도구 **[!UICONTROL 모음에서 워크플로우 모델]** 속성을 선택합니다.
1. 대화 상자에서 임시 **[!UICONTROL 워크플로우를 활성화]** (또는 필요한 경우 비활성화)합니다.

   ![wf-07](assets/wf-07.png)

1. 저장 및 닫기로 **[!UICONTROL 변경 사항을 확인합니다]**.다음으로 **[!UICONTROL 동기화]** (편집기 도구 모음)를 사용하여 런타임 모델을 생성합니다.
...
1. 편집을 **[!UICONTROL 선택한]** 다음 워크플로우 모델 **[!UICONTROL 속성을 엽니다]**. 기본 **[!UICONTROL 탭으로]** 이동합니다.
...
1. 대화 상자에서 다중 리소스 **[!UICONTROL 지원을 활성화]** (또는 필요한 경우 비활성화):
...
>[!CAUTION]
>
>워크플로우 단계가 페이지 **[!UICONTROL 속성에]**&#x200B;정의되었지만 워크플로우 단계에 사용되지 않는 경우 현재 워크플로우 단계에 상관없이 진행률 표시줄은 아무런 진행도 표시하지 않습니다.
...
1. 필요한 단계를 추가(및 배치) **[!UICONTROL 합니다]**. 워크플로우 모델에 대해 원하는 단계 수를 정의할 수 있습니다.
...
1. 패키지 관리자를 사용하여 새 패키지를 [만듭니다](/help/sites-administering/package-manager.md#package-manager).
   ...
   1. 도구, 배포, 패키지를 **[!UICONTROL 통해]**&#x200B;패키지 **[!UICONTROL 관리자로]****[!UICONTROL 이동합니다]**.
   1. 패키지 **[!UICONTROL 만들기를 클릭합니다]**.
   1. 패키지 **[!UICONTROL 이름]**&#x200B;및 기타 세부 사항을 필요에 따라 지정합니다.
...
1. 필터 **[!UICONTROL 추가를]** 선택하고 워크플로우 모델 *디자인의*&#x200B;경로를 지정합니다.
...
1. 저장을 **[!UICONTROL 클릭하여]** 필터 정의를 확인합니다.
1. 패키지 **[!UICONTROL 정의의]** 도구 모음에서 빌드를 선택합니다.
...
1. 페이지에 **[!UICONTROL 표시되는]** 양식 시작 구성 요소를 구성합니다.
...
1. 페이로드 **[!UICONTROL 정의]** 및 확인

1. 워크플로우가 진행되도록 필요한 작업을 수행합니다.
1. 워크플로가 실행되는 동안 로그 파일을 모니터링합니다.

로그 파일에 디버그 **[!UICONTROL 메시지를 표시하도록 AEM을]** 구성할 수도 있습니다. 자세한 [내용은](/help/sites-deploying/configure-logging.md) 로깅을 **[!UICONTROL 참조하고 개발이 완료되면 로그]** 수준을 다시 정보로 **[!UICONTROL 설정합니다]**.
...
1. 삭제 `Step 1` (이 예제의 잘못된 단계 유형이므로):
   ...
   * 단계를 클릭하고 구성 요소 도구 **[!UICONTROL 모음에서]** 삭제를 선택합니다. 동작을 확인합니다.

1. 브라우저 **[!UICONTROL 워크플로우의]** **[!UICONTROL 선택]** 워크플로우에서 참가자 단계를 **[!UICONTROL 워크플로우로 드래그하고 흐름 시작]** 단계와 **흐름 끝[!UICONTROL *]단계 사이에 배치합니다.
1. 속성 대화 상자를 열려면 다음 중 하나를 수행합니다.

   * 참가자 단계를 클릭하고 구성 요소 도구 **[!UICONTROL 모음에서 구성을]** 선택합니다.
   * 참가자 단계를 두 번 클릭합니다.

1. 일반 **[!UICONTROL 탭에서]** 제목 및 설명 `Validate Content` 모두에 **[!UICONTROL 대해]** 입력합니다 ****.
1. 사용자/ **[!UICONTROL 그룹]** 탭을 엽니다.

   * Activate **[!UICONTROL Notify user via email]**.
   * 사용자/그룹 `Administrator` 필드에 대해 ( `admin`) **[!UICONTROL 를]** 선택합니다.
   >[!NOTE]
   >
...
1. 워크플로우로 **[!UICONTROL 또는 분할을]** 드래그한 다음 흐름 `Validate Content` 끝과 흐름 **[!UICONTROL 끝 사이에 배치합니다]**.
1. 구성을 **[!UICONTROL 위해 또는]** 분할을 엽니다.
1. 구성:

   * **[!UICONTROL 일반]**:분기 **[!UICONTROL 2개 선택]**
   * **[!UICONTROL 분기 1]**:기본 **[!UICONTROL 경로를 선택합니다]**.
   * **[!UICONTROL 분기 2]**:기본 **[!UICONTROL 경로를]** 선택하지 않았는지 확인합니다.

1. OR 분할에 대한 업데이트를 **[!UICONTROL 확인합니다]**.
1. 참가자 **[!UICONTROL 단계를]** 왼쪽 분기로 드래그하여 속성을 열고 다음 값을 지정한 다음 변경 사항을 확인합니다.

   * **[!UICONTROL 제목]**: `Reject Publish Request`
   * **[!UICONTROL 사용자/그룹]**:예를 들면 `projects-administrators`
   * **[!UICONTROL 이메일을]**&#x200B;통해 사용자에게 알림:활성화하여 사용자가 이메일로 알림을 받도록 하십시오.

1. 프로세스 **[!UICONTROL 단계를]** 오른쪽 분기로 드래그하여 속성을 열고 다음 값을 지정한 다음 변경 사항을 확인합니다.

   * **[!UICONTROL 제목]**: `Publish Page as Requested`
   * **[!UICONTROL 프로세스]**:을 `Activate Page`선택합니다. 이 프로세스에서는 선택한 페이지를 게시자 인스턴스에 게시합니다.

1. 동기화( **[!UICONTROL 편집기]** 도구 모음)를 클릭하여 런타임 모델을 생성합니다.
...
1. 사용자가 컨텐츠 유효성 확인 단계를 완료하기 **[!UICONTROL 로]** 이동할 때 **[!UICONTROL 페이지를]** 요청된 **[!UICONTROL 상태로 게시할지 또는 게시]**&#x200B;요청 **[!UICONTROL 거부를 선택할 수]**&#x200B;있도록이 워크플로우를 페이지에 적용합니다.

   ![chlimage_1-182](assets/chlimage_1-182.png)

### 예:OR 분할에 대한 규칙 정의 {#example-defining-a-rule-for-an-or-split}

**[!UICONTROL 또는 분할]** 단계를 통해 조건부 처리 경로를 워크플로우에 적용할 수 있습니다.
...
1. 워크플로우를 편집하고 모델에 OR **[!UICONTROL 분할을]** 추가합니다.
1. OR 분할의 **[!UICONTROL 분기]** 1 속성을 **[!UICONTROL 편집합니다]**.

   * 값을 로 **[!UICONTROL 설정하여]** 이 경로를 **[!UICONTROL 기본]** 경로로 `true`정의합니다.
...
1. OR 분할의 **[!UICONTROL 분기 2]** 속성을 **[!UICONTROL 편집합니다]**.
   ...
   * 규칙을 **[!UICONTROL 사용하여]**&#x200B;경로를 다른 스크립트로 설정합니다. 예:
...
예를 들어 활성화 **[!UICONTROL 요청]**. 이 워크플로우는 사이트 내에서 페이지를 게시하는 **[!UICONTROL 데]** 사용되며 컨텐츠 작성자에게 적절한 복제 권한이 없을 때 자동으로 트리거됩니다. 자세한 [내용은 페이지 작성 사용자 지정 - 활성화 요청 워크플로우](/help/sites-developing/customizing-page-authoring-touch.md#customizing-the-request-for-activation-workflow) 사용자 정의를 참조하십시오.
