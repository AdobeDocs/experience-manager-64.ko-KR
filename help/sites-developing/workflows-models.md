...
>[!CAUTION]
>
>클래식 UI를 사용하려면 [AEM 6.3 설명서](https://helpx.adobe.com/experience-manager/6-3/sites-developing/workflows-models.html) 참조하십시오.
...
* 단계, **[!UICONTROL 흐름 시작]** 및 **[!UICONTROL 흐름 끝]**.
...
1. 를 엽니다. **[!UICONTROL 워크플로우 모델]** console; 다음을 통해 **[!UICONTROL 도구]**, **[!UICONTROL 워크플로우]**, **[!UICONTROL 모델]** 또는 예:

   [http://localhost:4502/aem/workflow](http://localhost:4502/aem/workflow)

1. 선택 **[!UICONTROL 만들기]**, 그런 다음 **[!UICONTROL 모델 만들기]**.
1. 다음 **[!UICONTROL 워크플로우 모델 추가]** 대화 상자가 나타납니다. 을(를) 입력합니다. **[!UICONTROL 제목]** 및 **[!UICONTROL 이름]** (선택 사항) **[!UICONTROL 완료]**.
1. 새 모델은 **[!UICONTROL 워크플로우 모델]** 콘솔.
1. 새 워크플로우를 선택한 다음 [**[!UICONTROL 편집&#x200B;]**구성에 대해 열기](#editing-a-workflow):
...
* [단계 정의](#adding-a-step-to-a-model) 그리고 [매개 변수](#configuring-a-workflow-step)

* 다음을 포함한 워크플로우 속성 구성 [단계](#configuring-workflow-stages-that-show-workflow-progress), [워크플로우가 일시적인지 여부](#creating-a-transient-workflow) 및/또는 [여러 리소스 사용](#configuring-a-workflow-for-multi-resource-support)

편집 [**기본값 또는 이전** (즉시 사용 가능한) 워크플로우](#editing-a-default-or-legacy-workflow-for-the-first-time) 에는 [안전 복사](/help/sites-developing/workflows-best-practices.md#locations-workflow-models) 가 변경되기 전에 변경되었습니다.

워크플로우에 대한 업데이트가 완료되면 **[!UICONTROL 동기화]** to **[!UICONTROL 런타임 모델 생성]**. 자세한 내용은 [워크플로우 동기화](#sync-your-workflow-generate-a-runtime-model) 자세한 내용

### 워크플로우 동기화 - 런타임 모델 생성 {#sync-your-workflow-generate-a-runtime-model}

**동기화** (편집기 도구 모음에서 오른쪽)에서 [런타임 모델](/help/sites-developing/workflows.md#runtime-model). 런타임 모델은 사용자가 워크플로우를 시작할 때 실제로 사용되는 모델입니다. 그렇지 않으면 **[!UICONTROL 동기화]** 그러면 변경 사항이 런타임 시 사용할 수 없습니다.

(또는 다른 사용자)가 워크플로우를 변경할 때 사용해야 합니다 **[!UICONTROL 동기화]** 개별 대화 상자(예: 단계)에 고유한 저장 옵션이 있는 경우에도 런타임 모델을 생성합니다.

변경 사항이 런타임(저장된) 모델과 동기화되면 **[!UICONTROL 동기화]** 가 대신 표시됩니다.

일부 단계에는 필수 필드 및/또는 유효성 검사가 빌드되어 있습니다. 이러한 조건이 충족되지 않으면 **[!UICONTROL 동기화]** 모델. 예를 들어, **[!UICONTROL 참가자]** 단계:
...
* 다음 **[!UICONTROL 단계]** 브라우저를 사용할 수 없습니다(왼쪽).
* 다음 항목이 있습니다 **[!UICONTROL 편집]** 작업 은 도구 모음(오른쪽)에서 사용할 수 있습니다.
* 처음에는 모델과 해당 속성이 다음과 같이 읽기 전용 모드로 표시됩니다.

   * 기본 워크플로우는 `/libs`
   * 기존 워크플로우는 `/etc`

선택 **[!UICONTROL 편집]** 다음 작업을 수행합니다.

* 워크플로우의 사본을 로 `/conf`
* make **[!UICONTROL 단계]** 브라우저 사용 가능
* 변경할 수 있도록 설정
...
모델을 편집할 때 사용 가능한 단계가 다양한 **[!UICONTROL 단계]** 브라우저. 예:
...
1. 편집할 기존 워크플로우 모델을 엽니다. 에서 **[!UICONTROL 워크플로우 모델]** 콘솔에서 필요한 모델을 선택한 다음 **[!UICONTROL 편집]**.
1. 를 엽니다. **[!UICONTROL 단계]** 브라우저; 사용 **[!UICONTROL 사이드 패널 전환]**&#x200B;상단 도구 모음의 맨 왼쪽에 있는 를 클릭합니다. 여기에서 다음과 같은 작업을 수행할 수 있습니다.
...
1. 적절한 단계를 모델의 필요한 위치로 드래그합니다.
   ...
   가 플로우에 추가되면 다음 작업을 수행할 수 있습니다 [단계 구성](#configuring-a-workflow-step).
...
1. 다음 방법으로 변경 내용을 확인합니다. **[!UICONTROL 동기화]** (편집기 도구 모음)를 사용하여 런타임 모델을 생성합니다.

   자세한 내용은 [워크플로우 동기화](#sync-your-workflow-generate-a-runtime-model) 자세한 내용

### 워크플로우 단계 구성 {#configuring-a-workflow-step}

다음을 수행할 수 있습니다 **구성** 을 사용하여 워크플로우 단계의 동작을 사용자 정의합니다. **[!UICONTROL 단계 속성]** 대화 상자

1. 를 열려면 **[!UICONTROL 단계 속성]** 다음 중 한 단계의 대화 상자:

   * 워크플로우 모델에서 단계를 탭하고 을(를) 선택합니다 **[!UICONTROL 구성]** 구성 요소 도구 모음에서 를 클릭합니다.
   * 단계를 두 번 클릭합니다.
...
1. 구성 **[!UICONTROL 단계 속성]** 필요한 경우 사용 가능한 속성은 단계 유형에 따라 다르며, 사용 가능한 여러 탭이 있을 수도 있습니다. 예를 들어, 기본값은 **[!UICONTROL 참가자 단계]**, 새 워크플로우에 다음 형식으로 표시 `Step 1`:
...
1. 에 대한 워크플로우 모델 열기 [편집](#editing-a-workflow).
1. 선택 **[!UICONTROL 워크플로우 모델 속성]** 를 클릭합니다.
1. 대화 상자에서 활성화 **[!UICONTROL 임시 워크플로우]** (또는 필요한 경우 비활성화):

   ![wf-07](assets/wf-07.png)

1. 다음 방법으로 변경 내용을 확인합니다. **[!UICONTROL 저장 및 닫기]**; 후 **[!UICONTROL 동기화]** (편집기 도구 모음)를 사용하여 런타임 모델을 생성합니다.

   자세한 내용은 [워크플로우 동기화](#sync-your-workflow-generate-a-runtime-model) 자세한 내용

>[!NOTE]
>
>에서 워크플로우를 실행할 때 [임시](/help/sites-developing/workflows.md#transient-workflows) 모드 AEM은 워크플로우 기록을 저장하지 않습니다. 따라서, [타임라인](/help/sites-authoring/basic-handling.md#timeline) 은 해당 워크플로우와 관련된 정보를 표시하지 않습니다. [](/help/sites-authoring/basic-handling.md#timeline)
...
1. 선택 **[!UICONTROL 만들기 > 태그 만들기]**. 설정 **[!UICONTROL 제목]** 로서의 `DAM` 및 **[!UICONTROL 이름]** 로서의 `dam`. 선택 **[!UICONTROL 제출]**.
...
1. 선택 **[!UICONTROL 편집]** , 그런 다음 엽니다. **[!UICONTROL 워크플로우 모델 속성]**. 로 이동합니다. **[!UICONTROL 기본]** 탭.
...
1. 대화 상자에서 활성화 **[!UICONTROL 다중 리소스 지원]** (또는 필요한 경우 비활성화):
...
>[!CAUTION]
>
>워크플로우 단계가 **[!UICONTROL 페이지 속성]**&#x200B;하지만 워크플로우 단계에는 사용되지 않으면 진행률 표시줄에는 현재 워크플로우 단계와 관계없이 진행 상태가 표시되지 않습니다.

사용할 수 있는 단계는 워크플로우 모델에서 정의됩니다. 기존 워크플로우 모델을 업데이트하여 단계 정의를 포함할 수 있습니다. 워크플로우 모델에 대한 단계 수를 정의할 수 있습니다.

을(를) 정의하려면 **[!UICONTROL 단계]** 워크플로우의 경우:

1. 편집할 워크플로우 모델을 엽니다.
1. 선택 **[!UICONTROL 워크플로우 모델 속성]** 를 클릭합니다. 그런 다음 를 엽니다. **[!UICONTROL 단계]** 탭.
1. 필요한 추가(및 위치) **[!UICONTROL 단계]**. 워크플로우 모델에 대한 단계 수를 정의할 수 있습니다.

   예:

   ![wf-08-1](assets/wf-08-1.png)

1. 클릭 **[!UICONTROL 저장 및 닫기]** 속성을 저장합니다.
...
1. 를 사용하여 새 패키지 만들기 [패키지 관리자](/help/sites-administering/package-manager.md#package-manager):
   ...
   1. 다음 방법으로 패키지 관리자로 이동합니다 **[!UICONTROL 도구]**, **[!UICONTROL 배포]**, **[!UICONTROL 패키지]**.
   1. 클릭 **[!UICONTROL 패키지 만들기]**.
   1. 을(를) 지정합니다. **[!UICONTROL 패키지 이름]**&#x200B;및 기타 모든 세부 사항을 필요에 따라 구성합니다.
   1. **[!UICONTROL 확인]**&#x200B;을 클릭합니다.

1. 클릭 **[!UICONTROL 편집]** 를 클릭합니다.

1. 를 엽니다. **[!UICONTROL 필터]** 탭.

1. 선택 **[!UICONTROL 필터 추가]** 워크플로우 모델의 경로를 지정합니다 *디자인*:

   `/conf/global/settings/workflow/models/<*your-model-name*>`

   클릭 **[!UICONTROL 완료]**.

1. 선택 **[!UICONTROL 필터 추가]** 및 의 경로를 지정합니다 *런타임* 워크플로우 모델:
...
1. 클릭 **[!UICONTROL 저장]** 필터 정의를 확인하려면
1. 선택 **[!UICONTROL 빌드]** 패키지 정의의 도구 모음에서 를 클릭합니다.
1. 선택 **[!UICONTROL 다운로드]** 패키지 도구 모음에서 를 클릭합니다.
...
1. 추가 **[!UICONTROL 양식]** 구성 요소를 생성하지 않습니다.
1. 구성 **[!UICONTROL 양식 시작]** 페이지에 표시된 구성 요소입니다.
1. 사용 **[!UICONTROL 워크플로우 시작]** 사용 가능한 워크플로우에서 원하는 워크플로우를 선택하려면 다음을 수행하십시오.
...
1. 을(를) 정의합니다 **[!UICONTROL 페이로드]** 확인

1. 워크플로우가 계속 진행되도록 필요한 작업을 수행합니다.
1. 워크플로우가 실행되는 동안 로그 파일을 모니터링합니다.

AEM을 표시하도록 구성할 수도 있습니다 **[!UICONTROL 디버그]** 로그 파일에 있는 메시지입니다. 자세한 내용은 [로깅](/help/sites-deploying/configure-logging.md) 자세한 내용 및 개발이 완료되면 **[!UICONTROL 로그 수준]** 뒤로 **[!UICONTROL 정보]**.
...
1. 삭제 `Step 1` (이 예제의 잘못된 단계 유형):
   ...
   * 단계를 클릭하고 을 선택합니다. **[!UICONTROL 삭제]** 구성 요소 도구 모음에서 를 클릭합니다. 작업을 확인합니다.

1. 에서 **[!UICONTROL 워크플로우]** 단계 브라우저를 선택하고 **[!UICONTROL 참가자 단계]** 작업 과정에 배치하여 다음 사이 **[!UICONTROL 흐름 시작]** 및 **[!UICONTROL 흐름 끝*]*.
1. 속성 대화 상자를 열려면 다음 중 하나를 수행합니다.

   * 참가자 단계를 클릭하고 을 선택합니다 **[!UICONTROL 구성]** 구성 요소 도구 모음에서 를 클릭합니다.
   * 참가자 단계를 두 번 클릭합니다.

1. 에서 **[!UICONTROL 공통]** 탭 입력 `Validate Content` 둘 다 **[!UICONTROL 제목]** 및 **[!UICONTROL 설명]**.
1. 를 엽니다. **[!UICONTROL 사용자/그룹]** 탭:

   * 활성화 **[!UICONTROL 이메일을 통해 사용자에게 알림]**.
   * 선택 `Administrator` ( `admin`) **[!UICONTROL 사용자/그룹]** 필드.
...
1. 끌어서 놓기 **[!UICONTROL 또는 분할]** 작업 과정에 배치하여 다음 사이 `Validate Content` 및 **[!UICONTROL 흐름 끝]**.
1. 를 엽니다. **[!UICONTROL 또는 분할]** 참조하십시오.
1. 다음을 구성하십시오.

   * **[!UICONTROL 공통]**: 선택 **[!UICONTROL 2개 분기]**
   * **[!UICONTROL 분기 1]**: 선택 **[!UICONTROL 기본 경로]**.
   * **[!UICONTROL 분기 2]**: 확인합니다. **[!UICONTROL 기본 경로]** 이 선택되어 있지 않습니다.

1. 업데이트에 대한 업데이트를 확인합니다. **[!UICONTROL 또는 분할]**.
1. 드래그 **[!UICONTROL 참가자 단계]** 왼쪽 분기에서 속성을 열고 다음 값을 지정한 다음 변경 사항을 확인합니다.
...
1. 드래그 **[!UICONTROL 프로세스 단계]** 오른쪽 분기에서 속성을 열고 다음 값을 지정한 다음 변경 사항을 확인합니다.

   * **[!UICONTROL 제목]**: `Publish Page as Requested`
   * **[!UICONTROL 프로세스]**: 선택 `Activate Page`. 이 프로세스는 선택한 페이지를 게시자 인스턴스에 게시합니다.

1. 클릭 **[!UICONTROL 동기화]** (편집기 도구 모음)를 사용하여 런타임 모델을 생성합니다.
...
1. 사용자가 이동할 때 이 워크플로우를 페이지에 적용합니다. **[!UICONTROL 완료]** a **[!UICONTROL 컨텐츠 유효성 검사]** 한 단계 후에 **[!UICONTROL 요청한 대로 페이지 게시]**, 또는 **[!UICONTROL 게시 요청 거부]**.

   ![chlimage_1-182](assets/chlimage_1-182.png)

### 예: OR 분할에 대한 규칙 정의 {#example-defining-a-rule-for-an-or-split}

**[!UICONTROL 또는 분할]** 단계를 통해 워크플로우에 조건부 처리 경로를 도입할 수 있습니다.
...
1. 워크플로우를 편집하고 추가 **[!UICONTROL 또는 분할]** 모델에 추가합니다.
1. 속성 편집 **[!UICONTROL 분기 1]** 의 **[!UICONTROL 또는 분할]**:

   * 이를 로 정의합니다 **[!UICONTROL 기본 경로]** 다음을 설정하여 **[!UICONTROL 값]** to `true`.
   * 로서의 **[!UICONTROL 규칙]**&#x200B;를 눌러 스크립트 경로를 설정합니다. 예:
...
1. 의 속성을 편집합니다 **[!UICONTROL 분기 2]** 의 **[!UICONTROL 또는 분할]**.

   * 로서의 **[!UICONTROL 규칙]**&#x200B;를 눌러 경로를 다른 스크립트로 설정합니다. 예:

      `/apps/myapp/workflow/scripts/myscript2.ecma`

1. 각 분기에 있는 개별 단계의 속성을 설정합니다. 다음을 확인합니다. **[!UICONTROL 사용자/그룹]** 이(가) 설정되어 있습니다.
...
예, **[!UICONTROL 활성화 요청]**. 이 워크플로우는 내에서 페이지를 게시하는 데 사용됩니다 **[!UICONTROL Sites]** 컨텐츠 작성자에게 적절한 복제 권한이 없을 때 자동으로 트리거됩니다. 자세한 내용은 [페이지 작성 사용자 지정 - 활성화 요청 사용자 정의](/help/sites-developing/customizing-page-authoring-touch.md#customizing-the-request-for-activation-workflow) 자세한 내용
