...
세션 저장소 활성화 및 등록 의견 수렴 방법을 제공합니다. 세션 [스토어가 정의되고 초기화되었는지 확인하십시오](/help/sites-developing/client-context.md#checking-that-a-session-store-is-defined-and-initialized).
...
지연을 밀리초 `true` 수로 설정하면 메서드가 콜백 메서드를 호출하기 전에 대기합니다. 지연이 전달되기 전에 다른 초기화 이벤트가 트리거되는 경우 초기화 이벤트가 없는 지연 시간이 초과될 때까지 대기합니다. 따라서 두 번째 초기화 이벤트가 트리거될 때까지 기다리고 최적의 경우 콜백 함수를 호출할 수 있습니다.

**매개 변수**

* storeName: 문자열. 수신기를 추가할 세션 저장소의 이름입니다.
* 콜백: 함수. 저장소 초기화 시 호출하는 함수입니다.
* 지연: 부울 또는 숫자. 콜백 함수에 대한 호출을 지연하는 시간(밀리초)입니다. 의 부울 값은 기본 지연을 `true` 사용합니다 `200 ms`. 부울 값 `false` 또는 음수로 인해 지연이 발생하지 않습니다.
...
세션 스토어가 등록될 때 호출되는 콜백 함수를 등록합니다. 등록 이벤트는 스토어가 [CQ_Analytics.ClientContextMgr에 등록될 때 발생합니다](#cq-analytics-clientcontextmgr).
...
JSON 데이터가 포함된 비지속적인 세션 저장소. 외부 JSONP 서비스에서 데이터가 검색됩니다. 이 클래스의 인스턴스를 만들려면 `getInstance` 또는 `getRegisteredInstance` 메서드를 사용합니다.
...
스토어를 식별하는 키. 이 값 `getInstance` 을 검색하려면 메서드를 사용합니다.

#### 스토어이름 {#storename}

스토어의 이름입니다. 이 값 `getInstance` 을 검색하려면 메서드를 사용합니다.
...
세션 저장소를 삭제하고 초기화 속성으로 초기화합니다. 초기화 플래그를 로 설정한 `true` 다음 `initialize` 및 `update` 이벤트를 실행합니다.
...
이벤트에 수신기를 등록합니다. 세션 [저장소 업데이트에 응답할 리스너 만들기를 참조하십시오](/help/sites-developing/client-context.md#creating-a-listener-to-react-to-a-session-store-update).
...
상속된 메서드 `clear``setProperty`를 사용하여 스토어 데이터 `setProperties`를 변경하는 경우 변경된 속성 `removeProperty` 이 [지속되지 않음]으로 플래그되지 않으면 변경 내용이 자동으로 유지됩니다.
...
속성이 지속된 경우 `true` 의 부울 값과 값이 지속적인 속성이 `false` 아닌 경우의 값.

#### persist() {#persist}

세션 저장소를 유지합니다. 기본 지속성 모드는 이름() `localStorage` 으로 브라우저를 `ClientSidePersistence` `window.localStorage.set("ClientSidePersistance", store);`사용합니다
...
* fairEvent: true 값을 사용하면 이벤트가 실행되지 `update` 않습니다. 값 `false` 은 업데이트 이벤트를 발생시킵니다.
...
저장소 데이터를 반환합니다. 선택적으로 데이터에서 이름 속성을 제외합니다. 스토어의 데이터 속성이 없는 경우 메서드를 `init` 호출합니다.
...
데이터 속성의 값입니다. 세션 저장소에 지정된 이름의 속성이 없는 `null` 경우 반환합니다.
...
속성의 값을 반환합니다. 이 값은 원시 속성 또는 XSS가 필터링된 값으로 반환됩니다. 스토어의 데이터 속성이 없는 경우 메서드를 `init` 호출합니다.
...
세션 저장소에 포함된 속성의 이름을 반환합니다. 스토어의 데이터 속성이 없는 경우 메서드를 `init` 호출합니다.
...
스토어가 초기화된 `true` 경우 값, 스토어가 초기화되지 않은 `false` 경우 값
...
세션 저장소에서 속성을 제거합니다. 완료 시 `update` 이벤트를 실행합니다. 스토어의 데이터 속성이 없는 경우 메서드를 `init` 호출합니다.
...
여러 속성의 값을 설정합니다. 완료 시 `update` 이벤트를 실행합니다. 스토어의 데이터 속성이 없는 경우 메서드를 `init` 호출합니다.
...
속성의 값을 설정합니다. 완료 시 `update` 이벤트를 실행합니다. 스토어의 데이터 속성이 없는 경우 메서드를 `init` 호출합니다.
