...
>[!NOTE]
>
>AEM의 핵심 개념으로 이동하기 전에 Adobe은 의 WKND 자습서를 완료하는 것을 권장합니다. [AEM Sites 개발 시작](/help/sites-developing/getting-started.md) AEM 개발 프로세스 개요 및 핵심 개념 소개에 대한 문서를 참조하십시오.
...
또한 을 읽고 따르는 것이 좋습니다 [지침 및 우수 사례](/help/sites-developing/dev-guidelines-bestpractices.md).

## Java Content Repository {#java-content-repository}

JCR(Java Content Repository) 표준, [JSR 283](https://www.adobe.io/experience-manager/reference-materials/spec/jcr/2.0/index.html)를 지정하는 경우, 컨텐츠 리포지토리 내의 세부 수준에서 양방향 컨텐츠에 액세스할 수 있는 공급업체와 독립적이고 구현에 독립적인 방법을 지정합니다.

Specification Lead는 Adobe Research(스위스) AG에 의해 소유됩니다.

다음 [JCR API 2.0](https://www.adobe.io/experience-manager/reference-materials/spec/javax.jcr/javadocs/jcr-2.0/index.html) 패키지, javax.jcr.&amp;ast; 저장소 컨텐츠에 대한 직접 액세스 및 조작에 사용됩니다.
...
[Apache Jackrabbit](https://jackrabbit.apache.org/) 는 JCR API 2.0의 구현을 완벽하게 따르는 오픈 소스입니다.

## Sling 요청 처리 {#sling-request-processing}

### Sling 소개 {#introduction-to-sling}

AEM이 [Sling](https://sling.apache.org/site/index.html)은 컨텐츠 중심 애플리케이션을 쉽게 개발할 수 있도록 REST 원칙을 기반으로 하는 웹 애플리케이션 프레임워크입니다. Sling은 Apache Jackrabbit와 같은 JCR 저장소 또는 AEM의 경우 CRX 컨텐츠 저장소를 데이터 저장소로 사용합니다. Sling은 Apache Software Foundation에 기여하여 Apache에서 추가 정보를 찾을 수 있습니다.
...
모든 Sling 스크립트는 다음 중 하나의 하위 폴더에 저장됩니다 `/apps` 또는 `/libs`: 이 순서로 검색됩니다( 참조). [구성 요소 및 기타 요소 사용자 지정](/help/sites-developing/dev-guidelines-bestpractices.md#customizing-components-and-other-elements)).
...
위의 예를 사용하는 경우 `sling:resourceType` is `hr/jobs` 다음에 대해 다음을 수행합니다.
...
* 선택기가 있는 URL
   ...
   선택기가 있을 수 있는 프린터 친화적인 버전을 살펴보겠습니다 *인쇄*; 로서의 `../content/corporate/jobs/developer.print.html`
...
* http 오류 처리(코드 403 또는 404)의 경우 Sling은 다음 중 하나에서 스크립트를 찾습니다.
   ...
   * 에 대한 위치 /apps/sling/servlet/errorhandler [사용자 지정 스크립트](/help/sites-developing/customizing-errorhandler-pages.md)
...
* 다음을 포함하여 GET 이외의 http 메서드 자동 처리:
   ...
   * a `POST.jsp` sling:resourceType 위치에 스크립트 작성

* 코드 아키텍처는 더 이상 깨끗하거나 구조화되지 않습니다. 대규모 개발을 위한 가장 중요한 요소

### Sling API {#sling-api}

Sling API 패키지 org.apache.sling을 사용합니다.및 태그 라이브러리(&amp;A)
...
이렇게 하려면 sling:include(&quot;/&lt;path>/&lt;resource>&quot;) 명령을 실행합니다. 이렇게 하면 이미지 렌더링을 위한 기존 정의를 참조하는 다음 문과 같이 참조된 리소스의 정의를 효과적으로 포함합니다.
...
이 아키텍처를 통해 Sling을 애플리케이션별 모듈로 확장할 수 있습니다. Sling에서 CQ5에서는 [Apache Felix](https://felix.apache.org/) OSGI(Open Services Gateway Initiative) 구현 및 OSGi Service Platform 릴리스 4 버전 4.2 사양을 기반으로 합니다. 둘 다 OSGi 프레임워크 내에서 실행되는 OSGi 번들 컬렉션입니다.
...
자세한 내용은 [웹 콘솔](/help/sites-deploying/web-console.md), [OSGI 구성](/help/sites-deploying/configuring-osgi.md) 및 [OSGi 구성 설정](/help/sites-deploying/osgi-configuration-settings.md) 추가 정보.
...
항목 객체 조작에 대한 자세한 내용은 [Javadocs](https://docs.adobe.com/docs/en/spec/javax.jcr/javadocs/jcr-2.0/javax/jcr/Item.html) 인터페이스 javax.jcr.Item의

**노드(및 해당 속성)** 노드 및 해당 속성은 JCR API 2.0 사양(JSR 283)에서 정의됩니다. 컨텐츠, 개체 정의, 렌더링 스크립트 및 기타 데이터를 저장합니다.
...
노드 객체 조작에 대한 자세한 내용은 [Javadocs](https://docs.adobe.com/docs/en/spec/javax.jcr/javadocs/jcr-2.0/javax/jcr/Node.html).

**위젯** AEM에서 모든 사용자 입력은 위젯에 의해 관리됩니다. 종종 컨텐츠 편집을 제어하는 데 사용됩니다.
...
**구성 요소** 소프트웨어 구성 요소는 사전 정의된 서비스 또는 이벤트를 제공하며 다른 구성 요소와 통신할 수 있는 시스템 요소입니다.
...
**템플릿** 템플릿은 특정 유형의 페이지에 대한 기준입니다. 웹 사이트 탭에서 페이지를 만들 때는 템플릿을 선택해야 합니다. 그런 다음 이 템플릿을 복사하여 새 페이지를 만듭니다.
...
currentPage가 현재 페이지 개체인 경우 페이지 개체 조작에 대한 자세한 내용은 [Javadocs](https://helpx.adobe.com/kr/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/Page.html).

**페이지 관리자** 페이지 관리자는 페이지 수준 작업에 대한 메서드를 제공하는 인터페이스입니다.

예를 들어 리소스의 포함 페이지를 가져오려면 스크립트에서 다음 코드를 사용할 수 있습니다.

Page myPage = pageManager.getContainingPage(myResource);

pageManager가 페이지 관리자 개체이고 myResource가 리소스 개체인 경우. 페이지 관리자에서 제공하는 방법에 대한 자세한 내용은 [Javadocs](https://helpx.adobe.com/kr/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/PageManager.html).
...
* `/apps`
   ...
   응용 프로그램 관련 웹 사이트와 관련된 구성 요소 정의를 포함합니다. 개발하는 구성 요소는 에서 사용할 수 있는 기본 구성 요소를 기반으로 할 수 있습니다. `/libs/foundation/components`.
...
* `/libs`
   ...
   AEM의 핵심에 속하는 라이브러리 및 정의. 의 하위 폴더 `/libs` 검색 또는 복제와 같이 즉시 사용 가능한 AEM 기능을 나타냅니다. 의 콘텐츠 `/libs` AEM 작동 방식에 영향을 주므로 수정하면 안 됩니다. 웹 사이트에 대한 기능은 `/apps` 자세한 내용은 [구성 요소 및 기타 요소 사용자 지정](/help/sites-developing/dev-guidelines-bestpractices.md#customizing-components-and-other-elements)).
...
AEM을 사용하는 프로덕션 환경은 종종 두 가지 유형의 인스턴스로 구성됩니다. an [작성자 및 게시 인스턴스](/help/sites-deploying/deploy.md#author-and-publish-installs).

## 디스패처 {#the-dispatcher}

Dispatcher는 캐싱 및/또는 로드 밸런싱 모두를 위한 Adobe의 도구입니다. 자세한 내용은 [디스패처](https://helpx.adobe.com/kr/experience-manager/dispatcher/user-guide.html).
...
자세한 내용은 [FileVault 도구](/help/sites-developing/ht-vlttool.md) 자세한 내용은 설명서 를 참조하십시오.

## 워크플로 {#workflows}

컨텐츠는 여러 참여자의 승인 및 로그오프와 같은 단계를 포함하여 조직 프로세스를 따르는 경우가 많습니다. 이러한 프로세스는 워크플로우로 표시될 수 있습니다. [AEM 내에서 정의 및 개발](/help/sites-developing/workflows-models.md)에 적용된 다음 [적절한 컨텐츠 페이지](/help/sites-administering/workflows.md) 또는 [디지털 자산](/help/assets/assets-workflow.md) 필요한 경우.
