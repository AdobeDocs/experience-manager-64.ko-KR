...
AEM 5.6을 통해 Adobe는 작성 환경을 위한 [반응형 디자인으로](/help/sites-authoring/responsive-layout.md) 새로운 터치 지원 UI를 도입했습니다. 터치 및 데스크톱 장치에서 모두 작동하도록 설계된 원래의 클래식 UI와 상당히 다릅니다.

이 터치 지원 UI는 이제 클래식 UI보다 우선하는 AEM의 표준 UI입니다.

>[!NOTE]
>
>터치 지원 UI는 AEM의 표준 UI이지만 클래식 UI는 계속 지원됩니다.
...
이렇게 하면 작성자가 인터랙티브한 웹 앱을 빌드할 의도를 표현할 수 있도록 제공된 HTML 어휘 확장을 사용합니다. WAI-ARIA와 [](https://www.w3.org/TR/wai-aria/) 마이크로 포맷에 [대한 접근 방식입니다](http://microformats.org/).
...
[ [MOCK] The Granite UI foundation components](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/index.html) provides the basic building blocks needed for building any UI. 여기에는 다음이 포함됩니다.
...
CUI(Coral UI)는 여러 제품에서 사용자 경험을 일관되게 제공하기 위해 고안된 터치 지원 UI에 대한 Adobe의 시각적 스타일 구현입니다. Coral UI 파섹
...
예제 마크업 추출(data-amp;ast;로 지정된 옵션 참고)속성):
