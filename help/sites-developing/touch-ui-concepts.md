...
AEM 5.6 Adobe에서는 작성 환경을 위한 [반응형 디자인을](/help/sites-authoring/responsive-layout.md) 갖는 새로운 터치 지원 UI를 도입했습니다. 터치 및 데스크톱 장치에서 모두 작동하도록 설계된 클래식 UI는 원래 클래식 UI와 상당히 다릅니다.

이 터치 지원 UI는 이제 AEM의 표준 UI로, 클래식 UI보다 우선합니다.

>[!NOTE]
>
>터치 지원 UI는 클래식 UI는 여전히 지원되지만 AEM의 표준 UI입니다.
...
>[!NOTE]
>
>거의 모든 AEM 기능을 터치 지원 UI로 이식했습니다. 그러나 일부 제한된 경우 기능이 클래식 UI로 돌아갑니다. 자세한 내용은 [터치 UI 기능 상태를](/help/release-notes/touch-ui-features-status.md) 참조하십시오.
...
이는 작성자가 인터랙티브한 웹 앱을 빌드할 의사를 표현할 수 있도록 제공된 HTML 어휘 확장을 사용합니다. 이것은 WAI-ARIA와 [마이크로 포맷과](https://www.w3.org/TR/wai-aria/) 비슷한 [접근 방식입니다](http://microformats.org/).
...
현재 JS와 CSS 코드는 범주 아래 [Granite [clientlibs](/help/sites-developing/clientlibs.md) ]로 전달됩니다.
...
이는 작성자가 웹 앱을 빠르게 *작성할* 수 있도록 하는 sling 구성 요소 모음을 통해 이루어집니다. 개발자는 구성 요소를 개발하고, 작성자는 구성 요소를 웹앱으로 취합합니다. 서버측 역할은 하이퍼미디어(markup)를 클라이언트에 제공하는 것입니다.
...
[MOCK] [The Granite UI foundation components](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/index.html) provides the basic building blocks for building any UI. 여기에는 다음이 포함됩니다.
...
CUI(Coral UI)는 여러 제품에서 사용자 경험을 일관되게 제공하기 위해 설계된 터치 지원 UI에 대한 Adobe 시각적 스타일 구현입니다. Coral UI는 작성 환경에서 사용되는 시각적 스타일을 적용하는 데 필요한 모든 것을 제공합니다.
...
CSS는 실제 모양과 느낌을 제공하는 데 사용됩니다. 실제 스타일 값을 쉽게 사용자 정의할 수 있도록 모양 및 느낌(예: 브랜딩의 경우)을 런타임 동안 [LESS](https://lesscss.org/) 사전 프로세서에서 확장하는 변수로 선언합니다.
...
* 부트스트랩에서 영감을 얻은 스타일을 사용하는 HTML [태그](https://twitter.github.com/bootstrap/)
...
* 일반. 예를 들어 레이아웃 관리자는 모든 목록 `DIV` 또는 요소 `LI` 에 대한 레이아웃을 제공합니다
...
양식 유효성 검사를 구현하는 데도 동일한 개념을 사용합니다. 유효성을 검사하려는 요소의 경우 필수 입력 양식을 사용자 지정 `data-*` 속성으로 지정해야 합니다. 그런 다음 이 속성은 유효성 검사 플러그인에 대한 옵션으로 사용됩니다.
...
이 `cardLayout` 플러그인은 해당하는 높이를 기준으로 닫힌 `UL` 요소를 배치하며 부모 너비를 고려합니다.
