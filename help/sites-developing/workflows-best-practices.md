...
높은 수집 로드를 최적화하려면 [임시 워크플로우](/help/sites-developing/workflows.md#transient-workflows).
...
DAM 워크플로우에 대한 성능 조정 지침은 [AEM Assets 성능 조정 가이드](/help/assets/performance-tuning-guidelines.md).

### 최대 동시 실행 워크플로우 수 구성 {#configure-the-maximum-number-of-concurrent-workflows}
...
이 문제를 해결하려면 **최대 병렬 작업 수** 시스템에 있는 프로세서 코어 수의 0.5~3/4 사이에 있어야 합니다. 이렇게 하면 시스템이 이러한 워크플로우를 처리할 때 응답성을 유지할 수 있는 충분한 용량을 사용할 수 있습니다.

구성하려면 **최대 병렬 작업 수**, 다음 중 하나를 수행할 수 있습니다.

* 구성 **[OSGi 구성](/help/sites-deploying/configuring-osgi.md)** AEM 웹 콘솔에서 게시할 수 있습니다. 대상 **큐: Granite Workflow 큐** (an) **Apache Sling 작업 큐 구성**).

* 큐에서 다음을 수행할 수 있습니다 **Sling 작업** AEM 웹 콘솔 옵션 대상 **작업 큐 구성: Granite Workflow 큐**, at `http://localhost:4502/system/console/slingevent`.

또한, **Granite Workflow 외부 프로세스 작업 큐**. 외부 바이너리를 실행하는 워크플로우 프로세스에 사용됩니다(예: ). **InDesign Server** 또는 **Image Magick**.

### 개별 작업 큐 구성 {#configure-individual-job-queues}

경우에 따라 개별 작업 기준으로 동시 스레드 또는 기타 큐 옵션을 제어하도록 개별 작업 대기열을 구성하는 것이 유용합니다. 를 통해 웹 콘솔에서 개별 큐를 추가하고 구성할 수 있습니다 **Apache Sling 작업 큐 구성** 공장. 나열할 적절한 항목을 찾으려면 워크플로우의 모델을 실행하고 **Sling 작업** console; 예를 들어 `http://localhost:4502/system/console/slingevent`.
...
기본적으로 **주간 유지 관리 기간** 에 **워크플로우 삭제** 작업을 수행하지만 이 작업을 실행하기 전에 구성해야 합니다. 워크플로우 삭제를 구성하려면 **Adobe Granite 워크플로우 제거 구성** 를 웹 콘솔에 추가해야 합니다.
...
>[!NOTE]
>
>참조 - [AEM 6.4의 저장소 구조 변경](/help/sites-deploying/repository-restructuring.md).
...
* 기존 워크플로우 디자인(디자인 타임 및 런타임 모두)은 다음 경로 아래에 유지됩니다.
   ...
   >[!NOTE]
   >
   >이러한 디자인을 편집하는 경우 *AEM UI 사용*&#x200B;그러면 세부 사항이 새 위치에 복사됩니다.
...
* 이전 워크플로우 런처는 다음 경로에 있습니다.
   ...
   >[!NOTE]
   >
   >이러한 정의를 편집하는 경우 *AEM UI 사용*&#x200B;그러면 세부 사항이 새 위치에 복사됩니다.
...
* 워크플로우 프로세스 내에서, `WorkflowSession` 저장소를 수정하는 데 사용 중이어서 세션을 명시적으로 저장하지 않습니다. 워크플로우가 완료되면 세션을 저장합니다.
* `Session.Save` 워크플로우 단계 내에서 호출하면 안 됩니다.

   * 워크플로우 jcr 세션을 조정하는 것이 좋습니다. 그런 다음 `save` 워크플로우 실행이 완료되면 워크플로우 엔진이 세션을 자동으로 저장하므로 은 필요하지 않습니다.
...
모든 수신자를 담당하는 수신기가 하나 있습니다 [워크플로우 런처](/help/sites-administering/workflows-starting.md#workflows-launchers) 등록된 항목:
...
### 런처를 위한 구성 개선 사항 {#configuration-enhancements-for-launchers}

사용자 지정 [시작 구성](/help/sites-administering/workflows-starting.md#workflows-launchers) 은 다음을 지원하도록 향상되었습니다.
...
예로는 컨텐츠 세트에서 비즈니스 프로세스를 구현한 다음 해당 컨텐츠를 활성화하는 워크플로우가 있습니다. 을(를) 시작하는 대신 이러한 각 노드를 활성화하는 사용자 정의 워크플로우 프로세스를 만드는 것이 좋습니다 **컨텐츠 활성화** 게시해야 하는 각 컨텐츠 노드에 대한 모델입니다. 이 접근 방식을 사용하려면 추가 개발 작업이 필요하지만, 각 활성화에 대해 별도의 워크플로우 인스턴스를 시작하는 것보다 실행 시 더 효율적입니다.
...
다음을 정의할 수 있습니다 [워크플로우 단계](/help/sites-developing/workflows.md#workflow-stages)그런 다음 특정 워크플로우 단계에 작업/단계를 할당합니다.

이 정보는 [**워크플로우 정보** 탭에서 작업 항목의 탭 **받은 편지함**](/help/sites-authoring/workflows-participating.md#opening-a-workflow-item-to-view-details-and-take-actions). 기존 워크플로우 모델을 편집하여 단계를 추가할 수 있습니다.

### 페이지 프로세스 단계 활성화 {#activate-page-process-step}

다음 **페이지 프로세스 활성화** 단계는 페이지를 활성화하지만 참조된 DAM 자산을 자동으로 찾아 활성화하지는 않습니다.
...
워크플로우 모니터링, 유지 관리 및 문제 해결에 도움이 되는 많은 시스템 도구를 사용할 수 있습니다. 아래의 모든 예제 URL은 `localhost:4502`, 하지만 모든 작성자 인스턴스( `<hostname>:<port>`).
