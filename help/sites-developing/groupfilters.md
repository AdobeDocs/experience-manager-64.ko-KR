...
장치 그룹 필터는 com.day.cq.wcm.mobile.api.device. [DeviceGroupFilter 인터페이스를 구현하는 OSGi](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/index.html?com/day/cq/wcm/mobile/api/device/DeviceGroupFilter.html) 구성 요소입니다. 배포 시 구현 클래스는 장치 그룹 구성에 사용할 수 있는 필터 서비스를 제공합니다.
...
필터 이름 `getTitle` 및 `getDescription` 설명은 각각 반환됩니다. 다음 코드는 가장 간단한 구현을 보여 줍니다.
...
장치 `matches` 기능이 모든 필터 기준을 충족하는 경우 함수가 반환됩니다 `true` . 메서드 인수에 제공된 정보를 평가하여 장치가 그룹에 속하는지 확인합니다. 다음 값이 인수로 제공됩니다.
...
패키지 `org.apache.commons.lang.math` 는 `NumberUtils` 클래스를 제공합니다.

>[!NOTE]
>
>AEM에 배포된 WURFL™ 데이터베이스에 필터 기준으로 사용하는 기능이 포함되어 있는지 확인합니다. 장치 [감지](/help/sites-developing/mobile.md#server-side-device-detection)참조
...
### Maben POM 파일 {#the-maven-pom-file}
...
작성 시 5.5.2는 최신 AEM 릴리스에 있는 WCM Mobile API 번들 버전입니다. Adobe 웹 콘솔([http://localhost:4502/system/console/bundles](http://localhost:4502/system/console/bundles))을 사용하여 환경에 배포된 번들 버전인지 확인하십시오.
...
공개 Adobe 리포지토리를 사용할 수 있도록 [콘텐츠 패키지 Maven 플러그인](/help/sites-developing/vlt-mavenplugin.md) 얻기 섹션에서 maven 설정 파일에 제공하는 프로필을 추가합니다.
