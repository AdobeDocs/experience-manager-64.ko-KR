...
구조화된 콘텐츠를 사용하는 콘텐츠 조각을 사용하면 JSON 내보내기 도구를 사용하여 (y) AEM 페이지의 콘텐츠를 JSON 데이터 모델 형식으로 전달하여 콘텐츠 서비스를 제공할 수 있습니다. 그런 다음 응용 프로그램에서 사용할 수 있습니다.

>[!NOTE]
>
>여기에 설명된 기능은 코어 구성 요소](https://docs.adobe.com/content/docs/en/core-components/v1.html)의 [릴리스 1.1.0 이후 모든 핵심 구성 요소에 사용할 수 있습니다.

## 컨텐츠 조각 핵심 구성 요소가 있는 JSON 내보내기 도구 {#json-exporter-with-content-fragment-core-components}

AEM JSON 내보내기 도구를 사용하여 (y) AEM 페이지의 콘텐츠를 JSON 데이터 모델 형식으로 제공할 수 있습니다. 그런 다음 응용 프로그램에서 사용할 수 있습니다.

AEM 내에서 `model` 선택기 및 `.json` 확장을 사용하여 배달을 수행할 수 있습니다.
...
이 작업은 조각에 대한 전체 경로( `jcr:content` 사용);예를 들어,

`.../jcr:content/root/responsivegrid/contentfragment.model.json`

페이지에 단일 컨텐츠 조각 또는 다양한 유형의 여러 구성 요소가 포함될 수 있습니다. 목록 구성 요소와 같은 메커니즘을 사용하여 관련 컨텐츠를 자동으로 검색할 수도 있습니다.
...
* 다음과 같은 컨텐츠를 제공합니다.
   ...
   >[!NOTE]
   >
   >표준 구현은 아니지만 [여러 선택기가 지원되지만 ](json-exporter-components.md#multiple-selectors), `model`는 첫 번째 구현이어야 합니다.
...
* Sling 모델:
   ...
   * [Sling Models - 130 이후의 리소스 유형에 모델 클래스 연결](https://sling.apache.org/documentation/bundles/models.html#associating-a-model-class-with-a-resource-type-since-130)
