...
* TagID [](#tagid) 는 컨텐츠 노드의 [`cq:tags`](#tagged-content-cq-tags-property) 속성에 추가되고 유형 노드로 확인됩니다. [`cq:Tag`](#tags-cq-tag-node-type)
...
* `cq:movedTo` 태그 B를 가리킵니다.
   ...
   이 속성은 태그 A가 태그 B로 이동 또는 병합되었음을 의미합니다. 태그 B를 이동하면 그에 따라 이 속성이 업데이트됩니다. 따라서 태그 A는 숨겨지고 태그 A를 가리키는 컨텐츠 노드의 태그 ID만 확인하기 위해 보관됩니다. 태그 가비지 수집기는 태그 A와 같은 태그를 더 이상 컨텐츠 노드가 가리키지 않게 합니다.
...
>[!NOTE]
>
>태그 페이지의 페이지 속성에서, 태그 기준 경로(예: )를 하드 코딩하는 대신 태그 ID(예: `geometrixx-outdoors:activity/biking`)를 사용하는 것이 `/etc/tags/geometrixx-outdoors/activity/biking`좋습니다.
>
