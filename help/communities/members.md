...
* 전역 탐색에서 `Tools, Security, Users` 선택
* 전역 탐색에서 `Tools, Security, Groups` 선택
...
(
*선택* 사항) 해당 그룹에 구성원을 추가하려면 기존 구성원 그룹 중에서 선택합니다.
...
* **[!UICONTROL 기여도]**
제한이 확인란을 선택하면 회원의 컨텐츠 게시 권한이 제한됩니다.
기본값은 기여도 제한의 구성에 따라 다릅니다.
[멤버 기여도 제한](limits.md)을 참조하십시오.

* **[!UICONTROL 암호]**
변경기존 구성원을 수정할 때 표시되는 링크입니다. 커뮤니티 관리자가 구성원의 암호를 재설정할 수 있는 기능을 제공합니다.
