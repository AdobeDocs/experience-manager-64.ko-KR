...
* 전역 탐색에서 선택 `Tools, Security, Users`
* 전역 탐색에서 선택 `Tools, Security, Groups`
...
사용자가 Adobe Analytics가 `Views`활성화된 `Posts`하나 이상의 커뮤니티 사이트에 속한 경우 표시, `Follows``Likes` 및 [업데이트되는 열입니다](sites-console.md#analytics).
...
* **[!UICONTROL ID]**(*필수*) 권한 부여 가능한 ID는 멤버의 로그인 ID입니다.
기본적으로 ID 파섹
   *만들어진 ID는 수정되지 않을 수 있습니다.*

* **[!UICONTROL 이메일 주소]**(*필수*) 구성원의 이메일 주소입니다.
구성원은 프로필을 업데이트할 때 이메일 주소를 변경할 수 있습니다.ID가 이메일 주소로 기본값으로 설정된 경우 이메일 주소가 변경될 때 ID가 *변경되지 않습니다* .

* **[!UICONTROL 암호]**(*필수*) 로그인 암호입니다.

* **[!UICONTROL 암호]**&#x200B;다시 입력(*필수*) 확인을 위해 암호를 다시 입력합니다.

* **[!UICONTROL 사이트에 구성원 추가]**(*선택*&#x200B;사항) 커뮤니티 사이트의 구성원 그룹에 구성원을 추가하려면 기존 커뮤니티 사이트에서 선택합니다.

* **[!UICONTROL 그룹에 구성원 추가]**(*선택*&#x200B;사항) 해당 그룹에 구성원을 추가하려면 기존 구성원 그룹에서 선택합니다.
...
* **[!UICONTROL 상태]**
   * 금지됨\
      회원이 로그인할 수 없어 페이지를 보거나 로그인해야 하는 활동에 참여하지 못했습니다. 그들은 여전히 익명으로 열린 커뮤니티 사이트를 방문할 수 있습니다.

   * Not 금지된 구성원에게는 커뮤니티 사이트에 대한 모든 액세스 권한이 있습니다.
   기본값은 `Not Banned`입니다.

* **[!UICONTROL 기여도]**제한 이 확인란을 선택하면 멤버의 컨텐츠 게시 권한이 제한됩니다.
기본값은 기여도 제한 구성에 따라 다릅니다.
멤버 [기여도 제한을 참조하십시오](limits.md).

* **[!UICONTROL 암호]**&#x200B;변경 기존 멤버를 수정할 때 표시되는 링크입니다. 커뮤니티 관리자가 구성원의 암호를 재설정할 수 있는 기능을 제공합니다.
...
* **[!UICONTROL ID]**(*필수*) 그룹 고유 ID입니다.
   *만들어진 ID는 수정되지 않을 수 있습니다.*

* **[!UICONTROL 이름]**(*선택*&#x200B;사항) 그룹의 표시 이름입니다.

   기본값은 ID입니다.

* **[!UICONTROL 설명]**(*선택*) 그룹의 목적과 권한에 대한 설명입니다.

* **[!UICONTROL 그룹에 구성원 추가]**(*선택*&#x200B;사항) 그룹의 초기 멤버로 포함할 게시 측 구성원을 선택합니다.
