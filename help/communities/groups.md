...
그룹 콘솔은 커뮤니티 사이트의 [템플릿 구조](sites-console.md#step1)에 [그룹 함수](functions.md#groups-function)이(가) 포함된 경우 커뮤니티 그룹을 만들기 위한 액세스 권한을 제공합니다.

* 그룹은 다른 그룹 내에 중첩될 수 있습니다. 이 문제는 새 그룹](tools-groups.md)의 [구조에 그룹 함수가 들어 있을 때 발생합니다.
* 작성 환경에만 사이트 만들기 마법사와 유사한 그룹 만들기 마법사가 있습니다.
* 커뮤니티 사이트 구조나 커뮤니티 그룹 구조에 그룹 기능을 추가할 때 게시 환경에서 그룹을 만들 수 있는지 여부를 구성할 수 있습니다.

포함된 3개의 그룹 템플릿 중 `Reference Group` 템플릿에만 그룹 함수가 해당 구조에 포함됩니다.

커뮤니티 그룹의 여러 패싯은 다음과 같습니다.

* 작성:작성자에 새 그룹을 만들고 선택적으로 게시 시 만들 수 있습니다.
* 컨트롤:그룹 공개 또는 비밀
...
>[!NOTE]
>
>커뮤니티 사이트 콘솔에서만 액세스할 수 있는 이 그룹 콘솔은 구성원 그룹을 관리하기 위한 구성원 [그룹 콘솔](members.md)과(와) 혼동되지 않습니다.
>
>구성원 그룹은 게시 환경에 등록되어 [터널 서비스](deploy-communities.md#tunnel-service-on-author)를 사용하여 작성 환경에서 액세스하는 사용자 그룹입니다.
...
* 폴더 내의 커뮤니티 사이트 인스턴스 선택
   ...
   * 이 스크린샷은 [게시](published-site.md)에 그룹을 만든 후 시작하기 자습서에서 가져옵니다.

![chlimage_1-133](assets/chlimage_1-133.png)

**[!UICONTROL 그룹 폴더]**&#x200B;을 선택하여 엽니다.
...
* **[!UICONTROL 커뮤니티 그룹 루트]**:그룹의 루트 경로입니다.
   ...
   기본 루트는 상위 사이트이지만 루트는 웹 사이트 내의 임의의 위치로 이동할 수 있습니다. 변경하지 않는 것이 좋습니다.

* **[!UICONTROL 추가 사용 가능한 커뮤니티 그룹 언어]** 메뉴:풀다운 메뉴를 사용하여 사용 가능한 커뮤니티 그룹 언어를 선택합니다. 메뉴에는 상위 커뮤니티 사이트를 만든 모든 언어가 표시됩니다. 사용자는 이러한 언어 중에서 선택하여 이 단일 단계에서 여러 로케일에 그룹을 만들 수 있습니다. 동일한 그룹이 각 커뮤니티 사이트의 그룹 콘솔에서 지정된 여러 언어로 만들어집니다.
...
커뮤니티 사이트가 게시되면 [속성](#modifying-group-properties)을 편집하고 다른 테마를 선택할 수 있습니다.
...
* 선택적 멤버십
   ...
   이 옵션을 선택하면 커뮤니티 그룹이 공개 그룹입니다. 사이트 구성원은 그룹에 명시적으로 참여하지 않고 참여할 수 있으며 게시할 수 있습니다. 기본값이 선택됩니다.
* 필수 멤버십

   선택하는 경우 커뮤니티 그룹은 열린 그룹입니다. 커뮤니티 사이트 구성원은 그룹의 컨텐츠를 볼 수 있지만 컨텐츠를 게시하려면 먼저 그룹에 가입해야 합니다. 멤버는 게시 환경에서 `Join` 단추를 선택하여 가입합니다. 기본값은 선택되지 않았습니다.

* 제한된 멤버십

   이 옵션을 선택하면 커뮤니티 그룹이 비밀 그룹입니다. 커뮤니티 구성원은 명시적으로 초대되어야 합니다. 초대된 구성원이 검색 상자에 입력됩니다. 구성 요소는 작성 환경에서 [구성원 및 그룹 콘솔](members.md)을 사용하여 나중에 추가할 수 있습니다. 기본값은 선택되지 않았습니다.
...
프로세스가 완료되면 새 하위 커뮤니티 사이트(그룹)에 대한 카드가 커뮤니티 사이트 그룹 콘솔에 표시됩니다. 이 콘솔에서 작성자는 페이지 컨텐츠를 추가하거나 사이트의 속성을 수정할 수 있습니다.

![createcommunitygroup-1](assets/createcommunitygroup-1.png)

>[!NOTE]
>
>그룹은 [1단계에서 지정한 모든 언어로 생성됩니다.커뮤니티 그룹 템플릿](groups.md#step1communitygrouptemplate)(추가 사용 가능한 커뮤니티 그룹 언어), 해당 커뮤니티 사이트의 커뮤니티 그룹 콘솔에서.

## 그룹 콘텐츠 작성 {#authoring-group-content}
...
다음 속성의 세부 사항은 [그룹 만들기](#group-creation) 섹션에 제공된 설명과 일치합니다. 게시 환경 또는 작성 환경에서 생성된 중첩 그룹은 모두 수정할 수 있습니다.
...
구조 패널에서는 작성 또는 게시 환경에서 하위 커뮤니티 사이트를 만들 때 선택한 커뮤니티 그룹 템플릿에서 처음 만든 구조를 수정할 수 있습니다. 패널에서
...
* 사이트 구조에서 커뮤니티 함수 인스턴스:
   ...
   * **`gear icon`**
      ...
      표시 제목 및 URL 이름과 [권한이 있는 구성원 그룹](users.md#privilegedmembersgroups)을 비롯한 설정을 편집합니다.
