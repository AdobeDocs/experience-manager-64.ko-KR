...
그룹 콘솔에서는 커뮤니티 사이트의 [템플릿 구조](sites-console.md#step1) include [그룹 함수](functions.md#groups-function).

* 그룹은 다른 그룹 내에서 중첩될 수 있습니다. 이 문제는 [새 그룹의 구조](tools-groups.md) 그룹 함수를 포함합니다.
...
포함된 세 개의 그룹 템플릿 중 `Reference Group` 템플릿에는 해당 구조의 그룹 함수가 포함되어 있습니다.
...
>[!NOTE]
>
>커뮤니티 그룹, 게시 환경에서 만든 후에 [커뮤니티 그룹 콘솔의 존재](https://helpx.adobe.com/in/experience-manager/6-3/communities/using/version-history.html#FeaturePack1FP1)는 커뮤니티 그룹 콘솔에 나열되지 않으므로 콘솔을 사용하여 수정할 수 없습니다.

>[!NOTE]
>
>커뮤니티 사이트 콘솔에서만 액세스할 수 있는 이 그룹 콘솔은 구성원과 혼동하지 않도록 합니다 [그룹 콘솔](members.md) 구성원 그룹을 관리하기 위한 것입니다.
>
>구성원 그룹은 게시 환경에 등록되고 작성 환경에서 [터널 서비스](deploy-communities.md#tunnel-service-on-author).
...
* 폴더 내에서 커뮤니티 사이트의 인스턴스를 선택합니다
   ...
   * 이러한 스크린샷은 다음 시작하기 자습서에서 확인할 수 있습니다 [게시할 때 그룹 만들기](published-site.md)

![chlimage_1-133](assets/chlimage_1-133.png)

을(를) 선택합니다 **[!UICONTROL 그룹 폴더]** 열려고
...
* **[!UICONTROL 커뮤니티 그룹 템플릿]** 메뉴: 풀다운 메뉴를 사용하여 사용 가능한 메뉴를 선택합니다. [커뮤니티 그룹 템플릿](tools.md).
...
프레임워크는 `Twitter Bootstrap` 반응형 및 유연한 디자인을 사이트로 가져올 수 있습니다. 미리 로드된 여러 Bootstrap 테마 중 하나를 선택하여 선택한 커뮤니티 그룹 서식 파일의 스타일을 지정하거나 Bootstrap 테마를 업로드할 수 있습니다.

이 옵션을 선택하면 테마가 불투명한 파란색 확인 표시로 오버레이됩니다.

상위 사이트의 테마와 다른 테마를 선택할 수 있습니다.

커뮤니티 사이트가 게시되면 다음을 수행할 수 있습니다 [속성 편집](#modifying-group-properties) 다른 테마를 선택합니다.
...
* 필수 멤버십
   ...
   선택된 경우 커뮤니티 그룹은 열린 그룹입니다. 커뮤니티 사이트 구성원은 그룹의 콘텐츠를 볼 수 있지만 컨텐츠를 게시하려면 먼저 그룹에 가입해야 합니다. 구성원은 `Join` 게시 환경의 단추. 기본값이 선택되어 있지 않습니다.

* 제한된 멤버십

   선택된 경우 커뮤니티 그룹은 비밀 그룹입니다. 커뮤니티 구성원은 명시적으로 초대되어야 합니다. 초대된 멤버가 검색 상자에 입력됩니다. 구성원은 나중에 [구성원 및 그룹 콘솔](members.md) 작성 환경. 기본값이 선택되어 있지 않습니다.
...
한 번 **만들기** 을(를) 선택하고 시작하면 그룹을 만드는 프로세스를 중단할 수 없습니다.
...
>[!NOTE]
>
>그룹은 지정된 대로 모든 언어로 생성됩니다 [1단계: 커뮤니티 그룹 템플릿](groups.md#step1communitygrouptemplate) 추가 사용 가능한 커뮤니티 그룹 언어 의 각 커뮤니티 사이트의 커뮤니티 그룹 콘솔에서 있을 수 있습니다.
...
다음 속성에 대한 세부 사항은 [그룹 만들기](#group-creation) 섹션을 참조하십시오. 중첩 그룹은 게시 환경에서 만들든 작성 환경에서 만들든 모두 수정할 수 있습니다.
...
대신, [구조](#modify-structure) 하위 커뮤니티의 을 수정할 수 있습니다.
...
* 사이트 구조의 커뮤니티 함수 인스턴스에서 다음을 수행합니다.
   ...
   * **`gear icon`**
      ...
      표시 제목 및 URL 이름과 같은 설정을 편집합니다 [권한 있는 구성원 그룹](users.md#privilegedmembersgroups)
...
설정 패널을 사용하면 커뮤니티를 추가할 수 있습니다 [중재자](#moderation).

### 멤버 자격 수정 {#modify-membership}

다음 [멤버십](#membership) 패널은 정보 제공용입니다. 설정된 그룹 구성원 유형을 선택 사항이든 필수 구성이든 제한할 수 없습니다.

### 축소판 수정 {#modify-thumbnail}

다음 [축소판](#thumbnail) 패널에서는 작성 환경의 커뮤니티 사이트 그룹 콘솔뿐만 아니라 게시 환경의 사이트 방문자에 대한 커뮤니티 그룹을 나타내기 위해 이미지를 업로드할 수 있습니다.

## 그룹 게시 {#publishing-the-group}

![chlimage_1-145](assets/chlimage_1-145.png)

커뮤니티 그룹을 새로 만들거나 수정한 후, `Publish Site` 아이콘.
