...
* 작성 환경에서:
   ...
   * [멤버 관리 콘솔](members.md)에 기여도 데이터 표시(보기, 게시물, 다음, 좋아요)
   * 지원 리소스 [보고서](reports.md)에 대한 트렌드 요약, 비디오 하트비트 및 비디오 장치
...
* 사용자 이름
   ...
   Analytics 계정을 관리할 수 있는 사용자의 로그인 사용자 이름
...
[회사 이름 및 사용자 이름](analytics.md#prerequisites)으로 [Adobe Marketing Cloud](https://docs.adobe.com/content/help/en/analytics/analyze/analysis-workspace/home.html)에 로그인하면 새 보고서 세트 또는 기존 보고서 세트가 다음과 같이 구성되도록 할 수 있습니다.
...
**기존 보고서 세트**&#x200B;를 사용하는 경우 커뮤니티 사이트에 대한 Analytics 프레임워크를 활성화하기 전에 [변수 매핑](#modifying-analytics-variable-mapping)을 수정해야 할 수 있습니다. 커뮤니티 전용 변수에 대한 우려 사항은 계정 담당자에게 문의하십시오.
...
`Marketing Cloud Org Id`은(는) [커뮤니티 사이트 생성](sites-console.md#enablement) 이상에서 커뮤니티 사이트 속성을 [수정](sites-console.md#modifying-site-properties)하여 입력할 수 있습니다.[](#aem-analytics-cloud-service-configuration)

![chlimage_1-264](assets/chlimage_1-264.png)

비디오 하트비트 분석이 활성화되면 비디오 플레이어에 대한 JS(Javascript) 코드는 비디오 하트비트 라이브러리 코드(또한 JS)를 인스턴스화하여 10초마다(구성 불가능) 비디오 상태 업데이트를 Analytics 비디오 추적 서버로 전송하는 모든 논리를 처리하고 결국 비디오 세션의 누적 보고서를 기본 Analytics 서버로 전송합니다.
...
* **[!UICONTROL Adobe Analytics]**&#x200B;로 스크롤
* **[!UICONTROL 지금 구성]** 또는 **[!UICONTROL 구성 표시]** 중 하나를 선택합니다.
...
* **[!UICONTROL 데이터 센터]**
   ...
   보고서 세트를 호스팅하는 분석 데이터 센터를 선택합니다.
...
1. **[!UICONTROL 항목 추가 +]** 드롭다운 상자 2개가 나타납니다.
1. 회사 계정과 연결된 보고서 세트를 선택할 수 있도록 `Report suite`을 선택합니다.
1. 대화 상자가 열리면 **[!UICONTROL 예]**&#x200B;를 선택합니다.```Load default server settings? Do you want to load the default server settings and overwrite current values in the Server section?```
1. `Run Mode` 선택\
   **[!UICONTROL 게시]** 선택
...
[새 커뮤니티 사이트](sites-console.md)를 만드는 동안 Analytics 클라우드 서비스를 추가하려면:


* 3단계
* [분석 탭](sites-console.md#analytics)에서:

   * **[!UICONTROL Analytics 사용]** 확인란을 선택합니다.
   * 드롭다운 상자에서 프레임워크 선택
...
커뮤니티 구성 요소를 포함하지만 [사이트 만들기 마법사](sites-console.md)로 만들지 않은 **일반 AEM 사이트**&#x200B;의 경우 요소를 추가해야 합니다. href 값은 사이트의 경로여야 합니다. 예를 들어 사이트 경로가 `/content/my/company/en`이면 다음을 사용합니다.
...
커뮤니티 사이트가 Analytics가 활성화되고 클라우드 구성 프레임워크가 선택된 상태에서 저장되면 AEM 변수는 각각 evar1 및 event1로 시작하여 Analytics evar 및 이벤트에 자동으로 매핑되고 1로 증가됩니다.
...
다음은 [시작하기 자습서](getting-started-enablement.md)를 수행한 후 기본 매핑의 예입니다.
...
| event1리소스 재생 | (관리) | - | - | - | - | - | - | - | (i) | - |
| event2SCFView | (관리) | 나. | (c) | (d) | (e) | (f) | (g) | (h) | (i) | (j) |
| event3SCFCreate(게시물) | - | 나. | (c) | (d) | (e) | (f) | (g) | (h) | (i) | (j) |
| event4SCFFollow | - | 나. | (c) | (d) | (e) | (f) | (g) | (h) | (i) | (j) |
| event5SCFVoteUp | - | 나. | (c) | (d) | (e) | (f) | (g) | (h) | (i) | (j) |
| event6SCFVoteDown | - | 나. | (c) | (d) | (e) | (f) | (g) | (h) | (i) | (j) |
| event7SCFRate | - | 나. | (c) | (d) | (e) | (f) | (g) | (h) | (i) | (j) |
...
* [UGC 구성 요소 리소스 유형](essentials.md):social/forum/components/hbs/topic
* UGC 구성 요소 제목:하이킹 주제
* 로그인(인증 가능한 ID):aaron.mcdonald@mailinator.com
* UGC 경로:/content/usergenerated/asi/.../forum/jmtz-topic3 또는 *다음에 올 구성 요소 경로*:/content/sites/communities/en/jcr:content/content/primary/forum
...
커뮤니티 evar 및 이벤트가 보고서 세트에 미리 있던 매핑을 덮어쓴 경우 데이터 손실을 방지하려면 커뮤니티 기능에 대한 AEM 변수를 다른 Analytics evar 및/또는 이벤트에 할당하고 원래 매핑을 복원합니다.
...
선택한 배포가 [게시 팜](topologies.md#tarmk-publish-farm)인 경우 보고서 데이터를 [SRP](working-with-srp.md)에 기록할 수 있도록 Adobe Analytics을 폴링하는 기본 게시자로 AEM 게시 인스턴스를 식별해야 합니다.
...
Adobe Analytics 자격 증명이 암호화됩니다. 작성자와 발행자 간에 암호화된 분석 자격 증명을 복제하거나 전송하기 쉽도록 하기 위해 모든 AEM 인스턴스는 동일한 기본 암호화 키를 공유해야 합니다.

이렇게 하려면 [암호화 키 복제](deploy-communities.md#replicate-the-crypto-key)의 지침을 따릅니다.

### 커뮤니티 사이트 및 Analytics Cloud 서비스 게시 {#publish-community-site-and-analytics-cloud-service}

커뮤니티 사이트에 대해 Analytics 클라우드 서비스가 활성화되고, 필요한 경우 [Analytics의 AEM 변수 매핑이 조정되면 ](#mapped-analytics-to-aem-variables)(re)커뮤니티 사이트](sites-console.md#publishing-the-site)을(를) 게시하여 게시 환경에 구성을 복제해야 합니다.[
...
폴링 간격은 데이터를 가져오고 [SRP](working-with-srp.md)에 저장할 Adobe Analytics에 대한 요청 빈도를 제어합니다.
...
* 기본 게시자에서
* 관리자 권한으로 로그인
* [CRXDE Lite](../../help/sites-developing/developing-with-crxde-lite.md)으로 이동합니다.

   * 예: [http://localhost:4503/crx/de](http://localhost:4503/crx/de)

* 언어 루트의 `jcr:content` 노드 아래
