...
* 게시 환경에서:
   ...
   * 사이트 방문자가 &quot;가장 많이 본&quot;, &quot;가장 활성&quot;, &quot;가장 좋아요&quot; 순으로 정렬할 수 있도록 허용
   * UGC 목록의 카운트 보기

* 작성 환경에서:

   * 기여도 데이터를 [멤버 관리 콘솔에서](members.md) 표시(보기, 게시물, 팔로잉, 좋아요)
   * 활성 리소스 [보고서를 위한 트렌드 요약, 비디오 하트비트 및 비디오 디바이스](reports.md)

지원되는 커뮤니티 기능은 다음과 같습니다.

* [역량 강화 리소스](resources.md)
* [포럼](forum.md)
...
* 사용자 이름
   ...
   Analytics 계정을 관리하도록 권한이 부여된 사용자의 로그인 사용자 이름
...
회사 이름 및 [사용자 이름으로](https://marketing.adobe.com/resources/help/en_US/analytics/getting-started/analytics-navigation.html) Adobe Marketing Cloud에 [](analytics.md#prerequisites)로그인하면 새 보고서 세트 또는 기존 보고서 세트를 구성할 수 있습니다.
...
커뮤니티 사이트 속성을 `Marketing Cloud Org Id`[수정하여](sites-console.md#enablement) [커뮤니티 사이트 생성](sites-console.md#modifying-site-properties) 시 또는 나중에 입력할수 있습니다. [](#aem-analytics-cloud-service-configuration)

![chlimage_1-264](assets/chlimage_1-264.png)

비디오 하트비트 분석이 활성화되면 비디오 플레이어에 대한 JS(Javascript) 코드는 비디오 하트비트 라이브러리 코드(JS라고도 함)를 인스턴스화하여 10초마다(구성할 수 없음) 비디오 추적 서버로 비디오 상태 업데이트를 전송하는 모든 논리를 처리하고 결국 비디오 세션의 누적 보고서를 기본 Analytics 서버로 보냅니다.
...
* 지금 구성 **[!UICONTROL 또는 구성]** 표시를 **[!UICONTROL 선택합니다.]**
...
보고서 세트가 아직 만들어지지 않았거나 제대로 설정되지 않은 경우 이전 섹션을 참조하십시오.\
[비디오 보고를 위한 Adobe Analytics 보고서 세트](#adobe-analytics-report-suite-for-video-reporting)
...
1. 항목 **[!UICONTROL 추가 선택 +]** 드롭다운 상자가 나타납니다
1. 회사 `Report suite` 계정과 연결된 보고서 세트를 선택하여 선택할 수 있습니다.
1. 대화 **[!UICONTROL 상자에서]** 예를 선택합니다. ```Load default server settings? Do you want to load the default server settings and overwrite current values in the Server section?```
1. 선택 `Run Mode`\
   게시 **[!UICONTROL 선택]**
...
* 3단계
* ANALYTICS [탭에서](sites-console.md#analytics)다음을 수행합니다.
...
Analytics 클라우드 서비스를 [기존 커뮤니티 사이트에](sites-console.md#modifying-site-properties)추가하려면:
...
커뮤니티 구성 요소를 포함하지만 **사이트 만들기 마법사로** 만들지 않은 일반 AEM 사이트의 [](sites-console.md)경우 요소를 추가해야 합니다. href 값은 사이트의 경로여야 합니다. 예를 들어 사이트 경로가 인 경우 `/content/my/company/en`다음을 사용합니다.
...
Analytics가 활성화되고 클라우드 구성 프레임워크가 선택된 상태에서 커뮤니티 사이트가 저장되면 AEM 변수는 각각 evar1 및 event1로 시작하여 Analytics evar 및 이벤트에 자동으로 매핑되고 1씩 증가합니다.
...
|  | 지원 리소스 유형 | 사이트 제목 | 함수 유형 | 그룹 제목 | 그룹 경로 | UGC 유형 | UGC 제목 | 사용자(구성원) | UGC 경로 | 사이트 경로 |
|------------------------|------------------------|-----------|--------------|------------|-----------|---------|----------|--------------|---------|----------|
...
* [커뮤니티 사이트 제목](sites-console.md#step13asitetemplate):Geometrixx Communities
...
* UGC에 대한 SRP 경로:/content/usergenerated/asi/.../forum/jmtz-topic3 또는 *팔로우할*&#x200B;구성 요소의 경로:/content/sites/communities/en/jcr:content/content/primary/forum
* 커뮤니티 사이트 컨텐츠 경로:/content/sites/community/en

### Analytics 변수 매핑 수정 {#modifying-analytics-variable-mapping}

Analytics evar 및 이벤트를 AEM 변수에 대한 매핑은 커뮤니티 사이트에 대해 Analytics가 활성화된 후에 프레임워크 구성에서 볼 수 있습니다.
...
Communities evar 및 events가 보고서 세트에 이전에 존재했던 매핑을 덮어쓰는 경우 데이터 손실을 방지하려면 Communities 기능에 대한 AEM 변수를 다른 Analytics evar 및/또는 이벤트에 할당하고 원래 매핑을 복원합니다.
...
선택한 배포가 [게시 팜인](topologies.md#tarmk-publish-farm)경우 SRP에 쓸 보고서 데이터를 위해 Adobe Analytics를 폴링하는 기본 게시자로 AEM 게시 인스턴스 하나가 식별되어야 [합니다](working-with-srp.md).
...
특정 지침은 커뮤니티 배포의 기본 게시자 섹션을 [참조하십시오](deploy-communities.md#primary-publisher).
...
Adobe Analytics 자격 증명은 암호화됩니다. 작성자와 게시자 간에 암호화된 분석 자격 증명을 복제하거나 전송하려면 모든 AEM 인스턴스가 동일한 마스터 암호화 키를 공유해야 합니다.
...
Analytics 클라우드 서비스가 커뮤니티 사이트에 대해 활성화되고 필요한 경우 AEM 변수에 대한 Analytics [매핑이 조정되면](#mapped-analytics-to-aem-variables), 커뮤니티 사이트를 [](sites-console.md#publishing-the-site)게시하여 게시 환경에 구성을 복제해야 합니다.
...
폴링 간격은 Adobe Analytics에 대한 SRP로 가져오고 저장할 데이터의 요청 빈도를 [제어합니다](working-with-srp.md).
...
* 기본 게시자에서
* 관리자 권한으로 로그인
* CRXDE [Lite로 이동](../../help/sites-developing/developing-with-crxde-lite.md)
...
* Analytics 보고를 위해 구성된 구성 요소로 이동합니다.
