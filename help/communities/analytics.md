...
비디오 하트비트 Analytics이 활성화되면 비디오 플레이어에 대한 JS(Javascript) 코드는 비디오 하트비트 라이브러리 코드(JS)를 인스턴스화하여 10초마다(구성할 수 없음) Analytics 비디오 추적 서버로 비디오 상태 업데이트를 전송하는 모든 논리를 처리하고 결국 기본 Analytics 서버로 비디오 세션의 누적 보고서를 보냅니다.
...
새 커뮤니티 사이트를 만드는 동안 Analytics 클라우드 서비스 [를 추가하려면](sites-console.md):


* 3단계
* ANALYTICS 탭 [에서 다음을 수행합니다](sites-console.md#analytics).

   * Analytics **[!UICONTROL 활성화]** 확인란 선택
   * 드롭다운 상자에서 프레임워크 선택
...
커뮤니티 구성 요소를 포함하지만 **사이트 만들기 마법사로** 만들지 않은 일반 AEM 사이트의 [](sites-console.md)경우 요소를 추가해야 합니다. href의 값은 사이트의 경로여야 합니다. 예를 들어 사이트 경로가 인 경우 다음 `/content/my/company/en`을 사용합니다.
...
커뮤니티 사이트가 Analytics이 활성화되고 클라우드 구성 프레임워크가 선택된 상태에서 저장되면 AEM 변수가 각각 evar1 및 event1로 시작하는 Analytics evar 및 이벤트에 자동 매핑되고 1로 증가됩니다.
...
Analytics evar 및 이벤트를 AEM 변수에 매핑하는 작업은 커뮤니티 사이트에 대해 Analytics이 활성화된 후에 프레임워크 구성에서 볼 수 있습니다.
...
커뮤니티 evar 및 이벤트가 보고서 세트에 이전에 존재했던 매핑을 덮어쓰는 경우 데이터 손실을 방지하려면, 커뮤니티 기능에 대한 AEM 변수를 다른 Analytics evar 및/또는 이벤트에 할당하고 원래 매핑을 복원합니다.
...
선택한 배포가 [게시 팜인](topologies.md#tarmk-publish-farm)경우 보고서 데이터를 SRP에 쓰기 위해 Adobe Analytics을 폴링하는 기본 게시자로 한 AEM 게시 인스턴스가 식별되어야 [합니다](working-with-srp.md).
...
Adobe Analytics 자격 증명이 암호화됩니다. 작성자와 게시자 간에 암호화된 분석 자격 증명을 복제하거나 전송하려면 모든 AEM 인스턴스가 동일한 기본 암호화 키를 공유해야 합니다.
...
Analytics 클라우드 서비스가 커뮤니티 사이트에 대해 활성화되고 필요한 경우 Analytics의 AEM 변수 [매핑이 조정되면](#mapped-analytics-to-aem-variables), 커뮤니티 사이트를 게시하여 게시 환경에 구성을 복제해야 [(re)합니다](sites-console.md#publishing-the-site).
...
폴링 간격은 데이터를 가져와서 SRP에 저장할 Adobe Analytics에 대한 요청 빈도를 [제어합니다](working-with-srp.md).
