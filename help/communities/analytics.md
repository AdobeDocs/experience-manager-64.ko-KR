...
Adobe Analytics과 Adobe Experience Manager(AEM)은 모두 Adobe Marketing Cloud의 해결책이다.

Adobe Analytics은 회원이 지원되는 커뮤니티 기능과 상호 작용하므로 보고서가 생성되는 Adobe Analytics으로 이벤트가 전송됩니다.
...
* 게시 환경에서:
   ...
   * 사이트 방문자가 &quot;가장 많이 본&quot;, &quot;가장 활성화된&quot;, &quot;가장 좋아요&quot; 순으로 정렬할 수 있도록 허용
   * UGC 목록의 카운트 보기

* 작성 환경에서:

   * 구성원 관리 콘솔에서 [기여도 데이터](members.md) 표시(보기, 게시물, 팔로잉, 좋아요)
   * 활성 리소스 [보고서를 위한 트렌드 요약, 비디오 하트비트 및 비디오 디바이스](reports.md)
...
회사 이름과 사용자 이름으로 [Adobe Marketing Cloud](https://docs.adobe.com/content/help/en/analytics/analyze/analysis-workspace/home.html) 에 [로그인하면](analytics.md#prerequisites)다음과 같은 새 또는 기존 보고서 세트를 구성할 수 있습니다.
...
커뮤니티 사이트 속성을 `Marketing Cloud Org Id` 수정하여 [커뮤니티 사이트](sites-console.md#enablement) 생성 [시 또는 이후에](sites-console.md#modifying-site-properties) 입력할수 있습니다. [](#aem-analytics-cloud-service-configuration)

![chlimage_1-264](assets/chlimage_1-264.png)

비디오 하트비트 분석이 활성화되면 비디오 플레이어에 대한 JS(Javascript) 코드는 비디오 하트비트 라이브러리 코드(JS)를 인스턴스화하여 10초마다(구성할 수 없음) 비디오 추적 서버로 비디오 상태 업데이트를 전송하는 모든 논리를 처리하고 결국 비디오 세션의 누적 보고서를 기본 Analytics 서버로 보냅니다.
...
* 지금 **[!UICONTROL 구성]** 또는 구성 **[!UICONTROL 표시 중 하나를 선택합니다.]**
...
* 사용 가능한 프레임워크 `[+]` 옆에 **[!UICONTROL 있는]** 아이콘을 선택하여 새 프레임워크를 만듭니다.
...
* **[!UICONTROL 제목]**
   ...
   예를 들어 *Enablement Community Framework를 입력합니다*
...
1. 항목 **[!UICONTROL 추가 +]** 두 개의 드롭다운 상자가 나타납니다
1. 회사 계정 `Report suite` 과 관련된 보고서 세트를 선택할 수 있습니다.
1. 대화 **[!UICONTROL 상자에서 예]** 를 선택합니다. ```Load default server settings? Do you want to load the default server settings and overwrite current values in the Server section?```
1. 선택 `Run Mode`\
   게시 **[!UICONTROL 선택]**
...
* 3단계
* ANALYTICS [탭](sites-console.md#analytics)아래:

   * Analytics **[!UICONTROL 활성화 확인란을]** 선택합니다
   * 드롭다운 상자에서 프레임워크 선택
...
Analytics 클라우드 서비스를 [기존 커뮤니티 사이트에 추가하려면](sites-console.md#modifying-site-properties):


* 커뮤니티 > **[!UICONTROL 사이트 콘솔로]** 이동합니다.
* 커뮤니티 사이트의 사이트 편집 아이콘 선택
* 설정 선택
* 분석 섹션에서 다음을 수행합니다.
...
커뮤니티 구성 요소를 포함하지만 **사이트** 만들기 마법사로 만들어지지 않은 [일반 AEM 사이트의](sites-console.md)경우 요소를 추가해야 합니다. href의 값은 사이트의 경로여야 합니다. 예를 들어 사이트 경로가 인 경우 다음 `/content/my/company/en`을 사용합니다.
...
참고: 이 `journal` 구성 요소는 블로그 기능을 구현하는 데 사용됩니다.

### AEM 변수에 Analytics 매핑 {#mapped-analytics-to-aem-variables}

Analytics가 활성화되고 클라우드 구성 프레임워크가 선택된 상태에서 커뮤니티 사이트가 저장되면 AEM 변수는 각각 evar1 및 event1로 시작하여 Analytics evar 및 이벤트에 자동 매핑되고 1로 증가합니다.
...
다음은 [시작하기 자습서](getting-started-enablement.md)다음에 나오는 기본 매핑의 예입니다.

![chlimage_1-274](assets/chlimage_1-274.png)

#### 각 이벤트와 함께 전송된 eVar 맵 {#map-of-evars-sent-with-each-event}

|  | 활성 리소스 유형 | 사이트 제목 | 함수 유형 | 그룹 제목 | 그룹 경로 | UGC 유형 | UGC 제목 | 사용자(구성원) | UGC 경로 | 사이트 경로 |
|------------------------|------------------------|-----------|--------------|------------|-----------|---------|----------|--------------|---------|----------|
|  | **eVar1** | **eVar2** | **eVar3** | **eVar4** | **eVar5** | **eVar6** | **eVar7** | **eVar8** | **eVar9** | **eVar10** |
| event1리소스 재생 | (관리) | - | - | - | - | - | - | - | 자. | - |
| event2SCFView | (관리) | 나. | 다. | (d) | (e) | (f) | (g) | (h) | 자. | 차. |
| event3SCFCreate(Post) | - | 나. | 다. | (d) | (e) | (f) | (g) | (h) | 자. | 차. |
| event4SCFFollow | - | 나. | 다. | (d) | (e) | (f) | (g) | (h) | 자. | 차. |
| event5SCFVoteUp | - | 나. | 다. | (d) | (e) | (f) | (g) | (h) | 자. | 차. |
| event6SCFVoteDown | - | 나. | 다. | (d) | (e) | (f) | (g) | (h) | 자. | 차. |
| event7SCFRate | - | 나. | 다. | (d) | (e) | (f) | (g) | (h) | 자. | 차. |
...
* 커뮤니티 그룹 콘텐츠 경로: /content/sites/communities/en/groups/hiking
* [UGC 구성 요소 리소스유형](essentials.md): social/forum/components/hbs/topic
* UGC 구성 요소 제목: 하이킹 항목
* 로그인(인증 가능한 ID): aaron.mcdonald@mailinator.com
* UGC 경로: /content/usergenerated/asi/.../forum/jmtz-topic3 또는 *팔로우할 구성 요소의 경로*: /content/sites/communities/en/jcr:content/content/primary/forum
* 커뮤니티 사이트 컨텐츠 경로: /content/sites/community/en
...
커뮤니티 evar 및 이벤트가 보고서 세트에 이전에 존재했던 매핑을 덮어쓰는 경우 데이터 손실을 방지하려면 커뮤니티 기능에 대한 AEM 변수를 다른 Analytics evar 및/또는 이벤트에 지정하고 원래 매핑을 복원합니다.
...
선택한 배포가 [게시 팜인](topologies.md#tarmk-publish-farm)경우 보고서 데이터를 SRP에 쓰기 위해 Adobe Analytics을 폴링하는 주 게시자로 AEM 게시 인스턴스 하나를 식별해야 [합니다](working-with-srp.md).
...
따라서 모든 보조 게시 인스턴스의 구성을 편집하여 기본 게시자 **확인란의 선택을 취소해야** 합니다.

특정 지침은 커뮤니티 배포의 기본 게시자 섹션 [을 참조하십시오](deploy-communities.md#primary-publisher).
...
Adobe Analytics 자격 증명이 암호화됩니다. 작성자와 게시자 간에 암호화된 분석 자격 증명을 복제하거나 전송하려면 모든 AEM 인스턴스는 동일한 기본 암호화 키를 공유해야 합니다.

이렇게 하려면 암호화 키 [복제의 지침을 따르십시오](deploy-communities.md#replicate-the-crypto-key).

### 커뮤니티 사이트 및 Analytics Cloud 서비스 게시 {#publish-community-site-and-analytics-cloud-service}

커뮤니티 사이트에 대해 Analytics 클라우드 서비스가 활성화되고, 필요한 경우 AEM 변수에 대한 [매핑이 조정되면, 커뮤니티 사이트를 게시하여 게시 환경에 구성을 복제해야](#mapped-analytics-to-aem-variables)(re) [합니다](sites-console.md#publishing-the-site).
...
폴링 간격은 데이터를 가져오고 SRP에 저장할 Adobe Analytics에 대한 요청 빈도를 [제어합니다](working-with-srp.md).
...
* 기본 게시자에서
* 관리자 권한으로 로그인
* CRXDE Lite으로 [이동](../../help/sites-developing/developing-with-crxde-lite.md)

   * 예: [http://localhost:4503/crx/de](http://localhost:4503/crx/de)

* 언어 루트 `jcr:content` 노드 아래

   * 예, `/content/sites/engage/en/jcr:content`

* Analytics 보고용으로 구성된 구성 요소로 이동합니다.
...
* Notice the `total`node
   ...
   * 속성을 `interval` 수정하면 보고서 가져오기 간격이 재정의됩니다
...
* Adobe Marketing Cloud: [분석 도움말 및 참조](https://docs.adobe.com/content/help/en/analytics/landing/home.html)
