...
AEM Communities에서는 관리자 및 커뮤니티 중재자(중재자로 지정된 신뢰할 수 있는 커뮤니티 구성원)가 작성 환경과 게시 환경 모두에서 커뮤니티 컨텐츠를 [일괄](moderate-ugc.md) 중재할 수 있습니다.

관리자 및 커뮤니티 중재자는 게시 환경에서 [상황에 맞는 중재를](in-context.md) 수행할 수도 있습니다.

모든 [커뮤니티 사이트의](sites-console.md) 기능은 관리자 권한으로 로그인한 사용자가 사용할 수 있는 `Administration`메뉴 항목입니다. 이 `Administration`링크는 중재 콘솔에 대한 액세스를 제공합니다.
...
관리자로 로그인하거나 함께 구성원으로 로그인된 경우에만 중재 작업을 수행할 ` [moderator permissions](in-context.md#identifyingtrustedmembers)`수 있습니다.
...
* 전역 탐색에서:탐색 **[!UICONTROL > 커뮤니티 > 중재]**

관리자 또는 구성원으로 로그인된 경우에만 중재 작업을 수행할 수 ` [moderator permissions](in-context.md#identifyingtrustedmembers)`있습니다. 표시된 유일한 커뮤니티 콘텐츠는 로그인한 구성원이 중재할 수 있는 것입니다.

>[!NOTE]
>
>게시 환경의 UGC는 선택한 SRP가 공용 저장소를 구현하는 경우에만 작성자에게 표시됩니다. 예를 들어, 기본적으로 스토리지는 JSRP로 작성자와 게시를 위한 일반적인 스토어가 아닙니다. 커뮤니티 [컨텐츠 저장소를 참조하십시오](working-with-srp.md).
...
컨텐츠를 필터링할 속성을 선택할 수 있는 검색을 선택할 때 레일을 엽니다.
...
* 게시되었을 때
* 게시물에 대한 답글 수
* [게시물과](moderate-ugc.md#sentiment) 연결된 센티멘트
* 승인되면 확인 표시가 표시됩니다
* 첨부 파일이 있으면 종이로 된 클립이 표시됩니다
...
>[!NOTE]
>
>관리자가 벌크 중재 콘솔에 액세스하면 Geometrixx 샘플과 같이 [사이트 만들기 마법사로](sites-console.md)만들지 않은 사이트를 비롯하여 UGC에 대한 모든 참조가 표시됩니다.
>
...
컨텐츠 일부가 플래그가 지정되면 플래그 **[!UICONTROL 단추를 다시 한 번 선택하여 해당 컨텐츠의 플래그를 해제할 때까지 플래그가]** 지정됩니다. 중요 또는 팔로우와 같은 플래그 지정은 없습니다.
...
[센티멘트는](moderate-ugc.md#sentiment) 긍정, 부정 또는 중립 센티멘트 값이 있는 게시물에 표시되는 참조된 UGC를 제한합니다.

![chlimage_1-479](assets/chlimage_1-479.png)

## 중재 작업 {#moderation-actions}

[중재 작업은](moderate-ugc.md#moderation-actions) 컨텐츠 영역에서 선택한 하나 이상의 사항이나 컨텐츠 세부 사항을 볼 때 수행할 수 있습니다.
...
* 게시물에 대한 링크를 선택하여 상황에 맞는 게시물 보기
* [답글](#reply)
* [허용](#allow)
* [거부](#deny)
* [삭제](#delete)
