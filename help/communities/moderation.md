...
AEM Communities에서 관리자 및 커뮤니티 중재자(중재자로 지정된 신뢰할 수 있는 커뮤니티 구성원)가 만든 환경과 게시 환경 모두에서 커뮤니티 콘텐츠 [](moderate-ugc.md) 를 일괄 중재할 수 있습니다.

관리자 및 커뮤니티 중재자는 게시 환경 [에서 상황에 맞는 중재를](in-context.md) 수행할 수도 있습니다.

모든 [커뮤니티 사이트의](sites-console.md) 기능은 관리자 권한으로 로그인한 사용자가 사용할 수 있는 `Administration`메뉴 항목입니다. 이 `Administration`링크를 통해 중재 콘솔에 액세스할 수 있습니다.
...
관리자 또는 함께 로그인한 경우에만 중재 작업을 수행할 수 ` [moderator permissions](in-context.md#identifyingtrustedmembers)`있습니다.
...
관리자로 로그인하거나 구성원으로 로그인된 경우에만 중재 작업을 수행할 수 ` [moderator permissions](in-context.md#identifyingtrustedmembers)`있습니다. 표시된 유일한 커뮤니티 컨텐츠는 로그인한 구성원이 중재할 수 있다는 것입니다.

>[!NOTE]
>
>선택한 SRP가 공용 스토어를 구현하는 경우에만 게시 환경의 UGC가 작성자에게만 표시됩니다. 예를 들어 기본적으로 JSRP는 작성자 및 게시를 위한 일반적인 스토어가 아닙니다. 커뮤니티 [컨텐츠 저장소를 참조하십시오](working-with-srp.md).
...
* [필터 레일은](moderation.md#filter-rail) 컨텐츠를 필터링할 속성을 선택할 수 있는 레일을 엽니다.
...
* 게시되었을 때
* 게시물에 대한 답글 수
* [게시물과 연관된 감정](moderate-ugc.md#sentiment)
* 승인되면 확인 표시가 표시됩니다
...
각 카테고리 내의 필터는 ****&#x200B;함께 사용할 수도 있고, 서로 다른 카테고리의 필터는 ****&#x200B;함께 사용할 수도 있습니다.
...
>[!NOTE]
>
>관리자가 벌크 중재 콘솔에 액세스하면 Geometrixx 샘플과 같이 [사이트 생성 마법사로](sites-console.md)만들어지지 않은 사이트를 비롯하여 UGC에 대한 모든 참조가 표시됩니다.
>
...
그 결과 필터 그룹 `Voting`아래에 대한 새로운 선택 가능한 `Content Type` 필터가 됩니다.
...
한 컨텐츠의 플래그가 지정되면 해당 단일 컨텐츠에 플래그를 해제할 때까지 플래그 지정된 상태로 남아 있습니다. 단, **[!UICONTROL 플래그]** 단추를 다시 선택합니다. 중요 또는 후속 조치와 같은 플래그 지정은 없습니다.
...
[센티멘트는](moderate-ugc.md#sentiment) 긍정, 부정 또는 중립적인 센티멘트 값이 있는 게시물에 표시된 참조된 UGC를 제한합니다.

![chlimage_1-479](assets/chlimage_1-479.png)

## 중재 작업 {#moderation-actions}

[컨텐츠 영역에 있는 하나 이상의 선택 사항이나 컨텐츠 세부 사항을 볼 때 중재 작업을](moderate-ugc.md#moderation-actions) 수행할 수 있습니다.
...
중재 작업 아이콘 위의 카드 보기에 나타나는 내용은 게시물의 텍스트이고 아래는
