...
이 페이지에는 다음과 같은 추가 기술 세부 사항이 포함되어 있습니다.
...
예를 들어, `this.isAssigned` 에서 `/libs/social/forum/components/hbs/topic/list-item.hbs`다음을 검색합니다.
...
이 비헤이비어에 대한 모든 변경 사항은 사용자 지정된 스크립트(재정의 또는 오버레이)에서 수행해야 합니다. 클라이언트측 [사용자 지정을 참조하십시오](client-customize.md).

## 점수 지정 및 배지 로그 디버그 {#debug-log-for-scoring-and-badging}

점수 지정 및 배지 디버깅에 도움이 되도록 사용자 지정 로그 파일을 설정할 수 있습니다. 이 기능에 문제가 발생하면 이 로그 파일의 내용을 고객 지원에 제공할 수 있습니다.

자세한 지침은 사용자 [정의 로그 파일 만들기를 참조하십시오](../../help/sites-deploying/monitoring-and-maintaining.md#create-a-custom-log-file).

슬링로그 파일을 신속하게 설정하려면

1. Adobe Experience **[!UICONTROL Manager 웹 콘솔 로그 지원에]**&#x200B;액세스(예:

   * http://localhost:4502/system/console/slinglog

1. 새 **[!UICONTROL 로거 추가 선택]**

   1. 로그 `DEBUG` 수준 **[!UICONTROL 선택]**
   1. 로그 파일의 이름을 **[!UICONTROL 입력합니다]**. 예를 들면 다음과 같습니다.

      * logs/scoring-debug.log
   1. 로거 **[!UICONTROL 항목]** 두 개(클래스) 입력( `+` 아이콘 사용)
...
## 점수 지정 및 배지 지정 UGC {#ugc-for-scoring-and-badging}

선택한 SRP가 JSRP 또는 MSRP일 때 점수 지정 및 배지 관련 UGC를 볼 수 있지만 ASRP는 볼 수 없습니다. (이러한 용어에 익숙하지 않은 경우 커뮤니티 컨텐츠 [스토리지](working-with-srp.md) 및 [스토리지 리소스 공급자 개요를 참조하십시오](srp.md).)
...
설치된 [릴리스에](deploy-communities.md#LatestReleases) 대한 최신 Javadocs는 Adobe 저장소의 개발자가 사용할 수 있습니다. 커뮤니티에 [대한 Maven 사용을 참조하십시오.Javadocs](maven.md#javadocs).

**저장소에서 UGC의 위치와 형식은 경고**&#x200B;없이 변경될 수 있습니다.

### 설치 예 {#example-setup}

저장소 데이터의 스크린샷은 두 개의 서로 다른 AEM 사이트에서 포럼에 대한 점수 지정 및 배지 설정으로부터 옵니다.

1. 고유 ID가 있는 AEM 사이트(마법사를 사용하여 만든 커뮤니티 사이트):

   * 시작하기 자습서(참여) 사이트 [사용](getting-started.md)
   * 포럼 페이지 노드 찾기
...
### 점수 UGC에 액세스 {#access-scoring-ugc}

API를 [사용하는](#scoring-and-badging-apis) 것이 좋습니다.
...
커뮤니티 구성 요소 가이드 사이트의 경우 사용자 및 점수는 점수 지정 규칙 이름, 기본 ID(), 고유 ID 및 사용자 ID로 구성된 경로에 있습니다. `default-site`

* `.../scoring/forums-scoring/default-site/b27a17cb4910a9b69fe81fb1b492ba672d2c086e/riley`

점수는 직접 값을 포함하거나 atomicCounter를 간접적으로 참조할 수 `scoreValue_tl` 있는 속성에 저장됩니다.

![chlimage_1-251](assets/chlimage_1-251.png)

### 배지 UGC에 액세스 {#access-badging-ugc}
...
포인트를 기준으로 정렬된 멤버 목록을 표시하려면 다음을 수행합니다.

* [커뮤니티 사이트 또는 그룹 템플릿에 포함하기 위한 리드 보드 기능입니다](functions.md#leaderboard-function) .
* [리더보드 구성 요소로서](enabling-leaderboard.md), 페이지 작성을 위한 리더보드 함수의 주요 구성 요소입니다.
