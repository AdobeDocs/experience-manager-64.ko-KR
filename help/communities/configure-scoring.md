...
예를 들어, 검색 `this.isAssigned` 위치 `/libs/social/forum/components/hbs/topic/list-item.hbs`,
...
이 비헤이비어에 대한 모든 변경 사항은 사용자 지정된 스크립트(재정의 또는 오버레이)로 이루어져야 합니다. 클라이언트측 [사용자 지정을 참조하십시오](client-customize.md).
...
1. 예를 들어 **[!UICONTROL Adobe Experience Manager 웹 콘솔 로그 지원]**&#x200B;액세스

   * http://localhost:4502/system/console/slinglog

1. 새 로거 **[!UICONTROL 추가 선택]**

   1. 로그 수준 `DEBUG` 에 대해 **[!UICONTROL 선택]**
   1. 로그 파일 이름 **[!UICONTROL 을]**&#x200B;입력합니다.
...
선택한 SRP가 JSRP 또는 MSRP일 때 점수 및 배지 지정과 관련된 UGC를 볼 수 있지만 ASRP는 볼 수 없습니다. (이러한 용어에 익숙하지 않은 경우 [커뮤니티 컨텐츠 저장소](working-with-srp.md) 및 [저장소 리소스 공급자 개요를 참조하십시오](srp.md).)
...
1. 고유 ID가 있는 AEM 사이트(마법사를 사용하여 만든 커뮤니티 사이트):
   ...
   * 시작하기 자습서(참여) 사이트 사용( [시작하기 자습서)](getting-started.md)
   * 포럼 페이지 노드 찾기
...
사용자가 두 개의 청동 배지를 획득했고 중재자 배지가 수여됨에 따라 포럼 참가자가 다음과 같이 나타납니다.
...
커뮤니티 구성 요소 안내서 사이트의 경우 사용자 및 점수는 점수 규칙 이름, 기본 id(), 고유 ID 및 사용자 ID로 구성된 경로에 있습니다. `default-site`
...
* [커뮤니티 사이트 또는 그룹 템플릿에 포함하기 위한 리드 보드 함수](functions.md#leaderboard-function) .
* [페이지 작성을 위한 Leaderboard 함수의 주요 구성 요소인](enabling-leaderboard.md)Leaderboard 구성 요소입니다.
