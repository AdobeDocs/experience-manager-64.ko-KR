...
이 경험에서는 [Aaron McDonald가](tutorials.md#demo-users) 커뮤니티 관리자 및 사회자의 역할을 할당받았다고 가정합니다 [](create-site.md#roles). 그렇지 않은 경우 작성 환경으로 돌아가 사이트 설정을 [수정하고 커뮤니티 관리자](sites-console.md#modifying-site-properties) 및 중재자로 Aaron McDonald를 선택합니다.

오른쪽 상단 모서리에서 사용자 이름 &quot;aaron.mcdonald@mailinator.com&quot; `Log in`및 암호 &quot;password&quot;를 선택하고 서명합니다. Twitter 또는 Facebook 자격 증명으로 로그인할 수 있습니다.

![chlimage_1-312](assets/chlimage_1-312.png)

로그인하면 새로운 메뉴 항목이 있는데, 이는 멤버가 중재자 `Administration`의 역할을 맡았기 때문입니다. 이제 다양한 링크를 선택하는 것이 더 흥미롭습니다.

![chlimage_1-313](assets/chlimage_1-313.png)

선택한 참조 사이트 템플릿에 달력 함수가 먼저 포함되고, 다음으로 활동 스트림 함수, 포럼 함수 등이 추가되었기 때문에 달력 페이지는 홈 페이지입니다. 이 구조는 [사이트 템플릿](sites.md#edit-site-template) 콘솔에서 또는 작성 환경에서 사이트 속성을 수정할 때 표시됩니다.
...
작성 환경에서 그룹을 만들고 작성 환경의 커뮤니티 사이트( [커뮤니티 그룹 콘솔](groups.md))에서 관리할 수도 있습니다. 이 튜토리얼에서는 작성자 [에서 그룹을](nested-groups.md) 만드는 환경이 다음으로 중요합니다.
...
* 이름 `Gymnastics`
   * 설정을 (제한된 `Member Only Group` 멤버십)으로 변경
   * 템플릿 탭에서 `Conversational Group`
...
* 작성자의 경우 [그룹 콘솔](groups.md) 은 세 번째 선택, 즉 `Public Group` (옵션 멤버십)을 제공합니다.

두 그룹이 모두 만들어지면, 야구 그룹, 열린 그룹을 선택하고 해당 링크를 확인합니다. `Discussions` `What's New` `Members`
...
Weston McCall(weston.mccall@dodgit.com / 암호)로 로그인한 다음 동일한 위치로 이동합니다. Weston은 공개 `Join` 그룹 `Baseball` 과 비공개 `enter or Leave` 그룹 `Gymnastics`중 하나를 사용할 수 있습니다.
...
예를 들어, **작성자** 인스턴스로 이동하고, `engage` 커뮤니티 사이트 콘솔에서 [폴더를 열고](sites-console.md)사이트 **열기** 아이콘을 선택하여 작성자 편집 모드로 전환합니다. 그런 다음 미리 보기 모드를 선택하여 `Web Page`링크를 선택한 다음 편집 모드를 선택하여 제목 및 텍스트 구성 요소를 추가합니다. 마지막으로, 페이지나 전체 사이트를 다시 게시합니다.

![chlimage_1-321](assets/chlimage_1-321.png)

## 관리 링크 {#administration-link}

커뮤니티 구성원이 중재 권한을 가지고 있으면 관리 링크가 표시되고 이 링크를 선택하면 게시된 커뮤니티 컨텐츠가 표시되고 작성 환경의 [중재 콘솔과](moderate-ugc.md) 유사한 방식으로 중재되도록 [](moderation.md) 됩니다.
...
로그인하면 표시된 첫 번째 페이지가 `Profile`페이지(개인화)입니다.
