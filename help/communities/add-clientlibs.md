...
1. CRXDE **[!UICONTROL Lite를]**&#x200B;사용하여 `/etc/designs`

1. 마우스 오른쪽 버튼을 클릭하고 `an-scf-sandbox` 선택합니다. `Create Node`
...
새 **[!UICONTROL 노드의 속성]** 탭에서 `clientlibs` **`categories`** 속성을 입력합니다.
...
참고:&#39;apps&#39;로 카테고리 값 미리 보기 는 &#39;소유 응용 프로그램&#39;이 /libs가 아닌 /apps 폴더에 있는 것으로 식별하기 위한 규칙입니다.  중요:자리 표시자 `js.txt` 및 `css.txt` 파일 추가 공식적으로 cq:ClientLibraryFolder가 없는 것은 아닙니다.
...
그런 다음 style.css라는 clientlibs 아래에 파일을 만들고 내용을 다음 항목으로 설정합니다.
...
노드의 **[!UICONTROL 속성]** 탭에서 다중 값 문자열 속성 `clientlibs` 포함을 ****&#x200B;입력합니다. 이렇게 하면 SCF 구성 요소에 필요한 [클라이언트측 라이브러리(clientlibs)가 포함됩니다](client-customize.md#clientlibs-for-scf). 이 자습서에서는 커뮤니티 구성 요소에 필요한 많은 클라이언트를 추가합니다.
...
이 경우 Adobe는 모든 SCF 클라이언트를 포함하므로 Author clientlibs인 보다 기본적인 SCF clientlibs를 선호합니다.
...
* 값: **`cq.social.scf`**
   ...
   *각 항목 다음에&#x200B;**[키를]**클릭하여 다음 clientlib 범주를 추가합니다.*
...
이제 저장소에 `/etc/designs/an-scf-sandbox/clientlibs` 표시되는 방식입니다.
...
1. CRXDE **[!UICONTROL Lite를]**&#x200B;사용하여 **`/libs/foundation/components/page/headlibs.jsp`**
1. 마우스 오른쪽 버튼을 클릭하고 **[!UICONTROL 복사]** (또는 도구 모음에서 복사)를 선택합니다.
1. 선택 **`/apps/an-scf-sandbox/components/playpage`**
1. 마우스 오른쪽 버튼을 클릭하고 **[!UICONTROL 붙여넣기를]** 선택합니다(또는 도구 모음에서 붙여넣기를 선택합니다).
...
이때, 최소 샌드박스는 존재하므로 패키지로 저장할 가치가 있으므로, 재생하는 동안 해당 저장소가 손상되고 다시 시작하고자 하는 경우, 서버를 끄고, 폴더 crx-quickstart/를 삭제하거나, 서버를 켜고, 이 저장된 패키지를 업로드 및 설치할 수 있으며 이러한 가장 기본적인 단계를 반복할 필요가 없습니다.

이 패키지는 바로 [들어가서](create-sample-page.md) 재생을 시작할 때까지 기다릴 수 없는 사용자를 위해 샘플 페이지 만들기 자습서에 있습니다...

패키지를 생성하려면


* CRXDE **[!UICONTROL Lite에서]**&#x200B;패키지 [아이콘을 클릭합니다](http://localhost:4502/crx/packmgr/)
...
이제 다운로드를 선택하여 **[!UICONTROL 디스크에]** 저장하고 다른 **[!UICONTROL 곳에서 패키지]** 업로드를 **[!UICONTROL 선택할 수 있을 뿐만 아니라]** 추가 > 복제를 선택하여 샌드박스를 localhost 게시 인스턴스로 푸시하여 샌드박스 영역을 확장할 수 있습니다.
