...
>[!NOTE]
>
>기본적으로 `communities`이라는 이름으로 설정되는 mongoDB 데이터베이스는 [노드 저장소 또는 데이터(이진) 스토어에 사용되는 데이터베이스의 이름으로 설정하면 안 됩니다](../../help/sites-deploying/data-store-config.md). AEM 6](../../help/sites-deploying/storage-elements-in-aem-6.md)의 [스토리지 요소를 참조하십시오.
...
오류를 해결하려면 [표준 MLS 설치](solr.md#installing-standard-mls)에 대한 지침을 따를 때
