...
* **[!UICONTROL 추가]**
   ...
   (*optional*) 멤버 할당 페이지 및 리소스 콘솔에서 지원 리소스에 대한 카드에 표시할 이미지입니다. 서버의 로컬 파일 시스템에서 이미지가 선택됩니다. 이미지를 제공하지 않으면 업로드된 리소스에 대해 축소판이 생성됩니다.
...
* **[!UICONTROL 태그]**
   ...
   (*선택 사항*) 활성 리소스를 하나 이상의 카탈로그에 연결하는 하나 이상의 태그를 선택할 수 있습니다. [역량 강화 리소스 태그 지정](tag-resources.md)을 참조하십시오.
...
* **[!UICONTROL 로컬]**
파일에서 업로드 로컬 파일 시스템에서 업로드하면 기본 파일 브라우저가 파일을 선택하고 업로드합니다. 지원되는 파일 유형은 SCORM.zip(HTML5 또는 SWF), MP4 비디오, SWF, PDF 및 이미지 유형(JPG, PNG, GIF, BMP)입니다. 파일 이름은 자산 라이브러리에 추가되는 자산의 이름이 됩니다.

* **[!UICONTROL 자산 라이브러리]**
찾아보기자산 라이브러리에서 선택하십시오. 선택은 커뮤니티 사이트에서 볼 수 있는 것으로 제한됩니다.
...
* **[!UICONTROL 담당자]**
추가 [풀다운 메뉴를 사용하여 수강생으로 등록할 사용자](#members-group)  및 사용자 그룹(굵게 표시된 사람) 중에서 선택합니다. 구성원이 커뮤니티 사이트에 로그인하면 해당 구성원이 등록된 활성 리소스(및 학습 경로)가 [할당](functions.md#assignments-function) 페이지에 표시됩니다.
...
* **[!UICONTROL 추가]**
   ...
   (*optional*) 구성원의 할당 페이지와 리소스 콘솔에서 학습 경로의 카드에 표시할 이미지입니다. 서버의 로컬 파일 시스템에서 이미지가 선택됩니다. 이미지를 제공하지 않으면 업로드된 리소스에 대해 축소판이 생성됩니다.
...
* **[!UICONTROL 태그]**
   ...
   (*선택적*) 하나 이상의 태그를 선택하여 학습 경로를 하나 이상의 카탈로그와 연결할 수 있습니다. [역량 강화 리소스 태그 지정](tag-resources.md)을 참조하십시오.
...
* **[!UICONTROL 리소스]**
   ...
   하나 이상의 리소스가 현재 커뮤니티 사이트에 대해 제작된 *게시된 *활성 리소스 중 하나입니다.
...
휴지통 아이콘을 선택하면 지원 리소스는 확인 후 `Delete`d가 됩니다.
