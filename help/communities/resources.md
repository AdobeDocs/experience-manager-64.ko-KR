...
title: 지원 리소스 콘솔
seo-title: 지원 리소스 콘솔
description: 리소스 콘솔은 활성 관리자가 활성 커뮤니티 사이트의 구성원에게 리소스를 만들고 관리하고 할당하는 곳입니다
seo-description: 리소스 콘솔은 활성 관리자가 활성 커뮤니티 사이트의 구성원에게 리소스를 만들고 관리하고 할당하는 곳입니다
...
* **[!UICONTROL 추가]**
   ...
   ***참고***:권장되는 이미지 크기는 단순히 480 x 480픽셀이 아닙니다. 카드의 반응형 디자인으로 다양한 브라우저 크기로 인해 디스플레이 크기는 220 X 165 픽셀에서 400 x 165 픽셀까지 다양합니다.

* **[!UICONTROL 사이트 이름]**

   (*readonly*) 리소스를 추가할 커뮤니티 사이트입니다.

* **[!UICONTROL 리소스 이름;amp;ast;]**

   (*필수*) 리소스의 표시 이름입니다. 표시 이름에서 유효한 노드 이름이 생성됩니다.

* **[!UICONTROL 태그]**

   (*선택*&#x200B;사항) 하나 이상의 태그를 선택하여 활성 리소스를 하나 이상의 카탈로그에 연결할 수 있습니다. 태깅 [지원 리소스를 참조하십시오](tag-resources.md).
...
* **[!UICONTROL 로컬 파일에서]**&#x200B;업로드 로컬 파일 시스템에서 업로드하면 기본 파일 브라우저를 사용하여 파일을 선택하고 업로드합니다. 지원되는 파일 유형은 SCORM.zip(HTML5 또는 SWF), MP4 비디오, SWF, PDF 및 이미지 유형(JPG, PNG, GIF, BMP)입니다. 파일 이름은 자산 라이브러리에 추가되는 자산의 이름이 됩니다.

* **[!UICONTROL 자산 라이브러리 찾아보기]**&#x200B;자산 라이브러리에서 선택합니다. 선택은 커뮤니티 사이트 내에서 표시되는 항목으로 제한됩니다.
...
* **[!UICONTROL 할당자]**&#x200B;추가 풀다운 메뉴를 사용하여 [수강생으로 등록할 사용자 및 사용자 그룹(굵게 표시된 얼굴)](#members-group) 중에서 선택할 수 있습니다. 구성원이 커뮤니티 사이트에 로그인하면 등록된 활성 리소스(및 학습 경로)가 [할당] [페이지에](functions.md#assignments-function) 표시됩니다.
...
* **[!UICONTROL 카탈로그에 표시]**
   ...
   이 옵션을 선택하지 않으면 학습 경로가 카탈로그에 표시되지 않습니다. 이 확인란을 선택하면 [사전 필터링되거나](catalog-developer-essentials.md#pre-filters) UI의 멤버 필터가 없는 경우 모든 카탈로그에 학습 경로가 표시됩니다. 카탈로그에 학습 경로를 표시하면 포함된 모든 리소스에 대한 읽기 액세스 권한이 간접적으로 부여됩니다. 기본값은 선택 취소입니다.
...
* **[!UICONTROL 사전 요구 사항 학습 경로]**(*선택*) 다른 게시된 학습 경로를 선택한 경우 수강생이 이 학습 경로를 선택하기 전에 수료해야 합니다.
...
* **[!UICONTROL 학습 경로에서 순서 적용]**
   ...
   (*선택 사항*) 켜기로 설정된 경우 학습 경로를 계속 진행해야 하는 수강생 순서는 역량 강화 리소스가 추가되는 순서입니다. 기본값은 해제입니다.

* **[!UICONTROL 리소스]**

   현재 커뮤니티 사이트에 대해 작성된 *게시된 *활성 리소스 중 하나 이상의 리소스를 선택합니다.
...
>[!NOTE]
>
>학습 경로에서 참조되는 역량 강화 리소스는 동일한 담당자(수강생)가 나열되지 않아야 합니다.
>
...
휴지통 아이콘을 선택하면 지원 리소스가 확인 `Delete`후 표시됩니다.
...
* 표시되는 대화 상자에서 다시 게시를 **[!UICONTROL 선택합니다]** .
* 닫기 **[!UICONTROL 선택]**
...
게시 환경의 구성원이 해당 리소스를 삭제하지 않고 활성 리소스를 일시적으로 액세스할 수 없게 하려면 world 아이콘을 사용하여 리소스를 `Unpublish`사용합니다.
...
* **재생 횟수**
   ...
   재생은 비디오 재생 또는 PDF 열기 등 모든 학습자가 리소스와 상호 작용하는 경우
...
등급 보고서는 역량 강화 리소스를 평가한 수강생 수를 기준으로 하며 각 별 등급 수와 총 등급 수와 평균 평점의 요약을 표시합니다.
...
* 를 참조하십시오. `Download Learning Path Report as CSV` 버튼
