...
* **[!UICONTROL 작은 자산]**
   ...
   (*선택*&#x200B;사항) AEM Assets에서 선택 카탈로그 등의 게시 환경에서 리소스를 나타내는 축소판 이미지입니다.

* **[!UICONTROL 큰 자산]**

   (*선택*&#x200B;사항) AEM Assets에서 선택 리소스의 기본 페이지와 같이 게시 환경에서 리소스를 나타내는 큰 이미지입니다.

* **[!UICONTROL 컨텐츠 조각 자산]**

   (*선택*&#x200B;사항) AEM Assets에서 선택 게시 환경에서 참조될 수 있지만 기본적으로 사용되지 않는 컨텐츠 조각.
...
* **[!UICONTROL 로컬 파일에서 업로드]**&#x200B;로컬 파일 시스템에서 업로드하면 기본 파일 브라우저가 파일을 선택하고 업로드합니다. 지원되는 파일 형식은 SCORM.zip(HTML5 또는 SWF), MP4 비디오, SWF, PDF 및 이미지 유형(JPG, PNG, GIF, BMP)입니다. 파일 이름은 자산 라이브러리에 추가되는 자산의 이름이 됩니다.

* **[!UICONTROL 자산 라이브러리 찾아보기]**&#x200B;자산 라이브러리에서 선택하십시오. 커뮤니티 사이트에서 볼 수 있는 선택으로 제한됩니다.
...
* **[!UICONTROL 참석자]**&#x200B;추가풀다운 메뉴를 사용하여 [수강생으로 등록할 사용자](#members-group) 및 사용자 그룹(굵게 표시된 사람)에서 선택합니다. 구성원이 커뮤니티 사이트에 로그인하면 등록된 활성 리소스(및 학습 경로)가 [할당] [페이지에](functions.md#assignments-function) 표시됩니다.
...
* **[!UICONTROL 학습 경로에서 순서 적용]**
   ...
   (*선택*&#x200B;사항) 켜짐으로 설정된 경우 학습 경로를 진행해야 하는 수강생 순서가 활성 리소스가 추가되는 순서입니다. 기본값은 해제입니다.
...
휴지통 아이콘을 선택하면 지원 리소스가 확인 `Delete`후 생성됩니다.
...
* 월드 아이콘을 선택하여 `Publish`
* 팝업 대화 상자에서 **[!UICONTROL 다시 게시를 선택합니다]**
* 닫기 **[!UICONTROL 선택]**
...
* 활성 리소스의 경우: `Download Resource Report as CSV` 버튼
* 을 참조하십시오. `Download Learning Path Report as CSV` 버튼
