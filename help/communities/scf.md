...
* **기능**:사용 사례 80%를 사용자 요구에 맞게 구성할 필요가 없거나 거의 통합되지 않는 즉시 사용 가능
* **스킨 변경 가능**:CSS 스타일링을 위해 HTML 속성을 일관되게 사용
...
* **노트북**:모든 기술을 기반으로 구축된 웹 페이지에 통합/임베드
...
* 스키닝
   * [CSS만 편집](client-customize.md#skinning-css) 필요
* 룩앤필
   * JS 템플릿 및 CSS 변경
* 살펴보기, 느낌 및 UX
...
* 새 사용자 정의 작업을 추가하려면
   * 새 [게시물 저장 작업](server-customize.md#postoperation-class) 만들기
   * 필요에 따라 기존 [OperationServices](server-customize.md#operationservice-class) 사용
   * 필요에 따라 클라이언트측에서 작업을 호출할 Javascript 코드를 추가합니다.
...
[커뮤니티 내용 저장소](working-with-srp.md)에 저장된 UGC 처리에 대해 알아보려면
...
* JavaScript를 기반으로 하는 UX를 위한 CSS 클래스와 스타일이 지정된 CSS 클래스를 별도로 유지
