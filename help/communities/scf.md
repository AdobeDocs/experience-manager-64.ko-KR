...
* **기능**: 80%의 사용 사례를 사용자 요구에 맞게 구성할 필요가 없거나 거의 통합되지 않고 즉시 사용 가능
* **스킨 변경이 가능**: CSS 스타일링을 위한 HTML 속성의 일관된 사용
* **확장 가능**: 구성 요소 구현은 객체 지향적이고 비즈니스 로직에 대한 간단한 개념으로 서버에 증가분 비즈니스 로그인을 손쉽게 추가할 수 있습니다.
...
SCF에서 구성 요소는 SocialComponent POJO, 핸들바 JS 템플릿(구성 요소를 렌더링하기 위한) 및 CSS(구성 요소의 스타일 지정)로 구성됩니다.

핸들막대 JS 템플릿은 클라이언트의 구성 요소와 사용자 상호 작용을 처리하기 위해 모델/뷰 JS 구성 요소를 확장할 수 있습니다.

구성 요소가 데이터 수정을 지원해야 하는 경우, 기존 웹 애플리케이션의 모델/데이터 개체와 유사한 데이터의 편집/저장을 지원하도록 SocialComponent API의 구현을 작성할 수 있습니다. 또한 작업 요청을 처리하고, 비즈니스 논리를 수행하고, 모델/데이터 개체에서 API를 호출하는 데 작업(컨트롤러)과 작업 서비스를 추가할 수 있습니다.
...
* 룩앤필
   * JS 템플릿 및 CSS 변경
* 살펴보기, 느낌 및 UX
   * JS 템플릿, CSS 및 Javascript [확장/재정의 변경](client-customize.md#extending-javascript)
* JS 템플릿 또는 GET 종단점에 사용할 수 있는 정보를 수정하려면
   * Social 구성 [요소 확장](server-customize.md#socialcomponent-interface)
* 작업 중에 사용자 정의 처리를 추가하려면
   * OperationExtension [작성](server-customize.md#operationextension-class)
* 새 사용자 지정 작업을 추가하려면
   * 새 [슬링 게시물 작업 만들기](server-customize.md#postoperation-class)
...
Java API는 쉽게 상속되거나 하위 분류되는 추상적인 클래스와 인터페이스를 제공합니다.
...
HTTP API는 PhoneGap 앱, 기본 앱, 기타 통합 및 매쉬업을 위한 클라이언트 플랫폼 및 사용자 정의 용이성을 지원합니다. 또한 HTTP API를 사용하면 프레임워크 구성 요소를 기술 기반의 모든 웹 페이지에 통합할 수 있도록 클라이언트 없이 커뮤니티 사이트를 서비스로 실행할 수 있습니다.
...
1. 공장을 호출하고 자원 및 요청을 처리할 `SocialComponent`수 있는 능력을 받습니다.
...
GET(읽기) 작업 외에도 프레임워크는 끝점 패턴을 정의하여 만들기, 업데이트 및 삭제를 포함한 구성 요소의 다른 작업을 활성화합니다. 이러한 끝점은 HTTP 상태 코드 또는 JSON 응답 개체를 사용하여 입력을 받아들이고 응답하는 HTTP API입니다.

이 프레임워크 종단점 패턴은 CUD 작업을 확장 가능하고 재사용 가능하며 안정적으로 만듭니다.

**`POST Request`**

모든 SocialComponent 작업에 대한 Sling POST:연산이 있습니다. 각 작업에 대한 비즈니스 논리 및 유지 관리 코드는 HTTP API를 통해 또는 다른 곳에서 OSGi 서비스로 액세스할 수 있는 OperationService로 래핑됩니다. 후크는 적용 전/후 작업에 대한 지원 연결 가능 작업 익스텐션이 제공됩니다.

![chlimage_1-27](assets/chlimage_1-27.png)

### SRP(Storage Resource Provider) {#storage-resource-provider-srp}

커뮤니티 콘텐츠 저장소에 저장된 UGC 처리에 대한 자세한 내용은 [를 참조하십시오](working-with-srp.md).
...
HBS(Handlebars) 템플릿 파일(.hbs)은 .jsp 및 .html 템플릿 파일과 유사하지만 클라이언트 브라우저와 서버에서 모두 렌더링하는 데 사용할 수 있습니다. 따라서 클라이언트측 템플릿을 요청하는 클라이언트 브라우저는 서버로부터 .hbs 파일을 수신하게 됩니다.
...
그 결과 JCR 하위 노드가 par 노드 아래에 있으며, 이는 Sling 주소 지정이 가능합니다.
...
일부 AEM Communities 구성 요소만 동적으로 포함될 수 있습니다. They are:
...
* CSS 클래스를 유지하여 스타일을 지정할 수 있으며 JavaScript 기반의 UX를 위한 CSS 클래스와 별도로 유지할 수 있습니다
