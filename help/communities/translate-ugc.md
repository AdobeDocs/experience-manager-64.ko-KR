...
AEM Communities의 번역 기능은 [페이지 컨텐츠 번역](../../help/sites-administering/translation.md) 를 사용하여 커뮤니티 사이트에 게시된 UGC(사용자 생성 컨텐츠)로 [SCF(소셜 구성 요소 프레임워크) 구성 요소](scf.md).
...
이 설명서 섹션에서는 AEM을 Analytics에 연결하는 방법을 이해한다고 가정하면서 번역 서비스가 UGC에서 작동하는 방식을 자세히 설명합니다 [번역 서비스 공급자](../../help/sites-administering/translation.md#connectingtoatranslationserviceprovider) 및 를 구성하여 웹 사이트에 해당 서비스 통합 [번역 통합 프레임워크](../../help/sites-administering/tc-tic.md).

번역 서비스 공급자가 사이트와 연결되면 사이트의 각 언어 사본은 주석과 같은 SCF 구성 요소를 통해 게시된 UGC 자체 스레드를 유지합니다.

번역 서비스 제공자 외에 번역 통합 프레임워크를 구성할 경우, 사이트의 각 언어 사본이 UGC의 단일 스레드를 공유할 수 있어 언어 사본 간에 글로벌 통신을 제공할 수 있다. 언어별로 구분된 토론 스레드 대신 구성된 [글로벌 공유 저장소](#global-translation-of-ugc) 표시 중인 언어 복사본에 관계없이 전체 스레드를 표시할 수 있습니다. 또한, 지역별 등 글로벌 참여자의 논리적 그룹화를 위해 서로 다른 글로벌 공유 스토어를 지정하여 여러 번역 통합 구성을 구성할 수 있습니다.

## 기본 번역 서비스 {#the-default-translation-service}

AEM Communities에 포함된 [평가판 라이선스](../../help/sites-administering/tc-msconf.md#microsoft-translator-trial-license) 대상 [기본 번역 서비스](../../help/sites-administering/tc-msconf.md) 여러 언어에 대해 활성화되었습니다.

When [커뮤니티 사이트 만들기](sites-console.md)로 설정되면 기본 번역 서비스는 `Allow Machine Translation` 이(가) [번역](sites-console.md#translation) 하위 패널.

>[!CAUTION]
>
>기본 번역 서비스는 데모용입니다.
>
>프로덕션 시스템의 경우 라이선스 번역 서비스가 필요합니다. 라이센스가 없는 경우 기본 번역 서비스는 [꺼짐](../../help/sites-administering/tc-msconf.md#microsoft-translator-trial-license-geometrixx-outdoors).

## UGC 글로벌 번역 {#global-translation-of-ugc}

웹 사이트에 여러 개가 있는 경우 [언어 복사](../../help/sites-administering/tc-prep.md)를 기본 번역 서비스에서는 UGC가 기본적으로 동일한 구성 요소(구성 요소를 포함하는 페이지의 언어 사본)에 의해 생성된 경우처럼, 한 사이트에 입력한 UGC가 다른 사이트에 입력한 UGC와 관련이 있음을 인식하지 못합니다.
...
>
>UGC가 여전히 [일반 상점](working-with-srp.md), 언어 특정 UGC 위치 아래에 있으며 글로벌 번역이 구성된 후 추가된 새 컨텐츠는 글로벌 공유 저장소 위치에서 검색됩니다.
>
...
* **[!UICONTROL Sites]** 탭: 기본값으로 둘 수 있음
* **[!UICONTROL 커뮤니티]** 탭:
   * **[!UICONTROL 번역 공급자]**
드롭다운 목록에서 번역 공급자를 선택합니다. 기본값은 입니다. 
`microsoft`: 평가판 서비스입니다.

   * **[!UICONTROL 컨텐츠 카테고리]**
번역되는 컨텐츠를 설명하는 카테고리를 선택합니다. 기본값은 입니다. 
`General.`

   * **[!UICONTROL 로케일 선택...]**
(선택 사항) UGC를 저장할 로케일을 선택하면 모든 언어 사본의 게시물이 하나의 글로벌 대화에 나타납니다. 규칙에 따라 [기본 언어](sites-console.md#translation) 참조하십시오. 선택 `No Common Store` 글로벌 번역을 비활성화합니다. 기본적으로 글로벌 번역은 비활성화됩니다.

* **[!UICONTROL 자산]** 탭: 기본값으로 둘 수 있음
* 선택 **[!UICONTROL 확인]**
