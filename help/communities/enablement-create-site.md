...
[커뮤니티 사이트 만들기는](sites-console.md) 커뮤니티 사이트를 만드는 단계를 안내하는 마법사를 사용합니다. 마지막 단계에서 사이트를 커밋하기 전에 `Next`단계나 이전 단계 `Back`로 이동할 수 있습니다.

새 커뮤니티 사이트 만들기를 시작하려면 다음을 수행하십시오.

작성 [인스턴스 사용](http://localhost:4502/)

* 관리자 권한으로 로그인
* 커뮤니티 **[!UICONTROL > 사이트로 이동합니다.]**

* **[!UICONTROL 만들기]**&#x200B;를 선택합니다

### 1단계: 사이트 템플릿 {#step-site-template}

![enablementsiteplate](assets/enablementsitetemplate.png)
...
* **[!UICONTROL 참조 사이트 템플릿]**: 아래로 끌다 `Reference Structured Learning Site Template`
...
![활성 사이트 테마](assets/enablementsitetheme.png)
...
역량 강화 [커뮤니티를](overview.md#enablement-community) 비공개로 설정하는 것이 좋습니다.

익명의 사이트 방문자가 액세스가 거부되면 커뮤니티 사이트는 비공개 사이트이고, 자가 등록을 할 수 없으며, 소셜 로그인을 사용할 수 없습니다.

사용자 관리에 대해 대부분의 확인란을 선택 [취소하는지 확인합니다](sites-console.md#user-management).
...
커뮤니티 컨텐츠에 적용할 수 있는 태그는 이전에 [Tagging Console](../../help/sites-administering/tags.md#tagging-console) (예: [Tutorial 네임스페이스)을 통해 정의된 AEM 네임스페이스를 선택하여 제어합니다](enablement-setup.md#create-tutorial-tags).

또한 커뮤니티 사이트에 대한 태그 네임스페이스를 선택하면 카탈로그 및 활성 리소스를 정의할 때 표시되는 선택이 제한됩니다. 중요 [정보는 태깅 지원 리소스](tag-resources.md) 를 참조하십시오.
...
[커뮤니티 구성원 역할은](users.md) 역할 섹션의 설정을 통해 할당됩니다.
...
>[!NOTE]
>
>[터널 서비스를](deploy-communities.md#tunnel-service-on-author) 통해 게시 환경에만 존재하는 구성원 및 그룹을 선택할 수 있습니다.

![community_roles](assets/community_roles.png)

#### MODERATION {#moderation}

사용자 생성 콘텐츠(UGC)를 [중재하기](sites-console.md#moderation) 위한 기본 전역 설정을 적용합니다.
...
스크린샷에 표시된 선택 `Communities`은 [구성 설명서의 프레임워크 예입니다.](analytics.md#aem-analytics-framework-configuration)

![chlimage_1-288](assets/chlimage_1-288.png)

#### TRANSLATION {#translation}

번역 설정 [은](sites-console.md#translation) UGC를 번역할지 여부와 언어(있는 경우)를 지정합니다.

* 기계 **[!UICONTROL 번역 허용 확인]**
* 기본 설정 사용
...
* **[!UICONTROL Marketing Cloud 조직 ID]**(선택 사항) 활성화 보고에서 [비디오 하트비트 분석을 포함하는 데 필요한 Adobe Analytics 계정의](analytics.md#video-heartbeat-analytics) ID입니다.
...
커뮤니티 사이트의 폴더를 선택한 후 다음 4개의 작업 아이콘이 나타나도록 사이트 아이콘을 마우스로 가리킵니다.
...
* **사이트**&#x200B;열기연필 아이콘을 선택하여 작성 편집 모드에서 커뮤니티 사이트를 열고 페이지 구성 요소를 추가 및/또는 구성합니다.

* **사이트 편집**&#x200B;속성 아이콘을 선택하여 커뮤니티 사이트를 열어 제목과 같은 속성을 수정하거나 테마를 변경합니다

* **사이트 게시**&#x200B;커뮤니티 사이트를 게시하려면 월드 아이콘을 선택합니다(기본적으로 localhost:4503으로).

* **사이트 내보내기**&#x200B;를 선택하여 [패키지 관리자에 저장되고 다운로드한 커뮤니티 사이트의 패키지를 만듭니다](../../help/sites-administering/package-manager.md) .

   UGC는 사이트 패키지에 포함되지 않습니다.

* **사이트 삭제**&#x200B;커뮤니티 사이트를 삭제하려면 커뮤니티 사이트 콘솔에서 마우스로 사이트 위에 마우스를 놓을 때 나타나는 사이트 삭제 아이콘을 선택합니다. 이 작업은 UGC, 사용자 그룹, 자산 및 데이터베이스 레코드 등 사이트와 연관된 모든 항목을 제거합니다.
...
새 커뮤니티 사이트와 함께 다양한 관리 기능에 적합한 권한이 설정된 새 사용자 그룹이 만들어집니다. 자세한 내용은 커뮤니티 사이트를 [위한 사용자 그룹을 참조하십시오](users.md#usergroupsforcommunitysites).

이 새 커뮤니티 사이트의 경우 1단계의 사이트 이름이 &quot;활성화&quot;인 경우 게시 환경에 있는 새 사용자 그룹이 [커뮤니티 구성원 및 그룹 콘솔에서 표시될 수 있습니다](members.md#groups-console).

![chlimage_1-294](assets/chlimage_1-294.png)

### 구성원을 커뮤니티에 지정 구성원 그룹 활성화 {#assign-members-to-community-enable-members-group}

작성자의 경우, 터널 서비스가 활성화되어 있으면, 초기 설정 [중에 만든](enablement-setup.md#publishcreateenablementmembers) 사용자를 새로 만든 커뮤니티 사이트의 커뮤니티 구성원 그룹에 할당할 수 있습니다.
...
* 커뮤니티 > **[!UICONTROL 그룹 콘솔로]** 이동
* 커뮤니티 **[!UICONTROL 활성화 구성원]** 그룹 선택
* 그룹에 `ski` 구성원 **[!UICONTROL 추가]** 검색 상자에 입력
* 커뮤니티 **[!UICONTROL 스키 클래스]** 선택(수강생 그룹)
* 검색 상자 `quinn` 에 입력
* 퀸 **[!UICONTROL 하퍼]** 선택(지원 리소스 연락처)
...
사이트가 구성되고 게시되도록 푸시되면 게시 인스턴스에 [로그인 매핑](sites-console.md#configure-for-authentication-error) () `Adobe Granite Login Selector Authentication Handler`을 구성합니다. 로그인 자격 증명이 올바르게 입력되지 않으면 인증 오류가 오류 메시지와 함께 커뮤니티 사이트의 로그인 페이지가 다시 표시됩니다.
...
이렇게 하려면 게시 시 [리소스 매핑](http://localhost:4503/crx/de) 테이블을 편집하려면 CRX|DE [](../../help/sites-deploying/resource-mapping.md) Lite를 사용해야 합니다.

시작하려면

1. 게시 시 CRXDE에 액세스하고 관리자 권한으로 로그인

   * 예를 들어 http://localhost:4503/crx/de으로 [이동하여](http://localhost:4503/crx/de) `admin/admin`
...
1. http://localhost:4503/으로 이동
   ...
   * http://localhost:4503/content/sites/enable/en.html에 도착

>[!NOTE]
>
>비활성화하려면 속성 값을 &#39;x&#39; - `sling:match` 및 `xlocalhost.4503/$` 모두 저장하기만 하면 됩니다 ****.
...
정규 표현식&#x200B;**문자열 끝에 있는 &#39;**$ `sling:match`&#39;는 반드시 매핑되어야 하며, `http://localhost:4503/` 그렇지 않으면 리디렉션 값이 URL에서 server:port 뒤에 있을 수 있는 모든 경로에 앞에 추가됩니다. 따라서 AEM에서 로그인 페이지로 리디렉션하려고 하면 실패합니다.

## 커뮤니티 사이트 수정 {#modifying-the-community-site}

사이트를 처음 만든 후 작성자는 사이트 [열기 아이콘을](sites-console.md#authoring-site-content) 사용하여 표준 AEM 작성 활동을 수행할 수 있습니다.

또한 관리자는 사이트 [편집 아이콘을](sites-console.md#modifying-site-properties) 사용하여 사이트 속성(예: 제목)을 수정할 수 있습니다.
...
사이트의 구조를 수정하려면 **[!UICONTROL 커뮤니티, 사이트]** 콘솔로 이동하고 `enable` 폴더를 연 다음 사이트 **편집** 아이콘을 선택하여 사이트의 `Enablement Tutorial`속성에 액세스하십시오.
...
오른쪽 **[!UICONTROL 위]** 모서리에서 저장을 선택하여 커뮤니티 사이트에 대한 변경 사항을 저장합니다.
