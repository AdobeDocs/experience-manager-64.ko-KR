...
# 활성화를 위해 새 커뮤니티 사이트 작성 {#author-a-new-community-site-for-enablement}
...
[커뮤니티 사이트 만들기](sites-console.md) 커뮤니티 사이트를 만드는 단계를 안내하는 마법사를 사용합니다. 앞으로 이동할 수 있습니다 `Next`단계 또는 `Back`를 클릭하여 마지막 단계에서 사이트를 커밋하기 전에 이전 단계로 이동합니다.

새 커뮤니티 사이트 만들기를 시작하려면 다음을 수행하십시오.

사용 [작성자 인스턴스](http://localhost:4502/)

* 관리자 권한으로 로그인
* 다음으로 이동 **[!UICONTROL 커뮤니티 > 사이트]**
...
설정 **사이트 템플릿** 단계와 같이 제목, 설명, URL의 이름을 입력하고 커뮤니티 사이트 템플릿을 선택합니다.
...
설정 단계에서 선택하기 전에 `Next`에는 사용자 관리, 태깅, 역할, 중재, analytics, 번역 및 활성화와 관련된 구성에 대한 액세스를 제공하는 7개의 섹션이 있습니다.

#### 사용자 관리 {#user-management}

권장 사항 [지원 커뮤니티](overview.md#enablement-community) 비공개로 설정하세요.

커뮤니티 사이트는 익명의 사이트 방문자가 액세스가 거부되고, 자체 등록을 하지 못할 수 있으며, 소셜 로그인을 사용하지 않을 수 있는 경우 비공개 사이트입니다.

에 대한 대부분의 확인란이 선택 취소되어 있는지 확인 [사용자 관리](sites-console.md#user-management):
...
커뮤니티 컨텐츠에 적용할 수 있는 태그는 [태깅 콘솔](../../help/sites-administering/tags.md#tagging-console) (예: [튜토리얼 네임스페이스](enablement-setup.md#create-tutorial-tags)).

또한 커뮤니티 사이트에 대해 태그 네임스페이스를 선택하면 카탈로그 및 지원 리소스를 정의할 때 표시되는 선택 사항이 제한됩니다. 자세한 내용은 [태깅 지원 리소스](tag-resources.md) 참조하십시오.
...
>[!NOTE]
>
>[터널 서비스](deploy-communities.md#tunnel-service-on-author) 게시 환경에만 있는 구성원 및 그룹을 선택할 수 있습니다.
...
스크린샷에 나타난 선택 내용인데 `Communities`는 의 프레임워크 예입니다. [구성 설명서.](analytics.md#aem-analytics-framework-configuration)

![chlimage_1-288](assets/chlimage_1-288.png)

#### 번역 {#translation}

다음 [번역 설정](sites-console.md#translation) UGC를 번역할지 여부 및 언어로 변환할지 여부를 지정합니다(해당하는 경우).
...
(선택 사항) 다음을 포함할 때 필요한 Adobe Analytics 계정의 ID입니다 [비디오 하트비트 분석](analytics.md#video-heartbeat-analytics) 를 클릭합니다.
...
이 새 커뮤니티 사이트의 경우, 1단계의 사이트 이름이 &quot;enable&quot;이면 게시 환경에 있는 새 사용자 그룹이 [커뮤니티 구성원 및 그룹 콘솔](members.md#groups-console):

![chlimage_1-294](assets/chlimage_1-294.png)

### 커뮤니티에 구성원 할당 구성원 그룹 활성화 {#assign-members-to-community-enable-members-group}

작성자의 경우, 터널 서비스를 활성화한 상태에서 [초기 설정 중에 생성된 사용자](enablement-setup.md#publishcreateenablementmembers) 새로 만든 커뮤니티 사이트에 대한 커뮤니티 구성원 그룹으로 이동합니다.

커뮤니티 그룹 콘솔을 사용하여 구성원을 개별적으로 추가하거나 그룹의 멤버십을 통해 추가할 수 있습니다.

이 예에서는 그룹 `Community Ski Class` 이(가) 그룹의 구성원으로 추가됩니다 `Community Enable Members` 구성원 뿐만 아니라 `Quinn Harper`.

* 다음으로 이동 **[!UICONTROL 커뮤니티 > 그룹]** 콘솔
* 선택 **[!UICONTROL 커뮤니티 구성원 활성화]** 그룹
* Enter 키 `ski` 로 **[!UICONTROL 그룹에 구성원 추가]** 검색 상자
* 선택 **[!UICONTROL 커뮤니티 스키 클래스]** (학습자 그룹)
* Enter 키 `quinn` 검색 상자에
* 선택 **[!UICONTROL 퀸 하퍼]** (지원 리소스 연락처)

* **[!UICONTROL 저장]**&#x200B;을 선택합니다

![chlimage_1-295](assets/chlimage_1-295.png)

## 게시할 때의 구성 {#configurations-on-publish}
...
이렇게 하려면 을 사용해야 합니다. [CRX|DE](http://localhost:4503/crx/de) Lite를 사용하여 [리소스 매핑](../../help/sites-deploying/resource-mapping.md) 게시할 표.

시작하려면

1. 게시 시 CRXDE에 액세스하고 관리자 권한으로 로그인합니다

   * 예를 들어 다음 위치로 이동합니다. [http://localhost:4503/crx/de](http://localhost:4503/crx/de) 및 `admin/admin`
...
사이트의 구조를 수정하려면 **[!UICONTROL 커뮤니티, 사이트]** 콘솔에서 를 엽니다. `enable` 폴더를 선택하고 **사이트 편집** 아이콘 을 클릭하여 속성 액세스 `Enablement Tutorial`.
...
선택 **[!UICONTROL 저장]** 오른쪽 상단 모서리에서 커뮤니티 사이트에 대한 변경 사항을 저장합니다.
