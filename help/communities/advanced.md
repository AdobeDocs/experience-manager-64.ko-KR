...
* 기본 및 고급 점수 지정 및 배지 규칙은 동일한 방식으로 컨텐츠에 [적용됩니다](implementing-scoring.md#apply-rules-to-content)
   * 동일한 컨텐츠에 기본 및 고급 점수 및 배지 규칙을 적용할 수 있습니다.
* [구성 요소에 대한 배지](implementing-scoring.md#enable-badges-for-component) 활성화

점수 지정 및 배지 규칙 설정의 차이점은 다음과 같습니다.

* 구성 가능한 고급 채점 엔진
* 고급 점수 지정 규칙:
   * `scoringType` [고급]으로 **[!UICONTROL 설정]**
   * 중지 단어 필요

* 고급 배지 규칙:
   * `badgingType` [고급]으로 **[!UICONTROL 설정]**
...
* **[!UICONTROL 점수 가중치]**&#x200B;점수를 계산할 때 가장 높은 우선순위가 주어져야 하는 동사를 주제에 지정합니다. 하나 이상의 항목을 입력할 수 있지만 항목당 **하나의 동사로 제한됩니다**. 항목 [및 동사를 참조하십시오](implementing-scoring.md#topics-and-verbs).
...
* **[!UICONTROL 채점 증가율]**
   ...
   이 값은 점수를 지정합니다. 0~10점 사이에서 성장 속도가 느려져서 전문가 수를 제한할 수 있습니다.
...
고급 배지 규칙 속성은 [기본 배지 규칙 속성과 다릅니다](implementing-scoring.md#badging-rules).
...
| **속성** | **유형** | **값 설명** |
|---------------|----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 배지 패스 | String[] | (필수) 배지 수준 수에 이르기까지 배지 이미지의 다중 값 문자열. 배지 이미지 경로는 먼저 최고 전문가에게 부여되도록 순서가 정해져야 합니다. 배지(badgingLevels)에 지정된 배지 수보다 적은 배지가 있으면 배열의 나머지 배지가 채워집니다. 예제 항목:/etc/community/badging/images/expert-badge/jcr:content/expert.png |
| badgingLevels | 긴 | (선택 사항) 수여할 전문성 수준을 지정합니다. 예를 들어 전문가 및 거의 전문가(배지 두 개)가 있어야 하는 경우 값을 2로 설정해야 합니다. badgingLevel은 badgingPath 속성에 나열된 전문가 관련 배지 이미지 수와 일치해야 합니다. 기본값은 1입니다. |
| badgingType | 문자열 | (필수) 점수 엔진을 &quot;기본&quot; 또는 &quot;고급&quot;으로 식별합니다. &quot;advanced&quot;로 설정하면 기본값은 &quot;basic&quot;입니다. |
| 점수 지정 규칙 | String[] | (선택 사항) 배지 규칙을 나열된 점수 지정 규칙으로 식별된 점수 이벤트로 제한하는 다중 값 문자열. 예 입력:/etc/community/scoring/rules/adv-comments-scoring기본값은 제한이 없습니다. |
...
활동의 보상으로 전문가 배지가 나타나려면 두 가지가 필요합니다.
...
베타 릴리스에 포함된 두 가지 [포럼 기능](functions.md#forum-function) (포럼 기능의 포럼 및 주석 구성 요소에 대해 하나씩)에 대한 고급 채점 규칙은 다음과 같습니다.
