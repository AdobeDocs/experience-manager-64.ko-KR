...
AEM Communities를 사용하면 블로그, Q&amp;A 및 이벤트 달력을 통해 정보를 제공하는 사이트 방문자와의 관계를 개발할 수 있을 뿐만 아니라 포럼, 댓글 및 기타 커뮤니티 컨텐츠(UGC)라고도 하는 통찰력을 얻을 수 있습니다.

또한 AEM Communities를 사용하면 게시 환경에서 신뢰할 수 있는 회원이 중재할 수 있습니다. Twitter 및 Facebook을 통한 소셜 로그인, 커뮤니티 콘텐츠의 인라인 번역, 게시된 커뮤니티 사이트에서 커뮤니티 그룹 만들기, 점수 지정, 배지, 파일 공유, 알림 및 활동 스트림을 사용할 수 있습니다.
...
* 선택한 [커뮤니티 사이트 템플릿을](sites.md) 기반으로 사이트의 기능 조합
...
* 필수 기능 제공:
   ...
   * 알림:구성원은 관련성 이벤트를 봅니다.
   * 메시지:구성원은 커뮤니티 사이트 내에서 메시지를 보내거나 받을 수 있습니다.
   * 검색:커뮤니티 사이트 내에서 검색 기능
   * 언어 전환:다중 언어 사이트의 언어 선택 [기능](../../help/sites-administering/translation.md)
...
새 커뮤니티 사이트를 빠르게 만드는 데 도움이 되도록 하려면 AEM Communities [시작하기를 참조하십시오](getting-started.md).
...
* [권장 토폴로지](topologies.md):사용 사례 및 SRP 선택에 따라 토폴로지 토론
* [AEM 6.3 Communities로 업그레이드](upgrade.md):aem 6.3으로 이동할 때 UGC에 대한 유용한 정보를 제공합니다.

## 커뮤니티 콘솔 {#communities-consoles}

작성 환경에서 전역 탐색 콘솔은 다음을 포함하는 [커뮤니티 콘솔에](consoles.md)대한 액세스를 제공합니다.
...
* [구성 요소 가이드](components-guide.md)
   ...
   * 모든 커뮤니티 구성 [요소의](http://localhost:4502/editor.html/content/community-components/en.html)샘플과 기본 구성을 제공하고 이를 실험해 볼 수 있는 기능을 제공하는 샘플 사이트 커뮤니티 구성 요소
...
사이트 [템플릿 콘솔을](sites.md)참조하십시오.

## 커뮤니티 기능 {#community-functions}

커뮤니티 경험에 필요한 기능은 잘 알려져 있습니다. AEM Communities에서는 이러한 기능을 커뮤니티 기능이라고 하는 빌드 블록으로 사용할 수 있습니다.

커뮤니티 기능은 소셜 사이트 템플릿에 쉽게 통합되는 기능으로 연결된 구성 요소로 구성된 일반적인 AEM 페이지입니다.

커뮤니티 [기능 콘솔을](functions.md)참조하십시오.

## 커뮤니티 그룹 및 그룹 템플릿 {#community-groups-and-group-templates}

커뮤니티 그룹 기능은 권한이 있는 사용자 및 작성 환경과 게시 환경 모두에서 커뮤니티 사이트에서 하위 커뮤니티를 동적으로 만들 수 있는 기능입니다.

작성 환경에서 템플릿 구조에 그룹 함수가 [들어 있으면 기존 커뮤니티 사이트 내에 커뮤니티 그룹(하위 커뮤니티)을 만들거나 기존 그룹 내에 중첩될 수](functions.md#groups-function)있습니다.
...
* [AEM Communities 시작하기](getting-started.md) - 중첩된 그룹을 포함한 커뮤니티 사이트를 빠르게 만드는 자습서

## 커뮤니티 구성 요소 {#community-components}

커뮤니티 사이트가 빌드되는 [커뮤니티 구성 요소는](author-communities.md) 모든 AEM 사이트에 커뮤니티 기능을 추가하는 데 사용할 수 있습니다.

구성 요소에 대한 대화형 탐색을 위해 [커뮤니티 구성 요소 안내서를](components-guide.md) 사용할 수 있습니다.
...
새 참여 커뮤니티를 빠르게 만드는 쉬운 방법을 살펴보려면 AEM Communities [시작하기를 참조하십시오](getting-started.md).
...
* 할당 완료 추적
* 보고 및 분석 액세스
* 포럼, 메시징, 댓글 및 등급을 통해 학습 리소스에 대한 대화를 나눌 수 있는 기능

활성화 Add-on이 구성되면 [](enablement.md)활성화 커뮤니티를 만들 수 있으며, 프로덕션 환경에서 사용하려면 추가 라이선스가 필요합니다. 활성 커뮤니티 사이트에는 [할당 기능이](#community-functions)포함됩니다.

새로운 활성 커뮤니티를 손쉽게 만들려면 AEM Communities [for Enablement 시작하기를 참조하십시오](getting-started-enablement.md).
...
* 설치할 패키지 AEM Demo Machine은 Windows, MacOS 및 Linux에서 CQ 5.5, CQ 5.6.1, AEM 6.0, AEM 6.1, AEM 6.2 및 AEM 6.3에서 테스트되었습니다.
