...
작성자 모드에서 페이지에 `Comments`구성 요소를 추가하려면 구성 요소 브라우저를 사용하여 을 찾습니다
...
필요한 [클라이언트 측 라이브러리](essentials-comments.md#essentials-for-client-side)가 포함되면 이 방법으로 `Comments`구성 요소가 표시됩니다.
...
**[!UICONTROL 댓글]** 탭에서 방문자가 댓글을 입력하는 방법을 지정합니다.
...
**[!UICONTROL 사용자 중재]** 탭에서 게시된 주석이 관리되는 방식을 지정합니다. 자세한 내용은 [사용자 생성 콘텐츠 중재](moderate-ugc.md)를 참조하십시오.
...
* **[!UICONTROL 플래그 지정 제한]**
   ...
   공개 보기에서 주석이 숨겨지기 전에 플래그를 지정해야 하는 횟수를 입력합니다. 이 숫자는 **[!UICONTROL 중재 임계값]**&#x200B;보다 크거나 같아야 합니다. 기본값은 5입니다.
...
개발자를 위한 [댓글 핵심 사항](essentials-comments.md) 페이지에서 자세한 정보를 찾을 수 있습니다.
