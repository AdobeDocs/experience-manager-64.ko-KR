...
* [주석](comments.md):구성원이 댓글에 댓글 또는 답글을 게시합니다.
* [포럼](forum.md):구성원은 새 주제를 게시하거나 주제에 답글을 달다
* [아이디어](ideation-feature.md):구성원들이 아이디어 또는 댓글을 게시합니다.
...
* [일괄 중재 콘솔](moderation.md)
   ...
   중재 콘솔은 관리자와 공용 환경의 [커뮤니티 중재자](users.md)뿐만 아니라 작성 환경의 관리자가 액세스할 수 있습니다. 이는 커뮤니티 콘텐트가 [공통 스토어](working-with-srp.md)에 저장되어 있을 때 가능합니다.
...
* `Moderator`
커뮤니티 중재자  [그룹](users.md#publishenvironmentusersandgroups) 의 구성원( [중재자 권한이 있음](in-context.md#moderatorpermissions))
* `Creator`\
   컨텐츠를 게시한 사용자
* `Member`\
   특별한 권한이 없는 로그인 사용자
...
![](assets/cutugc.png) ![cutcputbackugc](assets/putbackugc.png)
...
UGC가 사전 중재되는 경우 중재 작업에 의해 승인될 때까지 게시물은 게시된 사이트에 표시되지 않습니다. [커뮤니티 사이트](sites-console.md)를 만드는 동안 ` [Content is Premoderated](sites-console.md#moderation)` 상자를 선택하면 전체 사이트에 대한 사전 중재를 사용할 수 있습니다. 구성 요소를 페이지에 배치하면 편집 대화 상자의 설정을 사용하여 조정을 지원하는 구성 요소를 사전 중재하도록 구성할 수 있습니다.

* [주석 ](comments.md) 및  [검토](reviews.md)

   **[!UICONTROL 사용자 중재]** 탭에서 **[!UICONTROL 사전 중재]**&#x200B;를 선택합니다.

* [포럼](forum.md)  [아이디어](ideation-feature.md),  [QnA](working-with-qna.md) 및  [](calendar.md) 일정 설정  ****   **[!UICONTROL 탭, 확인]**
...
그러나 기본 스팸 단어를 사용자 정의하거나 확장하려면 [overlay](overlay-comments.md)을 통해 기본 스팸 단어 구조에 따라 /apps 디렉토리에 단어 집합을 만듭니다.
...
1. `/system/console/configMgr`으로 이동하여 [웹 콘솔](http://localhost:4502/system/console/configMgr)을 엽니다.

1. **[!UICONTROL AEM Communities 자동 중재]** 구성을 찾아 편집합니다.
1. `SpamProcess` 항목을 추가합니다.
...
센티멘트 분석은 사전 구성된 규칙 세트를 사용하고 UGC의 센티멘트를 계산합니다. 기본 규칙은 `/libs/cq/workflow/components/workflow/social/sentiments/rules.`에 있습니다.
...
* 규칙 2:음수가 없고 한 개 이상의 긍정적인 단어가 있을 경우 값을 10으로 설정합니다.
* 규칙 3:긍정적인 단어보다 음수가 많은 경우 값을 3으로 설정합니다.
* 규칙 4:음수보다 긍정적인 단어가 더 많을 경우 값을 8으로 설정합니다.

규칙을 덮어쓰거나 추가하려면 기본 규칙 구조에 따라 /apps 디렉토리에 규칙 세트를 만듭니다. 센티멘트 구성을 편집하여 규칙의 위치를 식별합니다.

분석하면 센티멘트가 UGC에 저장됩니다.

[벌크 중재 콘솔](moderation.md)에서 센티멘트가 부정, 중립 또는 긍정인지 여부에 따라 UGC를 필터링하고 볼 수 있습니다.

#### Watchwords {#watchwords}

AEM 커뮤니티에서는 [센티멘트](#sentiment)를 평가하는 과정의 한 단계로 *watchword analyzer*를 제공합니다. 감시자가 제공하는 센티멘트 값에 대한 기여는 게시된 컨텐츠에 사용된 네거티브 및 긍정 감시어와 금지된 단어를 비교하기 때문입니다.
...
* **긍정적인**
감시단어 기본값을 무시하는 긍정적인 감정에 기여하는 단어의 쉼표로 구분된 목록입니다. 기본값은 빈 목록입니다.

* **네거티브**
워치단어 기본값을 무시하는 네거티브 센티멘트에 기여하는 단어로 구분된 목록입니다. 기본값은 빈 목록입니다.

* **Watchwords 노드**
에 대한 명시적 경로기본 노드를 포함하는 노드의 저장소 위치입니다. 
`positive` 기본  `negative` 감시어를 지정하는 속성 기본값은 `/libs/settings/community/watchwords/default`입니다.

* **센티멘트**
규칙긍정 및 부정 감시 단어를 기반으로 센티멘트를 계산하기 위한 규칙의 저장소 위치입니다. 기본값은 
