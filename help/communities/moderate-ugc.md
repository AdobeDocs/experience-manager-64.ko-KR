...
* [댓글](comments.md):구성원은 댓글에 댓글 또는 답글을 게시합니다.
* [포럼](forum.md):구성원이 새 주제를 게시하거나 주제에 답글을 달다
* [아이디어](ideation-feature.md):구성원은 아이디어 또는 댓글을 게시합니다.
...
* [일괄 중재 콘솔](moderation.md)
   ...
   중재 콘솔은 공개 환경의 관리자 및 [커뮤니티 중재자와](users.md) 작성자 환경의 관리자가 액세스할 수 있습니다. 이는 커뮤니티 콘텐츠가 [일반 스토어에](working-with-srp.md)저장되어 있을 때 가능합니다.
...
게시된 컨텐츠(UGC)에서 수행할 수 있는 작업은 사용자 ID 및 환경에 따라 다릅니다. 아래 표에서는 다음 용어를 사용하여 사용자 ID에 따라 다양한 역할을 설명합니다.

* `Admin`\
   커뮤니티 관리자 [그룹의](users.md) 구성원인 사용자
* `Moderator`
커뮤니티 중재자 [그룹의 구성원(](users.md#publishenvironmentusersandgroups) 중재자 권한 [](in-context.md#moderatorpermissions)있음)
* `Creator`\
   컨텐츠를 게시한 사용자
* `Member`\
   특별한 권한이 없는 로그인 사용자
...
UGC가 사전 중재되면 중재 작업으로 승인될 때까지 게시물이 게시된 사이트에 표시되지 않습니다. 커뮤니티 사이트를 [만드는 동안 상자를 선택하면 전체 사이트에 대한 사전 중재가](sites-console.md)` [Content is Premoderated](sites-console.md#moderation)` 활성화됩니다. 구성 요소를 페이지에 배치하면, 중재 지원 구성 요소를 편집 대화 상자의 설정을 사용하여 사전 중재하도록 구성할 수 있습니다.

* [댓글](comments.md) 및 [검토](reviews.md)

   사용자 **[!UICONTROL 중재]** 탭에서 **[!UICONTROL 사전 중재 선택]**

* [포럼](forum.md)관념화 [,](ideation-feature.md)Qa [A](working-with-qna.md), [달력 및 On](calendar.md) **** **[!UICONTROL , TabModerated, CheckModerated]**
...
그러나 기본 스팸 단어를 사용자 정의하거나 확장하려면 [오버레이를](overlay-comments.md)통해 /apps 디렉토리에 기본 스팸 단어의 구조를 따라 일련의 단어를 만듭니다.
...
1. AEM **[!UICONTROL Communities 자동]** 중재 구성을 찾아 편집합니다.
...
센티멘트 분석은 사전 구성된 규칙 집합을 사용하고 UGC의 센티멘트를 계산합니다. 기본 규칙은 `/libs/cq/workflow/components/workflow/social/sentiments/rules.`
...
* 규칙 3:긍정적인 단어보다 음수가 많은 경우 값을 3으로 설정
* 규칙 4:음수 단어보다 긍정적인 단어가 더 많을 경우 값을 8로 설정합니다.
...
AEM 커뮤니티는 [센티멘트를](#sentiment)평가하는 프로세스의 한 단계로 *감시자 분석기 *를 제공합니다. 감시자가 제공하는 센티멘트 값에 대한 기여도는 게시된 컨텐츠에 사용된 네거티브 및 긍정적인 감시어와 금지된 단어를 비교한 것입니다.

#### 센티멘트 및 감시 단어 구성 {#configure-sentiment-and-watchwords}

긍정 및 부정 감시자 목록을 센티멘트 규칙처럼 사용자 지정할 수 있습니다.

기본 감시 단어 목록은 기본값과 유사하거나 단어 목록으로 OSGi 서비스를 구성하여 기본값을 덮어쓰는 방식으로 `sentimentprocess.name`응답지에서 노드의 속성으로 입력할 수 있습니다.

센티멘트 **process.name** 역시 사용자 지정 센티멘트 규칙의 위치를 참조하도록 수정할 수 있습니다.
...
* **긍정적인**&#x200B;감시단어 기본값을 무시하는 긍정적인 감정에 기여하는 단어로 구분된 목록입니다. 기본값은 빈 목록입니다.

* **네거티브 감시단어**&#x200B;기본값을 무시하는 네거티브 센티멘트에 기여하는 단어로 구분된 목록입니다. 기본값은 빈 목록입니다.

* **Explicit Path to Watchwords**&#x200B;노드 기본 `positive` 및 `negative` 속성을 포함하는 노드의 저장소 위치입니다. 기본값은 `/libs/settings/community/watchwords/default`입니다.

* **센티멘트**&#x200B;규칙긍정 및 부정 감시자를 기반으로 센티멘트를 계산하기 위한 규칙의 저장소 위치입니다. 기본값은 `/libs/cq/workflow/components/workflow/social/sentiments/rules` 입니다(하지만 더 이상 관련된 워크플로우가 없습니다).
