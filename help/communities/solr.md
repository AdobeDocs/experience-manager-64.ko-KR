...
1. Solaris가 배포된 로컬 서버로 다운로드
   ...
   * 다운로드 `view` 를 시작하려면 선택합니다.
   * 파일이 적절한 이름 및 인코딩(UTF8)과 함께 저장되었는지 확인

1. 독립 실행형 또는 SolrCloud 모드에 대한 설치 지침에 따릅니다.

#### SolrCloud 모드 - 표준 MLS {#solrcloud-mode-standard-mls}

1. SolrCloud 모드에서 솔루션 설치 및 구성
1. 새 구성 준비:

   1. solr-install-dir */myconfig/와 같은* new-config-dir **&#x200B;만들기
...
1. 다운로드한 **schema.xml** 및 **solrconfig.xml** 파일을 동일한 디렉토리로 복사합니다
