...
* 최신 설치 [AEM 6.4 업데이트](#aem-updates)
...
* 선택 사항 확인 [공유 UGC 저장소](working-with-srp.md) (**SRP**)
   ...
   * JCR SRP인 경우 [(JSRP)](jsrp.md)
   ...
   대상 **[사용 기능](overview.md#enablement-community)**
...
AEM 6.4 Communities GA에 커뮤니티 패키지가 포함되어 있습니다. AEM 6.4 업데이트에 대해 알려면 [커뮤니티](/help/release-notes/release-notes.md#experience-manager-communities)를 참조하고 [AEM 6.4 릴리스 노트](/help/release-notes/release-notes.md#release-information).

### AEM 6.4 업데이트 {#aem-updates}

AEM 6.3부터 Communities에 대한 업데이트는 AEM 누적 수정 팩 및 서비스 팩의 일부로 제공됩니다.

AEM 6.4에 대한 최신 업데이트를 보려면 다음을 확인하십시오 [Adobe Experience Manager 6.4 누적 수정 팩 및 서비스 팩](https://helpx.adobe.com/kr/experience-manager/aem-releases-updates.html).
...
* 대상 [활성화](enablement.md): scorm 활동 및 학습자 기록
* 대상 [DSRP](dsrp.md): 사용자 생성 컨텐츠 저장(UGC)
...
번들 설치에 대한 자세한 내용은 [웹 콘솔](/help/sites-deploying/web-console.md#bundles) 페이지.
...
AEM Communities SCORM 엔진은 [활성화](overview.md#enablement-community) 기능. AEM Communities 6.4 버전에서 지원되는 Scorm 패키지는 다음과 같습니다.
...
* **[cq -social-scorm-package, 버전 2.2.2](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Fadobe%2Fpackages%2Fcq640%2Fsocial%2Fscorm%2Fcq-social-scorm-2017-pkg)** include [SCORM 2017.1](https://rusticisoftware.com/blog/scorm-engine-2017-released/) 엔진. 이 SCORM 패키지는 AEM 6.4.2.x Communities에서 지원됩니다.

SCORM 엔진을 새로 설치하려면 [SCORM 2017.1](https://rusticisoftware.com/blog/scorm-engine-2017-released/) (다음 중 [  cq -social-scorm-package, 버전 2.2.2](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Fadobe%2Fpackages%2Fcq640%2Fsocial%2Fscorm%2Fcq-social-scorm-2017-pkg)) 를 사용해야 합니다. 따라서 SCORM 2017에서 지원하는 학습 리소스를 재생할 수 있습니다.

<!--This section used to be an accordion until converted to straight Markdown. When accordions are enabled, revert-->

### SCORM 패키지를 처음 설치하려면

1. 설치 **[cq-social-scorm-package, 버전 2.2.2](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Fadobe%2Fpackages%2Fcq640%2Fsocial%2Fscorm%2Fcq-social-scorm-2017-pkg).**
...
기존 SCORM 설치는 로 업그레이드할 수 있습니다. [**cq-social-scorm-package, 버전 2.2.2**](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Fadobe%2Fpackages%2Fcq640%2Fsocial%2Fscorm%2Fcq-social-scorm-2017-pkg) (사용 [SCORM 2017.1](https://rusticisoftware.com/blog/scorm-engine-2017-released/)). 작성된 교육 과정 컨텐츠에 SCORM 2017.1이 필요한 경우
...
1. 설치 **[cq-social-scorm-package, 버전 2.2.2](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Fadobe%2Fpackages%2Fcq640%2Fsocial%2Fscorm%2Fcq-social-scorm-2017-pkg).**
1. 다음 위치에서 패키지 다운로드 `/libs/social/config/scorm/ScormEngine.zip` 그리고 그것을 추출합니다.
1. 이동 **설치 관리자** 추출된 디렉토리의 폴더입니다.
1. 업데이트 `SystemDatabaseConnectionString` 사용 `scorm db connection url` 파일에서 **[!UICONTROL EngineInstall.xml]**.
1. 다음 명령을 사용하여 설치 프로그램 폴더에서 mysql 스키마 업그레이드 도구를 실행합니다.

   `java -Dlogback.configurationFile=logback.xml -cp "lib/*" RusticiSoftware.ScormContentPlayer.Logic.Upgrade.ConsoleApp EngineInstall.xml`
1. 모니터 `engine_upgrade.log` 파일 을 참조하십시오.
1. 추가 `/content/communities/scorm/RecordResults` in **[!UICONTROL 제외된 경로]** CSRF 필터의 속성 `https://<hostname>:<port>/system/console/configMgr` 게시자에 대해 설명합니다.
...
자세한 내용 및 설치 정보는 [솔루션 구성](solr.md) SRP용
...
* 을(를) 선택합니다 **[!UICONTROL 자산]** 탭
* 선택 **[!UICONTROL 디스크에 다운로드]**

로컬 AEM 인스턴스에서 패키지 관리자를 사용합니다(예: [http://localhost:4502/crx/packmgr/](http://localhost:4502/crx/packmgr/)). 로컬 AEM 패키지 리포지토리에 업로드합니다.

또는 로컬 AEM 인스턴스에서 패키지 공유를 사용하여 패키지에 액세스합니다(예: [http://localhost:4502/crx/packageshare/](http://localhost:4502/crx/packageshare/)), `Download`버튼이 로컬 AEM 인스턴스의 패키지 저장소에 다운로드됩니다.
...
AEM Communities에서 공용 저장소는 사용자 생성 컨텐츠(UGC)를 저장하는 데 사용되며 종종 을 [저장소 리소스 공급자(SRP)](working-with-srp.md). 권장 배포 중심은 공용 저장소에 대한 SRP 옵션을 선택하는 것입니다.

일반 스토어는 게시 환경에서 UGC를 조정하고 분석할 수 있도록 지원하지만 [복제](sync.md) UGC 배포
...
플랫폼을 업그레이드하는 것 외에도 [AEM Communities 6.4로 업그레이드](upgrade.md) 커뮤니티 변경 사항에 대해 알아보려면.

## 구성 {#configurations}

### 기본 게시자 {#primary-publisher}

선택한 배포가 [팜 게시](topologies.md#tarmk-publish-farm)를 지정하면 하나의 AEM 게시 인스턴스를 **`primary publisher`** 를 사용하는 기능과 같이, 일부 인스턴스에서 발생해서는 안 되는 **알림** 또는 **Adobe Analytics**.
...
* 두 에이전트에 대해 동일한 절차를 따릅니다.
   ...
   * **역방향 복제 에이전트(게시 취소)**
      ...
      1. 에이전트를 선택합니다
      1. 선택 **[!UICONTROL 편집]**
      1. 을(를) 선택합니다 **[!UICONTROL 전송]** 탭
      1. 포트가 아닌 경우 `4503`, 편집 **[!UICONTROL URI]** 올바른 포트를 지정하려면
      1. 사용자가 아닌 경우 `admin`, 편집 **[!UICONTROL 사용자]** 및 **[!UICONTROL 암호]** 의 멤버를 지정하려면 `administrators` 사용자 그룹
...
작성 환경을 사용하여 [사이트 만들기](sites-console.md), [사이트 속성 수정](sites-console.md#modifying-site-properties) 또는 [커뮤니티 구성원 관리](members.md)를 채울 때는 작성자에 등록된 사용자가 아니라 게시 환경에 등록된 구성원(사용자)에 액세스해야 합니다.
...
* 설정 **작성자**
* 관리 권한으로 로그인
* 게시자가 localhost가 아니거나 전송 사용자가 localhost:4503이 아닌 경우 `admin`,

   Then [복제 에이전트 구성](#replication-agents-on-author)
...
모든 AEM 서버 인스턴스가 동일한 암호화 키를 사용해야 하는 AEM Communities의 두 가지 기능이 있습니다. 이것들은 [Analytics](analytics.md) 및 [ASRP](asrp.md).
...
* 복사할 주요 자료가 포함된 AEM 인스턴스(일반적으로 작성자 인스턴스)에 액세스합니다
   ...
   * 을(를) 찾습니다 `com.adobe.granite.crypto.file` 로컬 파일 시스템에 번들
