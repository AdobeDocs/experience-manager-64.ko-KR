...
description: Handlebars 도우미 메서드를 사용하여 SCF로 작업 촉진
seo-description: Handlebars 도우미 메서드를 사용하여 SCF로 작업 촉진
...
AEM Communities으로 제공된 사용자 지정 SCF 도우미는 [클라이언트 라이브러리](../../help/sites-developing/clientlibs.md)에 정의됩니다.

* /etc/clientlibs/social/commons/scf/helpers.js

>[!NOTE]
>
>[최신 커뮤니티 기능 팩](deploy-communities.md#latestfeaturepack)을(를) 설치해야 합니다.
...
문자열 구분 모드 목록을 기준으로 [WCM 모드](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html)의 현재 값을 테스트하는 블록 도우미입니다.
...
* **모드**:문자열
   ...
   (선택 사항) 설정되면 테스트할 [WCM 모드](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html)의 쉼표로 구분된 목록입니다.
...
따라서 JCR 노드로 추가된 리소스에 대해 가능한 것보다 프로그래밍 방식으로 보다 쉽게 리소스를 사용자 지정할 수 있습니다. [커뮤니티 구성 요소 추가 또는 포함](scf.md#add-or-include-a-communities-component)을 참조하십시오.

일부 커뮤니티 구성 요소만 포함할 수 있습니다. AEM 6.1의 경우, 포함할 수 있는 사람은 [댓글](essentials-comments.md), [등급](rating-basics.md), [평가](reviews-basics.md) 및 [투표](essentials-voting.md)입니다.

이 도우미는 서버측에만 적합하며 JSP 스크립트용 [cq:include](../../help/sites-developing/taglib.md)와 유사한 기능을 제공합니다.
...
* **컨텍스트**:문자열 또는 개체
   ...
   현재 컨텍스트를 전달하려면 `this`을 사용합니다.

   `this.id`을(를) 사용하여 `id`에서 요청된 resourceType을 렌더링하기 위한 리소스를 가져옵니다.

* **resourceType**:문자열

   (선택 사항) 리소스 유형은 컨텍스트의 리소스 유형에 기본적으로 사용됩니다.
...
여기에는 `this.id` + /comments의 새 주석 구성 요소가 포함됩니다.
...
이 도우미는 서버측에만 적합하며 JSP 스크립트용 [ui:includeClientLib](../../help/sites-developing/taglib.md)와 유사한 기능을 제공합니다.
...
* Handlebars 도우미 호출은 간단한 식별자(helper의 *이름)로 0개 이상의 공간 구분 매개 변수를 차례로 사용합니다.
...
* 현재 컨텍스트인 `this`은 항상 Handlebars 도우미에게 사용할 수 있습니다.
...
### Handlebars.js 문서에서 제공하는 간단한 도우미 예:{#an-example-of-a-simple-helper-from-handlebars-js-documentation}
...
### 서버측 사용자 지정 도움말 도우미 {#server-side-custom-helpers}

서버측에서 사용자 지정 SCF 도우미를 구현하고 등록하려면 Java 인터페이스 [TemplateHelper](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/cq/social/handlebars/api/TemplateHelper.html)을 구현하고 [OSGi 서비스](../../help/sites-developing/the-basics.md#osgi)로 만들고 OSGi 번들의 일부로 설치합니다.
