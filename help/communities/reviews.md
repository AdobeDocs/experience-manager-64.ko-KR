...
[필수 클라이언트측 라이브러리](reviews-basics.md#essentials-for-client-side)가 포함될 때 `Reviews`구성 요소가 표시되는 방식입니다.
...
* **[!UICONTROL 답글]**
허용 확인란을 선택하면 답글을 검토에 허용합니다. 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 닫힘]**
이 선택되면 검토 내용이 새 검토 및 답글에 대해 닫히고, 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 파일 업로드]**
허용이 선택된 경우 검토를 위해 첨부 파일을 업로드하도록 허용합니다. 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 최대 파일]**
크기파일 업로드  **[!UICONTROL 허용]** 을 선택한 경우에만 관련됩니다. 이 필드는 업로드된 파일의 크기(바이트)를 제한합니다. 기본값은 10MB입니다.

* **[!UICONTROL 최대 메시지]**
길이텍스트 상자에 입력할 수 있는 최대 문자 수입니다. 기본값은 4096자입니다.

* **[!UICONTROL 허용되는 파일]**
형식파일 업로드  **[!UICONTROL 허용]** 을 선택한 경우에만 관련성이 있습니다. &quot;점&quot; 구분 기호가 있는 쉼표로 구분된 파일 확장자 목록입니다. 예:.jpg, .jpeg, .png, .doc, .docx, .pdf 파일 유형을 지정하면 지정되지 않은 파일 유형은 허용되지 않습니다. 모든 파일 형식을 사용할 수 있도록 기본값이 지정되지 않았습니다.

* **[!UICONTROL 리치 텍스트]**
편집기이 확인란을 선택하면 게시물에 마크업이 추가될 수 있습니다. 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 투표]**
허용이 선택된 경우 주제의 투표 기능을 포함합니다. 기본값은 선택되어 있지 않습니다.
...
* **[!UICONTROL 사전]**
중재이 선택된 경우 게시 사이트에 표시되기 전에 검토가 승인되어야 합니다. 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 검토]**
삭제이 확인란을 선택하면 검토를 게시한 구성원에게 검토 삭제 권한이 제공됩니다. 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 검토]**
거부이 확인란을 선택하면 중재자가 검토를 거부할 수 있습니다. 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 검토 닫기/다시]**
열기이 확인란을 선택하면 중재자가 검토를 닫고 다시 열 수 있습니다. 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 검토]**
플래그 지정이 확인란을 선택하면 멤버가 검토 작업을 부적절한 것으로 플래그를 지정할 수 있습니다. 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 플래그 사유]**
목록선택하는 경우 드롭다운 목록에서 해당 검토에 플래그를 지정하는 이유를 부적절한 것으로 선택할 수 있습니다. 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 사용자 지정 플래그]**
이유이 확인란을 선택하면 멤버가 검토를 부적절한 것으로 플래그를 지정한 이유를 직접 입력할 수 있습니다. 기본값은 선택되어 있지 않습니다.

* **[!UICONTROL 중재]**
임계값중재자에게 알림을 보내기 전에 구성원에 의해 검토가 플래그가 지정되는 횟수를 입력합니다. 기본값은 한 번(1)입니다.

* **[!UICONTROL 플래그]**
제한공개 보기에서 검토가 숨겨지기 전에 플래그가 지정되어야 하는 횟수를 입력합니다. 이 숫자는 **[!UICONTROL 중재 임계값]**&#x200B;보다 크거나 같아야 합니다. 기본값은 5입니다.
...
* **[!UICONTROL 주석 시스템 리소스 유형]**
   ...
   /apps에서 확장 `comments`구성 요소(주석 시스템)의 resourceType으로 이동합니다. 페이지 템플릿 [에 리소스(주석 노드)로 페이지에 추가되지 않고 기본 스크립트에 댓글 시스템(](scf.md#add-or-include-a-communities-component))이 동적으로 포함되어 있지 않으면 비워 둡니다. [{{include}} 도우미](handlebars-helpers.md#include)에 대한 자세한 내용을 읽으십시오.
