...
[필요한 클라이언트측 라이브러리가](calendar-basics-for-developers.md#essentials-for-client-side) 포함되어 있으면 구성 요소가 표시되는 `Calendar` 방식입니다.
...
번역 **[!UICONTROL 탭]** 아래에서 커뮤니티 사이트에 대한 번역이 활성화되면 특정 게시물 대신 전체 스레드(이벤트 및 댓글)를 번역하도록 번역을 설정할 수 있습니다.
...
로그인한 사용자가 커뮤니티 구성원 또는 [권한](users.md#privileged-members-group) 멤버인 경우(구성에 따라) 새 달력 이벤트 `New Event` 를 만들고 게시할 수 있습니다.
...
개발자를 위한 [달력 필수](calendar-basics-for-developers.md) 페이지에서 자세한 내용을 확인할 수 있습니다.

달력 이벤트 및 댓글을 중재하려면 사용자 생성 [컨텐츠 중재를 참조하십시오](moderate-ugc.md).

달력 이벤트 및 댓글에 태그를 지정하려면 사용자 생성 컨텐츠 [태그 지정을 참조하십시오](tag-ugc.md).

달력 이벤트 및 댓글을 번역하려면 사용자 생성 컨텐츠 [변환을 참조하십시오](translate-ugc.md).
