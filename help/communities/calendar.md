...
달력 기능은 모든 사이트 방문자 또는 로그인된 사이트 방문자(커뮤니티 구성원)에게만 달력 형식으로 커뮤니티 이벤트 정보를 제공하는 것을 지원하며, 승인된 구성원만 이벤트를 추가할 수 있습니다.

설명서의 이 섹션에서는 다음 사항에 대해 설명합니다.

* AEM 사이트에 달력 기능 추가
* 구성 `Calendar`요소에 대한 구성 설정
...
사용자가 검토할 기능과 관련된 위치와 같이 페이지에 드래그합니다.

필요한 정보를 보려면 커뮤니티 구성 [요소 기본 사항을 참조하십시오](basics.md).

필요한 [클라이언트측 라이브러리가](calendar-basics-for-developers.md#essentials-for-client-side) `Calendar` 포함되어 있으면 구성 요소가 표시되는 방식입니다.
...
설정 **[!UICONTROL 탭에서]** 태그를 달력 항목에 적용할지 여부를 지정합니다.
...
* **[!UICONTROL 조정됨]**
   ...
   이 확인란을 선택하면 달력 이벤트 및 댓글이 게시 사이트에 표시되기 전에 게시 승인이 필요합니다. 기본값은 선택 취소입니다.

* **[!UICONTROL 닫힘]**

   이 확인란을 선택하면 달력이 새 이벤트 항목 및 주석으로 닫힙니다. 기본값은 선택 취소입니다.
...
* **[!UICONTROL 허용되는 표지 이미지 유형]**
   ...
   &quot;dot&quot; 구분 기호가 있는 이미지 파일 확장명의 쉼표로 구분된 목록입니다. 기본값은 `.jpg,.jpeg,.png,.gif,.bmp`입니다.

* **[!UICONTROL 스레드된 회신 허용]**

   이 확인란을 선택하면 달력 이벤트에 게시된 댓글에 답글을 허용합니다. 기본값은 선택되어 있습니다.

* **[!UICONTROL 사용자가 댓글 및 이벤트를 삭제하도록 허용]**

   이 확인란을 선택하면 구성원이 게시한 댓글 및 달력 이벤트를 삭제할 수 있습니다. 기본값은 선택되어 있습니다.
...
* **[!UICONTROL 날짜 범위 필터]**
   ...
   달력 이벤트 목록 페이지 필터의 &quot;끝&quot; 값을 계산하기 위해 현재 날짜에 추가된 일 수를 정의합니다. 기본값은 30입니다.
...
* **[!UICONTROL 이벤트 닫기 / 다시 열기]**
   ...
   이 확인란을 선택하면 신뢰할 수 있는 멤버 중재자가 이벤트를 닫아서 추가 편집 및 주석을 추가하고 이벤트를 다시 열 수 있습니다. 기본값은 선택되어 있습니다.
...
* **[!UICONTROL 플래그 이유 목록]**
   ...
   이 확인란을 선택하면 구성원이 드롭다운 목록에서 이벤트나 댓글에 플래그를 지정하는 것이 적절하지 않은 이유를 선택할 수 있습니다. 기본값은 선택 취소입니다.

* **[!UICONTROL 사용자 지정 플래그 이유]**

   이 확인란을 선택하면 구성원이 이벤트 또는 댓글에 부적절한 이유를 입력할 수 있습니다. 기본값은 선택 취소입니다.

* **[!UICONTROL 관리 임계값]**

   중재자에게 알림을 보내기 전에 구성원에게 이벤트 또는 댓글에 플래그를 지정해야 하는 횟수를 입력합니다. 기본값은 1입니다(한 번).

* **[!UICONTROL 플래그 지정 제한]**

   이벤트나 댓글이 공개 보기에서 숨겨지기 전에 플래그로 표시되어야 하는 횟수를 입력합니다. -1로 설정하면 플래그가 지정된 주제 또는 댓글이 공개 보기에서 숨겨지지 않습니다. 그렇지 않은 경우 이 숫자는 중재 임계값보다 크거나 같아야 합니다. 기본값은 5입니다.
...
>[!NOTE]
>
>태그 [관리를](../../help/sites-administering/tags.md) 방문하여 새 태그 네임스페이스(분류)를 추가하는 방법을 알아보십시오.

#### 번역 탭 {#translation-tab}

번역 **[!UICONTROL 탭에서]** 커뮤니티 사이트에 대한 번역이 활성화되면 번역이 특정 게시물 대신 전체 스레드(이벤트 및 주석)를 번역하도록 설정될 수 있습니다.
...
게시 환경에서 달력 기능에는 기본 날짜 범위가 있는 검색 필드와 해당 범위에 속하는 달력 이벤트가 표시됩니다.
...
로그인한 사용자가 커뮤니티 멤버나 [권한 있는 멤버인](users.md#privileged-members-group) 경우(구성에 따라 다름) 새 달력 이벤트를 만들고 게시할 `New Event` 수 있습니다.
...
개발자를 위한 Calendar Essentials [페이지에](calendar-basics-for-developers.md) 자세한 내용이 나와 있습니다.

달력 이벤트 및 댓글을 중재하려면 사용자 생성 [컨텐츠 중재를 참조하십시오](moderate-ugc.md).

달력 이벤트 및 댓글에 태그를 지정하려면 사용자 생성 [컨텐츠 태깅을 참조하십시오](tag-ugc.md).

달력 이벤트 및 댓글을 번역하려면 사용자 생성 [컨텐츠 번역을 참조하십시오](translate-ugc.md).
