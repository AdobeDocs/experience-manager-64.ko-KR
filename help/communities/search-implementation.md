...
* 솔루션은 표준 검색 또는 다국어 검색(MLS)을 위해 구성할 수 있습니다.
...
AEM 플랫폼 검색을 위해 Oak 색인이 자동으로 만들어지지 않지만 AEM 6.2부터는 AEM Communities에 대한 기능이 추가되어 성능이 향상되고 UGC 검색 결과를 표시할 때 페이지 매김에 대한 지원이 제공됩니다.

사용자 지정 속성이 사용 중이고 검색이 느린 경우 사용자 지정 속성에 대해 추가 인덱스를 만들어 더 많은 성능을 구현해야 합니다. 이식성을 유지하려면 검색 가능한 사용자 정의 속성을 만들 때 [이름 지정 요구 사항](#naming-of-custom-properties)을 따릅니다.

기존 인덱스를 수정하거나 사용자 지정 인덱스를 만들려면 [Oak 쿼리 및 인덱싱](../../help/sites-deploying/queries-and-indexing.md)을 참조하십시오.
...
[CRXDE Lite](../../help/sites-developing/developing-with-crxde-lite.md)에서 기존 Oak 색인을 보려면 위치는 다음과 같습니다.
...
사용자 지정 속성을 추가할 때 [UGC 검색 API](#ugc-search-api)로 만든 정렬 및 검색에 이러한 속성을 표시하려면 속성 이름에 접미어를 추가해야 합니다. *가 필요합니다.
