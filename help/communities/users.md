...
AEM Communities의 게시 환경에서 사용자는 자신의 프로필을 직접 등록하고 편집할 수 있습니다. 적절한 권한이 주어지면,

* 커뮤니티 사이트 내에서 하위 커뮤니티 만들기( [커뮤니티 그룹 참조](creating-groups.md))
* [사용자 생성](moderation.md) 컨텐츠(UGC) 중재
* 역량 [강화 리소스](resources.md) 연락처
* 블로그, 달력, QnA 및 포럼 [에 대한 항목을 만들 수 있는 권한](#privileged-members-group) 부여

게시 환경에 등록된 사용자는 일반적으로 *커뮤니티 구성원(구성원)* (작성자 환경에서 *사용자와 구별하기 위한 구성원)입니다. *사용자
...
보안 체크리스트를 [따르십시오](../../help/sites-administering/security-checklist.md).

## 게시 환경 {#publish-environment}

### 회원이 되기 {#becoming-a-member}

게시 환경에서 커뮤니티 사이트의 [설정에](sites-console.md#user-management) 따라 사이트 방문자가 커뮤니티 멤버가 될 수 있습니다
...
* [블로그 함수](functions.md#blog-function) - 새 집필 만들기 제한
* [달력 함수](functions.md#calendar-function) - 새 이벤트 만들기 제한
* [포럼 함수](functions.md#forum-function) - 새 항목 만들기 제한
...
2. 오른쪽 창 `community-enablementmanagers` 에 표시된 새 사용자의 **[!UICONTROL 그룹]** 탭으로 이동하여 드래그합니다.
...
사용자 동기화를 통한 사용자 그룹 동기화는 향후 릴리스에 포함될 예정이지만, 현재 사용자 동기화가 실행되면 사용자 그룹의 *멤버십만 동기화됩니다.
...
* **[구성원 그룹](../../help/sites-administering/security.md)**은 구성원이 소속되고 그룹 콘솔을 통해 관리되는 그룹입니다. 이 페이지에 대한 논의의 상당 부분이 구성원 그룹에 할애되었습니다. 커뮤니티 사이트에 대해 자동으로 생성되는 멤버 그룹을 커뮤니티 그룹이라고&#x200B;*`Community`*할 수 있으므로 토론의 컨텍스트를 고려해야 합니다.
