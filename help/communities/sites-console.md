...
자세한 내용은 [AEM Communities 시작하기](getting-started.md) 작성 환경에서 커뮤니티 사이트를 만들 수 있는 속도 및 작성 및 게시 환경에서 커뮤니티 그룹을 만드는 방법을 경험하려면 다음을 수행하십시오.

>[!NOTE]
>
>기본 Communities 메뉴로서 [커뮤니티 사이트](sites-console.md), [커뮤니티 사이트 템플릿](sites.md), [커뮤니티 그룹 템플릿](tools-groups.md) 및 [커뮤니티 기능](functions.md) 작성 환경에서만 사용할 수 있습니다.
...
* 를 활성화합니다 [터널 서비스](deploy-communities.md#tunnel-service-on-author) 구성원 및 구성원 그룹을 관리합니다.
* 다음 식별 [기본 게시자](deploy-communities.md#primary-publisher)
* [복제 구성](deploy-communities.md#replication-agents-on-author) 기본 게시자 포트가 기본값이 아닌 경우(4503)

사이트에서 많은 기능을 지원하도록 준비되는 우수 사례는 다음과 같습니다.

* 설치 [최신 기능 팩](deploy-communities.md#latestfeaturepack)
* 활성화 [Adobe Analytics](analytics.md) AEM Communities
* 구성 [이메일](email.md)
* 식별 [커뮤니티 관리자](users.md#creating-community-members)
* [OAuth 처리기 활성화](social-login.md#adobe-granite-oauth-authentication-handler) 소셜 로그인
...
예를 들어 다음 이미지는 두 커뮤니티 사이트에 대한 폴더를 표시하는 기본 커뮤니티 사이트 콘솔을 보여줍니다. [활성화](getting-started-enablement.md) 및 [참여](getting-started.md):

![chlimage_1-448](assets/chlimage_1-448.png)

## 사이트 생성 {#site-creation}

사이트 작성 콘솔에서는 선택한 사이트에 따라 사이트의 기능을 취합하는 단계별 방법을 제공합니다 [커뮤니티 사이트 템플릿](sites.md) 및 설정을 지정합니다.
...
* **[!UICONTROL 커뮤니티 사이트 템플릿]** 메뉴: 풀다운 메뉴를 사용하여 사용 가능한 메뉴를 선택합니다 [커뮤니티 사이트 템플릿](tools.md).
...
>
>이 규칙은 게시 측을 위한 것입니다 [사용자 및 사용자 그룹](users.md) (구성원 및 구성원 그룹) 작성자 환경에서 복제되지 않습니다.
>
>따라서 작성 환경에서 커뮤니티 사이트를 만들고 신뢰할 수 있는 구성원을 다양한 역할에 할당할 때 게시 환경에서 멤버 데이터를 검색해야 합니다.
>
>이 작업은 를 활성화하여 수행됩니다 ` [AEM Communities Publish Tunnel Service](deploy-communities.md#tunnel-service-on-author)`작성 환경에 사용할 수 있습니다.
...
>[!NOTE]
>
>권장 사항 [지원 커뮤니티 사이트](overview.md#enablement-community) 비공개(자세한 내용은 계정 담당자에게 문의).
>
...
* **[!UICONTROL 익명 액세스 허용]**
   ...
   이 옵션을 선택하면 커뮤니티 사이트가 *open* 그리고 모든 사이트 방문자가 사이트에 액세스할 수 있습니다.
...
* **[!UICONTROL 소셜 로그인 허용: Facebook]**
   ...
   이 필드를 선택하면 사이트 방문자가 Facebook 계정 자격 증명으로 로그인할 수 있도록 허용합니다. 선택한 항목 [Facebook 클라우드 구성](social-login.md#create-a-facebook-connect-cloud-service) 커뮤니티 사이트가 만들어지면 커뮤니티 사이트의 구성원 그룹에 사용자를 추가하도록 구성해야 합니다.
...
* **[!UICONTROL 소셜 로그인 허용: Twitter]**
   ...
   이 필드를 선택하면 사이트 방문자가 Twitter 계정 자격 증명으로 로그인할 수 있도록 허용합니다. 선택한 항목 [Twitter 클라우드 구성](social-login.md#create-a-twitter-connect-cloud-service) 커뮤니티 사이트가 만들어지면 커뮤니티 사이트의 구성원 그룹에 사용자를 추가하도록 구성해야 합니다.
...
>
>샘플 Facebook 및 Twitter 구성이 존재하고 선택할 수 있지만, [프로덕션 환경](../../help/sites-administering/production-ready.md)를 지정하는 경우 사용자 지정 Facebook 및 Twitter 애플리케이션을 만들어야 합니다. 자세한 내용은 [facebook 및 Twitter을 사용하여 소셜 로그인](social-login.md).
...
또한 커뮤니티 사이트에 대해 태그 네임스페이스를 선택하면 카탈로그 및 리소스를 정의할 때 표시되는 선택 사항이 제한됩니다. 자세한 내용은 [태깅 지원 리소스](tag-resources.md) 참조하십시오.
...
다음 [커뮤니티 구성원의 역할](users.md) 이러한 설정에 할당됩니다.
...
* **[!UICONTROL 커뮤니티 권한이 있는 구성원]**
   ...
   입력을 시작하여 하나 이상의 커뮤니티 구성원 또는 구성원 그룹을 선택하여 새 콘텐츠를 작성할 수 있습니다. `Allow Privileged Member` 이(가) [커뮤니티 기능](functions.md).
...
* **[!UICONTROL 컨텐츠가 미리 중재되었음]**
   ...
   이 확인란을 선택하면 중재자가 승인하기 전까지 게시된 커뮤니티 콘텐츠가 표시되지 않습니다. 기본값은 선택 취소되어 있습니다. 자세한 내용은 [커뮤니티 콘텐츠 중재](moderate-ugc.md#premoderation).
...
* **[!UICONTROL Analytics 활성화]**
   ...
   Adobe Analytics이 [구성](analytics.md) Communities용.
...
* **[!UICONTROL Cloud 구성 프레임워크 참조]**
   ...
   `Communities`는 의 프레임워크 예입니다. [커뮤니티 기능에 대한 Analytics 구성](analytics.md#aem-analytics-framework-configuration) 설명서.
...
* **[!UICONTROL 기계 번역 허용]**
이 옵션을 선택하면(기본값 선택 안 함) 사이트 내에서 UGC에 대해 기계 번역이 활성화됩니다. 사이트가 다국어 사이트로 설정된 경우에도 페이지 콘텐츠와 같은 다른 콘텐츠에 영향을 주지 않습니다. 자세한 내용은 [사용자가 생성한 컨텐츠 번역](translate-ugc.md) AEM Communities에 대한 라이선스 번역 서비스 구성에 대한 정보입니다. 자세한 내용은 [다국어 사이트의 컨텐츠 번역](../../help/sites-administering/translation.md) 를 참조하십시오.

![chlimage_1-456](assets/chlimage_1-456.png)

* **[!UICONTROL 선택한 언어에 대한 기계 번역 사용]**

   기계 번역에 대해 활성화된 언어는 기본적으로 [번역 통합 구성](translate-ugc.md#translation-integration-configuration). 이러한 기본 설정은 기본값을 삭제하거나 풀다운 메뉴에서 다른 언어를 선택하여 이 사이트에 대해 대체할 수 있습니다.
...
다음 `ENABLEMENT`선택한 커뮤니티 사이트 템플릿에 [지정 함수](functions.md#assignments-function)- 사용 가능 기능에 라이센스가 있을 때 사용할 수 있습니다. [구성](enablement.md). 할당 기능을 포함하는 참조 사이트 템플릿은 다음과 같습니다 `Reference Structured Learning Site Template.`

* **[!UICONTROL 지원 관리자]**

   (필수) `Community Enablementmanagers` 그룹을 선택하여 이 지원 커뮤니티를 관리할 수 있습니다. 지원 관리자는 리소스에 구성원을 지정할 책임이 있습니다. 참조 - [사용자 및 사용자 그룹 관리](users.md).

* **[!UICONTROL Marketing Cloud 조직 ID]**

   (선택 사항) [비디오 하트비트 분석](analytics.md#video-heartbeat-analytics) 라이센스.
...
* 을 편집할 수 있습니다 [구조](#modify-structure) 속성을 수정하여 커뮤니티 사이트의 속성
...
* 추가 끌어서 놓기 [커뮤니티 기능](functions.md) 사이트 구조에
* 사이트 구조의 커뮤니티 함수 인스턴스에서 다음을 수행합니다.

   * **`gear icon`**

      표시 제목 및 URL name&amp;ast;를 포함하여 설정을 편집합니다.

      뿐만 아니라 [권한 있는 구성원 그룹](users.md#privilegedmembersgroups)
...
커뮤니티 사이트를 게시한 후에는 다음을 사용하여 만든 각 하위 커뮤니티(중첩된 그룹)를 개별적으로 게시해야 합니다 [그룹 콘솔](groups.md).
...
>
>* 커뮤니티 -*&lt;site-name>*-*&lt;uid>*-members
>
이(가) 생성되면 적용됨 [Facebook 클라우드 서비스](social-login.md#createafacebookcloudservice) 사용자를 이 그룹에 추가하도록 구성해야 합니다.
...
* 을(를) 선택합니다 `pencil`아이콘 을 클릭하여 편집할 구성을 엽니다.
* 을(를) 입력합니다. **[!UICONTROL 로그인 페이지 매핑]** 아래와 같이 변경하는 것을 의미합니다.
