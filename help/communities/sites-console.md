...
* 사이트 편집
* 사이트 관리
* [중첩된 그룹 만들기 및 편집](groups.md) (하위 커뮤니티)

작성 환경에서 커뮤니티 사이트를 얼마나 신속하게 만들 수 있는지, 작성 및 게시 환경에서 커뮤니티 그룹을 만드는 방법을 경험하려면 [AEM Communities 시작하기](getting-started.md)를 참조하십시오.

>[!NOTE]
>
>[커뮤니티 사이트](sites-console.md), [커뮤니티 사이트 템플릿](sites.md), [커뮤니티 그룹 템플릿](tools-groups.md) 및 [커뮤니티 기능](functions.md)을 만들기 위한 기본 커뮤니티 메뉴는 작성 환경에서만 사용할 수 있습니다.
...
* 구성원 및 구성원 그룹을 관리하려면 [터널 서비스](deploy-communities.md#tunnel-service-on-author) 사용
* [기본 게시자 식별](deploy-communities.md#primary-publisher)
* [기본 ](deploy-communities.md#replication-agents-on-author) 게시자 포트가 기본값이 아닌 경우 복제 구성(4503)

사이트가 많은 기능을 지원할 수 있도록 준비되도록 하려면 다음 단계를 수행하는 것이 좋습니다.

* [최신 기능 팩](deploy-communities.md#latestfeaturepack) 설치
* AEM Communities에 대해 [Adobe Analytics](analytics.md) 사용
* [이메일](email.md) 구성
* [커뮤니티 관리자 식별](users.md#creating-community-members)
* [소셜 로그인](social-login.md#adobe-granite-oauth-authentication-handler) 에 대한 OAuth 핸들러 사용
...
사이트 만들기 콘솔은 선택한 [커뮤니티 사이트 템플릿](sites.md) 및 설정을 기반으로 사이트의 기능을 조합하는 단계별 접근 방식을 제공합니다.
...
>
>게시 측 [사용자 및 사용자 그룹](users.md)(구성원 및 구성원 그룹)은 작성 환경에서 중복되지 않도록 합니다.
>
...
>[!NOTE]
>
>[역량 강화 커뮤니티 사이트](overview.md#enablement-community)는 비공개로 유지하는 것이 좋습니다(자세한 내용은 계정 담당자에게 문의).
>
...
* **[!UICONTROL 익명 액세스 허용]**
   ...
   이 확인란을 선택하면 커뮤니티 사이트가 *open*&#x200B;이고 사이트 방문자가 사이트에 액세스할 수 있습니다.
...
* **[!UICONTROL 소셜 로그인 허용: Facebook]**
   ...
   이 확인란을 선택하면 사이트 방문자가 Facebook 계정 자격 증명으로 로그인하도록 허용합니다. 커뮤니티 사이트가 만들어지면 선택한 [Facebook 클라우드 구성](social-login.md#create-a-facebook-connect-cloud-service)에 사용자를 커뮤니티 사이트의 구성원 그룹에 추가하도록 구성해야 합니다.
...
* **[!UICONTROL 소셜 로그인 허용: Twitter]**
   ...
   이 확인란을 선택하면 사이트 방문자가 Twitter 계정 자격 증명으로 로그인하도록 허용합니다. 커뮤니티 사이트가 만들어지면 선택한 [Twitter 클라우드 구성](social-login.md#create-a-twitter-connect-cloud-service)에 사용자를 커뮤니티 사이트의 구성원 그룹에 추가하도록 구성해야 합니다.
...
>**[!UICONTROL 소셜 로그인 허용]**
>
>샘플 Facebook 및 Twitter 구성이 존재하고 선택할 수 있지만 [제작 환경](../../help/sites-administering/production-ready.md)의 경우 사용자 지정 Facebook 및 Twitter 애플리케이션을 만들어야 합니다. [Facebook 및 Twitter와 소셜 로그인](social-login.md)을 참조하십시오.
...
또한 커뮤니티 사이트에 대한 태그 네임스페이스를 선택하면 카탈로그와 리소스를 정의할 때 표시되는 선택 영역이 제한됩니다. 중요한 정보는 [Tagging Enablement Resources](tag-resources.md)을 참조하십시오.

* 텍스트 검색 상자:사이트에서 사용할 수 있는 태그를 식별하기 위한 입력 시작

#### 역할 {#roles}

![chlimage_1-451](assets/chlimage_1-451.png)

커뮤니티 구성원](users.md)의 [역할이 이러한 설정과 함께 할당됩니다.
...
* **[!UICONTROL 커뮤니티 권한이 있는 구성원]**
   ...
   [커뮤니티 함수](functions.md)에 대해 `Allow Privileged Member`이(가) 선택되었을 때 새 컨텐츠를 만들 수 있는 기능을 제공할 커뮤니티 구성원 또는 구성원 그룹을 하나 이상 선택하려면 입력을 시작합니다.
...
* **[!UICONTROL 컨텐츠가 미리 중재되었음]**
   ...
   이 확인란을 선택하면 사회자가 승인하기 전까지 게시된 커뮤니티 컨텐츠가 나타나지 않습니다. 기본값은 선택되어 있지 않습니다. 자세한 내용은 [커뮤니티 콘텐츠 중재](moderate-ugc.md#premoderation)를 참조하십시오.
...
* **[!UICONTROL Analytics 활성화]**
   ...
   Adobe Analytics이 커뮤니티 기능에 대해 [구성된](analytics.md)인 경우에만 사용할 수 있습니다.
...
* **[!UICONTROL 기계]**
번역 허용이 선택되면(기본값은 선택 취소됨) 사이트 내 UGC에 대해 기계 번역이 활성화됩니다. 사이트가 다국어 사이트로 설정된 경우에도 페이지 컨텐츠와 같은 다른 컨텐트에는 영향을 주지 않습니다. AEM Communities에 대한 라이센스 번역 서비스를 구성하는 방법은 [사용자 생성 컨텐트 번역](translate-ugc.md)을 참조하십시오. 전체 개요는 [다국어 사이트의 콘텐츠 번역](../../help/sites-administering/translation.md)을 참조하십시오.

![chlimage_1-456](assets/chlimage_1-456.png)

* **[!UICONTROL 선택한 언어에 대한 기계 번역 사용]**

   기계 번역에 대해 활성화된 언어는 기본적으로 [번역 통합 구성](translate-ugc.md#translation-integration-configuration)에 의해 지정된 시스템 설정으로 지정됩니다. 기본값을 삭제하고 풀다운 메뉴에서 다른 언어를 선택하여 이 사이트에 대해 이러한 기본 설정을 재정의할 수 있습니다.
...
* **지속성 옵션 선택**
   ...
   * **[!UICONTROL 사용자 요청에 대한 기여도를 번역한 후 유지]**
      ...
      이 옵션을 선택하면 컨텐츠가 요청이 이루어질 때까지 번역되지 않습니다. 번역 후 번역이 저장소에 저장됩니다.
...
`ENABLEMENT`설정은 선택한 커뮤니티 사이트 템플릿에 [assignments 함수](functions.md#assignments-function)가 포함될 때 적용되며, 이 함수는 활성 기능에 라이선스가 부여되고 [configured](enablement.md)가 있을 때 사용할 수 있습니다. 할당 기능을 포함하는 참조 사이트 템플릿은 `Reference Structured Learning Site Template.`입니다.

* **[!UICONTROL 지원 관리자]**

   (필수) 이 지원 커뮤니티를 관리하기 위해 `Community Enablementmanagers` 그룹의 구성원만 선택할 수 있습니다. 역량 강화 관리자는 리소스에 구성원을 할당할 책임이 있습니다. [사용자 및 사용자 그룹 관리](users.md)도 참조하십시오.

* **[!UICONTROL Marketing Cloud 조직 ID]**

   (선택 사항) [비디오 하트비트 분석](analytics.md#video-heartbeat-analytics) 라이센스의 ID.
...
구조 패널을 사용하면 선택한 커뮤니티 사이트 템플릿에서 처음 만든 구조를 수정할 수 있습니다. 패널에서

* 추가 [커뮤니티 함수](functions.md)를 사이트 구조에 드래그하여 놓습니다.
* 사이트 구조에서 커뮤니티 함수 인스턴스:

   * **`gear icon`**

      표시 제목 및 URL 이름/amp;ast;를 비롯한 설정 편집;
...
가 만들어지면, 적용된 [Facebook 클라우드 서비스](social-login.md#createafacebookcloudservice)를 구성하여 이 그룹에 사용자를 추가해야 합니다.
