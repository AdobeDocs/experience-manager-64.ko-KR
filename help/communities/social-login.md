...
1. 편집할 연필 아이콘을 선택합니다.
   ...
   * **[!UICONTROL 사용자]**&#x200B;업데이트 이 확인란을 선택하면 프로필 변경 또는 요청된 추가 데이터를 반영하도록 각 로그인 시 저장소의 사용자 데이터를 새로 고칩니다. 기본값은 선택 취소입니다.
...
1. *설명* 입력.
1. 웹 *사이트* - https://&lt;server>/를 입력합니다.
1. 콜백 *URL* - https://&lt;server>/를 입력합니다.
...
1. 전역 탐색에서 **[!UICONTROL 도구 > Cloud Services > Twitter 소셜 로그인 구성을 선택합니다]**.
...
1. Twitter 클라우드 서비스 구성 만들기/편집을 참조하십시오.
   ...
   * **[!UICONTROL 제목]** (*필수*) Twitter 앱을 식별하는 표시 제목을 입력합니다. Twitter 앱에 대해 *표시 이름과* 동일한 이름을 사용하는 것이 좋습니다.

   * **[!UICONTROL 소비자 키]** (*필수*) **Twitter 앱용** 소비자(API) 키를입력합니다. 대화 상자에서 만든 [Adobe [OAuth 응용 프로그램 및 공급자](https://helpx.adobe.com/experience-manager/6-3/communities/using/social-login.html#AdobeGraniteOAuthApplicationandProvider) 인스턴스]를 식별합니다.

   * **[!UICONTROL 소비자 비밀]** (*필수****)*** Twitter 앱용소비자(API) 암호를입력합니다.

   * **[!UICONTROL 사용자]** 만들기 이 확인란을 선택하면 Twitter 계정으로 로그인하여 AEM 사용자 항목을 만들고 선택한 사용자 그룹에 구성원으로 추가합니다. 기본값이 선택됨(적극 권장)

   * **[!UICONTROL 마스크 사용자 ID는]** 선택 해제됩니다.
...
* 모든 게시 인스턴스에서 [Adobe [OAuth 인증 처리기](#adobe-granite-oauth-authentication-handler) ]가 활성화되었는지 확인
* 클라우드 서비스가 게시되었는지 확인
* 커뮤니티 사이트가 게시되었는지 확인
...
* 관리자 권한으로 로그인
* 웹 콘솔 [로](../../help/sites-deploying/configuring-osgi.md)이동합니다(예: http://localhost:4503/system/console/configMgr).
...
1. [MOCK] Locate Adobe Granite OAuth Application and Provider.
   ...
   * 클라이언트 ID가 **[!UICONTROL 앱 ID와]** 일치하는 **[!UICONTROL 인스턴스를 찾습니다.]**
...
1. ntBaseLucene 사본에 ntBaseLucene-oauth라는 이름의 인덱스를 만듭니다.
   ...
   * 선택 `/oak:index`
   * 붙여넣기 **[!UICONTROL 선택]**
   * ntBaseLucene의 복사본 이름을 ntBaseLucene-oauth로 변경
