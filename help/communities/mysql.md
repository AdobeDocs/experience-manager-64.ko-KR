...
* 기본 포트는 `3306`
* 선택한 `Connection Name` 항목이 JDBC OSGi `datasource` 구성에 [이름으로 입력됩니다.](#configure-jdbc-connections)
...
* sql 파일의 `jcr:content`노드 선택
* 속성 값이 보기 `jcr:data`링크임을 확인하십시오.
...
스크립트가 실행되면 새 데이터베이스를 보기 위해 `SCHEMAS`섹션 `Navigator` 을 새로 고쳐야 합니다. &#39;SCHEMAS&#39; 오른쪽에 새로 고침 아이콘을 사용합니다.
