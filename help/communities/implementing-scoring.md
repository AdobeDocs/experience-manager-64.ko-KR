...
* [배지 할당](#assign-and-revoke-badges) 커뮤니티에서 구성원의 역할을 식별하려면

* [기본 배지 훈장](#enable-scoring) 참여도를 유도하려면(컨텐츠 생성량)
* [고급 배지 훈장](advanced.md) 구성원을 전문가로 식별하려면(컨텐츠 작성 품질)

**참고** 그 배지는 [기본적으로 활성화되지 않음](implementing-scoring.md#main-pars-text-237875536).
...
커뮤니티 [배지 콘솔](badges.md) 은(는) 획득할 때(수상) 또는 커뮤니티에서 특정 역할을 수행할 때(지정된) 회원에게 표시할 수 있는 사용자 지정 배지를 추가할 수 있는 기능을 제공합니다.

### 지정된 배지 {#assigned-badges}

역할 기반 배지는 관리자가 커뮤니티에서 자신의 역할에 따라 커뮤니티 구성원에게 지정합니다.

지정된(및 꺼진) 배지는 선택한 위치에 저장됩니다 [SRP](srp.md) 및에 직접 액세스할 수 없습니다. GUI를 사용할 수 있을 때까지 역할 기반 배지를 할당하는 유일한 방법은 코드나 cURL을 사용하여 할당하는 것입니다. cURL 지침은 [배지 할당 및 취소](#assign-and-revoke-badges).
...
* 배지가 있어야 합니다. [활성화됨](#enable-badges-for-component) 기능 구성 요소의 경우
* 점수부여 및 배지 규칙은 [적용됨](#apply-rules-to-content) 구성 요소가 배치된 페이지(또는 상위 페이지)에
...
사용자 지정 배지는 [배지 콘솔](badges.md) 및 을 배지 규칙에 할당하거나 지정합니다.
...
점수부여 및 배지 규칙은 구성 요소의에서 구성 요소 구성을 편집하여 배지를 활성화한 구성 요소의 인스턴스에만 적용됩니다 [작성 모드](author-communities.md).

부울 속성, `allowBadges`에서는 구성 요소 인스턴스에 대한 배지 표시를 활성화/비활성화합니다. 구성 가능한 템플릿은 [구성 요소 편집 대화 상자](author-communities.md) 포럼, QnA 및 아래에 레이블이 지정된 확인란을 통해 구성 요소 설명 **배지 표시**.
   ...
   <td> 
    <ul> 
     ...
     <li>"advanced"이면 품질 및 수량을 기준으로 점수 엔진을 지정합니다 
      <ul> 
       <li>를 사용하려면 <a href="advanced.md">추가 패키지</a></li> 
      </ul> </li> 
     <li>기본값은 "기본"입니다.</li> 
    </ul> </td> 
  </tr> 
...
릴리스에 포함된 값은 [포럼 함수](functions.md#forum-function) (포럼 기능의 포럼 및 주석 구성 요소에 대해 각각 하나씩):
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>badgingType</td> 
   <td>문자열</td> 
   <td><em>(선택 사항)</em> 점수 엔진을 "기본" 또는 "고급"으로 식별합니다. 고급 점수 엔진이 필요한 경우 다음을 참조하십시오 <a href="advanced.md">고급 점수 및 배지</a>. 기본값은 "기본"입니다.</td> 
  </tr> 
  <tr> 
   <td> 
...
배지는 [구성원 콘솔](members.md#badges-tab) 프로그래밍 방식으로 cURL 명령을 사용할 수도 있습니다.
...
>
>* 다음 경우에 작성자 인스턴스를 참조할 수 있습니다. [터널 서비스](users.md#tunnel-service) 이 활성화되어 있습니다.
>* 알 수 없는 임의 이름일 수 있습니다. [Security 검사 목록](../../help/sites-administering/security-checklist.md#verify-that-you-are-not-disclosing-personally-identifiable-information-in-the-users-home-path) 인증 가능한 ID와 관련하여
>
...
>[!NOTE]
>
>새 부울 속성, `allowBadges`에서는 구성 요소 인스턴스에 대한 배지 표시를 활성화/비활성화합니다. 업데이트 시 구성할 수 있습니다 [구성 요소 편집 대화 상자](author-communities.md) 확인란을 통해 **배지 표시**.

**[달력 구성 요소](calendar.md)**
...
**[파일 라이브러리 구성 요소](file-library.md)**
...
점수를 매기고 배지를 할 때는 [시작하기 자습서](getting-started.md) (참여) 사이트:
...
* badgingRules 속성을 추가합니다.
   ...
   * 선택 **[!UICONTROL 다중]**
   * 선택 **[!UICONTROL 추가]**
   * Enter 키 `/etc/community/badging/rules/forums-badging`
   * 선택 `+`
   * Enter 키 `/etc/community/badging/rules/comments-badging`
   * 선택 **[!UICONTROL 확인]**
...
* scoringRules 등록 정보를 추가합니다.
   ...
   * 선택 **[!UICONTROL 다중]**
   * 선택 **[!UICONTROL 추가]**
   * Enter 키 `/etc/community/scoring/rules/forums-scoring`
   * 선택 `+`
   * Enter 키 `/etc/community/scoring/rules/comments-scoring`
   * 선택 **[!UICONTROL 확인]**
...
다음, [다시 게시](sites-console.md#publishing-the-site) 커뮤니티 사이트입니다.
...
자세한 내용은 [점수 책정 및 배지 핵심 사항](configure-scoring.md) 개발자를 위한 페이지입니다.

고급 점수 엔진에 대한 자세한 내용은 [고급 점수 및 배지](advanced.md).

구성 가능한 리드보드 [구성 요소](enabling-leaderboard.md) 및 [함수](functions.md#leaderboard-function) 커뮤니티 사이트에서 구성원 및 해당 점수를 간편하게 표시할 수 있습니다.
