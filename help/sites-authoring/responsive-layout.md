...
description: AEM에서 페이지에 대한 응답형 레이아웃을 실현할 수 있습니다
...
AEM을 사용하면 **레이아웃 컨테이너** 구성 요소.

이 레이아웃은 응답형 격자 내에 구성 요소를 배치할 수 있도록 해 주는 단락 시스템을 제공합니다. 이 격자를 사용하면 디바이스/창 크기 및 형식에 따라 레이아웃을 다시 정렬할 수 있습니다. 구성 요소는 [**레이아웃** 모드](/help/sites-authoring/responsive-layout.md#defining-layouts-layout-mode)와 함께 사용됩니다. 이 모드에서는 디바이스에 종속적인 응답형 레이아웃을 만들고 편집할 수 있습니다.

레이아웃 컨테이너:

* 구성 요소를 격자에 나란히 배치하고 언제 구성 요소가 축소되거나 리플로우되어야 하는지 정의하는 기능과 함께 수평 격자에 맞춤 기능을 제공합니다.
* 미리 정의된 중단점(예: 전화, 태블릿 등의 경우)을 사용합니다. 관련 디바이스/방향에 대한 필수 동작을 정의할 수 있습니다.

   * 예를 들어 구성 요소 크기나 구성 요소를 특정 디바이스에서 볼 수 있도록 할지 여부를 사용자 정의할 수 있습니다.

* 열 컨트롤을 허용하도록 중첩할 수 있습니다.
...
* [**레이아웃 컨테이너**](#adding-a-layout-container-and-its-content-edit-mode) 구성 요소
   ...
   이 구성 요소는 [구성 요소 브라우저](/help/sites-authoring/author-environment-tools.md#components-browser)에서 사용할 수 있으며, 응답형 그리드 내에 구성 요소를 추가 및 배치할 수 있도록 해 주는 그리드 단락 시스템을 제공합니다. 또한 페이지에서 기본 단락 시스템으로 설정할 수도 있습니다.
...
>[!NOTE]
>
>위의 메커니즘은 템플릿에 대한 구성으로 사용할 수 있게 됩니다. 자세한 내용은 [응답형 레이아웃 구성](/help/sites-administering/configuring-responsive-layout.md) 추가 정보.
...
에뮬레이터를 사용하여 에뮬레이션하고 레이아웃을 정의할 특정 디바이스를 선택하면 관련 중단점도 강조 표시됩니다. 수행하는 모든 레이아웃 변경 작업은 중단점이 적용되는 다른 디바이스(즉, 활성 중단점 마커의 왼쪽에, 하지만 다음 중단점 마커의 앞에 배치된 모든 디바이스)에 적용할 수 있게 됩니다.

예를 들어 에뮬레이션하고 레이아웃을 정의할 디바이스로 **iPhone 6 Plus**(너비 540픽셀로 정의됨)를 선택하면 중단점 **Phone**(768픽셀로 정의됨)도 활성화됩니다. **iPhone 6**&#x200B;에 대해 수행하는 모든 레이아웃 변경 작업은 **iPhone 5**(320픽셀로 정의됨)와 같은 **Phones** 중단점 아래의 다른 디바이스에 적용됩니다.
...
1. 특정 디바이스를 선택하면 다음 작업을 수행할 수 있습니다.
   ...
   * 파란색 점선은 *접힌 부분* 선택한 장치(여기서 **iPhone 6**).
...
>[!NOTE]
>
>아직 사용할 수 없는 경우 **레이아웃 컨테이너** 는 명시적으로 지정해야 합니다 [단락 시스템/페이지에 대해 활성화됨](/help/sites-administering/configuring-responsive-layout.md) (예: [**디자인** 모드](/help/sites-authoring/default-components-designmode.md)).
...
1. 레이아웃 구성 요소가 중첩된 경우 **상위** 옵션에는 중첩된 레이아웃 컨테이너 또는 상위 항목을 선택할 수 있는 드롭다운 선택 사항이 표시됩니다.

   드롭다운에서 컨테이너 이름 위로 마우스를 가져가면 해당 윤곽이 페이지에 표시됩니다.

   * 가장 낮게 중첩된 레이아웃 컨테이너는 검정색으로 윤곽이 표시됩니다.
   * 다음으로 낮게 중첩된 레이아웃 컨테이너는 어두운 회색으로 표시됩니다.
...
* When [개별 구성 요소 편집.](/help/sites-authoring/editing-content.md#edit-component-layout)

   * 를 사용하여 **레이아웃** 구성 요소의 빠른 작업 메뉴에서 옵션을 **레이아웃** 모드.
   * **레이아웃** 구성 요소를 편집하는 동안 모드가 지속되며 로 되돌아갑니다. **편집** 포커스가 다른 구성 요소로 변경되면 모드 로 이동합니다.

레이아웃 모드에 있는 경우 격자에 대해 다양한 작업을 수행할 수 있습니다.
...
* **레이아웃** 모드에서 **구성 요소를 여기로 드래그하십시오.**&#x200B;를 탭/클릭하여 전체 구성 요소를 선택할 수 있습니다. 그러면 이 모드에 대한 도구 모음이 표시됩니다.
   ...
   도구 모음에는 레이아웃 구성 요소의 상태 및 도구 모음에 속하는 구성 요소에 따라 다른 옵션이 있습니다. 예:
