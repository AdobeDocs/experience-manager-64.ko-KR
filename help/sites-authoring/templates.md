...
페이지를 만들 때에는 템플릿을 선택해야 합니다. 템플릿은 새 페이지를 만드는 기준으로 사용됩니다. 템플릿은 결과 페이지의 구조, 초기 콘텐츠 및 사용할 수 있는 구성 요소를 정의합니다.
...
이 문서에서는 **템플릿 작성자** 템플릿 콘솔 및 편집기를 사용하여 편집 가능한 템플릿을 만들고 관리할 수 있습니다.
...
>[!CAUTION]
>
>을 사용하여 만든 페이지 및 템플릿 **템플릿 콘솔** 클래식 UI에서 사용하기 위한 것이 아니며, 이러한 사용은 지원되지 않습니다.
...
* 인스턴스가 구성되는 방법에 따라, AEM에서 이제 다음을 제공한다는 것을 이해하는 것이 유용할 수 있습니다 [두 가지 기본 템플릿 유형](/help/sites-authoring/templates.md#editable-and-static-templates). 이 사실은 실제 방법에는 영향을 주지 않습니다 [템플릿을 사용하여 페이지 만들기](#using-a-template-to-create-a-page)그러나 만들 수 있는 템플릿 유형과 페이지가 해당 템플릿과 관련되는 방식에는 영향을 줍니다.
...
* **템플릿 작성자**:
   ...
   * `template-authors` 그룹의 멤버인 특정 작성자입니다.
      ...
      * 필요한 권한을 할당합니다.
   * 구성 요소 사용과 다음 요구 사항이 있는 기타 높은 수준의 세부 사항을 구성할 수 있습니다.

      * 일부 기술 지식

         * 예를 들어 경로를 정의할 때 패턴을 사용합니다.
      * 개발자의 기술 정보



폴더 만들기와 같은 일부 작업의 특성으로 인해 개발 환경이 필요하며, 이를 위해서는 지식/경험이 필요합니다.

이 문서에 자세히 설명된 작업은 해당 작업을 수행하는 역할과 함께 나열됩니다.

### 편집 가능 및 정적 템플릿 {#editable-and-static-templates}

이제 AEM에서는 두 가지 기본 유형의 템플릿을 제공합니다.

* 편집 가능한 템플릿

   * 다음을 수행할 수 있습니다. [생성됨](#creating-a-new-template-template-author) 및 [편집됨](#editing-templates-template-authors) 다음을 사용하는 템플릿 작성자 **템플릿** 콘솔 및 편집기. 다음 **템플릿** 콘솔에서 액세스 가능 **일반** 섹션 **도구** 콘솔.
   * 새 페이지가 만들어지면 페이지와 템플릿 간에 동적 연결이 유지됩니다. 즉, 템플릿 구조 및/또는 잠긴 컨텐츠에 대한 변경 사항이 해당 템플릿으로 만든 페이지에 반영됩니다. 잠금 해제된(즉, 초기) 컨텐츠에 대한 변경 사항은 반영되지 않습니다.
   * 템플릿 편집기에서 정의할 수 있는 컨텐츠 정책을 사용하여 디자인 속성을 유지합니다. 페이지 편집기 내의 디자인 모드는 편집 가능한 템플릿에 더 이상 사용되지 않습니다.

* 정적 템플릿

   * 정적 템플릿은 여러 버전의 AEM에 사용할 수 있습니다.
   * 그렇습니다 [개발자가 제공](/help/sites-developing/page-templates-static.md)따라서 작성자가 만들거나 편집할 수 없습니다.
   * 새 페이지를 만들기 위해 복사되지만, 그 후에는 동적 연결이 존재하지 않습니다(단, 템플릿 이름이 정보에 등록됨).
   * 사용 [디자인 모드](/help/sites-authoring/default-components-designmode.md) 디자인 속성을 유지합니다.
   * 정적 템플릿 편집은 개발자의 독점적인 작업이므로 개발자 문서를 참조하십시오 [페이지 템플릿 - 정적](/help/sites-developing/page-templates-static.md) 추가 정보.

정의에 따라 템플릿 콘솔 및 템플릿 편집기에서는 편집 가능한 템플릿을 만들고 편집할 수만 있습니다. 따라서 이 문서는 편집 가능한 템플릿에만 중점을 둡니다.

### 템플릿을 사용하여 페이지 만들기 {#using-a-template-to-create-a-page}

템플릿을 사용할 때 [새 페이지 만들기](/help/sites-authoring/managing-pages.md#creating-a-new-page) 정적 템플릿과 편집 가능한 템플릿 간에 차이점은 없으며 표시도 없습니다. 페이지 작성자의 경우 프로세스가 투명합니다.

## 템플릿 만들기 및 관리 {#creating-and-managing-templates}

편집 가능 템플릿을 새로 만들 때 다음 작업을 수행합니다.

* 를 사용하십시오 **템플릿** 콘솔. 이 기능은 **일반** 섹션 **도구** 콘솔.
...
* [추가 속성 정의](#defining-template-properties-template-author) 필요한 경우 템플릿에 대한
* [템플릿 편집](#editing-templates-template-authors) 를 정의하려면
...
* [템플릿 활성화](#enabling-a-template-template-author) 페이지를 만들 때 사용
* [템플릿 허용](#allowing-a-template-author) 웹 사이트의 필수 페이지 또는 분기에 대해
* [템플릿 게시](#publishing-a-template-template-author) 게시 환경에서 사용 가능하게 하려면 다음을 수행하십시오
...
>[!CAUTION]
>
>필요한 정보는 입력하지 마십시오 [국제](/help/sites-developing/i18n.md) 템플릿을 만듭니다.

### 템플릿 폴더 만들기 - 관리자 {#creating-a-template-folder-admin}

프로젝트에 프로젝트 특정 템플릿을 포함하려면 템플릿 폴더를 만들어야 합니다. 관리자 작업이며 문서에 설명되어 있습니다 [페이지 템플릿 - 편집 가능](/help/sites-developing/page-templates-editable.md#template-folders).

### 새 템플릿 만들기 - 템플릿 작성자 {#creating-a-new-template-template-author}

1. 를 엽니다. **템플릿 콘솔** (사용 시) **도구** -> **일반**)을 클릭한 다음 필요한 폴더로 이동합니다.

   >[!NOTE]
   >
   >표준 AEM 인스턴스에서는 **전역** 폴더가 템플릿 콘솔에 이미 있습니다. 이 폴더는 기본 템플릿을 보유하며, 현재 폴더에 정책 및/또는 템플릿 유형을 찾을 수 없는 경우 폴백으로 작동합니다.
   >
   >을 사용하는 것이 가장 좋습니다 [프로젝트용으로 만든 템플릿 폴더](/help/sites-developing/page-templates-editable.md#template-folders).

1. **만들기**, **템플릿 만들기**&#x200B;를 차례로 선택하여 마법사를 엽니다.

1. 선택 **템플릿 유형**&#x200B;를 선택하고 을 선택합니다. **다음**.

   >[!NOTE]
   >
   >템플릿 유형은 사전 정의된 템플릿 레이아웃이며 템플릿의 템플릿으로 간주할 수 있습니다. 이러한 템플릿은 개발자 또는 시스템 관리자가 미리 정의합니다. 개발자 문서에서 자세한 내용을 확인할 수 있습니다. [페이지 템플릿 - 편집 가능](/help/sites-developing/page-templates-editable.md#template-type).

1. 을(를) 완료합니다 **템플릿 세부 정보**:
...
템플릿에는 다음 속성이 있을 수 있습니다.

* 이미지

   * 이미지로 사용할 이미지 [템플릿의 축소판](/help/sites-authoring/templates.md#template-thumbnail-image) 를 눌러 페이지 만들기 마법사의 경우처럼 선택 항목을 지원합니다.
...
* 제목
   ...
   * 템플릿을 식별하는 데 사용되는 제목(예: **페이지 만들기** 마법사

* 설명

   * 템플릿 및 해당 사용 방법에 대한 자세한 내용을 제공하기 위한 선택적 설명으로, **페이지 만들기** 마법사
...
1. 썸네일을 업로드할지 또는 템플릿 콘텐츠에서 생성할지를 선택합니다.
   ...
   * 축소판을 업로드하려면 을(를) 클릭하거나 탭합니다 **이미지 업로드**
   * 썸네일을 생성하려면 **미리보기 생성**&#x200B;을 클릭하거나 탭합니다.

1. 두 방법 모두, 썸네일의 미리보기가 표시됩니다.

   만족스럽지 않은 경우 을(를) 클릭하거나 탭합니다 **지우기** 다른 이미지를 업로드하거나 축소판을 다시 생성하려면 다음을 수행합니다.

1. 축소판에 만족하면 을 클릭하거나 탭합니다 **저장 및 닫기**.
...
템플릿을 활성화하거나 비활성화하여 **페이지 만들기** 마법사

>[!CAUTION]
>
>템플릿이 활성화되면 템플릿 작성자가 템플릿을 추가로 업데이트하기 시작하면 경고가 표시됩니다. 이 경고는 템플릿이 참조될 수 있으므로 변경 사항이 템플릿을 참조하는 페이지에 영향을 줄 수 있음을 사용자에게 알리는 것입니다.
...
1. **템플릿 설정**&#x200B;에서 **필드 추가**&#x200B;를 사용하여 템플릿에 대한 경로를 지정합니다.
   ...
   경로는 명시적이거나 패턴을 사용할 수 있습니다. 예:

   `/conf/<your-folder>/settings/wcm/templates/.*`

   경로의 순서는 관련이 없으며 모든 경로가 검색되고 모든 템플릿이 검색됩니다.

   >[!NOTE]
   >
   >만약 **허용된 템플릿** 목록이 비어 있으면 값/목록이 발견될 때까지 상위 항목이 됩니다.
   >
...
1. 선택 **게시** 도구 모음에서 작업을 완료합니다.

## 템플릿 편집 - 템플릿 작성자 {#editing-templates-template-authors}

템플릿을 만들거나 편집할 때 다양한 측면을 정의할 수 있습니다. 템플릿 편집은 페이지 작성과 유사합니다.
...
>[!CAUTION]
>
>작성자가 이미 활성화된 템플릿을 편집하기 시작하면 경고가 표시됩니다. 이 경고는 템플릿이 참조될 수 있으므로 변경 사항이 템플릿을 참조하는 페이지에 영향을 줄 수 있음을 사용자에게 알리는 것입니다.

### 템플릿 편집 - 구조 - 템플릿 작성자 {#editing-a-template-structure-template-author}

in **구조** 모드에서는 템플릿에 대한 구성 요소 및 컨텐츠를 정의하고 템플릿 및 해당 구성 요소에 대한 정책을 정의합니다.
...
* **구성 요소 추가**
   ...
   * 에서 **구성 요소** 브라우저 를 클릭합니다.
   * 를 사용하여 **구성 요소 삽입** 옵션 (**+** 아이콘) 템플릿에 이미 있는 구성 요소의 도구 모음에서 사용 가능한 **구성 요소를 여기로 드래그하십시오** 상자.
   * 사이드 패널의 **에셋** 브라우저에서 템플릿으로 직접 에셋을 끌어와 제 위치에 해당 구성 요소를 생성하여

   추가된 각 구성 요소는 다음과 같이 표시됩니다.

   * 테두리
   * 구성 요소 유형을 표시하는 마커
   * 구성 요소 잠금이 해제된 경우 표시할 마커
   ...
   >[!NOTE]
   >
   >동일하지는 않지만 구성 요소 및 자산을 템플릿에 추가하는 것은 [페이지 작성](/help/sites-authoring/editing-content.md).
...
* **구조를 나타내는 테두리**
   ...
   작업 시 **구조** 모드 및 주황색 테두리는 현재 선택된 구성 요소를 나타냅니다. 점선은 상위 구성 요소도 나타냅니다.

   예를 들어 아래 스크린샷에서 **텍스트** 구성 요소 선택 **레이아웃 컨테이너** (responsivegrid).

   ![chlimage_1-364](assets/chlimage_1-364.png)

* **정책 및 속성(일반)**

   콘텐츠(또는 디자인) 정책은 구성 요소의 디자인 속성을 정의합니다. 예를 들어 사용 가능한 구성 요소나 최소/최대 차원을 사용할 수 있습니다. 템플릿(및 템플릿으로 만든 페이지)에 적용할 수 있습니다.
   ...
   **선택한 정책을 사용하는 다른 템플릿** 섹션에서 **정책 선택** 드롭다운에서 선택한 정책을 사용하는 다른 템플릿을 쉽게 확인할 수 있습니다.
   ...
   >[!NOTE]
   >
   >동일한 유형의 여러 구성 요소가 초기 콘텐츠로 추가되는 경우, 동일한 정책이 모든 구성 요소에 적용됩니다. 이에 따라 에도 동일한 제한이 적용됩니다 **[디자인 모드](/help/sites-authoring/default-components-designmode.md)** 정적 템플릿의 경우.

   ***속성***

   **속성** 제목에서 구성 요소의 설정을 정의할 수 있습니다. 제목에는 두 개의 탭이 있습니다.
   ...
   *기본*
   ...
   설정 **기본** 탭에서 구성 요소의 가장 중요한 설정이 정의됩니다.
   ...
   *기능*
   ...
   >[!NOTE]
   >
   >[리치 텍스트 편집기를 구현하는 구성 요소에 대한 콘텐츠 정책](/help/sites-administering/rich-text-editor.md)은 RTE가 해당 UI 설정을 통해 사용 가능한 옵션에 대해서만 정의할 수 있습니다.

* **정책 및 속성(레이아웃 컨테이너)**

   레이아웃 컨테이너의 정책 및 속성 설정은 일반 사용과 유사하지만 몇 가지 차이점이 있습니다.

   >[!NOTE]
   >
   >컨테이너 구성 요소에 대해 정책을 구성해야 합니다. 컨테이너 구성 요소에서 사용할 수 있는 구성 요소를 정의할 수 있기 때문입니다.
   ...
   **속성** 제목에서는 레이아웃 컨테이너에 사용할 수 있는 구성 요소를 선택하고 해당 설정을 정의할 수 있습니다. 제목에는 세 개의 탭이 있습니다.
   ...
   *허용된 구성 요소*
   ...
   설정 **허용된 구성 요소** 탭에서 레이아웃 컨테이너에 사용할 수 있는 구성 요소를 정의합니다.
   ...
   **기본 구성 요소** 탭에서는 작성자가 에셋 브라우저에서 에셋을 끌어 오면 AEM에서 연결할 구성 요소를 알 수 있도록, 지정된 미디어 유형과 자동으로 연결되는 구성 요소를 정의합니다. 놓기 영역이 있는 구성 요소만 이러한 구성에 사용할 수 있습니다.

   클릭 또는 탭 **매핑 추가** 완전히 새로운 구성 요소 및 MIME 유형 매핑을 추가하려면 다음을 수행하십시오.
...
* **구성 요소 잠금 해제/잠금**
   ...
   * 잠금 해제된 구성 요소의 상위 항목을 이동하거나, 잘라내거나, 삭제할 수 없습니다.
   ...
   공간을 절약하기 위해 레이아웃 컨테이너는 허용된 구성 요소 목록을 수용하도록 확장되지 않습니다. 대신 컨테이너가 스크롤 가능한 목록이 됩니다.
...
>[!NOTE]
>
>**초기 콘텐츠** 모드는 해당 템플릿으로 만든 페이지의 편집 모드로 간주할 수 있습니다. 따라서 정책은 **초기 콘텐츠** 모드에서 정의되지 않고 **[구조](/help/sites-authoring/templates.md#editing-a-template-structure-template-author)** 모드에서 정의됩니다.

* 편집에 사용할 수 있는 잠금 해제된 구성 요소가 표시됩니다. 선택한 경우 파란색 테두리가 표시됩니다.
...
1. 에서 **템플릿 편집기**, 선택 **페이지 정보** 도구 모음에서 **페이지 디자인** 대화 상자를 엽니다.
1. 다음 **페이지 디자인** 대화 상자가 열리고 다음 두 섹션으로 나누어집니다.

   * 왼쪽 절반 섹션에서는 [페이지 정책](/help/sites-authoring/templates.md#page-policies)
   * 오른쪽 절반 섹션에서는 를 정의합니다 [페이지 속성](/help/sites-authoring/templates.md#page-properties)
...
템플릿을 만들 때는 다음 사항을 고려해야 합니다.

1. 해당 템플릿에서 페이지를 작성한 후 템플릿 변경 사항이 템플릿에 미치는 영향

   다음은 템플릿에서 수행할 수 있는 다양한 작업 목록과 이러한 작업이 템플릿으로 만든 페이지에 영향을 미치는 방식입니다.
   ...
   * 초기 콘텐츠에 대한 변경 사항:
      ...
      * 이러한 사항은 템플릿을 변경한 후 작성된 페이지에만 적용됩니다.
   * 레이아웃 변경 사항은 수정된 구성 요소가 다음에 속하는지 여부에 따라 다릅니다.

      * 구조 전용 - 즉시 적용
      * 초기 콘텐츠 포함 - 변경 후에 생성된 페이지만

   다음과 같은 경우 특별히 주의하십시오.

   * 활성화된 템플릿에서 구성 요소 잠금 또는 잠금 해제.
   * 기존 페이지에서 이미 해당 구성 요소를 사용하고 있을 수 있으므로 부작용이 있을 수 있습니다. 일반적으로

      * 구성 요소(잠긴)의 잠금을 해제하면 기존 페이지에서 누락됩니다.
...
1. [자체 폴더 만들기](#creating-a-template-folder-admin) 사이트별 템플릿에 대해
1. [템플릿 게시](#publishing-a-template-template-author) 에서 **템플릿** 콘솔.
