...
# 모바일 애플리케이션 제작{#building-mobile-applications}
...
GitHub에서 사용 가능한 코드를 사용하여 모바일 애플리케이션을 구축하는 방법에 대한 단계별 전체 문서를 사용할 수 있습니다 [여기](https://helpx.adobe.com/experience-manager/using/aem62_mobile.html).
...
에서 파일을 이동하기만 하면 됩니다 `/content/phonegap/content/<application name>` 게시 인스턴스에 분기합니다. 의 파일 `/content/phonegap/apps/<application name>` 작성자가 페이지를 활성화하면 분기가 이동됩니다.

AEM에서는 벌크 컨텐츠를 게시 인스턴스로 이동하는 두 가지 방법을 제공합니다.

* [트리 활성화 명령을 사용합니다](/help/sites-authoring/publishing-pages.md) 복제 콘솔에서 게시할 수 있습니다.
* [패키지 만들기](/help/sites-administering/package-manager.md) 컨텐츠가 포함된 패키지를 복제하고
...
## PhoneGap 명령줄 인터페이스를 사용한 빌딩 {#building-using-the-phonegap-command-line-interface}
...
1. 다운로드 및 설치 [Node.js](https://nodejs.org/).
1. 터미널 또는 명령 프롬프트를 열고 다음 노드 명령을 입력하여 PhoneGap 유틸리티를 설치합니다.
   ...
   Unix 또는 Linux 시스템에서는 명령 접두사를 `sudo`.
...
1. (선택 사항) 타깃팅하는 모바일 플랫폼용 SDK를 받습니다.
   ...
   * iOS 플랫폼용 앱을 빌드하려면 최신 버전의 [Xcode](https://developer.apple.com/xcode/).
   * Android 앱을 빌드하려면 [Android SDK](https://developer.android.com/).

### 컨텐츠 ZIP 파일 다운로드 {#downloading-the-content-zip-file}
...
PhoneGap CLI를 사용하여 애플리케이션을 컴파일하고 설치합니다. PhoneGap CLI 사용 방법에 대한 자세한 내용은 PhoneGap 을 참조하십시오 [명령줄 인터페이스](https://docs.phonegap.com/en/3.0.0/guide_cli_index.md.html) 설명서.
...
1. 시작 페이지를 선택한 다음 원격 빌드 아이콘을 클릭합니다.
   ...
   **참고:** AEM Beta 베타 버전에서 빌드가 성공적으로 완료되면 받은 편지함 알림을 만들지 않습니다.

1. 성공 대화 상자에서 PhoneGap Build 를 클릭하여 Adobe PhoneGap Build 페이지를 엽니다. [https://build.phonegap.com/apps](https://build.phonegap.com/apps). 앱이 표시되도록 대기 중인 경우 [PhoneGap Build 상태](https://status.build.phonegap.com/) 페이지.

   빌드 설치에 대한 자세한 내용은 [PhoneGap Build 설명서](https://docs.build.phonegap.com/en_US/3.1.0/#googtrans%28en%29).
...
빌딩 프로세스의 다음 단계는 [앱 구조](/help/mobile/phonegap-structure-an-app.md).
