...
* ***selector - 문자열*** - 선택적인 선택기가 점으로 구분됩니다. 일반적인 예는 *페이지의 모바일 버전을 렌더링하기* 위한 터치입니다.

* ***deep - 부울*** - 하위 페이지를 포함해야 하는지 여부를 결정하는 선택적 부울 속성입니다. The default value is *true.*

* ***includeImages - Boolean*** - 이미지를 포함해야 하는지 여부를 결정하는 선택적 부울 속성입니다. The default value is *true*.

   * 기본적으로, 리소스 유형이 foundation/components/image인 이미지 구성 요소만 포함할 수 있습니다.

* ***includeVideos - Boolean*** - 비디오가 포함되어야 하는지 여부를 결정하는 선택적 부울 속성입니다. The default value is *true*.

* ***includeModifiedPagesOnly - 부울*** - false이거나 생략된 경우 모든 페이지를 렌더링하고 렌더링에서 업데이트를 확인합니다. true인 경우 마지막 수정됨 페이지의 변경 사항에 따라 다릅니다.
* ***+ 다시 쓰기(노드)***
...
* ***경로*** - 문자열 - 비어 있어야 하며 올바른 처리기로 표시되어야 하지만 경로가 현재 ContentSync 캐시로 추측됩니다. 이 값은 무시됩니다.
* ***targetRootDirectory *-**문자열 - 이 처리기에 대한 컨텐츠 업데이트에 대한 대상 루트로 경로에 추가할 접두사입니다.
* ***order - Long *-**ContentSync에서 이 핸들러를 실행하려면 순서가 필요합니다. 이 숫자는 100과 같은 다른 모든 핸들러보다 높게 설정해야 합니다. 기존 컨텐츠 핸들러에서 실행해야 합니다.
...
* ***경로&#x200B;**-**문자열*** - 앱 셸(page-type=app-instance가 있는 노드)에 대한 경로입니다.
* ***targetRootDirectory - 문자열*** - 이 처리기에 대한 컨텐츠 업데이트에 대한 대상 루트로 경로에 추가할 접두사입니다.
* ***order - Long *-**ContentSync에서 이 핸들러를 실행하려면 순서가 필요합니다. 이 숫자는 100과 같은 다른 모든 핸들러보다 높게 설정해야 합니다. 기존 컨텐츠 핸들러에서 실행해야 합니다.
...
**widgetconfig** 명령 센터를 통해 편집한 내용을 제공된 config.xml과 병합하는 업데이트된 config.xml을 포함합니다. 이 처리기에 관리 인터페이스를 통해 변경된 앱 세부 사항이 포함되지 않으면 캐시에 포함되지 않습니다.

이 핸들러는 AEM App Shell ContentSync 구성(page-type=[app-instance]가 있는 노드)에서 사용해야 합니다.
...
* ***경로 - 문자열*** - 앱 셸로의 경로(pge-type=app-instance 또는 /libs/mobileapps/core/components/instance를 확장하는 RT가 있는 노드)
...
* ***autoCreateFirstUpdateBeforeImport - 부울*** - true인 경우 가져오기 전에 대상 구성에 초기 **업데이트를** 만든 다음 기존에 없는 경우

* ***autoFillBeforeImport - 부울*** - true인 경우 가져오기 전에 대상 구성을 업데이트/채웁니다
* ***configSuffix - 문자열*** - app-content의 &quot;phonegap-exportTemplate&quot; 속성에 지정된 경로에 추가할 문자열입니다. 다른 내보내기 템플릿을 구분하는 데 사용할 수 있습니다. 예를 들어, 이 속성은 **&quot;/../../../appconfig-dev&quot;를** 사용하도록(&quot;/../.../appconfig&quot;와 *반대)를 나타내도록* &quot;-dev&quot;로 설정할 수 있습니다 **.

**app-assets** 앱 인스턴스와 연관된 모든 자산을 포함합니다. 이 처리기에는 앱 인스턴스의 appAssetPath 속성이 참조하는 모든 자산과 함께 지정된 경로에서 발견된 모든 자산이 포함됩니다.

* ***type - 문자열*** - app-assets

* ***path **-**문자열*** - 앱 에셋이 저장된 앱 인스턴스 아래의 위치에 대한 경로

**mobileappoffers** 타깃팅된 컨텐츠를 렌더링하기 위해 개인화 사용 사례에 대한 새로운 컨텐츠 동기화 핸들러가 도입되었습니다. &#39;mobileappoffers&#39; 핸들러는 컨텐츠 작성자가 만든 관련 대상 오퍼를 렌더링하는 방법을 알고 있습니다. mobileappoffers 핸들러는 abstract pages update 핸들러를 확장하므로 대부분의 속성이 유사합니다. mobileappoffers 핸들러의 세부 정보에는 다음 속성이 있습니다.
...
**mobileappconfig** mobileappconfig content sync 핸들러는 JSON 데이터를 MobileAppsConfig.json에 주입하는 방법을 제공합니다. 공급자 클래스 개발자는 해당 MobileAppsInfoProvider 클래스를 공급자 목록에 추가합니다. 이 핸들러는 MobileAppsInfoProviders 목록을 반복하여 제공자가 결과 json 파일에 데이터를 삽입할 수 있도록 합니다. 이 처리기가 지원하는 속성 목록은 다음과 같습니다.

* ***path **-**문자열*** - pge-type=app-instance 또는 /libs/mobileapps/core/components/instance를 확장하는 RT가 있는 앱 인스턴스 노드에 대한 경로
...
**로깅** 패키지의 OSGI 로거 구성을 통해 ContentSync 디버그 로깅 활성화 `com.day.cq.contentsync` 이렇게 하면 실행된 핸들러와 캐시를 업데이트했는지, 그리고 캐시 업데이트를 보고했는지 여부를 추적할 수 있습니다.
...
>[!NOTE]
>
>AEM Mobile 앱 개발을 시작하려면 [여기를 클릭하십시오](/help/mobile/getting-started-aem-mobile.md).
