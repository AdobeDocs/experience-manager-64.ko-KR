...
PhoneGap CLI를 사용하여 구축하는 경우 cordova 빌드 후크 스크립트로 수행할 수 있습니다. Geometrixx Outdoors 앱(*content/phonegap/geometrixx-outdoors/shell/_jcr_content/pge-app/app-content/phonegap/scripts/restore_plugins.js)에서 볼 수 있습니다.*
...
### Bloodhound {#testing-analytics-tracking-with-bloodhound}을(를) 사용하여 분석 추적 테스트
...
프로덕션 배포 전에 Adobe 도구 Bloodhound를 사용하여 분석 구성을 테스트할 수 있습니다(선택 사항). 분석 구성을 테스트하려면 ADBMobileConfig.json 파일을 편집하여 실제 Analytics 서버 대신 Bloodhound가 실행 중인 서버를 가리켜야 합니다. 이 변경 사항을 적용하려면 ADBMobileConfig.json에서 다음 항목을 변경합니다.
