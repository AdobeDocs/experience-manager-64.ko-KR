...
+ 자산 확장 {#extending}
   + [자산 사용자 지정 및 확장](extending-assets.md)
   + [미디어 핸들러 및 워크플로우를 사용한 자산 처리](media-handlers.md)
   + [자산 검색 확장](searchx.md)
...
+ 자산 관리 {#administer}
   ...
   + [메타데이터, 이미지 및 비디오를 처리하기 위한 프로필](processing-profiles.md)
   ...
   + [효율적인 자산 번역 모범 사례](best-practices-for-translating-assets-efficiently.md)
   ...
   + [자산 오프로드 우수 사례](assets-offloading-best-practices.md)
   ...
   + [컬렉션, 코드 조각 및 코드 조각 템플릿을 위한 멀티 테넌시](multi-tenancy.md)
   + [원본에 쓰기](watermarking.md)
   + [일괄 메타데이터 가져오기 및 내보내기](metadata-import-export.md)
   + [자산 템플릿](asset-templates.md)
   + [AEM-Creative Cloud 폴더 공유 우수 사례](aem-cc-folder-sharing-best-practices.md)
...
+ 자산 관리 {#managing}
   ...
   + [자산에서 파일 체크 인 및 체크 아웃](check-out-and-submit-assets.md)
   + [스마트 콘텐츠 서비스를 사용하여 자산 태그 지정 구성](config-smart-tagging.md)
   + [중복 검색 사용](duplicate-detection.md)
   + [AEM의 향상된 자산 정렬](enhance-sort.md)
   ...
   + [웹 페이지에서 페이지 추적기 및 포함 코드 사용](touch-ui-using-page-tracker.md)
...
+ 3D 자산 {#three-d}
   ...
   + [AEM 3D와 Autodesk 3ds Max 통합](integrating-aem-3d-with-autodesk-3ds-max.md)
