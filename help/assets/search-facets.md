...
AEM(Adobe Experience Manager) Assets의 엔터프라이즈급 배포는 많은 자산을 저장할 수 있는 용량을 제공합니다. AEM의 일반 검색 기능만 사용하는 경우 적절한 자산을 찾는 것이 어렵고 시간이 걸릴 수 있습니다.
...
전체 텍스트 검색의 경우 양식에 전체 텍스트 설명을 추가합니다. 속성 설명을 사용하여 지정한 단일 속성과 일치하는 자산을 검색합니다. 옵션 조건부를 사용하여 특정 속성에 대해 하나 이상의 값과 일치하는 자산을 검색합니다. 날짜 범위 설명을 추가하여 지정된 날짜 범위 내에서 만들어진 자산을 검색합니다.

1. 을 탭/클릭합니다. [!DNL Experience Manager] 로고로 이동한 다음 **[!UICONTROL 도구]** > **[!UICONTROL 일반]** > **[!UICONTROL Forms 검색]**.
1. Forms 검색 페이지에서 **[!UICONTROL Assets 관리 검색 레일]**&#x200B;를 탭한 다음,  **편집** ![aemassets_edit](assets/aemassets_edit.png).
   ...
   자산 관리 검색 레일을 찾아 선택합니다

   >[!NOTE]
   >
   >사전 구성된 폴더에서 폴더 검색 기능을 사용하려면 **Assets 관리 검색 레일** 이전 [!DNL Experience Manager] 버전을 사용하여 다음 단계를 수행하십시오.
   > 
   >1. 다음으로 이동 */conf/global/settings/dam/search/facets/assets/jcr:content/items* CRX-DE에서 지원됩니다.
   >1. 삭제 **유형** 노드 아래에 있어야 합니다.
...
1. [검색 편집] Forms 페이지에서 **[!UICONTROL 설명 선택]** 기본 창으로 이동합니다. 예를 들어 **[!UICONTROL 속성 설명]**.
   ...
   ![설명을 드래그 앤 드롭하여 검색 필터를 사용자 지정합니다](assets/drag_predicate.png)

   설명을 드래그 앤 드롭하여 검색 필터를 사용자 지정합니다

1. 설정 탭에서 조건자에 대한 필드 레이블, 자리 표시자 텍스트 및 설명을 입력합니다. 조건자와 연결할 메타데이터 속성의 올바른 이름을 지정합니다.

   설정 탭의 헤더 레이블은 선택한 조건부의 유형을 식별합니다.
...
1. 을 탭/클릭합니다. **[!UICONTROL 미리 보기]** ![미리 보기](assets/preview.png) 설명을 추가한 후 나타나는 [필터] 패널의 미리 보기를 생성하려면 다음을 수행합니다.
...
1. 미리 보기를 닫으려면 를 탭/클릭합니다 **[!UICONTROL 닫기]** ![close](assets/close.png) 미리 보기 오른쪽 상단 모서리에서 을(를) 클릭합니다.
...
1. 탭하기 [!DNL Experience Manager] 로고로 이동한 다음 **[!UICONTROL 도구 > 일반 > Forms 검색]**.
...
1. 다음 중 하나를 수행하십시오.
   ...
   * 탭 ![추가 아이콘](assets/do-not-localize/aem_assets_add_icon.png) 옵션 필드 옆에 있는 [필터] 패널에서 제공하려는 옵션의 표시 텍스트와 값을 지정합니다. 다른 옵션을 추가하려면 탭/클릭합니다 ![추가 아이콘](assets/do-not-localize/aem_assets_add_icon.png) 단계를 반복합니다.
...
다중 값 속성 설명을 사용하면 자산을 여러 값에 대한 검색을 수행할 수 있습니다. 에 여러 제품의 이미지가 있는 시나리오를 생각해 보십시오 [!DNL Experience Manager] 각 이미지에 대한 자산 및 메타데이터는 제품과 연결된 SKU 번호를 포함합니다. 이 설명을 사용하여 여러 SKU 번호를 기반으로 제품 이미지를 검색할 수 있습니다.
...
태그 설명을 사용하면 자산에 대한 태그 기반 검색을 수행할 수 있습니다. 기본적으로 [!DNL Experience Manager] 자산은 사용자가 지정하는 태그를 기준으로 일치하는 하나 이상의 태그가 있는지 자산을 검색합니다. 즉, 검색 쿼리는 지정된 태그를 사용하여 OR 작업을 수행합니다. 하지만 모든 태그 일치 옵션을 사용하여 지정하는 모든 태그를 포함하는 자산을 검색할 수 있습니다.
...
1. 자산을 검색할 위치에 대한 태그를 지정하거나 제안 목록에서 선택합니다.
   ...
   ![태그의 이름을 입력할 때 AEM 제공 제안](assets/chlimage_1-419.png)

   태그의 이름을 입력할 때 AEM 제공 제안
...
| 설명 이름 | 설명 | 속성 |
|---|---|---|
| [!UICONTROL 전체 텍스트] | 전체 자산 노드에서 전체 텍스트 검색을 수행하는 검색 조건입니다. jcr:contains 연산자와 함께 매핑됩니다. 자산 노드의 특정 부분에서 전체 텍스트 검색을 수행하려면 상대 경로를 지정할 수 있습니다. | <ul><li>레이블</li><li>자리 표시자</li><li>속성 이름</li><li>설명</li></ul> |
| [!UICONTROL 경로 브라우저] | 사전 구성된 루트 경로에서 폴더 및 하위 폴더의 자산을 검색하려면 검색 조건어를 사용하십시오 | <ul><li>자리 표시자</li><li>루트 경로</li><li>설명</li></ul> |
| [!UICONTROL 경로] | 결과를 위치에 대해 필터링하려면 이 변수를 사용하십시오. 다양한 경로를 옵션으로 지정할 수 있습니다. | <ul><li>레이블</li><li>경로</li><li>설명</li></ul> |
| [!UICONTROL 게시 상태] | 검색 조건자를 사용하여 게시 상태에 따라 자산을 검색합니다 | <ul><li>레이블</li><li>속성 이름</li><li>설명</li></ul> |
| [!UICONTROL 상대적 날짜] | 검색 조건자를 사용하여 자산의 상대적 날짜에 따라 자산을 검색합니다. 예를 들어 2개월 전, 3주 전 등과 같은 옵션을 구성할 수 있습니다. | <ul><li>레이블</li><li>속성 이름</li><li>상대적 날짜</li></ul> |
| [!UICONTROL 범위] | 검색 조건자를 사용하여 지정된 범위 내에 있는 자산을 검색합니다. [검색] 패널에서 범위에 대한 최소값과 최대값을 지정할 수 있습니다. | <ul><li>레이블</li><li>속성 이름</li><li>설명</li></ul> |
| [!UICONTROL 날짜 범위] | 검색 조건자를 사용하여 날짜 속성에 대해 지정된 범위 내에서 생성된 자산을 검색합니다. 검색 패널에서 날짜 선택기를 사용하여 시작 및 종료 날짜를 지정할 수 있습니다. | <ul><li>레이블</li><li>자리 표시자</li><li>속성 이름</li><li>범위 텍스트(시작)</li><li>범위 텍스트(끝)</li><li>설명</li></ul> |
| [!UICONTROL 날짜] | 날짜 속성을 기반으로 자산의 슬라이더 기반 검색에 대한 검색 조건입니다. | <ul><li>레이블</li><li>속성 이름</li><li>설명</li></ul> |
| [!UICONTROL 파일 크기] | 검색 조건자를 사용하여 크기를 기준으로 자산을 검색합니다. 구성 가능한 노드에서 슬라이더 옵션을 선택하는 실버 기반 조건입니다. 기본 옵션은 CRX 저장소의 /libs/dam/options/predicate/filesize에 정의됩니다. 파일 크기는 바이트 단위로 제공됩니다. | <ul><li>레이블</li><li>속성 이름</li><li>경로</li><li>설명</li></ul> |
| [!UICONTROL 마지막으로 수정된 자산] | 최근에 수정된 자산을 검색할 설명 검색 | <ul><li>속성 이름</li><li>속성 값</li><li>설명</li></ul> |
| [!UICONTROL 게시 상태] | 검색 조건자를 사용하여 게시 상태에 따라 자산을 검색합니다 | <ul><li>레이블</li><li>속성 이름</li><li>설명</li></ul> |
| [!UICONTROL 등급] | 검색 조건자를 사용하여 평균 등급을 기준으로 자산을 검색합니다 | <ul><li>레이블</li><li>속성 이름</li><li>옵션 경로</li><li>설명</li></ul> |
...
1. 탭 **[!UICONTROL 삭제]** ![deleteoutline](assets/deleteoutline.png) 클릭합니다.
...
| [!UICONTROL 삭제] | CRX의 /apps 노드에 대한 읽기, 쓰기 및 삭제 권한 |
| [!UICONTROL 미리보기] | CRX의 /var/dam/content 노드에 대한 읽기, 쓰기 및 삭제 권한. 또한 /apps 노드에 대한 읽기 및 쓰기 권한도 있습니다. |
