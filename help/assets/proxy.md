...
프록시는 특정(및 경우에 따라 별개인) [!DNL Experience Manager] 작업 처리 및 결과 생성을 담당하는 처리자로 프록시 작업자를 사용하는 인스턴스. 프록시 작업자는 다양한 작업에 사용될 수 있습니다. 의 경우 [!DNL Experience Manager] 자산 프록시 이 프록시는 내에서 렌더링하기 위해 자산을 로드하는 데 사용할 수 있습니다 [!DNL Experience Manager] 자산. 예: [ID 프록시 작업자](indesign.md) 에서 사용할 파일을 처리하기 위해 InDesign Server을 사용합니다. [!DNL Experience Manager] 자산.

프록시가 별도의 경우 [!DNL Experience Manager] 인스턴스 이렇게 하면 [!DNL Experience Manager] 작성 인스턴스. 기본적으로 [!DNL Experience Manager] 자산은 동일한 JVM에서 자산 처리 작업을 실행하여 프록시를 통해 외부화함으로써 [!DNL Experience Manager] 작성 인스턴스.
...
>[!NOTE]
>
>프록시 API에 대한 참조 설명서는 [`com.day.cq.dam.api.proxy`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/dam/commons/proxy/package-summary.html).

프록시 및 프록시 작업자 구성은 모두 [!DNL Experience Manager] 자산 **도구** 콘솔 또는 아래의 `/etc/cloudservices/proxy`. 각 프록시 작업자는 `/etc/cloudservices/proxy` 작업자 특정 구성 세부 정보(예: `/etc/cloudservices/proxy/workername`).
...
다음 [ID 프록시 작업자](indesign.md) 는 [!DNL Experience Manager] Indesign 자산의 처리를 아웃소싱하기 위해 이미 기본적으로 제공되는 Assets 프록시 작업자.

직접 개발하고 구성할 수도 있습니다 [!DNL Experience Manager] Assets 프록시 작업자를 사용하여 전문 작업자를 생성하여 [!DNL Experience Manager] 자산 처리 작업.
...
>[!NOTE]
>
>무슨 [!DNL Experience Manager] 자산 프록시 프레임워크는 풀 메커니즘으로 기본 제공 기능을 제공하지 않습니다.
>
>InDesign 통합을 통해 IDSPool(indesign server) 풀에 액세스할 수 있습니다. 이 풀링은 Indesign 통합에만 해당되며 [!DNL Experience Manager] 자산 프록시 프레임워크.

>[!NOTE]
>
>결과 동기화:
>
>동일한 프록시를 사용하는 인스턴스가 없는 경우 처리 결과는 프록시에 유지됩니다. 클라이언트의 작업입니다([!DNL Experience Manager] 작성자) 작업 작성 시 클라이언트에 제공된 것과 동일한 고유한 작업 ID를 사용하여 결과를 요청합니다. 프록시는 작업을 완료하고 결과를 요청할 준비가 된 상태로 유지합니다.
