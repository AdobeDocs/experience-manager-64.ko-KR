...
EXIF로 정의된 대부분의 메타데이터 필드는 기술적으로 매우 유용하며 설명적인 메타데이터 관리를 위해 제한된 용도로만 사용됩니다. 이러한 이유로 AEM 자산에서는 EXIF 속성을 [일반적인 메타데이터 구성표와](metadata-schemas.md) XMP에 매핑하는 기능을 [제공하며, AEM 자산에서는 메타데이터 관리를 위해](xmp-writeback.md)사용되는 강력한 메타데이터 형식을 사용합니다.
...
AEM에서는 가장 인기 있는 메타데이터 스키마 목록을 기본적으로 제공하므로 메타데이터 전략을 바로 시작하고 이미 정의된 스키마에서 필요한 메타데이터 속성을 선택할 수 있습니다.
...
>[!NOTE]
>
>애플리케이션별 메타데이터에는 기술 및 설명 메타데이터가 포함됩니다. 이러한 경우 다른 응용 프로그램에서 메타데이터를 사용할 수 없을 수 있습니다. 예를 들어 Adobe Photoshop 메타데이터가 있는 에셋을 보유하고 있고 다른 이미지 렌더링 응용 프로그램이 메타데이터에 액세스하려고 하는 경우 해당 메타데이터에 액세스할 수 없을 수 있습니다. 자산에 애플리케이션별 메타데이터가 많이 있는 경우 애플리케이션별 속성을 표준 메타데이터로 변경하는 워크플로우 단계를 만들 수 있습니다.
