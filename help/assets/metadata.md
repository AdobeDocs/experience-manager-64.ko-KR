...
description: 메타데이터 유형과 Adobe Experience Manager Assets를 통해 자산을 위한 메타데이터를 관리하여 자산을 보다 손쉽게 분류하고 구성할 수 있는 방법을 살펴볼 수 있습니다. Adobe Experience Manager Assets를 사용하면 에셋을 메타데이터에 따라 자동으로 구성하고 처리할 수 있으므로 임의의 메타데이터를 에셋을 관리하고 관리할 수 있습니다.
contentOwner: AG
...
Adobe Experience Manager Assets는 모든 자산의 메타데이터를 유지합니다. 이를 통해 보다 손쉽게 에셋을 분류하고 구성할 수 있으며 특정 에셋을 찾는 사람도 지원할 수 있습니다. 메타데이터 관리는 Experience Manager Assets에 업로드된 파일에서 메타데이터를 추출하는 기능을 통해 크리에이티브 워크플로우와 통합됩니다. Adobe Experience Manager 기능을 사용하면 메타데이터를 에셋으로 유지하고 관리할 수 있으므로 메타데이터를 기반으로 에셋을 자동으로 구성하고 처리할 수 있습니다.
...
Adobe Experience Manager가 자산에 대해 관리할 수 있는 기본 메타데이터 속성입니다. 사용자는 마지막 수정 날짜별로 정렬된 모든 자산을 볼 수 있으며 최근 저장소에 추가된 자산을 찾을 때 유용합니다.
...
메타데이터가 자산에 추가되면 자산이 늘어나기 때문에 자산의 값이 증가합니다.

* 보다 손쉽게 이용할 수 있습니다.
* 관리가 간편함 - 동일한 속성 세트를 사용하여 보다 손쉽게 에셋을 찾을 수 있고 변경 사항을 적용할 수 있습니다.
* 더욱 복잡해진 기능 - 자산에 추가한 메타데이터가 많을수록 메타데이터 관리가 더 중요해집니다.

이러한 이유로 Adobe Experience Manager Assets는 디지털 자산에 대한 메타데이터를 제작, 관리 및 교환할 수 있는 적절한 방법을 제공합니다.

## 메타데이터 유형 {#types-of-metadata}

두 가지 기본 메타데이터 유형은 기술 메타데이터와 설명 메타데이터입니다.

기술 메타데이터는 디지털 자산을 처리하는 소프트웨어 애플리케이션에 유용하며 수동으로 유지해서는 안 됩니다. Adobe Experience Manager Assets와 기타 소프트웨어는 기술 메타데이터를 자동으로 결정하며, 에셋이 수정되면 메타데이터가 변경될 수 있습니다. 자산의 사용 가능한 기술 메타데이터는 대개 자산의 파일 유형에 따라 달라집니다. 기술 메타데이터의 몇 가지 예는 다음과 같습니다.
...
Adobe Experience Manager Assets는 메타데이터 관리에 대한 모든 관련 표준을 지원합니다.
...
* XMP:experience Manager Assets에서 추출된 메타데이터를 저장소 내에 저장하는 데 사용됩니다.
...
XMP(Extensible Metadata Platform)는 모든 메타데이터 관리를 위해 Experience Manager Assets가 사용하는 개방형 표준입니다. 표준은 모든 파일 포맷에 임베드할 수 있는 범용 메타데이터 인코딩을 제공합니다. Adobe와 다른 회사는 리치 컨텐츠 모델을 제공하므로 XMP 표준을 지원합니다. XMP 표준 및 Experience Manager Assets의 사용자는 강력한 플랫폼을 통해 구축할 수 있습니다. 자세한 내용은 XMP를 [참조하십시오](https://www.adobe.com/products/xmp.html).
...
Exchange 이미지 파일 형식(Exif)은 디지털 사진 작업에 가장 많이 사용되는 메타데이터 형식입니다. JPEG, TIFF, RIFF 및 WAV와 같은 다양한 파일 포맷으로 메타데이터 속성의 고정 용어를 임베드할 수 있는 방법을 제공합니다. Exif는 메타데이터를 메타데이터 이름 및 메타데이터 값의 쌍으로 저장합니다. 이러한 메타데이터 이름-값 쌍을 태그라고도 합니다. Adobe Experience Manager의 태그 지정과 혼동하지 마십시오. Exif는 최신 디지털 카메라에서 자동으로 제작되고 최신 그래픽 소프트웨어를 통해 지원되므로 메타데이터 관리를 위한 가장 일반적인 분모로 볼 수 있습니다.

Exif의 주요 제한 사항은 BMP, GIF 또는 PNG와 같이 널리 사용되는 몇 가지 이미지 파일 형식이 지원되지 않는다는 것입니다.

일반적으로 Exif에서 정의한 메타데이터 필드는 기술적으로 정의되며 수사적 메타데이터 관리를 위해 제한된 용도로 사용됩니다. 이러한 이유로 Experience Manager Assets는 Exif 속성을 [일반적인 메타데이터 구성표와 XMP에](metadata-schemas.md) 매핑합니다 [](xmp-writeback.md).
...
또한 필요에 맞는 메타데이터가 없는 경우 고유한 메타데이터 스키마를 디자인할 수 있습니다. 기존 정보를 복제하지 마십시오. 조직 내에서 스키마를 구분하면 메타데이터를 쉽게 공유할 수 있습니다. Adobe Experience Manager는 가장 인기 있는 메타데이터 스키마 기본 목록을 제공합니다. 이 목록을 사용하면 메타데이터 전략을 바로 시작하고 필요한 메타데이터 속성을 신속하게 선택할 수 있습니다.
...
애플리케이션별 메타데이터에는 기술 및 설명 메타데이터가 포함됩니다. 이러한 경우 다른 응용 프로그램에서 메타데이터를 사용할 수 없을 수 있습니다. 예를 들어 Adobe Photoshop 메타데이터가 있는 자산이 있고 다른 이미지 렌더링 응용 프로그램이 메타데이터에 액세스하려고 하는 경우 메타데이터에 액세스할 수 없을 수 있습니다. 자산에 애플리케이션별 메타데이터가 많이 있는 경우 애플리케이션별 속성을 표준 속성으로 변경하는 워크플로우 단계를 만들 수 있습니다.

* acdsee - ACDSee 프로그램이 관리하는 메타데이터(www.acdsee.com [/](https://www.acdsee.com/))
* album - Adobe Photoshop Album.
* cq - Experience Manager Assets에서 사용됩니다.
* dam - Experience Manager Assets에서 사용
