...
특정 유형 또는 형식의 자산이 서버에 업로드될 때 자동으로 실행되도록 워크플로우를 정의할 수 있습니다. 처리 단계는 일련의 AEM Assets 미디어 처리기로 정의됩니다. AEM에서는 일부 [내장 핸들러를 제공하며,](#default-media-handlers) 추가적인 핸들러는 프로세스를 [명령줄 도구로 위임하여](#creating-a-new-media-handler) 사용자 [지정](#command-line-based-media-handler)또는 정의될 수 있습니다.
...
예를 들어 ImageMagick이 AEM 서버를 호스팅하는 디스크에 설치되어 있고 CommandLineProcess를 구현으로 사용하여 프로세스 단계를 만드는 경우 **다음 값을** 프로세스 인수로 **사용합니다**.
