...
# 자산 처리 미디어 핸들러 및 워크플로우 사용 {#processing-assets-using-media-handlers-and-workflows}

Adobe Experience Manager Assets는 자산을 처리하기 위한 기본 워크플로우 및 미디어 핸들러 세트를 제공합니다. 워크플로우는 일반적인 에셋 관리 및 처리 작업을 정의한 다음 축소판 생성 또는 메타데이터 추출 등의 미디어 핸들러에 특정 작업을 위임합니다.

특정 유형 또는 형식의 자산이 서버에 업로드될 때 자동으로 실행되도록 워크플로우를 정의할 수 있습니다. 처리 단계는 일련의 AEM Assets 미디어 처리기로 정의됩니다. AEM에서는 일부 [내장 핸들러를 제공하며,](#default-media-handlers) 추가 핸들러는 [명령줄 도구로](#creating-a-new-media-handler) 프로세스를 위임하여 [사용자 정의 개발](#command-line-based-media-handler)또는 정의될 수 있습니다.
...
1. 도구가 제대로 실행되는지 확인하려면 작업 디렉토리에 JPG 이미지를 추가하고 명령행에서 명령을 실행합니다. `convert <image-name>.jpg -flip <image-name>-flipped.jpg`
...
1. 에셋을 `myImage.tiff` 열고 역진행 이미지와 3개의 축소판이 작성되었는지 확인합니다.

#### CommandLineProcess 프로세스 단계 구성 {#configuring-the-commandlineprocess-process-step}

이 섹션에서는 의 프로세스 **[!UICONTROL 인수를 설정하는]** 방법에 대해 설명합니다 `CommandLineProcess`. 쉼표를 사용하여 [!UICONTROL 프로세스 인수의] 값을 구분하고 공백으로 값을 시작하지 마십시오.
