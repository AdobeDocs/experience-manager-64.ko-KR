...
특정 유형 또는 형식의 자산을 서버에 업로드할 때 자동으로 실행되는 워크플로우를 정의할 수 있습니다. 처리 단계는 일련의 Experience Manager Assets 미디어 핸들러로 정의됩니다. Adobe Experience Manager에서는 일부 [내장 핸들러,](#default-media-handlers) 등이 [사용자 정의 개발](#creating-a-new-media-handler)이거나, 프로세스를 [명령줄 도구](#command-line-based-media-handler)로 위임하여 정의할 수 있습니다.

미디어 핸들러는 자산에 대해 특정 작업을 수행하는 Experience Manager 자산 내의 서비스입니다. 예를 들어, MP3 오디오 파일이 Experience Manager에 업로드되면 워크플로우는 메타데이터를 추출하고 축소판을 생성하는 MP3 핸들러를 트리거합니다. 미디어 핸들러는 워크플로우에서 사용됩니다. 대부분의 일반적인 MIME 유형은 Experience Manager 내에서 지원됩니다. 다음 중 하나를 수행하여 자산에 대해 특정 작업을 수행할 수 있습니다
...
>[!NOTE]
>
>Experience Manager 자산에서 지원하는 모든 형식 및 각 형식에 대해 지원되는 기능에 대한 설명은 [자산 지원 형식](assets-formats.md) 페이지를 참조하십시오.
