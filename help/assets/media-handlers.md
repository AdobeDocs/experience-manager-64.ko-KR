...
특정 유형 또는 형식의 자산이 서버에 업로드될 때 자동으로 실행되도록 워크플로우를 정의할 수 있습니다. 처리 단계는 일련의 AEM Assets 미디어 핸들러로 정의됩니다. AEM에서는 일부 [내장 핸들러](#default-media-handlers)를 제공하며, 추가 핸들러는 [사용자 정의 개발](#creating-a-new-media-handler)이거나 프로세스를 [명령줄 도구](#command-line-based-media-handler)로 위임하여 정의할 수 있습니다.

미디어 핸들러는 자산에 대한 특정 작업을 수행하는 AEM Assets 내의 서비스입니다. 예를 들어 MP3 오디오 파일이 AEM에 업로드되면 워크플로우에서는 메타데이터를 추출하고 축소판을 생성하는 MP3 핸들러를 트리거합니다. 미디어 핸들러는 일반적으로 워크플로우와 함께 사용됩니다. AEM 내에서 가장 일반적인 MIME 유형이 지원됩니다. 워크플로우를 확장/생성하고 미디어 핸들러를 확장/만들거나 미디어 핸들러를 비활성화/활성화하여 자산에 대해 특정 작업을 수행할 수 있습니다.

>[!NOTE]
>
>AEM Assets에서 지원하는 모든 형식 및 각 형식에 지원되는 기능에 대한 설명은 [자산 지원 형식](assets-formats.md) 페이지를 참조하십시오.
...
AEM에는 자산을 처리하는 몇 가지 기본 워크플로우가 있습니다. 이 항목을 보려면 워크플로우 콘솔을 열고 **[!UICONTROL 모델]** 탭을 클릭합니다.aem assets으로 시작하는 워크플로우 제목은 자산별 제목입니다.
...
1. Eclipse에서 `myBundle` Maven 프로젝트를 만듭니다.
   ...
   1. 대화 상자에서 Maven 폴더를 확장하고 Maven 프로젝트를 선택하고 **[!UICONTROL 다음]**&#x200B;을 클릭합니다.
...
1. `myBundle/src/main/java` 아래에 Java 클래스가 들어 있는 패키지 `com.day.cq5.myhandler`을 만듭니다.
...
1. Java 클래스 `MyHandler` 만들기:
   ...
   1. 대화 상자 창에서 Java 클래스 MyHandler의 이름을 지정하고 [마침]을 클릭합니다. Eclipse는 MyHandler.java 파일을 만들고 엽니다.
...
1. 번들 `myBundle-0.0.1-SNAPSHOT.jar`을(를) 복사하고 `/apps/myApp/install` 아래에 저장합니다(예: WebDAV). 이제 AEM에서 새 텍스트 핸들러가 활성화됩니다.
...
AEM을 사용하면 워크플로우 내에서 명령줄 툴을 실행하여 에셋(예: ImageMagick)을 변환하고 에셋에 새 변환을 추가할 수 있습니다. AEM 서버를 호스팅하는 디스크에 명령줄 도구를 설치하고 워크플로우에 프로세스 단계를 추가하고 구성해야 합니다. 호출된 프로세스(`CommandLineProcess`)는 특정 MIME 유형에 따라 필터링하고 새 변환에 따라 여러 축소판을 만들 수도 있습니다.
...
>[!NOTE]
>
>Windows가 아닌 시스템에서 FPMpeg 도구는 파일 이름에 작은 따옴표(&#39;)가 있는 비디오 자산에 대한 변환을 생성하는 동안 오류를 반환합니다. 비디오 파일 이름에 단일 견적이 포함된 경우 AEM에 업로드하기 전에 해당 견적을 제거합니다.
...
다음 예제에서는 MIME 형식 gif 또는 tiff가 있는 에셋이 AEM 서버의 /content/dam에 추가될 때마다 역진행 이미지가 3개의 추가 축소판(140x100, 48x48 및 10x250)과 함께 작성되도록 명령줄 프로세스 단계를 설정하는 방법을 보여 줍니다.

이렇게 하려면 ImageMagick을 사용합니다. AEM 서버를 호스팅하는 디스크에 ImageMagick을 설치합니다.
...
1. 도구가 제대로 설치되었는지 확인하려면 명령줄에서 `convert -h` 명령을 실행하십시오.
   ...
   >[!NOTE]
   >
   >일부 버전의 Windows(예: Windows SE)에서는 변환 명령이 Windows 설치에 포함된 기본 변환 유틸리티와 충돌하기 때문에 실행되지 않을 수도 있습니다. 이 경우 이미지 파일을 축소판으로 변환하는 데 사용되는 ImageMagick 유틸리티의 전체 경로를 언급합니다. 예, `"C:\Program Files\ImageMagick-6.8.9-Q16\convert.exe" -define jpeg:size=319x319 ${filename} -thumbnail 319x319 cq5dam.thumbnail.319.319.png`.
...
| cmd:&lt;command> | 실행할 명령을 정의합니다. 구문은 명령줄 도구에 따라 다릅니다. 하나의 명령만 정의할 수 있습니다. <br>다음 변수를 사용하여 명령을 만들 수 있습니다<br>`${filename}`.입력 파일의 이름(예: original.jpg)  <br> `${file}`:입력 파일의 전체 경로 이름(예: /tmp/cqdam0816.tmp/original.jpg)  <br> `${directory}`:입력 파일의 디렉토리(예: /tmp/cqdam0816.tmp  <br>`${basename}`:입력 파일의 확장명이 없는 이름(예: 원본  <br>`${extension}`:입력 파일의 확장명(예: jpg) |

예를 들어 AEM 서버를 호스팅하는 디스크에 ImageMagick이 설치되어 있고 **CommandLineProcess**&#x200B;를 구현으로 사용하여 프로세스 단계를 만드는 경우 다음 값을 **프로세스 인수**&#x200B;로 사용합니다.

`mime:image/gif,mime:image/tiff,tn:140:100,tn:48:48,tn:10:250,cmd:convert ${directory}/${filename} -flip ${directory}/${basename}.flipped.jpg`

그런 다음 워크플로우가 실행되면 이 단계는 이미지/gif 또는 mime:image/tiff가 있는 에셋에만 적용되며, 전환된 원본 이미지를 만들고 .jpg로 변환하며 크기가 다음 3개의 축소판을 만듭니다.140x100, 48x48 및 10x250.
