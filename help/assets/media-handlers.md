...
description: 다양한 미디어 핸들러와 워크플로우에서 이러한 핸들러를 사용하여 자산에 대한 작업을 수행하는 방법에 대해 알아보십시오.
contentOwner: AG
...
# 자산 처리 미디어 핸들러 및 작업 과정을 사용하여 {#processing-assets-using-media-handlers-and-workflows}

Adobe Experience Manager Assets는 자산을 처리하기 위한 기본 워크플로우 및 미디어 핸들러 집합을 제공합니다. 워크플로우는 일반적인 에셋 관리 및 처리 작업을 정의한 다음 축소판 생성 또는 메타데이터 추출 등의 미디어 핸들러에 특정 작업을 위임합니다.

특정 유형 또는 형식의 자산이 서버에 업로드될 때 자동으로 실행되도록 워크플로우를 정의할 수 있습니다. 처리 단계는 일련의 Experience ManagerAssets 미디어 핸들러로 정의됩니다. Adobe Experience Manager에서는 일부 [내장 핸들러](#default-media-handlers) 등이 [사용자 정의 개발](#creating-a-new-media-handler)이거나 프로세스를 [명령줄 도구](#command-line-based-media-handler)로 위임하여 정의할 수 있습니다.

미디어 핸들러는 자산에 대한 특정 작업을 수행하는 Experience Manager 자산 내의 서비스입니다. 예를 들어 MP3 오디오 파일을 Experience Manager에 업로드하면 작업 과정에서는 메타데이터를 추출하고 축소판을 생성하는 MP3 핸들러를 트리거합니다. 미디어 핸들러는 워크플로우에 사용됩니다. 대부분의 일반 MIME 유형은 Experience Manager 내에서 지원됩니다. 다음 중 하나를 수행하여 자산에 대한 특정 작업을 수행할 수 있습니다
...
>[!NOTE]
>
>각 형식에 지원되는 Experience Manager 자산 및 기능에 대한 설명은 [자산 지원 형식](assets-formats.md) 페이지를 참조하십시오.

## 기본 미디어 핸들러 {#default-media-handlers}

다음 미디어 핸들러는 Experience Manager 에셋 내에서 사용할 수 있으며 가장 일반적인 MIME 형식을 처리합니다.
...
Experience Manager에는 자산을 처리하는 몇 가지 기본 워크플로우가 있습니다. 이 항목을 보려면 워크플로우 콘솔을 열고 **[!UICONTROL 모델]** 탭을 클릭합니다.Experience Manager 자산으로 시작하는 워크플로우 제목은 자산별 제목입니다.
...
### 미디어 처리기 {#disabling-enabling-a-media-handler} 비활성화/활성화
...
새 미디어 유형을 지원하거나 자산에서 특정 작업을 실행하려면 미디어 핸들러를 만들어야 합니다. 이 섹션에서는 진행 방법에 대해 설명합니다.
...
* `com.day.cq.dam.api.handler.AssetHandler` 인터페이스:이 인터페이스는 특정 MIME 유형에 대한 지원을 추가하는 서비스에 대해 설명합니다. 이 인터페이스를 구현하려면 MIME 형식을 추가해야 합니다. 인터페이스에는 축소판을 만들고 메타데이터를 추출하기 위해 특정 문서를 가져오거나 내보내는 방법이 포함되어 있습니다.
...
* `com.day.cq.dam.core.AbstractSubAssetHandler` 클래스:
   * 이 클래스는 다른 모든 에셋 처리기 구현의 기본 역할을 하며 일반적인 사용 기능뿐만 아니라 하위 에셋 추출에 사용되는 일반적인 기능을 제공합니다.
...
다음 절차를 수행한 후 텍스트 파일을 Experience Manager에 업로드하면 파일의 메타데이터가 추출되고 워터마크가 있는 2개의 축소판이 생성됩니다.
...
1. Eclipse에서 `myBundle` Maven 프로젝트를 만듭니다.
   ...
   1. **[!UICONTROL 간단한 프로젝트 만들기]** 상자 및 **[!UICONTROL 기본 작업 영역 위치 사용]** 상자를 선택한 다음 **[!UICONTROL 다음]**&#x200B;을 클릭합니다.
   1. 다음 값을 사용하여 Maven 프로젝트를 정의합니다.

      * 그룹 ID:com.day.cq5.myhandler
      * 아티팩트 ID:myBundle
      * 이름:내 Experience Manager 번들
      * 설명:이건 내 Experience Manager 번들이에요
   1. **[!UICONTROL 완료]**&#x200B;를 클릭합니다.


1. Java™ Compiler를 버전 1.5로 설정합니다.
...
1. Java™ 클래스 `MyHandler`을 만듭니다.

   1. Eclipse의 `myBundle/src/main/java`에서 `com.day.cq5.myhandler` 패키지를 마우스 오른쪽 단추로 클릭하고 새로 만들기, 클래스 순으로 선택합니다.
   1. 대화 상자 창에서 Java™ Class MyHandler의 이름을 지정하고 [마침]을 클릭합니다. Eclipse는 MyHandler.java 파일을 만들고 엽니다.
...
1. Java™ 클래스를 컴파일하고 번들을 만듭니다.
   ...
   1. myBundle 프로젝트를 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL Run As]**, **[!UICONTROL Maven Install]**&#x200B;을 선택합니다.
...
1. 번들 `myBundle-0.0.1-SNAPSHOT.jar`을(를) 복사하고 `/apps/myApp/install` 아래에 저장합니다(예: WebDAV). 이제 새 텍스트 핸들러가 Experience Manager에서 활성화됩니다.
...
Experience Manager을 사용하면 워크플로우 내에서 명령줄 도구를 실행하여 에셋을 변환하고(예: ImageMagick) 새 변환을 자산에 추가할 수 있습니다. Experience Manager 서버를 호스팅하는 디스크에 명령줄 도구를 설치하고 워크플로우에 프로세스 단계를 추가하고 구성합니다. 호출된 프로세스(`CommandLineProcess`)는 특정 MIME 유형에 따라 필터링하고 새 변환에 따라 여러 축소판을 만듭니다.

다음 변환은 Experience Manager 자산 내에서 자동으로 실행 및 저장할 수 있습니다.
...
* [LAME](http://lame.sourceforge.net/)을(를) 사용한 MP3 인코딩
* [SOX](http://sox.sourceforge.net/)을(를) 사용한 오디오 처리

>[!NOTE]
>
>Windows가 아닌 시스템에서 FPMpeg 도구는 파일 이름에 작은 따옴표(&#39;)가 있는 비디오 자산에 대한 변환을 생성하는 동안 오류를 반환합니다. 비디오 파일 이름에 단일 견적이 포함된 경우 Experience Manager에 업로드하기 전에 해당 견적을 제거합니다.

`CommandLineProcess` 프로세스는 나열된 순서대로 다음 작업을 수행합니다.

* 지정된 경우 특정 MIME 유형에 따라 파일을 필터링합니다.
* Experience Manager 서버를 호스팅하는 디스크에 임시 디렉토리를 만듭니다.
* 원본 파일을 임시 디렉토리로 스트리밍합니다.
* 단계의 인수로 정의된 명령을 실행합니다. Experience Manager을 실행하는 사용자의 권한이 있는 임시 디렉터리 내에서 명령이 실행되고 있습니다.
* 결과를 Experience Manager 서버의 변환 폴더에 다시 스트리밍합니다.
* 임시 디렉토리를 삭제합니다.
* 지정된 경우 이러한 변환에 따라 축소판을 만듭니다. 축소판의 수와 크기는 단계의 인수로 정의됩니다.

### ImageMagick {#an-example-using-imagemagick} 사용 예

다음 예제에서는 명령줄 프로세스 단계를 설정하는 방법을 보여 줍니다. Experience Manager 서버의 `/content/dam`에 MIME 유형 gif 또는 tiff가 포함된 에셋을 추가할 때마다 원본의 역진행 이미지가 3개의 축소판(140x100, 48x48 및 10x250)과 함께 만들어집니다.

이 프로세스 단계를 수행하려면 ImageMagick을 사용합니다. Experience Manager 서버를 호스팅하는 디스크에 ImageMagick을 설치합니다.

1. ImageMagick을 설치합니다. 자세한 내용은 [ImageMagick 설명서](https://www.imagemagick.org/script/download.php)를 참조하십시오.
...
1. 도구가 제대로 설치되었는지 확인하려면 명령줄에서 `convert -h` 명령을 실행하십시오.
   ...
   >[!NOTE]
   >
   >일부 버전의 Windows®(예: Windows® SE)에서는 변환 명령이 Windows® 설치에 포함된 기본 변환 유틸리티와 충돌하기 때문에 실행되지 않습니다. 이 경우 이미지 파일을 축소판으로 변환하는 데 사용되는 ImageMagick 유틸리티의 전체 경로를 언급합니다. 예, `"C:\Program Files\ImageMagick-6.8.9-Q16\convert.exe" -define jpeg:size=319x319 ${filename} -thumbnail 319x319 cq5dam.thumbnail.319.319.png`.

1. 도구가 제대로 실행되는지 확인하려면 작업 디렉토리에 JPG 이미지를 추가하고 명령줄에서 `convert <image-name>.jpg -flip <image-name>-flipped.jpg` 명령을 실행합니다.
...
1. `myImage.tiff` 에셋을 열고 역진행 이미지와 3개의 축소판이 만들어졌는지 확인합니다.

#### CommandLineProcess 프로세스 단계 {#configuring-the-commandlineprocess-process-step} 구성

이 섹션에서는 `CommandLineProcess`의 **[!UICONTROL 프로세스 인수]**&#x200B;를 설정하는 방법을 설명합니다. 쉼표를 사용하여 [!UICONTROL 프로세스 인수]의 값을 구분하고 공백으로 값을 시작하지 마십시오.
...
| cmd:&lt;command> | 실행되는 명령을 정의합니다. 구문은 명령줄 도구에 따라 다릅니다. 하나의 명령만 정의할 수 있습니다. <br>다음 변수를 사용하여 명령을 만들 수 있습니다<br>`${filename}`.입력 파일의 이름(예: original.jpg)  <br> `${file}`:입력 파일의 전체 경로 이름(예: /tmp/cqdam0816.tmp/original.jpg)  <br> `${directory}`:입력 파일의 디렉토리(예: /tmp/cqdam0816.tmp  <br>`${basename}`:입력 파일의 확장명이 없는 이름(예: 원본  <br>`${extension}`:입력 파일의 확장명(예: jpg) |

예를 들어 Experience Manager 서버를 호스팅하는 디스크에 ImageMagick이 설치되어 있고 **CommandLineProcess**&#x200B;를 구현으로 사용하여 프로세스 단계를 만드는 경우 다음 값을 **프로세스 인수**&#x200B;로 사용합니다.

`mime:image/gif,mime:image/tiff,tn:140:100,tn:48:48,tn:10:250,cmd:convert ${directory}/${filename} -flip ${directory}/${basename}.flipped.jpg`

그런 다음 워크플로우를 실행할 때 이 단계는 MIME 유형으로 `image/gif` 또는 `mime:image/tiff`이 있는 자산에만 적용됩니다. 원본 이미지의 대칭 이미지를 만들고 .jpg로 변환하며 크기가 다음 3개의 축소판을 만듭니다.140x100, 48x48 및 10x250.

ImageMagick을 사용하여 다음 [!UICONTROL Process 인수]를 사용하여 3개의 표준 축소판을 만듭니다.

`mime:image/tiff,mime:image/png,mime:image/bmp,mime:image/gif,mime:image/jpeg,cmd:convert ${filename} -define jpeg:size=319x319 -thumbnail "319x319>" -background transparent -gravity center -extent 319x319 -write png:cq5dam.thumbnail.319.319.png -thumbnail "140x100>" -background transparent -gravity center -extent 140x100 -write cq5dam.thumbnail.140.100.png -thumbnail "48x48>" -background transparent -gravity center -extent 48x48 cq5dam.thumbnail.48.48.png`

ImageMagick을 사용하여 웹이 활성화된 변환을 만들려면 다음 [!UICONTROL Process 인수]를 사용합니다.
