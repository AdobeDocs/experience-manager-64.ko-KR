...
AEM(Adobe Experience Manager) 자산에서 메타데이터 스키마는 속성 페이지의 레이아웃과 특정 스키마를 사용하는 자산에 대해 표시되는 메타데이터 속성을 정의합니다. 메타데이터 속성에는 제목, 설명, MIME 유형, 태그 등이 포함됩니다.

메타데이터 스키마 양식 편집기를 사용하여 기존 스키마를 수정하거나 사용자 정의 메타데이터 스키마를 추가할 수 있습니다.

1. 자산에 대한 속성 페이지를 보려면 카드 보기의 자산 타일의 **[!UICONTROL 빠른 작업]****[!UICONTROL 에서 속성]** 보기를 클릭하거나 탭합니다.
...
1. 다양한 탭에서 편집 가능한 메타데이터 속성을 편집합니다.
...
1. 양식/템플릿 목록을 보려면 AEM 로고를 클릭한 다음 **[!UICONTROL 도구 > 자산 > 메타데이터 스키마로 이동합니다]**.
...
1. 사용자 지정 템플릿을 목록에 추가하려면 도구 모음에서 **[!UICONTROL 만들기를]** 클릭합니다.
   ...
   >[!NOTE]
   >
   >편집되지 않은 템플릿은 그 앞에 잠금 아이콘을 표시합니다. 템플릿을 사용자 지정하면 템플릿이 사라지기 전에 잠금 아이콘이 사라집니다.
...
1. [ **[!UICONTROL 메타데이터 스키마 편집기]** ] 페이지에서 [양식 **[!UICONTROL 작성] 탭의 구성 요소 유형 목록에서 하나 이상의 구성 요소를]** 기본 **[!UICONTROL 탭으로 드래그하여 자산의 속성 페이지를 사용자]** 정의합니다.
...
양식에서 메타데이터 구성 요소의 속성을 편집하려면 구성 요소를 클릭하고 설정 **[!UICONTROL 탭에서 다음 속성의 전체 또는 하위 집합을]** 편집합니다.

**필드 레이블**: 자산의 속성 페이지에 표시되는 메타데이터 속성의 이름입니다.

**속성에 매핑**: 이 속성은 CRX 저장소에 저장된 자산 노드의 상대 경로/이름을 지정합니다. 경로가 자산의 노드 아래에 있음을 `./` 나타내므로 시작합니다.

다음은 이 속성에 유효한 값입니다.

* `./jcr:content/metadata/dc:title`: 자산의 메타데이터 노드에 값을 속성으로 저장합니다 `dc:title`.

* `./jcr:created`: 자산의 노드에 JCR 속성을 표시합니다. 보기 속성에 이러한 속성을 구성하는 경우 해당 속성이 보호되므로 편집 비활성화 상태로 표시하는 것이 좋습니다. 그렇지 않으면 에셋의 [!UICONTROL 속성을 저장할 때 오류] 에셋이 결과를 수정하지 못했습니다.
...
**삭제 아이콘** 스키마 양식에서 구성 요소를 삭제하려면 이 아이콘을 클릭합니다.
...
### JSON 파일에서 속성 지정 {#specifying-properties-in-json-file}
...
스키마 편집기를 사용하면 탭을 추가하거나 삭제할 수 있습니다. 기본 스키마 양식에는 **[!UICONTROL 기본]**, **[!UICONTROL 고급]**, **[!UICONTROL IPTC]**&#x200B;및 **[!UICONTROL IPTC 확장]** 탭이 기본적으로 포함되어있습니다.

![chlimage_1-181](assets/chlimage_1-181.png)

스키마 양식 `+` 에 새 탭을 추가하려면 클릭합니다. 기본적으로 새 탭에 이름이 지정됩니다 `Unnamed-1`. 설정 탭에서 이름을 수정할 수 **[!UICONTROL 있습니다]** . 탭 `X` 을 삭제하려면 클릭합니다.
...
>[!NOTE]
>
>기본 양식에 대한 사용자 지정 변경 사항을 삭제하면 이 잠금 아이콘이 메타데이터 스키마 인터페이스에 다시 표시되어 양식이 기본 상태로 돌아갔다는 것을 나타냅니다.
...
적절한 양식 유형 아래에서 새 양식을 만듭니다. 예를 들어 하위 유형에 대한 새 템플릿을 추가하려면 `image/png` 양식 아래에 양식을 `image` 만듭니다. 스키마 양식의 제목은 하위 유형 이름입니다. 이 경우 제목은 입니다 `png`.
...
이 경우 CRX 저장소의 새 노드 `/etc/dam/metadataeditor/mimetypemappings` 를 만듭니다. 노드의 이름을 지정하고 다음 속성을 정의합니다.
...
메타데이터 스키마 기능은 관리자만 사용할 수 있습니다. 그러나 관리자는 폴더에 대한 **[!UICONTROL 만들기]**, 수정 **[!UICONTROL 및]**&#x200B;삭제 **[!UICONTROL 권한을 제공하여 관리자가 아닌 사용자]** 에게 `/conf` 액세스 권한을 제공할수있습니다.
...
1. 폴더에 다른 메타데이터 스키마가 적용된 경우 기존 메타데이터 스키마를 덮어쓸지 묻는 메시지가 나타납니다. 덮어쓰기를 **[!UICONTROL 클릭합니다]**.
1. 확인 **[!UICONTROL 을]** 클릭하여 성공 메시지를 닫습니다.
...
>[!NOTE]
>
>예약된 확인 후에 자산이 추가되는 경우, 자산은 다음 예약된 확인 `hasValidMetadata` 이 될 때까지 플래그로 표시되지 않습니다. 자산은 중간 검색 결과에 나타나지 않습니다.
