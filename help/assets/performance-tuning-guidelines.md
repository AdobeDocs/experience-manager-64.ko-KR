...
S3 또는 공유 파일 데이터 저장소를 구현하면 대규모 구현에서 디스크 공간을 절약하고 네트워크 처리량을 늘리는 데 도움이 될 수 있습니다. 공유 데이터 저장소 사용에 대한 장단점에 대한 자세한 내용은 자산 크기 [조정 안내서를 참조하십시오](assets-sizing-guide.md).
...
1. 구성할 AEM `http://localhost:4502/miscadmin` 인스턴스에서 엽니다.

1. 탐색 트리에서 도구 > **[!UICONTROL 워크플로우]** > **[!UICONTROL 모델]** > **** 댐 ****&#x200B;를 확장합니다.
1. DAM 자산 **[!UICONTROL 업데이트를 두 번 클릭합니다]**.
1. 부동 도구 패널에서 페이지 **[!UICONTROL 탭으로]** 전환한 다음 페이지 속성을 **[!UICONTROL 클릭합니다]**.
1. 임시 **[!UICONTROL 워크플로우]** 선택 확인을 **[!UICONTROL 클릭합니다]**.
...
비디오 트랜스코딩과 같이 리소스를 많이 사용하는 워크플로우나 워크플로우의 경우 DAM 자산 업데이트 워크플로우를 두 번째 작성자 인스턴스로 옮길 수 있습니다. 종종 오프로드 시 문제는 워크플로우 처리를 오프로드하여 저장되는 모든 로드는 인스턴스 간에 컨텐츠를 복제하는 비용에 의해 상쇄됩니다.

AEM 6.2부터 AEM 6.1용 기능 팩이 있으면 바이너리 없는 복제를 통해 오프로딩을 수행할 수 있습니다. 이 모델에서 작성자 인스턴스는 일반적인 데이터 저장소를 공유하고 앞으로 복제를 통해 메타데이터를 주고 받을 수만 있습니다. 이 방법이 공유 파일 데이터 저장소에서 제대로 작동하지만 S3 데이터 저장소에 문제가 있을 수 있습니다. 백그라운드 쓰기 스레드는 지연을 유발할 수 있기 때문에 오프로딩 작업이 시작되기 전에 에셋이 데이터 저장소에 기록되지 않았을 수 있습니다.
...
ImageMagick을 사용하여 표현물을 생성하기 위해 DAM 자산 업데이트 워크플로우를 사용자 지정하는 경우, Adobe에서는 policy.xml 파일을 */etc/ImageMagick/에서 수정할 것을 권장합니다*. 기본적으로 ImageMagick은 OS 볼륨에서 사용 가능한 전체 디스크 공간과 사용 가능한 메모리를 사용합니다. policy.xml의 `policymap` 섹션 내에서 다음 구성을 변경하여 이러한 리소스를 제한합니다.
...
또한 *configure.xml* 파일에서 ImageMagick의 임시 폴더 경로를 충분한 공간과 IOPS가 있는 디스크 파티션으로 설정합니다(또는 환경 변수 `MAGIC_TEMPORARY_PATH`설정).

>[!CAUTION]
>
>ImageMagick이 사용 가능한 모든 디스크 공간을 사용하는 경우 구성이 잘못되면 서버가 불안정해질 수 있습니다. ImageMagick을 사용하여 대용량 파일을 처리하는 데 필요한 정책 변경 사항이 AEM 성능에 영향을 줄 수 있습니다. 자세한 내용은 ImageMagick [설치 및 구성을 참조하십시오](best-practices-for-imagemagick.md).
...
1. 각 복제 에이전트에서 트리거 **[!UICONTROL 탭에서 수신]** 시 **[!UICONTROL 를]** 활성화합니다.
...
자주 실행하는 쿼리에 대한 사용자 정의 인덱스를 만듭니다. 자세한 내용은 슬로우 쿼리 [분석 및](https://aemfaq.blogspot.com/2014/08/oak-query-log-file-analyzer-tool.html) 사용자 정의 인덱스 [](/help/sites-deploying/queries-and-indexing.md)작성을 위한 방법을 참조하십시오. 쿼리 및 색인 우수 사례에 대한 추가 인사이트를 보려면 쿼리 및 [색인 작성 우수 사례를 참조하십시오](/help/sites-deploying/best-practices-for-queries-and-indexing.md).
...
1. AEM **[!UICONTROL 6.2 이전 버전에서 CopyOnRead, CopyOnWrite 및 프리페치 인덱스 파일을]** 활성화합니다.이러한 값은 AEM 6.2 이상 버전에서 기본적으로 활성화됩니다.
...
1. 값이 &quot;/content/dam[] &quot; **[!UICONTROL 인 includedPaths라는]** 문자열 속성을 추가합니다.
1. 저장

(AEM6.1 및 6.2만 해당) ntBaseLucene 인덱스를 업데이트하여 자산 삭제 및 이동 성능을 개선합니다.

1. /oak:index/ntBaseLucene/indexRules/nt:base/properties 탐색 **
1. /oak:index/ntBaseLucene **[!UICONTROL /indexRules/nt:base/]** properties **[!UICONTROL 아래에 두 개의 nt:unstructured nodes slingResource 및]** *damResolvedPath를 추가합니다.*
