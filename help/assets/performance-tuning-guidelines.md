---
title: 자산 성능 조정 가이드
description: 주요 포커스 영역 [!DNL Experience Manager] 병목 현상을 제거하고 성능을 최적화하기 위해 하드웨어, 소프트웨어 및 네트워크 구성 요소의 구성, 변경 [!DNL Experience Manager] 자산.
contentOwner: AG
...
또한 특정 하드웨어 및 소프트웨어 최적화 지침을 식별하고 준수하면 [!DNL Experience Manager] 성능, 확장성 및 안정성에 대한 기대치를 충족하는 자산 배포

성능 저하 [!DNL Experience Manager] 자산은 사용자 환경에 대화형 성능, 자산 처리, 다운로드 속도 및 기타 영역에 영향을 줄 수 있습니다.
...
While [!DNL Experience Manager] 는 다양한 플랫폼에서 지원됩니다. Adobe은 Linux 및 Windows에서 기본 도구에 대한 가장 많은 지원을 발견하여 최적의 성능과 구현이 용이하도록 기여합니다. 가장 좋은 방법은 64비트 운영 체제를 배포하여 [!DNL Experience Manager] 자산 배포. 다른 어떤 것과도 마찬가지로 [!DNL Experience Manager] 배포하려면 가능한 모든 곳에서 TarMK를 구현해야 합니다. TarMK는 단일 작성자 인스턴스를 넘어 확장할 수 없지만 MongoMK보다 성능이 뛰어난 것으로 나타났습니다. TarMK 오프로드 인스턴스를 추가하여 워크플로우의 처리 능력을 높일 수 있습니다 [!DNL Experience Manager] 자산 배포.

### 임시 폴더 {#temp-folder}

자산 업로드 시간을 개선하려면 Java temp 디렉토리에 대한 고성능 저장소를 사용합니다. Linux 및 Windows에서는 RAM 드라이브 또는 SSD를 사용할 수 있습니다. 클라우드 기반 환경에서는 동등한 고속 스토리지 유형을 사용할 수 있습니다. Amazon EC2에서, [사용 후 드라이브](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/InstanceStorage.html) temp 폴더에 드라이브를 사용할 수 있습니다.
...
2015년 4월 현재 Oracle이 Java 7에 대한 업데이트 릴리스를 중단했으므로 Adobe은 배포를 권장합니다. [!DNL Experience Manager] Java 8의 자산. 일부 경우에는 성능이 개선되었습니다.
...
모든 세그먼트에 대해 데이터 저장소를 세그먼트 저장소에서 분리하는 것이 좋습니다 [!DNL Experience Manager] 자산 사용자. 또한, `maxCachedBinarySize` 및 `cacheSizeInMB` 매개 변수를 사용하면 성능을 극대화할 수 있습니다. 설정 `maxCachedBinarySize` 캐시에 저장할 수 있는 가장 작은 파일 크기로 가져옵니다. 내의 데이터 저장소에 사용할 메모리 내 캐시의 크기를 지정합니다 `cacheSizeInMB`. Adobe은 이 값을 총 heap 크기의 2-10% 이내로 설정할 것을 권장합니다. 그러나 로드/성능 테스트는 이상적인 설정을 결정하는 데 도움이 될 수 있습니다.

### 버퍼된 이미지 캐시의 최대 크기 구성 {#configure-the-maximum-size-of-the-buffered-image-cache}

많은 양의 자산을 Adobe Experience Manager에 업로드할 때, 메모리 소비에서 예기치 않은 스파이크가 발생하고 JVM이 OutOfMemoryErrors로 실패하지 않도록 버퍼링된 이미지 캐시의 구성된 최대 크기를 줄입니다. 시스템이 최대 힙이 있는 예를 생각해 보십시오(- `Xmx`매개 변수)(5GB), Oak BlobCache는 1GB로 설정되고 문서 캐시는 2GB로 설정됩니다. 이 경우 버퍼링된 캐시는 최대 1.25GB와 메모리가 필요하므로 예기치 않은 스파이크에 대해서는 0.75GB 메모리만 남게 됩니다.
...
From [!DNL Experience Manager] 6.1 SP1(를 사용하는 경우) `sling:osgiConfig` 이 속성을 구성하려면 데이터 형식을 Long으로 설정해야 합니다. 자세한 내용은 [CQBufferedImageCache는 자산 업로드 중 힙을 사용합니다](https://helpx.adobe.com/experience-manager/kb/cqbufferedimagecache-consumes-heap-during-asset-uploads.html).

### 공유 데이터 저장소 {#shared-data-stores}

S3 또는 공유 파일 데이터 저장소를 구현하면 디스크 공간을 절약하고 대규모 구현에서 네트워크 처리량을 늘리는 데 도움이 될 수 있습니다. 공유 데이터 저장소 사용의 장단점에 대한 자세한 내용은 [Assets 크기 조정 가이드](assets-sizing-guide.md).
...
* 인스턴스 유형에 따라(작은, 중간, 큰) 네트워크 대역폭이 충분한지 확인합니다. [!DNL Experience Manager] 인스턴스. 적절한 대역폭 할당은 특히 [!DNL Experience Manager] 는 AWS에서 호스팅됩니다.
...
>[!NOTE]
>
>기본적으로 DAM 자산 업데이트 워크플로우는 의 임시 로 설정됩니다 [!DNL Experience Manager] 6.3. 이 경우 다음 절차를 건너뛸 수 있습니다.

1. 열기 `http://localhost:4502/miscadmin` on [!DNL Experience Manager] 구성할 인스턴스입니다.

1. From the navigation tree, expand **[!UICONTROL Tools]** > **[!UICONTROL Workflow]** > **[!UICONTROL Models]** > **[!UICONTROL dam]**.
1. 두 번 클릭 **[!UICONTROL DAM 자산 업데이트]**.
1. 부동 도구 패널에서 **[!UICONTROL 페이지]** 탭을 클릭한 다음 **[!UICONTROL 페이지 속성]**.
1. 선택 **[!UICONTROL 임시 워크플로우]** 클릭 **[!UICONTROL 확인]**.

   >[!NOTE]
   >
   >일부 기능은 임시 워크플로우를 지원하지 않습니다. 만약 [!DNL Experience Manager] 자산 배포에는 이러한 기능이 필요하며 임시 워크플로우를 구성하지 않습니다.
   ...
   예를 들어, 워크플로우 인스턴스 노드를 만드는 비임시 워크플로우를 실행한 후 실행할 수 있습니다 [ACS AEM Commons Workflow Remover](https://adobe-consulting-services.github.io/acs-aem-commons/features/workflow-remover.html) 임시로. Granite Workflow 제거 스케줄러가 실행될 때까지 기다리지 않고 중복되고 완료된 워크플로우 인스턴스를 즉시 제거합니다.

### 최대 병렬 작업 수 {#maximum-parallel-jobs}

기본적으로 [!DNL Experience Manager] 서버의 프로세서 수와 동일한 최대 병렬 작업 수를 실행합니다. 이 설정의 문제는 로드가 많은 동안 모든 프로세서가 DAM 자산 업데이트 워크플로우에 의해 사용되고 UI 응답성이 느려지고 방지된다는 것입니다 [!DNL Experience Manager] 서버 성능 및 안정성을 보호하는 다른 프로세스 실행 이 값은 다음 단계를 수행하여 서버에서 사용할 수 있는 프로세서의 절반으로 설정하는 것이 좋습니다.

1. 설정 [!DNL Experience Manager] 작성자, 이동 [http://localhost:4502/system/console/slingevent](http://localhost:4702/system/console/slingevent).
...
기준 [!DNL Experience Manager] 6.2 및 [!DNL Experience Manager] 6.1 바이너리 없는 복제 기능을 사용하여 오프로드를 수행할 수 있습니다. 이 모델에서 작성자 인스턴스는 공통 데이터 저장소를 공유하고 순방향 복제만 통해 메타데이터를 앞뒤로 보냅니다. 이 접근 방식이 공유 파일 데이터 저장소에서 잘 작동하지만 S3 데이터 저장소에 문제가 있을 수 있습니다. 백그라운드 쓰기 스레드는 지연을 유도할 수 있으므로 오프로딩 작업이 시작되기 전에 자산이 데이터 저장소에 기록되지 않았을 수 있습니다.
...
또 다른 방법은 Dynamic Media Classic 기술을 사용하여 이미지 조작을 전적으로 전달하는 것입니다. 또한 Adobe Mobile Services에서 렌디션 생성 책임을 맡을 뿐만 아니라 [!DNL Experience Manager] 인프라뿐만 아니라 전체 게시 계층도 포함됩니다.

#### ImageMagick {#imagemagick}

ImageMagick를 사용하여 렌디션을 생성하도록 DAM 자산 업데이트 워크플로우를 사용자 지정하는 경우 policy.xml 파일을에서 수정하는 것이 좋습니다. */etc/ImageMagick/*. 기본적으로 ImageMagick는 OS 볼륨의 사용 가능한 전체 디스크 공간과 사용 가능한 메모리를 사용합니다. 내에서 다음 구성을 변경합니다. `policymap` policy.xml 섹션을 참조하십시오.
...
>[!CAUTION]
>
>ImageMagick가 사용 가능한 모든 디스크 공간을 사용하는 경우 구성이 잘못되면 서버가 불안정해질 수 있습니다. ImageMagick를 사용하여 큰 파일을 처리하는 데 필요한 정책 변경 사항은 [!DNL Experience Manager] 성능. 자세한 내용은 [ImageMagick 설치 및 구성](best-practices-for-imagemagick.md).

>[!NOTE]
>
>ImageMagick `policy.xml` 및 `configure.xml` 파일은 `/usr/lib64/ImageMagick-*/config/` 대신 `/etc/ImageMagick/`. 자세한 내용은 [ImageMagick 설명서](https://www.imagemagick.org/script/resources.php) 구성 파일 위치에 대한 자세한 내용을 참조하십시오.
...
XMP 원본에 쓰기 기능은 AEM에서 메타데이터가 수정될 때마다 원래 자산을 업데이트하여 다음 결과를 생성합니다.
...
1. 이러한 각 복제 에이전트에서 **[!UICONTROL 수신 시]** on **[!UICONTROL Triggers]** 탭
...
시스템 인덱스에 대한 업데이트가 자주 포함되므로 최신 서비스 팩 및 성능 관련 핫픽스를 구현하십시오. 자세한 내용은 [성능 조정 팁 | 6.x](https://helpx.adobe.com/experience-manager/kb/performance-tuning-tips.html) AEM 버전에 따라 적용할 수 있는 일부 색인 최적화의 경우.
...
개선을 위해 Oak 색인 구성에서 일부 최적화를 수행할 수 있습니다 [!DNL Experience Manager] 자산 성능:
...
1. 활성화 **[!UICONTROL CopyOnRead , CopyOnWrite 및 미리 가져오기 인덱스 파일]** 이전 버전에서 [!DNL Experience Manager] 6.2. 이러한 값은 기본적으로 [!DNL Experience Manager] 6.2 이상 버전.

색인 구성을 업데이트하여 다시 인덱싱 시간을 개선합니다.

1. CRXDe /crx/de/index.jsp 를 열고 관리 사용자로 로그인합니다
1. /oak:index/lucene으로 이동합니다.
1. 문자열 추가[] 명명된 속성 **[!UICONTROL excludedPaths]** &quot;/var&quot;, &quot;/etc/workflow/instances&quot; 및 &quot;/etc/replication&quot; 값이 있는 경우
1. /oak:index/damAssetLucene으로 이동합니다.
1. 문자열 추가[] 명명된 속성 **[!UICONTROL 포함된 경로]** &quot;/content/dam&quot; 값이 한 개 있음
1. 저장

(AEM6.1 및 6.2만 해당) ntBaseLucene 인덱스를 업데이트하여 자산 삭제 및 이동 성능을 개선합니다.

1. 찾아보기 */oak:index/ntBaseLucene/indexRules/nt:base/properties*
1. 구조화되지 않은 노드 두 개 추가 **[!UICONTROL slingResource]** 및 **[!UICONTROL damResolvedPath]** 아래에 */oak:index/ntBaseLucene/indexRules/nt:base/properties*
1. 노드(ordered 및 propertyIndex 속성이 형식인 경우)에서 아래 속성을 설정합니다 *부울*:
...
1. 색인화가 완료되면 CRXDe로 돌아가서 를 설정합니다. **[!UICONTROL 유형]** 이 두 인덱스에 대해 속성을 사용할 수 없습니다.
...
1. 로 이동합니다. [!DNL Experience Manager] 패키지 관리자 /crx/packmgr/index.jsp
...
AEM에는 대형 파일과 관련된 것으로 알려진 두 가지 주요 문제가 있습니다. 파일이 2GB보다 큰 크기에 도달하면 콜드 대기 동기화가 메모리 부족 상태로 발생할 수 있습니다. 대기 동기화가 실행되지 않는 경우도 있습니다. 다른 경우에는 기본 인스턴스가 충돌합니다. 이 시나리오는 의 모든 파일에 적용됩니다. [!DNL Experience Manager] 컨텐츠 패키지를 포함하여 2GB보다 큽니다.
...
효율적인 CPU 사용률 및 로드 공유를 통해 지연을 최소화하고 높은 처리량을 얻으려면 성능을 모니터링하십시오 [!DNL Experience Manager] 인스턴스를 정기적으로 표시합니다. 특히

* 에 대해 로드 테스트 실행 [!DNL Experience Manager] 인스턴스
...
* 사용 `guessTotal` 쿼리 성능을 최적화합니다.
* 다음을 구성하는 경우 [!DNL Experience Manager] 파일 컨텐츠에서 파일 유형을 검색하려면 다음을 구성합니다 [!UICONTROL 일 CQ DAM Mime 유형 서비스] 에서 [!UICONTROL [!DNL Experience Manager] 웹 콘솔]), 작업이 리소스 집약이므로 작업이 비스파이크 시간 동안 많은 파일을 대량으로 업로드하십시오.
