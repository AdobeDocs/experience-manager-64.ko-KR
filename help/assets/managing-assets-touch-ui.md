---
title: ' [!DNL Experience Manager] 자산을 사용하여 디지털 자산을 관리합니다'
description: ' [!DNL Experience Manager] Assets의 터치에 적합한 사용자 인터페이스를 사용하여 수행할 수 있는 다양한 자산 관리 및 편집 작업에 대해 알아봅니다'
contentOwner: AG
mini-toc-levels: 1
...
[!DNL Experience Manager] 자산의 터치에 적합한 사용자 인터페이스를 사용하여 수행할 수 있는 다양한 자산 관리 및 편집 작업에 대해 알아봅니다.

이 문서에서는 Adobe Experience Manager 자산 터치에 적합한 사용자 인터페이스를 사용하여 자산을 관리하고 편집하는 방법에 대해 설명합니다. 사용자 인터페이스에 대한 기본적인 지식이 필요하면 [Touch UI의 기본 처리](/help/sites-authoring/basic-handling.md)를 참조하십시오. 컨텐츠 조각을 관리하려면 [컨텐츠 조각 관리](content-fragments-managing.md) 자산 을 참조하십시오.

## 폴더 만들기 {#create-folders}

자산 컬렉션을 구성할 때(예: 모든 `Nature` 이미지) 폴더를 만들어 함께 유지할 수 있습니다. 폴더를 사용하여 자산을 분류하고 구성할 수 있습니다. [!DNL Experience Manager] 자산에 따라 폴더 내의 자산을 정리하여 보다 효율적으로 작업할 필요가 없습니다.
...
로컬 폴더 또는 네트워크 드라이브에서 [!DNL Experience Manager] Assets로 다양한 유형의 자산(이미지, PDF 파일, RAW 파일 등)을 업로드할 수 있습니다.
...
1. 자산을 업로드하려면 다음 중 하나를 수행합니다.
   ...
   **[!UICONTROL 일시 중지]** 아이콘을 클릭하면 **[!UICONTROL 재생]** 아이콘으로 전환합니다. 업로드를 재개하려면 **[!UICONTROL 재생]** 아이콘을 클릭합니다.
   ...
   진행 중인 업로드를 취소하려면 진행률 표시줄 옆에 있는 `X` 버튼을 클릭합니다. 업로드 작업을 취소하면 [!DNL Experience Manager] 자산이 자산의 부분적으로 업로드된 부분을 삭제합니다.
   ...
   업로드 작업 중에 [!DNL Experience Manager] 는 업로드되는 자산의 일부를 CRX 저장소에 데이터 청크로 저장합니다. 업로드가 완료되면 [!DNL Experience Manager] 은 이 청크를 저장소의 단일 데이터 블록으로 통합합니다.
   ...
   업로드한 자산이 [!DNL Experience Manager] 자산에 있는 경우 **[!UICONTROL 중복 감지]** 대화 상자가 중복 자산을 업로드하려고 한다는 경고를 표시합니다. 이 대화 상자는 기존 자산의 바이너리의 SHA 1 체크섬 값이 업로드하는 자산의 체크섬 값과 일치하는 경우에만 나타납니다. 이 경우 자산의 이름은 중요하지 않습니다. 즉, 바이너리의 SHA 1 값이 동일한 경우 이름이 다른 자산에 대해서도 대화 상자가 표시될 수 있습니다.
   ...
   **[!UICONTROL Keep]**&#x200B;을 눌러 [!DNL Experience Manager] Assets에서 중복 자산을 유지합니다. 업로드한 중복 자산을 삭제하려면 **[!UICONTROL 삭제]**&#x200B;를 누릅니다.

   [!DNL Experience Manager] Assets에서는 파일 이름에 금지된 문자가 있는 자산을 업로드할 수 없습니다. 허용되지 않는 문자가 포함된 자산을 업로드하려고 하면 [!DNL Experience Manager] Assets에 파일 이름에 사용할 수 없는 문자가 있는지에 대한 경고 메시지가 표시되고, 이러한 문자를 제거하거나 허용된 이름으로 업로드할 때까지 업로드를 중지합니다.
   ...
   * 자산 폴더 이름은 `* / : [ \ \ ] | # % { } ? \" . ^ ; + & \t`을 포함하지 않아야 합니다.
   ...
   파일이 업로드되기 전에 업로드 작업을 취소하면 [!DNL Experience Manager] Assets가 현재 파일 업로드를 중지하고 컨텐츠를 새로 고칩니다. 그러나 이미 업로드된 파일은 삭제되지 않습니다.

### 직렬 업로드 {#serial-uploads}

많은 자산을 일괄적으로 업로드하면 상당한 시스템 리소스가 소비되므로 [!DNL Experience Manager] 배포 성능에 부정적인 영향을 줄 수 있습니다. 잠재적인 병목 현상은 인터넷 연결, 디스크의 읽기 쓰기 작업, 동시 자산 업로드 시 POST 요청 수에 대한 웹 브라우저 제한 사항이 될 수 있습니다. 대량 업로드 작업이 실패하거나 조기에 종료될 수 있습니다. 즉, [!DNL Experience Manager] 자산은 여러 파일을 수집하는 동안 일부 파일을 누락하거나 모두 파일을 수집하지 못할 수 있습니다.

이 상황을 극복하기 위해 [!DNL Experience Manager] Assets는 모든 자산을 동시에 섭취하는 대신 일괄 업로드 작업 중에 한 번에 하나의 자산을 섭취(직렬 업로드)합니다.
...
>[!NOTE]
>
>Dynamic Media에서 FTP를 통해 자산을 업로드하려면 - Scene7 모드 설치 기능 팩(FP) 18912([!DNL Experience Manager] 작성자) FP-18912에 액세스하고 FTP 계정 설정을 완료하려면 Adobe 고객 지원 센터에 문의하십시오. 벌크 자산 마이그레이션에 대해서는 [기능 팩 18912 설치](/help/assets/bulk-ingest-migrate.md)를 참조하십시오.
>
>자산 업로드에 FTP를 사용하는 경우 [!DNL Experience Manager]에 지정된 업로드 설정이 무시됩니다. 대신 Dynamic Media Classic에 정의된 대로 파일 처리 규칙이 사용됩니다.
...
1. **[!UICONTROL 업로드]** 페이지의 오른쪽 아래 모서리에서 **[!UICONTROL 업로드]**&#x200B;를 탭합니다.
   ...
   업로드의 진행 상황을 보려면 전역 탐색 모음에서 **[!UICONTROL 작업]**&#x200B;을 누릅니다. **[!UICONTROL 작업]** 페이지에는 업로드의 진행 상태가 표시됩니다. [!DNL Experience Manager]에서 계속 작업하고 언제든지 Dynamic Media Classic의 작업 페이지로 돌아가 진행 중인 작업을 검토할 수 있습니다.
...
| 확장명에 상관없이 동일한 기본 에셋 이름으로 모든 폴더에 덮어쓰기 |  | 업로드하는 파일을 동일한 이름으로 바꾸도록 하려면 이 옵션을 선택합니다. 이 옵션의 이름은 **[!UICONTROL 응용 프로그램 설정]** > **[!UICONTROL 일반 설정]** > **[!UICONTROL 응용 프로그램에 업로드]** > **[!UICONTROL 이미지 덮어쓰기]**&#x200B;의 설정에 따라 다를 수 있습니다. |
...
|  | 하위 폴더 포함 | 업로드할 폴더 내에서 모든 하위 폴더를 업로드합니다. 업로드하는 폴더 및 하위 폴더의 이름은 [!DNL Experience Manager] Assets에 자동으로 입력됩니다. |
...
|  | 일괄처리 집합 사전 설정 | 업로드된 파일에서 이미지 세트 또는 스핀 세트를 만들려면 사용할 사전 설정에 대해 활성 열을 클릭합니다. 두 개 이상의 사전 설정을 선택할 수 있습니다. Dynamic Media Classic의 애플리케이션 설정/일괄처리 세트 사전 설정 페이지에서 사전 설정을 만듭니다.<br> 일괄처리  [집합 사전 설정 만들기에 대한 자세한 내용은 이미지 세트 자동 생성을 위한 일괄처리 집합 사전 ](config-dms7.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets) 설정 구성 및 스핀 세트 를 참조하십시오.<br> 업로드  [시 일괄처리 집합 사전 설정 설정을 참조하십시오](#setting-batch-set-presets-at-upload). |
...
많은 자산을 업로드하면 [!DNL Experience Manager] 서버에 대한 I/O 호출이 크게 증가하여 업로드 효율이 감소하고 시간이 초과될 수도 있습니다. [!DNL Experience Manager] 자산은 자산의 스트리밍되는 업로드를 지원합니다. 스트리밍된 업로드는 저장소에 복사하기 전에 서버의 임시 폴더에 있는 자산 저장소를 방지하여 업로드 작업 중에 디스크 I/O를 줄입니다. 대신 데이터가 리포지토리로 직접 전송됩니다. 이 방법으로 큰 자산을 업로드하는 시간과 시간 초과가 발생할 가능성이 줄어듭니다. 스트리밍된 업로드는 [!DNL Experience Manager] 자산에서 기본적으로 활성화됩니다.

3.1 이하의 서블릿-api 버전을 사용하는 JEE 서버에서 실행되는 [!DNL Experience Manager]에 대해 스트리밍 업로드가 비활성화됩니다.
...
추출이 완료되면 알림 영역에서 [!DNL Experience Manager]이 알림을 보냅니다. [!DNL Experience Manager] 은 ZIP을 추출하는 동안 추출을 중단하지 않고 다시 작업을 시작할 수 있습니다.
...
1. 자산을 복사할 위치로 이동합니다.
   ...
   >[!NOTE]
   >
   >자산을 동일한 위치에 복사하는 경우 [!DNL Experience Manager]은(는) 자동으로 변형된 이름이 생성됩니다. 예를 들어 정사각형이라는 자산을 복사하는 경우 [!DNL Experience Manager]은 해당 사본의 제목을 자동으로 Square1로 생성합니다.
...
1. **[!UICONTROL 표현물]** 패널에서 자산에 대해 생성된 표현물 목록을 확인합니다.
   ...
   >[!NOTE]
   >
   >기본적으로 [!DNL Experience Manager] Assets는 미리 보기 모드에서 자산의 원래 렌디션을 표시하지 않습니다. 관리자의 경우 오버레이를 사용하여 [!DNL Experience Manager] 자산을 구성하여 미리 보기 모드에서 원래 표현물을 표시할 수 있습니다.
...
1. 변환을 보거나 삭제할 변환을 선택합니다.
   ...
   **새 표현물 업로드**
   ...
   이미지 또는 비디오 자산의 세부 사항 페이지에 표시할 표현물에 대한 차원을 구성할 수 있습니다. 지정한 차원에 따라 [!DNL Experience Manager] Assets에 정확히 또는 가장 가까운 차원이 있는 표현물이 표시됩니다.
...
이것은 새로 업로드되거나 업데이트된 모든 자산에 대한 메타데이터를 색인화하고 이를 제안 목록에 추가하기 전에 [!DNL Experience Manager] 자산이 시간 제한 기간(기본적으로 1시간)이 만료될 때까지 대기하기 때문입니다.
...
**[!UICONTROL 선택]** 아이콘을 사용하여 사용 가능한 보기(카드, 열, 목록)가 있는 자산을 보고, 탐색하고 선택할 수 있습니다. **** 카드 보기에서 빠른 작업으로 나타납니다.
...
[!DNL Experience Manager] 자산 인터페이스의 편집 도구를 사용하여 이미지 자산에 대해 작은 편집 작업을 수행할 수 있습니다. 이미지에서 자르기, 회전, 뒤집기 및 기타 편집 작업을 수행할 수 있습니다. 이미지 맵을 자산에 추가할 수도 있습니다.
...
**** 타임라인 에는 컨텐츠  [조각에 관련된 여러 옵션이 포함되어 있습니다](content-fragments-managing.md#timeline-for-content-fragments). 이 기능을 사용하려면  [[!DNL Experience Manager] 6.4 서비스 팩 2(6.4.2.0) ](/help/release-notes/sp-release-notes.md)  이상이 필요합니다.
...
비디오 주석은 HTML5 호환 비디오 형식이 있는 브라우저에서만 지원됩니다. [!DNL Experience Manager] 자산이 지원하는 비디오 형식은 브라우저에 따라 다릅니다.

컨텐츠 조각의 경우 [주석은 편집기](content-fragments-variations.md#annotating-a-content-fragment)에 만들어집니다. 이 기능을 사용하려면 [[!DNL Experience Manager] 6.4 서비스 팩 2(6.4.2.0)](/help/release-notes/sp-release-notes.md) 이상이 필요합니다.
...
1. 알림을 보려면 [!DNL Experience Manager] Aaron MacDonald의 자격 증명으로 자산에 로그인하고 **[!UICONTROL 알림]** 아이콘을 탭하여 알림을 봅니다.
...
1. **[!UICONTROL Print]**&#x200B;를 누릅니다. 2단계에서 선택한 옵션에 따라 생성된 PDF에 지정된 위치에 주석이나 상태가 표시됩니다. 예를 들어 **[!UICONTROL 왼쪽 위]** 설정을 사용하여 주석과 검토 상태를 모두 인쇄하도록 선택하는 경우 생성된 출력은 여기에 표시된 PDF 파일과 유사합니다.
...
1. 오른쪽 상단의 옵션을 사용하여 PDF을 다운로드하거나 인쇄합니다.
   ...
   렌더링된 PDF 파일의 모양(예: 주석 및 상태의 글꼴 색상, 크기 및 스타일, 배경색)을 수정하려면 **[!UICONTROL 구성 관리자]**&#x200B;에서 **[!UICONTROL 주석 PDF 구성]**&#x200B;을 열고 원하는 옵션을 수정합니다. 예를 들어, 승인됨 상태의 표시 색상을 변경하려면 해당 필드에서 색상 코드를 수정합니다. 주석의 글꼴 색상 변경에 대한 자세한 내용은 [주석 달기](managing-assets-touch-ui.md#annotating)를 참조하십시오.

   ![chlimage_1-44](assets/chlimage_1-44.png)

   렌더링된 PDF 파일로 돌아가서 새로 고칩니다. 새로 고친 PDF은 사용자가 변경한 내용을 반영합니다.

**외국어로 주석을 인쇄하려면**: 자산에 외국어로 된 주석(특히 라틴어가 아닌 언어)이 포함되어 있는 경우, 먼저 이러한 주석을 인쇄할 수 있도록 서버에서 CQ-DAM-Handler-Gibson Font Manager  [!DNL Experience Manager] 서비스를 구성해야 합니다. CQ-DAM-Handler-Gibson Font Manager 서비스를 구성할 때 원하는 언어의 글꼴이 있는 경로를 제공합니다.
...
1. **[!UICONTROL CQ-DAM-Handler-Gibson Font Manager Service]**&#x200B;를 구성하려면 다음 중 하나를 수행합니다.
   ...
   * **[!UICONTROL 시스템 글꼴]** 디렉토리 옵션에서 시스템의 fonts 디렉토리에 대한 전체 경로를 지정합니다. 예를 들어 Mac 사용자인 경우 **[!UICONTROL 시스템 글꼴]** 디렉토리 옵션에서 경로를 `/Library/Fonts` 로 지정할 수 있습니다. [!DNL Experience Manager] 이 디렉터리에서 글꼴을 가져옵니다.
...
1. 다음과 같이 올바른 글꼴 패밀리 세트로 **[!UICONTROL 주석 PDF]**&#x200B;을 구성합니다.
...
1. [!DNL Experience Manager] 인스턴스를 다시 시작합니다.
...
* 다른 애플리케이션에서 이미지를 수정하고 [!DNL Experience Manager] 자산에 업로드합니다. 이미지의 버전이 만들어지므로 원본 이미지를 덮어쓰지 않습니다.
* 자산의 메타데이터를 편집합니다.
...
[자산 [!DNL Experience Manager] 에 워크플로우 적용 을 참조하십시오.](/help/assets/assets-workflow.md#apply-a-workflow-to-an-aem-asset)
