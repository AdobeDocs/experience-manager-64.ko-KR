---
title: AEM 자산을 Adobe InDesign Server와 통합
...
# AEM 자산을 Adobe InDesign Server와 통합 {#integrating-aem-assets-with-indesign-server}
...
이러한 기능에는 다양한 작업이 포함될 수 있습니다.예를 들어 Adobe InDesign Server를 사용하여 파일을 처리할 수 있습니다.
...
InDesign Server를 AEM Assets와 통합하여 InDesign( `.indd`)으로 만든 파일을 업로드하고, 표현물을 생성하며, *모든* 미디어(예: 비디오)를 추출하여 자산으로 저장할 수 있습니다.
...
1. 프레임워크는 SOAP(Simple Object Access Protocol)를 통해 InDesign Server로 명령 스크립트를 전송합니다.
   ...
   * 결과 파일을 다시 AEM 자산에 게시합니다.
   ...
   >[!CAUTION]
   >
   >InDesign Server가 설치되어 있지 않거나 구성되지 않은 경우에도 AEM에 `.indd` 파일을 업로드할 수 있습니다. 그러나 생성된 변환은 `png` 제한되며, `jpeg`사용자는 `html`또는 페이지 표현물을 생성할 수 없습니다 `idml` .
...
AEM Assets에는 사전 구성된 워크플로우 **DAM Update** Asset이 있으며, 여기에는 InDesign을 위한 몇 가지 프로세스 단계가 있습니다.
...
* **스크립트 확장**:여기에서 다양한 스크립트 조합을 지정할 수 있습니다. InDesign Server에서 스크립트를 직접 실행하려면 스크립트를 에 저장합니다 `/apps/settings/dam/indesign/scripts`.
...
사용자 정의하려면 페이지 추출 **[!UICONTROL 단계의]** 인수 탭을 편집할 **수** 있습니다.

![chlimage_1-289](assets/chlimage_1-289.png)

* **페이지 추출 처리기**:드롭다운 목록에서 사용할 처리기를 선택합니다. 추출 핸들러는 관련 API `RenditionPicker` 를 통해 선택한 특정 변환에서 작동합니다 `ExtractionHandler` . 기본적으로 IDML 내보내기 추출 핸들러를 사용할 수 있습니다. MediaExtract 단계에서 생성된 `IDML` 변환에서 작동합니다.
...
1. Day CQ Link **[!UICONTROL Externalizer 구성을]**&#x200B;찾아 편집 **[!UICONTROL 아이콘을 클릭하여]** 엽니다.
...
1. Felix **[!UICONTROL Console]** 의 구성 탭을 엽니다.예를 들면 다음과 같습니다.
...
1. 설정:
   ...
   * **[!UICONTROL 최대 병렬 작업]** - `<*x*>` (위에서 계산됨)

1. 이러한 변경 사항을 저장합니다.
1. Adobe CS6 이상 버전에 대한 다중 세션 지원을 활성화하려면 아래의 `enable.multisession.name` 확인란을 선택합니다 `com.day.cq.dam.ids.impl.IDSJobProcessor.name configuration`.
1. IDS Worker 구성에 [SOAP 끝점을 추가하여 &lt; `*x*>`](#configuring-the-proxy-worker-for-indesign-server)IDS 작업자 풀을 만듭니다.
   ...
   >
   >또한, `com.day.cq.dam.ids.impl.IDSPoolImpl.name` 구성 아래에서 IDS를 작업 처리기 목록에서 제외하기 전에 작업 검색의 수를 결정하는 max.errors.to.blacklist 매개 변수에 양의 값을 설정합니다
   >
