...
description: 자산을 AEM으로 가져오고, 메타데이터를 적용하고, 변환을 생성하며, 인스턴스를 게시하도록 활성화하는 방법.
contentOwner: AG
...
# 자산 마이그레이션 가이드 {#assets-migration-guide}

자산을 AEM으로 마이그레이션할 때 고려해야 할 몇 가지 단계가 있습니다. 현재 집에서 에셋과 메타데이터를 추출하는 작업은 구현 간에 다양하므로 이 문서의 범위를 벗어납니다. 대신 이 문서에서는 이러한 자산을 AEM으로 가져오고, 해당 메타데이터를 적용하고, 변환을 생성하며, 자산을 활성화 또는 게시하는 방법에 대해 설명합니다.

## 전제 조건 {#prerequisites}

아래 설명된 단계를 수행하기 전에 자산 성능 조정 팁에 [대한 지침을](performance-tuning-guidelines.md)검토하고 구현합니다. 최대 동시 작업 구성과 같은 많은 단계를 통해 로드 중인 서버의 안정성과 성능을 향상시킬 수 있습니다. 파일 데이터 저장소 구성과 같은 다른 단계는 시스템이 에셋과 함께 로드된 후 수행하기가 어렵습니다.

>[!NOTE]
>
>다음 에셋 마이그레이션 도구는 Adobe Experience Manager의 일부가 아닙니다. Adobe 고객 지원 센터에서는 이러한 툴을 지원하지 않습니다.
>
...
마이그레이션을 시작하기 전에 `DAM Update Asset` 워크플로우에 대해 런처를 비활성화합니다. 모든 자산을 시스템에 인제스트한 다음 워크플로우를 일괄로 실행하는 것이 가장 좋습니다. 마이그레이션이 진행되는 동안 이미 라이브한 경우 이러한 활동이 비시간 동안 실행되도록 예약할 수 있습니다.

### 태그 로드 {#load-tags}

이미지에 적용할 태그 분류법이 이미 있을 수 있습니다. CSV Asset Importer 및 메타데이터 프로필 기능과 같은 도구를 사용하여 자산에 대한 태그 적용을 자동화할 수 있습니다. 그 전에 Experience Manager에서 태그를 추가합니다. ACS [AEM Tools Tag](https://adobe-consulting-services.github.io/acs-aem-tools/features/tag-maker/index.html) Maker 기능을 사용하면 시스템에 로드된 Microsoft Excel 스프레드시트를 사용하여 태그를 채울 수 있습니다.

### 에셋 인제스트 {#ingest-assets}

에셋을 시스템에 인제스트할 때 성능과 안정성은 중요한 문제입니다. Adobe Experience Manager에서 많은 데이터를 로드할 때 시스템이 제대로 작동하는지 확인하십시오. 이렇게 하면 데이터를 추가하는 데 필요한 시간이 최소화되고 시스템이 오버로드되지 않습니다. 이는 시스템 충돌을 예방하는데 도움이 되며, 특히 이미 운영 중인 시스템에서 더욱 그렇습니다.
...
1. HTTP를 통해 에셋을 서버로 전송합니다. 따라서 오버헤드가 너무 많이 필요하고 시간이 많이 소요되므로 마이그레이션 수행에 소요되는 시간이 길어집니다.
...
ACS [AEM 도구 CSV 자산](https://adobe-consulting-services.github.io/acs-aem-tools/features/csv-asset-importer/index.html) 가져오기는 파일 시스템에서 자산 및 자산 가져오기를 위해 CSV 파일에서 자산 메타데이터를 가져옵니다. AEM Asset Manager API는 자산을 시스템으로 가져오고 구성된 메타데이터 속성을 적용하는 데 사용됩니다. 에셋은 네트워크 파일 마운트 또는 외부 드라이브를 통해 서버에 마운트하는 것이 좋습니다.

자산을 네트워크를 통해 전송하지 않으면 전반적인 성능이 크게 향상됩니다. 이 방법은 일반적으로 저장소에 자산을 로드하는 가장 효율적인 방법입니다. 또한 모든 에셋과 메타데이터를 한 번에 가져올 수 있습니다. 이 도구는 메타데이터 처리를 지원합니다. 별도의 도구를 사용하는 것과 같이 메타데이터를 적용하는 데는 다른 단계가 필요하지 않습니다.

### 변환 처리 {#process-renditions}

자산을 시스템에 로드한 후 DAM 자산 업데이트 워크플로우를 통해 자산을 처리하여 메타데이터를 추출하고 변환을 생성해야 합니다. 이 단계를 수행하기 전에 필요에 맞게 DAM 자산 업데이트 워크플로우를 복제하고 수정해야 합니다. Scene7 PTIFF 생성 또는 InDesign 서버 통합과 같은 기본 워크플로우의 일부 단계는 필요하지 않을 수 있습니다.
...
1. 새 환경에서 데이터 저장소의 위치를 `crx-quickstart/install/org.apache.jackrabbit.oak.plugins.blob.datastore.FileDataStore.config` 편집하고 `crx-quickstart/launchpad/config/org/apache/jackrabbit/oak/plugins/blob/datastore/FileDataStore.config` 지정합니다.
1. 환경을 시작합니다.
...
1. 워크플로우 비활성화:Adobe 자산과 함께 변환을 마이그레이션하기 때문에 DAM Update 자산에 대한 워크플로우 런처를 비활성화할 수 있습니다.
...
1. 자산 마이그레이션:한 AEM 인스턴스에서 다른 AEM 인스턴스로 자산을 이동하는 데 권장되는 두 가지 도구가 있습니다.
   ...
   * **저장소 원격 복사**&#x200B;또는 `vlt rcp`네트워크를 통해 vlt를 사용할 수 있습니다. 소스 및 대상 디렉토리를 지정하고 Vlt가 한 인스턴스에서 모든 저장소 데이터를 다운로드하고 다른 인스턴스로 로드할 수 있습니다. Vlt rcp는 https://jackrabbit.apache.org/filevault/rcp.html에 [있습니다.](https://jackrabbit.apache.org/filevault/rcp.html)
...
1. 자산 활성화:AEM으로의 초기 마이그레이션에 대해 문서화된 자산 [](#activate-assets) 활성화에 대한 지침을 따르십시오.

1. 복제 게시:새 마이그레이션과 마찬가지로, 단일 게시 인스턴스를 로드하고 복제하는 것이 두 노드에서 컨텐츠를 활성화하는 것보다 더 효율적입니다. 게시 [복제를 참조하십시오.](#clone-publish)

1. 워크플로우 활성화:마이그레이션을 완료한 후 DAM Update Asset 워크플로우용 방사선을 다시 활성화하여 지속적인 시스템 사용을 위해 변환 생성 및 메타데이터 추출을 지원합니다.
