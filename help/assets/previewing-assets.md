---
title: Dynamic Media 자산 미리 보기
seo-title: Dynamic Media 자산 미리 보기
description: Dynamic Media에서 자산을 미리 보는 방법 살펴보기
seo-description: Dynamic Media에서 자산을 미리 보는 방법 살펴보기
uuid: f0ff2fc4-a263-4dbe-ba46-b07077b49ae0
contentOwner: Rick Brough
products: SG_EXPERIENCEMANAGER/6.4/ASSETS
topic-tags: dynamic-media
content-type: reference
discoiquuid: 77296bff-8429-4240-af93-26076ae431ec
translation-type: tm+mt
source-git-commit: 8c6fdcea0def7720062edfc564c536f8d47e8402
workflow-type: tm+mt
source-wordcount: '1028'
ht-degree: 2%

---


# Dynamic Media 자산 미리 보기 {#previewing-assets}

[미리 **[!UICONTROL 보기]** ]를 사용하여 다이내믹 미디어 디지털 자산을 고객이 웹 브라우저에서 볼 때 어떻게 보이는지 확인할 수 있습니다. 자산에 할당된 기본 포함된 장치 간 뷰어는 미리 보기에 **[!UICONTROL 사용됩니다]**.

뷰어는 사용자의 컴퓨터 화면 및 모바일 장치에서 리치 미디어 에셋을 보는 방법을 결정하는 뷰어 표시 크기, 확대/축소 동작, 색상 구성표, 테두리, 글꼴 등 다양한 설정 또는 &quot;사전 설정&quot;의 집합입니다.

비디오, 스핀 세트 및 이미지 세트에 대한 전용 미리 보기 기능을 사용하는 것 외에도, 만든 뷰어 사전 설정을 사용하여 자산을 미리 볼 수도 있습니다. 또는 이미지 변환을 미리 보려면 이미지 사전 설정을 사용하십시오.

* [이미지 사전 설정 적용](image-presets.md)
* [뷰어 사전 설정 적용](viewer-presets.md)

>[!NOTE]
>
>AEM의 웹 페이지(사이트)에 있는 경우 **[!UICONTROL 편집]** 모드에서 자산을 미리 볼 수 없습니다. 오른쪽 상단 모서리에서 미리 보기 **[!UICONTROL 를]** 클릭하여 미리 보기 **** 모드로 이동해야 합니다.

**자산을 미리 보려면**:

1. **Adobe Experience Manager**&#x200B;의 **[!UICONTROL 탐색]** 페이지에서 **자산&#x200B;**을 누른 다음**[!UICONTROL 파일&#x200B;]**을 눌러 자산에 액세스합니다.
1. Near the upper-right corner of the page, from the **[!UICONTROL View]** drop-down list, tap **[!UICONTROL List View]**.
1. (선택 사항) **[!UICONTROL 유형]** 열을 사용하여 미리 보기할 유형별로 자산을 정렬합니다.
1. [ **[!UICONTROL 제목]** ] 열에서 미리 보려는 자산의 제목 이름(축소판 이미지가 아님)을 클릭합니다.
1. 클릭한 자산 유형에 따라 다음 중 하나를 수행합니다.

<table> 
 <tbody>
  <tr>
   <td><strong>탭한 자산 유형</strong><br /> </td> 
   <td><strong>특정 변환에서 자산을 미리 볼 수 있습니까?</strong></td> 
   <td><strong>특정 뷰어에서 자산을 미리 볼 수 있습니까?</strong></td> 
  </tr>
  <tr>
   <td><p>이미지</p> </td> 
   <td>예</td> 
   <td>예</td> 
   <td><p><strong>특정 변환에서 자산을 미리 보려면</strong></p> 
    <ul> 
     <li>페이지의 왼쪽 위 모서리 근처에 있는 아이콘을 클릭하면 드롭다운 목록이 나타납니다. 목록 <strong>에서 </strong>표현물을 클릭한 다음 미리 보려는 특정 표현물을 선택합니다.</li> 
    </ul> <p><strong>특정 뷰어에서 자산을 미리 보려면</strong></p> 
    <ul> 
     <li>페이지의 왼쪽 위 모서리 근처에 있는 아이콘을 클릭하면 드롭다운 목록이 나타납니다. 목록에서 <strong>뷰어를</strong> 클릭한 다음 자산에 적용할 뷰어를 선택합니다.</li> 
    </ul> <p>선택한 이미지의 확대/축소를 <strong>+</strong> 와 <strong>- </strong>아이콘을 각각 늘리거나 줄일 수 있습니다. [ <strong>재설정</strong> ]을 클릭하여 이미지를 원래 확대/축소로 돌아갑니다.<br /> 모바일 장치를 사용하는 경우 이미지를 두 번 눌러 단계별로 확대합니다. 최대 확대/축소에 도달하면 이미지를 다시 두 번 눌러 확대/축소 상태를 재설정합니다. 이미지를 드래그하여 이동합니다.</p> <p>사용자 인터페이스에서 뷰어 사전 설정을 활성화하거나 비활성화하려면 뷰어 사전 <a href="/help/assets/managing-viewer-presets.md">설정 관리를 참조하십시오</a>.<br /> </p> </td> 
  </tr>
  <tr>
   <td>멀티미디어</td> 
   <td>예</td> 
   <td>예</td> 
   <td><p><strong>특정 변환에서 자산을 미리 보려면</strong></p> 
    <ul> 
     <li>페이지의 왼쪽 위 모서리 근처에 있는 아이콘을 클릭하면 드롭다운 목록이 나타납니다. 목록 <strong>에서 </strong>표현물을 클릭한 다음 미리 보려는 특정 표현물을 선택합니다.</li> 
    </ul> <p>미리 보기할 고해상도 비디오 변환을 선택하면 비디오가 잘릴 수 있습니다. 이는 미리 보기에 사용되는 임베드된 뷰어의 컨텍스트에서 고객이 보게 될 정확한 해상도를 보여 주기 때문입니다.</p> <p>자산 수준에서 응용 비디오 세트를 미리 보면 변환이 하나의 재생 경험으로 그룹화됩니다. 즉, 응용 비디오가 보기 장치 및 연결 속도 컨텍스트에서 최상의 해상도로 보기 위해 제대로 크기가 조정되고 재생됩니다.<br /> </p> <p><strong>특정 뷰어에서 자산을 미리 보려면</strong></p> 
    <ul> 
     <li>페이지의 왼쪽 위 모서리 근처에 있는 아이콘을 클릭하면 드롭다운 목록이 나타납니다. 목록에서 <strong>뷰어를</strong> 클릭한 다음 자산에 적용할 뷰어를 선택합니다.</li> 
    </ul> <p>사용자 인터페이스에서 뷰어 사전 설정을 활성화하거나 비활성화하려면 뷰어 사전 <a href="/help/assets/managing-viewer-presets.md">설정 관리를 참조하십시오</a>.</p> </td> 
  </tr>
  <tr>
   <td>이미지 세트</td> 
   <td>아니오</td> 
   <td>예</td> 
   <td><p><strong>특정 뷰어에서 자산을 미리 보려면</strong></p> 
    <ul> 
     <li>페이지의 왼쪽 위 모서리 근처에 있는 아이콘을 클릭하면 드롭다운 목록이 나타납니다. 목록에서 <strong>뷰어를</strong> 클릭한 다음 자산에 적용할 뷰어를 선택합니다.</li> 
    </ul> <p>선택한 이미지의 확대/축소를 <strong>+</strong> 와 <strong>- </strong>아이콘을 각각 늘리거나 줄일 수 있습니다. [ <strong>재설정</strong> ]을 클릭하여 이미지를 원래 확대/축소로 돌아갑니다.<br /> 모바일 장치를 사용하는 경우 이미지를 두 번 눌러 단계별로 확대합니다. 최대 확대/축소에 도달하면 이미지를 다시 두 번 눌러 확대/축소 상태를 재설정합니다. 이미지를 드래그하여 이동합니다.</p> <p>사용자 인터페이스에서 뷰어 사전 설정을 활성화하거나 비활성화하려면 뷰어 사전 <a href="/help/assets/managing-viewer-presets.md">설정 관리를 참조하십시오</a>.</p> </td> 
  </tr>
  <tr>
   <td>회전 집합</td> 
   <td>아니오</td> 
   <td>예</td> 
   <td><p><strong>특정 뷰어에서 자산을 미리 보려면</strong></p> 
    <ul> 
     <li>페이지의 왼쪽 위 모서리 근처에 있는 아이콘을 클릭하면 드롭다운 목록이 나타납니다. 목록에서 <strong>뷰어를</strong> 클릭한 다음 자산에 적용할 뷰어를 선택합니다.</li> 
    </ul> <p>선택한 이미지의 확대/축소를 <strong>+</strong> 와 <strong>- </strong>아이콘을 각각 늘리거나 줄일 수 있습니다. [ <strong>재설정</strong> ]을 클릭하여 이미지를 원래 확대/축소로 돌아갑니다.<br /> 모바일 장치를 사용하는 경우 이미지를 두 번 눌러 단계별로 확대합니다. 최대 확대/축소에 도달하면 이미지를 다시 두 번 눌러 확대/축소 상태를 재설정합니다. 이미지를 드래그하여 이동합니다.</p> <p>사용자 인터페이스에서 뷰어 사전 설정을 활성화하거나 비활성화하려면 뷰어 사전 <a href="/help/assets/managing-viewer-presets.md">설정 관리를 참조하십시오</a>.</p> </td> 
  </tr>
  <tr>
   <td>혼합 미디어 집합</td> 
   <td>아니오</td> 
   <td>예</td> 
   <td><p><strong>특정 뷰어에서 자산을 미리 보려면</strong></p> 
    <ul> 
     <li>페이지의 왼쪽 위 모서리 근처에 있는 아이콘을 클릭하면 드롭다운 목록이 나타납니다. 목록에서 <strong>뷰어를</strong> 클릭한 다음 자산에 적용할 뷰어를 선택합니다.</li> 
    </ul> <p>선택한 이미지의 확대/축소를 <strong>+</strong> 와 <strong>- </strong>아이콘을 각각 늘리거나 줄일 수 있습니다. [ <strong>재설정</strong> ]을 클릭하여 이미지를 원래 확대/축소로 돌아갑니다.<br /> 모바일 장치를 사용하는 경우 이미지를 두 번 눌러 단계별로 확대합니다. 최대 확대/축소에 도달하면 이미지를 다시 두 번 눌러 확대/축소 상태를 재설정합니다. 이미지를 드래그하여 이동합니다.</p> <p>사용자 인터페이스에서 뷰어 사전 설정을 활성화하거나 비활성화하려면 뷰어 사전 <a href="/help/assets/managing-viewer-presets.md">설정 관리를 참조하십시오</a>.</p> </td> 
  </tr>
  <tr>
   <td>회전판 세트</td> 
   <td>아니오</td> 
   <td>예</td> 
   <td><p><strong>특정 뷰어에서 자산을 미리 보려면</strong></p> 
    <ul> 
     <li>페이지의 왼쪽 위 모서리 근처에 있는 아이콘을 클릭하면 드롭다운 목록이 나타납니다. 자산에 적용할 뷰어를 선택합니다.</li> 
    </ul> <p>사용자 인터페이스에서 뷰어 사전 설정을 활성화하거나 비활성화하려면 뷰어 사전 <a href="/help/assets/managing-viewer-presets.md">설정 관리를 참조하십시오</a>.</p> </td> 
  </tr>
 </tbody>
</table>

