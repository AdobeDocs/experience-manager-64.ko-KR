...
1. 게시 실행 모드를 타깃팅하는 이름 지정 규칙이 있는 폴더( `config.publish`즉,
   `/apps/<your-app-name>/config.publish`실행 모드에 대한 구성 속성을 정의하려면 [실행 모드를](/help/sites-deploying/configure-runmodes.md#defining-configuration-properties-for-a-run-mode) 참조하십시오.
