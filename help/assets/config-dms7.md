...
description: 다이내믹 미디어 - Scene7 모드를 구성하는 방법에 대한 정보입니다.
seo-description: 다이내믹 미디어 - Scene7 모드를 구성하는 방법에 대한 정보입니다.
...
다음 아키텍처 다이어그램에서는 다이내믹 미디어 - Scene7 모드가 작동하는 방식을 설명합니다.

새 아키텍처에서 자산 처리 및 게시에 대해 AEM은 마스터 자산을 담당하고 다이내믹 미디어와 동기화합니다.

1. 마스터 자산이 AEM에 업로드되면 Dynamic Media에 복제됩니다. 이때 Dynamic Media는 이미지의 비디오 인코딩 및 동적 변형과 같은 모든 자산 처리 및 변환 생성을 처리합니다.
1. 변환이 생성되면 AEM은 안전하게 원격 다이내믹 미디어 표현물에 액세스하고 미리 볼 수 있습니다(바이너리는 AEM 인스턴스로 다시 전송되지 않음).
...
>[메모]
>
> 동적 미디어 - Scene7 모드는 AEM 작성자 인스턴스에만 적용됩니다. 따라서 AEM 게시 인스턴스가 아닌 AEM 작성자 인스턴스 `runmode=dynamicmedia_scene7`에서 구성해야 합니다.

다이내믹 미디어를 활성화하려면 터미널 창에 다음 사항을 입력하여 명령줄에서 `dynamicmedia_scene7` 실행 모드를 사용하여 AEM을 시작해야 합니다(사용된 예제 포트는 4502).
...
AEM Dynamic Media를 6.3에서 6.4(이제 가동 중지 시간 없이 배포할 수 있는 기능이 포함)로 업그레이드하는 경우 CRXDE Lite에서 모든 사전 설정 및 구성을 마이그레이션하기 위해 다음 curl 명령을 실행해야 `/etc` `/conf` 합니다.
...
Dynamic Media Cloud 서비스를 구성하기 전에 암호를 변경합니다. Dynamic Media 자격 증명으로 프로비저닝 이메일을 받은 후 Dynamic Media Classic에 [로그인해야](https://www.adobe.com/marketing-cloud/experience-manager/scene7-login.html) 암호를 변경할 수 있습니다. 프로비저닝 이메일에 제공된 암호는 시스템에서 생성되며 임시 암호에만 사용됩니다. Dynamic Media Cloud 서비스가 올바른 자격 증명으로 설정되도록 암호를 업데이트해야 합니다.

>[!NOTE]
>
>기본적으로 클라우드 서비스의 구성 경로는 입니다 `/content/dam`. 다른 구성 경로는 Dynamic Media - Scene7 모드에서 지원되지 않습니다.

Dynamic Media Cloud 서비스를 구성하려면:

1. AEM에서 AEM 로고를 눌러 글로벌 탐색 콘솔에 액세스하고 도구 아이콘을 누른 다음 **[!UICONTROL 클라우드 서비스 > 다이내믹 미디어 구성을 누릅니다]**.
1. Dynamic Media 구성 브라우저 페이지의 왼쪽 창에서 **[!UICONTROL 글로벌]** 을 누르고 **[!UICONTROL 만들기를 누릅니다]**. 글로벌 왼쪽에 있는 폴더 아이콘을 탭하거나 [!UICONTROL 선택하지 마십시오].
1. [ [!UICONTROL 동적 미디어 구성] 만들기] 페이지에서 제목, Dynamic Media 계정 이메일 주소, 암호를 입력한 다음 지역을 선택합니다. 이러한 서비스는 제공 이메일에서 Adobe가 제공합니다. 수신하지 못한 경우 지원 센터에 문의하십시오.
...
1. 연결이 성공하면 다음을 설정할 수도 있습니다.
   ...
   * **[!UICONTROL 보안 미리 보기 서버]** - 보안 변환 미리 보기 서버에 대한 URL 경로를 지정할 수 있습니다. 즉, 변환이 생성된 후 AEM에서는 원격 다이내믹 미디어 표현물에 안전하게 액세스하고 미리 볼 수 있습니다(바이너리는 AEM 인스턴스로 다시 전송되지 않음).

      회사 서버 또는 특수 서버를 사용할 특별한 계획이 없는 경우 기본 설정을 사용하는 것이 좋습니다.
   >[!NOTE]
   >
   >DMS7에서는 버전 관리가 지원되지 않습니다. 또한, 지연된 활성화는 [다이내믹 미디어 구성 **** 편집  페이지의 [자산] 게시]가 [활성화 **[!UICONTROL 에]**]로설정된 경우에만 적용되며, 처음 자산이 활성화될 때까지 적용됩니다.
   >
...
구성을 추가로 사용자 지정하려면, Dynamic Media - Scene7 모드에서 고급 설정 구성 [(선택 사항)에서 원하는 작업을 완료할 수 있습니다](#optional-configuring-advanced-settings-in-dynamic-media-scene-mode).

## (선택 사항) Dynamic Media - Scene7 모드에서 고급 설정 구성 {#optional-configuring-advanced-settings-in-dynamic-media-scene-mode}

Dynamic Media - Scene7 모드의 구성 및 설정을 사용자 정의하거나 성능을 최적화하려는 경우 다음 선택 작업 중 하나 이상을 완료할 수 있습니다.
...
실행 모드에 있는 경우 **Dynamic Media_scene7**&#x200B;사용자 인터페이스를 사용하여 Dynamic Media 설정을 변경합니다.
...
응용 프로그램 [!UICONTROL 일반 설정] 페이지를 열려면 Dynamic Media Classic 전역 탐색 막대에서 **[!UICONTROL 설정 > 응용 프로그램 설정 > 일반 설정을 누릅니다]**.
...
AEM Assets에서 지원되지 않는 포맷에 대한 사용자 지정 MIME 유형을 추가할 수 있습니다. CRXDE Lite에 추가하는 새 노드가 AEM에서 삭제되지 않도록 하려면 **[!UICONTROL image_]** 및 해당 활성화된 값이 **[!UICONTROL false로 설정되기 전에 MIME 유형을 이동해야 합니다]**.
...
1. 시퀀스 순서 [!UICONTROL 의]경우, 설정이 Dynamic Media에서 그룹화된 후 이미지가 표시되는 순서를 정의합니다.
...
1. 최대 **[!UICONTROL 병렬 작업]** 필드에서 숫자를 원하는 값으로 변경합니다.
   ...
   Adobe에서는 Dynamic Media Classic에 대용량 파일 **** 업로드를 지원하기 위해 32개의 최대 병렬 작업을 구성할 것을 권장합니다.
...
1. 연결 [!UICONTROL 수] 필드 및/또는 [!UICONTROL 활성 작업 시간 초과] 필드에서 원하는 대로 숫자를 변경합니다.
   ...
   연결 **[!UICONTROL 수]** 설정은 AEM에서 Dynamic Media 업로드에 대해 허용되는 최대 HTTP 연결 수를 제어합니다. 일반적으로 사전 정의된 10개 연결 값이면 충분합니다.
...
비동적 미디어 배포에서는 AEM 작성 환경에서 모든 *자산(이미지와 비디오 모두)을 AEM 게시 노드로 복제합니다. AEM 게시 서버도 자산을 제공하므로 이 워크플로우가 필요합니다.
...
이미징 및/또는 비디오에 Dynamic Media를 사용하는 경우 Adobe가 제공하는 기본 필터를 그대로 사용할 수 있습니다. 다음 필터는 기본적으로 활성화됩니다.
