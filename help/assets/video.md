...
1. **다음을 수행하여 Dynamic Media** 비디오를 업로드합니다.
   ...
   * 비디오 처리 프로필을 마스터 비디오를 업로드할 하나 이상의 폴더에 연결합니다.
      ...
      * 처리 프로필](organize-assets.md#organize-using-folders)을 사용하기 위해 디지털 자산을 구성하는 우수 사례에 대해 자세히 알아보십시오.[
      * [디지털 자산 구성](organize-assets.md)에 대해 자세히 알아보십시오.
   * 폴더에 마스터 비디오를 업로드합니다. 각각 최대 15GB의 비디오 파일을 업로드할 수 있습니다. 폴더에 비디오를 추가하면 폴더에 할당한 비디오 처리 프로필에 따라 인코딩됩니다.
   ...
   * 자산 또는 워크플로우 보기에서 [비디오 인코딩이](#monitoring-video-encoding-and-youtube-publishing-progress)로 진행되는 방식을 모니터링합니다.
...
* **상수 비트율 인코딩** (CBR) - CBR 인코딩 중에 비트율 또는 초당 비트 수가 인코딩 프로세스 전체에서 동일하게 유지됩니다. CBR 인코딩은 전체 비디오에서 설정에 설정된 데이터 속도를 유지합니다. 또한 CBR 인코딩은 품질을 위해 미디어 파일을 최적화하지는 않지만 저장 공간에 저장합니다.
   ...
   비디오가 전체 비디오 전체에서 유사한 동작 수준을 포함하는 경우 CBR을 사용합니다. CBR은 비디오 컨텐츠를 스트리밍하는 데 가장 일반적으로 사용됩니다. [사용자 지정 추가된 비디오 인코딩 매개 변수 사용](video-profiles.md#using-custom-added-video-encoding-parameters)을 참조하십시오.
...
Dynamic Media에서는 MP4 H.264 비디오 인코딩 사전 설정을 사용하는 것이 좋습니다. MP4 파일은 H.264 비디오 코덱을 사용하기 때문에 고품질의 비디오를 제공하지만 압축된 파일 크기를 제공합니다.
...
YouTube에 게시하려면 Google 계정이 필요합니다. GMAIL 계정이 있는 경우 이미 Google 계정이 있습니다. Google 계정이 없는 경우 쉽게 만들 수 있습니다. 비디오 자산을 YouTube에 게시하려면 자격 증명이 필요하므로 계정이 필요합니다. 계정을 이미 만든 경우에는 이 작업을 건너뛰고 [YouTube 채널 만들기](#creating-a-youtube-channel)로 진행합니다.
...
1. **[!UICONTROL 대시보드]** 페이지의 맨 위에 있는 **[!UICONTROL API]**&#x200B;를 탭합니다.
1. **[!UICONTROL 라이브러리]** 페이지의 YouTube API에서 **[!UICONTROL YouTube 데이터 API]**&#x200B;를 탭합니다.
1. **[!UICONTROL YouTube 데이터 API v3]** 페이지의 맨 위에 있는 **[!UICONTROL 활성화]**&#x200B;를 탭하여 켜십시오.
1. API를 사용하려면 자격 증명이 필요할 수 있습니다. 필요한 경우 **[!UICONTROL 자격 증명 만들기]**&#x200B;를 탭합니다.
1. **[!UICONTROL 어디에서 API를 호출합니까?]** 드롭다운 목록에서  **[!UICONTROL 웹 서버(예: node.js, Tomcat)]**&#x200B;를 선택합니다.
1. **[!UICONTROL 어떤 데이터에 액세스합니까?]** 사용자  **[!UICONTROL 데이터]**&#x200B;를 선택합니다.
1. **[!UICONTROL 자격 증명을 탭합니다.]** 단추.
1. **[!UICONTROL OAuth 2.0 클라이언트 ID]** 제목 아래에 고유한 이름을 입력합니다.
1. **[!UICONTROL Authorized Javascript 원본]** 제목 아래의 텍스트 필드에 다음 경로를 입력하고 경로에 고유한 도메인과 포트 번호를 바꾼 다음 **[!UICONTROL Enter]** 키를 눌러 목록에 경로를 추가합니다.
...
YouTube에 비디오를 게시하려면 하나 이상의 채널이 있어야 합니다. YouTube 채널을 이미 만든 경우 이 작업을 건너뛰고 게시&#x200B;**에 대한 태그 추가 로 이동할 수 있습니다.**
...
1. YouTube 페이지의 오른쪽 위 모서리에서 프로필 사진(단색 원 내에 문자로 표시될 수 있음)을 탭한 다음, **[!UICONTROL YouTube 설정]**(라운드 톱니바퀴 아이콘)을 탭합니다.
1. **[!UICONTROL 개요]** 페이지의 **[!UICONTROL 추가 기능]** 제목 아래에서 **[!UICONTROL 내 채널을 모두 보거나 새 채널]**&#x200B;을 만듭니다.
1. **[!UICONTROL 채널]** 페이지에서 **[!UICONTROL 새 채널 만들기]**&#x200B;를 탭합니다.
1. **[!UICONTROL Brand Account]** 페이지의 **[!UICONTROL Brand Account Name]** 필드에서 비디오 자산을 게시할 위치에 선택하는 비즈니스 이름 또는 다른 채널 이름을 입력한 다음 **[!UICONTROL 만들기]**&#x200B;를 탭합니다.
...
1. 도구 모음의 설정 오른쪽에 있는 **[!UICONTROL 편집]**&#x200B;을 누릅니다.
1. 복제 에이전트를 켜려면 **[!UICONTROL Enabled]** 확인란을 선택하십시오.
...
1. **[!UICONTROL YouTube 계정 설정]** 대화 상자로 돌아갑니다. **[!UICONTROL JSON 구성]** 필드에서 JSON 텍스트를 붙여 넣습니다.
1. **[!UICONTROL 확인]**&#x200B;을 누릅니다.

   이제 AEM에서 YouTube 채널을 설정합니다.

1. **[!UICONTROL 사용 가능한 채널]** 오른쪽의 **[!UICONTROL +]**(더하기 기호 아이콘)를 탭합니다.
1. **[!UICONTROL YouTube 채널 설정]** 대화 상자의 **[!UICONTROL 제목]** 필드에서 **C[!UICONTROL YouTube 채널 만들기]**&#x200B;에서 만든 채널의 이름을 입력합니다.
...
1. YouTube/Google 인증이 표시됩니다. 아직 Google Cloud 계정에 로그인하지 않은 경우 이 단계를 건너뜁니다.
   ...
   * 다음 페이지에서 **[!UICONTROL Accept]**&#x200B;를 눌러 이 채널에 액세스할 수 있습니다.

1. **[!UICONTROL Allow]**&#x200B;을 누릅니다.

   이제 게시할 태그를 설정합니다.

1. **게시할 태그 설정**  -  **[!UICONTROL Cloud Services >]** YouTube 페이지에서 Pencilicon **** 을 탭하여 사용할 태그 목록을 편집합니다.
1. 드롭다운 목록 아이콘(위/아래로 캐럿)을 탭하여 AEM에서 사용 가능한 태그 목록을 표시합니다.
1. 태그를 하나 이상 탭하여 추가합니다.

   추가한 태그를 삭제하려면 태그를 선택하고 **[!UICONTROL X]**&#x200B;를 누릅니다.

1. 원하는 태그를 추가했으면 **[!UICONTROL 확인]**&#x200B;을 누릅니다.
...
메타데이터 처리 프로필을 만들려면 먼저 **[!UICONTROL 필드 레이블]**, **[!UICONTROL 속성에 매핑]** 및 **[!UICONTROL Choices]** 필드의 값을 복사해야 하며, 모두 비디오용 메타데이터 스키마에 있습니다. 그런 다음 해당 값을 추가하여 YouTube 비디오 메타데이터 처리 프로필을 만듭니다.

**선택적으로 업로드한 비디오에 대한 기본 YouTube 속성 설정을 자동화하려면**:

1. AEM의 왼쪽 위 모서리에서 AEM 로고를 탭한 다음, 왼쪽 레일에서 **[!UICONTROL 도구 > 자산 > 메타데이터 스키마]**&#x200B;를 누릅니다.
1. **[!UICONTROL default]**&#x200B;을 누릅니다. (선택 상자에 &quot;기본값&quot; 왼쪽에 선택 표시를 추가하지 마십시오.)
1. **[!UICONTROL 기본]** 페이지에서 **[!UICONTROL 비디오]** 왼쪽에 있는 상자를 선택한 다음 **[!UICONTROL 편집]**&#x200B;을 탭합니다.
1. **[!UICONTROL 메타데이터 스키마 편집기]** 페이지에서 **[!UICONTROL 고급]** 탭을 탭합니다.
1. YouTube 게시 제목 아래에서 **[!UICONTROL YouTube 범주]**&#x200B;를 탭합니다. (YouTube 카테고리 드롭다운 목록을 탭하지 마십시오.)
1. 페이지 오른쪽의 **[!UICONTROL 설정]** 탭 아래에서 다음을 수행합니다.
   ...
   * **[!UICONTROL 선택 항목]**&#x200B;에서 사용할 기본값(예: 사용자 및 블로그 또는 과학 및 기술)을 선택하고 복사합니다.

      복사한 값을 열려 있는 텍스트 편집기에 붙여넣습니다. 메타데이터 처리 프로필을 만들 때 나중에 이 값이 필요합니다. 텍스트 편집기를 열어 둡니다.

1. YouTube 게시 제목 아래에서 **[!UICONTROL YouTube 개인 정보 보호]**&#x200B;를 탭합니다. (YouTube 개인 정보 보호 드롭다운 목록을 탭하지 마십시오.)
1. 페이지 오른쪽의 **[!UICONTROL 설정]** 탭 아래에서 다음을 수행합니다.
   ...
   * **[!UICONTROL 선택 항목]**&#x200B;에서 사용할 기본값을 선택하고 복사합니다. 선택 사항은 두 쌍으로 그룹화됩니다. 쌍의 맨 아래 필드는 공용, 비상장 또는 개인 등의 복사할 기본값입니다.

      복사한 값을 열려 있는 텍스트 편집기에 붙여넣습니다. 메타데이터 처리 프로필을 만들 때 나중에 이 값이 필요합니다. 텍스트 편집기를 열어 둡니다.

1. **[!UICONTROL 메타데이터 스키마 편집기]** 페이지의 오른쪽 위 모서리 근처에 있는 **[!UICONTROL 취소]**&#x200B;를 탭합니다.
1. AEM의 왼쪽 위 모서리에서 AEM 로고를 탭한 다음, 왼쪽 레일에서 **[!UICONTROL 도구 > 자산 > 메타데이터 프로필]**&#x200B;을 누릅니다.

1. 페이지의 오른쪽 위 모서리 근처에 있는 **[!UICONTROL 메타데이터 프로필]** 페이지에서 **[!UICONTROL 만들기]**&#x200B;를 탭합니다. **[!UICONTROL 메타데이터 프로필 추가]** 대화 상자의 **[!UICONTROL 프로필 제목]** 텍스트 필드에 이름 `YouTube Video`을 입력합니다.
1. **[!UICONTROL 메타데이터 프로필 편집기]** 페이지에서 **[!UICONTROL 고급]** 탭을 탭합니다.
1. 다음을 수행하여 복사된 YouTube 게시 값을 프로필에 추가합니다.

   * 페이지 오른쪽에서 **[!UICONTROL 양식 작성]** 탭을 탭합니다.
   * **[!UICONTROL 섹션 헤더]** 레이블이 지정된 구성 요소를 왼쪽으로 드래그하고 양식 영역에 놓습니다.
   * **[!UICONTROL 필드 레이블]**&#x200B;을 눌러 구성 요소를 선택합니다.
   * 페이지의 오른쪽의 **[!UICONTROL 설정]** 탭의 **[!UICONTROL 필드 레이블]** 텍스트 필드에 `YouTube Publishing`를 입력합니다.
   * **[!UICONTROL 양식 작성]** 탭을 탭한 다음, **[!UICONTROL 단일 줄 텍스트]** 라는 구성 요소를 드래그하고 방금 만든 **[!UICONTROL YouTube 게시]** 제목 아래에 놓습니다.
   * **[!UICONTROL 필드 레이블]**&#x200B;을 눌러 구성 요소를 선택합니다.
   * 페이지 오른쪽의 **[!UICONTROL 설정]** 탭 아래에서 이전에 복사한 **[!UICONTROL YouTube 게시]** 값(**[!UICONTROL 필드 레이블]** 값 및 **[!UICONTROL 속성]** 값에 매핑)을 양식의 해당 필드에 붙여 넣습니다. **[!UICONTROL Choices]** 값을 **[!UICONTROL Default Value]** 필드에 붙여 넣습니다.
...
1. 다음을 수행하여 복사된 YouTube 개인 정보 보호 값을 프로필에 추가합니다.
   ...
   * **[!UICONTROL 섹션 헤더]** 레이블이 지정된 구성 요소를 왼쪽으로 드래그하고 양식 영역에 놓습니다.
   * **[!UICONTROL 필드 레이블]**&#x200B;을 눌러 구성 요소를 선택합니다.
   * 페이지의 오른쪽의 설정 탭의 필드 레이블 텍스트 필드에 `YouTube Privacy`을 입력합니다.
   * **[!UICONTROL 양식 작성]** 탭을 탭한 다음, **[!UICONTROL 단일 줄 텍스트]** 라는 구성 요소를 드래그하고 방금 만든 **[!UICONTROL YouTube 개인 정보 보호]** 제목 아래에 놓습니다.
   * **[!UICONTROL 필드 레이블]**&#x200B;을 눌러 구성 요소를 선택합니다.
   * 페이지 오른쪽의 **[!UICONTROL 설정]** 탭 아래에서 이전에 복사한 **[!UICONTROL YouTube 게시]** 값(**[!UICONTROL 필드 레이블]** 값 및 **[!UICONTROL 속성]** 값에 매핑)을 양식의 해당 필드에 붙여 넣습니다. **[!UICONTROL Choices]** 값을 **[!UICONTROL Default Value]** 필드에 붙여 넣습니다.
...
YouTube에서 컨텐츠를 게시하기 위해 AEM은 **[!UICONTROL YouTube에 게시]** 워크플로우를 사용하여 진행 상황을 모니터링하고 실패 정보를 볼 수 있습니다.
...
1. **[!UICONTROL 태그 선택]** 페이지에서 사용할 태그로 이동한 다음 태그를 하나 이상 선택합니다.
1. 페이지의 오른쪽 위 모서리에서 **[!UICONTROL Confirm]** 아이콘을 탭합니다.
1. 비디오 속성 페이지의 오른쪽 위 모서리에서 **[!UICONTROL 저장]**&#x200B;을 탭합니다.
1. 도구 모음에서 **[!UICONTROL 게시 > 게시]**&#x200B;를 누릅니다.
...
1. 복사할 URL이 있는 YouTube *게시된* 비디오 자산으로 이동한 다음 선택합니다.
   ...
   YouTube URL은 *이후에*&#x200B;을 복사하는 데에만 사용할 수 있으며, 먼저 *게시된*&#x200B;비디오 자산을 YouTube에 게시했습니다.
...
YouTube에서 컨텐츠를 제거하려면 AEM에서는 진행 상황을 모니터링하고 실패 정보를 볼 수 있는 **[!UICONTROL YouTube에서 게시 취소 워크플로우를 사용합니다.]**
[비디오 인코딩 및 YouTube 게시 진행률 모니터링](#monitoring-video-encoding-and-youtube-publishing-progress)을 참조하십시오.

**YouTube에서 제거할 비디오 게시를 취소하려면**:

1. AEM의 왼쪽 위 모서리에서 AEM 로고를 탭한 다음 왼쪽 레일에서 **[!UICONTROL 도구 > Assets]**&#x200B;을 탭합니다.
...
1. 자산 폴더에서 비디오 인코딩 진행 상태 보기:
   ...
   * **[!UICONTROL 카드 보기]**&#x200B;에서 비디오 인코딩 진행 상태가 백분율로 자산에 표시됩니다. 오류가 있으면 이 정보가 자산에도 표시됩니다.
   ...
   * **[!UICONTROL 목록 보기]**&#x200B;에서 비디오 인코딩 진행 상태가 **[!UICONTROL 처리 상태]** 열에 표시됩니다. 오류가 있으면 이 메시지가 동일한 열에 표시됩니다.
      ...
      이 열은 기본적으로 표시되지 않습니다. 열을 사용하려면 **[!UICONTROL 보기]** 드롭다운 메뉴에서 **[!UICONTROL 설정 보기]**&#x200B;를 선택하고 **[!UICONTROL 처리 상태]** 열을 추가하고 **[!UICONTROL 업데이트]**&#x200B;을 탭합니다.

      ![chlimage_1-431](assets/chlimage_1-431.png)

1. 자산 세부 정보에서 진행 상황을 봅니다. 자산을 탭할 때 드롭다운 메뉴를 열고 **[!UICONTROL 타임라인]**&#x200B;을 선택합니다. 인코딩 또는 YouTube 게시과 같은 워크플로우 활동으로 범위를 좁히려면 **[!UICONTROL 워크플로우]**&#x200B;를 선택합니다.

   ![chlimage_1-432](assets/chlimage_1-432.png)

   인코딩과 같은 모든 워크플로우 정보는 타임라인에 표시됩니다. YouTube 게시의 경우 **[!UICONTROL 워크플로우]** 타임라인에는 YouTube 채널의 이름과 YouTube 비디오 URL도 포함되어 있습니다. 또한 **[!UICONTROL 워크플로우]** 타임라인에 오류 알림이 표시됩니다.

   >[!NOTE]
   >
   >**[!UICONTROL 다시 시도]**, **[!UICONTROL 다시 시도 지연]** 및 [http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr)의 **[!UICONTROL timeout]**&#x200B;에 대한 여러 워크플로우 구성으로 인해 실패/오류 메시지가 마지막으로 기록되는 데 시간이 오래 걸릴 수 있습니다. 예:
   >
...
1. 진행 중인 워크플로우의 경우 **워크플로우 인스턴스**&#x200B;를 참조하여 **[!UICONTROL 도구 > 워크플로우 > 인스턴스]**&#x200B;에서 사용할 수 있습니다.
   ...
   인스턴스를 선택하고 **[!UICONTROL 기록 열기]**&#x200B;를 누릅니다.

   ![chlimage_1-434](assets/chlimage_1-434.png)

   **[!UICONTROL 워크플로우 인스턴스]** 영역에서 워크플로우를 일시 중단, 종료 또는 변경할 수도 있습니다. 자세한 내용은 [워크플로우 관리](/help/sites-administering/workflows-administering.md) 를 참조하십시오.

1. 실패한 작업의 경우 **워크플로우 실패**&#x200B;를 참조하십시오(**[!UICONTROL 도구 > 워크플로우 > 실패]**). **[!UICONTROL 워크플로우 실패]**&#x200B;는 실패한 모든 워크플로우 활동을 나열합니다.
   ...
   >[!NOTE]
   >
   >[다시 시도&#x200B;]**,**[!UICONTROL &#x200B;다시 시도 지연&#x200B;]**및**[!UICONTROL &#x200B;시간 초과&#x200B;]**(예: http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr)에서)에 대한 여러 워크플로우 구성으로 인해 오류 메시지가 마지막으로 기록되는 데 시간이 오래 걸릴 수 있습니다.**[!UICONTROL 
   >
   >* Apache Sling 작업 큐 구성
   >* Adobe Granite Workflow 외부 프로세스 작업 처리기
   >* Granite 워크플로우 시간 초과 큐
...
1. 완료된 워크플로우의 경우 **[!UICONTROL 워크플로우 아카이브]**&#x200B;를 참조하십시오(**[!UICONTROL 도구 > 워크플로우 > 아카이브]**). **[!UICONTROL 워크플로우 아카이브]**&#x200B;는 완료된 모든 워크플로우 활동을 나열합니다.
...
* 인코딩 작업의 경우 **[!UICONTROL 도구 > 작업 > 웹 콘솔]**&#x200B;에서 그리고 **[!UICONTROL 일 CQ 워크플로우 이메일 알림 서비스]**&#x200B;를 검색하여 모든 AEM 워크플로우 이메일 알림에 대한 구성 페이지에 액세스할 수 있습니다. AEM](/help/sites-administering/notification.md)에서 이메일 알림 구성 을 참조하십시오. [ **[!UICONTROL Notify on Abort]** 또는 **[!UICONTROL Notify on Complete]**&#x200B;에 대한 확인란을 선택하거나 선택 취소할 수 있습니다.

* YouTube 게시 작업의 경우 다음을 수행합니다.

1. AEM에서 **[!UICONTROL 도구 > 워크플로우 > 모델]**&#x200B;을 선택합니다.
1. **[!UICONTROL YouTube에 게시]** 워크플로우를 선택한 다음 **[!UICONTROL 편집]**&#x200B;을 누릅니다.
1. **[!UICONTROL YouTube 업로드]** 워크플로우 단계를 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL 편집]**&#x200B;을 누릅니다.
1. **[!UICONTROL 인수]s** 탭을 탭합니다.
1. 다음 확인란을 선택하거나 취소할 수 있습니다.
   ...
   * **[!UICONTROL 채널 및 URL에 대한 정보가 포함된 게시 완료]**
   ...
   >[!NOTE]
   >
   >이러한 이메일은 YouTube에만 해당되며 일반 워크플로우 이메일 알림 외에도 제공됩니다. 그 결과, **일 CQ 워크플로우 이메일 알림 서비스**&#x200B;에서 사용할 수 있는 일반 알림과 구성 설정에 따라 YouTube에 고유한 두 개의 이메일 알림 세트를 받을 수 있습니다.
...
게시된 모든 *게시된* 비디오도 나열되므로 총 비디오 시작을 기반으로 웹 사이트에서 가장 많이 본 비디오를 추적할 수 있습니다.
...
또한 [Dynamic Media Cloud Services 구성](/help/assets/config-dynamic.md)을 참조하십시오.
...
비디오 보고서가 올바르게 작동하려면 Dynamic Media Cloud Services이 구성되면 보고서 세트 ID가 자동으로 생성됩니다. 동시에 보고서 세트 ID가 게시 서버로 푸시되어 자산을 미리 볼 때 URL 복사 기능에 사용할 수 있습니다. 그러나 게시 서버를 이미 설정해야 합니다. 게시 서버가 설정되지 않은 경우에는 여전히 게시하여 비디오 보고서를 볼 수 있지만 Dynamic Media 클라우드 구성으로 돌아가 **OK**&#x200B;을 탭해야 합니다.

**비디오 보고서를 보려면 다음을 수행하십시오**.

1. AEM의 왼쪽 위 모서리에서 AEM 로고를 탭한 다음, 왼쪽 레일에서 **[!UICONTROL 도구 > 자산 > 비디오 보고서]**&#x200B;를 탭합니다.
1. 비디오 보고서 페이지에서 다음 중 하나를 수행합니다.
   ...
   * 오른쪽 위 모서리 근처에 있는 **[!UICONTROL 날짜 선택기]** 아이콘을 탭합니다.
      ...
      비디오 데이터를 저장할 시작 날짜 및 종료 날짜 범위를 지정한 다음 **[!UICONTROL 보고서 실행]**&#x200B;을 누릅니다.
   **[!UICONTROL 최상위 지표]** 그룹 상자는 사이트에 게시된 모든 *에 대한 다양한 집계 측정을 식별합니다.*
...
1. 포함 코드에서 다음을 사용하여 줄을 찾습니다.
   ...
   config2 매개 변수에 대한 올바른 값은 **[!UICONTROL 포함 코드]** 와 copy **[!UICONTROL URL]** 함수 모두에 있습니다. copy **[!UICONTROL URL]** 명령의 URL에서 찾을 매개 변수는 `&config2=<value>` 입니다. 값은 거의 항상 `companypreset`이지만, 일부 경우에는 `companypreset-1`, `companypreset-2` 등이 될 수도 있습니다.

1. 사용자 지정 비디오 뷰어 코드에서 다음을 수행하여 뷰어 페이지에 AppMeasurementBridge.jsp를 추가합니다.

   * 먼저 `&preset` 매개 변수가 필요한지 확인합니다.

      `config2` 매개 변수가 `companypreset`이면 *에 `&preset=parameter`가 필요하지 않습니다.*

      `config2` 이 다른 경우 사전 설정된 매개 변수를 `config2` 매개 변수와 동일하게 설정합니다. 예를 들어 `config2=companypreset-2`이면 AppMeasurementBridge.jsp URL에 `&param2=companypreset-2`을 추가합니다.
...
1. 다음을 수행하여 TrackingManager 구성 요소를 만듭니다.
   ...
   * 다음을 수행하여 구성 요소를 TrackingManager에 연결합니다.
      ...
      예를 들어 구성 요소가 `videoPlayer`이면 를 추가합니다.
...
1. 타사 응용 프로그램 또는 서비스를 사용하여 비디오 캡션/자막 파일을 만듭니다.
   ...
   도구의 **[!UICONTROL 비디오 파일의 URL 입력]** 필드에서 복사한 비디오 파일의 URL을 붙여 넣은 다음 **[!UICONTROL 로드]**&#x200B;를 누릅니다. [비디오 파일 자체에 URL을 가져올 수 있는 URL을 가져오려면 ](linking-urls-to-yourwebapplication.md#obtaining-a-url-for-an-asset)비디오 파일 필드의 URL 입력&#x200B;]**을 참조하십시오.**[!UICONTROL  Internet Explorer, Chrome 또는 Safari는 기본적으로 비디오를 재생할 수 있습니다.
...
1. 다음 중 하나를 수행하십시오.
   ...
   * 팝업 비디오 뷰어 환경의 경우 **[!UICONTROL URL]**&#x200B;을 누릅니다. URL 대화 상자에서 URL을 선택하여 클립보드에 복사한 다음 URL을 단순 텍스트 편집기에 복사합니다. 비디오의 복사된 URL을 다음 구문과 함께 추가합니다.

      `&caption=<server_path>/is/content/<path_to_caption.vtt_file,1>`

      캡션 경로의 끝에 `,1`을 참고하십시오. 경로에서 .vtt 파일 이름 확장자 바로 다음에 `,1` 또는 `,0`로 설정하여 비디오 플레이어 모음에서 닫힌 캡션 단추를 활성화(켜기) 또는 비활성화(해제)하는 옵션이 각각 있습니다.

   * 포함된 비디오 뷰어 환경의 경우 **[!UICONTROL 포함 코드]**&#x200B;를 누릅니다. 포함 코드 대화 상자에서 포함 코드를 선택하고 클립보드에 복사한 다음 단순 텍스트 편집기에 붙여넣습니다. 복사된 포함 코드를 다음 구문과 함께 추가합니다.
...
1. 장 제목 텍스트에서 문자 표현물과 관련된 문제를 방지하려면 `.vtt` 파일을 UTF8 인코딩으로 저장하십시오.
   ...
   <table> 
     <tbody> 
      <tr> 
       ...
       <td> 
       <ol> 
       <li>업로드한 장 파일과 연결할 <i>게시된 </i>비디오 자산으로 이동합니다. URL은 </i> 다음에 <i>을 복사하는 데에만 사용할 수 있으며, 먼저 <i>자산을 게시했습니다</i>. <a href="/help/assets/publishing-dynamicmedia-assets.md">자산 게시를 참조하십시오.</a></i></li> 
       <li>드롭다운 메뉴에서 <strong>Viewers</strong>를 누릅니다.</li> 
       <li>왼쪽 레일에서 비디오 뷰어 사전 설정 이름을 탭합니다. 비디오 미리 보기가 별도의 페이지에 열립니다.</li> 
       <li>왼쪽 레일의 하단에서 <strong>URL</strong>을 누릅니다.</li> 
      ...
      <tr> 
       ...
       <li>드롭다운 메뉴에서 <strong>Viewers</strong>를 누릅니다.</li> 
       <li>왼쪽 레일에서 비디오 뷰어 사전 설정 이름을 탭합니다. 비디오 미리 보기가 별도의 페이지에 열립니다.</li> 
       <li>왼쪽 레일의 하단에서 <strong>포함</strong>을 누릅니다.</li> 
...
1. **[!UICONTROL 축소판 그림 변경]** 페이지의 도구 모음에서 **[!UICONTROL 프레임 선택]**&#x200B;을 누릅니다.

   Dynamic Media은 사용자 정의한 기본 시간 간격이나 시간 간격에 따라 비디오에서 시리즈 축소판 이미지를 생성합니다.

1. 생성된 축소판 이미지를 미리 보고 비디오에 추가할 이미지를 선택합니다.
1. **[!UICONTROL 변경 내용 저장]**&#x200B;을 누릅니다.

   선택한 축소판을 사용하도록 비디오의 축소판 이미지가 업데이트됩니다. 나중에 축소판 이미지를 변경하기로 결정한 경우 **[!UICONTROL 축소판 그림 변경]** 페이지로 돌아가서 새 이미지를 선택할 수 있습니다.
...
[비디오 축소판 추가](#adding-a-video-thumbnail)를 참조하십시오.

비디오 축소판이 생성되는 기본 시간 간격을 구성하려면

1. AEM에서 **[!UICONTROL 도구 > 일반 > CRXDE Lite]**&#x200B;를 누릅니다.

1. CRXDE Lite 페이지의 왼쪽 디렉토리 패널에서 `o etc/dam/imageserver/configuration/jcr:content/settings.` 로 이동합니다.
...
1. **[!UICONTROL 확인]**&#x200B;을 눌러 **[!UICONTROL 속성]** 탭으로 돌아갑니다.
1. CRXDE Lite 페이지의 왼쪽 위 모서리 근처에 있는 **[!UICONTROL 모두 저장]**&#x200B;을 탭한 다음, 왼쪽 위 모서리에 있는 **[!UICONTROL 홈]** 아이콘을 탭하여 AEM으로 돌아갑니다.
...
1. 사용할 축소판 이미지로 이동하여 선택한 다음 **[!UICONTROL 열기]**&#x200B;를 탭하여 AEM에 이미지 업로드를 시작합니다
1. 이미지가 업로드되면 **[!UICONTROL 축소판 변경]** 페이지에서 **[!UICONTROL 변경 내용 저장]**&#x200B;을 누릅니다.
