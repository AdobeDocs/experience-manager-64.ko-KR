...
**Google 클라우드 설정을 구성하려면 다음을 수행하십시오**.
...
1. 자산 세부 사항에서 진행 상황을 봅니다. 자산을 탭할 때 드롭다운 메뉴를 열고 **[!UICONTROL 타임라인]**&#x200B;을 선택합니다. 인코딩 또는 YouTube 게시와 같은 워크플로우 활동으로 범위를 좁히려면 **[!UICONTROL 워크플로]**&#x200B;를 선택합니다.
   ...
   인코딩과 같은 모든 워크플로우 정보가 타임라인에 표시됩니다. YouTube 게시의 경우 **[!UICONTROL 워크플로우]** 타임라인에도 YouTube 채널의 이름과 YouTube 비디오 URL이 포함됩니다. 또한 **[!UICONTROL 워크플로]** 타임라인에 오류 알림이 표시됩니다.
...
1. 진행 중인 워크플로우의 경우 **도구 > 워크플로 > 인스턴스]**&#x200B;에서 사용할 수 있는 워크플로 인스턴스&#x200B;**를 참조하십시오.**[!UICONTROL 
   ...
   **[!UICONTROL 워크플로우 인스턴스]** 영역에서 워크플로우를 일시 중단하거나 종료하거나 이름을 변경할 수도 있습니다. 자세한 내용은 [워크플로우 관리](/help/sites-administering/workflows-administering.md)를 참조하십시오.
...
1. 제3자 응용 프로그램 또는 서비스를 사용하여 비디오 캡션/자막 파일을 만듭니다.
   ...
   도구에서 **[!UICONTROL 비디오 파일]** 필드의 URL 입력 필드에서 비디오 파일의 복사한 URL을 붙여 넣은 다음 **[!UICONTROL 로드]**&#x200B;를 누릅니다. 비디오 파일 자체에 URL을 가져온 다음 비디오 파일의 **[!UICONTROL URL 입력 필드]**&#x200B;에 붙여 넣을 수 있는 에셋](linking-urls-to-yourwebapplication.md#obtaining-a-url-for-an-asset)에 대한 URL 얻기를 참조하십시오. [ 그러면 Internet Explorer, Chrome 또는 Safari에서 비디오를 기본적으로 재생할 수 있습니다.
...
장 마커를 단일 비디오 또는 응용 비디오 집합에 추가하여 긴 형식의 비디오를 보다 쉽게 보고 탐색할 수 있습니다. 사용자가 비디오를 재생하면 비디오 타임라인의 장 마커(비디오 스크러버라고도 함)를 눌러 원하는 지점을 손쉽게 탐색하거나 새로운 컨텐츠, 데모, 자습서 등으로 바로 이동할 수 있습니다.
   ...
   <table> 
     <tbody> 
      <tr> 
       ...
       <td> 
       <ol> 
       <li>업로드한 장 파일과 연결할 <i>게시된 </i>비디오 자산으로 이동합니다. URL은 <i>after</i> 다음에 <i>게시된</i>만 복사할 수 있습니다. <a href="/help/assets/publishing-dynamicmedia-assets.md">자산 게시를 참조하십시오.</a></li> 
       <li>드롭다운 메뉴에서 <strong>뷰어</strong>를 탭합니다.</li> 
       <li>왼쪽 레일에서 비디오 뷰어 사전 설정 이름을 누릅니다. 비디오의 미리 보기가 별도의 페이지에 열립니다.</li> 
       <li>왼쪽 레일의 하단에서 <strong>URL</strong>을 탭합니다.</li> 
       <li>URL 대화 상자에서 URL을 선택하여 클립보드에 복사한 다음 URL을 지나 간단한 텍스트 편집기로 복사합니다.</li> 
       <li>비디오의 복사된 URL을 다음 구문과 함께 추가하여 복사한 URL을 장 파일에 연결합니다.<br /> <br /> <code>&amp;navigation=&lt;<i>full_copied_URL_path_to_chapter_file</i>.vtt&gt;</code><br /> </li> 
      </ol> </td> 
      </tr> 
      <tr> 
       ...
       <li>드롭다운 메뉴에서 <strong>뷰어</strong>를 탭합니다.</li> 
       <li>왼쪽 레일에서 비디오 뷰어 사전 설정 이름을 누릅니다. 비디오의 미리 보기가 별도의 페이지에 열립니다.</li> 
       <li>왼쪽 레일의 하단에서 <strong>포함</strong>을 탭합니다.</li> 
...
Dynamic Media에서 자동으로 생성한 10개의 축소판 이미지 중 하나를 선택하여 비디오에 추가할 수 있습니다. 비디오 에셋이 AEM Sites, AEM Mobile 또는 AEM Screens의 작성 환경에서 Dynamic Media 구성 요소와 함께 사용될 경우 비디오 플레이어에 선택한 축소판이 표시됩니다. 축소판은 전체 비디오의 컨텐츠를 가장 잘 나타내는 정적 사진 역할을 하며 사용자가 [재생] 단추를 누를 수 있도록 합니다.

비디오의 총 시간을 기반으로 Dynamic Media은 10개의 축소판 이미지를 비디오에 1%, 11%, 21%, 31%, 41%, 51%, 61%, 71%, 81% 및 91%로 캡처합니다. 10개의 축소판은 지속되므로 나중에 다른 축소판을 선택하더라도 시리즈를 다시 생성할 필요가 없습니다. 10개의 축소판 이미지를 미리 본 다음 비디오에 사용할 이미지를 선택합니다. 기본값으로 변경하려면 CRXDE Lite을 사용하여 축소판 이미지가 생성되는 시간 간격을 구성할 수 있습니다. 예를 들어 비디오에서 간격을 일정하게 두고 4개의 축소판 이미지만 생성하려는 경우 간격 시간을 24%, 49%, 74% 및 99%로 구성할 수 있습니다.

가장 좋은 방법은 비디오를 업로드한 후 언제든지 비디오 축소판을 추가할 수 있지만 웹 사이트에 비디오를 게시하기 전에 추가합니다.

원하는 경우 Dynamic Media에서 생성된 축소판을 사용하는 대신 사용자 정의 축소판을 업로드하여 비디오를 나타낼 수 있습니다. 예를 들어 비디오의 제목, 시선을 사로잡는 열기 이미지 또는 비디오에서 캡처한 매우 특정 이미지가 있는 사용자 정의 축소판 이미지를 만들 수 있습니다. 업로드하는 사용자 정의 비디오 축소판 이미지는 최대 해상도 1280 x 720픽셀(최소 폭 640픽셀)이며 2MB보다 크지 않아야 합니다.

>[!NOTE]
>
>사용자 정의 비디오 축소판은 Dynamic Media - 하이브리드 모드를 실행할 때만 사용할 수 있습니다.

### 비디오 축소판 {#adding-a-video-thumbnail} 추가

1. 비디오 축소판을 추가할 업로드된 비디오 자산으로 이동합니다.
1. 자산 선택 모드에서 **[!UICONTROL 목록 보기]** 또는 **[!UICONTROL 카드 보기]**&#x200B;에서 비디오 자산을 탭합니다.
1. 도구 모음에서 **[!UICONTROL 속성 보기]** 아이콘(&quot;i&quot;가 포함된 원)을 누릅니다.
1. 비디오의 **[!UICONTROL 속성]** 페이지에서 **[!UICONTROL 축소판 변경]**&#x200B;을 탭합니다.
1. **[!UICONTROL 축소판 변경]** 페이지의 도구 모음에서 **[!UICONTROL 프레임 선택]**&#x200B;을 탭합니다.

   Dynamic Media은 사용자가 지정한 기본 시간 간격 또는 시간 간격에 따라 비디오에서 축소판 이미지를 생성합니다.

1. 생성된 축소판 이미지를 미리 본 다음 비디오에 추가할 이미지를 선택합니다.
1. **[!UICONTROL 변경 내용 저장]**&#x200B;을 누릅니다.

   선택한 축소판을 사용하도록 비디오의 축소판 이미지가 업데이트됩니다. 나중에 축소판 이미지를 변경하기로 결정한 경우 **[!UICONTROL 축소판 변경]** 페이지로 돌아가서 새 이미지를 선택할 수 있습니다.

   새 기본 시간 간격을 구성했거나 기존 비디오를 대체할 새 비디오를 업로드한 경우 Dynamic Media에서 축소판을 다시 생성하도록 해야 합니다.
...
새 기본 시간 간격을 구성하고 저장할 때 변경 내용은 나중에 업로드하는 비디오에만 자동으로 적용됩니다. 이전에 업로드한 비디오에 새 기본값이 자동으로 적용되지 않습니다. 기존 비디오의 경우 축소판을 다시 생성해야 합니다.
...
1. CRXDE Lite 페이지의 왼쪽에 있는 디렉토리 패널에서 `o etc/dam/imageserver/configuration/jcr:content/settings.`으로 이동합니다.

   디렉토리 패널이 보이지 않으면 [홈] 탭의 왼쪽에 있는 >> 아이콘을 탭해야 할 수 있습니다.
...
1. [축소판 편집] 대화 상자에서 텍스트 필드를 사용하여 간격 값을 백분율로 입력합니다.
   ...
   * 간격 값 필드의 오른쪽에 있는 빼기 기호(-) 아이콘을 눌러 목록에서 삭제합니다.
   * 위쪽 화살표 아이콘과 아래쪽 화살표 아이콘을 눌러 간격 값의 순서를 변경합니다.

1. **[!UICONTROL 확인]**&#x200B;을 눌러 **[!UICONTROL 속성]** 탭으로 돌아갑니다.
1. CRXDE Lite 페이지의 왼쪽 위 모서리 근처에 있는 **[!UICONTROL 모두 저장]**&#x200B;을 탭한 다음 왼쪽 위 모서리에 있는 **[!UICONTROL 뒤로 홈]** 아이콘을 탭하여 AEM으로 돌아갑니다.

   [비디오 축소판 추가를 참조하십시오.](#adding-a-video-thumbnail)

### 사용자 정의 비디오 축소판 {#adding-a-custom-video-thumbnail} 추가
...
1. 도구 모음에서 **[!UICONTROL 속성 보기]** 아이콘(&quot;i&quot;가 포함된 원)을 누릅니다.
1. 비디오의 **[!UICONTROL 속성]** 페이지에서 **[!UICONTROL 축소판 변경]**&#x200B;을 탭합니다.
1. **[!UICONTROL 축소판 변경]** 페이지의 도구 모음에서 **[!UICONTROL 새 축소판 업로드]**&#x200B;를 탭합니다.
1. 사용할 축소판 이미지로 이동하여 선택한 다음 **[!UICONTROL 열기]**&#x200B;를 눌러 AEM에 이미지를 업로드합니다.
