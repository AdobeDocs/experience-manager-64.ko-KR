...
메타데이터 가져오기는 비동기 방식이므로 시스템 성능을 방해하지 않습니다. 워크플로우 플래그를 선택하는 경우 XMP writeback 작업 때문에 여러 자산에 대한 메타데이터를 동시에 업데이트할 경우 리소스가 많이 소모될 수 있습니다. 서버 사용량 감소 시 이러한 가져오기를 계획하여 다른 사용자에게 성능 영향을 줄 수 있습니다.
...
1. CSV 파일에 다음 매개 변수가 포함되어 있는지 확인합니다.
   ...
   | [!UICONTROL 워크플로우 실행] | 기본적으로 False입니다. true로 설정되고 기본 론처 설정이 이진 XMP 데이터에 메타데이터를 쓰는 `DAM Metadata WriteBack Workflow` 경우 해당 항목에 대해 적용됩니다. 실행 워크플로우 활성화는 시스템에 성능에 영향을 줍니다. |
