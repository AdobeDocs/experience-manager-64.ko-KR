...
1. (선택 사항) 메타데이터 스키마 편집기에서 조건부 적용할 새 필드를 만듭니다. **[!UICONTROL 설정]** 탭에서 이름과 속성 경로를 지정합니다.
   ...
   새 탭을 만들려면 **[!UICONTROL +]** 를 탭/클릭하여 탭을 추가한 다음 메타데이터 필드를 추가합니다.
...
1. 키-값 쌍은 양식 사용자에게 제공되는 옵션입니다. 수동으로 또는 JSON 파일에서 키-값 쌍을 제공할 수 있습니다.
   ...
   * JSON 파일에서 값을 동적으로 가져오려면 **[!UICONTROL JSON 경로 추가]**&#x200B;를 선택하고 JSON 파일의 경로를 제공합니다. AEM은 양식이 사용자에게 표시될 때 실시간으로 키-값 쌍을 가져옵니다.
...
1. **[!UICONTROL 자산 유형]** 필드와 페이지 수 및 작성자와 같은 필드(예: 문서 자산(PDF 및 Word) 간에 종속성을 만들려면 단계를 반복합니다.
