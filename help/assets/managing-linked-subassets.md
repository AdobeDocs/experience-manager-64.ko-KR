...
# 하위 자산을 사용하여 복합 자산 관리 {#managing-compound-assets}
...
1. AEM [데스크탑 앱을](https://helpx.adobe.com/experience-manager/desktop-app/aem-desktop-app.html)사용하여 AEM 자산 저장소를 로컬 시스템의 드라이브로 마운트합니다. 마운트된 드라이브 내에서 참조할 자산의 위치로 이동합니다.
1. 마운트된 드라이브에서 Illustrator 파일로 에셋을 드래그합니다.
1. Illustrator 파일을 마운트된 드라이브에 저장하거나 AEM 저장소에 [업로드합니다](managing-assets-touch-ui.md#uploading-assets) .
...
1. 도구 모음에서 **[!UICONTROL 속성]** 보기 아이콘을 클릭합니다. 속성 페이지에서 현재 자산을 참조하는 파일 목록이 기본 **[!UICONTROL 탭의 참조]** 열 아래에 **[!UICONTROL 나타납니다]** .
...
1. Photoshop 파일에 저장하여 마운트된 드라이브에 저장하거나 AEM 저장소에 [업로드할](managing-assets-touch-ui.md#uploading-assets) 수 있습니다.
...
1. 참조된 자산에는 참조되는 자산의 목록도 포함되어 있습니다. 참조된 자산 목록을 보려면 자산 세부 사항 페이지로 이동하여 [레일을](/help/sites-authoring/basic-handling.md#rail-selector)닫습니다.
