...
AEM 6에서 Oak로 전환하는 것과 함께 쿼리와 색인을 관리하는 방식에 몇 가지 주요 변경 사항이 적용되었습니다. Jackrabbit 2에서 모든 컨텐츠는 기본적으로 인덱싱되어 있으며 자유롭게 쿼리할 수 있습니다. Oak에서 인덱스는 노드 아래에 수동으로 만들어야 `oak:index` 합니다. 쿼리가 인덱스 없이 실행될 수 있지만 큰 데이터 집합의 경우 매우 느리게 실행되거나 중단됩니다.
...
또한 택소노미를 디자인할 때 주문 중요성이 있는지 고려해야 합니다. 명시적 순서가 필요하지 않고 많은 수의 동기 노드가 예상될 경우 `sling:Folder` 또는 `oak:Unstructured`과 같이 순서가 지정되지 않은 노드 유형을 사용하는 것이 좋습니다. 주문이 필요한 경우, `nt:unstructured` 그리고 `sling:OrderedFolder` 더욱 적절할 것입니다.
...
노드를 간단히 검색할 수 있고, 해당 하위 노드를 구문 분석하여 구성 요소를 렌더링하는 데 사용할 수 있습니다. `/content/myUnstructuredContent/parentCategory/childCategory`
...
인덱스를 사용하지 않는 쿼리를 실행하면 노드 트래픽에 대한 경고가 기록됩니다. 이 쿼리가 자주 실행될 경우 인덱스를 만들어야 합니다. 특정 쿼리를 사용하는 인덱스를 확인하려면 쿼리 설명 도구 [를](/help/sites-administering/operations-dashboard.md#explain-query) 사용하는 것이 좋습니다. 자세한 내용은 관련 검색 API에 대해 DEBUG 로깅을 활성화할 수 있습니다.
...
>[!NOTE]
>
>QueryBuilder를 사용할 때 기본적으로 결과 카운트가 결정됩니다. 이전 버전의 Jackrabbit와 비교하여 Oak에서는 더 느립니다. 이를 보상하려면 [guessTotal 매개 변수를 사용할 수 있습니다](/help/sites-developing/querybuilder-api.md#using-p-guesstotal-to-return-the-results).
...
이 방법에 대한 자세한 내용은 [로깅 설명서를 참조하십시오](/help/sites-deploying/configure-logging.md).

### 색인 통계 {#index-statistics}

Lucene은 각 색인에 있는 문서의 크기 및 수를 비롯하여 인덱싱된 컨텐츠에 대한 세부 사항을 제공하는 JMX bean을 등록합니다.

JMX 콘솔에 액세스하면 `https://server:port/system/console/jmx`

JMX 콘솔에 로그인하고 나면 Lucene **Index Statistics를** 검색하여 찾습니다. 다른 인덱스 통계는 IndexStats **MBean에서** 찾을 수 있습니다.

쿼리 통계를 보려면 Oak 쿼리 통계라는 MBean을 **살펴보십시오**.

Luke [와](https://code.google.com/p/luke/)같은 도구를 사용하여 색인을 분석하려면 Oak 콘솔을 사용하여 색인을 파일 시스템 디렉토리 `NodeStore` 로 덤프해야 합니다. 이 방법에 대한 지침은 [Lucene 설명서를 참조하십시오](https://jackrabbit.apache.org/oak/docs/query/lucene.html).

시스템의 색인을 JSON 형식으로 추출할 수도 있습니다. 이렇게 하려면 `https://server:port/oak:index.tidy.-1.json`

### 쿼리 제한 {#query-limits}

**개발 중**

(예: `oak.queryLimitInMemory` 1000) 및 oak `queryLimitReads` (예: 5000) &quot;쿼리가 x 노드보다 많이 읽었습니다.&quot;라는 UnsupportedOperationException을 히트할 때 고가의 쿼리를 최적화합니다.
...
* Lucene 색인은 속성 색인보다 많은 기능을 제공합니다. 예를 들어 속성 색인은 단일 속성만 색인화할 수 있고 Lucene 색인은 여러 개를 포함할 수 있습니다. Lucene 색인에서 사용 가능한 모든 기능에 대한 자세한 내용은 [설명서를 참조하십시오](https://jackrabbit.apache.org/oak/docs/query/lucene.html).
...
* 비동기적 관계로 루센 색인은 고유성 제약 조건을 적용할 수 없습니다. 필요한 경우 속성 색인을 배치해야 합니다.
...
개발 환경을 위해 AEM 서버에 임베디드 상태를 실행하도록 솔루션 인덱스를 구성하거나 원격 인스턴스로 오프로드하여 프로덕션 및 스테이징 환경에서 검색 확장성을 향상시킬 수 있습니다. 검색 오프로드는 확장성을 향상시키지만 지연을 유발하며, 이로 인해 필요한 경우가 아니면 권장되지 않습니다. Solr 통합을 구성하는 방법과 Solr 색인을 만드는 방법에 대한 자세한 내용은 [Oak 쿼리 및 색인 설명서를 참조하십시오](/help/sites-deploying/queries-and-indexing.md#the-solr-index).

>[!NOTE]
>
>통합 솔루션 검색 방법을 사용하면 Solr 서버에 색인 로드를 해제할 수 있습니다. 크롤러 기반 접근 방식을 통해 Solaris 서버의 고급 기능을 사용하는 경우 추가 구성 작업이 필요합니다. Headwire는 이러한 유형의 구현을 가속화하기 위해 [오픈 소스 커넥터를](https://www.aemsolrsearch.com/#/) 만들었습니다.
...
* 쿼리에서 다른 경로 제한을 사용하는 경우 을 활용합니다 `evaluatePathRestrictions`. 이렇게 하면 쿼리는 지정된 경로 아래의 결과 하위 집합을 반환한 다음 쿼리를 기준으로 필터링할 수 있습니다. 그렇지 않으면 쿼리는 저장소의 쿼리 매개 변수와 일치하는 모든 결과를 검색한 다음 경로를 기반으로 필터링합니다.
* 쿼리에 정렬을 사용하는 경우 정렬된 속성에 대한 명시적 속성 정의 `ordered` 를 가지고 있고 해당 속성에 대해 설정 `true` 을 할 수 있습니다. 이렇게 하면 결과를 인덱스와 같이 정렬하고 쿼리 실행 시 비용이 많이 드는 정렬 작업에 저장할 수 있습니다.

* 필요한 것만 색인에 넣어주세요 불필요한 기능 또는 속성을 추가하면 인덱스가 증가하고 성능이 저하됩니다.
* 속성 인덱스에서 고유한 속성 이름을 갖는 것은 색인의 크기를 줄이는 데 도움이 되지만 Lucene 색인의 경우, 응집된 색인을 얻기 위해 를 사용하고 `nodeTypes` 있어야 `mixins` 합니다. 특정 `nodeType` 또는 `mixin` 을 쿼리하는 것은 쿼리하는 것보다 더 많은 성능입니다 `nt:base`. 이 방법을 사용할 때 해당 `indexRules` 에 대해 `nodeTypes` 정의합니다.
...
이 파일을 원격으로 저장하는 경우 `NodeStore` , 호출된 옵션을 활성화할 `CopyOnRead` 수 있습니다. 이 옵션을 선택하면 원격 인덱스가 읽힐 때 로컬 파일 시스템에 기록됩니다. 이렇게 하면 이러한 원격 색인에 대해 종종 실행되는 쿼리의 성능을 향상시키는 데 도움이 됩니다.

LuceneIndexProvider **** 서비스의 OSGi 콘솔에서 구성할 수 있으며 Oak 1.0.13을 기준으로 기본적으로 활성화됩니다.

### 인덱스 제거 {#removing-indexes}

인덱스를 제거할 때는 항상 속성을 로 설정하고 실제로 삭제하기 전에 응용 프로그램이 제대로 작동하는지 확인하기 위해 테스트를 수행하여 인덱스 `type` 를 일시적으로 비활성화하는 `disabled` 것이 좋습니다. 색인은 비활성화되어 있는 동안 업데이트되지 않으므로, 다시 활성화되고 색인을 다시 설정해야 하는 경우 올바른 내용이 없을 수 있습니다.
...
MongoDB 인스턴스에서 인덱스를 제거하면 삭제 비용이 인덱스의 노드 수에 비례합니다. 큰 인덱스를 삭제하면 문제가 발생할 수 있으므로 유지 관리 기간 중에만 인덱스를 비활성화하고 **oak-mongo.js와 같은 도구를 사용하는 것이 좋습니다**. 데이터 불일치를 도입할 수 있으므로 일반 노드 컨텐츠에 이 방법을 사용하면 안 됩니다.
...
이 섹션에서는 Oak 색인을 다시 색인화할 수 있는 **유일한** 가능한 이유에 대해 설명합니다.

아래 설명된 이유 이외에 Oak 색인의 재색인을 시작하는 것은 동작을 변경하거나 문제를 해결하지 **않으며** AEM의 부하를 일시적으로 증가시키지 않습니다.
...
### Oak 색인 구성 변경 {#oak-index-configuration-changes}
...
* 해결 방법:
   ...
   * [루센 색인을 다시](/help/sites-deploying/best-practices-for-queries-and-indexing.md#how-to-re-index) 색인화합니다.
...
* 확인 방법:
   ...
   * Lucene Index 통계 JMX Mbean(LuceneIndex) 메서드를 사용하여 인덱스 정의가 변경되었는지 확인합니다 `diffStoredIndexDefinition`.

* 해결 방법:

   * 1.6 이전의 Oak 버전:

      * [루센 색인을 다시](#how-to-re-index) 색인화합니다.
   * Oak 버전 1.6+

      * 기존 컨텐츠가 변경 사항으로 영향을 받지 않으면 새로 고침만 필요합니다

         * [oak:queryIndexDefinition](https://jackrabbit.apache.org/oak/docs/query/lucene.html#stored-index-definition) []@refresh=true를 설정하여 lucene 인덱스 새로 고침
      * 그렇지 않으면 [루센 색인을 다시](#how-to-re-index) 색인화합니다.

         * 참고: 마지막 올바른 다시 색인(또는 초기 색인)의 인덱스 상태는 새로운 다시 색인이 트리거될 때까지 사용됩니다



### 예외 및 탁월한 상황 {#erring-and-exceptional-situations}

다음 표에서는 Oak 색인을 다시 인덱싱하여 문제를 해결할 수 있는 허용 가능한 예외 및 예외 상황에 대해 설명합니다.

AEM에서 아래에 나와 있는 기준과 일치하지 않는 문제가 발생하는 경우 문제를 해결하지 않으므로 색인을 다시 색인화하지 **마십시오** .
...
* 해결 방법:
   ...
   * 그렇지 않으면 [모든](#how-to-re-index) 루센 색인을 다시 ** 색인화합니다.
   * 메모:
...
* 확인 방법:
   ...
   * error.log에서 `AsyncIndexUpdate` (5초)는 예외로 인해 실패합니다.
...
* 해결 방법:
   ...
   * lucene 색인의 로컬 복사본 제거
      ...
      1. AEM 다시 시작
   * 문제가 해결되지 않고 예외가 계속되는 경우 `AsyncIndexUpdate` 다음을 수행합니다.

      1. [참조 색인](#how-to-re-index) 다시 색인 지정
      1. 또한 [Adobe 지원](https://helpx.adobe.com/support.html) 티켓 파일
...
>[!NOTE]
>
>AEM 6.4에서 [oak-run.jar는 MongoMK 또는 RDBMK 저장소에서 재색인화를 위해 지원되는 유일한](/help/sites-deploying/indexing-via-the-oak-run-jar.md#reindexingapproachdecisiontree) 방법입니다.

#### 속성 인덱스 다시 인덱싱 {#re-indexing-property-indexes}

* oak-run.jar [](/help/sites-deploying/oak-run-indexing-usecases.md#usecase3reindexing) 를 사용하여 속성 색인을 다시 인덱싱합니다.
* 속성 인덱스에서 async-reindex 속성을 true로 설정합니다.

   * `[oak:queryIndexDefinition]@reindex-async=true`

* PropertyIndexAsyncReindex MBean을 통해 웹 콘솔을 사용하여 **속성** 색인을 비동기식으로 다시 인덱싱합니다.
...
* oak- [run.jar를 사용하여 Lucene 속성 색인을 다시](/help/sites-deploying/oak-run-indexing-usecases.md#usecase3reindexing) 인덱싱합니다.
...
>[!NOTE]
>
>위의 섹션에서는 AEM 컨텍스트에서 [Apache Oak 설명서의 Oak 색인 지침을](https://jackrabbit.apache.org/oak/docs/query/indexing.html#reindexing) 요약하고 프레임을 만듭니다.

### 이진 파일의 텍스트 사전 추출 {#text-pre-extraction-of-binaries}

텍스트 사전 추출은 분리된 프로세스를 통해 데이터 저장소에서 바로 바이너리에서 텍스트를 추출하고 처리한 후 추출된 텍스트를 Oak 인덱스의 후속 재인덱스에 직접 노출시키는 프로세스입니다.

* 추출할 수 있는 텍스트(예: 이진 파일)가 많은 저장소에서 Lucene 색인을 다시 인덱싱하는 데 Oak 텍스트 사전 추출을 사용하는 것이 좋습니다. PDF, Word Docs, PPTs, TXT 등 배포되는 Oak 색인을 통해 전체 텍스트 검색을 수행할 수 있습니다. 예를 들면 다음과 같습니다 `/oak:index/damAssetLucene`.
...
이진 추출이 활성화된 **기존** 루센 색인 다시 인덱싱

* 저장소의 **모든** 후보 컨텐츠를 다시 인덱싱하고 전체 텍스트를 추출할 바이너리가 많거나 복잡한 경우 전체 텍스트 추출을 수행하는 계산 부담이 AEM에 배치됩니다. 텍스트 사전 추출을 사용하면 텍스트 추출 작업의 &quot;계산 비용이 많이 드는 작업&quot;을 AEM Data Store에 직접 액세스하는 분리된 프로세스로 옮길 수 있으므로 AEM에서 발생하는 오버헤드 및 리소스 경합을 방지할 수 있습니다.

이진 추출을 사용하는 AEM에 **새** lucene 인덱스 배포 지원

* 새로운 인덱스(이진 추출을 사용하는 경우)가 AEM에 배포되면 Oak는 다음 비동기 전체 텍스트 색인 실행 시 모든 후보 컨텐츠를 자동으로 인덱싱합니다. 위의 다시 색인화에 설명된 동일한 이유로 AEM에서 과도하게 로드될 수 있습니다.
...
#### 텍스트 사전 추출 사용 전제 조건 {#prerequisites-to-using-text-pre-extraction}
...
1a. 텍스트 `oak-run.jar --generate` 를 사전에 추출할 노드 목록을 생성하려면 실행합니다.

1b. 노드 목록(1a)이 파일 시스템에 CSV 파일로 저장됩니다

Note that the entire Node Store is browsed (as specified by the paths in the oak-run command) `--generate` and a **new** CSV file is created. CSV 파일은 텍스트 사전 추출 프로세스의 개별 실행 간에 다시 사용되지 **않습니다** (단계 1 - 2).
...
3a. [AEM에서 Lucene](#how-to-re-index) 색인의 재색인 호출

3b. Apache Jackrabbit Oak DataStore PreExtractedTextProvider OSGi 구성(파일 시스템 경로를 통해 추출된 텍스트를 가리키도록 구성)은 Oak가 압축을 푼 파일에서 전체 텍스트 텍스트를 소스에 직접 캡처하여 처리하지 않도록 합니다.
