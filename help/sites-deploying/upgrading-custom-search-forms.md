...
자세한 내용은 검색 패싯 [에 대한 설명서를 참조하십시오](/help/assets/search-facets.md).

## resourceType 속성 변경 {#changing-the-resourcetype-property}

별도로 명시되어 있지 않는 한, 업그레이드 후에 수행해야 하는 조정 사항 대부분은 구성된 사용자 지정 검색 Forms의 `sling:resourceType` 속성을 변경해야 합니다. 속성이 렌더링 스크립트의 올바른 위치를 가리키도록 필요합니다.

다음을 수행하여 속성을 변경할 수 있습니다.

1. 이동 `https://server:port/crx/de/index.jsp`
...
아래에는 모든 사용자 정의 검색 Forms의 목록과 업그레이드 후 필요한 수정 사항이 나와 있습니다. 그들은 안에 있는 이름을 말한다 `/conf/global/settings/cq/search/facets/sites/items`.
...
<table> 
 <tbody>
  ...
  <tr>
   <td>리소스 유형(6.2)</td> 
   <td><p>cq/gui/components<strong>/coral/</strong>common/admin/customsearch/searchiteschemates/daterangede술어</p> </td> 
  </tr>
 </tbody>
</table>
...
<table> 
 <tbody>
  ...
  <tr>
   <td>리소스 유형(6.2)</td> 
   <td><p>cq/gui/components<strong>/coral/</strong>siteadmin/admin/searchpanel/search예측자/analytics사용자 지정</p> </td> 
  </tr>
 </tbody>
</table>
...
<table> 
 <tbody>
  ...
  <tr>
   <td>리소스 유형(6.2)</td> 
   <td><p>cq/gui/components<strong>/coral/</strong>siteadmin/admin/searchpanel/searchecates/optionspredicate</p> </td> 
  </tr>
 </tbody>
</table>
...
<table> 
 <tbody>
  ...
  <tr>
   <td>리소스 유형(6.2)</td> 
   <td><p>cq/gui/components<strong>/coral/</strong>siteadmin/admin/searchpanel/search예측자/sliderrange술어</p> </td> 
  </tr>
 </tbody>
</table>
...
<table> 
 <tbody>
  ...
  <tr>
   <td>리소스 유형(6.2)</td> 
   <td><p>cq/gui/components<strong>/coral/</strong>siteadmin/admin/searchpanel/searchitects/componentspredicate</p> </td> 
  </tr>
 </tbody>
</table>
...
<table> 
 <tbody>
  ...
  <tr>
   <td>리소스 유형(6.2)</td> 
   <td><p>cq/gui/components<strong>/coral/</strong>siteadmin/admin/searchpanel/search예측/사용자 설명</p> </td> 
  </tr>
 </tbody>
</table>
...
<table> 
 <tbody>
  ...
  <tr>
   <td>리소스 유형(6.2)</td> 
   <td><p>cq/gui/components<strong>/coral/</strong>siteadmin/admin/searchpanel/search예측자/templateschemer</p> </td> 
  </tr>
 </tbody>
</table>
...
|---|---|
| 리소스 유형(6.1) | dam/gui/components/admin/customsearch/searchitets/publishpredicate |
| 리소스 유형(6.2) | dam/gui/coral/components/admin/customsearch/searchitets/publish술어 |

**작업:**

* 속성 `resourceType` 조정(위에 표시된 6.2 위치와 같이 &quot;**/coral**&quot; 추가)

* 값을 사용하여 (문자열 유형) `optionPaths` 속성을 추가합니다. `/libs/dam/options/predicates/publish`

* 부울 `singleSelect` 값을 사용하여 속성을 추가합니다 `true`.
...
**작업:** 속성 `resourceType` 조정(위에 표시된 6.2 위치와 같이 &quot;**/coral**&quot; 추가)
...
* 동일한 노드에서 속성과 동일한 값 `fieldLabel` `text` 의 속성을 추가합니다.

* 동일한 노드의 속성과 동일한 값 `emptyText` `text` 의 속성을 추가합니다.

* 동일한 노드의 속성과 동일한 값 `rootPath` `optionPaths` 의 속성을 추가합니다.
