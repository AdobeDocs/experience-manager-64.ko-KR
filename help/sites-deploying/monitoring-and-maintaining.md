...
description: AEM을 모니터링하는 방법을 알아봅니다.
seo-description: AEM을 모니터링하는 방법을 알아봅니다.
...
여기에서 중요한 점은 시스템이 정상적인 조건에서 어떻게 보이고 동작하는지 알아야 할 잠재적 문제를 인식하기 때문입니다. 이는 시스템을 모니터링하고 일정 기간 동안 정보를 수집하여 수행하는 것이 가장 좋습니다.

| 확인 | 고려 사항 | 주석/작업 |
|---|---|---|
| 백업 플랜. |  | 인스턴스 [백업 방법을 참조하십시오](/help/sites-deploying/monitoring-and-maintaining.md#backups). |
| 재해 복구 계획 | 귀사의 재해 복구 지침 |  |
| 오류 추적 시스템을 사용하여 문제를 보고할 수 있습니다. | 예를 들어 [bugzilla](https://www.bugzilla.org/), [jira](https://www.atlassian.com/software/jira/)또는 기타 여러 항목 중 하나입니다. |  |
...
| [AEM 성능이 모니터링되고](/help/sites-deploying/monitoring-and-maintaining.md#monitoring-performance)있습니다. | 트래픽 [수준을](/help/sites-deploying/monitoring-and-maintaining.md#request-counters) 모니터링하기 위한 요청 카운터를 포함합니다. | 장기적으로는 상당한 성과손실이 확인될 경우 상세한 조사를 실시해야 한다. |
| 복제 에이전트를 모니터링하고 [있습니다](/help/sites-deploying/monitoring-and-maintaining.md#monitoring-your-replication-agents).&quot; |  |  |
| 정기적으로 워크플로우 인스턴스 제거 | 저장소 크기 및 워크플로우 성능 | 워크플로우 [인스턴스의 정기적인 제거를 참조하십시오](/help/sites-administering/workflows-administering.md#regular-purging-of-workflow-instances). |
...
백업 관련 추가 고려 사항, 백업 대상 및 백업 시기에 대한 추가 고려 사항 등이 있을 수 있습니다.

* 시스템 및 데이터의 중요성
* 소프트웨어 또는 데이터가 변경되는 빈도
* 데이터 용량;경우에 따라 백업 수행에 필요한 시간과 마찬가지로 용량에 문제가 있을 수 있습니다.
* 사용자가 온라인 상태일 때 백업을 수행할 수 있는지 여부그리고 가능하다면 성능에 어떤 영향을 미칩니까?
* 사용자의 지리적 분포즉, 백업을 위한 최적의 시기는 언제입니까?
* 재해 복구 정책백업 데이터를 저장해야 하는 위치(예: 오프사이트, 특정 미디어 등)에 대한 지침이 있습니다.

종종 전체 백업이 정기적인 간격(예: 일별, 주별 또는 월별)으로 이루어지며, 증분 백업은 시간별, 일별 또는 주별) 사이에 수행됩니다.

>[!CAUTION]
>
>운영 인스턴스의 백업을 구현할 때 백업이 성공적으로 복원될 수 있도록 테스트를 *수행해야* 합니다.
>
>이렇게 하지 않으면 백업이 잠재적으로 무용지물이 될 수 있습니다(최악의 경우 시나리오).

>[!NOTE]
>
>백업 성능에 대한 자세한 내용은 백업 성능 [섹션을](/help/sites-deploying/configuring-performance.md#backup-performance) 참조하십시오.
...
1. AEM을 중지합니다.
1. 파일 `<cq-installation-dir>` 시스템에서 전체 백업

>[!CAUTION]
>
>타사 응용 프로그램 서버를 운영 중인 경우 다른 위치에 추가 폴더가 있을 수 있으며 백업해야 할 수도 있습니다. 애플리케이션 [서버 설치에 대한 자세한 내용은 Application Server와](/help/sites-deploying/application-server-install.md) 함께 AEM을 설치하는 방법을 참조하십시오.

>[!CAUTION]
>
>파일 데이터 저장소의 증분 백업이 지원됩니다.다른 구성 요소(예: Lucene 인덱스)에 대한 증분 백업을 사용하는 경우 삭제된 파일도 백업에서 삭제된 것으로 표시되는지 확인하십시오.
...
CRX [설명서의](/help/sites-administering/backup-and-restore.md) 백업 및 복원 섹션에서는 CRX 저장소의 백업과 관련된 모든 문제를 다룹니다.

온라인 &quot;핫&quot; 백업을 만드는 방법에 대한 자세한 내용은 온라인 백업 [만들기를 참조하십시오](/help/sites-administering/backup-and-restore.md#online-backup).

## 버전 제거 {#version-purging}

버전 **제거** 도구는 저장소의 노드 버전 또는 계층 구조를 제거하기 위한 것입니다. 기본 목적은 노드의 이전 버전을 제거하여 저장소 크기를 줄이는 데 도움이 됩니다.

이 섹션에서는 AEM의 버전 관리 기능과 관련된 유지 관리 작업을 다룹니다. 버전 **제거** 도구는 저장소의 노드 버전 또는 계층 구조를 제거하기 위한 것입니다. 기본 목적은 노드의 이전 버전을 제거하여 저장소 크기를 줄이는 데 도움이 됩니다.

### 개요 {#overview}

버전 **삭제** 도구는 버전 **[관리](/help/sites-administering/tools-consoles.md)콘솔에서&#x200B;**또는**다음 위치에서&#x200B;**직접 사용할 수 있습니다.&quot;
...
**재귀** 데이터를 제거할 때 재귀성을 선택하여 한 노드에서 작업을 수행하거나 전체 계층 구조에서 작업을 수행할 수 있습니다. 마지막 경우 지정된 경로는 계층의 루트 노드를 정의합니다.

**유지할** 최대 버전 노드에 유지할 최대 버전 수입니다. 이 수가 이 값을 초과하면 가장 오래된 버전은 삭제됩니다.

**최대 버전 페이지** 노드 버전의 최대 페이지 버전 사용 기간이 이 값을 초과하면 삭제됩니다.

**연습** 실행 컨텐츠 버전을 제거하는 것이 명확하며 백업을 복원하지 않으면 되돌릴 수 없으므로 제거 버전 도구는 삭제된 버전을 미리 볼 수 있는 드라이 실행 모드를 제공합니다. 제거 프로세스의 연습 실행을 실행하려면 연습 실행을 클릭합니다.
...
1. 도구 **[콘솔로](/help/sites-administering/tools-consoles.md)이동하고&#x200B;**버전**관리를 선택하고&#x200B;**버전**삭제를 두 번&#x200B;**클릭합니다.
1. 제거할 컨텐츠의 시작 경로(예:
...
1. 유지할 최대 버전 기간(일 단위)을 설정합니다. 이 설정을 사용하지 않으려면 비워 둡니다.

1. 연습 **실행을** 클릭하여 제거 프로세스를 미리 봅니다.
1. 삭제를 **클릭하여** 프로세스를 시작합니다.

>[!CAUTION]
>
>저장소를 복원하지 않으면 제거된 노드를 되돌릴 수 없습니다. 구성을 관리해야 하므로 제거하기 전에 항상 연습 실행을 수행하는 것이 좋습니다.

### 콘솔 분석 {#analyzing-the-console}

연습 **실행** 및 **제거** 프로세스는 처리된 모든 노드를 나열합니다. 이 프로세스 동안 노드에 다음 상태 중 하나가 있을 수 있습니다.

* `ignore (not versionnable)`:노드는 버전 지정을 지원하지 않으며 프로세스 동안 무시됩니다.
* `ignore (no version)`:노드에 버전이 없으며 프로세스 동안 무시됩니다.&quot;
...
* `Thu Mar 15 2012 08:37:32 GMT+0100`:버전 날짜.
...
AEM(Adobe Experience Manager)과 관련된 레코드 및 로그 파일을 감사하는 방법은 다양한 위치에서 찾을 수 있습니다. 찾을 수 있는 항목에 대한 개요를 제공하기 위해 다음과 같이 제공됩니다.
...
로그 파일 회전은 새 파일을 주기적으로 만들어 파일 증가를 제한하는 프로세스를 말합니다. AEM에서 호출된 로그 파일은 지정된 규칙에 따라 하루에 한 번 `error.log` 회전합니다.

* {original_filename} 패턴에 따라 `error.log` 파일의 이름이 바뀝니다 `.yyyy-MM-dd`. 예를 들어 2010년 7월 11일에 현재 로그 파일의 이름이 변경된 `error.log-2010-07-10`다음 새 파일이 `error.og` 만들어집니다.
* 이전 로그 파일은 삭제되지 않으므로 디스크 사용을 제한하기 위해 정기적으로 이전 로그 파일을 정리해야 합니다.

>[!NOTE]
>
>AEM 설치를 업그레이드하는 경우 AEM에서 더 이상 사용되지 않는 기존 로그 파일은 디스크에 남아 있습니다. 위험 없이 제거할 수 있습니다. 모든 새 로그 항목이 새 로그 파일에 기록됩니다.
...
여기에는 다양한 수준의 오류 메시지가 등록되어 있습니다.
   ...
   * [ `ImageServer-<PortId>-yyyy>-<mm>-<dd>.log`](https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/c_image_server_log.html)
      ...
      이 로그는 다이내믹 미디어가 활성화된 경우에만 사용됩니다. 내부 ImageServer 프로세스의 동작을 분석하는 데 사용되는 통계 및 분석 정보를 제공합니다.
...
시작 중에 이벤트를 나타내는 로깅 메시지를 보유합니다.

   * `upgrade.log`

      
및 패키지에서 실행되는 모든 업그레이드 작업 로그를 `com.day.compat.codeupgrade` 제공합니다 `com.adobe.cq.upgradesexecutor` .
...
>[!NOTE]
>
>ImageServer 및 s7access 로그는 **시스템/콘솔/status-Bundelist** 페이지에서 생성된 전체 **다운로드 패키지에 포함되지** 않습니다. 지원 목적으로 다이내믹 미디어 문제가 있는 경우 고객 지원에 문의할 때 ImageServer 및 s7access 로그를 추가하십시오.

### 디버그 로그 수준 활성화 {#activating-the-debug-log-level}

기본 로그 수준 Apache [Sling](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings) 로깅 구성은 정보이므로 디버그 메시지가 기록되지 않습니다.

로거에 대한 디버그 로그 수준을 활성화하려면 저장소에서 디버깅할 속성을 `org.apache.sling.commons.log.level` 설정합니다. 예를 들어 `/libs/sling/config/org.apache.sling.commons.log.LogManager` 전역 Apache Sling [로깅을 구성하려면](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)켜십시오.

>[!CAUTION]
>
>로그 항목이 많이 생성되므로 로그를 필요한 것보다 더 오래 디버그 로그 수준에서 유지하지 마십시오.
...
|---|---|---|
| 1 | 오류 | 작업이 실패했습니다. 설치가 진행되지만 AEM WCM의 일부가 올바로 설치되지 않아 작동하지 않습니다. |
...
1. 에서 `/apps/<project-name>/config`새 Apache Sling 로깅 [로거 구성에 대한 노드를 만듭니다](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings).

   * 이름:
   `org.apache.sling.commons.log.LogManager.factory.config-<identifier>` (로거이므로)

   여기서 은 인스턴스를 식별하기 위해 입력해야 하는 자유 텍스트로 `<identifier>` 대체됩니다(이 정보는 생략할 수 없음). 예, `org.apache.sling.commons.log.LogManager.factory.config-MINE`
...
1. 이 노드에서 다음 속성을 설정합니다.
   ...
   * 필요에 따라 다른 매개 변수를 구성합니다.
   ...
   >(예: 옆에 `<cq-installation-dir>/crx-quickstart/`)
...
1. 이 단계는 새 작성기가 필요한 경우에만 필요합니다(예: 기본 작성기와 다른 구성).
   ...
   에서 `/apps/<project-name>/config`새 Apache Sling 로깅 [기록기 구성에 대한 노드를 만듭니다](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings).
...
또한 Felix Console에서는 Sling Log 지원에 대한 정보를 `../system/console/slinglog`제공합니다.예를 `http://localhost:4502/system/console/slinglog`들면 다음과 같습니다.

### 감사 레코드 찾기 {#finding-the-audit-records}

감사 기록은 누가 언제 무엇을 했는지 기록하기 위해 보관된다. AEM WCM 및 OSGi 이벤트 모두에 대해 다른 감사 레코드가 생성됩니다.
...
폴더 `/var/audit` 내에서 감사 레코드는 리소스에 따라 보관됩니다. 개별 레코드와 포함된 정보를 볼 수 있을 때까지 드릴다운할 수 있습니다.

이러한 항목은 페이지를 편집할 때 표시되는 정보와 동일합니다.
...
대기열이 [](/help/sites-deploying/replication.md) 중지되거나 차단되는 경우를 감지하기 위해 복제 대기열을 모니터링할 수 있습니다. 이는 게시 인스턴스 또는 외부 시스템에 문제가 있을 수 있습니다.

* 모든 필수 큐를 사용할 수 있습니까?
* 비활성화된 대기열이 계속 필요합니까?
* 모든 `enabled` 대기열은 상태 `idle` 또는 `active`즉 정상적인 작업을 나타내야 합니다.대기열은 없어야 `blocked`하며, 이는 수신자측에서 문제가 발생할 수 있습니다.

* 대기열의 크기가 시간의 경과에 따라 증가하면 차단된 대기열이 표시될 수 있습니다.

복제 에이전트를 모니터링하려면

1. AEM의 **도구** 탭에 액세스합니다.
1. **복제**&#x200B;를 클릭합니다.
1. 해당 환경의 에이전트(왼쪽 또는 오른쪽 창)에 대한 링크를 두 번 클릭합니다.예를 **들어 작성자의**&#x200B;에이전트입니다.
...
1. 해당 에이전트 이름(링크임)을 클릭하여 해당 에이전트에 대한 자세한 정보를 표시합니다.
   ...
   * 에이전트의 활성화 여부를 확인합니다.
   * 모든 복제 타겟을 확인합니다.
   * 복제 큐가 현재 활성 상태인지(활성화) 여부를 확인합니다.
   * 대기열에 항목이 있는지 확인합니다.
   * **대기열** 항목 **표시를** 업데이트하려면 새로 고침 또는 지우기;이렇게 하면 항목이 큐에 들어가 대기하지 않는 것을 볼 수 있습니다.
   * **로그** 보기를 사용하여 복제 에이전트가 수행하는 모든 작업의 로그에 액세스합니다.
   * **대상** 인스턴스에 대한 연결 테스트를 참조하십시오.
   * **필요한 경우 모든** 큐 항목에 대해 강제로 다시 시도하십시오.
   >[!CAUTION]
   >
   >게시 인스턴스의 역방향 복제 보낼 상자에 대해 &quot;연결 테스트&quot; 링크를 사용하지 마십시오.
   >
   >Outbox 큐에 대한 복제 테스트가 수행되는 경우 테스트 복제보다 오래된 항목은 모든 역방향 복제를 통해 다시 처리됩니다.
   >
   >이러한 항목이 이미 대기열에 있으면 다음 XPath JCR 쿼리를 통해 찾을 수 있으며 제거해야 합니다.
   >
   >`/jcr:root/var/replication/outbox//*[@cq:repActionType='TEST']`

다시 한 번 솔루션 개발을 통해 `/etc/replication/author` 또는 에 있는 모든 복제 에이전트를 검색한 다음 에이전트(, `/etc/replication/publish`)와 기본 대기열( `enabled`, `disabled`, `active``idle``blocked`)의 상태를 확인할 수 있습니다.

## 모니터링 성능 {#monitoring-performance}

[성능 최적화는](/help/sites-deploying/configuring-performance.md) 개발 중에 초점을 맞춘 대화형 프로세스입니다. 배포 후에는 일반적으로 특정 간격 또는 이벤트 후에 검토됩니다.

최적화를 위한 정보를 수집하는 동안 사용되는 메서드를 지속적인 모니터링에도 사용할 수 있습니다.

>[!NOTE]
>
>성능을 [](/help/sites-deploying/configuring-performance.md#configuring-for-performance) 개선하기 위해 사용할 수 있는 특정 구성도 확인할 수 있습니다.

다음은 일반적인 성과 문제와 이를 찾아 대응하는 방법에 대한 제안서를 함께 나열합니다.

| 영역 | 증상 | 용량 증가 | 볼륨 줄이기... |
|---|---|---|---|
| 클라이언트 | 높은 클라이언트 CPU 사용. | 더 높은 성능을 가진 클라이언트 CPU를 설치합니다. | 레이아웃 간소화(HTML) |
|  | 서버 CPU 사용량이 낮습니다. | 더욱 빨라진 브라우저로 업그레이드 | 클라이언트측 캐시 향상 |
|  | 어떤 고객들은 빠르고 느리기도 합니다. |  |  |
| 서버 |  |  |  |
| 네트워크 | 서버와 클라이언트 모두에서 CPU 사용량이 낮습니다. | 네트워크 병목 현상을 제거합니다. | 클라이언트 캐시의 구성을 개선/최적화합니다. |
|  | 서버에서의 로컬 브라우징은 비교적 빠릅니다. | 네트워크 대역폭 증가 | 웹 페이지의 &quot;무게&quot;(예: 이미지 감소, 최적화된 HTML) 감소 |
...
|  | 응답 시간 단축 |  | 템플릿 및 구성 요소(예: 구조, 논리) 최적화 |
| 보관소 |  |  |  |
| 캐시 |  |  |  |

성능 문제는 연결 속도 저하, CPU 로드 등 웹 사이트와 관련이 없는 여러 가지 원인에서 발생할 수 있습니다.

또한 모든 방문자 또는 일부 방문자에게만 영향을 줄 수 있습니다.

일반적인 성능을 최적화하거나 특정 문제를 해결하려면 이 모든 정보를 얻고 정렬 및 분석해야 합니다.

* 성능 문제가 발생하기 전:

   * 정상 상황에서 시스템에 대한 충분한 정보를 축적하여 유용한 정보를 축적하다

* 성능 문제가 발생하는 경우:

   * 일반적인 성능이 뛰어난 다른 클라이언트 또는 서버 자체(가능한 경우)에서 표준 웹 브라우저 중 하나(또는 이상)와 복제해 보십시오.
   * 적절한 시간 공간 내에서 변경된 것이 있는지, 그리고 이러한 변경 사항 중 하나라도 성능에 영향을 줄 수 있는지 확인
   * 다음과 같은 질문

      * 문제는 특정 시에만 발생합니까?
      * 문제는 특정 페이지에서만 발생합니까?
      * 다른 요청이 영향을 받습니까?
   * 정상적인 상황에서 시스템에 대한 귀하의 지식과 비교할 수 있도록 가능한 많은 정보를 수집합니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>request.log</td> 
   <td>응답 시간 및 동시 시청</td> 
   <td><a href="#interpreting-the-request-log">request.log</a>해석.</td> 
  </tr> 
  <tr> 
   <td>트러스/스트레이트</td> 
   <td>페이지 로드</td> 
   <td><p>시스템 호출 및 신호를 추적하는 Unix/Linux 명령 로그 수준을 다음으로 <code>INFO</code>늘립니다.</p> <p>요청당 페이지 로드 수, 어떤 페이지 등을 분석합니다.</p> </td> 
  </tr> 
  <tr> 
   <td>스레드 덤프</td> 
  ...
  <tr> 
   ...
   <td><p>기본 사용:</p> <p><code>ab -k -n &lt;<em>requests</em>&gt; -c &lt;<em>concurrency</em>&gt; &lt;<em>url</em>&gt;</code></p> <p>자세한 <a href="#apache-bench">내용은 Apache Bench</a> 및 <a href="https://httpd.apache.org/docs/2.2/programs/ab.html">ab 매뉴얼 페이지를</a> 참조하십시오.</p> </td> 
  </tr> 
  <tr> 
   <td>검색 분석</td> 
   <td> </td> 
   <td>오프라인에서 검색 쿼리를 실행하고 쿼리 응답 시간을 식별하고 테스트 및 결과 집합을 확인합니다.<br /> </td> 
  </tr> 
  <tr> 
   <td>JMeter</td> 
   <td>로드 및 기능 테스트.</td> 
  ...
  <tr> 
   ...
   <td><p>사용:jvisualvm 또는 visualvm<br /> </p> <p>( <a href="https://java.sun.com/javase/6/docs/technotes/tools/share/jvisualvm.html">J)VisualVM을</a>사용하여 <a href="https://visualvm.github.io/releases.html">jvisualvm</a> , <a href="#monitoring-performance-using-j-visualvm">시각화</a>및모니터링 성능을 참조하십시오.</p> <p><strong></strong> 참고:JDK 1.6을 사용하면 VisualVM은 플러그인으로 확장할 수 있습니다.</p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>페이지 렌더링에 대한 시간 통계를 보려면 Ctrl-Shift- <strong>U를</strong> URL에 설정된 <code>?debugClientLibs=true</code> 세트와 함께 사용할 수 있습니다.</p> </td> 
  </tr> 
  <tr> 
   <td>CPU 및 메모리 프로파일링 툴<br /> </td> 
   <td><a href="#interpreting-the-request-log">개발</a>중에 느린 요청을 분석할 때 사용됩니다.</td> 
   <td>예: YourKit <a href="https://www.yourkit.com/">를 참조하십시오</a>.</td> 
  </tr> 
  <tr> 
   <td><a href="#information-collection">정보 수집</a></td> 
   <td>설치 진행 상태입니다.</td> 
   <td>또한 설치에 대해 가능한 한 많은 것을 알고 있으면 성능 변경을 야기했을 수 있는 내용과 이러한 변경 사항이 정당한지 여부를 추적하는 데 도움이 됩니다. 이러한 지표를 정기적으로 수집하여 중요한 변경 사항을 쉽게 확인할 수 있습니다.</td> 
  </tr> 
 </tbody> 
</table>
...
이 파일은 AEM에 대한 모든 요청에 대한 기본 정보를 등록합니다. 이 귀중한 결론을 도출할 수 있다.
...
특정 기간(예: 다양한 24시간 동안) 내의 모든 GET 항목을 합산하여 웹 사이트의 평균 트래픽에 대해 설명할 수 있습니다.
...
이 로그에는 요청당 한 줄이 있습니다.

* 각 요청이나 응답이 이루어진 날짜입니다.
* 대괄호 안의 요청 수입니다. 이 번호는 요청 및 응답에 대해 일치합니다.
* 요청인지(오른쪽 화살표) 또는 응답인지(왼쪽 화살표) 나타내는 화살표.
...
작은 스크립트를 사용하여 로그 파일에서 필요한 정보를 추출하고 원하는 통계를 취합할 수 있습니다. 따라서 어떤 페이지 또는 유형의 페이지가 느리는지, 그리고 전반적인 성능이 만족스러운지 알 수 있습니다.
...
위와 같이 스크립트를 사용하여 관련 정보를 추출하고 통계를 작성할 수 있습니다.

그러나 응답 시간을 결정했으면 요청이 수행하는 데 시간이 걸리는 이유와 응답을 개선하기 위해 수행할 수 있는 사항을 분석해야 할 수 있습니다.

#### 동시 사용자의 수와 영향 모니터링 {#monitoring-the-number-and-impact-of-concurrent-users}

다시 `request.log` 이 시스템은 동시성과 시스템의 반응을 모니터하는 데 사용될 수 있습니다.

부정적인 영향이 나타나기 전에 시스템에서 처리할 수 있는 동시 사용자 수를 결정하기 위해 테스트를 수행해야 합니다. 다시 스크립트를 사용하여 로그 파일에서 결과를 추출할 수 있습니다.

* 특정 시간 범위(예: 1분) 내에서 수행되는 요청 수 모니터링
* 동일한 요청을 동시에(가능한 한 가까이) 수행하는 특정 수의 사용자의 효과를 테스트합니다.예를 들어 30명의 사용자가 **동시에 저장을** 클릭합니다.
...
### rlog.jar를 사용하여 오랜 기간 동안 요청 찾기 {#using-rlog-jar-to-find-requests-with-long-duration-times}

AEM에는 다음 위치에 있는 다양한 지원 도구가 포함되어 있습니다.\
`<cq-installation-dir>/crx-quickstart/opt/helpers`

이 중 `rlog.jar`하나를 사용하여 요청을 가장 긴 시간에서 가장 짧은 시간 동안 기간별로 표시할 `request.log` 수 있습니다.

다음 명령은 가능한 인수를 보여줍니다.
...
예를 들어 `request.log` 파일을 매개 변수로 지정하고 길이가 가장 긴 10개의 첫 번째 요청을 표시할 수 있습니다.
...
대규모 데이터 샘플에서 이 작업을 수행해야 하는 경우 개별 `request.log` 파일을 연결해야 할 수 있습니다.

### Apache Bench {#apache-bench}

특수 사건(예: 가비지 수집 등)의 영향을 최소화하려면 메모리 누수를 식별하고 선택적으로 응답 시간을 분석하기 위해 `apachebench` (예: [추가 설명서의 경우](https://httpd.apache.org/docs/2.2/programs/ab.html) ab)와 같은 도구를 사용하는 것이 좋습니다.
...
요청 트래픽(특정 기간 동안의 요청 수)에 대한 정보는 인스턴스의 로드를 나타냅니다. 이 정보는 [request.log](#interpreting-the-request-log)에서 추출할 수 있지만 카운터를 사용하면 데이터 수집을 자동화하여 다음과 같은 내용을 확인할 수 있습니다.

* 활동의 중대한 차이점(예: &quot;많은 요청&quot;과 &quot;낮은 활동&quot;을 구별함)
* 인스턴스가 사용되지 않는 경우
* 다시 시작(카운터가 0으로 재설정됨)

정보 수집을 자동화하기 위해 RequestFilter를 설치하여 모든 요청에 대한 카운터를 늘릴 수도 있습니다. 여러 기간에 여러 카운터를 사용할 수 있습니다.
...
서버 성능을 위해 모든 프로젝트에 `html comments` 포함되는 것이 좋습니다. 좋은 사례가 많이 보인다.페이지를 선택하고 페이지 소스를 열어 보고 아래쪽으로 스크롤하여 다음과 같은 코드를 볼 수 있습니다.
...
도구 명령은 JDK에서 사용할 `jconsole` 수 있습니다.

1. AEM 인스턴스를 시작합니다.
1. 실행 `jconsole.`
1. AEM 인스턴스와 Connect를 **선택합니다**.

1. 애플리케이션 내에서 `Local` 두 번 클릭 `com.day.crx.quickstart.Main`;개요는 기본값으로 표시됩니다.

   ![chlimage_1-87](assets/chlimage_1-87.png)

   그 후에는 다른 옵션을 선택할 수 있습니다.

### (J)VisualVM을 사용한 성능 모니터링 {#monitoring-performance-using-j-visualvm}

JDK 1.6부터는 도구 명령을 사용할 `jvisualvm` 수 있습니다. JDK 1.6을 설치한 후 다음을 수행할 수 있습니다.

1. AEM 인스턴스를 시작합니다.

   >[!NOTE]
   >
   >Java 5를 사용하는 경우 JVM을 시작하는 java 명령줄에 `-Dcom.sun.management.jmxremote` 인수를 추가할 수 있습니다. JMX는 Java 6에서 기본적으로 활성화됩니다.

1. 다음 중 하나를 실행합니다.

   * `jvisualvm`:JDK 1.6 저장소 폴더(테스트된 버전)
   * `visualvm`:visualVM에서 다운로드할 [수](https://visualvm.github.io/releases.html) 있음(Breading Edge 버전)
...
이 도구를 사용하여 스레드 덤프와 메모리 헤드 덤프를 생성할 수 있습니다. 이러한 정보는 기술 지원 팀에서 요청하는 경우가 많습니다.

### 정보 수집 {#information-collection}

설치에 대해 가능한 한 많은 것을 알고 있으면 성능 변경을 야기했을 수 있는 내용과 이러한 변경 사항이 정당한지 여부를 추적하는 데 도움이 됩니다. 이러한 지표를 정기적으로 수집하여 중요한 변경 사항을 쉽게 확인할 수 있습니다.

다음 정보가 유용할 수 있습니다.

* [몇 명의 작성자가 시스템을 사용하고 있습니까?](#how-many-authors-are-working-with-the-system)
* [일별 평균 페이지 활성화 수는 무엇입니까?](#what-is-the-average-number-of-page-activations-per-day)
* [이 시스템에서 현재 몇 개의 페이지를 유지 관리하고 있습니까?](#how-many-pages-do-you-currently-maintain-on-this-system)
* [MSM을 사용하는 경우 월당 평균 롤아웃 수는 어떻게 됩니까?](#if-you-use-msm-what-is-the-average-number-of-rollouts-per-month)
* [월평균 Live Copy 수는 얼마입니까?](#what-is-the-average-number-of-live-copies-per-month)
* [AEM Assets를 사용하는 경우 현재 자산에 몇 개의 자산을 유지 관리하고 있습니까?](#if-you-use-aem-assets-how-many-assets-do-you-currently-maintain-in-assets)
* [자산의 평균 크기는 어떻게 됩니까?](#what-is-the-average-size-of-the-assets)
* [현재 사용되는 템플릿 수는 몇 개입니까?](#how-many-templates-are-currently-used)
* [현재 사용되는 구성 요소는 몇 개입니까?](#how-many-components-are-currently-used)
* [최고 시간에 작성자 시스템에 대해 시간당 몇 개의 요청이 있습니까?](#how-many-requests-per-hour-do-you-have-on-the-author-system-at-peak-time)
* [최대 시간에 게시 시스템에 대한 요청 수는 시간당 얼마나 됩니까?](#how-many-requests-per-hour-do-you-have-on-the-publish-system-at-peak-time)
...
그런 다음 설치 후 경과한 일 수를 계산하여 평균을 계산합니다.
...
다시 설치 후 경과된 개월 수를 사용하여 평균을 계산합니다.

#### AEM Assets를 사용하는 경우 현재 자산에 몇 개의 자산을 유지 관리하고 있습니까? {#if-you-use-aem-assets-how-many-assets-do-you-currently-maintain-in-assets}

현재 유지 관리하는 DAM 자산 수를 확인하려면 저장소 쿼리를 사용합니다.CRXDE 사용 - 도구 - 쿼리:
...
폴더의 전체 크기를 결정하려면 `/var/dam` 다음을 수행합니다.
...
1. 명령줄을 사용합니다.
   ...
   평균 크기를 얻으려면 글로벌 크기를 에서 `/var/dam` (위 획득)의 총 자산 수로 나누십시오.
...
1. 설치 이후 총 요청 수를 확인하려면 명령줄을 사용하십시오.
...
다음은 특정 성능 문제가 발생하면 확인할 항목에 대한 제안 목록입니다. 목록이 완전히 포괄적이지 않습니다.
...
시스템의 CPU가 100%에서 계속 실행 중이면 다음을 참조하십시오.
...
개발 및 테스트 중에 이러한 오류를 검색해야 하지만 특정 시나리오가 발생할 수 있습니다.

시스템의 메모리가 부족한 경우 성능 저하 및 하위 텍스트를 포함한 오류 메시지 등 다양한 방법으로 볼 수 있습니다.
...
시스템에 디스크 공간이 부족하거나 디스크 스래싱 시작 부분을 확인할 수 있습니다.
...
* 디버그 정보 수집을 비활성화했는지 여부;다음을 포함하여 다양한 위치에서 구성할 수 있습니다.
   ...
   * [CQ HTML 라이브러리 관리자](/help/sites-deploying/osgi-configuration-settings.md#osgi-configuration-settings)
...
* 버전 제거를 구성했는지 여부와 [방법](/help/sites-deploying/version-purging.md)
* 기술 자료:
...
각 재부팅 후(경우에 따라 일주일 이상) 인스턴스 성능이 저하되는 경우 다음을 확인할 수 있습니다.
...
결과 로그는 다음과 같은 GC 시각화 도우미가 인제스트할 수 있습니다.
...
* 그런 다음 JConsole을 사용하여 JVM에 연결합니다.참조:\
   ` [https://docs.oracle.com/javase/6/docs/technotes/guides/management/jconsole.html](https://docs.oracle.com/javase/6/docs/technotes/guides/management/jconsole.html)`

이렇게 하면 사용 중인 메모리 양, 사용 중인 GC 알고리즘, 실행 시간 및 애플리케이션 성능에 미치는 영향을 확인할 수 있습니다. 이렇게 하지 않으면 조정은 &quot;임의로 조작되는 노브&quot;일 뿐입니다.
