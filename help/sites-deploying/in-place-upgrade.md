...
# 업그레이드 수행{#performing-an-in-place-upgrade}

>[!NOTE]
>
>이 페이지에서는 AEM 6.4에 대한 업그레이드 절차에 대해 설명합니다. 응용 프로그램 서버에 배포된 설치가 있는 경우 응용 프로그램 서버 설치에 대한 [업그레이드 단계를 참조하십시오](/help/sites-deploying/app-server-upgrade.md).
...
1. 새 AEM jar 파일을 다운로드하여 폴더 외부에 있는 이전 JAR를 교체하는 데 사용합니다 `crx-quickstart` .
...
업그레이드를 실행하려면 jar 파일을 사용하여 AEM을 시작하여 인스턴스를 불러와야 합니다. 6.4로 업그레이드하려면 업그레이드 명령을 사용하여 선택할 수 있는 [레이지 컨텐츠 마이그레이션](/help/sites-deploying/lazy-content-migration.md) 시 다른 컨텐츠 조정 및 마이그레이션 옵션을 참조하십시오.
...
1. 기존 jar(이 경우)의 경로 `crx-quickstart/app/aem-quickstart*.jar` 를 폴더의 동위 항목인 새 jar로 대체하여 명령을 `crx-quickstart` 수정합니다. Adobe의 이전 명령을 예로 들면 다음과 같습니다.
...
업그레이드 절차가 완료되면 업데이트된 코드 베이스를 배포해야 합니다. AEM의 대상 버전에서 작동하도록 코드 베이스를 업데이트하는 단계는 [업그레이드 코드 및 사용자 지정 페이지에서 확인할 수 있습니다](/help/sites-deploying/upgrading-code-and-customizations.md).
