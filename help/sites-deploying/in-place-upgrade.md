...
>[!NOTE]
>
>이 페이지에서는 AEM 6.4에 대한 업그레이드 절차에 대해 설명합니다.응용 프로그램 서버에 배포된 설치가 있는 경우 Application Server 설치에 [대한 업그레이드 단계를 참조하십시오](/help/sites-deploying/app-server-upgrade.md).
...
업그레이드를 실행하려면 jar 파일을 사용하여 AEM을 시작하여 인스턴스를 불러와야 합니다. 6.4로 업그레이드하려면 업그레이드 명령으로 선택할 수 있는 [게으른](/help/sites-deploying/lazy-content-migration.md) 컨텐츠 마이그레이션의 다른 컨텐츠 재구성 및 마이그레이션 옵션을 참조하십시오.
