...
모든 설정과 정의는 하나의 저장소에 저장되고 실행 모드를 설정하여 **활성화됩니다**.
...
* 둘 중 하나 `author` 또는 `publish`를 동시에 정의하지 않음

* 둘 중 하나 `author` 와 `samplecontent` 또는 `nosamplecontent` (둘 다 제외)으로 결합

>[!CAUTION]
>
>위의 실행 모드(작성자, 게시, 샘플 컨텐츠, nosampleccontent) 중 하나를 사용하는 경우 설치 시 사용되는 값은 해당 설치의 *전체 수명* 동안 실행 모드를 정의합니다.
>
>이러한 실행 모드에서는 설치 후 변경할 *수* 없습니다.

## 사용자 정의된 실행 모드 {#customized-run-modes}
...
## 샘플 컨텐츠 및 nosampleccontent 사용 {#using-samplecontent-and-nosamplecontent}
...
* 샘플 `nosamplecontent` 컨텐츠는 모드를 통해 설치되지 않습니다.
...
이러한 폴더 [에서 개별 구성 노드를 정의하고 여러 실행 모드 조합을 위한 구성을 만드는 방법에 대한 자세한 내용은 저장소의](/help/sites-deploying/configuring-osgi.md#osgi-configuration-in-the-repository) OSGi 구성을 참조하십시오.
...
이러한 폴더는 유형이며 적절한 번들을 포함해야 `nt:folder` 합니다.
...
응용 프로그램 서버를 사용하는 경우 web.xml에서 실행 모드를 [정의할 수도 있습니다](#defining-the-run-mode-in-web-xml-with-application-server).

### sling.properties 파일 사용 {#using-the-sling-properties-file}

이 `sling.properties` 파일을 사용하여 필요한 실행 모드를 정의할 수 있습니다.
...
빠른 시작을 시작할 때 옵션을 사용하여 사용자 정의 실행 모드 `-r` 를 활성화할 수 있습니다. 예를 들어 다음 명령을 사용하여 실행 모드가 dev로 설정된 AEM 인스턴스를 시작합니다. &quot;
...
AEM `war` 파일에 있으므로 배포하기 전에 업데이트해야 합니다.

자세한 [내용은 응용 프로그램 서버와](/help/sites-deploying/application-server-install.md) 함께 AEM 설치를 참조하십시오.
