...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>액세스 가능한 요청 및 일반 Dispatcher 성능 최적화 조언 <a href="/help/sites-deploying/configuring-performance.md#optimizing-performance-when-using-the-dispatcher">에 대한 자세한 내용은 Dispatcher 성능 최적화를 참조하십시오</a>.</td> 
  </tr> 
  <tr> 
   <td><strong>4단계</strong></td> 
   <td>Dispatcher에서 요청이 오고 있습니까?</td> 
   <td><p>Dispatcher <a href="https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#debugging">디버깅 설명서를</a> 확인하여 요청이 올바르게 캐시되는지 확인하십시오.<br /> </p> </td> 
  </tr> 
  <tr> 
   <td><strong>5단계</strong></td> 
   <td>디스패처가 AEM을 통해 각 요청을 인증하려고 합니까?</td> 
   <td>디스패처가 캐시된 리소스를 전달하기 전에 AEM에 인증을 위해 <code>HEAD</code> 요청을 전송하는지 확인합니다. AEM에서 <code>HEAD</code> 요청을 찾아 볼 수 있습니다 <code>access.log</code>. 자세한 내용은 로깅 <a href="/help/sites-deploying/configure-logging.md">을 참조하십시오</a>.<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><br /> <p>Touch <a href="/help/sites-developing/tough-day.md">Day</a> 를 사용하여 프로덕션 인스턴스에서 "실제" 조건을 복제할 수 있습니다. 개발 단계에서 이것이 현실적이지 않은 경우 다른 네트워크 컨텍스트에서 프로덕션 인스턴스(또는 동일한 스테이징 인스턴스)를 테스트해야 합니다.<br /> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>네트워크 레이어 조사</td> 
   <td><p>채도 및 지연 문제에 대한 네트워크 레이어를 조사합니다.</p> <p>작성자 계층의 경우 지연이 100밀리초를 넘지 않는 것이 좋습니다.</p> <p>성능 최적화 팁에 대한 자세한 내용은 <a href="https://helpx.adobe.com/experience-manager/kb/performance-tuning-tips.html">이 페이지를 참조하십시오</a>.</p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>하드웨어 요구 사항 확인</td> 
   <td>하드웨어 <a href="/help/managing/hardware-sizing-guidelines.md">크기 조정 지침에 대한 설명서를 확인하십시오</a>.<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>느린 요청 찾기</td> 
   <td><p>또는 를 사용하여 느린 요청을 분석함으로써 <code>request.log</code> 확인할 수 있습니다 <code>rlog.jar</code>.</p> <p>rlog.jar 사용에 대한 자세한 내용은 이 페이지를 참조하십시오.</p> <p>장시간 <a href="/help/sites-deploying/monitoring-and-maintaining.md#using-rlog-jar-to-find-requests-with-long-duration-times">동안 요청을 찾으려면 rlog.jar 사용을 참조하십시오</a>.<br /> </p> <p> </p> </td> 
  </tr> 
  <tr> 
   <td><strong>16단계</strong></td> 
   <td>프로필 서버</td> 
   <td><p>AEM에서 사용할 수 있는 프로파일링 도구에 대한 자세한 내용은 성능 모니터링 및 분석을 <a href="/help/sites-deploying/monitoring-and-maintaining.md#tools-for-monitoring-and-analyzing-performance">위한 도구를 참조하십시오</a>.<br /> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>성능 <a href="/help/sites-deploying/monitoring-and-maintaining.md#analyzing-specific-scenarios">최적화 섹션의 특정 시나리오</a> 분석을 참조하십시오.<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>디스크 I/O</td> 
   <td><p>모니터링 및 유지 관리 설명서의 <a href="/help/sites-deploying/monitoring-and-maintaining.md#disk-i-o">디스크 입출력</a> 섹션을 참조하십시오.</p> </td> 
  </tr> 
  <tr> 
   <td><strong>단계 22 및 22.1</strong></td> 
   <td>캐시 비율</td> 
   <td>디스패처 <a href="/help/sites-deploying/configuring-performance.md#calculating-the-dispatcher-cache-ratio">캐시 비율 계산을 참조하십시오</a>.<br /> <br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     ...
     <li>성능 조정 팁 아티클은 <a href="https://helpx.adobe.com/experience-manager/kb/performance-tuning-tips.html">여기와</a> <a href="https://helpx.adobe.com/experience-manager/kb/performance-tuning-tips.html">여기에</a>게재됩니다.<br /> </li> 
    </ol> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 
    ...
    </ol> <p>캐시 비율을 개선하는 방법; 요청 캐시 사용 가능(발송자 우수 사례)</p> <p>또한 캐싱 구성을 최적화하기 위해 아래 설정을 고려하십시오<br /> </p> 
    <ol> 
  ...
  <tr> 
   ...
   <td>발송자 구성</td> 
   <td><a href="https://helpx.adobe.com/kr/experience-manager/dispatcher/using/dispatcher-configuration.html을 참조하십시오.">디스패처 구성</a><br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>위에 표시된 Gem 세션을 참조하십시오. 또한 W3c에 대한 추가 설명서 미리 연결:<a href="https://www.w3.org/TR/resource-hints/#dfn-preconnect"> https://www.w3.org/TR/resource-hints/#dfn-preconnect</a></td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>HTTP/2 사용</td> 
   <td>37,38 및 39단계는 Gem Session을 참조하십시오. 또한 HTTP/2 지원에 대한 <a href="https://help-forums.adobe.com/content/adobeforums/en/experience-manager-forum/adobe-experience-manager.topic.html/forum__kdzc-does_anyoneknowwhe.html">이</a> 포럼 게시물을 확인하십시오.<br /> </td> 
  </tr> 
  <tr> 
   <td><strong>49단계</strong></td> 
   <td>페이로드 크기 축소</td> 
   <td><a href="/help/sites-deploying/osgi-configuration-settings.md">Gzip을</a> 활성화하고 <a href="https://docs.adobe.com/ddc/en/gems/aem-web-performance.html">이미지 크기를 축소합니다</a>.<br /> </td> 
  </tr> 
  <tr> 
   <td><strong>42단계 및 43단계</strong></td> 
   <td>Keep-Alive</td> 
   <td><p>연결 재사용을 위한 여러 요청에 헤더가 <code>Keep-Alive</code> 있습니까? 그렇지 않으면 각 요청이 다른 연결 설정으로 연결되므로 불필요한 오버헤드가 발생할 수 있습니다. (브라우저의 표준 HTTP 요청 분석)</p> <p>프록시 <a href="/help/sites-administering/proxy-jar.md">서버 도구를</a> 확인하여 연결 상태를 유지할 수 있습니다.<br /> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     <li>연결 리소스(이미지, CSS 스프라이트, JSON 등)<br /> </li> 
     <li>Clientlibs 포함: 
      <ol> 
       <li><a href="/help/sites-developing/clientlibs.md#creating-client-library-folders">클라이언트 라이브러리 폴더</a> 만들기 - 임베드 사용을 참조하여 요청을 최소화합니다.</li> 
      </ol> </li> 
    </ol> </td> 
  </tr> 
  <tr> 
