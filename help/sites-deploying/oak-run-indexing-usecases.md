...
손상된 색인을 확인하는 작업은 작업을 통해 수행할 수 `--index-consistency-check` 있습니다.
...
시나리오에 따라 [](https://jackrabbit.apache.org/oak/docs/query/indexing.html#reindexing)경우에 따라 색인 재지정을 수행해야 하는 경우가 있습니다. 현재 재인덱스는 CRXDE를 통해 또는 Index Manager 사용자 인터페이스를 통해 인덱스 정의 노드 `reindex` `true` 에 플래그를 설정하여 수행됩니다. 플래그가 설정되면 다시 색인화가 비동기적으로 수행됩니다.

다시 색인화에 대해 주목할 점이 있습니다.

* 재인덱스는 모든 컨텐츠가 로컬인 설정에 `DocumentNodeStore` 비해 설정 시 훨씬 더 `SegmentNodeStore` 느립니다.
...
* 다시 인덱싱하는 데 상당한 시간이 걸릴 수 있는 `DocumentNodeStore` 설치의 경우, Mongo 데이터베이스에 대한 연결이 작업 중에 실패할 경우 인덱싱을 처음부터 다시 시작해야 합니다.
...
* **오프라인 재인덱싱** - 색인 작업에 영향을 주지 않고 다시 인덱싱이 수행됩니다. 이것은 비동기 인덱서가 계속해서 다른 인덱스를 인덱싱할 수 있음을 의미합니다.

* **DocumentNodeStore 설치에** 대한 간소화된 재색인 - `DocumentNodeStore` 설치의 경우 단일 명령으로 다시 색인화를 수행할 수 있으므로 최적의 방식으로 재색인을 수행할 수 있습니다.

* **색인 정의 업데이트 및 새로운 색인 정의 소개 지원**

### 다시 색인 - DocumentNodeStore {#reindexdocumentnodestore}

설치 재인덱싱을 `DocumentNodeStore` 수행하려면 단일 oak-run 명령을 통해 다음을 수행할 수 있습니다.
...
* 또한 사용자는 옵션을 통해 새로운 또는 업데이트된 색인의 JSON을 제공할 수도 `--index-definitions-file` 있습니다.

### 다시 색인 - SegmentNodeStore {#reindexsegmentnodestore}

설치 재색인화의 경우 다음 방법 중 하나를 사용하여 수행할 수 있습니다. `SegmentNodeStore`

#### 온라인 재색인 - SegmentNodeStore {#onlinereindexsegmentnodestore}
...
한 프로세스만 `SegmentNodeStore` 설치하면 읽기-쓰기 모드에서 세그먼트 파일에 액세스할 수 있습니다. 이러한 참화 실행 인덱싱에서 일부 작업을 수행하려면 추가적인 수동 단계가 필요합니다.

여기에는 다음이 포함됩니다.

1. 단계 텍스트
1. 읽기 전용 모드 `oak-run` 에서 AEM에서 사용하는 동일한 저장소에 연결하고 색인을 수행합니다. 이를 실현하는 방법에 대한 예:
...
#### 온라인 재색인 - SegmentNodeStore - AEM 인스턴스가 종료되었습니다. {#onlinereindexsegmentnodestoreaeminstanceisdown}

재인덱싱을 `SegmentNodeStore` 설치하려면 단일 oak 실행 명령을 통해 수행할 수 있습니다. 하지만 AEM 인스턴스를 종료해야 합니다.
...
1. JMX 작업을 통해 체크포인트를 만듭니다. 이렇게 하려면 [JMX 콘솔로](/help/sites-administering/jmx-console.md) 이동하여 검색할 수 `CheckpointManager`있습니다. 그런 다음 만료에 높은 값(예: **2592000** )을 사용하여 createCheckpoint(long p1) ****&#x200B;작업을 클릭합니다.
...
이 사용 사례에서는 EBS와 같은 클라우드 기반 스토리지 솔루션에 배치된 경우 가능하지 않을 수 있는 다른 인스턴스 `FileDataStore` 에서 데이터 저장소에 액세스할 수 있다고 가정합니다. 여기에는 복제되는 시나리오 `FileDataStore` 가 제외됩니다. 인덱스 정의가 전체 텍스트 인덱싱을 수행하지 않으면 액세스할 필요가 `DataStore` 없습니다.

## 사용 사례 4 - 색인 정의 업데이트 {#usecase4updatingindexdefinitions}

현재 ACS 인덱스 확인 패키지를 통해 색인 정의 변경 사항을 [전달할](https://adobe-consulting-services.github.io/acs-aem-commons/features/ensure-oak-index/index.html) 수 있습니다. 이렇게 하면 나중에 플래그를 다음으로 설정하여 색인 재지정을 수행해야 하는 컨텐츠 패키지를 통해 색인 정의를 `reindex` 전달할 수 있습니다 `true`.
...
1. 개발자는 로컬 인스턴스의 인덱스 정의를 업데이트한 다음 옵션을 통해 색인 정의 JSON 파일을 생성합니다. `--index-definitions`
