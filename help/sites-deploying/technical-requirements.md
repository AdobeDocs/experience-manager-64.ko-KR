...
자세한 내용은 [하드웨어](/help/managing/hardware-sizing-guidelines.md) 크기 조정 지침을 참조하십시오.
...
1. &#39;File System&#39;에는 POSIX 호환 블록 스토리지가 포함됩니다. 여기에는 네트워크 스토리지 기술이 포함됩니다. 파일 시스템 성능이 다를 수 있으며 전체 성능에 영향을 줄 수 있습니다. 네트워크/원격 파일 시스템과 함께 테스트 AEM을 로드하는 것이 좋습니다.
...
>
>MongoDB를 사용하여 AEM 배포를 최대한 활용하려면 Adobe에서 전문 지원을 받으려면 MongoDB Enterprise 버전의 라이선스를 부여하는 것이 좋습니다. 자세한 [내용은 권장](/help/sites-deploying/recommended-deploys.md#prerequisites-and-recommendations-when-deploying-aem-with-mongomk) 배포를 참조하십시오.
>
...
>
>지원되는 관계형 데이터베이스와 함께 AEM 6.4를 실행하려면 데이터베이스 공급업체와의 별도의 지원 계약이 필요합니다. Adobe 고객 지원 센터는 AEM 6.4를 사용하여 관계형 데이터베이스 사용과 관련된 적격 문제를 지원합니다.
>
>**대부분의 관계형 데이터베이스는 현재 AEM 6.4의 Level-R에서 지원되며, 여기에는 위의 Level-R 설명에 명시된 지원 기준 및 지원 프로그램이 포함되어 있습니다.**
...
필요한 최소 Servlet API 버전은 Servlet 3.1이지만 Servlet 4.0 이합니다.

| 플랫폼 | 지원 수준 |
|---|---|
| **Quickstart 내장된 Servlet Engine(Jetty 9.3)** | A:지원됨 |
| Oracle WebLogic Server 12.2(12cR2) | A:지원됨 |
| IBM WebSphere Application Server Continuous Delivery (LibertyProfile) with Web Profile 7.0 and IBM JRE 1.8 | A:지원됨 |
| IBM WebSphere Application Server 9.0 | A:지원됨 |
| Apache Tomcat 8.5.x | A:지원됨 |
| JBoss 애플리케이션 서버가 있는 JBoss EAP 7.1.0 | A:지원(1) |
| JBoss 애플리케이션 서버가 있는 JBoss EAP 7.0.0 | A:지원됨 |

1. AEM Forms에서는 지원되지 않습니다.

### 서버 운영 체제 {#server-operating-systems}

Adobe Experience Manager는 다음 서버 플랫폼과 연동됩니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>A:제한 사항(3,5,7)<br /> 지원 R:새로운 계약에 대한 제한된 지원</td> 
  </tr> 
  <tr> 
   <td>IBM AIX 7.2</td> 
   <td>A:제한 사항(2,5,7)<br /> 지원 R:새로운 계약에 대한 제한된 지원</td> 
  </tr> 
 </tbody> 
</table>

1. Linux Kernel 2.6, 3.x 및 4.x에는 Red Hat Enterprise Linux, CentOS, Oracle Linux 및 Amazon Linux를 비롯한 Red Hat 배포판에서 파생되는 내용이 포함되어 있습니다. AEM 양식 추가 기능은 CentOS 7 및 Red Hat Enterprise Linux 6.5 및 7에서만 지원됩니다.
1. AEM 자산:XMP 메타데이터 [다시 쓰기 지원 섹션을 참조하십시오.](#requirements-for-aem-assets-xmp-metadata-write-back)
1. AEM 자산:다이내믹 미디어 이미징을 지원하지 않습니다. 다이내믹 미디어 비디오가 지원됩니다.
1. AEM Forms는 Ubuntu 16.04 LTS에서만 지원됩니다.
1. AEM 자산:Raw 파일 [변환 지원 없음](/help/assets/camera-raw.md)
1. AEM Forms:프로덕션 환경 지원 안 됨
1. AEM 자산:향상된 PDF 래스터라이저 [지원 안 함](/help/assets/aem-pdf-rasterizer.md)
1. AEM Forms:지원되지 않음
...
Azure 또는 AWS에 AEM을 배포하려면 Adobe Managed Services를 사용하는 것이 좋습니다. Adobe Managed Services는 이러한 클라우드 컴퓨팅 환경에서 AEM을 배포 및 운영하는 경험과 기술을 전문가에게 제공합니다. Adobe Managed Services에 대한 [추가 설명서를 확인하십시오](https://www.adobe.com/marketing-cloud/enterprise-content-management/managed-services-cloud-platform.html?aemClk=t).
...
Azure 또는 AWS에서 AEM을 배포하는 방법에 대한 권장 사항은 Adobe Managed Services 외부에서 클라우드 공급자나 Adobe 파트너와 직접 작업하여 원하는 클라우드 환경에서 AEM을 배포하는 것을 지원하는 것이 좋습니다. 선택한 클라우드 제공업체 또는 파트너는 고객의 특정 성능, 로드, 확장성 및 보안 요구 사항을 충족하기 위해 사이징 사양, 설계 및 아키텍처 구현에 대한 책임을 집니다.

### 발송자 플랫폼(웹 서버) {#dispatcher-platforms-web-servers}

Dispatcher는 캐싱 및 로드 밸런싱 구성 요소입니다. [최신 Dispatcher 버전을](https://helpx.adobe.com/experience-manager/dispatcher/release-notes.html)다운로드합니다. Experience Manager 6.4를 사용하려면 Dispatcher 버전 4.3.1 이상이 필요합니다.
...
>[!NOTE]
>
>다음 시스템 요구 사항은 **_다이내믹 미디어 - 하이브리드 모드를 사용하는 경우에만_** 적용됩니다.다이내믹 미디어 - 하이브리드 모드에는 임베드된 이미지 서버가 있으며 이는 특정 운영 체제에서만 인증됩니다.
>
>Dynamic Media - Scene7 모드(즉, **dynamicmedia_scene7** 런타임 모드)를 실행하는 Dynamic Media 고객의 경우 추가 시스템 요구 사항이 없습니다.aem과 동일한 시스템 요구 사항만 해당됩니다. 다이내믹 미디어 - Scene7 모드 아키텍처는 AEM에 포함된 서비스가 아닌 클라우드 기반 이미지 서비스를 사용합니다.
...
>[!NOTE]
>
>LC_CTYPE이 en_US.UTF-8과 같지 않도록 로케일이 설정되면 다이내믹 미디어가 작동하지 않습니다. 이 값이 명령 프롬프트에서 &quot;locale&quot;을 입력합니다. 이렇게 설정되지 않은 경우 AEM을 실행하기 전에 &quot;export LC_CTYPE=&quot;를 입력하여 LC_CTYPE 환경 변수를 빈 문자열로 설정합니다.
...
>[!NOTE]
>
>**** 서버의 호스트 이름은 확인할 수 있어야 합니다.서버의 호스트 이름을 IP 주소로 확인할 수 있는지 확인합니다. 가능하지 않은 경우 정규화된 호스트 이름 및 IP 주소를 **/etc/hosts에**&#x200B;추가하십시오.
>
...
Windows에서 Dynamic Media를 사용하려면 x64 및 x86용 Microsoft Visual Studio 2010, 2013 및 2015 재배포용 파일이 설치되어 있어야 합니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>ODT, ODP, ODS, ODG, ODF, SXW, SXI, SXC, SXD, XLS, XLSX, DOC, DOCX, PPT, PPTX, 이미지 형식(BMP, GIF, JPEG, JPG, TIF, TIFF, TIFF, TIFF PNG, JPF, JPX, JP2, J2K, J2C, JPC), HTML, HTM, RTF 및 TXT</p> </td> 
  </tr> 
 </tbody> 
</table>
...
>추가:
>
>* PDF Generator [를 사용하려면 Acrobat 2017 클래식 트랙 버전 17.011.30078 이상이](https://helpx.adobe.com/acrobat/release-note/release-notes-acrobat-reader.html) 필요합니다.
>* AEM Forms는 지원되는 32비트 소프트웨어 에디션만 지원합니다.
>* OCR PDF(검색 가능한 PDF), PDF 최적화 및 PDF 내보내기 기능은 Microsoft Windows에서만 지원됩니다.
