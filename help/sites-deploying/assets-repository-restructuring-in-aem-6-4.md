...
description: AEM 6.4 for Assets의 새 저장소 구조로 마이그레이션하기 위해 필요한 변경 방법을 알아봅니다.
seo-description: AEM 6.4 for Assets의 새 저장소 구조로 마이그레이션하기 위해 필요한 변경 방법을 알아봅니다.
...
AEM 6.4 [의 상위 리포지토리 재조정](/help/sites-deploying/repository-restructuring.md) 페이지에 설명된 대로, AEM 6.4로 업그레이드하는 고객은 이 페이지를 사용하여 AEM Assets 솔루션에 영향을 주는 리포지토리 변경과 관련된 작업 노력을 평가해야 합니다. 일부 변경 사항은 AEM 6.4 업그레이드 프로세스 동안 작업해야 하는 반면, 다른 변경 사항은 6.5 업그레이드 시까지 연기될 수 있습니다.
   ...
   <td><p>고객이 이메일 템플릿을 수정한 경우 새 저장소 구조에 맞게 다음 작업을 수행합니다.</p> 
    <ol> 
     ...
     <li>폴더 제거:이메일 템플릿이 이동한 <strong><code>/etc/dam/notification/email/default</code></strong> 후<br /> 
      <ol> 
       <li>아래의<strong> 전자 메일 템플릿에 대한 업데이트가 없는 <code>/etc/notification/email/default</code></strong>경우 AEM 6.4 설치의 일부로 원래 전자 메일 템플릿이 존재하므로 폴더를 제거할 수 <strong><code>/libs/settings/notification/email/default</code></strong> 있습니다.</li> 
      </ol> </li> 
    </ol> </td> 
  </tr> 
  <tr> 
   ...
   <td><p>고객이 이메일 템플릿을 수정한 경우 새 저장소 구조에 맞게 정렬합니다.</p> 
    <ol> 
     ...
     <li>폴더 제거:이메일 템플릿이 이동한 <strong><code>/etc/dam/adhocassetshare</code></strong> 후<br /> 
      <ol> 
       <li>아래의<strong> 전자 메일 템플릿에 대한 업데이트가 없는 <code>/etc</code></strong>경우 AEM 6.4 설치의 일부로 원래 전자 메일 템플릿이 존재하므로 폴더를 제거할 수 <strong><code>/libs/settings/dam/adhocassetshare</code></strong> 있습니다.</li> 
      </ol> </li> 
    </ol> </td> 
  </tr> 
  <tr> 
   ...
   <td><p>새 저장소 구조에 정렬하려면</p> 
    <ol> 
     <li>에서 사용자 정의 스크립트 또는 수정된 스크립트 모두 <strong><code>/etc/dam/indesign/scripts</code></strong> 복사 <strong><code>/apps/settings/dam/indesign/scripts</code></strong><br /> 
      <ol> 
       <li>AEM에서 제공한 수정되지 않은 스크립트로 새 스크립트 또는 수정된 스크립트만 AEM 6.4 <strong><code>/libs/settings</code></strong> 에서 사용할 수 있습니다.</li> 
      </ol> </li> 
