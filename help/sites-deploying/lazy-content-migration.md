...
| `CQ63MonitoringDashboardsConfigUpdate` | &lt; 6.3 | 즉시 | 모니터링 대시보드에 대한 사용자 지정 구성 **을 /libs** 및 **/apps에서 이동합니다.** |
...
| `CQ64CommunitiesConfigsCleanupTask` | &lt; 6.3 | 지연 | SRP 클라우드 구성 이동, 커뮤니티 감시자 구성 정리 정리, up **/etc/social** 및 **/etc/enablement** (지연 마이그레이션이 실행될 때 참조와 데이터를 조정해야 함 - 이 구조에 따라 애플리케이션 부분이 더 이상 필요하지 않음). |
