...
사용 가능한 주제 목록을 만드는 논리는 먼저 허용 목록에 있는 모든 주제를 허용하는 다음 차단 목록에 있는 주제를 제거합니다. 기본적으로 모든 주제가 활성화됩니다(허용 목록 값은 `*`)이고 사용할 수 없는 항목이 없습니다(차단 목록에 값이 없음).
...
|---|---|---|
| 주제 화이트리스트 | job.consumermanager.whitelist | 로컬 JobManager 서비스가 처리하는 주제 목록입니다. 기본값(&amp;A) 모든 항목을 등록된 TopicConsumer 서비스로 보냅니다. |
...
기본적으로, Experience Manager은 DAM 자산이 변경되거나 DAM에 자산이 추가되면 DAM 자산 업데이트 워크플로우를 실행합니다. Experience Manager이 DAM 자산 업데이트 오프로더 워크플로우를 대신 실행하도록 기본 동작을 변경합니다. 이 워크플로우는 주제가 있는 JobManager 작업을 생성합니다 `com/adobe/granite/workflow/offloading`. 그런 다음 전용 작업자에게 작업이 오프로드되도록 토폴로지를 구성합니다.

>[!CAUTION]
>
>워크플로우 오프로딩과 함께 사용할 때는 워크플로우를 일시적이지 않아야 합니다. 예를 들어 자산 오프로딩에 사용할 때 DAM 자산 업데이트 워크플로우가 일시적이지 않아야 합니다. 워크플로우에서 임시 플래그를 설정/해제하려면 다음을 참조하십시오 [임시 워크플로우](/help/assets/performance-tuning-guidelines.md#workflows).
...
1. 각 Experience Manager 인스턴스에서 루트 지형 커넥터를 가리키도록 검색 서비스를 구성합니다. (자세한 내용은 [토폴로지 멤버 자격 구성](#title4))
1. 연결 인스턴스가 허용 목록에 있도록 루트 지형 커넥터를 구성합니다.
1. 오프로딩 브라우저 를 열고 을 비활성화합니다 `com/adobe/granite/workflow/offloading` 사용자가 DAM 자산을 업로드하거나 변경하기 위해 상호 작용하는 인스턴스에 대한 항목.
...
1. 사용자가 DAM 자산을 업로드하거나 변경하기 위해 상호 작용하는 각 인스턴스에서 DAM 자산 업데이트 오프로딩 워크플로우를 사용하도록 워크플로우 런처를 구성합니다.
   ...
   1. DAM 자산 업데이트 오프로딩 워크플로우를 실행하도록 두 이벤트 유형을 모두 변경합니다. (런처 구성에 대한 자세한 내용은 [노드가 변경될 때 워크플로우 시작](/help/sites-administering/workflows-starting.md))
...
* 자산 오프로딩에 대한 일반 지침 및 우수 사례에 대해서는 [자산 오프로딩에 대한 일반 지침 및 우수 사례](/help/assets/assets-offloading-best-practices.md#general-guidance-and-best-practices-for-asset-offloading).
* 오프로딩 에이전트의 자동 만들기를 비활성화하는 방법은 를 참조하십시오 [자동 에이전트 관리 해제](/help/assets/assets-offloading-best-practices.md#turning-off-automatic-agent-management).
