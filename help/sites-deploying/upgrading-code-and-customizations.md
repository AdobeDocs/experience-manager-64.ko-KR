...
1. **패턴 탐지기** - 업그레이드 계획에 설명된 대로 패턴 탐지기를 실행하고 [이 페이지](/help/sites-deploying/pattern-detector.md) AEM Target 버전에서 사용할 수 없는 API/번들 외에 해결해야 하는 영역에 대한 자세한 내용을 포함하는 패턴 탐지기 보고서를 가져오기 위해. 패턴 감지 보고서는 코드에서 호환되지 않는 점을 표시해야 합니다. 코드가 없는 경우 배포가 이미 6.4 호환성이 있는 경우에는 6.4 기능을 사용하기 위해 새로운 개발을 수행하도록 선택할 수 있지만 호환성을 유지하기 위해 새로운 개발을 수행할 필요는 없습니다. 호환되지 않는 기능이 보고된 경우 a) 호환성 모드에서 실행 및 새로운 6.4 기능 또는 호환성에 대한 개발을 연기하거나, b) 업그레이드 후 개발을 결정한 후 2단계로 이동할 수 있습니다. 자세한 내용은 [AEM 6.4의 이전 버전과의 호환성](/help/sites-deploying/backward-compatibility.md) 자세한 내용

1. **6.4용 코드 베이스 개발** - Target 버전에 대한 코드 베이스에 대한 전용 분기 또는 저장소를 만듭니다. 업데이트할 코드 영역을 계획하려면 업그레이드 전 호환성 정보를 사용하십시오.
1. **6.4 Uber jar로 컴파일합니다.** - 6.4 uber jar를 가리키도록 코드 기본 POM을 업데이트하고 이에 대해 코드를 컴파일합니다.
1. **AEM 사용자 지정 업데이트** - AEM에 대한 모든 사용자 지정 또는 확장은 6.4에서 작동하도록 업데이트/검증하고 6.4 코드 베이스에 추가해야 합니다. UI 검색 Forms, 자산 사용자 지정, /mnt/overlay를 사용하는 모든 항목 포함

1. **6.4 환경에 배포** - AEM 6.4(작성자 + 게시)의 클린 인스턴스는 개발/QA 환경에서 일어나야 합니다. 업데이트된 코드 베이스와 현재 프로덕션에서 제공하는 대표적인 컨텐츠 샘플을 배포해야 합니다.
1. **QA 유효성 검사 및 버그 수정** - QA는 6.4의 작성자 및 게시 인스턴스 둘 다에서 애플리케이션의 유효성을 확인해야 합니다. 발견된 버그는 모두 수정하고 6.4 코드 베이스에 커밋해야 합니다. 모든 버그가 수정될 때까지 필요에 따라 개발 주기를 반복합니다.
...
6.4는 새로운 AEM 버전에서 작동하도록 코드 베이스와 사용자 지정을 업그레이드하는 옵션 외에도 다음과 같이 이전 버전과의 호환성 기능을 사용하여 사용자 지정을 보다 효율적으로 관리하는 데 도움이 됩니다 [이 페이지](/help/sites-deploying/backward-compatibility.md).

위에서 언급했듯이 아래 다이어그램에 표시된 대로 [패턴 탐지기](/help/sites-deploying/pattern-detector.md) 첫 번째 단계에서는 업그레이드의 전반적인 복잡성을 평가하고 호환성 모드에서 실행할지 또는 새로운 AEM 6.4 기능을 모두 사용하도록 사용자 지정을 업데이트할지 여부를 평가하는 데 도움이 됩니다. 자세한 내용은 [AEM 6.4의 이전 버전과의 호환성](/help/sites-deploying/backward-compatibility.md) 페이지를 참조하십시오.
...
클래식 UI 작성은 AEM 6.4에서 계속 사용할 수 있지만 더 이상 사용되지 않습니다. 자세한 내용 [여기](/help/release-notes/deprecated-removed-features.md#pre-announcement-for-next-release). 애플리케이션이 현재 클래식 UI 작성 환경에서 실행 중인 경우 AEM 6.4로 업그레이드하고 클래식 UI를 계속 사용하는 것이 좋습니다. 그런 다음 터치 UI로의 마이그레이션을 여러 개발 주기에 걸쳐 완료할 수 있도록 별도의 프로젝트로 계획할 수 있습니다. AEM 6.4에서 클래식 UI를 사용하려면 코드 베이스에 몇 가지 OSGi 구성을 커밋해야 합니다. 구성 방법에 대한 자세한 내용은 다음을 참조하십시오 [여기](/help/sites-administering/enable-classic-ui.md).

>[!NOTE]
>
>클래식 UI에서 벗어나 최신 AEM 기술을 활용하려면 다음을 활용하는 것이 좋습니다 [AEM 현대화 도구](/help/sites-developing/modernization-tools.md) 마이그레이션을 더 쉽게 수행할 수 있습니다.
...
기존 자산에 대한 자산 ID를 생성하려면 AEM 인스턴스를 업그레이드하여 AEM 6.4를 실행합니다. 이 ID를 사용하려면 [자산 통찰력 기능](/help/assets/touch-ui-asset-insights.md). 자세한 내용은 [포함 코드 추가](/help/assets/touch-ui-using-page-tracker.md#adding-embed-code).
...
ContextHub 구성은 업그레이드에 의해 적용됩니다. 기존 ContextHub 구성 복구 방법에 대한 지침을 찾을 수 있습니다 [여기](/help/sites-administering/contexthub-config.md#recovering-contexthub-configurations-after-upgrading).
...
편집 가능한 템플릿의 구조가 AEM 6.2와 6.3 간에 변경되었습니다. 6.2 또는 이전 버전에서 업그레이드하는 경우 및 사이트 컨텐츠가 편집 가능한 템플릿을 사용하여 빌드되는 경우 를 사용해야 합니다 [응답형 노드 정리 도구](https://github.com/Adobe-Marketing-Cloud/aem-sites-template-migration). 이 도구는 **_after_** 컨텐츠를 정리하기 위한 업그레이드. 작성 계층과 게시 계층 모두에서 실행해야 합니다.

### CUG 구현 변경 {#cug-implementation-changes}

닫힌 사용자 그룹 구현이 이전 버전의 AEM의 성능 및 확장성 제한을 해결하기 위해 크게 변경되었습니다. 이전 버전의 CUG는 6.3에서 더 이상 사용되지 않으며, 새 구현은 Touch UI에서만 지원됩니다. 6.2 이상에서 업그레이드하는 경우 새 CUG 구현으로 마이그레이션하기 위한 지침을 찾을 수 있습니다 [여기](/help/sites-administering/closed-user-groups.md#upgrade-cug).
