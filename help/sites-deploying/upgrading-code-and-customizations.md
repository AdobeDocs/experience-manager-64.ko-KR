...
* [6.4 저장소 구조와 정렬](#align-repository-structure)
* [AEM 사용자 지정](#aem-customizations)
* [테스트 절차](#testing-procedure)

## 개요 {#overview}

1. **패턴 탐지기**  - 업그레이드 계획에 설명된 대로 패턴 탐지기 [를 실행하여 AEM의 Target 버전에서 사용할 수 없는 API/번들 외에도 해결해야 하는 영역에 대한 자세한 내용을 포함하는 패턴 탐지기 보고서를 ](/help/sites-deploying/pattern-detector.md) 가져옵니다. 패턴 감지 보고서는 코드에서 호환되지 않는 점을 표시해야 합니다. 코드가 없는 경우 배포가 이미 6.4 호환성이 있는 경우에는 6.4 기능을 사용하기 위해 새로운 개발을 수행하도록 선택할 수 있지만 호환성을 유지하기 위해 새로운 개발을 수행할 필요는 없습니다. 호환되지 않는 기능이 보고된 경우 a) 호환성 모드에서 실행 및 새로운 6.4 기능 또는 호환성에 대한 개발을 연기하거나, b) 업그레이드 후 개발을 결정한 후 2단계로 이동할 수 있습니다. 자세한 내용은 [AEM 6.4](/help/sites-deploying/backward-compatibility.md)의 이전 호환성 을 참조하십시오.

1. **6.4용 코드 베이스 개발**  - Target 버전에 대한 코드 베이스에 대한 전용 분기 또는 저장소를 만듭니다. 업데이트할 코드 영역을 계획하려면 업그레이드 전 호환성 정보를 사용하십시오.
1. **6.4 Uber jar로 컴파일**  - 코드 기본 POM을 업데이트하여 6.4 uber jar를 가리키고 이 기능에 대해 코드를 컴파일합니다.
1. **AEM 사용자 지정 업데이트**  - AEM에 대한 모든 사용자 지정 또는 확장은 6.4에서 작동되고 6.4 코드 베이스에 추가되도록 업데이트/검증해야 합니다. UI 검색 Forms, 자산 사용자 지정, /mnt/overlay를 사용하는 모든 항목 포함

1. **6.4 환경에 배포**  - AEM 6.4의 깨끗한 인스턴스(작성자 + 게시)는 개발/QA 환경에서 일어나야 합니다. 업데이트된 코드 베이스와 현재 프로덕션에서 제공하는 대표적인 컨텐츠 샘플을 배포해야 합니다.
1. **QA 유효성 검사 및 버그 수정**  - QA가 6.4의 작성자 및 게시 인스턴스에서 모두 애플리케이션의 유효성을 검사해야 합니다. 발견된 버그는 모두 6.4 코드 베이스에 수정되고 커밋되어야 합니다. 모든 버그가 수정될 때까지 필요에 따라 개발 주기를 반복합니다.
...
6.4는 새 AEM 버전에서 작동하도록 코드 베이스와 사용자 지정을 업그레이드하는 옵션 외에도 [이 페이지](/help/sites-deploying/backward-compatibility.md)에 설명된 대로 이전 버전과의 호환성 기능을 사용하여 사용자 지정을 보다 효율적으로 관리하는 데 도움이 됩니다.

위에서 언급되고 아래 다이어그램에 표시된 대로 첫 번째 단계에서 [패턴 탐지기](/help/sites-deploying/pattern-detector.md)를 실행하면 업그레이드의 전반적인 복잡도를 평가하고 호환성 모드에서 실행할지 아니면 새로운 AEM 6.4 기능을 모두 사용하도록 사용자 지정을 업데이트할지 여부를 평가하는 데 도움이 됩니다. 자세한 내용은 [AEM 6.4](/help/sites-deploying/backward-compatibility.md)의 이전 호환성 페이지를 참조하십시오.
...
클래식 UI 작성은 AEM 6.4에서 계속 사용할 수 있지만 더 이상 사용되지 않습니다. 자세한 내용은 [여기](/help/release-notes/deprecated-removed-features.md#pre-announcement-for-next-release)를 참조하십시오. 애플리케이션이 현재 클래식 UI 작성 환경에서 실행 중인 경우 AEM 6.4로 업그레이드하고 클래식 UI를 계속 사용하는 것이 좋습니다. 그런 다음 터치 UI로의 마이그레이션을 여러 개발 주기에 걸쳐 완료할 수 있도록 별도의 프로젝트로 계획할 수 있습니다. AEM 6.4에서 클래식 UI를 사용하려면 코드 베이스에 몇 가지 OSGi 구성을 커밋해야 합니다. 이 구성 방법에 대한 자세한 내용은 [여기](/help/sites-administering/enable-classic-ui.md)에서 확인할 수 있습니다.

>[!NOTE]
>
>클래식 UI에서 벗어나 최신 AEM 기술을 활용하려면 [AEM 현대화 도구](/help/sites-developing/modernization-tools.md)를 활용하여 마이그레이션을 더 쉽게 수행할 수 있도록 하십시오.

## 6.4 저장소 구조 {#align-repository-structure}와 정렬
...
1. 업그레이드해야 하는 인스턴스에서 `https://server:port/crx/de/index.jsp`(으)로 이동하여 CRXDE Lite을 엽니다.
...
기존 자산에 대한 자산 ID를 생성하려면 AEM 인스턴스를 업그레이드하여 AEM 6.4를 실행합니다. 이 기능은 [자산 통찰력 기능](/help/assets/touch-ui-asset-insights.md)을 활성화하는 데 필요합니다. 자세한 내용은 [포함 코드 추가](/help/assets/touch-ui-using-page-tracker.md#adding-embed-code)를 참조하십시오.
...
ContextHub 구성은 업그레이드에 의해 적용됩니다. 기존 ContextHub 구성을 복구하는 방법에 대한 지침은 [여기](/help/sites-administering/contexthub-config.md#recovering-contexthub-configurations-after-upgrading)에서 확인할 수 있습니다.

### 워크플로우 사용자 지정 {#workflow-customizations}

필요한 기능을 추가하거나 제거하기 위해 즉시 수정 워크플로우를 업데이트하는 것이 일반적인 방법입니다. 사용자 지정된 일반적인 워크플로우는 DAM 자산 업데이트 워크플로우입니다. 사용자 지정 구현에 필요한 모든 워크플로우는 업그레이드 중에 덮어쓸 수 있으므로 백업 및 버전 제어에 저장해야 합니다.
...
편집 가능한 템플릿의 구조가 AEM 6.2와 6.3 간에 변경되었습니다. 6.2 이하에서 업그레이드하는 경우 및 편집 가능한 템플릿을 사용하여 사이트 콘텐츠를 빌드하는 경우 [응답형 노드 정리 도구](https://github.com/Adobe-Marketing-Cloud/aem-sites-template-migration)를 사용해야 합니다. 이 도구는 컨텐츠를 정리하기 위해 **_after_** 업그레이드를 실행하기 위한 것입니다. 작성 계층과 게시 계층 모두에서 실행해야 합니다.

### CUG 구현 변경 {#cug-implementation-changes}

닫힌 사용자 그룹 구현이 이전 버전의 AEM의 성능 및 확장성 제한을 해결하기 위해 크게 변경되었습니다. 이전 버전의 CUG는 6.3에서 더 이상 사용되지 않으며, 새 구현은 Touch UI에서만 지원됩니다. 6.2 이상에서 업그레이드하는 경우 새 CUG 구현으로 마이그레이션하기 위한 지침 은 [여기](/help/sites-administering/closed-user-groups.md#upgrade-cug)에서 찾을 수 있습니다.
