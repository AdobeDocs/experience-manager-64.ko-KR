...
1. **패턴** 탐지기 - 업그레이드 계획에 설명되어 있고 [이 페이지에](/help/sites-deploying/pattern-detector.md) 자세히 설명된 대로 패턴 탐지기 보고서를 실행하여 AEM의 타겟 버전에서 사용할 수 없는 API/번들 외에도 해결해야 하는 영역에 대한 자세한 내용을 포함하는 패턴 탐지 보고서를 가져옵니다. 패턴 감지 보고서에서는 코드에 비호환성이 있다는 것을 표시해야 합니다. 배포가 이미 6.4와 호환되는 것이 없는 경우 6.4 기능을 활용하는 새로운 개발 작업을 선택할 수 있지만 호환성을 유지하기 위해서만 새로운 개발 작업이 필요한 것은 아닙니다. 비호환성이 보고되는 경우 a) 호환성 모드에서 실행하고 새로운 6.4 기능 또는 호환성에 대한 개발을 연기하거나, b) 업그레이드 후 개발을 결정한 다음 2단계로 이동합니다. 자세한 내용은 AEM [6.4의](/help/sites-deploying/backward-compatibility.md) 이전 버전과의 호환성을 참조하십시오.

1. **6.4용 코드** 베이스 개발 - Target 버전의 코드 베이스에 대한 전용 분기 또는 저장소를 만듭니다. 업그레이드 전 호환성의 정보를 사용하여 업데이트할 코드 영역을 계획합니다.
1. **6.4 Uber jar로 컴파일** - 코드 기반 POM을 업데이트하여 6.4 uber jar를 가리키고 이에 대한 코드를 컴파일합니다.
1. **AEM 사용자** 지정 업데이트 - AEM에 대한 모든 사용자 지정 또는 익스텐션은 6.4에서 작동되고 6.4 코드 베이스에 추가되도록 업데이트/검증되어야 합니다. UI 검색 양식, 자산 사용자 지정, /mnt/overlay 사용 항목 포함

1. **6.4 환경에 배포** - AEM 6.4(작성자 + 게시)의 깔끔한 인스턴스는 개발/QA 환경에서 일어나야 합니다. 업데이트된 코드 베이스와 현재 프로덕션에서 제공하는 대표적인 컨텐츠 샘플을 배포해야 합니다.
1. **QA 유효성 검사 및 버그 수정** - QA는 작성자 및 게시 인스턴스 6.4에서 애플리케이션의 유효성을 확인해야 합니다.발견된 모든 버그는 수정되고 6.4 코드 베이스에 커밋되어야 합니다. 모든 버그가 수정될 때까지 필요에 따라 개발 주기를 반복합니다.

업그레이드를 진행하기 전에 AEM의 대상 버전에 대해 철저히 테스트된 안정적인 애플리케이션 코드 베이스가 있어야 합니다. 테스트의 관찰을 기반으로 사용자 지정 코드를 최적화할 수 있는 방법이 있습니다. 여기에는 저장소 트래버스하지 않도록 하기 위해 코드를 리팩토링하거나, 검색을 최적화하기 위해 사용자 정의 색인 지정 또는 JCR에서 순서가 없는 노드를 사용하는 등의 작업이 포함될 수 있습니다.

6.4는 코드 베이스와 사용자 지정을 업그레이드하여 새로운 AEM 버전에서 사용할 수 있도록 하는 옵션 외에도 [이 페이지에](/help/sites-deploying/backward-compatibility.md)설명된 대로 이전 버전과의 호환성 기능을 사용하여 사용자 지정을 보다 효율적으로 관리하는 데 도움이 됩니다.

위에 언급되어 아래 다이어그램에 표시된 바와 같이, [첫 번째 단계에서](/help/sites-deploying/pattern-detector.md) 패턴 탐지기 실행을 수행하면 업그레이드의 전반적인 복잡성과 호환성 모드에서 실행할지 또는 모든 새로운 AEM 6.4 기능을 사용하도록 사용자 지정을 업데이트할지 여부를 평가하는 데 도움이 됩니다. 자세한 내용은 [AEM 6.4의](/help/sites-deploying/backward-compatibility.md) 이전 버전과의 호환성 페이지를 참조하십시오.
...
기존 자산에 대한 자산 ID를 생성하려면 AEM 인스턴스를 업그레이드하여 AEM 6.4를 실행하십시오.자산 인사이트 기능을 활성화하려면 [이 기능이](/help/assets/touch-ui-asset-insights.md)필요합니다. 자세한 내용은 포함 [코드](/help/assets/touch-ui-using-page-tracker.md#adding-embed-code)추가를 참조하십시오.
...
ContextHub 구성은 업그레이드에 의해 영향을 받습니다. 기존 ContextHub 구성을 복구하는 방법에 대한 지침은 [여기에서](/help/sites-administering/contexthub-config.md#recovering-contexthub-configurations-after-upgrading)확인할 수 있습니다.
...
편집 가능한 템플릿의 구조가 AEM 6.2와 6.3 간에 변경되었습니다.6.2 이전 버전에서 업그레이드하는 경우 사이트 컨텐츠가 편집 가능한 템플릿을 사용하여 빌드된 경우 응답형 노드 정리 [도구를 사용해야 합니다](https://github.com/Adobe-Marketing-Cloud/aem-sites-template-migration). 이 도구는 컨텐츠를 정리하기 위해 업그레이드 **_후에_** 실행되어야 합니다. 작성자 및 게시 계층 모두에서 실행해야 합니다.

### CUG 구현 변경 {#cug-implementation-changes}

닫힌 사용자 그룹 구현이 이전 버전의 AEM에서 성능 및 확장성 제한을 해결하기 위해 크게 변경되었습니다. CUG의 이전 버전은 6.3에서 더 이상 사용되지 않으며, 새로운 구현은 터치 UI에서만 지원됩니다. 6.2 이상에서 업그레이드하는 경우 새 CUG 구현으로 마이그레이션하기 위한 지침을 [여기에서](/help/sites-administering/closed-user-groups.md#upgrade-cug)찾을 수 있습니다.
