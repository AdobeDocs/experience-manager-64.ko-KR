---
title: AEM 6.4의 Forms 리포지토리 구조조정
seo-title: AEM 6.4의 Forms 리포지토리 구조조정
description: Forms용 AEM 6.4의 새로운 저장소 구조로 마이그레이션하기 위해 필요한 변경 방법을 알아봅니다.
seo-description: Forms용 AEM 6.4의 새로운 저장소 구조로 마이그레이션하기 위해 필요한 변경 방법을 알아봅니다.
...
# AEM 6.4의 Forms 리포지토리 구조조정{#forms-repository-restructuring-in-aem}

AEM 6.4 [페이지의 상위](/help/sites-deploying/repository-restructuring.md) 저장소 재구성 페이지에 설명된 대로 AEM 6.4로 업그레이드하는 고객은 이 페이지를 사용하여 AEM Forms 솔루션에 영향을 주는 저장소 변경과 관련된 작업 노력을 평가해야 합니다. 일부 변경 사항은 AEM 6.4 업그레이드 프로세스 중에 작업해야 하는 반면, 다른 변경 사항은 6.5 업그레이드 때까지 연기될 수 있습니다.
...
| **메모** | 이러한 클라이언트 라이브러리는 수정되거나 확장될 수 없습니다. |

| **이전 위치** | `/etc/clientlibs/fd/rte` |
|---|---|
| **새 위치** | `/libs/fd/rte` |
| **구조 조정 지침** | 절대 경로에 의해 참조될 수 있는 클라이언트 라이브러리의 리소스를 보려면 새 자산에 새로운 경로를 사용해야 합니다. |
| **메모** | N/A |
...
| **구조 조정 지침** | 이러한 clientlibs를 변경하는 것은 권장되거나 지원되지 않았습니다. 이러한 clientlibs를 수정한 경우 AEM 제공 코드를 사용하기 위해 롤백해야 합니다. |
| **메모** | N/A |
...
| **구조 조정 지침** | Forms 마이그레이션 UI에서 [트리거되는 Lazy Content Migration](/help/sites-deploying/lazy-content-migration.md) 유틸리티입니다. |
| **메모** | N/A |
...
|---|---|
| **새 위치** | `/libs/fd/fp` |
| **구조 조정 지침** | /etc 템플릿에 대한 모든 참조는 해당 템플릿을 가리키도록 `/libs` 업데이트해야 합니다. |
| **메모** | N/A |
