...
* 일부는 [일반적으로 모든 프로젝트 설치에](#primary-configuration-considerations) 대해 구성되며 프로젝트에 해당되는지 여부를 확인하기 위해 검토해야 합니다.
* [필수 구성](#further-configuration-considerations) 없이 일반 구성 기능 또는 시스템 성능 및 안정성과 관련됨
...
* **Adobe CQ 웹 콘솔**
   ...
   자세한 [내용 및 권장 사항은 OSGi](/help/sites-deploying/configuring-osgi.md) 구성을 참조하십시오.

* **저장소**

   저장소에서 OSGi 구성의 하위 세트를 사용할 수 있습니다. 이를 통해 저장소 컨텐츠를 복사하거나 복제하면 동일한 구성이 다시 만들어집니다. 실행 모드에 따라 저장소에 고유한 구성을 추가할 수도 있습니다.

   자세한 내용은 저장소 [의 OSGi 구성](/help/sites-deploying/configuring-osgi.md#osgi-configuration-in-the-repository) 및 특히 저장소에 [새 구성 추가](/help/sites-deploying/configuring-osgi.md#adding-a-new-configuration-to-the-repository) 를참조하십시오.
...
>
>특정 변경 사항은 애플리케이션에 큰 영향을 줄 수 있습니다. 따라서 AEM 구성을 시작하기 전에 필요한 경험과 지식을 갖추고 있어야 하며, 필요한 변경 사항만 수행해야 합니다. OSGi 콘솔을 통해 변경한 내용은 실행 중인 시스템에 **즉시** 적용됩니다(다시 시작할 필요 없음).
...
>[!NOTE]
>
>UI 선택에 대한 자세한 내용은 UI 선택 [에서 확인할 수 있습니다](/help/sites-authoring/select-ui.md).
...
새 버전 [이](/help/sites-deploying/version-purging.md) 만들어질 때 AEM에서 이전 버전을 삭제하도록 [구성하는 방법에 대한 자세한 내용은 특정](/help/sites-deploying/version-purging.md#version-manager) Version Manager의 버전 삭제를 참조하십시오.
...
자세한 [내용은 리소스](/help/sites-deploying/resource-mapping.md) 매핑을 참조하십시오.
...
프로젝트 [구현과 관련된 다양한 번들(번들에 따라 나열됨)의 목록은 OSGi 구성 설정](/help/sites-deploying/osgi-configuration-settings.md) 을 참조하십시오. 나열된 모든 설정을 조정할 필요는 없으며 AEM 작동 방식을 이해하는 데 도움이 되는 경우도 있습니다.
...
LDAP 인증은 저장소 수준에서 발생하므로 저장소에서 직접 처리됩니다. 자세한 내용은 AEM과 [함께 LDAP 구성을 참조하십시오](/help/sites-administering/ldap-config.md).

AEM 내 사용자 관리(액세스 권한 할당 포함)의 경우 [사용자 관리 및 보안을 참조하십시오](/help/sites-administering/security.md).

### 디스패처 구성 {#configuring-the-dispatcher}

Dispatcher는 Adobe 캐싱 및/또는 로드 밸런싱 도구입니다. 또한 Dispatcher를 사용하면 AEM 서버가 공격으로부터 보호됩니다. 따라서 Dispatcher를 엔터프라이즈급 웹 서버와 함께 사용하여 AEM 인스턴스의 보안을 강화할 수 있습니다.

자세한 [구성](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher.html) 내용은 Dispatcher [에서](https://helpx.adobe.com/kr/experience-manager/dispatcher/using/dispatcher-configuration.html을 참조하십시오.) 특히 Dispatcher 구성을 참조하십시오.

### AEM LiveCycle 커넥터 구성 {#configuring-aem-livecycle-connector}

AEM Doc Services 및 AEM Doc Security가 출시되면서 이제 LiveCycle 문서 서비스를 불러와 XFA 양식을 렌더링하고 문서를 PDF로 변환하며 문서를 보호하는 기능이 제공됩니다. 자세한 내용은 [AEM LiveCycle 커넥터를](https://helpx.adobe.com/livecycle/help/aem/aem-livecycle-connector.html) 참조하십시오.

### 작업 오프로드 및 토폴로지 관리 {#job-offloading-and-topology-administration}

[오프로드는](/help/sites-deploying/offloading.md) 토폴로지의 Experience Manager 인스턴스 중 처리 작업을 배포합니다. 오프로드를 사용하면 특정 유형의 처리를 수행하기 위해 특정 Experience Manager 인스턴스를 사용할 수 있습니다. 전문적인 처리를 통해 사용 가능한 서버 리소스의 사용을 극대화할 수 있습니다.
...
표시되는 링크를 구성할 수 있습니다. 자세한 내용은 [시작 콘솔](/help/sites-developing/customizing-the-welcome-console.md) 구성을 참조하십시오.

### 성능 구성 {#configuring-for-performance}

[성능은](/help/sites-deploying/configuring-performance.md) 프로젝트의 핵심 요소입니다. AEM(및/또는 기본 저장소)의 특정 측면을 성능 최적화를 위해 구성할 수 있습니다.

자세한 [내용은 성능](/help/sites-deploying/configuring-performance.md#configuring-for-performance) 구성을 참조하십시오.
...
자세한 내용은 [포털 및 포틀릿을](/help/sites-administering/aem-as-portal.md) 참조하십시오.

### 정적 개체 만료 {#expiration-of-static-objects}

정적 객체(예: 아이콘)는 변경되지 않습니다. 따라서 시스템이 만료되지 않도록(적절한 시간 동안) 구성되어야 불필요한 트래픽을 줄일 수 있습니다.

자세한 [내용은 정적 개체](/help/sites-deploying/expiration-static-objects.md) 만료를 참조하십시오.
...
AEM 프로세스가 이 최대값을 초과하면 &quot;&quot; 메시지 `too many open files`&quot;가 표시됩니다 `error.log`.
...
1. 허용되는 최대 수를 설정합니다.
   ...
   기본적으로 `serverctl` 는 다음 `CQ_MAX_OPEN_FILES` 으로 `8192`구성됩니다. 대부분의 시나리오에 충분해야 합니다.
...
페이지 편집을 위한 실행 취소 및 다시 실행 명령의 동작을 제어하는 여러 가지 속성이 있습니다. 이러한 설정을 구성할 수 있습니다. 자세한 내용은 [페이지 편집을 위한 실행 취소](/help/sites-administering/config-undo.md) 구성을 참조하십시오.
...
자세한 [내용은 이메일 알림](/help/sites-administering/notification.md) 구성을 참조하십시오.
