...
모두 [OSGi 구성](/help/sites-deploying/configuring-osgi.md).

>[!NOTE]
>
>AEM에 로그인하는 것은 Sling 원칙을 기반으로 합니다. 자세한 내용은 [Sling 로깅](https://sling.apache.org/site/logging.html) 추가 정보.

## 전역 로깅 {#global-logging}

[Apache Sling 로깅 구성](/help/sites-deploying/osgi-configuration-settings.md) 루트 로거를 구성하는 데 사용됩니다. AEM에서 로그인하기 위한 전역 설정을 정의합니다.
...
1. A **로깅 로거** 이 메시지를 가져와 사양에 따라 형식을 지정합니다.
1. A **로깅 작성기** 정의한 실제 파일에 이 모든 메시지를 기록합니다.
...
첫 번째는 두 가지를 모두 제어할 수 있는 특별한 경우입니다 `request.log` 및 `access.log` 파일:
...
>[!NOTE]
>
>특정 상황에서 [사용자 지정 로그 파일](/help/sites-deploying/monitoring-and-maintaining.md#create-a-custom-log-file).
