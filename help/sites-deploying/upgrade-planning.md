...
지원되는 운영 체제, Java 런타임, httpd 및 Dispatcher 버전을 실행 중인지 확인하는 것이 중요합니다. 자세한 내용은 [AEM 6.4 기술 요구 사항 페이지](/help/sites-deploying/technical-requirements.md). 이러한 구성 요소를 업그레이드하려면 프로젝트 계획에서 고려해야 하며 AEM을 업그레이드하기 전에 수행해야 합니다.
...
AEM 6.4에는 업그레이드 작업을 보다 원활하게 수행하는 데 도움이 되는 저장소 구조 변경 사항이 도입되었습니다. 변경 사항에는 Adobe 또는 고객이 컨텐츠를 소유하는지 여부에 따라 /etc 폴더의 컨텐츠를 /libs, /apps 및 /content를 포함하는 폴더로 이동하여 릴리스 중 컨텐츠를 덮어쓸 가능성을 제한합니다. 저장소 구조 변경은 6.4 업그레이드 시 세부 사항을 검토하 는 것이 좋지만 이러한 방식으로 수행되었습니다. [AEM 6.4의 저장소 구조 변경](/help/sites-deploying/repository-restructuring.md) 업그레이드를 계획하는 동안
...
6.4에 도입된 패턴 탐지기는 대부분의 경우 업그레이드 중에 예상되는 내용을 상당히 정확하게 추정해야 합니다. 그러나 호환되지 않는 변경 사항이 있는 복잡한 사용자 지정 및 배포의 경우, 다음 지침에 따라 개발 인스턴스를 AEM 6.4로 업그레이드할 수 있습니다. [즉석 업그레이드 수행](/help/sites-deploying/in-place-upgrade.md). 완료되면 이 환경에서 높은 수준의 연기 테스트를 수행합니다. 이 연습의 목적은 테스트 사례 인벤토리를 철저하게 완료하고 결함 대한 공식 인벤토리를 작성하는 것이 아니라 6.4 호환성을 위해 코드를 업그레이드하는 데 필요한 작업량의 대략적인 추정치를 제공하는 것입니다. 와 결합하는 경우 [패턴 감지](/help/sites-deploying/pattern-detector.md) 그리고 이전 섹션에서 결정된 아키텍처 변경 사항을 통해 업그레이드를 계획하기 위해 프로젝트 관리 팀에 대략적인 견적을 제공할 수 있습니다.
...
우리는 [코드 및 사용자 지정 업그레이드](/help/sites-deploying/upgrading-code-and-customizations.md) AEM 6.4와 호환되도록 합니다. 이 반복 프로세스가 실행되면 필요에 따라 Runbook을 변경해야 합니다. 참조: [AEM 6.4의 이전 버전과의 호환성](/help/sites-deploying/backward-compatibility.md) 업그레이드 후 즉시 개발할 필요 없이 대부분의 경우 사용자 지정 사항이 이전 버전과 호환될 수 있는 방법에 대한 정보입니다.
