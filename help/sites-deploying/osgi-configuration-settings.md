...
* **설치 폴더 이름** 은 설치 폴더의  **최대 계층 깊이를 나타냅니다** . 저장소 폴더의 설치 위치와 깊이를 지정합니다. 와일드카드를 사용할 때(에서와 같이)&amp;ast;/install) 적절한 모든 일치 항목(예: `/libs/sling/install` 및 `/libs/cq/core/install`)이 검색됩니다.
...
AEM에 사용할 수 있는 [두 개의 UI](/help/sites-authoring/select-ui.md)가 있습니다.
...
* **페이지 통계** 를 추적하는 데 사용되는 URL을 구성하여 데이터를 전송할 URL입니다(추적기 요청이 디스패처를 통과하는 경우 필수).예를 들어 기본값은  `http://localhost:4502/libs/wcm/stats/tracker`입니다.
