...
description: 구성 요소는 사이드 킥의 구성 요소 탭 및 새 구성 요소 삽입 선택기(구성 요소나 자산을 여기로 드래그 영역을 두 번 클릭)에서 페이지를 편집할 때 사용할 수 있습니다.
...
다음 구성 요소는 표준 웹 페이지의 컨텐츠를 작성할 때 사용하기 위한 것입니다. 이러한 구성 요소는 AEM의 표준 설치에 기본적으로 사용할 수 있는 구성 요소의 하위 집합을 형성합니다.

일부 구성 요소는 사이드 킥을 통해 즉시 사용할 수 있으며, 다른 구성 요소는 를 사용하여 사용할 수도 있습니다 [디자인 모드](/help/sites-classic-ui-authoring/classic-page-author-design-mode.md) 활성화하거나 비활성화하는 중입니다.
...
구성 요소는 [페이지 편집](/help/sites-classic-ui-authoring/classic-page-author-edit-content.md) 에서 **구성 요소** 사이드 킥의 탭 및 **새 구성 요소 삽입** 선택기(를 두 번 클릭하면 **구성 요소 또는 자산을 여기로 드래그하십시오** 영역 참조).

구성 요소를 선택하여 페이지에서 필요한 위치로 드래그할 수 있습니다 [컨텐츠 및 속성 편집](/help/sites-classic-ui-authoring/classic-page-author-edit-content.md#editing-a-component-content-and-properties).
...
클릭 가능한 컨트롤을 통해 사용자가 표시되는 페이지를 원하는 대로 실시간으로 순환하게 할 수도 있습니다. 현재 표시된 페이지 이미지를 클릭하면 해당 페이지로 이동합니다. 즉, 회전판은 탐색 컨트롤의 역할을 합니다.
...
여기서는 회전판에 페이지가 포함되는 방법을 지정합니다.
   ...
   * **목록 작성 방법**
      ...
      페이지 목록을 만드는 방법에는 하위 페이지, 고정 목록, 검색 또는 고급 검색(아래에 모두 설명됨)이 있습니다.

      어떤 방법을 선택하든 목록에 포함된 각 페이지에는 이미지가 이미 연결되어 있어야 합니다. 회전판에 표시될 이 이미지입니다. 해당 페이지의 속성 아래에 지정된 페이지에 대한 이미지가 없는 경우 작업을 시작하기 전에 페이지에 이미지를 연결해야 합니다. 그러지 않으면 회전판에 비어 있거나 대부분이 비어 있는 페이지가 표시됩니다. 자세한 내용은 [페이지 속성 편집](/help/sites-classic-ui-authoring/classic-page-author-edit-page-properties.md).
      ...
      * **고정 목록 옵션**
         ...
         * **페이지**
            ...
            페이지 목록을 선택합니다. 사용 `+` 항목을 더 추가하려면 위쪽/아래쪽 단추를 눌러 순서를 조정합니다.
      * **검색 옵션**
...
>[!NOTE]
>
>AEM DAM에 있는 디지털 자산을 표시하는 Adobe Experience Manager의 사용자 지정 회전판 구성 요소를 만들 수 있습니다. 자세한 내용은 [Adobe Experience Manager용 사용자 지정 회전판 구성 요소 만들기](https://helpx.adobe.com/kr/experience-manager/using/custom-carousel-components.html).
...
>[!NOTE]
>
>AEM JCR에 있는 데이터를 표시하는 사용자 지정 AEM 차트 컨트롤을 만들 수 있습니다. 자세한 내용은 [차트에 Adobe Experience Manager 데이터 표시](https://helpx.adobe.com/kr/experience-manager/using/displaying-experience-manager-data-chart.html).
...
[컨텐츠 조각](/help/sites-classic-ui-authoring/classic-page-author-content-fragments.md) 페이지와 독립된 자산으로 작성 및 관리됩니다. 그런 다음 콘텐츠 페이지를 작성할 때 이러한 조각과 해당 변형을 사용할 수 있습니다.
...
* **다운로드**
   ...
   * **파일**
      ...
      결과 웹 페이지에서 다운로드할 수 있는 파일입니다. Content Finder에서 자산을 드래그하거나 영역을 클릭하여 다운로드할 수 있는 파일을 업로드합니다.
...
>[!NOTE]
>
>사용 사례에 따라 외부 애플리케이션 통합을 위해 다른 옵션을 사용할 수도 있습니다. 예를 들면 다음과 같습니다. a [포틀릿 통합](/help/sites-administering/aem-as-portal.md).
...
이미지를 [컨텐츠 파인더](/help/sites-classic-ui-authoring/classic-page-author-env-tools.md#the-content-finder) 구성 요소 또는 해당 편집 대화 상자에 바로 액세스할 수 있습니다. 편집 대화 상자의 가운데 영역을 두 번 클릭하여 로컬 파일 시스템을 탐색하고 이미지를 업로드할 수도 있습니다. 편집 대화 상자의 두 탭은 이미지의 모든 정의 및 조작도 제어합니다.
...
>[!CAUTION]
>
>레이아웃 컨테이너 구성 요소는 클래식 UI에서 사용할 수 있지만 전체 기능은 터치 가능 UI에서만 사용할 수 있습니다. 자세한 내용은 [응답형 레이아웃](/help/sites-classic-ui-authoring/classic-page-author-responsive-layout.md).
...
* **목록**
   ...
   * **목록 작성 방법**
      ...
      여기에서 목록의 내용을 검색할 위치를 지정합니다. 다음과 같은 몇 가지 방법이 있습니다.
   ...
   * 선택한 항목에 따라 새 패널이 나타납니다.
      ...
      * **고정 목록 옵션**
         ...
         * **페이지** - 페이지 목록을 선택합니다. 항목을 더 추가하려면 +를 사용하고, 순서를 조정하려면 위쪽/아래쪽 단추를 사용하십시오.
      * **검색 옵션**

         * **시작 위치** - 시작 경로를 수동으로 또는 선택기를 사용하여 입력합니다. 검색 쿼리
일반 텍스트 검색 쿼리를 입력할 수 있습니다.
      * **고급 검색 옵션**

         * **QueryBuilder 설명 표기법** - QueryBuilder 설명 표기법을 사용하여 검색 쿼리를 입력할 수 있습니다. 예를 들어 &quot;fulltext=Marketing&quot;을 입력하면 컨텐츠에 &quot;Marketing&quot;이 포함된 모든 페이지를 회전판에 표시할 수 있습니다. 자세한 내용은 [QueryBuilder API](/help/sites-developing/querybuilder-api.md) 쿼리 표현식 및 추가적인 예에 대한 전체 논의.
      * **태그** - 을(를) 지정합니다. **상위 페이지**, **태그/키워드** 및 필요한 일치 기준을 제공합니다.
   * **다음으로 표시**
...
이 구성 요소는 일련의 이미지를 순서대로 로드하여 페이지에 표시합니다. 이미지를 추가 또는 제거하고 각 이미지에 제목을 지정할 수 있습니다. 고급(Advanced)에서 표시 영역의 크기를 지정할 수도 있습니다.
...
>[!NOTE]
>
>다음 **표** 구성 요소는 [리치 텍스트 편집기](/help/sites-classic-ui-authoring/classic-page-author-rich-text-editor.md), 그대로 **[텍스트](#text)** 구성 요소.
>
>을 사용하는 것이 좋습니다 **표** 구성 요소를 사용하여 구성할 수도 있지만 **텍스트** 구성 요소.
...
>[!NOTE]
>
>다음 **텍스트** 구성 요소는 [리치 텍스트 편집기](/help/sites-classic-ui-authoring/classic-page-author-rich-text-editor.md), 그대로 **[표](#table)** 구성 요소.
>
>을 사용하는 것이 좋습니다 **표** 구성 요소를 사용하여 구성할 수도 있지만 **텍스트** 구성 요소.

텍스트 구성 요소로 WYSIWYG 편집기를 사용하여 다음을 통해 제공된 기능과 함께 텍스트 블록을 입력할 수 있습니다 [리치 텍스트 편집기](/help/sites-classic-ui-authoring/classic-page-author-rich-text-editor.md). 여러 가지 아이콘을 사용하여 글꼴 특성, 정렬, 링크, 목록 및 들여쓰기 등의 텍스트 서식을 지정할 수 있습니다.
...
* 현재 페이지의 이름을 표시합니다. 제목 필드를 비워 두면 됩니다
...
* 비디오
   ...
   * **크기** - 크기 옆의 상자에 비디오의 원래 크기(픽셀 단위 너비 x 높이)가 나타납니다(위 그림 참조). 비디오의 기본 차원을 재정의하려면 여기에 너비와 높이 차원을 수동으로 입력합니다. 클릭 **확인** 대화 상자를 닫습니다.
...
열은 AEM에서 컨텐츠 레이아웃을 제어하는 메커니즘입니다. 표준 설치 구성 요소에서 2 및/또는 3개의 열을 만들 수 있습니다.

다음 예는 사용 중인 2열 및 3열 구성 요소를 보여줍니다. 새 구성 요소에 대한 자리 표시자를 사용할 수 있습니다.
...
* **열 컨트롤**
   ...
   * **열 레이아웃**
      ...
      렌더링할 열 개수를 선택합니다. 만들어진 각 열에는 컨텐츠를 추가할 때 구성 요소나 자산을 드래그하기 위한 링크가 생깁니다.
...
다음은 예제 양식입니다. 이것은 **양식** 구성 요소(시작 및 끝), 두 개 사용 **양식 텍스트** 입력에 사용되는 필드, **일반 텍스트** 리드 인 텍스트 및 **제출** 버튼을 클릭합니다.
...
이러한 탭에서는 필요한 매개 변수를 제공합니다. 개별 구성 요소 유형에 따라 달라질 수 있지만 다음을 포함할 수 있습니다.
...
* **초기값**
   ...
   * **기본 값**
      ...
      양식을 열 때 필드에 표시되는 값입니다. 즉, 사용자가 입력을 하기 전에 채워집니다.
...
* **제한**
   ...
   * **제한** - 선택할 수 있는 제한 사항은 양식 구성 요소 유형에 따라 다릅니다.
   * **제한 메시지**
...
양식 구성 요소는 **양식 시작** 및 **양식 끝** 요소를 생성하지 않습니다. 이러한 매개 변수는 항상 쌍을 이루어야 양식이 올바르게 정의됩니다.
...
* **고급**
   ...
   * **클라이언트 유효성 검사**
      ...
      이 양식에 대해 클라이언트 유효성 검사가 필요한지 여부를 나타냅니다(서버 유효성 검사). *항상* 발생합니다. 이 작업은 와 함께 수행할 수 있습니다 **Forms Captcha** 구성 요소.
...
새 계정을 만들 때 사용되는 페이지입니다.
      ...
      * **계정 만들기 및 업데이트**
         ...
         * **데이터 보기...**
            ...
            벌크 편집기에서 양식 결과에 대한 정보에 액세스하려면 이 단추를 클릭하십시오. 여기에서 정보를 `.tsv`(탭으로 구분) 파일로 내보낼 수 있습니다(예를 들면 Excel 스프레드시트에서 사용하기 위해).
      * **메일**
...
>[!NOTE]
>
>사용자 지정 업로드 구성 요소를 만들어 파일을 Sling 서블릿에 업로드할 수 있습니다. 자세한 내용은 [Adobe Experience Manager에 파일 업로드](https://helpx.adobe.com/kr/experience-manager/using/uploading-files-aem1.html).
...
특수 탭을 사용하여 네임스페이스를 비롯한 다양한 매개 변수를 지정할 수 있습니다.
