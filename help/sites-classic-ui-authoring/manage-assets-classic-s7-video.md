...
이러한 질문에 대한 답변이 하나 또는 둘 다 &quot;예&quot;인 경우, 비디오를 Adobe DAM에 바로 업로드하십시오. 두 질문에 대한 답변이 &quot;아니요&quot;인 경우 비디오를 Dynamic Media Classic에 바로 업로드하십시오. 각 시나리오에 대한 워크플로우는 다음 섹션에 설명되어 있습니다.
...
1. 비디오 자산을 Adobe Assets에 업로드하고 자동으로 인코딩하여 Dynamic Media Classic에 게시할 수 있습니다.
...
자산에 대한 워크플로우나 버전 관리가 필요하지 않은 경우 Dynamic Media Classic에 자산을 업로드해야 합니다. 다음은 권장되는 워크플로우입니다.

1. Dynamic Media Classic에서 예약된 FTP 업로드 및 Dynamic Media Classic에 인코딩(시스템 자동화)을 [설정합니다](https://help.adobe.com/en_US/scene7/using/WS70B173EC-4CAD-4b4c-BF9C-43A11F3A5950.html).
...
1. **[!UICONTROL 비디오]** 탭을 선택합니다.
   ...
   >[!NOTE]
   >
   >페이지에 클라우드 구성이 없는 경우 **[!UICONTROL 비디오]** 탭이 표시되지 않습니다. WCM [용 Dynamic Media Classic 사용을 참조하십시오](#enablingscene7forwcm).
...
1. 선택한 인코딩 프로필은 이 Dynamic Media Classic 클라우드 구성에 대해 설정한 CQ DAM 대상 폴더에 업로드된 모든 비디오에 자동으로 적용됩니다. 필요에 따라 다른 대상 폴더를 사용하여 여러 Dynamic Media Classic 클라우드 구성을 설정하여 다른 인코딩 프로필을 적용할 수 있습니다.
...
Adobe DAM에서 Dynamic Media Classic에 마스터 비디오를 업로드하려면:

1. Dynamic Media Classic 인코딩 프로필을 사용하여 클라우드 구성을 설정한 CQ DAM 대상 폴더로 이동합니다.
1. **[!UICONTROL 업로드]**&#x200B;를 클릭하여 마스터 비디오를 업로드합니다. Video uploading and encoding is complete after the [!UICONTROL DAM Update Asset] workflow is complete and **[!UICONTROL Publish to Dynamic Media Classic]** has a checkmark.
...
AEM을 사용하는 경우 사이트에서 사용할 수 있는 비디오 구성 요소와 Dynamic Media Classic(Scene7) 비디오 구성 요소 모두에 액세스할 수 있습니다. 이러한 구성 요소는 서로 바꿔서 사용할 수 없습니다.

Dynamic Media Classic 비디오 구성 요소는 Dynamic Media Classic 비디오에만 작동합니다. 기본 구성 요소는 AEM(ffmpeg 사용) 및 Dynamic Media Classic 비디오에서 저장된 비디오와 함께 작동합니다.
...
>[!NOTE]
>
>기본적으로 Dynamic Media Classic 비디오 구성 요소는 유니버설 비디오 프로필을 사용합니다. 그러나 AEM에서 사용할 HTML5 기반 비디오 플레이어를 가져올 수 있습니다. Dynamic Media Classic에서 기본 HTML5 비디오 플레이어의 포함 코드를 복사하여 AEM 페이지에 넣습니다.


## AEM 비디오 구성 요소 {#aem-video-component}

Dynamic Media Classic 비디오 구성 요소를 사용하는 것이 Dynamic Media Classic 비디오를 보는 데 권장되지만, 이 섹션에서는 완벽함을 위해 AEM의 기본 비디오 구성 [!UICONTROL 요소와 함께 Dynamic Media] Classic 비디오를 사용하는 방법에 대해 설명합니다.
...
Dynamic Media Classic 클라우드 구성에서 선택한 Dynamic Media Classic 인코딩 사전 설정에 따라 다양한 비디오 인코딩이 생성됩니다. 기본 비디오 구성 요소가 해당 구성 요소를 사용하려면 선택한 각 Dynamic Media Classic 인코딩 사전 설정에 대해 비디오 프로필을 만들어야 합니다. 이렇게 하면 비디오 구성 요소가 DAM 표현물을 적절히 선택할 수 있습니다.
...
1. 새 비디오 프로필을 편집합니다. 먼저 클라우드 구성을 선택합니다. 그런 다음 클라우드 구성에서 선택한 것과 동일한 인코딩 사전 설정을 선택합니다.
   ...
   | HTML5 비디오 유형 | 이 속성을 사용하면 HTML5 비디오 소스 요소의 type 속성 값을 설정할 수 있습니다. 이 정보는 Dynamic Media Classic 인코딩 사전 설정에서 제공되지 않지만 HTML5 비디오 요소를 사용하여 비디오를 제대로 렌더링하는 데 필요합니다. 공통 형식의 목록이 제공되지만 다른 형식으로 덮어쓸 수 있습니다. |
...
1. 기본 비디오 구성 요소의 디자인 대화 상자를 열고 **[!UICONTROL 프로필]** 탭으로 변경합니다. 그런 다음 곧바로 사용할 수 있는 프로필을 삭제하고 새로운 Dynamic Media Classic 비디오 프로필을 추가합니다. 디자인 대화 상자의 프로필 목록 순서는 렌더링할 때 비디오 소스 요소의 순서를 정의합니다.
...
1. 클라우드 구성에서 선택한 각 비디오 인코딩 사전 설정에 대한 Dynamic Media Classic 비디오 프로필을 만듭니다.
