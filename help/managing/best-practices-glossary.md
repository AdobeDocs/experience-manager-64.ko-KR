...
## 테스트 시스템 통합 액세스 {#access-to-test-system-coordinated}
...
해당 사용자에 대한 기술 교육 제공 예를 들어 개발자, 설계자, 엔지니어 및 비즈니스 실무 담당자들이 있습니다.

## 프로젝트 목표로 정의된 KPI에 대한 계약 {#agreement-on-kpis-defined-as-goals-for-the-project}
...
## KPI를 사용하여 컨텐츠 아키텍처 정렬 {#alignment-of-content-architecture-with-kpis}

제안된 컨텐츠 아키텍처가 관련 주요 성능 지표(KPI)와 일치하는지 확인합니다.

## 프로젝트 타임라인에 고객 로드맵 조정 {#alignment-of-the-customer-roadmap-with-project-timeline}

고객 로드맵은 높은 수준의 이정표와 비즈니스 목표로 구성되어 있습니다. 프로젝트 타임라인은 이 전략을 준수하며 이에 부합해야 하므로 잠재적 위험 및/또는 가능한 편차를 강조 표시하고 추적해야 합니다.
...
## KPI와 비교하여 실제 컨텐츠 및 결과에 맞게 조정된 자동화된 테스트 세트 {#automated-test-suite-adapted-for-real-content-and-results-compared-to-kpis}
...
비즈니스 사례 문서는 작업 수행, 대체 작업 수행(가능한 경우) 또는 작업을 수행하지 않는 것과 관련된 인수를 제공합니다. 논거는 구체적인 사실(가능한/관련성이 있는 곳)에 기초하여 균형 있게 조정되어야 하며 모든 사건에 대한 장점과 위험을 강조해야 한다.
...
모든 적절한 성향/역할이 작업 설명서를 수신했는지 확인합니다.

## 성과 테스트 보고서 전달 {#communicate-performance-test-report}

모든 적절한 성향/역할이 성능 테스트 보고서를 받았는지 확인합니다.

## 릴리스 노트 전달 {#communicate-release-notes}

모든 적절한 성향/역할이 릴리스 노트를 수신했는지 확인합니다.

## 팀에 범위 및 기대 전달 {#communicate-scope-and-expectations-to-team}
...
## 컨텐츠 아키텍처 문서 {#content-architecture-document}
...
고객이 보고와 관련된 모든 정책 및/또는 요구 사항입니다. 여기에는 다음이 포함될 수 있습니다.
...
* 구체적인 보안 요구 사항 모든 입력 필드 이스케이프 처리, 암호화 사용(SSL), 인증서, 인증 및 세션화 등.
...
## 업그레이드에 영향을 주는 사용자 지정 및 핫픽스가 문서화되었습니다. {#customizations-and-hotfixes-that-affect-upgrades-documented}
...
프로젝트의 배포 및 릴리스를 모두 다루는 공식 정책. 여기에는 다음이 포함될 수 있습니다.
...
## 개발 팀이 프로젝트 및 기대 범위 이해 {#development-team-understands-scope-of-project-and-expectations}
...
솔루션의 전체 경험에 대한 요구 사항입니다. 개인화, 교차 장치 지속성 및 사용자 경험과 같은 요소에 대해 다룹니다.
...
## 테스트 대상 시스템 및 절차 {#fallback-system-and-procedure-tested}
...
행복한 경로는 예외적인 또는 오류 조건이 없는 기본 시나리오입니다. 모든 것이 예상대로 진행될 때 실행되는 활동의 시퀀스로 구성됩니다.
...
## 요구 사항을 충족하기 위한 하드웨어 사용 가능 {#hardware-will-be-available-to-fulfill-requirements}
...
## 주요 솔루션/기능 확인 {#identify-critical-key-solutions-functionalities}
...
## Go Live 일정에 대해 Adobe 지원 팀에 알림 {#inform-adobe-support-about-the-go-live-schedule}
...
## 문제 추적 시스템 프로세스가 설정 및 통합되었습니다. {#issue-tracking-system-process-is-set-up-and-integrated}

선택한 툴링이 완전히 통합되고 모든 필수 역할에 액세스가 부여됩니다.

## 레거시 시스템 {#legacy-system}

프로젝트의 경우 기존 시스템은 새 솔루션으로 대체되는 기존 기술, 컴퓨터 시스템 또는 응용 프로그램 프로그램입니다.
...
## 유지 관리 작업(AEM 특정) 테스트 및 사용 {#maintenance-tasks-aem-specific-tested-and-enabled}
...
## 시스템 엔지니어와 통신되는 모니터링 정책 {#monitoring-policy-communicated-to-system-engineer}
...
## Penetration Tests - 통과 {#penetration-tests-passed}

필요한 모든 기준이 전달됩니다.

## Penetration Tests - 결과 {#penetration-tests-results}
...
## 성과 테스트 - 보고서 {#performance-tests-report}

성능 테스트 결과를 자세히 설명하는 비즈니스용으로 생성된 보고서입니다.

## 성과 테스트 - 결과 일치 성과 KPI {#performance-tests-results-match-performance-kpis}

결과는 정의된 KPI 및 성능에 대한 기대치와 일치해야 합니다.

## 성향 기반 테스트 개념 {#persona-based-testing-concept}
...
POC(Proof Concept)는 두 요구 사항이 모두 일치하는지 확인하는 데 도움이 됩니다.
...
## 프로덕션 환경 지원 {#production-environment-ready}
...
타임라인과 책임에 대한 시각적 개요를 제공하기 위해 종종 양식을 작성하거나 차트를 포함합니다. 이 작업을 지원하는 데 사용할 수 있는 많은 도구가 있습니다.
...
## 정의된 대상 내의 프로젝트 상태 보고서 {#project-status-reports-within-a-defined-cadence}
...
## Go Live를 지원하기 위한 리소스 {#resources-available-to-support-go-live}
...
그들은 예상 투자 비용과 관련하여 예상되는 이익/이익을 규정함으로써 솔루션의 효율성을 경제적인 측면에서 나타내고자 한다.
...
## 목록에 추가된 보안 문제 및 평가 {#security-issues-listed-and-assessed}

나열된 솔루션 및 평가된 솔루션의 모든 보안 문제 작업 예상 포함

## 비즈니스 이해 당사자로부터 보안 로그오프 {#security-sign-off-from-business-stakeholders}
...
소프트웨어 아키텍처를 위한 높은 수준의 전략 서비스, 서블릿, 프레임워크 및 기타 구현 결정을 포함합니다.
...
## 이해 관계자는 성공 정의 및 기준을 알고 있습니다 {#stakeholders-are-aware-of-success-definitions-and-criteria}
...
시스템 아키텍처를 설명하는 문서 다른 정보 중에서 모든 환경을 위한 인터페이스, 네트워크 위치 및 통합을 포함합니다.
...
## 시스템 위험 요소 확인 및 확인 {#system-risk-factors-identified-and-verified}
...
## 팀이 통신 계획을 알고 있음 {#team-is-aware-of-the-communication-plan}
...
## 기술 위험 요소 검증 {#technical-risk-factors-verified}
...
모든 릴리스 후보 코드가 최신 테스트 코드를 사용할 수 있도록 자동화된 배포를 준비하여 테스트 환경이 준비되었는지 확인합니다.

## 테스트 보고서 {#test-reports}

테스트 결과를 자세히 설명하는 보고서 다음을 포함합니다.
...
## 선택한 테스트 도구 세트 {#test-tooling-suite-selected}
...
## 프로젝트 및 기대 범위 이해 {#understands-scope-of-project-and-expectations}
...
URL 처리 개념은 다음을 포함한 AEM 특정 URL 기능을 다룹니다.
