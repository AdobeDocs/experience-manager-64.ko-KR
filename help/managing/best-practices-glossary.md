...
title: 용어 설명
seo-title: 용어 설명
description: 이 용어집은 프로젝트 체크리스트에 있는 모든 결과물 문서의 세부 사항(알파벳순)을 나열합니다.
seo-description: 이 용어집은 프로젝트 체크리스트에 있는 모든 결과물 문서의 세부 사항(알파벳순)을 나열합니다.
...
이 용어에는 [프로젝트 체크리스트](/help/managing/best-practices-checklist.md)의 모든 결과물 문서의 세부 정보가 표시됩니다(알파).
...
[Adobe 보안 검사 목록](/help/sites-administering/security-checklist.md)은 AEM이 설치 시 안전한지 확인하기 위해 제공되는 공식 확인 목록입니다. 인스턴스의 무결성을 보장하기 위해 수행해야 하는 보안 측정과 확인 단계가 포함되어 있습니다.
...
솔루션 관리 직원을 위한 트레이닝 자세한 내용은 [Adobe 교육 서비스](https://training.adobe.com/training/courses.html#solution=adobeExperienceManager)를 참조하십시오.

## AEM 작성자 교육 {#aem-author-training}

솔루션용 컨텐츠를 작성(작성)할 직원을 위한 트레이닝 자세한 내용은 [Adobe 교육 서비스](https://training.adobe.com/training/courses.html#solution=adobeExperienceManager)를 참조하십시오.

## AEM 인증 시험 {#aem-certification-exam}

관련 [인증 시험](https://training.adobe.com/certification/exams.html#p=1&amp;solution=adobeExperienceManager)을(를) 수강하기 위해 적절한 사람이 등록되었는지 확인합니다.

## AEM 인증 {#aem-certified}

적절한 사람이 관련 [인증 시험](https://training.adobe.com/certification/exams.html#p=1&amp;solution=adobeExperienceManager)에 합격했는지 확인합니다.
...
## 비즈니스 및 성과 KPI 정렬 {#align-business-and-performance-kpis}
...
## KPI {#alignment-of-content-architecture-with-kpis}에 콘텐츠 아키텍처 정렬
...
## 응용 프로그램 특정 유지 관리 작업 정의 {#application-specific-maintenance-tasks-defined}
...
## 실제 컨텐츠와 결과에 맞게 조정된 자동화된 테스트 세트를 KPI {#automated-test-suite-adapted-for-real-content-and-results-compared-to-kpis}과 비교
...
## 실제 로드와 예상 부하(a0/>)에 대해 검증된 자동화된 테스트 전략{#automated-testing-strategy-validated-against-realistic-and-expected-load}
...
## 비즈니스 분석가는 프로젝트의 범위와 기대 {#business-analyst-understands-scope-of-project-and-expectations} 이해
...
## ROI 및 KPI 기대치에 맞게 조정되는 솔루션 또는 아키텍처에 대한 필요한 조정 사항에 대한 비즈니스 서명: {#business-sign-off-on-any-required-adjustments-to-the-solution-or-architecture-identified-and-aligned-against-roi-and-kpi-expectations}
...
## 알림 릴리스 노트 {#communicate-release-notes}

해당 모든 페르소나/역할이 릴리스 노트를 수신했는지 확인합니다.

## 팀 {#communicate-scope-and-expectations-to-team}에 범위 및 기대치 전달

프로젝트 팀이 프로젝트 범위와 전달에 대한 기대치를 충분히 인식하고 그에 부합하는지 확인합니다.

## 교육 자료 및 사용자 안내서 통신 {#communicate-training-materials-and-user-guides}
...
## 마이그레이션{#content-validated-for-migration}에 대해 유효성 검사된 콘텐트
...
## 업그레이드에 영향을 주는 사용자 지정 및 핫픽스 문서화된 {#customizations-and-hotfixes-that-affect-upgrades-documented}
...
## 배포 대상 설정됨 {#deployment-cadence-established}
...
## 문서 처리 오류 {#error-handling-documentation}
...
## 일반 품질 검토 세션 설정 {#establish-regular-quality-review-sessions}
...
## 폴백 시스템 및 절차가 테스트됨 {#fallback-system-and-procedure-tested}

폴백 시스템의 엔드 투 엔드 테스트

## 비즈니스 이해 관계자로부터 시스템 로그오프 폴백 {#fallback-system-sign-off-from-business-stakeholders}
...
## 계약 완료 {#finalized-contract}

프로젝트를 진행하기 전에 확정되고 서명한 계약이 필요하다. 이것은 [계약 초안](#contract-draft)을 기반으로 합니다.
...
## 하드웨어를 요구 사항 충족에 사용할 수 있음 {#hardware-will-be-available-to-fulfill-requirements}
...
## 수준 높은 솔루션 디자인 {#high-level-solution-design}
...
## 중요한 주요 솔루션/기능 식별 {#identify-critical-key-solutions-functionalities}
...
## 구현 - 컨텐트 아키텍처 {#implementation-content-architecture}
...
## 구현 - {#implementation-url-handling} URL 처리
...
## 이동 실시간 일정 {#inform-adobe-support-about-the-go-live-schedule}에 대해 Adobe 지원에 알림
...
## Adobe 지원 포털 {#list-of-users-that-require-access-to-adobe-support-portal}에 액세스해야 하는 사용자 목록
...
## 유지 관리 작업(AEM 특정) 테스트되고 활성화된 {#maintenance-tasks-aem-specific-tested-and-enabled}
...
## 모니터링 - 보안 포인트 {#monitoring-security-points}
...
## 모니터링 보고서 - 제자리에 {#monitoring-reports-structure-in-place} 구조
...
## POC 요구 사항 문서에 대해 테스트되고 확인됨 {#poc-tested-and-verified-against-requirement-documentation}
...
## 비즈니스 이해 관계자로부터 프로덕션 서명 {#production-sign-off-from-business-stakeholders}
...
## 프로덕션 서명 해제 프로세스 및 정책 {#production-sign-off-process-and-policy}
...
[초기 예측](#project-efforts-initial-estimates)은 높은 수준이었으며 구현을 위한 높은 수준의 요구 사항에 따라 수행됩니다.
...
## 지원 가능한 리소스 {#resources-available-to-support-go-live} 라이브 실행
...
## 보안 문제 목록 및 평가 {#security-issues-listed-and-assessed}

목록에 기재되어 평가된 솔루션의 모든 보안 문제노력 예상치를 포함하여.

## 비즈니스 이해 관계자로부터 보안 서명 {#security-sign-off-from-business-stakeholders}
...
## 타사 시스템 {#slas-for-third-party-systems} SLA
...
## 솔루션 검토 위원회가 설치되고 회의 대상 집합 {#solution-review-board-established-and-meeting-cadence-set}
...
## 솔루션 로그오프 및 수락 프로세스 {#solution-sign-off-and-acceptance-process}
...
## AEM 관리자 교육 {#staff-selected-for-aem-administrator-training}에 대해 선택한 직원
...
## 이해 관계자는 성공 정의 및 기준 {#stakeholders-are-aware-of-success-definitions-and-criteria}에 대해 알고 있습니다.
...
## 이해 관계자는 프로젝트 및 기대치를 파악합니다. {#stakeholders-understand-project-and-expectations}
...
## 지원 프로세스 및 Adobe 지원 포털 {#support-processes-and-access-to-adobe-support-portal} 액세스
...
## 팀이 성공 정의 및 기준 {#team-is-aware-of-success-definitions-and-criteria}에 대해 알고 있습니다.

전체 팀이 성공 정의와 기준을 알고 있는지 확인합니다.

## 팀이 통신 계획 {#team-is-aware-of-the-communication-plan}에 대해 알고 있습니다.

모든 팀원이 고객과 소통해야 하는 사람과 구체적인 방법 및 시기를 알고 있는지 확인합니다.

## 팀은 프로젝트와 기대를 이해함 {#team-understands-project-and-expectations}
...
## 테스트 도구 세트가 선택된 {#test-tooling-suite-selected}
...
이러한 계획은 각 개발 단계에 대한 테스트 실행에 대해 보다 자세히 설명하고 [테스트 전략](#testing-strategy)을 기반으로 합니다.
...
## 제3자 통합 개념 {#third-party-integration-concept}

제3자 시스템과의 통합을 위한 건축 및 시스템 수준 개념.

## 제3자 통합 사양 {#third-party-integration-specification}
...
## 통합을 위한 제3자 시스템 {#third-party-system-for-integration}

통합 구현을 위해 적절한 설명서와 함께 모든 제3자 시스템을 사용할 수 있는지 확인합니다.

## 타사 시스템 액세스 활성화 {#third-party-systems-access-enabled}

제3자 시스템과 함께 사용되는 각 역할에 부여된 필수 액세스 권한.

## 제3자 테스트 개념 {#third-party-testing-concept}
...
## 유효성이 확인된 예산 계획 {#validated-budget-plan}
