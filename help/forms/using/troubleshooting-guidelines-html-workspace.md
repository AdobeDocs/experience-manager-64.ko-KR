---
title: AEM Forms 작업 영역에 대한 문제 해결 지침
seo-title: AEM Forms 작업 영역에 대한 문제 해결 지침
...
# AEM Forms 작업 영역에 대한 문제 해결 지침 {#troubleshooting-guidelines-for-aem-forms-workspace}

이 문서에서는 로깅을 활성화하고 브라우저에서 디버거를 사용하여 AEM Forms 작업 영역을 디버깅하는 방법에 대해 설명합니다. 또한 AEM Forms 작업 영역 및 해결 방법을 사용할 때 발생할 수 있는 몇 가지 일반적인 문제에 대해서도 설명합니다.

## AEM Forms 작업 영역 패키지를 설치할 수 없습니다. {#unable-to-install-aem-forms-workspace-package}

패치를 설치한 후 AEM Forms 작업 영역을 엽니다. 리소스 찾을 수 없음 오류가 발생하면 CRX 패키지 관리자를 열고 패키지를 다시 `adobe-lc-workspace-pkg-<version>.zip` 설치합니다.
...
다양한 수준에서 로그를 생성하여 오류를 최적의 방식으로 해결할 수 있습니다. 예를 들어 복잡한 애플리케이션에서 구성 요소 수준에서 로깅하면 특정 구성 요소를 디버깅 및 해결하는 데 도움이 됩니다.

AEM Forms 작업 영역에서 다음을 수행합니다.

* 특정 구성 요소 파일에 대한 로깅 정보를 얻으려면 URL `/log/<ComponentFile>/<LogLevel>` 에 추가하고 을 누릅니다 `Enter`. 지정된 로그 수준에서 구성 요소 파일에 대한 모든 로깅 정보가 콘솔에 인쇄됩니다.

* 모든 구성 요소 파일의 로깅 정보를 가져오려면 URL `/log/all/trace` 에 추가하고 키를 누릅니다 `Enter`.
...
* **IE에서 디버깅**:IE 파섹https://msdn.microsoft.com/en-us/library/hh772704(v=vs.85).aspx [](https://msdn.microsoft.com/en-us/library/hh772704(v=vs.85).aspx).
...
* **Firefox에서 디버깅**:Firefox에서 스크립트와 스타일을 디버깅하는 데 여러 가지 추가 기능을 사용할 수 있습니다. 예를 들어 Firebug는 이러한 디버깅 유틸리티(https://getfirebug.com)[입니다](https://getfirebug.com).
...
1. 사용자가 데스크탑에서 양식을 열 때 HTML로 렌더링할 수 있도록 하려면 어떻게 해야 합니까?
...
1. 사용자가 양식 애플리케이션에 로그인되어 있습니다. 사용자가 작업 영역에 로그인하려고 하면 사용자에게 작업 영역 권한이 없는 경우 해당 작업 영역이 로드되지 않을 수 있습니다.
