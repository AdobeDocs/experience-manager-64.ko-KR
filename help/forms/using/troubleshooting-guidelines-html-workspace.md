...
# AEM Forms 작업 공간에 대한 문제 해결 지침 {#troubleshooting-guidelines-for-aem-forms-workspace}
...
패치를 설치한 후 AEM Forms 작업 공간을 엽니다. 리소스를 찾을 수 없음 오류가 발생하면 CRX 패키지 관리자를 열고 패키지를 다시 `adobe-lc-workspace-pkg-<version>.zip` 설치합니다.

패키지를 설치하는 동안 오류가 발생하면 다음 단계 `javax.jcr.nodetype.ConstraintViolationException: OakConstraint0025: Authorizable property rep:authorizableId may not be removed`를 수행하십시오.

1. CRX DE lite에 로그인합니다. The default url is `https://[localhost]:[port]/lc/crx/de/index.jsp`
1. 다음 노드를 삭제합니다.
...
* 특정 구성 요소 파일에 대한 로깅 정보를 보려면 URL에 추가하고 키를 누릅니다 `/log/<ComponentFile>/<LogLevel>` `Enter`. 지정된 로그 수준에서 구성 요소 파일에 대한 모든 로깅 정보가 콘솔에 인쇄됩니다.

* 모든 구성 요소 파일의 로깅 정보를 얻으려면 URL `/log/all/trace` 에 추가하고 키를 누릅니다 `Enter`.
