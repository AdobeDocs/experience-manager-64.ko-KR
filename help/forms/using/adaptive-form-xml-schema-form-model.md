...
* 적응형 양식의 작성 모드에서 XSD 구조는 콘텐츠 파인더 탭에 트리로 표시됩니다. XSD 계층의 요소를 적응형 양식으로 드래그하여 추가할 수 있습니다.
* 연결된 스키마를 준수하는 XML을 사용하여 양식을 미리 채울 수 있습니다.
* 제출 시 사용자가 입력한 데이터가 연관된 스키마와 정렬되는 XML로 제출됩니다.
...
>[!NOTE]
>
>XML 스키마에 하나의 루트 요소만 있는지 확인합니다. 둘 이상의 루트 요소가 있는 XML 스키마는 지원되지 않습니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>최소 발생 수를 지정합니다.</p> <p>(반복 가능한 하위 양식(복잡한 유형)</p> </td> 
   <td>요소(복잡한 유형)</td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>스키마 요소를 응용 양식으로 드래그하면 기본 캡션이 다음과 같이 생성됩니다.
>
>* 요소 이름의 첫 번째 문자 대문자
>* 카멜 케이스 경계선에 공백을 삽입합니다.

>
>
예를 들어 `userFirstName` 스키마 요소를 추가하는 경우 적응형 양식에 생성된 캡션은 `User First Name`입니다.

## 응용 양식 구성 요소 {#limit-acceptable-values-for-an-adaptive-form-component}에 사용할 수 있는 값 제한

적응형 양식 구성 요소에 사용할 수 있는 값을 제한하기 위해 XML 스키마 요소에 다음 제한 사항을 추가할 수 있습니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>문자열</p> </td> 
   <td><p>구성 요소에 허용되는 최대 자릿수를 지정합니다. 지정된 자릿수는 0보다 커야 합니다.</p> </td> 
   <td> 
    <ul> 
     <li>숫자 상자</li> 
     <li>숫자 스텝퍼</li> 
...
bindRef</code> 필드는 트리 요소와 스키마의 요소 또는 특성 간의 연결을 보여 줍니다.

>[!NOTE]
>
>속성에 `bindRef` 값에 `@` 기호가 있으므로 요소와 구별할 수 있습니다. 예, `/config/projectDetails/@duration`.
...
`bindRef`은 적응형 양식 구성 요소와 스키마 요소 또는 특성 간의 연결입니다. 출력 XML에서 이 구성 요소나 필드에서 캡처한 값을 사용할 수 있는 `XPath`이 지시됩니다. 미리 입력(미리 채워진) XML에서 필드 값을 미리 채울 때도 `bindRef`이 사용됩니다.
