...
>
>
예를 들어 스키마 요소를 추가하는 경우 적응형 양식에서 생성된 캡션은 입니다 `userFirstName` `User First Name`.
...
Content Finder에서 요소를 두 번 클릭하면 팝업에는 필드 이름과 속성( `bindRef` 이 속성은 트리 요소를 스키마의 요소 또는 특성에 매핑합니다.
...
>[!NOTE]
>
>속성에는 요소와 구별하기 위해 해당 `@` `bindRef`값에 기호가 있습니다. 예, `/config/projectDetails/@duration`.
...
A `bindRef` 는 적응형 양식 구성 요소와 스키마 요소 또는 속성 간의 연결입니다. 출력 XML에서 이 구성 요소나 필드에서 캡처된 값을 사용할 수 있는 위치를 나타냅니다. `XPath` 미리 입력(미리 `bindRef`채워진) XML에서 필드 값을 미리 채울 때도 a가 사용됩니다.
