...
description: AEM Forms JEE 워크플로우는 비즈니스 프로세스를 디자인, 만들기 및 관리하는 도구를 제공합니다. 사용자 데이터에 액세스 및 삭제, 워크플로우 개시자나 참여자가 알려진 경우 프로세스 인스턴스 ID 식별, 사용자 데이터가 기본 변수에 저장되어 있을 때 프로세스 인스턴스 ID 식별, 프로세스 인스턴스 ID를 기반으로 워크플로우 인스턴스에서 사용자 데이터 제거 및 고아 작업 작업에 대해 자세히 알아봅니다.
...
AEM Forms JEE 워크플로우 프로세스 만들기에 대한 자세한 내용은 다음을 참조하십시오 [Workbench 도움말](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/pdf/WorkbenchHelp.pdf).
...
* **이메일을 통해 시작된 프로세스**: 보낸 사람의 이메일 ID는 페이지의 `tb_job_instance` 직접 쿼리할 수 없는 데이터베이스 테이블입니다.

### 워크플로우 개시자 또는 참여자가 알려진 경우 프로세스 인스턴스 ID 식별 {#initiator-participant}

워크플로우 개시자 또는 참여자에 대한 프로세스 인스턴스 ID를 식별하려면 다음 단계를 수행하십시오.

1. AEM Forms 서버 데이터베이스에서 다음 명령을 실행하여 다음 명령에서 워크플로우 개시자 또는 참여자에 대한 주체 ID를 검색합니다. `edcprincipalentity` 데이터베이스 테이블.
...
1. (**워크플로우 개시자의 경우**) 다음 명령을 실행하여 `tb_task` 데이터베이스 테이블.

   ```sql
   select * from tb_task where start_task = 1 and create_user_id= 'initiator_principal_id'
   ```

   쿼리는 지정된 `initiator`_ `principal_id`. 작업은 두 가지 유형으로 구성됩니다.

   * **완료된 작업**: 이러한 작업이 제출되었으며 영숫자 값을 `process_instance_id` 필드. 제출된 작업에 대한 모든 프로세스 인스턴스 ID를 기록하고 단계를 계속합니다.
   * **작업이 시작되었지만 완료되지 않음**: 이러한 작업은 시작되었지만 아직 제출되지 않았습니다. 의 값 `process_instance_id` 이러한 작업의 필드 **0** (영). 이 경우 해당 작업 ID를 기록하고 [고아 작업](#orphan).

1. (**워크플로우 참가자의 경우**) 다음 명령을 실행하여 다음 명령에서 개시자에 대한 프로세스 참여자의 주도자 ID와 연관된 프로세스 인스턴스 ID를 검색합니다. `tb_assignment` 데이터베이스 테이블.
   ...
   이 없는 작업 또는 작업의 경우 `process_instance_id` 0이면 해당 작업 ID를 기록하고 [고아 작업](#orphan).

1. 다음 지침을 따르십시오. [프로세스 인스턴스 ID를 기반으로 워크플로우 인스턴스에서 사용자 데이터 제거](/help/forms/using/forms-workflow-jee-handling-user-data.md#purge) 식별된 프로세스 인스턴스 ID에 대한 사용자 데이터를 삭제하는 섹션을 참조하십시오.
...
1. 다음 데이터베이스 명령을 실행합니다.
   ...
   쿼리에서 `tb_<number>` 지정한 응용 프로그램의 형식( `app_name`) 및 워크플로우( ) `workflow_name`).

   >[!NOTE]
   >
   >의 값 `name` 워크플로우가 애플리케이션 내의 하위 폴더 내에 중첩된 경우 속성을 복잡할 수 있습니다. 워크플로우에 대한 정확한 전체 경로를 지정합니다. 이 경로는 `omd_object_type` 데이터베이스 테이블.

1. 를 검토합니다. `tb_<number>` 테이블 스키마. 표에는 지정된 워크플로우의 사용자 데이터를 저장하는 변수가 포함되어 있습니다. 테이블의 변수는 워크플로우의 변수에 해당합니다.
...
1. 다음 명령을 실행하여 `tb_process_instance` 테이블.

   ```sql
   select long_lived_invocation_id, status from tb_process_instance where id='process_instance_id'
   ```

   쿼리는 지정된 `process_instance_id`.

1. 공개 인스턴스 만들기 `ProcessManager` 클라이언트 ( `com.adobe.idp.workflow.client.ProcessManager`) 사용 `ServiceClientFactory` 올바른 연결 설정을 사용하는 인스턴스입니다.

   자세한 내용은 다음을 위한 Java API 참조 를 참조하십시오 [클래스 ProcessManager](https://helpx.adobe.com/experience-manager/6-4/forms/ProgramLC/javadoc/com/adobe/idp/workflow/client/ProcessManager.html).
...
1. 다음 메서드를 호출하여 워크플로우 인스턴스를 삭제합니다.
   ...
   다음 `purgeProcessInstance` 메서드 가 구성된 경우 AEM Forms 서버 데이터베이스 및 GDS에서 지정된 호출 ID에 대한 모든 데이터를 완전히 삭제합니다.

### 고아 작업 {#orphan}

Orphan 작업은 포함 프로세스가 시작되었지만 아직 제출되지 않은 작업입니다. 이 경우 `process_instance_id` is **0** (영). 따라서 프로세스 인스턴스 ID를 사용하여 고아 작업에 대해 저장된 사용자 데이터는 추적할 수 없습니다. 그러나 고아 작업에 대한 작업 ID를 사용하여 추적할 수 있습니다. 에서 작업 ID를 식별할 수 있습니다 `tb_task` 에 설명된 대로 사용자의 표 [워크플로우 개시자 또는 참여자가 알려진 경우 프로세스 인스턴스 ID 식별](/help/forms/using/forms-workflow-jee-handling-user-data.md#initiator-participant).
...
1. AEM Forms 서버 데이터베이스에서 다음 명령을 실행하여 식별된 작업 ID에 대한 ID를 검색합니다.
   ...
   쿼리는 ID 목록을 반환합니다. 각 ID( `fd_id`)가 결과에 반환되면 다음과 같이 세션 ID 문자열 목록을 만듭니다.
...
1. GDS가 파일 시스템을 가리키는지 아니면 데이터베이스를 가리키는지에 따라 다음 단계 중 하나를 수행합니다.
   ...
   1. **파일 시스템의 GDS**
      ...
      * `_wftaskformid<fd_id>`
      ...
      1. 모든 마커 파일 및 다른 파일 이름을 로 정확히 삭제 `<file_name_guid>` 파일 시스템
   1. **데이터베이스의 GDS**
