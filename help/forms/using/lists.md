...
1. **만들기** > **텍스트를 누릅니다.** 또는 텍스트 에셋을 선택하고 편집을  **누릅니다**.
1. 텍스트에 다음 정보를 지정합니다.
...
1. 텍스트 편집기가 열리고 텍스트를 입력합니다. 페이지 상단의 도구 모음을 사용하여 텍스트 서식 지정, 조건 삽입, 링크 및 페이지 나누기의 서식을 지정합니다.
   ...
   * **링크**:텍스트에 하이퍼텍스트 링크를 삽입합니다. **[!UICONTROL 링크]**&#x200B;를 누르고 **[!UICONTROL URL]** 필드에 링크를 지정하고 **[!UICONTROL 대체 텍스트]** 필드에서 기본 개체 이름을 제거하고 ![저장](assets/save_icon.svg)을 누릅니다.
...
1. 편집 가능한 인라인 모듈에서 텍스트의 일부를 강조\n강조하려면 텍스트를 선택하고 강조 색상을 누릅니다.
   ...
   ![textbackgroupcolor](assets/textbackgroundcolor.png)
...
1. **표** 탭을 누르고 레이아웃에 대해 다음 정보를 지정합니다.
   ...
   * **구성**:구성할 테이블을 선택합니다. 드롭다운에서 테이블 이름에 대한 접미어는 테이블이 정적 테이블이면 (정적), 동적 테이블이면 (동적)입니다. 정적 테이블에는 고정된 개수의 행이 포함되어 있습니다. 정적 테이블에는 대상 영역과 필드가 포함될 수 있습니다. 이러한 대상 영역 및 필드는 반복 DDE에 바인딩할 수 없습니다. 표 셀에 바인딩된 데이터는 동적 표의 행 수를 결정합니다.
   ...
   * **너비 비율:** 표 열 너비의 비율.
   ...
   레이아웃 조각에서 정적 및 동적 표를 사용하는 자세한 예는 [샘플 파일 사용 예를 참조하십시오.문자](create-letter.md#insert-data-modules-and-layout-fragments-in-a-letter-and-configure-them)에서 정적 및 동적 표 사용
