...
title: 제출 작업 구성
seo-title: 제출 작업 구성
description: AEM Forms에서는 제출 작업을 구성하여 제출 후 적응형 양식이 처리되는 방식을 정의할 수 있습니다. 내장된 제출 동작을 사용하거나 직접 작성할 수 있습니다.
seo-description: AEM Forms에서는 제출 작업을 구성하여 제출 후 적응형 양식이 처리되는 방식을 정의할 수 있습니다. 내장된 제출 동작을 사용하거나 직접 작성할 수 있습니다.
...
사용자가 적응형 양식의 제출 단추를 클릭하면 제출 작업이 트리거됩니다. 적응형 양식에 제출 작업을 구성할 수 있습니다. 적응형 양식에서는 즉시 실행 작업을 몇 가지 제공합니다. 기본 제출 작업을 복사하고 확장하여 고유한 제출 작업을 만들 수 있습니다. 그러나 요구 사항에 따라 본인의 실행 동작을 작성하여 등록하면 제출된 양식에서 데이터를 처리할 수 있습니다.

양식이 미리 입력되거나 제출되면 제출된 데이터가 중간 형식으로 데이터를 정렬하기 위해 AEM을 통해 라우팅됩니다. 적응형 양식이 Adobe Sign, verify, forms 포털 초안 또는 제출 또는 AEM 워크플로우를 사용하는 경우를 제외하고는 데이터는 AEM 인스턴스에 저장되지 않습니다.

세로 막대의 응용 양식 컨테이너 속성의 **[!UICONTROL 제출]** 섹션에서 제출 작업을 구성할 수 있습니다.

![제출 ](assets/thank-you-setting.png)
**작업 구성 그림:** *제출 작업 구성*
...
>[!NOTE]
>
>[AEM_Installation_Directory]\crx-quickstart\temp\datamanager\ASM folder exists을 참조하십시오. 첨부 파일을 일시적으로 저장하려면 디렉토리가 필요합니다. 디렉토리가 없는 경우 디렉토리를 만듭니다.
...
**[!UICONTROL REST 끝점에 제출]** 제출 옵션은 HTTP GET 요청의 일부로 양식에 입력된 데이터를 구성된 확인 페이지로 전달합니다. 요청할 필드의 이름을 추가할 수 있습니다. 요청의 형식은 다음과 같습니다.
...
**[!UICONTROL REST 끝점에 제출]** 작업을 사용하여 제출된 데이터를 나머지 URL에 게시합니다. URL은 내부(양식이 렌더링되는 서버) 또는 외부 서버일 수 있습니다.

내부 서버에 데이터를 게시하려면 리소스의 경로를 제공합니다. 데이터는 리소스의 경로에 게시됩니다. 예: /content/restEndPoint. 이러한 게시물 요청의 경우 제출 요청의 인증 정보가 사용됩니다.

외부 서버에 데이터를 게시하려면 URL을 제공합니다. URL의 형식은 https:// 호스트:port/path_to_rest_end_point입니다. POST 요청을 익명으로 처리하도록 경로를 구성해야 합니다.
...
**[!UICONTROL 전자 메일 보내기]** 전송 작업은 양식을 성공적으로 제출할 때 하나 이상의 받는 사람에게 전자 메일을 보냅니다. 생성된 이메일에는 양식 데이터가 사전 정의된 형식으로 포함될 수 있습니다.
...
**[!UICONTROL 전자 메일]**&#x200B;을 통해 PDF 보내기 작업은 양식 데이터가 포함된 PDF가 포함된 이메일을 하나 이상의 수신자에게 보내어 양식을 성공적으로 제출합니다.

**참고:** *이 제출 동작은 기록 문서 템플릿이 있는 XFA 기반 적응형 양식 및 XSD 기반 적응형 양식에 사용할 수 있습니다.*

## 양식 워크플로 {#invoke-a-forms-workflow} 호출

**[!UICONTROL Forms 워크플로우에 제출]** 제출 옵션은 JEE 프로세스에서 기존 Adobe LiveCycle 또는 AEM Forms에 데이터 xml 및 첨부 파일(있는 경우)을 전송합니다.

양식 제출 워크플로우 제출 작업을 구성하는 방법에 대한 자세한 내용은 [양식 워크플로우를 사용하여 양식 데이터 제출 및 처리](/help/forms/using/submit-form-data-livecycle-process.md)를 참조하십시오.

## 양식 데이터 모델을 사용하여 제출 {#submit-using-form-data-model}

**[!UICONTROL 양식 데이터 모델을 사용하여 제출]** 작업을 제출하면 양식 데이터 모델의 지정된 데이터 모델 개체에 대해 제출된 적응형 양식 데이터를 해당 데이터 소스에 기록합니다. 제출 작업을 구성할 때 데이터 소스에 다시 쓸 데이터를 포함하는 데이터 모델 개체를 선택할 수 있습니다.
...
**[!UICONTROL Forms Portal 제출 작업]** 옵션을 사용하면 AEM Forms 포털을 통해 양식 데이터를 사용할 수 있습니다.
...
서버에서 다시 실행되는 적응형 양식의 OTB(즉시 사용 가능한 필드 유효성 검사):
...
![서버측 ](assets/revalidate-on-server.png)
**유효성 검사 활성화 그림:** *서버측 유효성 검사 활성화*
...
때때로 **복잡한 유효성 검사 규칙**&#x200B;의 경우 정확한 유효성 검사 스크립트가 사용자 지정 함수에 존재하며 작성자는 필드 유효성 검사 표현식에서 이러한 사용자 지정 함수를 호출합니다. 양식 작성자는 서버측 유효성 검사를 수행하는 동안 이 사용자 지정 함수 라이브러리를 알리고 사용할 수 있도록 하려면 아래 표시된 대로 응용 양식 컨테이너 속성의 **[!UICONTROL 기본]** 탭 아래에 AEM 클라이언트 라이브러리의 이름을 구성할 수 있습니다.

![유효성 검사 표현식의 사용자 지정 ](assets/clientlib-cat.png)
**함수 지원그림:유효성 검사 표현식에서** *사용자 지정 함수 지원*

작성자는 적응형 양식당 사용자 지정 javascript 라이브러리를 구성할 수 있습니다. 라이브러리에서 재사용할 수 있는 함수만 유지하면 jquery 및 belline.js 타사 라이브러리에 종속됩니다.
