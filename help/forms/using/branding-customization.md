...
description: 애플리케이션 아이콘, 애플리케이션 이름, 시작 이미지 및 로그인 페이지를 사용자 정의하여 AEM Forms 앱에 고유한 룩앤필을 제공할 수 있습니다.
seo-description: 애플리케이션 아이콘, 애플리케이션 이름, 시작 이미지 및 로그인 페이지를 사용자 정의하여 AEM Forms 앱에 고유한 룩앤필을 제공할 수 있습니다.
...
응용 프로그램 아이콘, 응용 프로그램 이름, 시작 이미지 및 로그인 페이지를 사용자 정의하여 AEM Forms 앱에 고유한 조직별 모양을 제공할 수 있습니다. 예를 들어 이미지를 변경하여 조직의 로고를 사용할 수 있습니다. AEM Forms 앱은 다음과 같은 사용자 지정을 지원합니다.
...
기본 앱 아이콘과 AEM Forms 앱의 실행 이미지를 사용자 정의하려면 다음 단계를 수행하십시오.

>[!NOTE]
>
>모든 아이콘 및 이미지에는 비인터레이스 PNG 형식을 사용합니다.
...
1. (***사용자 정의 아이콘***) 캡처의 내비게이터 보기에서 **[!UICONTROL 캡처 > 캡처 > 지원 파일 > Capture-info.plist]**&#x200B;로 이동합니다. 아이콘 파일 옆의 드롭다운을 클릭합니다. 아이콘 파일(.png)의 이름을 지정하고 **[!UICONTROL 캡처 > 캡처 > 리소스 > 아이콘]**&#x200B;에서 파일을 업로드합니다. 현재 지원되는 디멘션은 다음과 같습니다.29x29, 50x50, 58x58, 72x72, 100x100 및 144x144.
1. (***시작 이미지를 사용자 정의하는 경우***) 이미지의 파일 이름이 다음 파일인지 확인합니다.

   * 세로:`Default-Portrait~ipad.png` 및 `Default-Portrait@2x~ipad.png`
   * 가로:`Default-Landscape~ipad.png` 및 `Default-Landscape@2x~ipad.png`

   캡처 프로젝트에 업로드하여 프로젝트의 기존 파일을 바꿉니다.

   >[!NOTE]
   >
   >이미지의 이름과 해상도가 프로젝트에서 대체한 이미지와 일치하는지 확인합니다.

1. iOS 디바이스 또는 iOS 시뮬레이터에서 AEM Forms 앱을 빌드하고 실행합니다.

#### Android {#for-android}의 경우

1. 응용 프로그램 아이콘 파일의 이름을 다음과 같이 지정합니다.
...
## 앱 이름 {#customize-the-app-name} 사용자 정의
...
AEM Forms 앱의 로그인 페이지에는 로고 및 배경 이미지가 있습니다. 로고는 로그인 대화 상자 위에 있으며 배경 이미지는 로그인 대화 상자 아래에 있습니다. 로그인 페이지에서 기본 이미지를 사용자 정의하려면 다음 단계를 수행합니다.
...
### Eclipse {#to-customize-images-on-the-login-pages-using-eclipse}을 사용하여 로그인 페이지의 이미지를 사용자 정의하려면
...
### Visual Studio {#to-customize-images-on-the-login-pages-using-visual-studio}을 사용하여 로그인 페이지의 이미지를 사용자 정의하려면
...
## 앱 메뉴 {#customizing_images_on_the_login_page-1}에서 로고 사용자 정의

AEM Forms 앱에 로그인하고 메뉴 단추를 누르면 메뉴 위에 로고가 표시됩니다. 기본 로고를 사용자 정의하려면 다음 단계를 수행합니다.
...
1. `www/wsmobile/images`폴더로 이동합니다.
1. 로고를 변경하려면 기본 `aem_icon.png` 파일을 사용자 지정 `aem_icon.png` 파일로 바꿉니다.
1. iOS 디바이스 또는 iOS 시뮬레이터에서 AEM Forms 앱을 빌드하고 실행합니다.

### Eclipse {#to-customize-images-on-the-login-pages-using-eclipse-1}을 사용하여 로그인 페이지의 이미지를 사용자 정의하려면
...
### Visual Studio {#to-customize-images-on-the-login-pages-using-visual-studio-1}을 사용하여 로그인 페이지의 이미지를 사용자 정의하려면
