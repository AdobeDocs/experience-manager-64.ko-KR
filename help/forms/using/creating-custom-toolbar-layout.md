...
도구 모음 레이아웃에서는 복잡한 JavaScript 및 CSS 코드를 기반으로 하는 클라이언트측 처리에 상당히 의존합니다. 이 코드의 제공을 구성하고 최적화하는 것은 복잡한 문제가 될 수 있습니다. 이 문제를 해결하기 위해 AEM은 클라이언트측 라이브러리 폴더를 제공하여 클라이언트측 코드를 저장소에 저장하고 카테고리로 구성하고 각 코드 카테고리가 클라이언트에 제공될 시기와 방법을 정의할 수 있도록 합니다. 그런 다음 클라이언트측 라이브러리 시스템은 최종 웹 페이지에 올바른 링크를 만들어 올바른 코드를 로드합니다. 자세한 내용은 [클라이언트측 라이브러리가 AEM에서 작동하는 방식을 참조하십시오.](/help/sites-developing/clientlibs.md)

![도구 모음의 샘플 ](assets/default_toolbar_layout.png)
**레이아웃그림:** *도구 모음의 샘플 레이아웃*

적응형 양식은 즉시 사용 가능한 레이아웃 세트를 제공합니다.

![즉시 사용 가능한 툴바 레이아웃  ](assets/toolbar1.png)
**그림:** *즉시 사용 가능한 툴바 레이아웃*

또한 사용자 정의 도구 모음 레이아웃을 만들 수도 있습니다.

다음 절차에서는 도구 모음에 3개의 작업이 표시되고, 도구 모음의 드롭다운 목록에 다른 작업이 표시되는 사용자 정의 도구 모음을 만드는 절차에 대해 자세히 설명합니다.
...
다운로드데모 사용자 정의 도구 모음 레이아웃 만들기

## 사용자 정의 도구 모음 레이아웃 {#layout-1}을 만들려면
...
1. 사용자 정의 도구 모음 레이아웃을 유지할 폴더를 만듭니다. 예:
   ...
   >[!NOTE]
   >
   >사용자 정의 레이아웃을 유지하기 위해 만드는 폴더는 `apps` 폴더로 많이 만들어집니다.
...
1. 이 사용자 정의 도구 모음 레이아웃을 선택하고 확인을 클릭합니다.
   ...
   >[!NOTE]
   >
   >레이아웃에 대한 guidetoolbar 클래스를 추가합니다. 도구 모음에 대한 기본 스타일 지정은 도구 모음 클래스와 관련하여 정의됩니다.
...
![사용자 정의 레이아웃 도구 모음의 데스크톱 ](assets/toolbar_1.png)
**보기그림:** *사용자 정의 레이아웃 도구 모음의 데스크톱 보기*
