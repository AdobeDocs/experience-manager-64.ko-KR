...
* **[표현식 액세스](/help/forms/using/adaptive-form-expressions.md#main-pars-header-4)**:을 클릭하여 필드를 활성화/비활성화합니다.
* **[표현식](/help/forms/using/adaptive-form-expressions.md#p-calculate-expression-p)**계산:to auto compute value of a field.
* **[표현식을](/help/forms/using/adaptive-form-expressions.md#p-click-expression-p)**클릭합니다.를 클릭하여 단추의 클릭 이벤트에 대한 작업을 처리합니다.
* **[](/help/forms/using/adaptive-form-expressions.md#p-initialization-script-p)초기화 스크립트&#x200B;**:필드의 초기화에 대한 작업을 수행합니다.

* **[옵션 표현식](/help/forms/using/adaptive-form-expressions.md#p-options-expression-p)**:을 사용하여 드롭다운 목록을 동적으로 채웁니다.
* [**요약 표현식&#x200B;**](#summary):를 사용하여 아코디언 제목을 동적으로 계산합니다.
* **[표현식](/help/forms/using/adaptive-form-expressions.md#p-validate-expression-p)**유효성 검사:to validate a field.
* **[](/help/forms/using/adaptive-form-expressions.md#p-value-commit-script-p)값 커밋 스크립트&#x200B;**:필드의 값이 변경된 후 양식의 구성 요소를 변경합니다.

* **[가시성 표현식](/help/forms/using/adaptive-form-expressions.md#p-visibility-expression-p)**:to control visibility of a field and panel.
* **[단계 완료 표현식](/help/forms/using/adaptive-form-expressions.md#p-step-completion-expression-p)**:사용자가 마법사의 다음 단계로 이동하지 못하도록 합니다.
...
**예**:단추의 클릭 동작에 텍스트 상자 **textbox1을** AEM Forms 값으로 채우려면 **단추**&#x200B;클릭 표현식은 `textbox1.value="AEM Forms"` &quot;
...
**예**:다른 필드의 값을 기반으로 필드의 값을 채우려면 간단한 옵션 표현식을 제공합니다. 예를 들어, 다른 필드에 **표시된 혼인** 상태에 따라 **필드를 채우려면** 이표현식은 다음과 같습니다.

**`marital_status.value == "married" ? ["1=One", "2=two"] : ["0=Zero"]`.**

mobilly_status **필드의 값이** 변경될 때마다 표현식이 다시 트리거됩니다. REST 서비스에서 드롭다운을 채울 수도 있습니다. 자세한 내용은 동적으로 [드롭다운을](/help/forms/using/dynamically-populate-dropdowns.md)채우기를 참조하십시오.
...
>[!NOTE]
>
>필드의 값이 프로그래밍 방식으로 변경될 때 값 커밋 스크립트 실행을 비활성화할 수 있습니다. 이렇게 하려면 적응형 양식 `https://[server]:[port]/system/console/configMgr and change` 버전으로 이동하여 AEM Forms **6.1** 과의 호환을 **참조하십시오**. 이후, 값 커밋 스크립트는 사용자가 UI에서 필드 값을 변경할 때에만 실행됩니다.
...
구성 요소를 필수 구성 요소로 만들려면 구성 요소의 **[!UICONTROL 편집]** 대화 상자에서 제목 및 텍스트 > 필수 옵션을 **[!UICONTROL 선택할 수 있습니다]**. 적절한 **필수 메시지** (선택 사항)도 추가할 수 있습니다..

### 유효성 검사 패턴 {#validation-patterns}

필드에 사용할 수 있는 유효성 검사 패턴은 여러 개 있습니다. 검증 패턴을 선택하려면 구성 요소의 **[!UICONTROL 편집]** 대화 상자에서 패턴 **[!UICONTROL 섹션을]** 찾아 **[!UICONTROL 패턴을]**&#x200B;선택합니다. [패턴] 텍스트 상자에서 사용자 정의 유효성 검사 패턴을 만들 **수** 있습니다. 유효성 검사 상태는 채워진 **데이터가** 유효성 검사 패턴을 준수하면 True **가** 반환됩니다. 사용자 정의 유효성 검사 패턴을 작성하려면 HTML5 양식에 [대한 그림 절 지원을 참조하십시오](/help/forms/using/picture-clause-support.md).

### 유효성 검사 표현식 {#validation-expressions}

필드의 유효성 검사는 다른 필드의 표현식을 사용하여 계산할 수도 있습니다. 이러한 표현식은 구성 요소의 **[!UICONTROL 편집]** 대화 **[!UICONTROL 상자의 스크립트]** 탭에 있는 **[!UICONTROL 유효성]** 검사 스크립트 필드 내에작성됩니다. 필드의 유효성 검사 상태는 표현식이 반환하는 값에 따라 달라집니다. 이러한 표현식을 작성하는 방법에 대한 자세한 내용은 표현식 유효성 검사를 [참조하십시오](/help/forms/using/adaptive-form-expressions.md#p-validate-expression-p).

## 추가 정보 {#additional-information}

### 필드 표시 형식 사용 {#using-field-display-format}

표시 형식을 사용하여 데이터를 다른 형식으로 표시할 수 있습니다. 예를 들어 디스플레이 형식을 사용하여 하이픈, 우편 번호 형식 지정 또는 날짜 선택기와 함께 전화 번호를 표시할 수 있습니다. 이러한 표시 패턴은 구성 요소의 **[!UICONTROL 편집]** 대화 **[!UICONTROL 상자의 패턴 섹션에서]** 선택할 수 있습니다. 위에 언급된 유효성 검사 패턴과 유사한 사용자 정의 표시 패턴을 작성할 수 있습니다.
