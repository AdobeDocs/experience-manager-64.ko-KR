...
JavaScript는 적응형 양식의 표현식 언어입니다. 모든 표현식은 유효한 JavaScript 표현식이며 적응형 양식 스크립팅 모델 API를 사용합니다. 이러한 표현식은 특정 유형의 값을 반환합니다. 응용 양식 클래스, 이벤트, 개체 및 공용 API의 전체 목록은 응용 양식에 대한 [JavaScript 라이브러리 API 참조를 참조하십시오](https://helpx.adobe.com/aem-forms/6/javascript-api/index.html).
...
* **[표현식을 클릭합니다](/help/forms/using/adaptive-form-expressions.md#p-click-expression-p)**. 를 클릭하여 단추의 클릭 이벤트에 대한 작업을 처리합니다.
* **[초기화 스크립트](/help/forms/using/adaptive-form-expressions.md#p-initialization-script-p):**필드의 초기화 작업을 수행합니다.
...
* **[표현식 유효성 검사](/help/forms/using/adaptive-form-expressions.md#p-validate-expression-p)**: to validate a field.
* **[값 커밋 스크립트](/help/forms/using/adaptive-form-expressions.md#p-value-commit-script-p):**필드 값이 변경된 후에 양식의 구성 요소를 변경합니다.

* **[가시성 식](/help/forms/using/adaptive-form-expressions.md#p-visibility-expression-p)**: to control visibility of a field and panel.
* **[단계 완료 표현식](/help/forms/using/adaptive-form-expressions.md#p-step-completion-expression-p)**: 사용자가 마법사의 다음 단계로 이동하지 않도록 하려면
...
**예**: 다른 필드의 값을 기반으로 필드의 값을 채우려면 간단한 옵션 표현식을 제공합니다. 예를 들어, 다른 필드에 **표시된 혼인** 상태를 기반으로 한 필드 **의** 자녀의수를 채우려면 다음과 같습니다.

**`marital_status.value == "married" ? ["1=One", "2=two"] : ["0=Zero"]`.**

mobilly_status **** 필드의 값이 변경될 때마다 표현식이 다시 트리거됩니다. REST 서비스에서 드롭다운을 채울 수도 있습니다. 자세한 내용은 동적으로 드롭다운을 [채우기를 참조하십시오](/help/forms/using/dynamically-populate-dropdowns.md).
...
한 필드에 사용할 수 있는 여러 가지 유효성 검사 패턴이 있습니다. 검증 패턴을 선택하려면 구성 요소의 **[!UICONTROL 편집]** 대화 상자에서 **[!UICONTROL 패턴]** 섹션을 찾아 **[!UICONTROL 패턴을]**&#x200B;선택합니다. 패턴(Pattern) 텍스트 상자에서 사용자 정의 유효성 검사 패턴을 직접 만들 **수** 있습니다. 채워진 데이터가 유효성 검사 패턴을 **준수하면** 유효성 검사 상태가 **True로** 반환되고 그렇지 않으면False가반환됩니다. 사용자 지정 유효성 검사 패턴을 작성하려면 HTML5 양식에 [대한 그림 절 지원을 참조하십시오](/help/forms/using/picture-clause-support.md).

### 유효성 검사 표현식 {#validation-expressions}

필드의 유효성 검사는 다른 필드에 있는 표현식을 사용하여 계산할 수도 있습니다. 이러한 표현식은 구성 요소의 **[!UICONTROL 편집 대화 상자]** 의 **[!UICONTROL 스크립트]** 탭 **[!UICONTROL 의 유효성 검사 스크립트]** 필드내에작성됩니다. 필드의 유효성 검사 상태는 표현식이 반환하는 값에 따라 달라집니다. 이러한 표현식을 쓰는 방법에 대한 자세한 내용은 표현식 유효성 [검사를 참조하십시오](/help/forms/using/adaptive-form-expressions.md#p-validate-expression-p).

## 추가 정보 {#additional-information}

### 필드 표시 형식 사용 {#using-field-display-format}

표시 형식을 사용하여 데이터를 다른 형식으로 표시할 수 있습니다. 예를 들어 디스플레이 형식을 사용하여 하이픈, ZIP 코드 형식 지정 또는 날짜 선택기와 함께 전화 번호를 표시할 수 있습니다. 이러한 표시 패턴은 구성 요소의 **[!UICONTROL 편집]** 대화 상자 **[!UICONTROL 의 패턴]** 섹션에서 선택할수 있습니다. 위에 언급된 유효성 검사 패턴과 유사한 사용자 지정 표시 패턴을 작성할 수 있습니다.
