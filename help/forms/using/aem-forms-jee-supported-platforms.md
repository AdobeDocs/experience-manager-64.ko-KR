...
>[!NOTE]
>
>* 지원되는 서버 플랫폼에 대한 전체 예외 목록은 지원되는 서버 [플랫폼에](#exceptions-to-supported-server-platforms)대한 예외를 참조하십시오.
...
* JEE의 AEM Forms는 Oracle 멀티 테넌트 아키텍처를 지원합니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>Oracle</td> 
   <td><p>Oracle Database 12.1.0.2.0 JDBC 드라이버</p> <p>ojdbc7.jar(버전 12.1.0.2.0)<br /> </p> </td> 
   <td><p>JEE 설치 시 AEM Forms와 함께 제공됩니다.</p> </td> 
  </tr> 
  <tr> 
   <td>IBM DB2</td> 
   <td><p>IBM® DB2 Universal JDBC 드라이버 4.16.53 (db2jcc4.jar)</p> </td> 
   <td><p>IBM 웹 사이트에서 드라이버 <a href="https://www-01.ibm.com/support/docview.wss?uid=swg21363866" target="_blank">다운로드</a></p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><p>JBoss® 엔터프라이즈 애플리케이션 플랫폼(EAP) 7.0.6 <sup>[1] [4] [5] [7] [8] [11]</sup></p> </td> 
   <td><p>A:지원됨</p> </td> 
...
>[!NOTE]
>
>JEE의 AEM Forms는 64비트 운영 체제만 지원합니다.

#### 가상화 환경 {#virtualized-environment}

물리적 시스템 또는 가상 환경에서 JEE에서 AEM Forms를 실행할 수 있습니다. 그러나 가상 환경에서 AEM Forms에 문제가 발생하면 물리적 컴퓨터에서 해당 문제를 복제해 보십시오. 실제 컴퓨터에서 문제가 지속되는 경우 Adobe 지원 팀에 해결 방법을 문의하십시오. 물리적 시스템에서 복제되지 않는 문제는 가상 환경 공급업체에 문의하십시오.
...
1. JEE의 AEM Forms는 IBM® AIX®에서 Oracle WebLogic 및 JBoss®를 지원하지 않습니다.
1. JEE의 AEM Forms는 MySQL과 함께 Oracle WebLogic 및 IBM® WebSphere®를 지원하지 않습니다.
1. JEE의 AEM Forms는 Intel® 아키텍처에서 Oracle Solaris™를 지원하지 않습니다(SPARC®만 지원).
1. JEE의 AEM Forms는 SUSE Linux Enterprise Server 12에서 Oracle WebLogic 및 JBoss를 지원하지 않습니다. SUSE Linux Enterprise Server 12에서는 IBM WebSphere만 지원됩니다.
...
1. JEE의 AEM Forms는 JBoss®와 함께 IBM® DB2를 지원하지 않습니다.
...
* JEE의 AEM Forms는 서드 파티 소프트웨어 지원 정책에 따라 다양한 서드 파티 [소프트웨어를 지원합니다](#third-party-patch-support-policy).
* JEE의 AEM Forms는 타사 공급업체의 지원에 따라 플랫폼을 지원합니다. 일부 조합은 타사 공급업체에서 허용하지 않을 수 있습니다. 예를 들어, 많은 벤더가 IBM® DB2를 사용하여 애플리케이션 서버를 인증하지 않았습니다. 따라서 JEE의 AEM Forms도 이러한 조합을 지원하지 않습니다. 지원되는 소프트웨어 버전을 선택하려면 타사 공급업체의 지원 매트릭스를 확인하십시오.
...
* 패키지 JDBC 모듈이 Weblogic에서 구성된 경우 RDBMK는 DB2, MYSQL, MS SQL 및 Oracle 데이터베이스와 함께 작동하지 않습니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>버전 8.5<strong> 지원 </strong>여부</p> </td> 
  </tr> 
  <tr> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>Sun Solaris</td> 
   <td>UltraSPARC® IIIi, 1.5GHz 프로세서<br /> Solaris 컨테이너(영역) 분할<br /> RAM:6GB(64비트 JVM이 있는 64비트 OS)<br /> 디스크 여유 공간:6GB의 임시 공간 및 JEE에서 AEM<br /> Forms를 위한 22GB</td> 
  </tr> 
  <tr> 
   <td>IBM AIX</td> 
   <td>P6 pSeries 520 (Model 52A) 9131-52A, 1.8GHz 프로세서<br /> LPAR 파티션<br /> RAM:6GB(64비트 JVM이 있는 64비트 OS)<br /> 디스크 여유 공간:6GB의 임시 공간 및 JEE에서 AEM<br /> Forms를 위한 22GB</td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 
    <ul> 
     ...
     <li><strong></strong> Sun SPARC 기반 환경:UltraSPARC V 이상</li> 
     <li><strong></strong> IBM AIX 기반 환경:Power6 이상<br /> </li> 
...
* [JEE 배포 시 클러스터된 AEM Forms에 대한 시스템 요구 사항](https://www.adobe.com/go/learn_aemforms_sysreq_cluster_64)
...
* 설치를 위한 디스크 공간:워크벤치의 경우 1.7GB, Workbench, Designer 및 임시 설치 디렉토리의 경우 단일 드라이브의 경우 2.7GB, 임시 설치 디렉토리의 경우 샘플 어셈블리 400MB - 사용자 임시 디렉토리의 경우 200MB, Windows 임시 디렉토리의 경우 200MB

>[!NOTE]
>
>이러한 위치가 모두 단일 드라이브에 있는 경우 설치 중에 1.5GB의 사용 가능한 공간이 있어야 합니다. 설치가 완료되면 임시 디렉토리에 복사된 파일이 삭제됩니다.
...
| Google Android | Android 4.4(Andoird Kit Kat) 및 API *[Level 19 이상]*. AEM Forms 앱은 7~10인치 삼성 갤럭시 태블릿과 7인치 Google Nexus 태블릿 및 인기 스마트폰에서 인증되었습니다. |
...
또한 JEE의 AEM Forms는 지정된 주요 버전 및 보조 버전의 지원되는 소프트웨어 위에 있는 업데이트, 패치 및 수정 팩을 지원합니다. 하지만 다음 주 또는 부 버전으로 업데이트할 수 있는 경우는 구체적으로 명시되지 않습니다.

## 타사 패치 지원 정책 {#third-party-patch-support-policy}

JEE에서 AEM Forms에 대한 타사 소프트웨어 요구 사항은 해당 제품 문서의 &quot;시스템 요구 사항&quot; 섹션에 설명되어 있습니다. 모든 설명서는 https://adobe.com/go/learn_aemforms_documentation_64에서 액세스할 수 [있습니다](https://adobe.com/go/learn_aemforms_documentation_64) .
