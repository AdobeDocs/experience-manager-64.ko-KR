...
* XFA 양식 템플릿을 [사용하는 적응형 양식의](/help/forms/using/prepopulate-adaptive-form-fields.md)경우 XFA 템플릿의 데이터 스키마를 준수하는 프리필 XML을 사용합니다.
* XML 스키마를 사용하는 적응형 [양식의](/help/forms/using/prepopulate-adaptive-form-fields.md#main-pars-header-3)경우 XML 스키마 구조를 준수하는 프리필 XML을 사용합니다.
* JSON 스키마를 사용하는 적응형 [양식의](/help/forms/using/prepopulate-adaptive-form-fields.md#json-schema-based-adaptive-forms)경우 JSON 스키마를 준수하는 프리필 JSON을 사용하십시오.
* FDM 스키마를 사용하는 적응형 양식의 경우 FDM 스키마와 호환되는 프리필 JSON을 사용합니다.
* 양식 모델이 [없는 적응형 양식의](/help/forms/using/prepopulate-adaptive-form-fields.md#p-adaptive-form-with-no-form-model-p)경우 바인딩된 데이터가 없습니다. 모든 필드는 언바운드 필드이며 언바운드 XML을 사용하여 프리필됩니다.
...
JSON 스키마를 기반으로 하는 적응형 양식의 경우, JSON과 제출된 JSON의 구조가 아래에 설명되어 있습니다. 자세한 내용은 JSON 스키마를 [사용하여 적응형 양식 만들기를 참조하십시오](/help/forms/using/adaptive-form-json-schema-form-model.md).
...
>[!NOTE]
>
>바인딩되지 않은 필드(사이드 킥이나 데이터 소스 탭에서 구성 요소를 드래그하여 만든 비어 있지 않은 bindRef가 있는 패널)를 사용하면 바인딩되지 않은 필드의 데이터가 손실될 수 있으므로 사용하지 **않는** 것이 좋습니다. 특히 언바운드 필드의 경우 양식에 고유한 필드 이름을 지정하는 것이 좋습니다.
...
1. 데이터 파일 위치에 **[!UICONTROL 대한 데이터 위치 또는 정규식(정규식)을]**&#x200B;입력합니다. 유효한 데이터 파일 위치의 예는 다음과 같습니다.
...
* SERVICE_NAME은 OSGI 미리 채우기 서비스의 이름을 나타냅니다. 미리 [채우기 서비스](/help/forms/using/prepopulate-adaptive-form-fields.md#create-and-run-a-prefill-service)만들기 및 실행을 참조하십시오.
...
* [AEM Forms 클라이언트 SDK 다운로드](https://helpx.adobe.com/aem-forms/kb/aem-forms-releases.html)
...
1. 속성 콘솔에서 AEM Forms 컨테이너 > **[!UICONTROL 기본 > 미리 채우기 서비스로 이동합니다]**.
