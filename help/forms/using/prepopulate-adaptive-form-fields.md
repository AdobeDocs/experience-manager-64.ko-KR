...
* XFA [양식 템플릿을 사용하는 적응형 양식의](/help/forms/using/prepopulate-adaptive-form-fields.md)경우 XFA 템플릿의 데이터 스키마를 준수하는 자동 완성 XML을 사용합니다.
* XML 스키마를 사용하는 적응형 양식의 경우 [XML 스키마](#xml-schema-af)구조에 맞는 자동 완성 XML을 사용합니다.
* JSON 스키마를 사용하는 적응형 양식의 [경우](/help/forms/using/prepopulate-adaptive-form-fields.md#json-schema-based-adaptive-forms)JSON 스키마를 준수하는 프리필 JSON을 사용하십시오.
* FDM 스키마를 사용하는 적응형 양식의 경우 FDM 스키마와 호환되는 프리플 JSON을 사용하십시오.
* 양식 모델이 [없는 적응형 양식의](/help/forms/using/prepopulate-adaptive-form-fields.md#p-adaptive-form-with-no-form-model-p)경우 바인딩된 데이터가 없습니다. 모든 필드는 제본되지 않은 필드이며 제본되지 않은 XML을 사용하여 미리 입력됩니다.

### XML 구조 샘플 {#sample-prefill-xml-structure}
...
[데이터](assets/prefill-submit-data-contentpackage.zip)와 제출된 데이터가 포함된 파일 샘플 가져오기
...
>[!NOTE]
>
>바인딩 패널(사이드 킥이나 데이터 소스 탭에서 구성 요소를 드래그하여 만든 비어 있지 않은 패널 `bindRef` )에 언바운드 필드를 사용하지 않는 것이 좋습니다. 제본되지 않은 필드의 데이터가 손실될 수 있습니다. 또한 필드 이름은 제본되지 않은 필드 전용으로 양식 전체에서 고유하도록 권장합니다.
...
JSON 스키마를 기반으로 하는 적응형 양식의 경우 자동 완성 JSON 및 제출된 JSON의 구조가 아래에 설명되어 있습니다. 자세한 내용은 JSON 스키마를 [사용하여 적응형 양식 만들기를 참조하십시오](/help/forms/using/adaptive-form-json-schema-form-model.md).
...
JSON 스키마 모델을 사용하는 필드의 경우 아래 샘플 JSON에 표시된 대로 데이터는 afBoundData 개체에서 프리플로 입력됩니다. 하나 이상의 언바운드 텍스트 필드를 사용하여 응용 양식을 미리 입력하는 데 사용할 수 있습니다. 다음은 `afData/afBoundData` 래퍼와 관련된 데이터의 예입니다.
...
다음은 `afData/afBoundData` 래퍼 없는 예입니다.
...
>[!NOTE]
>
>바인딩 해제된 필드(사이드 킥이나 데이터 소스 탭에서 구성 요소를 드래그하여 만든 빈 바인딩 해제된 Ref가 있는 패널)를 사용하면 필드의 데이터가 손실될 수 있으므로 사용하지 **않는** 것이 좋습니다. 특히 제본되지 않은 필드에 대해 양식에 고유한 필드 이름이 있는 것이 좋습니다.
...
1. 데이터 파일 위치에 대한 데이터 위치 또는 **[!UICONTROL 정규식(정규식)을 입력합니다]**. 유효한 데이터 파일 위치의 예는 다음과 같습니다.
...
지정한 노드에는 호출된 속성 `jcr:data` 이 있어야 하며 데이터를 보유해야 합니다.
...
* SERVICE_NAME은 OSGI 미리 채우기 서비스의 이름을 나타냅니다. 자동 완성 [서비스 만들기를 참조하십시오](/help/forms/using/prepopulate-adaptive-form-fields.md#create-and-run-a-prefill-service).
...
아래 샘플 코드와 같이 속성 `data` 이 `slingRequest``data` XML 또는 JSON을 포함하는 문자열인 XML에 속성을 설정할 수도 있습니다(예: XML).
...
예를 들어 특정 유형의 헤더가 있는 페이지의 특정 디자인을 원하는 경우 이를 위해 페이지 구성 요소에 포함할 수 `header.jsp`있는 사용자 고유의 속성을 작성할 수 `data` 있습니다.

다른 좋은 예는 Facebook, Twitter 또는 LinkedIn과 같은 소셜 계정을 통해 로그인 시 데이터를 미리 입력하려는 사용 사례입니다. 이 경우 사용자 계정에서 데이터를 가져오고 데이터 매개 변수를 설정하는 간단한 JSP를 포함할 수 `header.jsp`있습니다.
...
1. 상용구 프로젝트의 `AEM Forms Client SDK` 빌드 경로에 패키지를 추가합니다.
