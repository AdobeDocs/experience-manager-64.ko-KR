...
description: 테마를 사용하여 적응형 양식 또는 인터랙티브한 커뮤니케이션에 스타일을 지정하고 시각적 ID를 제공할 수 있습니다. 다양한 적응형 양식 또는 인터랙티브한 커뮤니케이션에서 테마를 공유할 수 있습니다.
seo-description: 테마를 사용하여 적응형 양식 또는 인터랙티브한 커뮤니케이션에 스타일을 지정하고 시각적 ID를 제공할 수 있습니다. 다양한 적응형 양식 또는 인터랙티브한 커뮤니케이션에서 테마를 공유할 수 있습니다.
...
1. Adobe Experience **[!UICONTROL Manager를]**&#x200B;클릭하고 **[!UICONTROL 양식을]**&#x200B;클릭한 다음 테마를 **[!UICONTROL 클릭합니다]**.

1. 테마 페이지에서 만들기 > **[!UICONTROL 테마를 클릭합니다]**.

   테마 만들기 마법사가 실행됩니다.

1. 테마 만들기 마법사의 기본 탭에서 **[!UICONTROL 테마의 제목]** 및 **[!UICONTROL 이름을]** 제공합니다. 필수 필드입니다.
...
1. 만들기를 **[!UICONTROL 클릭한]** 다음 **[!UICONTROL 편집을 클릭하여]** 테마 편집기에서 테마를 열거나 **[!UICONTROL 완료를 클릭하여]** 테마 페이지로 돌아갑니다.
...
1. 테마 페이지에서 테마를 선택하고 다운로드를 **[!UICONTROL 클릭합니다]**. 테마의 세부 사항이 있는 대화 상자가 표시됩니다.

1. 다운로드를 **[!UICONTROL 클릭합니다]**. 테마가 zip 파일로 다운로드됩니다.
...
1. 파일 업로드 프롬프트에서 컴퓨터에서 테마 패키지를 찾아 선택하고 업로드를 **[!UICONTROL 클릭합니다]**.
  ...
  <tr> 
   ...
   <td> 
    <ul> 
     ...
     <li>위치가 존재하지 않으면 폴더 계층 구조가 자동으로 생성됩니다.</li> 
     <li>이 값이 변경되면 clientlib 노드 구조가 입력한 새 위치로 이동됩니다.<br /><em> 참고 <strong></strong>사항:기본 clientlib 위치를 변경하는 경우, CRXDE 보관소에서 <code>crx:replicate, rep:write, rep:glob:*, rep:itemNames:: js.txt, jcr:read </code>새 <code>forms-users</code> 위치에 및 <code>crx:replicate</code>에 <code>jcr:read </code>을 지정합니다 <code>fd-service</code> . 또한 <span class="kbd"></span> 거부 jcr:addChildNodes를 <code>forms-user</code></em></li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td>13.</td> 
   ...
   <td> 
    <ul> 
     ...
     <li>이름이 이미 다른 기존 테마에서 사용 중인 경우 오류가 표시됩니다.</li> 
     <li>기본값 - 테마 위치를 사용하여 계산됩니다.</li> 
     <li>이 값이 변경되면 해당 clientlib 노드에서 카테고리 이름이 업데이트됩니다. clientlib 카테고리 이름이 참조에 사용되므로 jsp 파일에서 Clientlib 카테고리 이름을 업데이트할 필요가 없습니다.</li> 
    </ul> </td> 
  </tr> 
 </tbody> 
</table>
...
* **캔버스** - 오른쪽에 나타납니다. 모든 스타일 변경 사항이 즉시 반영되는 적응형 양식 또는 인터랙티브한 커뮤니케이션의 예를 보여줍니다. 캔버스에서 직접 개체를 선택하여 개체와 관련된 스타일을 조회하고 이러한 스타일을 편집할 수도 있습니다. 맨 위의 장치 해상도 눈금자가 캔버스를 제어합니다. 눈금자에서 해상도 중단점을 선택하면 샘플 양식의 미리 보기 또는 각 해상도에 대한 대화형 통신이 표시됩니다. 캔버스는 [아래에](/help/forms/using/themes.md#using-canvas)자세히 설명되어 있습니다.
...
* **사이드바**- 왼쪽에 나타납니다. 다음과 같은 항목이 있습니다.
   ...
   * **CSS 보기**:선택한 구성 요소의 CSS를 볼 수 있습니다.
   또한 세로 막대의 맨 아래에 화살표가 있습니다. 화살표를 클릭하면 두 가지 옵션이 더 제공됩니다.성공을 **시뮬레이션하고** 오류를 **시뮬레이션합니다.** 이러한 옵션은 위에서 설명한 옵션과 함께 [아래에](/help/forms/using/themes.md#using-rail)자세히 설명되어 있습니다.

   [ 레일 ![및 캔버스가 강조 표시된 테마 편집기](assets/themes.png)](assets/themes-1.png)**** A. **A.사이드바** B.캔버스
...
![세로 막대의 스타일 지정 가능한 구성 요소](assets/stylable-components.png)
...
세로 **막대에서 [모두 확장** ]을 클릭하여 앞에 보이지 않는 분류된 구성 요소를 보고 선택하고 스타일을 지정합니다.
...
테마 편집기에서 사용자 정의 레이아웃 스타일 지정에는 다음이 포함됩니다.
...
이 예에서는 필드 레이블의 스타일이 변경되고, 스타일을 위해 [반응형 패널 설명]을 선택하면 자산 라이브러리에 목록 항목이 추가됩니다. 자산 라이브러리의 항목을 사용하여 반응형 패널 설명의 스타일을 변경할 수 있습니다.

에셋 라이브러리에 스타일이 추가되면 다른 테마와 양식 편집기 또는 대화형 통신 편집기 UI의 [스타일 모드에서](/help/forms/using/inline-style-adaptive-forms.md) 스타일을 사용할 수 있습니다. 마찬가지로 양식 편집기 또는 대화형 통신 편집기 UI의 스타일 모드를 사용하여 구성 요소의 스타일을 지정하면 스타일이 캐시되고 테마에서 사용할 수 있습니다.

자산 라이브러리에 대한 더하기 단추를 사용하면 입력한 이름으로 스타일을 영구적으로 저장할 수 있습니다. 세로 막대에서 [저장] 단추를 클릭하지 않아도 더하기 단추를 누르면 스타일이 저장됩니다. 스타일 모드에서 나중에 사용하기 위해 스타일을 저장하는 더하기 단추를 사용할 수 없습니다.

![자산 라이브러리에 사용자 정의 스타일 이름 제공](assets/custom-style-name.png)

스타일에 대한 사용자 정의 이름을 제공하면 스타일이 테마에 연결되어 있으므로 더 이상 다른 테마에 사용할 수 없습니다. 저장된 스타일을 삭제하려면

1. CANVAS 도구 모음에서 테마 옵션 **** 테마 옵션 ![> 스타일](assets/theme-options.png) 관리를 **[!UICONTROL 클릭합니다]**.
...
스타일링에서 변경한 내용은 Canvas에 로드된 양식이나 대화형 통신에 즉시 반영됩니다. 라이브 미리 보기를 통해 인터랙티브하게 정의하고 스타일이 미치는 영향을 확인할 수 있습니다. 구성 요소의 스타일링을 변경하면 세로 막대에서 **[!UICONTROL [완료]** ] 단추가 활성화됩니다. 변경 내용을 유지하려면 [완료] **[!UICONTROL 단추를]** 사용합니다.
...
### 다른 적응형 양식 또는 인터랙티브한 커뮤니케이션을 사용한 테마 {#theme-with-another-adaptive-form-or-interactive-communication}
...
1. 테마 편집기 패널에서 테마 옵션 **[!UICONTROL 테마 옵션]** ![> 구성을](assets/theme-options.png) 클릭합니다 ****.

1. 일반 탭에서 적응형 양식/문서 필드에 대한 양식이나 대화형 통신을 **[!UICONTROL 찾아 선택합니다]** .

### 다시 실행/실행 취소 {#redo-undo}

실수로 발생하는 원하지 않는 변경 사항을 실행 취소하거나 다시 실행할 수 있습니다. 캔버스에서 다시 실행/실행 취소 단추를 사용합니다.

![재실행 취소](assets/redo-undo.png)
...
테마 편집기를 사용하면 만들거나 업로드한 테마를 편집할 수 있습니다. [양식 및 **[!UICONTROL 문서] > [테마]**]로 이동하여 테마를 선택하고 엽니다. 테마가 테마 편집기에서 열립니다.
...
* **[!UICONTROL 사이드 패널]** 전환 ![사이드 패널](assets/toggle-side-panel.png):세로 막대를 표시하거나 숨길 수 있습니다.
...
* **[!UICONTROL 에뮬레이터]** ![눈금자](assets/ruler.png):다양한 표시 크기에 따라 테마 모양을 에뮬레이션합니다. 표시 크기는 에뮬레이터에서 중단점으로 처리됩니다. 중단점을 선택하고 스타일을 지정할 수 있습니다. 예를 들어, 데스크톱 및 태블릿은 두 개의 중단점입니다. 각 중단점에 대해 다른 스타일을 지정할 수 있습니다.
...
* **[!UICONTROL 숫자 상자 위젯]**:세로 막대의 모양을 사용자 정의할 구성 요소를 선택할 수 있습니다.
* **[!UICONTROL 필드 위젯]**:스타일 지정을 위한 일반 구성 요소를 선택할 수 있습니다. 이 예에서는 모든 텍스트 입력 구성 요소(텍스트 상자/숫자 상자/숫자 스테퍼/날짜 입력)가 스타일링을 위해 선택됩니다.

* ![필드 수준](assets/field-level.png):스타일 지정을 위해 일반 구성 요소로 전환할 수 있습니다. 숫자 상자를 선택하고 이 아이콘을 탭하면 필드 구성 요소가 선택됩니다. 필드 구성 요소를 선택하고 이 아이콘을 탭하면 패널이 선택됩니다. 선택을 위해 이 아이콘을 계속 탭하면 스타일링할 레이아웃을 선택합니다.
...
캔버스에서 구성 요소를 선택하거나 세로 막대의 선택기를 선택하면 세로 막대가 스타일을 사용자 정의할 수 있는 모든 옵션을 표시합니다.\
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><p>치수 및 위치</p> </td> 
   <td><p>테마에 구성 요소의 정렬, 크기, 배치 및 배치 스타일을 지정할 수 있습니다. </p> <p>표시 설정, 패딩, 여백, 폭, 높이 및 Z 색인이 표시됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td><p>텍스트</p> </td> 
   <td><p>테마 구성 요소에서 텍스트 스타일을 사용자 정의할 수 있습니다.</p> <p>예를 들어 텍스트 상자에 입력한 텍스트가 표시되는 방식을 변경할 수 있습니다.</p> <p>글꼴 모음, 두께, 색상, 크기, 선 높이, 텍스트 정렬, 레터간격, 텍스트 들여쓰기, 밑줄, 기울임체, 텍스트 변형, 세로 정렬, 기준선 및 방향도 선택할 수 있습니다. </p> </td> 
  </tr> 
  <tr> 
   <td><p>배경 </p> </td> 
  ...
  <tr> 
   ...
   <td><p>다음을 추가할 수 있습니다.</p> 
    <ul> 
     <li>선택기에서 기본 컨텐츠 앞 또는 뒤에 컨텐츠를 추가할 <code>::before</code> 수 있는 및 <code>::after</code> 유사 요소의 속성을 확인하고 스타일을 지정합니다.<br /> CSS <a href="https://www.w3schools.com/css/css_pseudo_elements.asp" target="_blank">의사 요소를</a>참조하십시오.</li> 
     <li>구성 요소에 대한 사용자 정의 CSS 코드 인라인을 만들고 사용자 정의 선택기를 작성합니다. </li> 
    </ul> <p>사용자 정의 CSS 코드를 추가하면 세로 막대의 옵션을 사용하여 추가한 사용자 정의 코드를 무시합니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
세로 막대의 아래쪽에서 [오류 및 성공 시뮬레이션] 옵션을 사용할 수 있습니다. 세로 막대의 아래쪽에 표시되는 표시/숨기기 화살표를 사용하여 볼 수 있습니다. 테마 편집기를 사용하여 구성 요소의 다양한 상태에 스타일을 지정할 수 있습니다.
...
1. 세로 막대 제목이 숫자 상자 위젯으로 변경되고 모양을 사용자 정의하는 옵션을 표시합니다.

   세로 **[!UICONTROL 막대의 [크기]** 및 위치] 옵션을 사용하여 구성 요소의 크기를 사용자 정의합니다. 상태가 기본값인지 **[!UICONTROL 확인합니다]**.

숫자 상자 **[!UICONTROL 위젯을 선택하는]**&#x200B;대신 구성 요소 **[!UICONTROL 도구 모음에서 필드 위젯을]** 선택하고 위의 단계를 수행합니다. [필드 위젯] **[!UICONTROL 옵션의 크기를]** 선택하면 숫자 상자를 제외한 모든 텍스트 상자의 크기가 동일합니다.
...
1. 세로 막대의 테두리, **[!UICONTROL 배경]** 등과 같은 옵션을 사용하여 구성 요소의 모양을 사용자 정의합니다.
1. 사이드바 **[!UICONTROL 아래쪽에 있는]** [오류 시뮬레이션] 옵션을 사용하여 편집할 때 스타일링 모양을 확인합니다.
...
오류 및 성공 이외의 상태 동작을 시뮬레이션하려면 미리 보기 모드를 사용합니다. 미리 보기 모드를 사용하려면 페이지 **[!UICONTROL 도구 모음에서]** 미리 보기를 클릭합니다.

### 더 작은 디스플레이를 위한 레이아웃 스타일 지정 {#styling-layouts-for-smaller-displays}

캔버스의 눈금자를 사용하여 작은 디스플레이가 있는 장치의 중단점을 선택합니다. 눈금자와 중단점을 보려면 캔버스에서 에뮬레이터 ![눈금자를](assets/ruler.png) 클릭합니다. 중단점을 사용하면 휴대폰 및 태블릿과 같은 다양한 장치에 대한 표시 크기에 대한 양식이나 대화형 통신을 미리 볼 수 있습니다. 테마 편집기에서 다양한 표시 크기가 지원됩니다.
...
1. 세로 막대를 사용하여 선택한 표시 크기의 테마에서 양식 또는 대화형 통신 구성 요소의 스타일을 사용자 정의할 수 있습니다.
...
이제 적응형 양식이나 인터랙티브한 커뮤니케이션에서 웹 서비스에서 사용할 수 있는 글꼴을 사용할 수 있습니다. Adobe의 웹 글꼴 [서비스인](https://typekit.com/)Typekit는 구성으로 제공됩니다. Typekit을 사용하려면 키트와 글꼴을 만들고 Typekit 웹 사이트에서 [키트 ID를](https://typekit.com/)받습니다.

AEM에서 Typekit를 구성하려면 다음 단계를 수행하십시오.

1. 작성자 인스턴스에서 adobeexperience ManagerAdobe Experience ![](assets/adobeexperiencemanager.png)**[!UICONTROL Manager > 도구&#x200B;]**![](assets/hammer.png)> 배포**[!UICONTROL > 클라우드 서비스를&#x200B;]**클릭합니다.
1. 클라우드 서비스 **[!UICONTROL 페이지에서]** 타사 서비스 **[!UICONTROL >]** Typekit으로 **[!UICONTROL 이동하고]** Typekit **[!UICONTROL 에서 지금 구성을]** 클릭합니다. 구성을 이미 사용할 수 있는 경우 **+** 단추를 클릭하여 새 인스턴스를 만듭니다.
1. 구성 **[!UICONTROL 만들기]** 대화 상자에서 구성 제목을 지정하고 만들기를 **[!UICONTROL 클릭합니다]**.

   구성 페이지로 리디렉션됩니다.

1. 표시되는 구성 요소 편집 대화 상자에서 키트 ID를 입력하고 확인을 **클릭합니다**.

TypeKit 구성을 사용하도록 테마를 구성하려면 다음 단계를 수행하십시오.

1. 작성자 인스턴스의 테마 편집기에서 테마를 엽니다.
1. 테마 편집기에서 테마 옵션 **[!UICONTROL 테마 옵션]** > ![구성으로 이동합니다](assets/theme-options.png)****.Configure.
...
1. 적응형 **[!UICONTROL 양식 테마 구성 서비스를 엽니다]**.
...
![테마 편집기 세로 막대에서 사용할 수 있는 마스크 옵션](assets/mask-styles.png)
...
1. 편집 모드에서 구성 요소를 선택한 다음 ![필드 수준](assets/field-level.png) > 적응형 양식 **[!UICONTROL 컨테이너를]**&#x200B;클릭한 다음 ![cmppr](assets/cmppr.png)을클릭합니다.

   세로 막대에서 양식의 속성을 편집할 수 있습니다.

1. 세로 막대에서 스타일 지정을 **[!UICONTROL 클릭합니다]**.
1. 적응형 양식 테마 **[!UICONTROL 드롭다운에서]** 테마를 선택하고 **[!UICONTROL 완료]** ![](assets/check-button.png)확인 단추를 클릭합니다.

대화형 통신에 테마를 적용하려면:

1. 편집 모드에서 인터랙티브한 커뮤니케이션을 열 수 있습니다. 편집 모드에서 대화형 통신을 열려면 양식을 선택하고 열기를 **[!UICONTROL 클릭합니다]**.
1. 편집 모드에서 구성 요소를 선택한 다음 ![필드 수준](assets/field-level.png) > 문서&#x200B;**[!UICONTROL 컨테이너를]**&#x200B;클릭한 다음 ![cmppr](assets/cmppr.png)을클릭합니다.

   세로 막대에서 양식의 속성을 편집할 수 있습니다.

1. 기본 **[!UICONTROL 아래의 테마]**&#x200B;드롭다운에서 **[!UICONTROL 테마를 선택하고]** 완료 ****![체크-](assets/check-button.png)단추사이드바를 클릭합니다.
...
이 `themeOverride` 옵션을 사용하면 테마에 대한 경로를 제공할 수 있습니다. 양식의 테마가 변경되고 업데이트된 스타일로 양식이 새로 고쳐집니다.
...
1. 작성 인스턴스에서 Adobe Experience Manager > **[!UICONTROL 양식 > 테마로 이동합니다]**.
1. 테마 라이브러리 폴더를 엽니다.
1. 테마 라이브러리 폴더에서 해당 기본 테마 위로 포인터를 가져간 다음 복사를 **[!UICONTROL 누릅니다]**.
1. 복사한 테마를 테마 라이브러리 폴더 밖에 붙여 넣습니다.
1. 복사한 테마를 사용자 정의합니다.
...
* **세로 막대에서** CSS 보기 옵션:테마에서 구성 요소를 선택하면 세로 막대에 CSS 보기 옵션이 표시됩니다. 생성된 CSS를 표시합니다. 여기에는 `::before` 및 `::after` 의사 요소에 대한 CSS가 포함됩니다.
...
* **다른 테마에서 자산 방지**
   ...
   테마를 편집할 때 다른 테마에서 자산(예: 이미지)을 찾아 추가할 수 있습니다. 예를 들어 페이지의 배경을 편집하고 있습니다. 예를 들어 페이지 **[!UICONTROL 편집 단추]** > ![배경](assets/edit-button.png)> **[!UICONTROL 추가]**> 이미지를 선택하면다른 테마의 이미지를 검색하고 추가할 수 있는 대화 상자가 표시됩니다.
...
* **기본 clientlib, 테마 편집기 및 인라인 스타일 사용**
   ...
   * **기본 clientlib**:
      ...
      1. Experience Manager **[!UICONTROL > 양식 > 테마를 탐색합니다]**.
      1. 테마 페이지에서 테마를 선택하고 속성 보기를 **[!UICONTROL 클릭합니다]**.
      1. 표시되는 속성 페이지에서 고급을 **[!UICONTROL 클릭합니다]**.
      1. 고급 탭의 Clientlib 위치 필드에서 사용할 클라이언트 라이브러리를 찾아 선택합니다.
      1. **[!UICONTROL 저장]**&#x200B;을 클릭합니다.
      클라이언트 라이브러리에서 지정한 스타일이 이 스타일을 사용하는 테마에서 가져옵니다. 예를 들어, 텍스트 상자, 숫자 상자에 대한 스타일링을 지정하고 클라이언트 라이브러리에서 전환합니다. 테마에서 클라이언트 라이브러리를 가져오면 텍스트 상자, 숫자 상자 및 스위치를 위한 스타일링을 가져옵니다. 그런 다음 테마 편집기를 사용하여 다른 구성 요소의 스타일을 지정할 수 있습니다.\
      테마를 만들고, 테마를 만든 다음, 유사한 사용 사례용으로 복사한 테마에 제공된 스타일링을 수정할 수도 있습니다.
   ...
   * **인라인 스타일:**
      ...
      양식 또는 대화형 통신 다중 채널 편집기에서 스타일 모드를 사용하여 구성 요소의 스타일을 지정할 수 있습니다. 스타일 모드를 사용하여 양식 구성 요소 스타일을 변경하면 테마에 지정된 스타일이 재정의됩니다. 특정 양식의 특정 구성 요소에 대한 스타일링을 변경하려면 구성 요소의 [인라인 스타일을](/help/forms/using/inline-style-adaptive-forms.md)참조하십시오.
