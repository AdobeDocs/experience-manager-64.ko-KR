...
1. 내 양식에 바코드 및 서명 필드가 표시되지 않는 이유는 무엇입니까?
   ...
   답변: 바코드 및 서명 필드는 HTML 또는 모바일 시나리오에서 관련이 없습니다. 이러한 필드는 비대화형 영역으로 표시됩니다. 그러나 AEM Forms 디자이너는 서명 필드 대신 사용할 수 있는 새로운 서명 스크리블 필드를 제공합니다. 또한, [사용자 지정 위젯](/help/forms/using/custom-widgets.md) 바코드를 생성하고 통합합니다.
...
1. 권장 이름 지정 규칙이 있으며, 피할 예약된 키워드가 있습니까?
   ...
   * AEM Forms 디자이너에서 밑줄(_). 이름 시작 부분에 밑줄을 사용하려면 밑줄 뒤에 접두어를 추가합니다._&lt;prefix>&lt;objectname>. *
