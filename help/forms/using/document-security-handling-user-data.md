...
1. 다음 데이터베이스 명령을 실행하여 문서 보안을 위해 데이터베이스 테이블에서 보안 주체 ID에 대한 데이터를 삭제합니다. `Delete` 명령에서 `<principal_id>`을(를) 데이터를 삭제할 사용자의 주체 ID로 바꿉니다.
   ...
   >[!NOTE]
   >
   >`EdcAuditEntity` 테이블에서 데이터를 삭제하려면 `principalId`, `policyId` 또는 `licenseId`을(를) 기준으로 감사 데이터를 삭제할 매개 변수로 [EventManager.deleteEvents](https://helpx.adobe.com/experience-manager/6-4/forms/ProgramLC/javadoc/index.html?com/adobe/livecycle/rightsmanagement/client/EventManager.html) API를 사용합니다.[](https://helpx.adobe.com/experience-manager/6-4/forms/ProgramLC/javadoc/com/adobe/livecycle/rightsmanagement/client/infomodel/EventSearchFilter.html)
