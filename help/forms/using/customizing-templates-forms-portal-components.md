...
1. */apps * 아래에 sling:Folder 노드 만들기
   ...
   *다음 이미지는 검색 및 라이브러리 구성 요소에 대한 구성을 보여 줍니다.* 슬링 ![만들기:폴더](assets/1-3.png)
...
*설명한 예에서 사용자 지정 템플릿의 맨 위에는 두 개의 Div 요소가 있습니다. 첫 번째 클래스는 &quot;__FP_boxes-container&quot; CSS 클래스를 사용하여 나열된 양식의 컨테이너 요소로 작동합니다. &quot;__FP_boxes&quot; CSS 클래스가 있는 두 번째 항목은 기본 개체에 대한 템플릿입니다(이 경우 Form). Div 요소에 있는 **데이터 반복** 속성은 **true**&#x200B;값을 갖습니다.
...
1. **현지화 지원**:정적 텍스트를 현지화하려면 **${localize-***YOUR_TEXT***} 속성을 사용하고** 해당 지역화된 값을 사용할 수 있도록 하십시오(없는 경우).
...
1. **구성 속성**&#x200B;사용:검색 및 라이브러리 구성 요소에는 사용자 인터페이스에서 사용할 수 있는 여러 구성이 있습니다. 예를 들어 편집 대화 상자를 통해 저장된 HTML 도구 설명 텍스트를 표시하려면 **${config-htmlLinkText} 속성을 사용합니다.**&#x200B;마찬가지로 PDF 도구 설명 텍스트의 경우&#x200B;**${config-pdfLinkText}** 속성을 사용하십시오.
...
1. 초안 및 제출 서류를 열기 위한 링크를 만드는 동안 **$path.html** 을 앵커 태그의 **href** 속성 값으로 지정할 수 있습니다.
