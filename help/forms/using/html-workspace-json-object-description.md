...
description: 사용자 정의, 확장, 수정 및 재사용을 위해 LiveCycle AEM Forms 작업 영역에 사용되는 JSON JavaScript 개체에 대한 개념 정보입니다.
seo-description: 사용자 정의, 확장, 수정 및 재사용을 위해 LiveCycle AEM Forms 작업 영역에 사용되는 JSON JavaScript 개체에 대한 개념 정보입니다.
...
<table> 
 <tbody>
  ...
  <tr>
   <td>categoryList</td> 
   <td>T</td> 
   <td>카테고리<br type="_moz" />의 직접 하위 카테고리 목록을 포함합니다. </td> 
  </tr>
 </tbody>
</table>
...
1. 시작 지점
   ...
   시작점은 호출될 때 작업 공간에서 프로세스를 시작하는 데 사용됩니다.

   | **속성** | **클라이언트만** | **댓글** |
   |---|---|---|
   | categoryId | F | 시작 지점이 속하는 카테고리의 ID를 포함합니다. |
   | 설명 | F | 시작점에 대한 설명이 포함되어 있습니다. |
   | 이름 | F | 시작 지점 이름이 포함되어 있습니다. |
   | serializeImageTicket | F | 시작점에 해당하는 이미지 티켓이 포함되어 있습니다. 이 이미지 티켓은 서버로부터 시작점에 대한 이미지를 가져오기 위해 시작점의 imageUrl 필드에서 사용됩니다. |
   | serviceName | F | 시작 지점에 대한 서비스 이름이 포함되어 있습니다. |
   | startpointId | F | 시작 지점 ID가 포함되어 있습니다. |
   | isFavorite | T | 시작점이 즐겨찾기인지 여부를 나타냅니다. startpoint가 favorite else false이면 true입니다. |
   | isDefaultImage | T | 프로세스에 대해 지정된 이미지가 있는지 여부를 나타냅니다. 프로세스 또는 false와 연결된 이미지가 없는 경우 true입니다. |
   | 작업 | T | 시작점이 호출될 때 만들어진 작업이 포함되어 있습니다. |
   | imageUrl | T | 시작점에 해당하는 이미지의 URL이 포함되어 있습니다. |

1. 작업

   작업은 사용자/그룹에 할당되고 데이터로 채울 수 있는 사용자 인터페이스(양식 또는 안내서(더 이상 사용되지 않음)를 포함합니다. 사용자에게 작업이 할당되면 완료하고 제출할 양식 또는 안내서가 제공됩니다.
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>F</td> 
   <td>true인 경우 작업을 열 때 양식에서 먼저 전체 화면이 표시됩니다.<br /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   <td>isVisible<br /> </td> 
   <td>F</td> 
   <td>작업 영역에 작업이 표시되면 true입니다.<br /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   <td>상태<br /> </td> 
   <td>F</td> 
   <td>1 = 생성됨(작업은 시작점에서 생성됩니다.)<br /> 2 = 생성 및 저장됨(작업은 시작점에서 생성되어 저장됨)<br /> 3 = 할당됨(프로세스가 시작된 후 작업이 사용자에게 지정됩니다.)<br /> 4 = 지정됨 및 저장됨(작업이 할당되고 저장됨)<br /> 100 = 완료(작업이 완료됨)<br /> 101 = 마감 날짜(작업이 마감 시간에 도달했습니다.)<br /> 102 = 종료됨<br /> </td> 
  </tr>
  <tr>
   <td>stepName<br /> </td> 
   <td>F</td> 
  ...
  <tr>
   ...
   <td>T</td> 
   <td>작업 양식 유형이 포함되어 있습니다. 이 필드를 사용하면 작업이 클라이언트에서 SWF 양식 등을 위해 pdf로 렌더링됩니다.<br /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>T</td> 
   <td>true이면 양식을 오프라인으로 전환할 수 있습니다. pdf 양식에만 해당됩니다.<br /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>T</td> 
   <td>프로세스에 대해 지정된 이미지가 있는지 여부를 나타냅니다. 프로세스와 연결된 이미지가 없는 경우 true이고, 그렇지 않은 경우 false입니다.<br /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>T</td> 
   <td>작업에 사용할 수 있는 경우 앞으로, 공유 및 참조와 같은 명령을 포함합니다.<br /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>T<br /> </td> 
   <td>여기에는 프로세스 변수의 객체(있는 경우)의 배열이 포함됩니다.<br /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>T</td> 
   <td>개체 배열입니다. 각 개체에는 경로 및 해당 확인 메시지가 있을 경우 그에 대한 세부 사항이 포함됩니다.<br /> </td> 
  </tr>
  <tr>
   <td>dataUrl<br /> </td> 
   <td>T</td> 
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>F</td> 
   <td>대기열에 로그인한 사용자의 기본 대기열이 같으면 true이고, 그렇지 않으면 false입니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   <td>유형</td> 
   <td>F</td> 
   <td>큐 유형을 포함합니다.<br /> 0 - 사용자 큐.<br /> 1. 공유 큐를 참조하십시오.<br /> 2. 그룹 큐.<br type="_moz" /> </td> 
  </tr>
  <tr>
   <td>쿼리</td> 
   <td>T</td> 
   <td>필터와 연관된 쿼리가 포함되어 있습니다. 이 쿼리는 전체 작업 목록에서 작업을 검색하는 데 사용됩니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
   <td>작업</td> 
   <td>T</td> 
...
1. 부재 중
   ...
   부재 중 일정을 관리하고 부재 시 자신에게 할당된 작업 흐름을 제어할 수 있습니다.
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>F</td> 
   <td>사용자 부재 중 스케줄의 배열 객체를 포함합니다. 각 예약 객체에서 startDate 필드는 일정의 시작 날짜와 endDate 필드에 일정의 종료 날짜가 포함되어 있습니다. endDate가 일정에 null인 경우 사용자가 부재 중 스케줄의 종료 날짜를 예약하지 않았음을 의미합니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>F</td> 
   <td>사용자가 부재 중일 경우 true입니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
   <td>outOfOfficeDesignate<br type="_moz" /> </td> 
   <td>F</td> 
   <td>여기에는 사용자가 기본 지정으로 지정하는 사용자의 세부 정보가 포함되어 있습니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
   <td>processSpecificDesicts<br type="_moz" /> </td> 
   <td>F</td> 
   <td>프로세스 특정 부재 중 지정을 위한 개체 배열을 포함합니다. 각 프로세스 특정 지정 객체에서 processName은 프로세스 이름을 포함하며, 해당 프로세스에 대해 사용자가 할당되지 않은 경우 isNotDesignated는 true이고, 사용자가 해당 프로세스에 할당된 사용자의 다른 세부 정보를 할당하지 않은 경우 userDesignated는 null입니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>T</td> 
   <td>여기에는 날짜까지 로그인한 사용자가 검색하는 사용자 목록이 포함되어 있습니다.<br type="_moz" /> </td> 
  </tr>
 </tbody>
</table>
...
1. 프로세스 인스턴스
   ...
   프로세스 인스턴스는 작업 공간 또는 워크벤치를 통해 프로세스를 호출할 때 생성됩니다.
...
<table> 
 <tbody>
  ...
  <tr>
   <td>이니시에이터</td> 
   <td>F</td> 
   <td>프로세스 인스턴스의 시작자 이름입니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
   <td>initiatorId</td> 
   <td>F</td> 
  ...
  <tr>
   ...
   <td>F</td> 
   <td>0 = 시작<br /> 1 = 실행 중<br /> 2 = 완료<br /> 3 = 완료<br /> 4 = 종료됨<br /> 5 = 종료 중<br /> 6 = 일시 중단됨<br /> 7 = 일시 중단 해제<br /> 8 = 일시 중단 해제<br type="_moz" /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>F</td> 
   <td>프로세스 변수의 객체 배열입니다. 각 프로세스 변수 객체에는 프로세스 변수의 이름인 이름, 프로세스 변수의 값인 값 및 프로세스 변수의 유형인 유형이 포함됩니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>F</td> 
   <td>이 작업 할당을 만들 때의 타임스탬프.<br type="_moz" /> </td> 
  </tr>
  <tr>
   <td>assignmentType<br type="_moz" /> </td> 
   <td>F</td> 
   <td>0 = 초기 할당<br /> 1 = 앞으로(작업이 현재 작업 소유자에게 전달되었습니다.)<br /> 2 = 반환됨(이전 작업 소유자가 작업을 현재 작업 소유자에게 반환했습니다.)<br /> 3 = 요청(현재 작업 소유자가 작업을 요청했습니다.)<br /> 4 = 문제 제기(문제 제기 후 작업의 현재 소유자에게 작업이 할당되었습니다.)<br /> 5 = 관리자 지정(작업이 현재 작업 소유자에게 관리자가 할당했습니다.)<br /> 6 = Consulting(현재 작업 소유자에게 작업이 상담되었습니다.)<br type="_moz" /> </td> 
  </tr>
  <tr>
...
1. 작업 ACL 개체
   ...
   작업 ACL 개체에는 전달, 공유, 컨설팅 등 사용 권한에 대한 정보가 포함되어 있습니다. 를 선택합니다. 다음은 작업의 ACL 속성입니다.
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>F</td> 
   <td>true이면 작업을 확인할 수 있습니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>F</td> 
   <td>첨부 파일이 추가되는 작업의 ID입니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>T</td> 
   <td>사용자의 UI 설정에 따른 첨부 파일 만들기 날짜가 포함되어 있습니다.<br type="_moz" /> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>T</td> 
   <td>서식이 지정된 첨부 파일 이름. AEM Forms 작업 영역의 첨부 파일에 있는 특수 문자를 표시하는 데 사용됩니다. 참고용입니다.<br type="_moz" /> </td> 
  </tr>
 </tbody>
</table>
