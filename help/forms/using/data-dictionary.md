...
서신 관리는 백 엔드, 고객, `` ``또는 데이터 사전의 구조에 따라 저장된 수신자별 데이터를 통해 서로 다른 고객을 위한 서신을 만들 수 있습니다. 예를 들어 &quot;Addrese {First Name}&quot;,&quot;Mr.&quot;과 같이 친숙한 이름으로 문서를 만들 수 있습니다. {성}&quot;.
...
1. While [편지 미리 보기](/help/forms/using/create-letter.md#p-types-of-linkage-available-for-each-of-the-fields-p): 작성자가 데이터를 사용하여 편지를 미리 보기하도록 선택합니다(사용자 지정 미리 보기). 작성자가 제공한 데이터가 미리 채워져 있는 편지가 열립니다. 서신 만들기 인터페이스에서 열립니다. 이 편지를 미리 보고 있는 에이전트는 이 편지에서 내용, 데이터 및 첨부 파일을 수정할 수 있으며 최종 편지를 제출할 수 있습니다. 문자 만들기에 대한 자세한 내용은 [서신 만들기](/help/forms/using/create-letter.md).

## 전제 조건 {#prerequisite}

설치 [호환성 패키지](https://helpx.adobe.com/in/experience-manager/6-4/forms/using/compatibility-package.html) 를 클릭하여 **데이터 사전** 옵션 **Forms** 페이지.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>유형 DDE - 컬렉션 -<br /> DDE 노드는 상위 COLLECTION 노드의 정보를 캡처하는 COLLECTION DDE 옆에 만들어집니다. 단순/복합 데이터 유형의 두 컬렉션에 대해 동일한 항목이 만들어집니다. 유형 조합의 COLLECTION이 있을 때마다 데이터 사전 트리에서는 유형 정보를 캡처하기 위해 만든 DDE의 1차 하위 구성요소의 필드를 캡처합니다.<br /> - DDE(수집)<br /> - DDE(유형 정보용 복합)<br /> - DDE(문자열) 필드1<br /> - DDE(문자열) 필드2<br /> <br /> </p> </td> 
...
1. 탭 **확인** 경고 메시지에 추가합니다.
   ...
   서신 관리에서는 선택한 데이터 사전의 구조를 기반으로 XML 파일을 만들고 이름을 사용하여 컴퓨터에 다운로드합니다 &lt;data-dictionary-name>-SampleData. 이제 XML 또는 텍스트 편집기에서 이 파일을 편집하여 [편지 만들기](/help/forms/using/create-letter.md).
