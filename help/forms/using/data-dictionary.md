...
통신 관리는 데이터 사전의 구조에 따라 저장된 백엔드, 고객 `` ``또는 수신자별 데이터를 사용하여 서로 다른 고객을 위한 메시지를 작성합니다. 예를 들어 &quot;Dear {First Name}&quot;,&quot;Mr.&quot;과 같이 친숙한 이름으로 문서를 만들 수 있습니다. {성}&quot;.
...
1. 편지 [미리 보기](/help/forms/using/create-letter.md#p-types-of-linkage-available-for-each-of-the-fields-p)중에 작성자가 데이터를 사용하여 편지 미리 보기(사용자 정의 미리 보기)를 선택합니다. 작성자가 제공한 데이터로 미리 채워진 문자가 열립니다. 통신 만들기 인터페이스에서 열립니다. 이 편지를 미리 보고 있는 에이전트는 이 서신의 컨텐트, 데이터 및 첨부 파일을 수정할 수 있으며 최종 편지를 제출할 수 있습니다. 편지 작성에 대한 자세한 내용은 편지 [작성을](/help/forms/using/create-letter.md)참조하십시오.

## 전제 조건 {#prerequisite}

[양식] [페이지에서](https://helpx.adobe.com/in/experience-manager/6-4/forms/using/compatibility-package.html) [데이터 사전 **]** 옵션을 보려면 **호환성** 패키지를설치합니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>DDE 유형 - 합성<br /> </p> </td> 
   <td>java.util.Map<br /> </td> 
  </tr> 
  <tr> 
   <td><p>xs:element where maxOccurs &gt; 1<br /> </p> </td> 
   <td><p>DDE of type - COLLECTION-<br /> A DDE 노드는 상위 COLLECTION 노드의 정보를 캡처하는 COLLECTION DDE 옆에 생성됩니다. 단순/복합 데이터 유형 모두에 대해 동일한 항목이 생성됩니다. 복합 유형의 COLLECTION이 있을 때마다 데이터 딕셔너리 트리는 유형 정보를 캡처하기 위해 생성된 DDE의 자식의 구성 요소 필드를 캡처합니다.<br /> - DDE (COLLECTION)<br /> - DDE(유형 정보에 대한 합성)<br /> - DDE(STRING) field1<br /> - DDE(문자열) 필드2<br /><br /> </p> </td> 
  ...
  <tr> 
   <td><p>xs:attribute /xs:유형 - xs:string</p> </td> 
   <td>DDE 유형 - 문자열<br /> </td> 
   <td>java.lang.String<br /> </td> 
  </tr> 
  <tr> 
...
1. 경고 **메시지에서** 확인을 누릅니다.
   ...
   통신 관리는 선택한 데이터 사전의 구조를 기반으로 XML 파일을 만들고 &lt;data-dictionary-name>-SampleData라는 이름으로 컴퓨터에 다운로드합니다. 이제 XML 또는 텍스트 편집기에서 이 파일을 편집하여 문자를 [만드는](/help/forms/using/create-letter.md)동안 데이터 항목을 만들 수 있습니다.
