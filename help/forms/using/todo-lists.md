...
1. 다음 단계 중 하나를 수행합니다.
   ...
   * 작업을 열지 않고 그룹 또는 공유 할 일 목록에서 요청하려면 마우스 포인터를 작업 **위로 가져가서** 클레임을 클릭합니다. 또는 작업이 열려 있으면 작업창 아래의 작업 표시줄에서 클레임 단추를 사용할 수 있습니다. 청구 시 작업은 그룹 또는 공유 할 일 목록에서 사용자의 목록으로 이동합니다.
   * 그룹 또는 공유 할 일 목록에서 작업을 요청하고 열려면 클레임 **을 클릭하고 엽니다**.

## Working with tasks {#working-with-tasks}

작업을 연 후에는 기본 창에 표시되는 탭과 작업에 따라 사용 가능한 도구가 달라집니다. 표시되는 탭은 아래에 설명되어 있습니다.

**작업 요약**: 작업이 열리면 [작업 요약] 창을 통해 작업 지정 단계의 프로세스에 지정된 URL을 사용하여 작업에 대한 정보를 표시할 수 있습니다(있는 경우). AEM Forms 작업 영역의 최종 사용자에 대한 추가 값을 추가하기 위해 작업 요약 창 추가 및 작업에 대한 관련 정보를 표시할 수 있습니다. 작업 요약 URL이 없는 경우 이 탭을 사용할 수 없습니다.
...
### 작업 진행, 공유 또는 컨설팅 {#forward-share-or-consult-on-a-task}
...
1. 유지하려는 작업과 연관된 양식을 변경하는 경우 **저장을 클릭합니다**. 저장 옵션은 각 탭 하단의 작업 표시줄에서 사용할 수 있습니다. 그렇지 않으면 업데이트된 양식 없이 작업이 전송됩니다.
...
1. 모든 탭에서 **돌아가기를 클릭합니다**. 작업이 이전에 작업을 자신에게 전달했거나 작업을 공유하거나 귀하와 상담한 사용자의 할 일 목록으로 반환됩니다.
...
1. 첨부 파일 **에** 대해 편집을 클릭합니다. 찾아보기를 클릭하여 기존 첨부 파일을 바꿀 파일을 **선택합니다**.

**첨부 파일 삭제**

1. 첨부 파일 **을** 보려면 삭제를 클릭합니다.
...
1. 모든 탭에서 **저장을 누릅니다**.
   ...
   >
   >자세한 내용은 기본 설정 관리에서 주기적으로 초안 [저장을 참조하십시오](/help/forms/using/getting-started-livecycle-html-workspace.md).
...
* 작업을 열지 않고 할 일 목록에서
   ...
   참고: 이 옵션은 프로세스를 디자인하는 동안 워크벤치의 `isMustOpenToComplete` `Assign Task` 단계에서 필드를 선택하지 않은 경우 사용할 수 있습니다.
...
1. 다음 단계 중 하나를 수행합니다.
   ...
   * 양식에 단추가 없고 AEM Forms 작업 영역에서 완료 단추를 사용할 수 있는 경우 완료를 **클릭합니다**.
   * 양식에 단추가 있고 AEM Forms 작업 영역의 완료 단추를 사용할 수 없는 경우 프로세스에 필요한 다음 단계에 해당하는 양식에서 해당 단추를 클릭합니다.

   양식에 단추가 없고 AEM Forms 작업 영역에서 완료 단추를 사용할 수 없는 경우 양식을 제출할 수 없다는 메시지가 나타납니다.

1. 확인 대화 상자가 나타나면 다음 작업 중 하나를 수행합니다.

   * 작업을 **완료하고** 작업을 시작할 준비가 되었으면 확인을 클릭합니다.
   * 작업으로 **돌아가고** 아직 로그오프할 준비가 되지 않은 경우 취소를 클릭합니다.

>[!NOTE]
>
>HTML 양식 내에 제출 단추가 표시될 수 있습니다. 동일한 양식이 PDF로 렌더링될 때 이 단추가 보이지 않습니다. 작업을 완료하려면 AEM Forms 작업 영역의 아래쪽에 있는 [제출] 단추가 양식 내에 있는 [제출] 단추가 아닌 다른 쪽을 클릭하십시오.
...
먼저 이메일 알림을 수신하도록 AEM Forms 작업 영역 환경 설정을 설정합니다. AEM Forms 작업 공간에서는 할 일 목록 또는 사용자가 속한 그룹 할 일 목록에 있는 작업에 대한 이메일 알림을 보낼 수 있습니다. 관리자는 이메일 알림 메시지가 전송되는 시점과 수신자를 결정합니다.

이메일 메시지에는 AEM Forms 작업 영역에서 작업을 여는 링크, 작업에 사용되는 양식의 첨부 파일 또는 이메일을 통해 작업을 완료하는 데 필요한 작업이 포함될 수 있습니다. 이메일 메시지에 양식이 포함된 경우 양식을 열고 작업을 완료하는 단추를 양식에 작성할 수 있습니다. 작업을 완료하는 작업이 이메일 메시지에 포함된 경우, 이메일의 동작을 클릭하거나 이메일 본문에 첫 번째 줄에 입력한 동작을 이메일로 응답하여 작업을 완료할 수 있습니다.

>[!NOTE]
>
>적절한 이메일 템플릿을 사용하도록 작업 영역을 구성하려면 [AEM Forms JEE 관리자 안내서를 참조하십시오](https://help.adobe.com/en_US/AEMForms/6.1/AdminHelp/).
...
>[!NOTE]
>
>작업을 다른 사람에게 할당하려면 이메일 알림을 전송할 수 없습니다. AEM Forms 작업 영역에서 다른 사용자에게만 작업을 전달할 수 있습니다.
...
AEM Forms 작업 영역 앱을 사용하여 모바일 장치의 작업에 참여할 수 있습니다. 애플리케이션을 설치하기 전에 시스템 관리자에게 문의하여 조직에서 AEM Forms 작업 영역 앱 사용을 지원하는지 확인하십시오.

## 일정 및 미리 알림 {#about-deadlines-and-reminders}

마감일은 *작업을 완료해야 하는 날짜와 시간을 결정합니다* . 마감 시간이 경과하면 서버가 작업을 프로세스의 다음 단계로 라우팅하고(다른 사용자의 할 일 목록이 될 수 있음) 마감 시간 아이콘이 작업에 표시됩니다. 최종 기한 아이콘은 프로세스와 연관된 규칙과 상관없이 나타납니다.

주의 *가 필요한 작업을 알려 줍니다* . 미리 알림은 미리 지정된 시간에 발생한 다음 연관된 작업을 완료할 때까지 정기적으로 발생합니다. 미리 알림을 받으면 작업에 미리 알림 아이콘이 나타납니다.
...
1. 사용자 이름을 선택하고 [공유]를 **클릭합니다**.
   ...
   >[!NOTE]
   >
   >사용자가 나열되는 행 끝에 있는 **X** 아이콘을 클릭하여 사용자가 할 일 목록을 공유에서 제거할 수 있습니다.
...
1. 사용자 이름을 선택하고 [요청]을 **클릭합니다**.
   ...
   >[!NOTE]
   >
   >액세스 권한이 있는 사용자 큐 목록에서 사용자 이름을 선택하고 사용자 이름이 나와 있는 행 끝에 있는 **X를** 클릭하여 다른 할 일 목록에 대한 액세스 권한을 제거할 수 있습니다. 할 일 목록에 대한 액세스 요청이 여전히 보류 중일 때는 다른 할 일 목록에 대한 액세스 권한을 제거할 수 없습니다.

## 사무실 외 환경 설정 지정 {#setting-out-of-office-preferences}
...
### 사무실 외 환경 설정 지정 {#set-out-of-office-preferences}

1. 환경 **설정** 을 클릭하고 **사무실 밖**&#x200B;을 클릭합니다.
1. 사무실 밖에 있는 시간을 지정하려면 다음 단계 중 하나를 수행합니다.

   * 사용자가 무기한으로 사무실 밖 **으로 지정하도록 지정하려면 현재** **내** 목록에서 [사무실 밖]을 선택하고 날짜 범위를 추가하지 마십시오.
   * 부재 중인 시작 날짜 및 시간을 지정하고 부재 중 예약에 대해 &#39;+&#39; **를 클릭합니다**. 달력 및 시간 목록을 사용하여 시작 날짜와 시간을 지정합니다. 종료 날짜 및 시간을 지정하지 않으면 기본 설정을 변경할 때까지 시작 날짜 및 시간이 무기한으로 사무실에서 벗어나는 것으로 간주됩니다.

1. 기본적으로 작업을 처리하는 방법을 지정하려면 [사무실 밖 **에 있는 경우]에서 다음 옵션 중 하나를 선택합니다. 부재 중 작업에 대한 기본 사용자** 목록:

   * [ **할당** 안 함]을 선택하여 작업을 [할 일] 목록에 보관합니다.
   * 사용자 **찾기를** 선택하여 작업을 할당할 사용자를 검색합니다. 사용자를 선택하면 사무실 외 일정을 볼 수도 있습니다.

1. 예외를 기본값으로 설정하려면 **프로세스 예외**&#x200B;처리에 대해 +를 클릭하고, 예외를 만들 프로세스를 선택한 다음, 다른 사용자를 선택하거나 목록에 지정된 사용자 **에서** 지정 안 **함** 을선택합니다.
