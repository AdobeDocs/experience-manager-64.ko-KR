...
이 자습서에서는 인터랙티브 커뮤니케이션 [계획 섹션에 설명된 분석을 기반으로 여러 텍스트 문서 조각을 만드는 단계를 안내합니다](/help/forms/using/planning-interactive-communications.md) . 이 튜토리얼의 끝에서 다음 작업을 수행할 수 있습니다.
...
각 문서 조각에는 정적 텍스트, 양식 데이터 모델에서 받은 데이터 및 에이전트 UI를 사용하여 입력된 데이터가 포함됩니다. 이러한 필드는 모두 [대화형 통신 [계획] 섹션에 설명되어](/help/forms/using/planning-interactive-communications.md) 있습니다.

이 자습서에서 문서 조각을 만드는 동안 에이전트 UI를 사용하여 데이터를 받는 필드에 대한 변수가 만들어집니다.

이 자습서에서 **문서 단편을 만드는 양식 데이터 모델**&#x200B;생성 섹션에 설명된 대로 FDM_Create_First_IC를 [](create-form-data-model-tutorial.md) 사용합니다.

## 1단계: 청구 상세내역 텍스트 문서 조각 생성 {#step-create-bill-details-text-document-fragment}
...
1. 양식 **[!UICONTROL > 문서 조각]** 을 **[!UICONTROL 선택합니다]**.
...
1. 다음 정보를 지정합니다.
   ...
   1. 양식 **데이터 모델로 FDM_Create_First_IC를** 선택하고 **선택을 누릅니다**.
   1. 다음을 **누릅니다**.

1. 왼쪽 창에서 **변수** 탭을 선택하고 만들기를 **누릅니다**.
1. 변수 **만들기** 섹션에서 다음을 수행합니다.

   1. 변수 **이름으로 송장** 번호를 입력합니다.
   1. 문자열을 **유형으로** 선택합니다.
   1. 만들기를 **누릅니다**.
...
1. 다음 정보를 지정합니다.
   ...
   1. 제목 필드에 **이름으로 customer_details_first_ic** 을 **입력합니다** . 이름 필드에 제목이 자동으로 **채워집니다** .
   1. 데이터 모델 **섹션에서** 양식 데이터 모델 **을** 선택합니다.
   1. 양식 **데이터 모델로 FDM_Create_First_IC를** 선택하고 **선택을 누릅니다**.
   1. 다음을 **누릅니다**.
...
1. [대체 연락처 번호] 필드 옆에 **커서를 놓고** 고객 **>** 대체 **번호** 속성을 두 번 클릭합니다.
1. 관계 번호 필드 옆에 커서를 놓고 **고객** > **관계 번호** 속성을 두 번 **** 클릭합니다.
1. 변수 **탭** 을 선택하고 공급처 **필드 옆에 커서를** 놓고 배치 **변수** 를 두 번클릭합니다.
...
1. 저장 **을** 클릭하여 고객 세부 정보 텍스트 문서 조각을 생성합니다.
...
1. 기한 필드 옆에 커서를 놓고 **기간** 변수를 두 번 **클릭합니다** .
1. 데이터 모델 **객체** 탭을 선택하고 오른쪽 창에 있는 청구 현재 청구 기간 **필드 옆에 커서를 놓고** 청구 **>** 청구 **> 자산** 을 두 번 클릭합니다.

   ![bill_summary_static_variables](assets/bill_summary_static_variables.png)

1. 저장 **을** 클릭하여 고객 세부 정보 텍스트 문서 조각을 생성합니다.

## 4단계: 요금 요약 텍스트 문서 조각 만들기 {#step-create-summary-of-charges-text-document-fragment}
...
1. 전화 통화 요금 필드 옆에 커서를 놓고 **청구** > **호출** 속성 **을 두 번 클릭합니다** .
1. 컨퍼런스 콜 **요금** 필드 옆에 커서를 놓고 [청구 **] >** 수수료 **속성을 두 번** 클릭합니다.
1. SMS **요금** 필드 옆에 커서를 놓고 [ **청구** ] > **수수료** 속성을 두 번클릭합니다.
1. [ **모바일 인터넷 요금** ] 필드 옆에 커서를 놓고 [ **청구** ] > **인터넷 요금** 속성을 두 번 클릭합니다.
1. 국가 로밍 요금 필드 옆에 커서를 놓고 **법안** > **국가** 의 **속성을 두 번 클릭합니다** .
1. 국제 로밍 비용 필드 옆에 커서를 놓고 **청구지** > **roamingnl** 속성을 두 번 클릭합니다 **** .
1. 부가가치 서비스 비용 필드 옆에 **커서를 놓고** [청구 **] >** vas **속성을 두 번** 클릭합니다.
1. [ **총 비용** ] 필드 옆에 커서를 놓고 [청구 **] > [** 청구 **]** 속성을 두 번클릭합니다.
1. TOTAL PAYABLE 필드 옆에 **커서를 놓고** [ **어음** ] > [ **자동이체** ] 속성을 두 번클릭합니다.

   ![summary_charts_static_fdm](assets/summary_charges_static_fdm.png)

1. VIP(Value Added Services Charts) 행 **의 텍스트를** 선택하고 규칙 **만들기** 를 탭하여 대화형 통신에서 행이 표시되는 조건에 따라 조건을 만듭니다.
1. 규칙 **만들기** 팝업 창에서:

   1. 데이터 **모델 및 변수를** 선택한 다음 **청구** > **콜러스를**&#x200B;선택합니다.
   1. 연산자 **보다** 작음을 선택합니다.
   1. 번호 **를** 선택하고 값을 **60으로 입력합니다**.
   이 조건에 따라, 부가가치 서비스 비용 행은 호출 비용 필드의 값이 60보다 작은 경우에만 표시됩니다.

   ![create_rules_caption](assets/create_rules_caption.gif)

1. 저장 **을** 클릭하여 비용 청구 텍스트 문서 조각 요약을 생성합니다.
