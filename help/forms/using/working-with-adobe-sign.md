...
Adobe Sign을 사용하면 적응형 양식에 전자 서명 워크플로우를 사용할 수 있습니다. 전자 서명은 법률, 영업, 급여, 인적 자원 관리 등 다양한 분야에 대한 문서를 처리하기 위한 워크플로우를 개선합니다.

일반적인 Adobe Sign 및 적응형 양식 시나리오에서 사용자는 적응형 양식을 작성하여 서비스에 적용합니다. 예를 들어, 대출 및 신용 카드 신청은 모든 대출자와 공동 지원자의 법적 서명을 필요로 합니다. 유사한 시나리오에 대해 전자 서명 워크플로우를 활성화하려면 Adobe Sign을 AEM Forms과 통합할 수 있습니다. 몇 가지 더 예를 들자면 Adobe Sign을 사용하여 다음을 수행할 수 있습니다.
...
AEM Forms과 Adobe Sign 통합은 다음을 지원합니다.
...
1. [Adobe 기호에 대한 적응형 양식 속성 편집](#enableadobesign)
1. [적응형 양식에 Adobe Sign 필드 추가](#addadobesignfieldstoanadaptiveform)
1. [적응형 양식에 Adobe Sign 활성화](#enableadobsignforanadaptiveform)
1. [적응형 양식에 대한 Adobe Sign Cloud Service 선택](#selectadobesigncloudserviceforanadaptiveform)
...
### Adobe Sign에 대한 적응형 양식 속성 편집 {#enableadobesign}

기존 또는 새로운 적응형 양식에 대해 Adobe Sign에 대한 적응형 양식 속성을 구성합니다.
...
1. **[!UICONTROL 기본]** 탭에서 다음을 수행합니다.
   ...
   1. AEM Forms에서 Adobe Sign을 구성하는 동안 만든 [구성 컨테이너](/help/forms/using/adobe-sign-integration-adaptive-forms.md#configure-adobe-sign-with-aem-forms)를 선택합니다.
...
1. **[!UICONTROL 만들기 를 누릅니다.]** Adobe Sign 필드를 추가하는 데 사용할 수 있는 서명 사용 적응형 양식이 만들어집니다.
...
1. **[!UICONTROL 저장 및 닫기]**&#x200B;를 누릅니다. Adobe Sign에 대해 적응형 양식이 활성화됩니다.

### 적응형 양식에 Adobe Sign 필드 추가 {#addadobesignfieldstoanadaptiveform}

Adobe Sign에는 적응형 양식에 배치할 수 있는 다양한 필드가 있습니다. 이러한 필드는 서명, 이니셜, 회사 또는 제목과 같은 다양한 유형의 데이터를 허용하며 서명과 함께 서명 중에 추가 정보를 수집하는 데 도움이 됩니다. Adobe Sign 블록 구성 요소를 사용하여 적응형 양식의 다양한 위치에 Adobe Sign 필드를 배치할 수 있습니다.

적응형 양식에 필드를 추가하고 이러한 필드와 관련된 다양한 옵션을 사용자 지정하려면 다음 단계를 수행하십시오.

1. 구성 요소 브라우저에서 적응형 양식으로 **Adobe Sign Block** 구성 요소를 드래그하여 놓습니다. Adobe Sign 블록 구성 요소에는 지원되는 모든 Adobe Sign 필드가 있습니다. 기본적으로 적응형 양식에 **서명** 필드가 추가됩니다.

   ![사인블록](assets/sign-block.png)

   기본적으로 게시된 적응형 양식에는 Adobe Sign 블록이 표시되지 않습니다. 서명 문서에서만 볼 수 있습니다. Adobe Sign 블록 구성 요소의 속성에서 Adobe Sign 블록 가시성을 변경할 수 있습니다.

   >[!NOTE]
   >
   >* 적응형 양식에서 Adobe Sign을 반드시 사용해야 하는 것은 Adobe Sign 블록을 사용하는 것이 아닙니다. Adobe Sign 블록을 사용하지 않고 서명자에 대한 필드를 추가하지 않으면 서명 문서 하단에 기본 서명 필드가 표시됩니다.
   >* 자동으로 기록 문서를 생성하는 적응형 양식에 대해서만 Adobe Sign 블록을 사용합니다. 사용자 지정 XDP를 사용하여 레코드 문서 또는 적응형 양식 기반의 양식 템플릿을 사용하는 경우 Adobe Sign 블록이 필요하지 않습니다.


1. **Adobe Sign 블록** 구성 요소를 선택하고 **편집** ![aem_6_3_edit](assets/aem_6_3_edit.png) 아이콘을 탭합니다. 필드의 필드와 형식 모양을 추가하는 옵션이 표시됩니다.

   ![adobe sign-block-select-fields](assets/adobe-sign-block-select-fields.png)

   **A.** Adobe Sign 필드를 선택하고 추가합니다. **B.** Adobe Sign 블록을 전체 화면 보기로 확장합니다

1. **Adobe Sign 필드** ![aem_6_3_adobesign](assets/aem_6_3_adobesign.png) 아이콘을 탭합니다. Adobe Sign 필드를 선택하고 추가하는 옵션이 표시됩니다.

   **유형** 드롭다운 필드를 확장하여 Adobe Sign 필드를 선택하고 완료 ![aem_6_3_forms_save](assets/aem_6_3_forms_save.png) 아이콘을 탭하여 선택한 필드를 Adobe Sign 블록에 추가합니다. **유형** 드롭다운 필드에는 서명, 서명자 정보 및 데이터 필드 유형이 포함됩니다. 유형 드롭다운 상자에 나열된 AEM Forms 지원 필드와 Adobe Sign 통합을 사용합니다. Adobe Sign 필드에 대한 자세한 내용은 [Adobe Sign 설명서](https://helpx.adobe.com/sign/help/field-types.html)를 참조하십시오.

   ![adobe-sign-block-fields-options](assets/adobe-sign-block-fields-options.png)

   필드에 고유한 이름을 입력해야 합니다. 필수 필드를 선택하여 표시할 수도 있습니다. **이름** 및 **필수** 옵션 외에도 일부 Adobe Sign 필드에 더 많은 옵션이 있습니다. 예를 들어, 마스크 및 여러 줄. 또한 필드가 동일한 Adobe Sign 블록에 있는지 아니면 다른 블록에 있는지 여부에 따라 각 Adobe Sign 필드에 고유한 이름을 지정합니다.

### 적응형 양식에 Adobe Sign 활성화 {#enableadobsignforanadaptiveform}

기본적으로 Adobe Sign은 적응형 양식에 대해 활성화되지 않습니다. 다음 단계를 수행하여 활성화하십시오.

1. 컨텐츠 브라우저에서 **양식 컨테이너**&#x200B;를 탭하고 **구성** ![구성](assets/configure.png) 아이콘을 탭합니다. 속성 브라우저를 열고 적응형 양식 컨테이너 속성을 표시합니다.
1. 속성 브라우저에서 **전자 서명** 아코디언을 확장하고 **Adobe Sign 활성화** 옵션을 선택합니다. 적응형 양식에 Adobe Sign을 사용할 수 있습니다.

### Adobe Sign Cloud Service 및 서명 순서 선택 {#selectadobesigncloudserviceforanadaptiveform}

AEM Forms 인스턴스에 대해 여러 Adobe Sign 서비스를 구성할 수 있습니다. 각 기능(인사, 재무 등)에 대해 별도의 서비스 세트를 갖는 것이 좋습니다. 서명된 문서의 추적 및 보고가 더 쉬워집니다. 예를 들어, 은행에는 여러 부서가 있습니다. 문서 추적을 더 잘 하기 위해 각 부서에 대해 별도의 구성을 지정할 수 있습니다.
...
1. 이미 구성된 Adobe Sign Cloud Services 목록에서 클라우드 서비스를 선택합니다.
   ...
   **Adobe Sign Cloud Service** 목록이 비어 있는 경우 [AEM Forms으로 Adobe Sign 구성](/help/forms/using/adobe-sign-integration-adaptive-forms.md) 문서에 따라 서비스를 구성하십시오.
...
1. **서명자 수** 대화 상자에서 서명 순서를 선택합니다. Adobe Sign 가수들은 임의의 순서로 적응형 양식 **순차적으로** 또는 **동시에** 서명할 수 있습니다.

   순차적으로, 한 서명자가 한 번에 서명 양식을 수신합니다. 서명자가 문서 서명을 완료하면 다음 서명자에게 양식이 전송됩니다.

   동시에 여러 서명자가 한 번에 양식에 서명할 수 있습니다.

1. [적응형 양식에 서명자](#addsignerstoanadaptiveform) 를 추가하고 완료 아이콘을 탭하여 변경 사항을 저장합니다.
...
1. 컨텐츠 브라우저에서 **양식 컨테이너**&#x200B;를 탭하고 **구성** ![구성](assets/configure.png) 아이콘을 탭합니다. 응용 양식 컨테이너 속성이 있는 속성 브라우저를 엽니다.
1. 속성 브라우저에서 **전자 서명** 아코디언을 확장하고 **Adobe Sign 활성화** 옵션을 선택합니다. 적응형 양식에 Adobe Sign을 사용할 수 있습니다.
1. **서명자 구성에서**&#x200B;서명자 추가&#x200B;**를 누릅니다.** 적응형 양식에 서명자가 추가됩니다. 적응형 양식에 여러 Adobe Sign 서명자를 추가할 수 있습니다.
1. ![전화 세부 사항](assets/phone-details.png)
   ...
   * **서명자와 양식 작성자가 동일합니까? 양식 작성자와 첫 번째 서명자가 동일한 경우**  **예**&#x200B;를 선택합니다. 옵션이 **아니요,**&#x200B;로 설정되어 있으면 적응형 양식에서 서명 단계 구성 요소를 사용하지 마십시오. 양식에 서명 단계 구성 요소가 포함되어 있으면 필드가 자동으로 예로 설정됩니다.
   * **서명자 이메일 주소:** 서명자의 이메일 주소를 지정합니다. 서명자가 지정된 전자 메일 주소에서 서명된 문서/양식을 수신합니다. 양식 필드에 제공된 이메일 주소를 사용하거나, 로그인한 사용자의 AEM 사용자 프로필에서 이메일 주소를 사용하도록 선택하거나, 수동으로 이메일 주소를 입력할 수 있습니다. 이것은 필수 단계입니다. 또한 한 명의 서명자만 구성한 경우, 서명자의 이메일 주소가 AEM 클라우드 서비스를 구성하는 데 사용되는 Adobe Sign 계정과 동일하지 않은지 확인하십시오.
   * **서명자 인증 방법:** 서명을 위한 양식을 열기 전에 사용자를 인증하는 방법을 지정합니다. 전화, 기술 자료 및 소셜 ID 기반 인증 중에서 선택할 수 있습니다.

   >[!NOTE]
   >
   >* 기본적으로 소셜 ID 기반 인증은 Facebook, Google 및 LinkedIn을 사용하여 인증하는 옵션을 제공합니다. Adobe Sign 지원 센터에 문의하여 다른 소셜 인증 공급자를 활성화할 수 있습니다.


   * **채우거나 서명할 Adobe Sign 필드:**  서명자에 대한 Adobe Sign 필드를 선택합니다. 적응형 양식에는 여러 Adobe Sign 필드가 있을 수 있습니다. 서명자에 대해 특정 필드를 활성화하도록 선택할 수 있습니다. 이 필드에는 사용 가능한 모든 Adobe Sign 블록이 표시됩니다. 블록을 선택하면 블록의 모든 필드가 선택됩니다. X 아이콘을 사용하여 필드를 선택 취소할 수 있습니다.
...
그런 다음 적응형 양식에 Adobe Sign 필드를 추가하고, 양식 컨테이너에서 Adobe Sign 을 활성화하고, Adobe Sign Cloud Service을 선택하고, Adobe Sign 서명자를 추가하고, 적응형 양식에 적합한 제출 작업을 선택합니다. 적응형 양식 제출 작업에 대한 자세한 내용은 [제출 작업 구성](/help/forms/using/configuring-submit-actions.md)을 참조하십시오.

또한 Adobe Sign이 활성화된 적응형 양식은 모든 서명자가 양식에 서명한 후에만 제출됩니다. 양식 포털의 보류 중인 서명 섹션에서 부분적으로 서명된 양식을 찾을 수 있습니다. Adobe Sign 구성 서비스는 서명 상태를 확인하기 위해 폴링 Adobe Sign 서버를 [정규 간격](/help/forms/using/adobe-sign-integration-adaptive-forms.md)에 유지합니다. 모든 서명자가 양식 서명을 완료하면 제출 작업 서비스가 시작되고 양식이 제출됩니다. 사용자 지정 제출 작업을 사용 중이며 양식에서 Adobe Sign을 사용하는 경우 사용자 지정 제출 작업을 업데이트하여 제출 작업 서비스를 사용합니다.

>[!NOTE]
>
>적응형 양식의 데이터는 일시적으로 Forms Portal에 저장됩니다. Forms Portal](/help/forms/using/configuring-draft-submission-storage.md)에 [사용자 지정 저장소를 사용하는 것이 좋습니다. 이렇게 하면 PII(개인 식별 정보) 데이터가 AEM 서버에 저장되지 않습니다.
...
Adobe 기호](#enableadobesign)에 대한 적응형 양식 속성을 편집한 후 다음 단계를 수행하여 클라우드 서명 필드를 적응형 양식에 추가합니다.[
...
1. **Adobe Sign 필드** ![aem_6_3_adobesign](assets/aem_6_3_adobesign.png) 아이콘을 탭합니다. Adobe Sign 필드를 선택하고 추가하는 옵션이 표시됩니다.
   ...
   **유형** 드롭다운 필드를 확장하여 **디지털 서명**&#x200B;을 선택하고 완료 ![aem_6_3_forms_save](assets/aem_6_3_forms_save.png) 아이콘을 탭하여 선택한 필드를 Adobe Sign 블록에 추가합니다.
...
1. [서명 단계 구성 요소를 추가 및 구성합니다](#add-and-configure-the-signature-step-component).
...
**Adobe Sign 블록:** Adobe Sign 블록을 사용하여 적응형 양식의 어디에나 Adobe Sign 필드를 추가할 수 있습니다. 서명자에게 특정 필드를 할당하는 데에도 도움이 됩니다. 적응형 양식을 미리 보거나 게시된 Adobe Sign 블록이 기본적으로 표시되지 않습니다. 이러한 블록은 서명 문서에서만 사용할 수 있습니다. 서명 문서에서는 서명자에게 지정된 필드만 활성화됩니다. Adobe Sign 블록은 첫 번째 및 후속 서명자와 함께 사용할 수 있습니다.
