...
1. 에서 JavaScript 파일을 업데이트합니다. `integration/javascript` 폴더를 입력합니다(필요에 따라).
   ...
   * 다음 `getCommitValue` 메서드는 `commit`이벤트가 발생합니다. 일반적으로 드롭다운, 라디오 단추 및 변경 시 발생하는 확인란 요소를 제외한 종료 이벤트입니다. 자세한 내용은 [적응형 Forms 표현식](/help/forms/using/adaptive-form-expressions.md#p-value-commit-script-p).
