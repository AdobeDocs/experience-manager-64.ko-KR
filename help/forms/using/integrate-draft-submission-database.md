...
1. https://호스트 **[!UICONTROL :]** port [*/system*]/console/configMgr의&#x200B;[*AEM 웹 콘솔 구성*]&#x200B;페이지로 이동합니다.
1. 편집 모드에서 **[!UICONTROL Forms 포털 초안 및 제출 구성을]** 열려면 클릭하십시오.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>데이터 소스 이름</td> 
   <td><p>데이터 소스 풀의 드라이버를 필터링하기 위한 데이터 원본 이름</p> <p><strong>참고: </strong><em>샘플 구현에서는 FormsPortal을 데이터 소스 이름으로 사용합니다.</em></p> </td> 
  </tr> 
  <tr> 
   <td>JDBC 드라이버 클래스</td> 
  ...
  <tr> 
   <td>유효성 검사 쿼리</td> 
   <td>SELECT 1(mysql), dual(oracle), SELECT 1(MS Sql Server)(validationQuery) 값의 예</td> 
  </tr> 
  <tr> 
...
1. 데이터베이스 스키마에 테이블(데이터, 메타데이터 및 추가 메타 데이터 가능)이 이미 있는 경우 다음 변경 테이블 쿼리를 실행합니다.
...
1. /libs/fd/af/runtime/clientlibs/guideRuntimeWithXFA로 이동하고 `fp.validation` 값을 포함 **속성에 추가합니다** .
