...
데이터 통합은 양식 데이터 모델을 구성하고 사용하여 작업할 수 있는 양식 데이터 모델 편집기를 제공합니다.
...
1. 읽기 서비스 인수 ![에 대해 aem_6_3_edit](assets/aem_6_3_edit.png) 을 탭하여 인수를 사용자 프로필 속성, 요청 속성 또는 리터럴 값에 바인딩하고 바인딩 값을 지정합니다. 서비스 인수를 지정된 바인딩 특성 또는 리터럴 값에 바인딩합니다. 이 값은 인수로 서비스에 전달되어 데이터 소스의 지정된 값과 관련된 세부 정보를 가져옵니다.

   이 예제에서 `id` 인수는 사용자 프로필의 `empid` 속성 값을 가져와서 읽기 서비스에 인수로 전달합니다. 지정된 데이터 모델 개체에서 연결된 속성의 값을 읽고 `employee` 반환합니다 `empid`. 따라서 양식의 `empid` 필드에 00250을 지정하면 읽기 서비스는 00250 직원 id가 있는 직원의 세부 정보를 읽습니다.

   또한 인수를 필수 또는 선택 사항으로 만들 수도 있습니다.

   ![edit-argument](assets/edit-argument.png)

   AEM 사용자 프로필의 empid 특성에 id 인수 바인딩
...
양식 데이터 모델의 서비스 탭에 `GET LINK` 서비스를 추가하면 속성을 편집하여 출력 모델 객체와 서비스에 사용할 탐색 속성을 선택할 수 있습니다. For example, the following `GET LINK of Person /People` service in the following example uses Trip as the output model object and the navigation property as Trips.

![edit-prop-nav-prop](assets/edit-prop-nav-prop.png)

>[!NOTE]
>
>The values available in the **[!UICONTROL Default Value]** field of the **NavigationPropertyName** argument depend on the state of the **[!UICONTROL Return array?]** toggle button. 활성화되면 컬렉션 유형의 탐색 속성이 표시됩니다.

이 예에서는 출력 모델 개체를 Person으로 선택하고 탐색 속성 인수를 Friends 또는 BestFriend로 선택할 수도 있습니다( **[!UICONTROL 반환 배열 여부에 따라 다름).]** 이 활성화되어 있거나 비활성화되어 있습니다.)

![edit-prop-nav-prop2](assets/edit-prop-nav-prop2.png)

마찬가지로, 양식 데이터 모델에서 연결을 추가할 때 `GET LINK` 서비스를 선택하고 해당 탐색 속성을 구성할 수 있습니다. 그러나 탐색 속성을 선택하려면 [바인딩 대상] 필드 **[!UICONTROL 가]** Literal로 설정되어 있는지 **[!UICONTROL 확인하십시오]**.
...
1. 양식 데이터 모델을 열고 샘플 데이터 **[!UICONTROL 편집을 누릅니다]**. It generates and displays the sample data in Edit Sample Data window.
...
Your form data model is configured but before putting it in use, you may want to test if the configured data model objects and services are working as expected. 데이터 모델 개체 및 서비스를 테스트하려면
...
You have a working form data model that is now ready for use in adaptive forms and interactive communications workflows. 자세한 내용은 양식 데이터 모델 [사용을 참조하십시오](/help/forms/using/using-form-data-model.md).
