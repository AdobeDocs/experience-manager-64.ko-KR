...
AEM Forms은 AEM Forms 앱의 전체 소스 코드를 제공합니다. 소스에는 사용자 정의 작업 공간 응용 프로그램을 만드는 모든 구성 요소가 포함되어 있습니다. 소스 코드 아카이브 `adobe-lc-mobileworkspace-src-<version>.zip`는 소프트웨어 배포 패키지 `adobe-aemfd-forms-app-src-pkg-<version>.zip` 의 일부입니다.
...
1. Visual Studio 솔루션 탐색기에서 프로젝트 **CordovaApp.Windows** 를 마우스 오른쪽 단추로 클릭하고 **스토어 > 앱 패키지 만들기를 선택합니다**.
   ...
   오류가 발생하는 경우 오류 `Retarget to windows 8.1 required`를 마우스 오른쪽 단추로 클릭하고 팝업 메뉴에서 [Windows 8.1로 **재타깃팅]을 선택합니다**.

   ![재타깃팅 솔루션](assets/retarget-solution.png)

1. 앱 패키지 생성 마법사에서 앱을 Windows 스토어에 업로드하지 않으려는 날씨를 선택한 다음 **다음을 클릭합니다**.
...
1. 프로젝트가 빌드되면 다음을 사용하여 앱을 설치할 수 있습니다.
   ...
   * Visual Studio
   ...
   이 `.appx` 패키지를 설치하려면 다음 항목이 필요합니다.

   1. WinJS 라이브러리
   1. 패키지에 자체 서명된 인증서나 VeriSign과 같은 신뢰할 수 있는 기관이 서명한 공용 인증서가 포함되어 있는지 확인합니다.
   1. 개발자 라이선스

   Platforms\windows\AppPackages\CordovaApp.Windows_3.0.2.0_anycpu_Test 디렉토리에는 네 개의 기본 구성 요소가 포함되어 있습니다.

   1. `.appx` 파일
   1. 인증서(현재 Apache Cordova가 자체 서명된 인증서임)
   1. 종속성 폴더
...
1. 설치 프로그램에서 개발자 라이선스를 얻으라는 메시지가 표시됩니다. Microsoft 계정 자격 증명을 사용하여 개발자 라이선스를 획득합니다.
...
Microsoft에서는 VeriSign과 같은 신뢰할 수 있는 권한이 발급된 공용 인증서를 설치할 것을 권장합니다.
...
1. 디바이스에서 실행 `gpedit.msc`
1. 컴퓨터 **구성 > 관리 템플릿 > Windows 구성 요소 > 앱 패키지 배포로 이동합니다**.
1. 신뢰할 수 있는 모든 앱 **의 설치 허용을 마우스 오른쪽 단추로 클릭합니다**.
1. 편집 **을** 클릭하고 **활성화됨을 선택합니다**.
...
자세한 내용을 보려면 [여기를 클릭하십시오](https://blogs.msdn.com/b/mvpawardprogram/archive/2014/03/24/side-loading-deployment-of-windows-store-apps-in-enterprises-step-by-step.aspx).
