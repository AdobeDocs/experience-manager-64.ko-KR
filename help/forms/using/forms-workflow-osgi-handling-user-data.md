...
>[!NOTE]
>
>또한 AEM Forms 앱은 오프라인 모드로 데이터를 저장합니다. 워크플로우 인스턴스의 데이터는 개별 장치에 로컬로 저장되고 앱이 서버와 동기화될 때 Forms 서버에 제출될 수 있습니다.
...
1. CRXDE Lite 콘솔으로 이동하고 워크플로우 인스턴스의 페이로드 경로로 이동한 다음 `payload` 노드를 삭제합니다.
