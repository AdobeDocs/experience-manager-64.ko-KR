...
>[!NOTE]
>
>AEM Dispatcher를 사용하여 적응형 양식을 캐시하는 경우 미리 채워진 데이터가 있는 양식이 포함된 적응형 양식도 캐시합니다. 이러한 양식이 AEM Dispatcher 캐시에서 제공되는 경우 미리 채워진 데이터나 오래된 데이터를 사용자에게 제공할 수 있습니다. 따라서 AEM Dispatcher를 사용하여 사전 채워진 데이터를 사용하지 않는 적응형 양식을 캐싱합니다. 또한 디스패처 캐시는 캐시된 조각을 자동으로 무효화하지 않습니다. 따라서 양식 조각을 캐시하는 데 이 필드를 사용하지 마십시오. 이러한 양식 및 조각의 경우 [적응형 양식 캐시](/help/forms/using/configure-adaptive-forms-cache.md)를 사용하십시오.
