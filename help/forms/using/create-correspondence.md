...
통신 관리에서 [서신 템플릿이 작성되면 최종 사용자/에이전트/청구 조정자는 서신 작성 사용자 인터페이스에서 편지를 열고 데이터를 입력하고, 컨텐츠를 설정하고, 첨부 파일을 관리하여 편지를 작성할 수 있습니다](/help/forms/using/create-letter.md). 마지막으로, 청구 조정자나 대리점은 미리 보기 모드에서 컨텐츠를 관리하고 편지를 제출할 수 있습니다.
...
1. 데이터 사전 기반 서신의 경우 [미리 보기] > **[미리 보기** ]를 **선택합니다**. 또는 데이터 사전 기반이 아닌 서신의 경우 미리 보기를 **선택합니다**. 문자 위로 마우스를 가져가면(선택하지 않음) 편지 미리 보기 아이콘을 눌러 미리 볼 수도 있습니다.
   ...
   >[!NOTE]
   >
   >데이터 사전이 문자와 연결되지 않으면 문자 미리 보기가 열립니다. 그렇지 않으면 문자가 데이터 사전을 기반으로 하는 경우, 통신 관리에서는 [미리 보기] 메뉴에 [미리 보기] 및 [사용자 정의] 옵션을 표시하고 두 옵션 중 하나를 선택할 수 있습니다. 테스트 데이터를 데이터 사전에 연결할 수도 있습니다. 데이터 사전 [에 테스트 데이터가](/help/forms/using/data-dictionary.md#p-working-with-test-data-p)연결되어 있으면 미리 보기 옵션을 선택하면 테스트 데이터가 채워진 상태로 일반 미리 보기가 열립니다.
...
1. [미리 보기] > [사용자 **정의** ]를 **선택한**&#x200B;경우 대화 상자가 열립니다. 대화 상자에서 데이터 사전에 해당하는 데이터 파일을 선택하여 문자를 미리 본 다음 미리 보기를 **선택합니다**. 데이터 파일은 특정 서신의 데이터 사전을 기반으로 만들어집니다. 데이터 파일에 대한 자세한 내용은 [데이터 사전을 참조하십시오](/help/forms/using/data-dictionary.md#p-working-with-test-data-p).
...
1. 필요에 따라 [콘텐트] 탭에서 콘텐트 모듈을 편집합니다. 콘텐츠 계층 구조에서 관련 콘텐츠 모듈에 초점을 맞추려면 문자 미리 보기에서 관련 라인 또는 단락을 누르거나 컨텐츠 계층 구조에서 직접 컨텐츠 모듈을 누를 수 있습니다.

   예를 들어 아래 그래픽에서 &quot;We reviewed..&quot; 라인이 선택되고 컨텐츠 탭에서 관련 컨텐츠 모듈이 선택됩니다.
   ...
   통신 사용자 인터페이스의 다양한 모듈에 사용할 수 있는 작업에 대한 자세한 내용은 통신 사용자 [생성 인터페이스에서 사용 가능한 작업 및 정보를 참조하십시오](create-letter.md).
...
1. 인라인 또는 편집 가능한 텍스트 모듈을 편집하려면 관련 **편집** 아이콘( ![edittextmodule](assets/edittextmodule.png))을 누르거나 편지 미리 보기에서 관련 텍스트 모듈을 두 번 클릭합니다.
   ...
   텍스트 편집기에서 다양한 키보드 단축키를 사용하여 텍스트를 관리, 편집 및 포맷할 수도 있습니다. 통신 관리 키보드 단축키의 [텍스트 편집기](/help/forms/using/keyboard-shortcuts.md#correspondence-management) 키보드 단축키에 대한 자세한 내용을 살펴보십시오.

   ![5_edittextmodule](assets/5_edittextmodule.png)

   문서의 다른 응용 프로그램에 있는 텍스트 단락 중 하나를 재사용할 수 있습니다. MS Word, HTML 페이지 또는 기타 모든 애플리케이션에서 바로 텍스트를 복사하여 붙여넣을 수 있습니다.
   ...
   ![pretteditabllodule](assets/pastetexteditablemodule.png)
...
1. 편집 가능한 인라인 모듈에서 텍스트의 일부를 강조 표시하려면 텍스트를 선택하고 강조 색을 누릅니다.
   ...
   기본 색상 팔레트에 있는 기본 색상 **[A]** 를 **** 직접 누르거나 슬라이더 **[B를]** 사용하여 적절한 색상의 음영을 선택할 수 있습니다.

   또한 고급 탭으로 이동하여 적절한 색조, 밝기 및 채도 **[C]** 를 선택하여 정확한 색상을 만든 다음 선택 **[D를]** 눌러 색상을 적용하여 텍스트를 강조 표시할 수도 있습니다.
...
1. 채우지 않은 변수를 올바르게 입력하면 채우지 않은 변수의 수가 0으로 설정됩니다.
   ...
   통신 사용자 인터페이스에서 채워지지 않은 변수 개수는 최소 하나의 변수가 포함된 모듈의 계층 구조의 각 수준에 표시됩니다. 모듈에 채워지지 않은 변수가 있으면 변수, 모듈, 대상 영역 및 문자 템플릿 수준에서 카운트가 표시됩니다.

   채워지지 않은 변수 개수에는 다음이 포함됩니다.

   * 보호되지 않은 데이터 사전 및 자리 표시자 변수만 있습니다. 변수 개수에는 레이아웃 또는 보호된 데이터 사전 변수가 포함되지 않습니다.
   * 필수 필드.
   * 레이아웃 필드가 필수이고 사용자에게 바인딩되어 있는 경우.
   * 고유 변수 인스턴스만. 모듈, 대상 영역 또는 문자 템플릿에 동일한 변수의 인스턴스가 두 개 이상 포함되어 있으면 개수가 1(하나)으로 표시됩니다. 하지만 각 인스턴스에 대해 카운트가 1로 표시됩니다.

   채워지지 않은 변수 개수에는 선택 해제된 모듈이 포함되지 않습니다. 모듈이 문자 템플릿에 포함되어 있지만 편지에 없는 경우 이 모듈의 채워지지 않은 변수에 대한 카운트가 표시되지 않습니다.

   대상 영역, 모듈 및 변수의 경우 문자 템플릿의 각 개체 오른쪽에 개수가 표시됩니다. 그러나 전체 템플릿의 경우 [통신 만들기] 상태 표시줄에 카운트가 표시됩니다.
...
* 주문 잠금(정보):콘텐츠 순서를 변경할 수 없음을 나타냅니다.
* 채워지지 않은 값(정보):대상 영역의 채워지지 않은 변수의 수를 나타냅니다.

**모듈**

* 선택(눈 아이콘):문자에서 모듈을 포함\제외합니다.
* 불릿 건너뛰기(목록 모듈 및 하위 모듈에 적용 가능):특정 모듈에서 글머리 기호를 건너뜁니다.
...
1. 나중에 계속 작업할 수 있는 초안으로 편지를 저장하려면 초안으로 저장을 누릅니다. 이 옵션을 사용하려면 문자를 [게시해야 합니다](/help/forms/using/publishing-unpublishing-forms.md#publishanasset). 자세한 내용은 초안 저장 및 서신 인스턴스 [제출에서 초안 인스턴스를 참조하십시오](#savingdrafts).
...
1. 병합된 PDF 미리 보기가 만족스러운 경우 **제출을** 눌러 편지를 제출합니다. 또는 문자를 변경하려면 미리 보기 **종료** 를 눌러 편지 작성의 UI 미리 보기로 돌아가 문자를 변경합니다. 제출을 누르면 Publish 인스턴스에서 Letter 인스턴스 관리 구성이 활성화되면 전송 편지 인스턴스가 생성됩니다.
   ...
   나중에 문자를 변경하려면 초안으로 편지를 저장할 수도 있습니다.
...
* **초안 인스턴스**:초안 인스턴스는 미리 보고 있는 글자의 현재 상태를 캡처합니다. 초안 인스턴스를 저장하려면 먼저 문자를 참조하는 서신과 모든 자산이 게시됨 상태인지 확인합니다. 편지 게시에 대한 자세한 내용은 자산 [게시를 참조하십시오](/help/forms/using/publishing-unpublishing-forms.md#publishanasset). 편지를 초안으로 저장하려면 먼저 편지를 게시해야 합니다. 편지를 게시할 때 편지 버전, 종속 자산 및 데이터가 해당 시점에 생성되기 때문입니다. 게시된 문자 버전은 사용자나 다른 사용자가 편집할 수 없으며 나중에 게시된 버전과 예기치 않은 불일치도 없이 복원할 수 있습니다. 나중에 이 인스턴스로 돌아가 중단한 부분부터 다시 시작할 수 있습니다.
...
이 옵션은 일반적으로 회사 방화벽 외부에 있는 게시 인스턴스의 사용자 데이터를 저장할 수 있다는 우려 사항이 있는 사람에게 적용됩니다. 원격 저장을 활성화하면 문자 인스턴스가 게시 인스턴스에 저장되지 않지만 LiveCycle 클라이언트 SDK 구성을 통해 지정된 처리 작성자에 원격으로 저장됩니다.
...
* 서신 템플릿과 연결된 게시 프로세스가 서신 작성 사용자 인터페이스를 사용하여 서신 전송 또는 완료 작업의 일부로 로드되면 첨부 파일이 AttachmentDocs 매개 변수의 목록&lt;com.adobe.idp.Document>으로 전달됩니다.
* 이메일 및 인쇄와 같은 기본 제공 메커니즘은 생성된 서신의 PDF와 함께 첨부 파일을 제공합니다.
