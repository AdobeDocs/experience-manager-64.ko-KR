...
양식 데이터 통합 및 양식 데이터 모델에 대한 자세한 내용은 [AEM Forms 데이터 통합](/help/forms/using/data-integration.md)을 참조하십시오.
...
* [첫 번째 적응형 양식 만들기](/help/forms/using/create-your-first-adaptive-form.md)의 전제 조건 섹션에 명시된 샘플 데이터가 있는 MySQL 데이터베이스
* [JDBC 데이터베이스 드라이버 번들링](/help/sites-developing/jdbc.md#bundling-the-jdbc-database-driver)에 설명된 MySQL JDBC 드라이버용 OSGi 번들
* 첫 번째 자습서 [적응형 양식 만들기](/help/forms/using/create-adaptive-form.md)에 설명된 적응형 양식

## 1단계:MySQL 데이터베이스를 데이터 소스 {#config-database}로 구성

양식 데이터 모델을 만들도록 서로 다른 유형의 데이터 소스를 구성할 수 있습니다. 이 자습서에서는 사용자가 구성하고 샘플 데이터로 채워진 MySQL 데이터베이스를 구성합니다. 지원되는 다른 데이터 소스에 대한 정보 및 구성 방법은 [AEM Forms 데이터 통합](/help/forms/using/data-integration.md)을 참조하십시오.

MySQL 데이터베이스를 구성하려면 다음을 수행합니다.

1. MySQL 데이터베이스의 JDBC 드라이버를 OSGi 번들로 설치합니다.

   1. AEM Forms 작성자 인스턴스에 관리자로 로그인하고 AEM 웹 콘솔 번들로 이동합니다. 기본 URL은 [http://localhost:4502/system/console/bundles](http://localhost:4502/system/console/bundles)입니다.
...
1. MySQL 데이터베이스를 데이터 소스로 구성:
   ...
   1. 구성 대화 상자에서 다음 세부 정보를 지정합니다.
      ...
      * **유효성 검사 쿼리:** SQL SELECT 쿼리를 지정하여 풀의 연결을 확인합니다. 쿼리는 하나 이상의 행을 반환해야 합니다. 예: **select &amp;ast;고객 세부 사항**&#x200B;에서.
...
1. 양식 데이터 모델 만들기 대화 상자에서 양식 데이터 모델에 대해 **name**&#x200B;을 지정합니다. 예: **customer-shipping-billing-details**. **다음**&#x200B;을 누릅니다.
...
1. 데이터 모델 개체에 대한 읽기 및 쓰기 서비스를 구성합니다.
   ...
   1. 읽기 서비스 드롭다운에서 **get**&#x200B;을 선택합니다. 사용자 지정 세부 정보 데이터 모델 개체의 기본 키인 **id** 인수가 자동으로 추가됩니다. ![aem_6_3_edit](assets/aem_6_3_edit.png)을 누르고 다음과 같이 인수를 구성합니다.

      ![읽기 기본값](assets/read-default.png)

   1. 마찬가지로 쓰기 서비스로 **업데이트**&#x200B;를 선택합니다. **customerdetails** 개체는 인수로 자동으로 추가됩니다. 인수는 다음과 같이 구성됩니다.
   ...
   1. **완료**&#x200B;를 눌러 데이터 모델 개체 속성을 저장합니다. 그런 다음 **저장**&#x200B;을 눌러 양식 데이터 모델을 저장합니다.
...
1. **서비스** 탭으로 이동하여 **get** 및 **update** 서비스를 구성합니다.
   ...
   1. **get** 서비스를 선택하고 **속성 편집**&#x200B;을 누릅니다. 속성 대화 상자가 열립니다.
   ...
   1. **업데이트** 서비스를 선택하고 **속성 편집**&#x200B;을 누릅니다. 속성 대화 상자가 열립니다.
   ...
   1. 속성 편집 대화 상자에서 다음을 지정합니다.
      ...
      **Done**&#x200B;을 누릅니다. MySQL 데이터베이스에서 고객 세부 사항을 업데이트하기 위한 **업데이트** 서비스가 구성되어 있습니다.
...
1. **모델** 탭으로 이동하여 **고객 세부 사항** 데이터 모델 개체를 선택하고 **테스트 모델 개체**&#x200B;를 탭합니다.
1. **테스트 모델 / 서비스** 창의 **모델 / 서비스 선택** 드롭다운에서 **모델 개체 읽기**&#x200B;를 선택합니다.
1. **customerdetails** 섹션에서 구성된 MySQL 데이터베이스에 있는 **id** 인수의 값을 지정하고 **테스트**&#x200B;를 누릅니다.

   지정된 ID와 연결된 고객 세부 정보를 가져오고 아래에 표시된 대로 **출력** 섹션에 표시됩니다.
