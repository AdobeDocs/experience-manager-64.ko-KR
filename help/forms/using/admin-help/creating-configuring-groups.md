...
* @adobe.com과 같은 이메일 도메인을 기반으로 동적 그룹을 자동으로 만들 수 있도록 설정합니다. 이 기능을 활성화하면 사용자 관리에서 AEM Forms 데이터베이스의 각 고유 이메일 도메인에 대한 동적 그룹을 만듭니다. User Management가 AEM Forms 데이터베이스에서 새 이메일 도메인을 검색하는 빈도를 지정하려면 cron 표현식을 사용합니다. 이러한 동적 그룹은 DefaultDom 로컬 도메인에 추가되며 &quot;All users with an *[이메일 도메인] *메일 ID가 있는 모든 사용자&quot;라고 합니다.
