...
**기본 URL:** 서버 이름과 포트를 포함하는 기본 문서 보안 URL. 기본에 추가된 정보는 연결 URL을 만듭니다. 예를 들어 웹 페이지에 액세스하기 위해 /edc/Main.do이 추가됩니다. 또한 사용자는 이 URL을 통해 외부 사용자 등록 초대에 응답합니다.
...
오프라인 임대 및 동기화 작동 방법에 대한 자세한 내용은 오프라인 [임대 및 동기화 구성에 대한 입문서를 참조하십시오](https://blogs.adobe.com/security/2009/05/primer_on_configuring_offline.html).
...
***참고&#x200B;**: 이 대화 상자의 너비와 높이 제한은 다음과 같습니다.*너비: 최소 = 400, 최대 = 900

높이: 최소 = 450; 최대 = 800

**클라이언트 자격 증명 캐싱 사용:** 사용자가 자격 증명(사용자 이름 및 암호)을 캐시하도록 하려면 이 옵션을 선택합니다. 사용자 자격 증명이 캐시되면 문서를 열 때마다 또는 Adobe Acrobat의 [보안 정책 관리] 페이지에서 [새로 고침] 단추를 클릭할 때마다 사용자 자격 증명을 입력할 필요가 없습니다. 사용자가 자격 증명을 다시 제공해야 하기 전 일 수를 지정할 수 있습니다. 일 수를 0으로 설정하면 자격 증명을 무기한으로 캐시할 수 있습니다.
...
1. 서버 구성 페이지에서 확장 인증을 활성화합니다. 서버 [구성 설정을 참조하십시오](configuring-client-server-options.md#server-configuration-settings).
1. 사용자 관리 구성 파일에 필요한 SSO 리디렉션 URL을 추가합니다. 확장 인증을 [위해 SSO 리디렉션 URL 추가를 참조하십시오](configuring-client-server-options.md#add-sso-redirect-urls-for-extended-authentication).
...
1. 노드에서 `AllowedUrls` 다음 줄을 추가합니다. `<entry key="sso-l" value="/ssoexample/login.jsp"/> <entry key="sso-s" value="/ssoexample"/> <entry key="sso-o" value="/ssoexample/logout.jsp"/>`
...
document security는 인터넷 또는 네트워크에 연결하지 않고도 오프라인으로 정책으로 보호된 문서를 사용할 수 있는 기능을 제공합니다. 이 기능을 사용하려면 사용자 및 그룹에 대한 문서 권한 [지정에 설명된 대로 정책에서 오프라인 액세스를 허용해야 합니다](/help/forms/using/admin-help/creating-policies.md#specify-the-document-permissions-for-users-and-groups). 이러한 정책을 갖는 문서를 오프라인으로 사용하려면 먼저 수신자가 온라인 상태에서 문서를 열고 메시지가 표시되면 예를 클릭하여 오프라인 액세스를 활성화해야 합니다. 수신자는 자신의 ID 인증을 요청할 수도 있습니다. 그러면 받는 사람이 정책에 지정된 오프라인 임대 기간 동안 오프라인으로 문서를 사용할 수 있습니다.

오프라인 제공 기간이 끝나면 수신자는 온라인으로 문서를 열거나 Acrobat 또는 Acrobat Reader DC 확장 메뉴 명령을 사용하여 동기화하여 문서 보안과 다시 동기화해야 합니다. ( *Acrobat 도움말* 또는 해당 *Acrobat Reader DC 확장 도움말*&#x200B;참조)
...
문서 보안은 암호화 알고리즘과 라이센스를 사용하여 문서를 보호합니다. 문서를 암호화할 때 문서 보안은 클라이언트 응용 프로그램으로 전달하는 *DocKey라는* 암호 해독 키를 생성하고 관리합니다. 문서를 보호하는 정책이 오프라인 액세스를 허용하는 경우, *주* 키라는 오프라인 키도 문서에 대한 오프라인 액세스 권한이 있는 각 사용자에 대해 생성됩니다.

>[!NOTE]
>
>주요 키가 없는 경우 문서 보안은 문서 보안을 위해 해당 키를 생성합니다.
...
1. 10분 정도 기다리십시오 다음 로그 메시지가 서버 로그에 나타납니다. `Done RightsManagement key rollover for`*아니요&#x200B;*.`principals`. 여기서* N은&#x200B;*문서 보안 시스템의 사용자 수입니다.
1. 지금 롤오버 키를 클릭한 다음 확인을 클릭합니다.
1. 10분 정도 기다리십시오
...
1. 각 이벤트 카테고리 아래에서 예를 선택한 경우 다음 작업 중 하나를 수행하여 감사할 옵션을 선택합니다.
   ...
   * 일부 이벤트만 감사하려면 [모두]를 선택 해제한 다음 감사할 이벤트 옆의 확인란을 선택합니다.
      ...
      자세한 내용은 [이벤트 감사 옵션을 참조하십시오](configuring-client-server-options.md#event-auditing-options).
...
**저해상도 인쇄** 수신자는 저해상도 옵션을 지정한 정책으로 보호된 문서를 인쇄합니다.

**고해상도 인쇄:** 받는 사람은 고해상도 옵션이 지정된 정책으로 보호된 문서를 인쇄합니다.

**문서에 주석 추가:** 수신자가 PDF 문서에 주석을 추가합니다.

**문서 폐지:** 사용자나 관리자가 정책으로 보호된 문서에 대한 액세스를 폐지합니다.

**문서 취소:** 사용자나 관리자는 정책으로 보호된 문서에 대한 액세스를 다시 가져옵니다.
...
**초대된 사용자:** Document security에서는 사용자가 등록하도록 초대합니다.
...
**동기화된 클라이언트** 클라이언트 응용 프로그램은 정보를 서버와 동기화하여 오프라인 액세스를 허용해야 합니다.
...
시작하기 전에 서버 감사를 사용하도록 설정했는지 확인하십시오. 감사 [에 대한 자세한 내용은 이벤트 감사 및 개인 정보 설정](configuring-client-server-options.md#configuring-event-auditing-and-privacy-settings) 구성을 참조하십시오.

1. 관리 콘솔에서 서비스 > Document Security > 구성 > 감사 및 개인 정보 설정을 클릭합니다.
1. 확장 사용 추적을 구성하려면 추적 활성화에서 예 또는 아니오를 선택합니다.
1. 로그인 페이지에서 자세한 사용 데이터 수집 허용 확인란을 선택하려면 추적 활성화 기본값 아래에서 예 또는 아니요를 선택합니다.

추적된 이벤트를 보려면 [이벤트] 페이지에서 [문서 이벤트] 필터를 사용할 수 있습니다. JavaScript를 사용하여 추적한 이벤트는 자세한 사용 추적으로 레이블이 지정됩니다. 이벤트에 대한 자세한 [내용은 이벤트](/help/forms/using/admin-help/monitoring-events.md#monitoring-events) 모니터링을 참조하십시오.
...
**시작 텍스트:** &quot;사용자 이름과 암호를 사용하여 로그인하십시오&quot;와 같은 환영 메시지 텍스트. 환영 메시지 텍스트에는 문서 보안에 로그인하는 방법 및 조직의 관리자 또는 기타 지원 담당자에게 도움을 요청하는 방법에 대한 정보가 포함되어 있어야 합니다. 예를 들어 외부 사용자가 암호를 잊거나 등록 또는 로그인 프로세스에 대한 지원이 필요한 경우 관리자에게 문의해야 할 수 있습니다. 시작 텍스트의 최대 길이는 512자입니다.
...
**신뢰할 수 있는 자격 증명 발급자만 표시:** 이 옵션을 선택하면 클라이언트 응용 프로그램은 AEM 양식이 트러스트하도록 구성된 자격 증명 발급자의 인증서만 사용자에게 제공합니다(인증서 및 자격 증명 관리 참조). 이 옵션을 선택하지 않으면 사용자 시스템에 모든 인증서 목록이 표시됩니다.

## 동적 워터마크 구성 {#configure-dynamic-watermarks}

문서 보안을 사용하면 정책을 만들 때 적용할 수 있는 동적 워터마크 옵션에 대한 기본 설정을 구성할 수 있습니다. 워터마크는 *문서의 텍스트 위에 겹쳐진 이미지입니다* . 이 기능은 문서의 컨텐츠를 추적하는 데 유용하며 컨텐츠의 불법 사용을 식별하는 데 도움이 됩니다.
...
문서 보안 구성 파일을 수정하여 워터마크 요소 수와 PDF 파일 크기의 제한을 변경할 수 있습니다. 워터마크 [구성 매개 변수 변경을 참조하십시오](configuring-client-server-options.md#change-the-watermark-configuration-parameters).
...
1. 이름 상자에 새 워터마크 이름을 입력합니다.
   ...
   ***참고&#x200B;**: 워터마크 또는 워터마크 요소의 이름이나 설명에 일부 특수 문자를 사용할 수는 없습니다. 정책 편집을 위한 고려 사항[에 나열된 제한 사항을 참조하십시오](/help/forms/using/admin-help/creating-policies.md#considerations-for-editing-policies).*
...
1. 다양한 워터마크 옵션으로 PDF를 선택한 경우 [워터마크 PDF 선택] 옆에 있는 **찾아보기를** 클릭하여 워터마크로 사용할 PDF 문서를 선택합니다.
...
초대받은 사용자가 활성화되면 해당 사용자는 로컬 사용자가 됩니다. 로컬 사용자는 초대된 사용자 및 로컬 사용자 영역을 사용하여 구성 및 관리할 수 있습니다. (초대된 사용자 [및 로컬 사용자 계정 관리를 참조하십시오](/help/forms/using/admin-help/invited-local-user-accounts.md#managing-invited-and-local-user-accounts).)
...
**이메일 컨텐츠 유형:** 텍스트/일반 또는 텍스트/html과 같은 허용되는 MIME 형식입니다.

**이메일 인코딩:** 이메일 메시지를 보낼 때 사용할 인코딩 형식입니다. 인코딩을 지정할 수 있습니다(예: 유니코드의 경우 UTF-8, 라틴어의 경우 ISO-8859-1). 기본값은 UTF-8입니다.

**리디렉션 이메일 주소:** 이 설정의 이메일 주소를 지정하면 새 초대장이 제공된 주소로 전송됩니다. 이 설정은 테스트용으로 유용할 수 있습니다.
...
**보낸 사람:** 정품 인증 이메일이 전송되는 이메일 주소입니다. 이 이메일 주소는 등록자의 이메일 주최자로부터 전달 실패 통지서를 받고 수신자가 등록 이메일에 회답하여 보내는 메시지도 받습니다. 보낸 사람 이메일 주소의 기본 형식은 postmaster@[your_installation_domain].com입니다.

**제목:** 정품 인증 이메일 메시지의 기본 제목입니다.

**시간 초과:** 사용자가 계정을 활성화하지 않을 경우 활성화 초대가 만료된 후 일 수입니다. 기본값은 30일입니다.

**메시지:** 메시지 본문에 받는 사람의 사용자 계정을 활성화해야 한다는 메시지가 표시되는 텍스트입니다. 또한 관리자에게 연락하여 새 암호를 받는 방법과 같은 정보를 포함시킬 수도 있습니다.
...
**메시지:** 받는 사람의 외부 사용자 암호가 재설정되었음을 나타내는 메시지가 메시지 본문에 나타나는 텍스트입니다.
...
**표시되는 사용자 및 그룹 탭:** 사용자 검색 결과에 표시되는 사용자와 그룹을 제어하는 데 사용합니다. 슈퍼 사용자 또는 정책 집합 관리자는 사용자 관리에서 만든 도메인을 선택하여 각 정책 집합에 대해 표시되는 사용자 및 그룹에 추가해야 합니다. 이 목록은 정책 집합 코디네이터에서 볼 수 있으며 정책에 추가할 사용자를 선택할 때 정책 집합 코디네이터가 검색할 수 있는 도메인을 제한하는 데 사용됩니다.
...
1. 편집기에서 구성 파일을 열고 노드를 `PolicyServer` 찾습니다. 해당 노드 아래에서 노드를 `ServerSettings` 찾습니다.
1. 노드에서 `ServerSettings` 다음 항목을 추가한 다음 파일을 저장합니다.

   `<entry key="BackgroundSyncFrequency" value="`*time *`"/>`

   여기서 *time은* 자동 백그라운드 동기화 사이의 초 수입니다. 이 값을 다음으로 보낸 경우 동기화가 항상 `0`발생합니다. 기본값은 `14400` 초(4시간마다)입니다.
...
* **애플리케이션 호환성:** 일부 응용 프로그램이 Document Security 서버의 정책 또는 비헤이비어와 호환되지 않을 수 있습니다.
...
1. 문서 보안 구성 파일을 내보냅니다. 자세한 내용은 [문서 보안 구성 파일](configuring-client-server-options.md#manually-editing-the-document-security-configuration-file)수동 편집을 참조하십시오.
1. 편집기에서 구성 파일을 열고 노드를 `ServerSettings` 찾습니다.
...
1. 노드의 `SmtpUseTls` 키 값 `ExternalUser` 을 **true로 설정합니다**.
1. 노드의 `SmtpUseSsl` 키 값을 `ExternalUser` false로 **설정합니다**.
1. Save the `config.xml`.
1. 구성 파일을 가져옵니다. 자세한 내용은 [문서 보안 구성 파일](configuring-client-server-options.md#manually-editing-the-document-security-configuration-file)수동 편집을 참조하십시오.

### Document Security 문서의 SOAP 끝점 비활성화 {#disable-soap-endpoints-for-document-security-documents}
...
### 문서 보안 서버 확대 확장성 {#increasingscalability}
...
1. ServerSettings 노드에서 `DisableGlobalOfflineSynchronizationData`속성 값을 로 설정합니다 `true`.
