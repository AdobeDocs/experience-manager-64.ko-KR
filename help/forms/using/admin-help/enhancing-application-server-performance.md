...
실행 중인 AEM 양식 모듈 수와 애플리케이션에 액세스하는 동시 사용자 수에 따라 데이터 소스에 대한 액세스가 중요할 수 있습니다. 연결 풀링을 사용하여 데이터 소스 액세스를 최적화할 수 있습니다. *연결 풀링은* 응용 프로그램 또는 서버 개체가 데이터베이스에 액세스해야 할 때마다 새 데이터베이스 연결을 만드는 오버헤드를 방지하는 데 사용되는 기술입니다. 연결 풀링은 보통 웹 기반 및 엔터프라이즈 응용 프로그램에서 사용되며, 응용 프로그램 서버에서 처리되지만 이에 국한되지 않습니다.
...
### DB2용 WebSphere 연결 풀 설정 구성 {#configure-connection-pool-settings-for-websphere-for-db2}
...
### Oracle용 WebSphere 연결 풀 설정 구성 {#configure-connection-pool-settings-for-websphere-for-oracle}

1. 탐색 트리에서 리소스 > JDBC > JDBC 제공자를 클릭합니다. 오른쪽 창에서 생성한 Oracle JDBC 드라이버 데이터 소스를 누릅니다.
1. 추가 속성에서 데이터 소스를 클릭한 다음 IDP_DS를 선택합니다.
1. 다음 화면의 추가 속성에서 연결 풀 속성을 클릭하고 최대 연결 상자 및 최소 연결 상자에 값을 입력합니다.
1. [확인] 또는 [적용]을 클릭한 다음 [구성에 직접 저장]을 기본 클릭합니다.
...
Configuration Manager를 실행 중이거나 명령줄 유틸리티 *ejb(Enterprise JavaBeans)를 사용하여 코드를 생성하려 할 경우* OutOfMemory 오류가 발생하면 JVM에 할당된 메모리 양을 증가시킵니다.
...
1. 매개 변수 `-Xmx256M` 를 찾아 같은 더 높은 값으로 변경합니다 `-Xmx1024M`.
1. 파일을 저장합니다.
1. 명령을 `ejbdeploy` 실행하거나 Configuration Manager를 사용하여 재배포합니다.
...
### 연결 풀링을 위해 Windows Server 구성 {#configure-your-windows-server-for-connection-pooling}

1. 시작 > 실행을 클릭하여 레지스트리 편집기를 시작하고 열기 상자에 입력한 다음 확인 `regedit` 을 클릭합니다.
...
1. 이름 상자에 `TcpTimedWaitDelay`
   ...
   >[!NOTE]
   >
   >깜박이는 커서가 보이지 않고 상자 `New Value #` 안에 있는 경우 오른쪽 패널 내부를 마우스 오른쪽 단추로 클릭하고 이름 변경을 선택하고 이름 상자에 입력합니다 `TcpTimedWaitDelay`*.*

1. 값 이름 MaxUserPort, MaxHashTableSize 및 MaxFreeTcbs에 대해 4단계를 반복합니다.
1. 오른쪽 창 내부를 두 번 클릭하여 TcpTimedWaitDelay 값을 설정합니다. [기준]에서 [소수]를 선택하고 값 상자에 을 입력합니다 `30`.
1. 오른쪽 창 내부를 두 번 클릭하여 MaxUserPort 값을 설정합니다. [기준]에서 [소수]를 선택하고 값 상자에 을 입력합니다 `65534`.
1. 오른쪽 창 내부를 두 번 클릭하여 MaxHashTableSize 값을 설정합니다. [기준]에서 [소수]를 선택하고 값 상자에 을 입력합니다 `65536`.
1. 오른쪽 창 내부를 두 번 클릭하여 MaxFreeTcbs 값을 설정합니다. [기준]에서 [소수]를 선택하고 값 상자에 을 입력합니다 `16000`.
