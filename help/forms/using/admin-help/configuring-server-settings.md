...
* **전자 메일 설정** 보내는 이메일 메시지와 해당 메시지에 사용된 이메일 서버 설정과 함께 사용할 수 있도록 합니다. (자세한 내용은 [전자 메일 설정 구성](configuring-server-settings.md#configuring-email-settings))
* **작업 알림 설정** 전자 메일 알림에 전송된 메시지를 최종 사용자 및 그룹에 대한 작업과 관련하여 활성화, 비활성화 또는 수정합니다. (자세한 내용은 [사용자 및 그룹에 대한 알림 구성](configuring-server-settings.md#configuring-notifications-for-users-and-groups))
* **관리자 알림 설정** 관리 작업을 위해 이메일 알림에 전송된 메시지를 활성화, 비활성화 또는 수정합니다. (자세한 내용은 [관리자용 알림 구성](configuring-server-settings.md#configuring-notifications-for-administrators))
...
AEM Forms에서 사용자로부터 들어오는 전자 메일 메시지를 받고 처리할 수 있도록 하려면 전체 작업 서비스에 대한 전자 메일 끝점을 만듭니다. (자세한 내용은 [전체 작업 서비스에 대한 전자 메일 끝점 만들기](/help/forms/using/admin-help/configuring-email-endpoints.md#create-an-email-endpoint-for-the-complete-task-service)).
...
1. 이메일 주소 상자에 Forms Workflow에서 보내는 이메일 메시지의 반환 주소로 사용할 이메일 주소를 입력합니다.
   ...
   >[!NOTE]
   >
   >Microsoft Exchange Server를 사용 중인데 전자 메일 주소가 잘못된 전자 메일 주소인 경우 Microsoft Exchange Server에서 배포 목록에 전자 메일을 보내지 못합니다. 문제를 해결하려면 **외부 통신 활성화** Microsoft Exchange Server의 모든 배포 목록에 대해 개별적으로 선택할 수 있습니다.
...
기본적으로 AEM Forms에서 보내는 이메일에는 Flex 작업 공간에 대한 링크(JEE에서 AEM Forms용으로 더 이상 사용되지 않음)가 포함되어 있습니다. AEM Forms Workspace에 대한 링크가 있는 이메일을 보내도록 AEM Forms를 구성할 수 있습니다. Flex 작업 공간 의 이점(JEE에서 AEM Forms용으로 사용되지 않음)에 대해 자세히 알아보려면 [이](/help/forms/using/features-html-workspace-available-flex.md) 문서.
...
1. 알림 템플릿 상자에서 전자 메일 메시지 본문에 대한 텍스트를 입력합니다. 이 필드는 기본 텍스트로 미리 채워집니다. 이 필드 사용자 지정에 대한 자세한 내용은 [알림 콘텐츠 사용자 지정](configuring-server-settings.md#customizing-the-content-of-notifications).
...
작업 알림 및 관리자 알림 페이지에서는 알림 메시지를 사용자 정의할 수 있는 몇 가지 기능을 제공합니다.
...
솔루션이 클러스터된 환경에 배포되면 을 바꿉니다 `@@notification-host@@` 클러스터 주소로

`<`*포트* `>` 는 애플리케이션 서버의 HTTP 리스너 포트 번호입니다. 지원되는 응용 프로그램 서버의 기본 HTTP 리스너 포트는 다음과 같습니다.
...
이러한 URL이 올바르게 작동하도록 하려면 를 `<`*포트* `>` 사용자 환경에 적합한 포트 번호를 사용합니다.
...
변수 선택기 목록에서는 제목 또는 알림 템플릿 상자로 드래그하여 놓을 수 있는 유용한 변수를 제공합니다. 제목 또는 알림 템플릿 상자에 변수를 놓으면 실제 양식 워크플로우 변수 이름이 바뀌며 두 @ 기호가 사용됩니다. 예를 들면 다음과 같습니다. `@@taskid@@`.

사용자 및 그룹에 대한 미리 알림, 작업 지정 및 기한의 경우 제목 및 통지 템플리트 상자에서 다음 변수를 사용할 수 있습니다.

**설명** Workbench에서 프로세스의 사용자 단계(시작 지점, 태스크 지정 작업 또는 복수 태스크 지정 작업)에 정의된 내용 속성의 컨텐츠입니다.
...
**타키드** 현재 작업의 고유 식별자입니다.

**작업** 수신자가 클릭할 수 있는 유효한 경로(예: 승인, 거부)의 번호 목록을 생성합니다.
...
관리자는 분기 번호가 4868이고 서버 이름이 인 경우 다음 내용이 포함된 이메일 메시지를 수신합니다 `ServerXYZ`:
