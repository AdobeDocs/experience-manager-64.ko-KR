...
# Acrobat Reader DC 확장 {#certificate-types-used-by-acrobat-reader-dc-extensions}에서 사용하는 인증서 유형
...
Acrobat Reader DC 확장 인증서의 &quot;친숙한&quot; 이름은 다음 예제와 같이 인증서의 속성을 설명하는 문자열입니다.

2D 바코드 풀 프로덕션 V6.1 P8 0002054

문자열에는 다음 요소가 포함됩니다.

**인증서 유형:** 인증서가 활성화되는 AEM 양식 모듈과 활성화 수준(예: ARE 2D 바코드 풀)을 설명합니다. 사용 가능한 인증서 유형의 목록은 인증서 프로필 섹션에 있는 테이블의 유형 열을 참조하십시오.
...
**프로필 코드:** 프로필 코드는 전체 인증서 속성의 대표 설명입니다(예: P8). 각 파일 형식과 연관된 프로필 코드 목록은 [인증서 프로필] 섹션의 표의 [프로필 코드] 열을 참조하십시오.

**일련 번호:** 일련 번호가 Adobe에서 발행한 인증서(예: 0002054)에 할당됩니다. Adobe 엔터프라이즈 지원 또는 Adobe Enterprise 계정 담당자는 이 일련 번호를 사용하여 특정 제품 주문 또는 OEM 관계로 인증서를 추적할 수 있습니다.

## 인증서 프로필 {#certificate-profiles}

다음 표는 Acrobat Reader DC 확장 인증서를 분석할 때 발생할 수 있는 인증서 프로필을 나열합니다.
...
제품 샘플 애플리케이션을 평가 및 개발할 수 있도록 평가 인증서가 고객과 개발자에게 발급됩니다. 이러한 인증서의 유효 기간은 60~90일입니다. 발행물 데이터에 이어 두 번째 달 말에 만료됩니다.

파트너 통합 인증서는 소프트웨어 개발, 통합, 프로토타이핑 및 데모를 지원하기 위해 Adobe 비즈니스 파트너에게 발급됩니다. 이 인증서는 발행 일로부터 2년 동안 유효합니다.

Adobe 내부 사용 인증서는 Adobe 내에서 소프트웨어 개발, 통합, 프로토타이핑 및 데모를 지원하는 데 사용됩니다. 이 인증서는 발행 일로부터 2년 동안 유효합니다.

프로덕션 인증서는 Acrobat Reader DC 확장을 구입한 고객에게 발급됩니다. 이러한 인증서는 인증서 프로필 테이블에 *Max*&#x200B;로 표시된 인증 기관(CA)에서 허용하는 최대 기간 동안 유효합니다.
...
공동 작업 환경이 아닌 환경에서 온라인 주석 달기가 필요한 경우 Adobe 지원 센터에 자세한 내용을 문의하십시오. Mode 속성은 배포 유형과 일치하며 *production* 또는 *evaluation*&#x200B;입니다.
...
<table> 
 <thead> 
  <tr> 
   <th><p>사용 권한 요소</p></th> 
   <th><p>권한이 활성화된 PDF 문서를 볼 때 Adobe Reader에서 사용 가능한 기능</p></th> 
  </tr> 
 </thead> 
 <tbody>
  <tr> 
   <td><p>FormFillInAndSave</p></td> 
   <td><p>양식 필드를 입력하고 파일을 로컬에 저장할 수 있습니다.</p></td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>PDF 양식의 필드 및 필드 속성을 추가, 변경 또는 삭제합니다.</p></td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>양식 데이터가 포함된 문서를 디코딩할 때 라이센스 서버 소프트웨어가 필요하지 않은 암호화되지 않은 형태로 인쇄할 수 있습니다.</p></td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>PDF 양식 내에 정의된 웹 서비스 또는 데이터베이스에 연결할 수 있습니다.</p></td> 
  </tr> 
  <tr> 
   <td><p>EPModif</p></td> 
...
>[!NOTE]
>
>Acrobat Reader DC 확장 사용 권한은 함께 작동하는 특정 조합에서만 Adobe에서 라이선스를 부여할 수 있습니다. 이러한 기능을 독립적으로 라이선스를 부여할 수 없습니다. 사용 권한 조합에 대한 자세한 내용은 AEM 양식 계정 담당자에게 문의하십시오.
