...
# Acrobat Reader DC 익스텐션에서 사용하는 인증서 유형 {#certificate-types-used-by-acrobat-reader-dc-extensions}
...
**인증서 유형:** 인증서가 활성화되는 AEM 양식 모듈과 활성화 수준(예: ARE 2D 바코드 풀)에 대해 설명합니다. 사용 가능한 인증서 유형 목록은 인증서 프로필 섹션에 있는 테이블의 유형 열을 참조하십시오.
...
**일련 번호:** 일련 번호는 0002054와 같이 Adobe이 발행한 각 인증서에 할당됩니다. Adobe 엔터프라이즈 지원 또는 Adobe 엔터프라이즈 계정 담당자는 이 시리얼 번호를 사용하여 특정 제품 주문 또는 OEM 관계로 인증서를 추적할 수 있습니다.
...
프로덕션 인증서는 Acrobat Reader DC 익스텐션을 구입한 고객에게 발급됩니다. 이러한 인증서는 인증서 프로필 테이블에 *최대* 로 표시된 인증 기관(CA)에서 허용하는 최대 기간 동안 유효합니다.
...
공동 작업 환경이 아닌 환경에서 온라인 주석 달기가 필요한 경우 Adobe 지원에 문의하여 자세한 내용을 확인하십시오. Mode 속성은 배포 유형과 일치하며 *프로덕션* 또는 *평가입니다*.
