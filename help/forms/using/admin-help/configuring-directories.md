...
1. 디렉토리 서버 설정을 구성합니다. (자세한 내용은 [디렉토리 설정](configuring-directories.md#directory-settings))
1. LDAP 서버에 연결할 수 있는지 확인하려면 테스트를 누릅니다. 테스트가 실패하면 Application Server 로그 파일에서 예외를 검토하여 오류의 근본 원인을 확인합니다. 닫기 를 클릭하고 다음 을 클릭합니다.
1. 사용자 설정 을 선택하고 필요에 따라 설정을 구성합니다. (자세한 내용은 [디렉토리 설정](configuring-directories.md#directory-settings))
...
1. 그룹 설정 을 선택하고 필요에 따라 설정을 구성합니다. (자세한 내용은 [디렉토리 설정](configuring-directories.md#directory-settings))
...
사용자 지정 SPI 만들기에 대한 내용은 의 &quot;AEM Forms용 SPI 개발&quot;을 참조하십시오. [AEM 양식을 사용한 프로그래밍](https://www.adobe.com/go/learn_aemforms_programming_63). 새로 배포된 사용자 지정 SPI를 도메인과 연결할 수 있도록 하려면 서버를 다시 시작합니다.
...
1. 필요에 따라 디렉토리, 사용자 및 그룹 설정을 구성합니다. (자세한 내용은 [디렉토리 설정](configuring-directories.md#directory-settings))
1. 확인을 클릭합니다.
...
**이름:** 익명 액세스를 사용할 수 없는 경우 LDAP 데이터베이스에 연결하는 데 사용할 수 있는 이름입니다. Active Directory 2003의 경우 `[domain name]\[userid]`. Sun™ One, eDirectory 또는 IBM Tivoli Directory Server의 경우 uid=lcuser,ou=it,o=company.com과 같이 사용자의 정규화된 이름을 지정합니다.

**암호:** 익명 액세스를 사용할 수 없는 경우 LDAP 데이터베이스에 연결하기 위해 지정한 이름과 일치하는 암호입니다.

**다음 방법으로 페이지 채우기:** 이 옵션을 선택하면 사용자 및 그룹 설정 페이지의 속성을 해당 기본 LDAP 값으로 채웁니다.
...
디렉토리 설정에서 참조 활성화 옵션을 선택한 경우 기본 DN 옵션을 로 설정합니다. *dc* DN의 일부. 참조 작업을 수행하려면 검색 범위에 상위 도메인과 하위 도메인이 모두 포함되어야 합니다.

>[!NOTE]
>
>이 설정에 사용자의 DN을 포함하지 마십시오. 특정 사용자를 동기화하려면 검색 필터 설정을 사용합니다.

기본 DN이 관리 콘솔에서 필수 설정이지만 IBM Domino Enterprise Server와 같은 일부 디렉토리 서버에는 빈 BaseDN이 필요할 수 있습니다. 빈 기본 DN을 지정하려면 config.xml 파일을 내보내고 config.xml 파일의 설정을 편집한 다음 다시 가져옵니다. (자세한 내용은 [구성 파일 가져오기 및 내보내기](/help/forms/using/admin-help/importing-exporting-configuration-file.md#importing-and-exporting-the-configuration-file))
...
**업무 일정:** 이 설정의 값(비즈니스 달력 키)을 기준으로 사용자에게 비즈니스 달력을 매핑할 수 있습니다. 업무 달력은 업무 및 비업무 일수를 정의합니다. AEM Forms에서는 미리 알림, 기한 및 에스컬레이션과 같은 이벤트의 미래 날짜와 시간을 계산할 때 비즈니스 달력을 사용할 수 있습니다. 사용자에게 비즈니스 달력 키를 할당하는 방법은 엔터프라이즈, 로컬 또는 하이브리드 도메인을 사용하는지에 따라 다릅니다. 업무 달력 구성을 참조하십시오.

엔터프라이즈 도메인을 사용하는 경우 LDAP 디렉토리의 필드에 비즈니스 달력 설정을 매핑할 수 있습니다. 예를 들어, 디렉토리에 있는 각 사용자 레코드에 *국가* 필드를 지정하고 사용자가 위치한 국가를 기준으로 업무 달력을 지정하려면 *국가* 필드 이름을 비즈니스 달력 설정 값으로 지정합니다. 그런 다음 비즈니스 달력 키(에 대해 정의된 값)를 매핑할 수 있습니다 *국가* Forms Workflow에서 업무 달력에 대한 LDAP 디렉토리의 필드)입니다.
...
**VLV(가상 목록 보기) 컨트롤 사용:** AEM Forms에서 디렉터리 서버에서 데이터를 일괄로 검색할 수 있도록 하는 LDAP 컨트롤입니다. Sun One을 LDAP 디렉토리로 사용하고 디렉토리에 많은 사용자가 포함되어 있는 경우 VLV를 활성화하면 사용자 관리에서 사용자를 검색할 때 사용할 수 있는 인덱스가 생성됩니다. 이 기능은 제한된 양의 데이터만 동기화할 수 있는 일반 사용자 계정을 사용하는 경우에 유용합니다. 그룹에 대해 VLV를 활성화할 수도 있습니다. VLV(가상 목록 보기) 컨트롤 사용을 선택하는 경우 정렬 필드 상자에 이름을 지정합니다.

>[!NOTE]
>
>VLV를 사용하려면 Sun One을 구성합니다. 자세한 내용은 [VLV(가상 목록 보기)를 사용하도록 사용자 관리 구성](configuring-directories.md#configure-user-management-to-use-virtual-list-view-vlv).
...
기본 DN은 관리 콘솔에서 필수 설정이지만 IBM Domino Enterprise Server와 같은 일부 디렉토리 서버에는 빈 BaseDN이 필요합니다. 빈 기본 DN을 지정하려면 config.xml 파일을 내보내고 config.xml 파일의 설정을 편집한 다음 다시 가져옵니다. (자세한 내용은 [구성 파일 가져오기 및 내보내기](/help/forms/using/admin-help/importing-exporting-configuration-file.md#importing-and-exporting-the-configuration-file))

**검색 필터:** (필수) 그룹과 연관된 레코드를 찾는 데 사용할 검색 필터입니다. 한 수준 검색 또는 하위 수준 검색을 수행할 수 있습니다.

**설명:** 그룹에 대한 설명을 위한 스키마 속성

**전체 이름:** (필수) 그룹의 전체 이름에 대한 스키마 속성

**멤버 DN:** (필수) 그룹 내 멤버의 고유 이름에 대한 스키마 속성
...
**기본 이메일:** 그룹의 기본 전자 메일 주소에 대한 스키마 속성

**보조 이메일:** 그룹의 보조 전자 메일 주소에 대한 스키마 특성

**타임스탬프 수정:** 델타 디렉토리 동기화를 사용하려면 이 값을 설정하여 타임스탬프를 수정합니다. 델타 디렉토리 동기화 사용을 참조하십시오.

**VLV(가상 목록 보기) 컨트롤 사용:** AEM Forms에서 디렉터리 서버에서 데이터를 일괄로 검색할 수 있도록 하는 LDAP 컨트롤입니다. Sun One을 LDAP 디렉토리로 사용하고 디렉토리에 많은 그룹이 포함되어 있는 경우 VLV를 사용하면 그룹을 검색할 때 사용자 관리에서 사용할 수 있는 인덱스가 생성됩니다. 이 기능은 제한된 양의 데이터만 동기화할 수 있는 일반 사용자 계정을 사용하는 경우에 유용합니다. 사용자에 대해 VLV를 활성화할 수도 있습니다. VLV(가상 목록 보기) 컨트롤 사용을 선택하는 경우 정렬 필드 이름을 지정합니다.
...
>[!NOTE]
>
>테스트 를 클릭하여 사용자 및 그룹 설정이 기본 DN 및 검색 기준에 따라 수집되는지 확인합니다. 사용자 및 그룹이 반환되면 속성 세트에 따라 각 필드에 할당된 값이 결과에 표시됩니다.
...
1. 디렉토리를 구성할 때 [사용자 설정] 페이지와 [그룹 설정] 페이지에서 VLV(가상 목록 보기) 제어 사용을 선택합니다. 확인란을 선택하는 경우 정렬 필드 상자에 정렬 이름을 지정해야 합니다. 기본값은 uid입니다. (자세한 내용은 [디렉토리 또는 사용자 지정 SPI 추가](configuring-directories.md#adding-directories-or-custom-spis) 또는 [디렉토리 편집](configuring-directories.md#edit-a-directory))
1. Sun ONE 관리 콘솔 또는 명령줄 스크립트를 사용하여 사용자 및 그룹에 대한 LDAP VLV 항목을 생성합니다. 명령줄 스크립트를 사용하는 경우 샘플 사용자 및 그룹 LDIF 파일을 사용할 수 있습니다. (자세한 내용은 [VLV용 Sun ONE Directory Server 구성](configuring-directories.md#configuring-the-sun-one-directory-server-for-vlv))
1. 서버를 중지하고 필요한 인덱스를 만듭니다. (자세한 내용은 [VLV용 디렉터리 서버 인덱스 만들기](configuring-directories.md#create-the-directory-server-index-for-vlv))

### VLV용 Sun ONE Directory Server 구성 {#configuring-the-sun-one-directory-server-for-vlv}

VLV를 만들려면 `vlvSearch` 및 `vlvIndex` 개체 클래스 vlvSearch 항목에는 검색 베이스와 `vlvFilter` 지정할 속성을 포함하는 객체 클래스를 지정하는 속성입니다. 다음 `vlvIndex` 객체 클래스에는 `vlvSort` 지정할 속성 및 정렬 순서를 지정합니다. (빼기 기호(-)는 역방향 알파벳 순서를 나타냅니다.) AEM Forms에서 VLV를 사용하려면 사용자 및 그룹에 대해 별도의 항목이 필요합니다.
...
1. 샘플 스크립트에 이름이 인 LDAP 항목이 있습니다. `lcuser`. 이 항목은 AEM Forms에서 사용자 동기화를 위한 VLV 관련 구성용입니다. 다음 속성을 그에 따라 수정합니다.

   **항목 이름:** 이 샘플의 항목 이름은 `lcuser`. If `lcuser` 가 변경되면 샘플 스크립트의 모든 영역에서 변경해야 합니다.

   **vlvBase:** 사용자 설정 페이지에 지정된 기본 DN입니다.

   **vlvFilter:** 사용자 설정 페이지에 지정된 검색 필터.

   **vlvSort:** 사용자 설정 페이지의 VLV 설정 섹션에 지정된 정렬 필드. VLV 컨트롤을 사용하려면 정렬 컨트롤을 지정해야 합니다. 이 필드는 생성된 vlv 인덱스에 대한 정렬 매개 변수로 사용됩니다.
   ...
   **aci:** 샘플 스크립트에 지정된 액세스 컨트롤은 인증된 사용자에게 읽기, 검색 및 비교 작업을 위해 VLV 인덱스에 액세스할 권한을 부여합니다. 관리자는 사용자 관리 사용자 인터페이스에 지정된 디렉토리 서버 설정 페이지에 구성된 바인딩 사용자에 대한 액세스를 제한할 수 있습니다. 권한이 부여되지 않은 경우 사용자 검색에서 VLV를 사용할 수 없으며 LDAP 서버에서 권한 예외가 발생합니다.
   ...
   >[!NOTE]
   >
   >규칙에서 vlvIndex 항목 이름도 `lcuser`하지만 다른 이름을 지정할 수 있습니다. vlvindex 도구에서 동일한 이름을 사용합니다. (자세한 내용은 [VLV용 디렉터리 서버 인덱스 만들기&#x200B;](configuring-directories.md#create-the-directory-server-index-for-vlv)*)*

1. 사용 `ldapmodify` Sun ONE Server와 함께 제공되는 툴은 그룹의 기본 DN, 검색 필터 및 정렬 필드를 사용하여 그룹에 대해 유사한 항목을 각각 생성합니다.
...
1. vlvindex 도구를 사용하여 다음 텍스트를 입력하여 인덱스를 생성합니다.
   ...
   *디렉토리 서버 인스턴스* `\vlvindex.bat -n userRoot -T lcuser`
   ...
   vlvindex 도구가 디렉터리 서버 인스턴스 디렉터리에 있습니다. Sun ONE Server에 server1과 server2를 실행하는 두 개의 인스턴스가 있는 경우 vlvindex 도구는 *Sun ONE 서버 디렉토리*\server1 디렉토리. 매개 변수의 값 `-T` 는 의 값입니다 `cn` 샘플 LDIF에서 이전에 만든 vlvindex 항목의 속성입니다. 이 경우 `lcuser`.

1. 그룹에 대해서도 VLV가 활성화되어 있으면 그룹에 대해 해당 인덱스를 만듭니다. 다음 명령을 실행하여 인덱스가 생성되는지 확인합니다.

   *sun one server directory* `\shared\bin>ldapsearch -h`*호스트 이름* `-p`*포트 번호* `-s base -b "" objectclass=*`
