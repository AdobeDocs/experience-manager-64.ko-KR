...
MySQL을 사용할 때 사용자 ID에는 확장 문자가 포함될 수 있습니다. 그러나 갑판대, 갑판대 등 두 문자열 사이의 비교가 이뤄지면 동일한 것으로 간주됩니다. 예를 들어, 동기화 시 새 사용자가 데이터베이스에 추가된 경우 동일한 사용자 ID를 가진 사용자가 데이터베이스에 있는지 여부를 확인하기 위해 비교가 수행됩니다. 새 사용자 *bcdgee*가 추가되면 사용자 *abcde*&#x200B;이(가) 데이터베이스에 이미 있는 경우 두 이름을 비교할 수 없습니다. 사용자가 데이터베이스에 이미 있으며 새 사용자는 무시되고 추가되지 않는다고 가정합니다.
...
오류 처리 페이지를 사용자 지정하는 방법에 대한 자세한 내용은 Adobe Experience Manager 설명서의 오류 처리기](https://docs.adobe.com/docs/en/cq/current/developing/customizing_error_handler_pages.html)에 표시된 페이지 사용자 지정을 참조하십시오.[
