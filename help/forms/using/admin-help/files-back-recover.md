...
GDS는 프로세스 내에서 사용되는 오래 지속되는 파일을 저장하는 데 사용되는 디렉토리입니다. 긴 기간 파일의 수명은 AEM Forms 시스템을 하나 이상 실행하기 위한 것으로, 일 수 및 여러 해에 걸쳐 있을 수 있습니다. 이러한 긴 기간 파일은 PDF, 정책 및 양식 템플릿을 포함할 수 있습니다. 긴 기간 파일은 많은 AEM forms 배포의 전체 상태에 중요한 부분입니다. 일부 또는 모든 장기간 문서가 손실되거나 손상된 경우 Forms 서버가 불안정해질 수 있습니다.
...
GDS의 위치는 AEM Forms 설치 프로세스 동안 또는 나중에 관리 콘솔을 사용하여 결정됩니다. GDS의 고가용성 위치를 유지할 뿐만 아니라 문서에 대한 데이터베이스 저장소를 활성화할 수도 있습니다. 자세한 내용은 [데이터베이스를 문서 저장소에 사용할 때 백업 옵션](files-back-recover.md#backup-options-when-database-is-used-for-document-storage).
...
원래 위치를 더 이상 사용할 수 없는 경우 복구 중에 GDS 위치를 변경할 수 있습니다. (자세한 내용은 [복구 중 GDS 위치 변경](/help/forms/using/admin-help/recovering-aem-forms-data.md#changing-the-gds-location-during-recovery))
...
IBM에는 데이터베이스 관리자가 백업 및 복구 작업을 관리하는 데 도움이 되는 도구 및 도움말 시스템 세트가 있습니다.

* IBM DB2 Archive Log Accelerator
* IBM DB2 데이터 아카이브 전문가(참조) [IBM DB2 Data Archive Expert 사용 안내서 및 참조](https://publib.boulder.ibm.com/infocenter/mptoolic/v1r0/topic/com.ibm.db2tools.aeu.doc.ug/ahxugb13.pdf?noframes=true))
...
DB2 데이터베이스 백업 및 복구에 대한 자세한 내용은 [DB2를 위한 백업 및 복구 전략 개발](https://publib.boulder.ibm.com/infocenter/db2luw/v9/index.jsp?topic=/com.ibm.db2.udb.admin.doc/doc/c0005945.htm).

### Oracle {#oracle}

스냅샷 백업을 사용하거나 아카이브 로그 모드에서 실행하도록 Oracle 데이터베이스를 구성합니다. (자세한 내용은 [Oracle 백업: 소개](https://www.databasedesign-resource.com/oracle-backup.md)) oracle 데이터베이스 백업 및 복구에 대한 자세한 내용은 다음 사이트를 참조하십시오.

[Oracle 백업 및 복구:](https://www.oracle.com/technetwork/database/features/availability/br-overview-097160.html) 백업 및 복구 개념과 백업, 복구 및 보고를 위해 RMAN(Recovery Manager)을 사용하는 가장 일반적인 기술에 대해 설명하고 백업 및 복구 전략을 계획하는 방법에 대한 자세한 정보를 제공합니다.

[Oracle 데이터베이스 백업 및 복구 사용 안내서:](https://download.oracle.com/docs/cd/E11882_01/backup.112/e10642.pdf) RMAN 아키텍처, 백업 및 복구 개념 및 메커니즘, 시점 복구 및 데이터베이스 플래시백 기능과 같은 고급 복구 기법, 백업 및 복구 성능 조정에 대한 자세한 정보를 제공합니다. 또한 RMAN 대신 호스트 운영 체제 설비를 사용하여 사용자가 관리하는 백업 및 복구를 다룹니다. 이 볼륨은 보다 정교한 데이터베이스 구축 및 고급 복구 시나리오에 필요한 백업 및 복구에 필요합니다.

[Oracle 데이터베이스 백업 및 복구 참조:](https://download.oracle.com/docs/cd/E11882_01/backup.112/e10643.pdf) 모든 RMAN 명령의 구문 및 의미에 대한 전체 정보를 제공하고, 백업 및 복구 작업에 대해 보고하는 데 사용할 수 있는 데이터베이스 뷰를 설명합니다.
...
MySQLAdmin을 사용하거나 Windows에서 INI 파일을 수정하여 이진 로그 모드에서 실행할 MySQL 데이터베이스를 구성합니다. (자세한 내용은 [MySQL 이진 로깅](https://dev.mysql.com/doc/refman/5.1/en/binary-log.html)) MySQL용 핫 백업 툴은 InnoBase 소프트웨어에서도 사용할 수 있습니다. (자세한 내용은 [Innobase 핫 백업](https://www.innodb.com/hot-backup/features.md))

>[!NOTE]
>
>MySQL의 기본 이진 로깅 모드는 Content Services에서 사용하는 테이블과 호환되지 않는 &quot;문&quot;입니다(사용되지 않음). 이 기본 모드에서 이진 로깅을 사용하면 Content Services(사용 중지)에 실패합니다. 시스템에 Content Services(사용 중지)가 포함되어 있는 경우 &quot;혼합&quot; 로깅 모드를 사용하십시오. &quot;혼합&quot; 로깅을 사용하려면 my.ini 파일에 다음 인수를 추가하십시오.
>
>`binlog_format=mixed log-bin=logname`

myqldump 유틸리티를 사용하여 전체 데이터베이스 백업을 얻을 수 있습니다. 전체 백업이 필요하지만 항상 편리하지는 않습니다. 대용량 백업 파일을 만들고 생성하는 데 시간이 걸립니다. 증분 백업을 수행하려면 - `log-bin` 이전 섹션에 설명된 옵션입니다. MySQL Server가 다시 시작될 때마다 현재 이진 로그에 대한 쓰기를 중단하고 새 이진 로그를 만든 후 이후에 새 서버가 현재 이진 로그가 됩니다. 수동으로 스위치를 `FLUSH LOGS SQL` 명령. 첫 번째 전체 백업 후 다음 증분 백업은 myqladmin 유틸리티를 사용하여 수행합니다. `flush-logs` 명령. 다음 로그 파일을 생성합니다.

자세한 내용은 [백업 전략 요약](https://dev.mysql.com/doc/refman/5.5/en/backup-strategy-summary.html).
...
컨텐츠 저장소 루트 디렉토리의 기본 위치는 다음과 같습니다 *[aem forms 루트]*/lccs_data를 참조하십시오.
...
**인덱스 루트 디렉터리:** 클러스터의 각 노드에서 항상 동일한 경로 및 디렉토리 이름을 갖는 디렉토리

컨텐츠 저장소 루트 디렉토리의 기본 위치는 다음과 같습니다 *[GDS 루트]*/lccs_data, 여기서 *[GDS 루트]* 은 에 설명된 위치입니다. [GDS 위치](files-back-recover.md#gds-location). 컨텐츠 저장소 루트 디렉토리에 있는 다음 디렉토리를 백업합니다.
...
인덱스 루트 디렉토리의 기본 위치는 다음과 같습니다. *[aem forms 루트]*/lucene-indexes를 각 노드에 지정합니다.
...
>[!NOTE]
>
>기본적으로 AEM Forms와 함께 설치된 Adobe 글꼴은 [aem forms 루트]/fonts 디렉토리.
