...
**파일 패턴:**  공급자가 허용하는 들어오는 파일 첨부 패턴입니다. 여기에는 특정 확장(&amp;ast;.dat, &amp;ast;.xml)이 있는 파일, 특정 이름(데이터) 및 확장()의 복합 표현식이 포함됩니다.[dD][a][Tt]) 기본값은 &amp;ast;입니다.amp;ast;(&amp;M)
...
**실패한 작업 수신자:** 실패한 작업을 나타내기 위해 이메일을 전송하는 데 사용되는 하나 이상의 이메일 주소입니다. 기본적으로 실패한 작업 메시지는 항상 초기 작업을 보낸 발신자에게 전송됩니다. 최대 100명의 수신자가 지원됩니다. 이 설정을 끄려면 이 필드를 비워 둡니다.
...
**파일 패턴:**  공급자가 허용하는 들어오는 파일 첨부 패턴을 지정합니다. 여기에는 이름 및 확장(&amp;ast;.dat, &amp;ast;.xml), 특정 이름(데이터) 또는 복합 표현식(&amp;ast;)이 있는 파일이 포함됩니다.[dD][a][Tt])
