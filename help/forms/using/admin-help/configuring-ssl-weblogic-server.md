...
<table> 
 ...
 <tbody>
  ...
  <tr> 
   ...
   <td><p>에 사용한 것과 동일한 암호 사용 <code>-storepass</code> 선택 사항입니다. 키 암호는 6자 이상이어야 합니다.</p></td> 
  </tr> 
  <tr> 
   <td><p>-dname</p></td> 
...
1. 다음 명령을 입력하여 ads-credentials 키 저장소에서 인증서를 추출합니다.
   ...
   >[!NOTE]
   >
   >바꾸기 `[JAVA_HOME]` JDK가 설치된 디렉터리로 를 교체하고 `store`*_* `password`* 사용자 지정 ID 키 저장소에 사용할 수 있는 암호 포함.*
...
1. 다음 명령을 입력하여 새 키 저장소 파일(사용자 지정 트러스트 키 저장소)에 인증서를 삽입합니다.
   ...
   >[!NOTE]
   >
   >바꾸기 `[JAVA_HOME]` JDK가 설치된 디렉터리로 를 교체하고 `store`*_* `password` 및 `key`*_* `password` *개인 암호를 사용하여 암호를 변경할 수 있습니다.*
...
1. 환경의 도메인 구성에서 **서버 > [server] > 구성 > 일반**.
1. General 의 Configuration에서 **수신 포트 사용** 및 **SSL 수신 포트 사용** 을 선택합니다. 활성화되지 않은 경우 다음을 수행합니다.

   1. 변경 센터에서 **잠금 및 편집** 선택 및 값을 수정하려면
   1. 을(를) 확인합니다. **수신 포트 사용** 및 **SSL 수신 포트 사용** 확인란을 선택합니다.
...
1. 클릭 **릴리스 구성**.
1. 환경 아래의 도메인 구성에서 **서버 > [*관리 대상 서버*] > 구성 > 일반**.
1. 일반에서 구성에서 **키 저장소**.
1. 변경 센터에서 **잠금 및 편집** 선택 및 값을 수정하려면
1. 클릭 **변경** 키 저장소 목록을 드롭다운 목록으로 가져오려면 을(를) 선택하고 을(를) 선택합니다 **사용자 지정 Id 및 사용자 지정 신뢰**.
1. ID에서 다음 값을 지정합니다.
