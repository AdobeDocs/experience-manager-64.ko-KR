...
사전 구성된 Microsoft Dynamics 개체 및 서비스가 포함된 OData Cloud Service 및 양식 데이터 모델은 AEM 인스턴스의 실행 모드가 `samplecontent`(기본값)으로 설정된 경우에만 AEM Forms 인스턴스에서 사용할 수 있습니다. AEM 인스턴스의 실행 모드 구성에 대한 자세한 내용은 실행 모드 [를 참조하십시오](https://helpx.adobe.com/in/experience-manager/6-4/sites-deploying/configure-runmodes.html).

## 전제 조건 {#prerequisites}

Microsoft Dynamics 설정 및 구성을 시작하기 전에 다음을 확인하십시오.

* [AEM 6.4 Forms Add-on 패키지를 설치함](https://helpx.adobe.com//experience-manager/6-4/forms/using/installing-configuring-aem-forms-osgi.html)
...
* [Microsoft Azure Active Directory에 Microsoft Dynamics 온라인 서비스 응용 프로그램을 등록했습니다](https://docs.microsoft.com/en-us/dynamics365/customer-engagement/developer/walkthrough-register-dynamics-365-app-azure-active-directory). 등록된 서비스에 대한 클라이언트 ID(응용 프로그램 ID라고도 함) 및 클라이언트 암호 값을 메모해 둡니다. 이러한 값은 Microsoft Dynamics 서비스용 클라우드 서비스를 [구성하는 동안 사용됩니다](/help/forms/using/ms-dynamics-odata-configuration.md#configure-cloud-service-for-your-microsoft-dynamics-service).
...
1. Microsoft Azure Active Directory 계정으로 이동하고 등록된 응용 프로그램에 대한 회신 URL **[!UICONTROL 설정에 다음 클라우드 서비스]** 구성 URL을 추가합니다.
...
1. Windows PowerShell을 사용하여 다음 명령을 실행하여 IFD가 활성화된 Microsoft Dynamics에서 클레임 설정을 구성합니다.
   ...
   자세한 내용은 [CRM 온프레미스(IFD)에 대한 앱](https://msdn.microsoft.com/sl-si/library/dn531010(v=crm.7).aspx#bkmk_ifd) 등록을 참조하십시오.
...
1. Microsoft Dynamics는 HTTPS 프로토콜을 사용합니다. Forms 서버에서 AD FS 끝점을 호출하려면 AEM Forms을 실행하는 컴퓨터의 명령을 사용하여 Microsoft Dynamics 사이트 인증서를 Java 인증서 저장소에 `keytool` 설치합니다.
...
1. 도구 **[!UICONTROL > Cloud Services > 데이터 소스로]**&#x200B;이동하고 `global` 구성 폴더를 누릅니다.
1. MS **[!UICONTROL Dynamics OData Cloud Service(OData 서비스) 구성을 선택하고 속성을]** 누릅니다 ****. 클라우드 서비스 구성 속성 대화 상자가 열립니다.

   인증 **[!UICONTROL 설정]** 탭에서 다음을 수행합니다.

   1. 서비스 루트 **[!UICONTROL 필드 값을]** 입력합니다. Dynamics 인스턴스로 이동하고 **[!UICONTROL 개발자 리소스로]** 이동하여 서비스 루트 필드에 대한 값을 확인합니다. 예: https://&lt;테넌트 이름>/api/data/v9.1/
   1. 클라이언트 ID **[!UICONTROL (]** 응용 프로그램 ID **[!UICONTROL 라고도 함)의 기본 값]**&#x200B;을 대체하고, 클라이언트 클라이언트 클라이언트 클라이언트 클라이언트 **[!UICONTROL ID]**, URL **[!UICONTROL , UTH OAuth URL URL URL 새로 고침, Microsoft Service의 Dynamics 구성 값을 사용하여 ResourceAccess 및 Microsoft Dynamics의 기본 값을 사용하여 리소스를 대체합니다]************** . 양식 데이터 모델로 Microsoft Dynamics를 구성하려면 **[!UICONTROL 리소스]** 필드에 동적 인스턴스 URL을 지정해야 합니다. 서비스 루트 URL을 사용하여 동적 인스턴스 URL을 파생합니다. 예: https://org.crm.dynamics.com [](https://org.crm.dynamics.com/).
   1. Microsoft Dynamics **[!UICONTROL 에서]** 인증 프로세스를 위해 **[!UICONTROL 권한]** 범위 필드에 openid를 지정합니다.
...
AEM Forms 패키지를 설치할 때 양식 데이터 모델인&#x200B;**Microsoft Dynamics FDM**&#x200B;이 AEM 인스턴스에 배포됩니다. 기본적으로 양식 데이터 모델은 MS Dynamics OData Cloud Service(OData 서비스)에 구성된 Microsoft Dynamics 서비스를 데이터 소스로 사용합니다.
...
양식 데이터 모델을 검토하려면 **[!UICONTROL Forms > 데이터 통합으로 이동합니다]**. Microsoft **[!UICONTROL Dynamics FDM을]** 선택하고 **[!UICONTROL 편집을]** 클릭하여 양식 데이터 모델을 편집 모드로 엽니다. 또는 다음 URL에서 바로 양식 데이터 모델을 열 수 있습니다.
...
비즈니스 워크플로우에서 양식 데이터 모델을 만들고 사용하는 방법에 대한 자세한 내용은 [데이터 통합을 참조하십시오](/help/forms/using/data-integration.md).
