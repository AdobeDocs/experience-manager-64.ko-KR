...
사전 구성된 Microsoft Dynamics 엔티티 및 서비스가 포함된 OData 클라우드 서비스 및 양식 데이터 모델은 AEM 인스턴스의 실행 모드가 `samplecontent`(기본값)으로 설정된 경우에만 AEM Forms 인스턴스에서 사용할 수 있습니다. AEM 인스턴스에 대한 실행 모드 구성에 대한 자세한 내용은 실행 [모드를 참조하십시오](https://helpx.adobe.com/in/experience-manager/6-4/sites-deploying/configure-runmodes.html).

## 전제 조건 {#prerequisites}

Microsoft Dynamics 설정 및 구성을 시작하기 전에 다음을 확인하십시오.

* AEM [6.4 양식 추가 기능 패키지를 설치했습니다.](https://helpx.adobe.com//experience-manager/6-4/forms/using/installing-configuring-aem-forms-osgi.html)
...
1. Microsoft Azure Active Directory 계정으로 이동하고 등록된 응용 프로그램에 대한 회신 URL **[!UICONTROL 설정에서 다음 클라우드 서비스]** 구성 URL을 추가합니다.
...
1. MS **[!UICONTROL Dynamics OData Cloud Service(OData Service)]** 구성을 선택하고 속성을 **[!UICONTROL 누릅니다]**. 클라우드 서비스 구성 속성 대화 상자가 열립니다.

   인증 **[!UICONTROL 설정]** 탭에서 다음을 수행합니다.

   1. 서비스 루트 **[!UICONTROL 필드에 값을 입력합니다]** . Dynamics 인스턴스로 이동하여 개발자 **[!UICONTROL 리소스로]** 이동하여 서비스 루트 필드의 값을 확인합니다. 예: https://&lt;임차인 이름>/api/data/v9.1/
   1. 클라이언트 ID( **[!UICONTROL 응용 프로그램 ID라고도 함]** )의 **[!UICONTROL 기본값을]****[!UICONTROL 바꾸십시오]**. **[!UICONTROL 클라이언트 클라이언트 클라이언트]**, **[!UICONTROL URL]********** , URL 및 URL URL 암호 새로 고침 URL 암호, URL 새로 고침 URL, URL 액세스 URL 및 Microsoft 서비스의 동적 값을 사용하여 리소스 필드에 입력합니다. 양식 데이터 모델로 Microsoft Dynamics를 **[!UICONTROL 구성하려면 리소스]** 필드에 동적 인스턴스 URL을 지정해야 합니다. 서비스 루트 URL을 사용하여 다이내믹 인스턴스 URL을 파생합니다. 예: https://org.crm.dynamics.com [를 참조하십시오](https://org.crm.dynamics.com/).
   1. Microsoft **[!UICONTROL Dynamics]** 에서 인증 **** 프로세스를 위해 권한 부여 범위 필드에 openid를 지정합니다.
...
양식 데이터 모델을 검토하려면 양식 > 데이터 **[!UICONTROL 통합으로 이동합니다]**. Microsoft **[!UICONTROL Dynamics FDM을]** 선택하고 **[!UICONTROL 편집을 클릭하여]** 양식 데이터 모델을 편집 모드로 엽니다. 또는 다음 URL에서 바로 양식 데이터 모델을 열 수 있습니다.
