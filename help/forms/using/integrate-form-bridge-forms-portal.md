---
title: HTML5 양식의 사용자 지정 포털과 양식 브리지 통합
seo-title: HTML5 양식의 사용자 지정 포털과 양식 브리지 통합
...
# HTML5 양식 {#integrating-form-bridge-with-custom-portal-for-html-forms}에 대한 사용자 지정 포털과 양식 브리지 통합

FormBridge는 양식과 상호 작용할 수 있는 HTML5 양식 bridge API입니다. FormBridge API 참조에 대해서는 [FormBridge API 참조](/help/forms/using/form-bridge-apis.md)를 참조하십시오.
...
## HTML5 양식을 웹 페이지에 통합 {#integrating-html-forms-to-a-web-page}
...
1. **HTML 프로필 수정**
   ...
   >[!NOTE]
   >
   >**라인 9**&#x200B;에는 페이지를 디자인할 CSS 스타일 및 JavaScript 파일에 대한 추가 JSP 참조가 포함되어 있습니다.
   >
   >**라인 18의 &lt;div id=&quot;rightdiv&quot;> 태그에는 XFA 양식의 HTML 코드 조각이 포함되어 있습니다.**
   페이지는 다음 두 개의 컨테이너로 스타일이 지정됩니다.**left** 및 **right** 오른쪽 컨테이너에는 양식이 있습니다. 왼쪽 컨테이너에는 두 개의 입력 필드와 외부 HTML 페이지의 일부가 있습니다.
   다음 스크린샷은 브라우저에서 양식이 표시되는 방식을 보여줍니다.

   ![포털](assets/portal.jpg)

   왼쪽은 **HTML 페이지**&#x200B;의 일부입니다. 필드가 포함된 오른쪽은 **xfa form**&#x200B;입니다.

1. **페이지에서 양식 필드에 액세스**

   다음은 양식 필드에 값을 설정하기 위해 추가할 수 있는 샘플 스크립트입니다.

   예를 들어, **EmployeeName** 필드 **이름** 및 **성**&#x200B;의 값을 사용하여 **EmployeeName을 설정하려면 window.formBridge.setFieldValue** 함수를 호출합니다.
