...
양식 데이터 통합 및 양식 데이터 모델에 대한 자세한 내용은 [AEM Forms 데이터 통합](data-integration.md).
...
* 에 명시된 대로 샘플 데이터가 있는 MySQL 데이터베이스 [데이터베이스 설정](#step-set-up-the-database) 섹션을 참조하십시오.
* 에 설명된 대로 MySQL JDBC 드라이버용 OSGi 번들 [JDBC 데이터베이스 드라이버 번들](https://helpx.adobe.com/experience-manager/6-3/sites-developing/jdbc.html#bundling-the-jdbc-database-driver)
...
양식 데이터 모델을 만들기 위해 다양한 유형의 데이터 소스를 구성할 수 있습니다. 이 자습서에서는 샘플 데이터로 구성되고 채워지는 MySQL 데이터베이스를 구성합니다. 지원되는 다른 데이터 소스 및 구성 방법에 대한 자세한 내용은 [AEM Forms 데이터 통합](data-integration.md).
...
1. MySQL 데이터베이스를 데이터 소스로 구성:
   ...
   1. 구성 대화 상자에서 다음 세부 정보를 지정합니다.
      ...
      * **데이터 소스 이름:** 이름을 지정할 수 있습니다. 예를 들어 **MySQL**.
      ...
      * **유효성 검사 쿼리:** 풀에서 연결을 검증할 SQL SELECT 쿼리를 지정합니다. 쿼리는 하나 이상의 행을 반환해야 합니다. 예, **마지막(&amp;ast) 선택 고객으로부터**.
...
AEM Forms은 [양식 데이터 모드 만들기](data-integration.md)l을 구성합니다. 양식 데이터 모델에서 여러 데이터 소스를 사용할 수 있습니다. 이 자습서의 사용 사례에서는 MySQL을 데이터 소스로 사용합니다.

양식 데이터 모델을 만들려면 다음을 수행하십시오.

1. AEM 작성자 인스턴스에서 **Forms** >  **데이터 통합**.
1. 탭 **만들기** >  **양식 데이터 모델**.
1. 양식 데이터 모델 만들기 마법사에서 **이름** 참조하십시오. 예, **FDM_Create_First_IC**. 탭 **다음**.
1. 데이터 소스 선택 화면에는 구성된 모든 데이터 소스가 나열됩니다. 선택 **MySQL** 데이터 소스 및 탭 **만들기**.

   ![fdm_mysql_data_source](assets/fdm_mysql_data_source.png)

1. 클릭 **완료**. 다음 **FDM_Create_First_IC** 양식 데이터 모델이 만들어집니다.
...
1. 를 확장합니다. **MySQL** 데이터 소스 트리. 다음 데이터 모델 개체 및 서비스를 선택합니다 **텔레코드** 스키마:
   ...
   * **서비스:**
   ...
   ![select_data_model_object_services](assets/select_data_model_objs_services.png)
...
* 사용 요금 = 전화 요금 + 전화 요금 + SMS 요금 + 모바일 인터넷 요금 + 로밍 국가 + 로밍 국제 + VAS(이러한 모든 속성은 bom 데이터 모델 개체에 있음)
   ...
   에 대한 자세한 내용은 **usagechards** 하위 계산 속성 [대화형 통신 계획](/help/forms/using/planning-interactive-communications.md).

다음 단계를 실행하여 bom 데이터 모델 객체에 대해 계산된 하위 속성을 생성합니다.

1. 상단의 확인란을 선택합니다 **청구서** 데이터 모델 개체를 선택하고 탭합니다. **하위 속성 만들기**.
1. 에서 **하위 속성 만들기** 창:

   1. Enter 키 **usagechards** 를 하위 속성의 이름으로 지정합니다.
   1. 활성화 **계산됨**.
   1. 선택 **부동** 를 입력하고 탭합니다. **완료** 하위 속성을 **청구서** 데이터 모델 개체.

   ![create_child_property_float](assets/create_child_property_float.png)

1. 탭 **규칙 편집** 규칙 편집기를 엽니다.
1. 탭 **만들기**. 다음 **값 설정** 규칙 창이 열립니다.
1. 옵션 선택 드롭다운에서 을 선택합니다 **수식**.

   ![usage_charge_rule_editor](assets/usage_charges_rule_editor.png)

1. 수학 표현식에서 을 선택합니다 **통화 요금** 및 **고충** 를 각각 첫 번째 및 두 번째 개체로 사용합니다. 선택 **plus** 를 반환합니다. 수식 내에서 탭하고 탭합니다 **표현식 확장** 를 추가하려면 **smschars**, **인터네트전하**, **로아밍국립**, **roamingintnl**, 및 **vas** 표현식에 대한 객체입니다.

   다음 이미지는 규칙 편집기의 수학 표현식을 설명합니다.

   ![usage_charge_rule_all](assets/usage_charges_rule_all.png)

1. 탭 **완료**. 규칙 편집기에서 규칙이 만들어집니다.
1. 탭 **닫기** 규칙 편집기 창을 닫습니다.
...
1. 상단의 확인란을 선택합니다 **고객** 데이터 모델 개체를 선택하고 탭합니다. **연결 추가**. 다음 **연결 추가** 속성 창이 열립니다.
1. 에서 **연결 추가** 창:

   * 연결의 제목을 지정합니다. 선택적 필드입니다.
   * 선택 **일대다** 에서 **유형** 드롭다운 목록.
   * 선택 **호출** 에서 **모델 개체** 드롭다운 목록.
   * 선택 **get** 에서 **서비스** 드롭다운 목록.
   * 탭 **추가** 를 **고객** 데이터 모델 개체 **호출** 속성을 사용하는 데이터 모델 개체. 사용 사례를 기반으로 호출 데이터 모델 개체가 고객 데이터 모델 개체의 모바일 번호 속성에 연결되어 있어야 합니다. 다음 **인수 추가** 대화 상자가 열립니다.
...
1. 탭 **완료** 고객 및 호출 데이터 모델 개체 간에 1:n 연결을 만들려면
...
1. 상단의 확인란을 선택합니다 **고객** 데이터 모델 개체를 선택하고 탭합니다. **속성 편집**. 다음 **속성 편집** 창이 열립니다.
1. 지정 **고객** 로서의 **최상위 모델 객체**.
1. 선택 **get** 에서 **서비스 읽기** 드롭다운 목록.
1. 에서 **인수** 섹션:

   * 선택 **요청 속성** 에서 **바인딩 대상** 드롭다운 목록.
   * 지정 **mobilenum** 를 바인딩 값으로 설정합니다.

1. 선택 **업데이트** 에서 **쓰기** 서비스 드롭다운 목록입니다.
1. 에서 **인수** 섹션:

   * 대상 **mobilenum** 속성, 선택 **고객** 에서 **바인딩 대상** 드롭다운 목록.
   * 선택 **mobilenum** 에서 **바인딩 값** 드롭다운 목록.
...
1. 상단의 확인란을 선택합니다 **호출** 데이터 모델 개체를 선택하고 탭합니다. **속성 편집**. 다음 **속성 편집** 창이 열립니다.
1. 비활성화 **최상위 모델 객체** 대상 **호출** 데이터 모델 개체.
1. 탭 **완료**.
...
1. 로 이동합니다. **서비스** 탭.
1. 을(를) 선택합니다 **get** 서비스 및 탭 **속성 편집**. 다음 **속성 편집** 창이 열립니다.
1. 에서 **속성 편집** 창:

   * 선택적 제목과 설명을 입력합니다.
   * 선택 **고객** 에서 **출력 모델 개체** 드롭다운 목록.
   * 탭 **완료** 속성을 저장합니다.

   ![edit_properties_get_details](assets/edit_properties_get_details.png)

1. 을(를) 선택합니다 **업데이트** 서비스 및 탭 **속성 편집**. 다음 **속성 편집** 창이 열립니다.
1. 에서 **속성 편집** 창:

   * 선택적 제목과 설명을 입력합니다.
   * 선택 **고객** 에서 **입력 모델 개체** 드롭다운 목록.
   * 탭 **완료**.
   * 탭 **저장** 양식 데이터 모델을 저장합니다.
...
1. 로 이동합니다. **모델** 탭에서 을 선택합니다 **고객** 데이터 모델 개체 및 탭 **테스트 모델 개체**.
1. 에서 **양식 데이터 모델 테스트** 창, 선택 **모델 개체 읽기** 에서 **모델/서비스 선택** 드롭다운 목록.
1. 에서 **입력** 섹션에서 **mobilenum** 구성된 MySQL 데이터베이스에 있는 속성을 탭하고 **테스트**.
...
1. 로 이동합니다. **서비스** 탭.
1. 을(를) 선택합니다 **get** 서비스 및 탭 **테스트 서비스.**
1. 에서 **입력** 섹션에서 **mobilenum** 구성된 MySQL 데이터베이스에 있는 속성을 탭하고 **테스트**.
...
1. 양식 데이터 모델 페이지에서 **샘플 데이터 편집**. 샘플 데이터 편집 창에서 샘플 데이터를 생성하고 표시합니다.

   ![edit_sample_data](assets/edit_sample_data.png)

1. in **샘플 데이터 편집** 필요에 따라 창을 열고 데이터를 편집하고 탭합니다. **저장**. 창을 닫습니다.
