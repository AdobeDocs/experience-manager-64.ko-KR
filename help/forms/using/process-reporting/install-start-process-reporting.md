...
description: JEE 프로세스 보고에서 AEM Forms를 시작하기 위해 수행해야 하는 단계
seo-description: JEE 프로세스 보고에서 AEM Forms를 시작하기 위해 수행해야 하는 단계
...
프로세스 보고는 AEM Forms 사용자에게 현재 AEM Forms 구현에 정의된 AEM Forms 프로세스에 대한 정보를 쿼리하는 기능을 제공합니다. 하지만 프로세스 보고는 AEM Forms 저장소의 데이터에 직접 액세스하지 않습니다. 데이터는 먼저 ProcessDataPublisher 및 ProcessDataStorage *서비스*&#x200B;에 의해 예약된 기준으로 프로세스 보고 저장소에 게시됩니다. 그런 다음 프로세스 보고의 보고서와 쿼리는 저장소에 게시된 프로세스 보고 데이터에서 생성됩니다. 프로세스 보고는 양식 워크플로우 모듈의 일부로 설치됩니다.

이 문서에서는 AEM Forms 데이터를 프로세스 보고 저장소에 게시하기 위한 절차에 대해 자세히 설명합니다. 이후 프로세스 보고를 사용하여 보고서와 쿼리를 실행할 수 있습니다. 또한 이 문서에서는 프로세스 보고 서비스를 구성하는 데 사용할 수 있는 옵션도 다룹니다.
...
>[!NOTE]
>
>제거 유틸리티에 대한 팁과 트릭은 제거 프로세스 및 작업 [에 대한 Adobe Developer Connection 아티클을 참조하십시오](https://www.adobe.com/content/dam/Adobe/en/devnet/livecycle/pdfs/purging_processes_jobs.pdf).
...
1. JVM 인수 추가 `-Dreporting.publisher.cron = <expression>.`
   ...
   예: 다음 cron 표현식으로 인해 프로세스 보고가 5시간마다 AEM Forms 데이터를 프로세스 보고 저장소에 게시합니다.
...
1. AEM Forms 서버 인스턴스를 중지합니다.
1. WebSphere 관리 콘솔에 로그인합니다. 탐색 트리에서 **서버** > **애플리케이션 서버** 를 클릭한 다음 오른쪽 창에서 서버 이름을 클릭합니다.

1. 서버 인프라에서 **Java 및 프로세스 관리** > **프로세스 정의를 클릭합니다**.

1. 추가 속성에서 **Java 가상 시스템을 클릭합니다**.

   일반 JVM 인수 상자에 인수를 추가합니다 `-Dreporting.publisher.cron = <expression>.`

   **예**: 다음 cron 표현식으로 인해 프로세스 보고가 5시간마다 AEM Forms 데이터를 프로세스 보고 저장소에 게시합니다.

   * `-Dreporting.publisher.cron = 0_0_0/5_*_*_?`

1. 적용 **을**&#x200B;클릭하고 확인을 클릭한 다음 **저장을 클릭하여 마스터 구성에 직접 적용합니다**.
...
1. AEM Forms 서버 인스턴스를 중지합니다.
1. WebLogic 관리 콘솔에 로그인합니다. WebLogic 관리 콘솔의 기본 주소는 입니다 `https://[hostname]:[port]/console`.

1. 변경 센터에서 **잠금 및 편집을 클릭합니다**.

1. 도메인 구조에서 **환경** > **서버** 를 클릭하고 오른쪽 창에서 관리되는 서버 이름을 클릭합니다.
...
1. 저장 **을** 클릭한 다음 변경 사항 **활성화를 클릭합니다**.
...
관리 콘솔을 사용하여 루트(**기본값**: `/content/reporting/pm`) location and 하위 폴더 (**default**: `/yyyy/mm/dd/hh/mi/ss`) 계층 형식으로 지정합니다.
...
1. 홈 > **서비스** > **애플리케이션** 및 서비스 **> 서비스** 관리 **를 탐색하고 서비스** **** 관리 프로세스를 연 다음DataStorageProviderService로 이동합니다.
...
1. CRX 관리자 자격 **증명을 사용하여** Configuration Manager에 로그인합니다. 구성 관리자의 기본 URL은 `https://[*server*]:[*port*]/lc/system/console/configMgr`
...
1. 관리자 자격 증명을 **사용하여** 관리 콘솔에 로그인합니다.
   ...
   기본 URL은 입니다 `https://[server]:port]/adminui/`.

1. 홈 > **서비스** > **애플리케이션** 및 서비스 **> 서비스** 관리 **** **** 를 탐색하고 Adobe ProcessDataPublisherService를 엽니다.
...
**프로세스 보고 트리 보기:** [홈] 화면의 왼쪽에 있는 트리 보기에는 [프로세스 보고] 모듈의 항목이 포함되어 있습니다.

트리 보기는 다음과 같은 최상위 항목으로 구성됩니다.

**보고서:** 이 항목에는 프로세스 보고 기능이 포함된 기본 보고서가 포함되어 있습니다.

사전 정의된 보고서에 대한 자세한 내용은 프로세스 보고 [에서 사전 정의된 보고서를 참조하십시오](pre-defined-reports-in-process-reporting.md).

**애드혹 쿼리:** 이 항목에는 프로세스 및 작업에 대한 필터 기반 검색을 수행하는 옵션이 포함되어 있습니다.

임시 쿼리에 대한 자세한 내용은 프로세스 보고 [에서 임시 쿼리를 참조하십시오](adhoc-queries-in-process-reporting.md).

**사용자 지정:** 사용자 지정 노드는 사용자가 만드는 사용자 지정 보고서를 표시합니다.

사용자 지정 보고서를 만들고 표시하는 절차는 프로세스 보고 [의 사용자 지정 보고서를 참조하십시오](/help/forms/using/process-reporting/process-reporting-custom-reports.md).
...
데이터 게시 서비스에 대한 자세한 내용과 이 서비스를 예약하는 방법은 프로세스 보고 시작 [의 프로세스 데이터 게시](/help/forms/using/process-reporting/install-start-process-reporting.md#p-schedule-process-data-publishing-p) 예약을 참조하십시오.

**프로세스 보고 사용자:** 로그인한 사용자 이름이 마지막 업데이트 시간 오른쪽에 표시됩니다.

**프로세스 보고 제목 표시줄 드롭다운 목록:** [프로세스 보고] 제목 표시줄 오른쪽 모서리의 드롭다운 목록에는 다음 옵션이 포함되어 있습니다.

* **[!UICONTROL 동기화]**: 포함된 프로세스 보고 리포지토리를 AEM Forms 데이터베이스와 동기화합니다.
