...
description: JEE 프로세스 보고에서 AEM Forms를 시작하기 위해 수행해야 하는 절차
seo-description: JEE 프로세스 보고에서 AEM Forms를 시작하기 위해 수행해야 하는 절차
...
프로세스 보고는 AEM Forms 사용자에게 현재 AEM Forms 구현에 정의된 AEM Forms 프로세스에 대한 정보를 쿼리할 수 있는 기능을 제공합니다. 하지만 프로세스 보고는 AEM Forms 저장소에서 직접 데이터에 액세스하지 않습니다. 데이터는 ProcessDataPublisher 및 ProcessDataStorage *서비스에*&#x200B;의해 예약된 기준으로 프로세스 보고 저장소에 처음 게시됩니다. 그런 다음 프로세스 보고의 보고서 및 쿼리는 저장소에 게시된 프로세스 보고 데이터에서 생성됩니다. 프로세스 보고는 양식 워크플로우 모듈의 일부로 설치됩니다.

이 문서에서는 AEM Forms 데이터를 프로세스 보고 저장소에 게시하기 위한 절차에 대해 자세히 설명합니다. 그런 다음 프로세스 보고를 사용하여 보고서 및 쿼리를 실행할 수 있습니다. 또한 이 문서에서는 프로세스 보고 서비스를 구성하는 데 사용할 수 있는 옵션도 다룹니다.

## 프로세스 보고 전제 조건 {#process-reporting-pre-requisites}

### 중요하지 않은 프로세스 제거 {#purge-non-essential-processes}

현재 양식 워크플로우를 사용하고 있는 경우 AEM Forms 데이터베이스에 상당한 양의 데이터가 포함될 수 있습니다

Process Reporting 게시 서비스는 현재 데이터베이스에서 사용할 수 있는 모든 AEM Forms 데이터를 게시합니다. 이것은 데이터베이스에 보고서와 쿼리를 실행하지 않으려는 기존 데이터가 포함되어 있는 경우 보고에 필요하지 않더라도 해당 모든 데이터가 저장소에 게시된다는 것을 의미합니다. 데이터를 프로세스 보고 저장소에 게시하기 위해 서비스를 실행하기 전에 이 데이터를 삭제하는 것이 좋습니다. 이렇게 하면 게시자 서비스와 보고를 위해 데이터를 쿼리하는 서비스의 성능이 모두 개선됩니다.

AEM Forms 프로세스 데이터 제거에 대한 자세한 내용은 프로세스 [데이터 제거를 참조하십시오](https://help.adobe.com/en_US/livecycle/11.0/AdminHelp/WS92d06802c76abadb-5145d5d12905ce07e7-7cb2.2.html).

>[!NOTE]
>
>제거 유틸리티에 대한 팁과 트릭은 제거 프로세스 및 [작업](https://www.adobe.com/content/dam/Adobe/en/devnet/livecycle/pdfs/purging_processes_jobs.pdf)제거에 대한 Adobe Developer Connection 문서를 참조하십시오.

## 프로세스 보고 서비스 구성 {#configuring-process-reporting-services}

### 프로세스 데이터 게시 예약 {#schedule-process-data-publishing}

Process Reporting Services는 AEM Forms 데이터베이스의 데이터를 예약 기반으로 프로세스 보고 저장소로 게시합니다.

이 작업은 리소스를 많이 사용하므로 AEM Forms 서버의 성능에 영향을 줄 수 있습니다. AEM Forms 서버 사용 시간 슬롯 외부에서 예약하는 것이 좋습니다.
...
1. JVM 인수 추가 `-Dreporting.publisher.cron = <expression>.`
   ...
   예:다음 cron 표현식으로 인해 프로세스 보고에서 AEM Forms 데이터를 5시간마다 프로세스 보고 저장소에 게시합니다.

   * `-Dreporting.publisher.cron = 0_0_0/5_*_*_?`

1. 파일을 저장하고 `run.conf.bat` 닫습니다.

1. AEM Forms 서버 인스턴스를 다시 시작합니다.

1. AEM Forms 서버 인스턴스를 중지합니다.
1. WebSphere 관리 콘솔에 로그인합니다. 탐색 트리에서 **서버** > **애플리케이션 서버를** 클릭한 다음 오른쪽 창에서 서버 이름을 클릭합니다.

1. 서버 인프라에서 Java 및 프로세스 **관리** > 프로세스 **정의를 클릭합니다**.
...
1. 적용을 **클릭하고**&#x200B;확인을 클릭한 다음 **마스터 구성에**&#x200B;직접 저장을 클릭합니다.
...
![procedapubliserservice](assets/processdatapublisherservice.png)
...
관리 콘솔을 사용하여 루트(**기본값**: `/content/reporting/pm`) 위치 및 하위 폴더(**기본값**:) 프로세스 데이터를 저장할 계층 `/yyyy/mm/dd/hh/mi/ss`형식
...
1. [홈] > [ **서비스** **** ] > [ **애플리케이션 및 서비스** ] > [서비스 **관리** 프로세스 열기 **** ]DataStorageProviderService로 이동합니다.
   ...
   **폴더 계층**
   ...
   프로세스 생성 시간을 기반으로 프로세스 데이터가 저장되는 폴더 계층 구조
...
1. **레코드 수**
   ...
   ReportConfiguration 서비스는 큰 결과 집합을 처리하기 위해 쿼리 처리를 레코드 집합으로 분할합니다. 이렇게 하면 시스템 부하가 줄어듭니다.
   ...
   **CRX 스토리지 경로**
   ...
   >[!NOTE]
   >
   >이 위치는 ProcessDataStorage 구성 옵션 루트 폴더에 지정된 위치와 **동일합니다**.
   >
   >ProcessDataStorage 구성의 루트 폴더 옵션을 업데이트하는 경우 ReportConfiguration 서비스에서 CRX 저장소 경로 위치를 업데이트해야 합니다.

1. 저장을 **클릭하고** CQ 구성 **관리자를 닫습니다**.

### ProcessDataPublisher 서비스 {#processdatapublisher-service}

ProcessDataPublisher 서비스는 AEM Forms 데이터베이스에서 프로세스 데이터를 가져오고 해당 데이터를 저장을 위해 ProcessDataStorageProvider 서비스에 게시합니다.
...
1. [홈] > [ **서비스** ] **및** [서비스 **] > [서비스** ] **Service** Management **** 를 탐색하고 Open The Open ProcessDataPublisherService로 이동합니다.
...
ProcessDataPublisher 서비스가 실행될 때마다 서비스는 먼저 배치 간격에 의해 서비스의 마지막 실행 이후 시간을 분할합니다. 그런 다음 서비스는 AEM Forms 데이터의 각 간격을 별도로 처리합니다.

이렇게 하면 주기 내에서 각 실행(일괄 처리) 동안 게시자가 처리하는 데이터의 크기를 제어할 수 있습니다.

예를 들어 발행자가 매일 실행하는 경우, 한 번의 실행으로 하루 동안의 전체 데이터를 처리하는 대신, 기본적으로 처리를 24개의 한 시간의 배치로 분할합니다.
...
게시자 서비스는 데이터 처리를 시작할 때 잠금을 획득하여 게시자의 여러 인스턴스가 동시에 실행 및 처리를 시작하지 않도록 합니다.

잠금을 취득한 게시자 서비스가 잠금 시간 초과 값으로 정의된 시간(초) 동안 유휴 상태인 경우, 다른 게시자 서비스 인스턴스가 처리를 계속할 수 있도록 잠금을 해제합니다.
...
보고 요구에 따라 특정 날짜 및 시간 이후에 데이터에 대한 보고서 및 쿼리를 실행하려는 경우 날짜 및 시간을 지정하는 것이 좋습니다. 그런 다음 게시 서비스는 해당 시점부터 날짜를 게시합니다.
...
프로세스 보고 URL(https://&lt;서버>:&lt;포트>/lc/pr)로 이동하면 로그인 화면이 표시됩니다.
...
프로세스 보고에 로그인하면 홈 **[!UICONTROL 화면이]** 표시됩니다.
...
사전 정의된 보고서에 대한 자세한 내용은 프로세스 [보고에서 사전 정의된 보고서를 참조하십시오](pre-defined-reports-in-process-reporting.md).

**** 애드혹 쿼리:이 항목에는 프로세스 및 작업에 대한 필터 기반 검색을 수행하는 옵션이 포함되어 있습니다.

임시 쿼리에 대한 자세한 내용은 프로세스 보고에서 [임시 쿼리를 참조하십시오](adhoc-queries-in-process-reporting.md).
