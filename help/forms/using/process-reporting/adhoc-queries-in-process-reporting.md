...
description: 프로세스 보고에서 JEE 프로세스 및 작업 세부 사항에 대한 AEM Forms을 검색할 사용자 정의 쿼리를 만듭니다.
seo-description: 프로세스 보고에서 JEE 프로세스 및 작업 세부 사항에 대한 AEM Forms을 검색할 사용자 정의 쿼리를 만듭니다.
...
프로세스 보고에서 애드혹 쿼리를 사용하면 AEM Forms 환경에 정의된 AEM Forms 프로세스 인스턴스의 프로세스 및 작업 세부 사항을 검색하는 데 사용할 수 있는 사용자 지정 쿼리를 만들 수 있습니다.
...
필터 만들기 및 프로세스 세부 정보에 대한 쿼리 실행에 따르는 단계는 작업에 대한 단계와 동일합니다.
...
1. 쿼리를 만들려면 **추가** (+)를 클릭합니다.
   ...
   **필터를 정의하려면**
   ...
   1. 쿼리에 다른 필터를 추가하려면 필터 행 오른쪽에 있는 **추가(+)**&#x200B;를 클릭합니다.
      ...
      쿼리에서 필터를 제거하려면 필터 행 오른쪽에 있는 **Delete(-)**&#x200B;을 클릭합니다.
...
**내 필터** 패널의 옵션을 **추가** ![lc_pr_add_filter](assets/lc_pr_add_filter.png), **편집** ![lc_pr_delete_filter](assets/lc_pr_delete_filter.png) 또는 **삭제**> ![lc_pr_edit_filter](assets/lc_pr_edit_filter.png)애드혹 쿼리

![my_filters_options](assets/my_filters_options.png)

### 검색 쿼리 {#to-execute-a-search-query}을(를) 실행하려면

1. 쿼리를 실행하려면 필터를 만들거나 편집하는 경우 **내 필터** 패널에서 필터를 클릭하거나 **실행** 단추를 클릭합니다.
1. 쿼리 결과는 **프로세스 보고** 창의 **보고서** 패널에 표시됩니다.

   ![process_search_result](assets/process_search_result.png)

   보고서 하단에 표시되는 페이지 매김 패널의 도움말로 검색 결과를 게시할 수 있습니다.
...
1. 프로세스 검색 결과에 다음 필드가 표시됩니다.
   ...
   * **작업 ID**:작업의 ID입니다. 필드가 하이퍼링크되어 있습니다. 이 필드에서 작업 ID를 클릭하면 작업에 대한 **[!UICONTROL 작업 세부 사항]** 패널로 리디렉션됩니다.
   * **이니시에이터**:프로세스 인스턴스를 시작한 AEM Forms 사용자
   * **만든 시간**:프로세스 인스턴스가 시작된 날짜 및 시간
   ...
   * **상태**:프로세스 인스턴스의 현재 상태입니다.
   ...
   기본적으로 결과는 작업 ID별로 정렬됩니다. 하지만 결과를 필드 기준으로 정렬하려면 필드 제목을 클릭합니다. 결과는 열 머리글 옆에 있는 어두운 화살표로 표시되는 열별로 정렬됩니다.

   정렬은 전환 작업이므로 필드 헤더를 클릭하여 결과를 오름차순으로 정렬하고 다시 클릭하여 내림차순으로 정렬합니다. 현재 정렬 순서(오름차순/내림차순)는 열 머리글 옆에 있는 어두운 화살표 방향으로 표시됩니다.

   ![task_search_result](assets/task_search_result.png)

1. 왼쪽 상단에 있는 레일 단추 ![lc_pr_raile_button](assets/lc_pr_rail_button.png)을 클릭하여 **내 필터** 창을 축소하고 **보고서** 패널에 사용할 수 있는 공간을 확장합니다.
1. 쿼리 결과에 대해 작업을 수행하려면 **보고서** 패널의 오른쪽 위 모서리에 있는 옵션을 사용합니다.
   ...
   * **CSV로 내보내기**:보고서 데이터를 쉼표로 구분된 파일로 내보냅니다.
   ...
   >[!NOTE]
   >
   >보고서를 내보낼 때 검색의 전체 결과가 현재 페이지뿐만 아니라 CSV 파일로 내보내집니다
...
1. 프로세스/작업 검색을 실행합니다. 자세한 내용은 [프로세스 검색 쿼리](/help/forms/using/process-reporting/adhoc-queries-in-process-reporting.md#p-to-execute-a-search-query-p)를 참조하십시오.

   결과에 표시된 프로세스 ID가 하이퍼링크되어 있습니다.
...
1. **프로세스 세부 정보** 패널에서 이 프로세스의 세부 사항을 보려면 목록에서 프로세스 ID를 클릭합니다.
   ...
   **프로세스/작업 세부 사항** 쿼리 결과에 프로세스/작업에 포함된 작업/양식의 세부 사항이 표시됩니다.
   ...
   기본적으로 결과는 작업/양식 ID별로 정렬됩니다. 하지만 결과를 필드 기준으로 정렬하려면 필드 제목을 클릭합니다. 결과가 정렬되는 열은 열 머리글 옆에 있는 어두운 화살표로 표시됩니다.
   ...
   **오른쪽 위 패널:** 선택한 프로세스를 구성하는 작업에 대한 다음 세부 사항을 표시합니다.
...
1. 프로세스/작업 ID 텍스트 상자에 프로세스/작업 ID를 입력합니다.
   ...
   **프로세스/작업 세부 사항** 쿼리 결과의 필드는 AEM Forms 프로세스/작업에 해당하는 필드입니다.
