...
HTML5 양식 서비스 프록시는 제출 서비스에 대한 프록시를 등록하는 구성입니다. 서비스 프록시를 구성하려면 요청 매개 변수 submissionServiceProxy를 통해 전송 서비스의 URL을 *지정합니다*.

## 서비스 프록시의 이점 {#benefits-of-service-proxy-br}

서비스 프록시는 다음을 제거합니다.

* HTML5 양식 워크플로우를 사용하려면 HTML5 양식 사용자를 위한 제출 서비스 &quot;/content/exforms/submission/default&quot;를 열어야 합니다. 의도하지 않은 광범위한 사용자에게 AEM 서버를 노출합니다.
* 서비스 URL은 양식의 런타임 모델에 포함됩니다. 서비스 URL 경로를 변경할 수 없습니다.
* 제출은 두 단계로 이루어집니다. 양식 데이터를 제출하려면 서버에 두 번 이상 체류해야 합니다. 따라서 서버의 로드를 증가시킵니다.
* HTML5 양식은 PDF 요청 대신 POST 요청에 데이터를 전송합니다. PDF 및 HTML5 양식을 모두 포함하는 워크플로우의 경우 제출 과정에 대해 두 가지 다른 방법이 필요합니다.

## 토폴로지 {#topologies-br}

HTML5 양식은 다음 토폴로지를 사용하여 AEM 서버에 연결할 수 있습니다.

* AEM Server 또는 HTML5 양식에서 POST을 통해 서버로 데이터를 전송하는 토폴로지.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>contextSom에는 이벤트가 실행되는 개체의 SOM 식이 포함됩니다.</p> </td> 
  </tr> 
  <tr> 
   <td><p>템플릿</p> </td> 
  ...
  <tr> 
   <td><p>데이터</p> </td> 
   <td><p>데이터에는 양식을 렌더링하는 데 사용되는 바타 바이트가 포함됩니다.</p> </td> 
  </tr> 
  <tr> 
   <td><p>formDom</p> </td> 
  ...
  <tr> 
   <td><p>debugDir</p> </td> 
   <td><p>debugDir은 양식을 렌더링하는 데 사용되는 디버그 디렉토리를 포함합니다.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
전송 단추를 클릭하면 HTML5 양식이 데이터를 서버로 보냅니다. 다음 표에는 HTML5 양식이 서버로 전송하는 모든 매개 변수가 나와 있습니다.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><p>데이터</p> </td> 
   <td><p>양식을 렌더링하는 데 사용되는 bata 바이트.</p> </td> 
  </tr> 
  <tr> 
   <td><p>formDom</p> </td> 
...
제출 서비스 프록시는 제출 URL이 요청 매개 변수에 없는 경우 통과(pass through) 역할을 합니다. 그것은 통과로 작용한다. 요청을 /bin/xfaforms/submitaction 종료 지점에 보내고 XFA 런타임에 응답을 보냅니다.

제출 서비스 프록시는 요청 매개 변수에 제출 URL이 있는 경우 토폴로지를 선택합니다.

* AEM 서버가 데이터를 게시하면 프록시 서비스는 통과(pass-through) 역할을 합니다. 요청을 /bin/xfaforms/submitaction 종료 지점에 보내고 XFA 런타임에 응답을 보냅니다.
...
* 데이터(POST 요청)를 서버로 보내기 전에 HTML5 양식이 서버의 연결 및 가용성을 확인합니다. 연결 및 가용성을 확인하기 위해 HTML 양식은 서버에 빈 헤드 요청을 보냅니다. 서버가 사용 가능한 경우 HTML5 양식이 서버에 데이터(POST 요청)를 전송합니다. 서버를 사용할 수 없는 경우 서버에 연결할 *수 없다는 오류 메시지가* 표시됩니다. 사전 감지 기능을 사용하면 사용자가 양식을 새로 입력하는 번거로움을 방지할 수 있습니다. 프록시 서블릿은 헤드 요청을 처리하며 예외를 throw하지 않습니다.
