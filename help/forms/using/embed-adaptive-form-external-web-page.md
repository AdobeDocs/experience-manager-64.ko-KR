...
다음을 수행할 수 있습니다 [AEM Sites에 적응형 양식 포함](/help/forms/using/embed-adaptive-form-aem-sites.md) AEM 외부에서 호스팅되는 웹 페이지 또는 페이지입니다. 포함된 적응형 양식은 완전히 기능하며 사용자는 페이지를 종료하지 않고 양식을 작성하고 제출할 수 있습니다. 따라서 사용자는 웹 페이지에서 다른 요소의 컨텍스트에 남아 있고 동시에 양식과 상호 작용할 수 있습니다.
...
* AEM 서버와 웹 페이지가 다른 도메인에 있는 경우 다음 섹션에 나열된 단계를 수행합니다. [AEM Forms에서 도메인 간 사이트에 적응형 양식을 제공할 수 있도록 설정](#cross-domain-sites).
* [역방향 프록시 설정](#reveseproxy) 외부 페이지와 AEM Forms 서버 간의 통신을 사용하도록 설정하려면 다음을 수행하십시오.
...
1. 포함된 코드에서:
   ...
   * 의 값 변경 `options.path` 적응형 양식의 게시 URL 경로가 있는 변수입니다. AEM 서버가 컨텍스트 경로에서 실행 중인 경우 URL에 컨텍스트 경로가 포함되어 있는지 확인하십시오. 예를 들어 위의 코드 및 적응형 은 동일한 aem forms 서버에 있으므로 이 예제에서는 적응형 양식의 컨텍스트 경로 /content/forms/af/locbasic.html 를 사용합니다.
...
* 웹 페이지 CSS에 정의된 스타일 규칙이 양식 개체 CSS와 충돌하지 않도록 합니다. 충돌을 방지하기 위해 AEM 클라이언트 라이브러리를 사용하여 적응형 양식 테마에서 웹 페이지 CSS를 다시 사용할 수 있습니다. 적응형 양식 테마에서 클라이언트 라이브러리를 사용하는 방법에 대한 자세한 내용은 [AEM Forms의 테마](/help/forms/using/themes.md).
...
* 사용  [getData](https://helpx.adobe.com/experience-manager/6-4/forms/javascript-api/GuideBridge.html) 클라이언트에서 양식 데이터의 XML 또는 JSON 표현을 가져오는 API입니다.
* 사용 [unloadAdaptiveForm](https://helpx.adobe.com/experience-manager/6-4/forms/javascript-api/GuideBridge.html) HTML DOM에서 적응형 양식을 언로드하기 위한 API입니다.
* AEM 서버에서 응답을 보낼 때 액세스 제어 원본 헤더를 설정합니다.

## 도메인 간 사이트에 적응형 양식을 제공할 수 있도록 AEM Forms 활성화  {#cross-domain-sites}

1. AEM 게시 인스턴스에서 AEM Web Console 구성 관리자로 이동합니다. `http://[server]:[port]/system/console/configMgr`.
1. 을(를) 찾아 엽니다. **Apache Sling 레퍼러** 구성을 필터링합니다.
1. 에서 **허용된 호스트** 필드에서 웹 페이지가 있는 도메인을 지정합니다. 이렇게 하면 호스트가 AEM 서버에 POST 요청을 수행할 수 있습니다. 정규 표현식을 사용하여 일련의 외부 애플리케이션 도메인을 지정할 수도 있습니다.
