...
DDX **호출** 작업 과정에서는 `Invoke` Assembler 서비스 API를 호출하여 문서를 조합 또는 분해하고 PDF에 워터마크를 추가하는 등의 작업을 수행할 수 있습니다.

1. 사이드 킥의 **[!UICONTROL Forms Workflow 탭 아래 DCX]** 워크플로우 호출 단계를 드래그합니다.
...
* **PayLoad에서 맵 생성**: 선택하면 페이로드 폴더 아래의 모든 문서가 어셈블러의 API에 대한 입력 문서의 맵에 `invoke` 추가됩니다. 각 문서의 노드 이름은 맵에서 키로 사용됩니다.
...
1. 사이드 킥의 **[!UICONTROL Forms Workflow 탭 아래에 있는 ConvertToPDFA]** 워크플로우 단계를 드래그합니다.
...
비대화형 PDF 생성 작업 과정은 Output `generatePDFOutput` service API의 래퍼입니다. XDP 템플릿 및 데이터 xml에서 비대화형 PDF 문서를 생성하는 데 사용됩니다.
