...
description: 모든 구성 요소를 사용하여 인터랙티브 커뮤니케이션 제작
seo-description: 모든 구성 요소를 사용하여 인터랙티브 커뮤니케이션 제작
...
모든 구성 요소를 사용하여 인터랙티브 커뮤니케이션 제작
...
웹 버전에 대한 양식 데이터 모델, 문서 조각, 템플릿 및 테마와 같은 모든 구성 요소를 만들었으면 대화형 통신을 만들 수 있습니다.

인터랙티브 커뮤니케이션은 다음 두 채널을 통해 전달할 수 있습니다.인쇄 및 웹. 또한 인쇄 채널을 마스터로 사용하여 대화형 통신을 만들 수도 있습니다. 웹 채널에 대한 마스터 옵션으로 인쇄하면 웹 채널의 컨텐츠, 상속 및 데이터 바인딩이 인쇄 채널에서 파생됩니다. 또한 인쇄 채널에서 수행한 변경 내용이 웹 채널에서 동기화되도록 합니다. 그러나 대화형 통신 작성자는 웹 채널의 특정 구성 요소에 대한 상속을 중단할 수 있습니다.
...
## {#create-interactive-communications-for-print-and-web-with-no-synchronization} 동기화 없이 인쇄 및 웹용 대화형 통신 만들기

### 인쇄 채널 {#create-interactive-communication-for-print-channel}에 대한 대화형 통신 만들기

다음은 이 자습서에서 이미 만들어진 리소스 목록이며 인쇄 채널용 대화형 통신을 만드는 동안 필요합니다.
...
1. **자산** 탭으로 이동하여 필터를 적용하여 왼쪽 창에 문서 조각만 표시합니다.
1. 다음 문서 조각을 인터랙티브 커뮤니케이션에서 대상 영역으로 드래그하여 놓습니다.

   | 문서 단편 | 대상 영역 |
   |---|---|
   | bill_details_first_ic | BillDetails |
   | customer_details_first_ic | 고객 정보 |
...
1. 왼쪽 창에서 **Data Sources** 탭을 선택하고 **호출** 데이터 모델 개체를 **항목별 호출** 대상 영역으로 드래그하여 놓습니다. **호출** 데이터 모델 개체의 모든 속성은 오른쪽 창의 **항목별 호출** 대상 영역에 테이블 열로 표시됩니다.
   ...
   사용 사례에 따라 테이블의 호출 날짜, 호출 시간, 호출 번호, 호출 기간 및 호출 요금 열이 필요합니다.

   ![table_ic_web](assets/table_ic_web.png)

1. **Mobilenum** 표 열 머리글을 선택하고 **추가 옵션** > **열 삭제**&#x200B;를 선택합니다. 마찬가지로 **Caltype** 열을 삭제합니다.
1. **Caldate** 표 열 제목을 선택하고 ![편집](assets/edit.png)(편집)을 눌러 텍스트 이름을 **호출 날짜**&#x200B;로 변경합니다. 마찬가지로 테이블의 다른 열 머리글의 이름을 바꿉니다.
1. 사용 사례에 따라, 단추를 클릭하여 결제를 수행할 수 있는 옵션을 제공하는 대화형 통신에 **지금 지불** 단추를 삽입합니다. 단추를 삽입하려면 다음 단계를 실행합니다.
...
1. 다음 단계를 사용하여 대화형 통신에 하이퍼링크 텍스트에 스타일을 지정하여 단추로 표시합니다.
   ...
   1. **배경** 섹션에서 단추의 배경색으로 [빨강]을 선택합니다.
...
1. 사용 사례에 따라, 단추를 클릭하여 추가된 값에 가입할 수 있는 옵션을 제공하는 대화형 통신에 **구독** 단추를 삽입합니다.

   13 - 17단계를 반복하여 **Subscribe** 단추를 **부가 서비스** 대상 영역에 추가하고 **ValueAddedServicesWeb** 이미지를 추가합니다.
...
1. **채널** 마법사에서:
   ...
   1. **웹 채널에 기본 다른 이름으로 인쇄 사용** 확인란을 선택합니다.
   1. **Create_First_IC_templates** 폴더 > **Create_First_IC_Web_Template**&#x200B;을 웹 템플릿으로 지정하고 **Select**&#x200B;를 누릅니다.
   1. **만들기**&#x200B;를 누릅니다.
...
1. 4단계에서 **웹 채널에 대해 인쇄 기본 사용** 확인란을 선택하면 내용 및 바인딩이 인쇄 채널에서 웹 채널에 대해 자동으로 생성됩니다.

   인쇄 채널 컨텐츠는 웹 채널 템플릿 컨텐츠 아래에 삽입됩니다. 인쇄 채널에서 자동으로 생성된 웹 채널 컨텐츠를 수정하려면 대상 영역에 대한 상속을 취소할 수 있습니다.

   웹 채널에서 관련 대상 영역 위로 마우스를 가져간 후 ![cancele상속](assets/cancelinheritance.png)(상속 취소)을 선택한 다음 **상속 취소** 대화 상자에서 **Yes**&#x200B;를 탭합니다.

   ![cancel_inheritance_web_channel](assets/cancel_inheritance_web_channel.png)

   구성 요소의 상속을 취소한 경우 다시 활성화할 수 있습니다. 상속을 다시 활성화하려면 구성 요소를 포함하는 관련 대상 영역의 경계를 가리키고 ![reenableinheritance](assets/reenableinheritance.png)을 탭합니다.
...
1. 컨텐츠 트리를 사용하여 자동 생성된 웹 채널 컨텐츠를 웹 템플릿의 기존 패널에 드래그하여 놓습니다. 다음은 다시 정렬해야 하는 구성 요소 목록입니다.
   ...
   * 비용 구성 요소 요약과 비용 패널 요약
   * 항목별 호출 패널에 대한 레이아웃 조각(테이블)

   ![ic_web_content_tree](assets/ic_web_content_tree.png)

1. [웹 채널에 대한 대화형 통신 만들기](#create-interactive-communication-for-web-channel) 중 13 - 18단계를 반복하여 대화형 커뮤니케이션의 웹 채널에 있는 **지금 지불** 및 **구독** 하이퍼링크를 삽입합니다.
