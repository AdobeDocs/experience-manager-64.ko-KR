...
참조 사이트 사용 사례의 이메일 커뮤니케이션을 위한 게시 인스턴스에 AEM DS 서비스 설정이 필요합니다. 게시 인스턴스에서 AEM DS 서비스 설정을 구성하는 자세한 단계는 AEM DS 설정 [구성을 참조하십시오](/help/forms/using/configuring-the-processing-server-url-.md).
...
1. 서버에서 A/B 테스트를 활성화하려면 jvm 인수 `-Dabtesting.enabled=true` 로 author quickstart를 시작합니다.
   ...
   **참고**: 턴키 설치에서 서비스로 시작된 JBoss에서 AEM 인스턴스가 실행 중인 경우 파일의 다음 항목에 매개 변수를 `-Dabtesting.enabled=true` `jboss\bin\standalone.conf.bat` 추가합니다.
