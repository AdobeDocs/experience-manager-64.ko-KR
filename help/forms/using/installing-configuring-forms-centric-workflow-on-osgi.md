...
OSGi](/help/forms/using/aem-forms-workflow.md)에서 [Forms 중심의 워크플로우를 사용하여 적응형 양식 기반 워크플로우를 신속하게 구축할 수 있습니다. 이러한 워크플로우를 사용하면 검토 및 승인 워크플로우, 비즈니스 프로세스 워크플로우 및 기타 반복적인 작업을 자동화할 수 있습니다. 이러한 워크플로우에서는 문서 처리(PDF 문서 작성, 조합, 배포 및 보관, 문서에 대한 액세스를 제한하는 디지털 서명 추가, 바코드 양식 디코딩 등)와 양식 및 문서에서 Adobe Sign 서명 워크플로우를 사용할 수도 있습니다.
...
AEM Forms은 강력한 엔터프라이즈급 플랫폼입니다. OSGi 기반의 Forms 중심의 워크플로우는 AEM Forms 기능 중 하나입니다. 전체 기능 목록을 보려면 [AEM Forms 소개](/help/forms/using/introduction-aem-forms.md)을 참조하십시오.

>[!NOTE]
>
>OSGi의 Forms 중심 워크플로우를 사용하면 JEE 스택에 프로세스 관리 기능을 완전히 설치하지 않고도 OSGi 스택에서 다양한 작업을 위한 워크플로우를 신속하게 구축 및 배포할 수 있습니다. 이 기능의 차이점과 유사점에 대한 자세한 내용은 JEE의 OSGi 및 프로세스 관리에 대한 Forms 중심 AEM 워크플로우의 [비교](/help/forms/using/capabilities-osgi-jee-workflows.md)를 참조하십시오.
>
...
>[!NOTE]
>
>[데이터 캡처 기능 설치 및 구성](/help/forms/using/installing-configuring-aem-forms-osgi.md) 문서에 설명된 대로 OSGi에 AEM Forms을 이미 설치한 경우 문서의 [다음 단계](#next-steps) 섹션으로 건너뜁니다.
...
1. **적응형 양식 구성 서비스**&#x200B;를 클릭하여 구성 값을 편집합니다. 구성 값 편집 대화 상자에서 AEM Forms 서버의 인스턴스가 캐시할 수 있는 최대 양식 또는 문서 수를 **응용 Forms의 수** 필드에 지정합니다. 기본값은 100입니다. **저장**&#x200B;을 클릭합니다.
...
OSGi 시나리오의 일반적인 Adobe Sign 및 Forms 중심 워크플로우에서 사용자는 적응형 양식을 작성하여 서비스에 적용합니다. 예를 들어 신용카드 신청서와 시민 혜택 양식입니다. 사용자가 애플리케이션 양식을 채우고, 제출하고 서명하면 승인/거부 워크플로우가 시작됩니다. 서비스 제공업체는 AEM 받은 편지함에서 애플리케이션을 검토하고 Adobe Sign을 사용하여 애플리케이션에 전자 서명합니다. 유사한 전자 서명 워크플로우를 사용하려면 Adobe Sign을 AEM Forms과 통합할 수 있습니다.
