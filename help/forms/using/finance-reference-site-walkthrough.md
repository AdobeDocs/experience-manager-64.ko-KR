...
사라는 신용카드를 신청하기로 하고 이메일에 있는 지금 **[!UICONTROL 적용]** 버튼을 탭한다. We.Finance 포털의 신용카드 신청으로 사라가 간다. 애플리케이션 양식은 카드 레이아웃을 사용하여 섹션으로 구성됩니다.
...
### Sarah는 모바일 장치에서 신용카드 응용 프로그램에 액세스합니다 {#a-sarah-access}
...
![모바일에서](assets/form-on-mobile.png)

### 작동 방식 {#a-how-it-works}

지금 **[!UICONTROL 적용]** 단추가 사라를 신용카드 신청서로 안내한다. 응용 프로그램은 응용 양식이며, 작성 인스턴스에서 검토할 수 있습니다 `https://[host]:[Port]/editor.html/content/forms/af/we-finance/cc-app.html`.
...
Sarah Rose로 로그인하면, 신용카드 응용 프로그램에서 **[!UICONTROL 지금]** 적용 단추를 클릭합니다. 일부 세부 사항을 채우고 다양한 적응형 양식 구성 요소를 탐색한 다음 **[!UICONTROL 내 진행]** 상황 **[!UICONTROL 을]** 저장하여 초안 애플리케이션에 연결된다시 시작단추가 포함된 이메일을 수신합니다. 이메일을 수신할 이메일 ID를 응용 프로그램 양식에 지정해야 합니다.
...
새라는 나중에 돌아와서 We.Finance로부터 이메일을 찾습니다. e메일에서 **[!UICONTROL 이력서]** 버튼을 클릭하면 신용카드 신청서 영장에 도착한다. 그녀가 이전에 작성한 정보는 미리 채워져 있다. 그녀는 남은 신청서 양식을 작성하고, 서명하여 제출한다.
...
이전 단계에서 **[!UICONTROL 받은 이메일에서]** 다시 시작 단추를 클릭합니다. 초안 응용 프로그램을 엽니다. 모든 세부 사항을 기입하고 신청서를 제출해 주십시오. 그러면 환영 키트를 받게 됩니다 시작 키트를 검토합니다.

다음 URL에서 시작 키트를 볼 수도 있습니다.

https://&lt;*host*>:&lt;*port*>/content/aemforms-refsite/doclink.html?document=/content/forms/af/we-finance/credit-card/creditcard/creditcard&amp;customerId=197&amp;channel=web
...
1. 도구 모음 **[!UICONTROL 에서]** 자세히 **[!UICONTROL 를 클릭하고 A/B 테스트 구성을 선택합니다]**. A/B 테스트 구성 페이지가 열립니다.

1. 활동 **[!UICONTROL 이름을 지정합니다]**.
1. 대상 드롭다운 목록에서 양식의 다른 경험을 제공할 대상을 선택합니다. 예를 들어 Chrome을 **사용하는 방문자**.
1. 경험 A와 **[!UICONTROL B에 대한 경험 배포]** 필드에서, 전체 대상 간에 경험의 배포를 결정하기 위해 백분율로 분포를 지정합니다. 예를 들어 경험 A와 경험 B에 각각 40, 60을 지정하는 경우 경험 A는 대상의 40%에게 제공되며 나머지 60%는 경험 B를 보게 됩니다.
...
블루칩 성장 기금 설립설명서는 상호 소통이다. 텍스트, 이미지, 차트 및 표 구성 요소(문서 조각)를 사용하여 제품 요약, 스톡 스타일, 펀드 실적, 자금 세부 정보 및 기타 관련 정보를 표시합니다. https://작성자[호스트]:[ authorPort]/editor.html/content/forms/af/we-finance/wealth-management/wealth-management/channels/web.html

차트 및 표는 양식 데이터 모델에서 데이터를 검색합니다. 양식 데이터 모델은 이 연습의 데이터베이스인 구성된 데이터 소스에 연결하여 펀드와 관련된 정보를 검색합니다. https:// authorHost[:]authorPort[]/aem/fdm/editor.html/content/dam/formsanddocuments-fdm/we-finance/resources-management에서 양식 데이터 모델을 검토할 수 있습니다.

#### 직접 보기  {#see-it-yourself-15}

https://[publishHost]:[publishPort]/wefinance의 We.Finance 포털로 이동하고, 자산 관리를 탭하고, 자산 클래스로 자금을 확장하고, We.Finance Blue Chip Growth Fund를 탭합니다. We.Finance Blue Chip Growth Fund Programming (WE.금융 블루칩 성장 기금) 약관이 열립니다.
...
Blue Chip Growth Fund의 인터랙티브한 커뮤니케이션으로 이동합니다(https://[publishHost]:).[ publishPort]/content/forms/af/we-finance/wealth-management/wealth-management/channels/web.html?wcmmode=disabled. 모든 탭을 살펴보십시오.
...
PDF 버전을 다운로드하려면 Blue Chip Growth Fund 대화형 통신 https://[publishHost로 이동합니다].[ publishPort /content/forms/af/we-finance/wealth-management/wealth-management/channels/web.html]에서 PDF 다운로드를 누릅니다.

PDF를 이메일로 전송하려면 Blue Chip Growth Fund 대화형 커뮤니케이션 https://[publishHost로 이동합니다].[ publishPort /content/forms/af/we-finance/wealth-management/wealth-management/channels/web.html]에서 EMAIL PDF를 누릅니다. 전체 **이름** 및 **이메일 주소를 지정합니다**. [ **이메일 보내기]를 클릭합니다**.
