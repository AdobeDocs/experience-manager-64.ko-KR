...
1. 새로 만든 오버레이 노드(/apps/fd/cm/ma/gui/content/commetadataproperties/commonproperties/col1/items) 아래

   nt:unstructured 유형의 드롭다운(여기)을 만들어야 하는 각 속성(필드)에 대한 노드를 만듭니다. `geographicallocation`
...
1. 속성 노드(위치 지리적 할당)에서 이름을 가진 새 노드를 추가합니다 `items`. 항목 노드 아래에서 드롭다운의 값에 대해 각각 노드를 추가합니다. 우수 사례로, 드롭다운의 기본값으로 사용할 첫 번째 노드를 공백으로 추가하고 사용자가 필드에 값을 지정하지 않도록 옵션을 지정합니다. 여러 옵션/드롭다운 값을 추가하려면 다음 단계를 반복합니다.
   ...
   1. 새로 만든 노드(here item1)에 다음 속성을 추가한 다음 모두 **저장을 클릭합니다**.
      ...
      | 이름 | 유형 | 값 |
      |--- |--- |--- |
      | text | 문자열 | 사용자가 볼 수 있는 드롭다운 옵션의 값입니다. 빈 값(기본값)을 비워 두거나 국제 또는 미국 내 값과 같은 **값을** **입력합니다**. |
      | 정렬 단추 | 문자열 | 텍스트의 CRXDE에 저장된 값입니다. 고유한 키워드를 입력합니다. |
...
1. (선택 사항) 기본적으로 새 열이 UI의 마지막 열로 표시됩니다. 열을 특정 위치에 표시하려면 열 노드에 다음 속성을 추가합니다.
   ...
   | 이름 | 유형 | 값 |
   |--- |--- |--- |
   | sling:orderBefore | 문자열 | 사용자 지정 열이 UI에 표시되기 전에 경로 &quot;`/libs/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns`&quot;에 있는 열 노드의 이름입니다. 여기에서 [지리적 위치] 열을 [버전] 열 앞(왼쪽에 표시)으로 지정하려면 &quot;경로&quot; `sling:orderBefore` 에서 GeoLocation 노드에 속성을 추가하고 속성 값을`/apps/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns/GeoLocation`버전으로 설정합니다. |
