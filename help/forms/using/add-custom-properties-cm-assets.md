...
# 메일 관리 자산 {#add-custom-properties-to-correspondence-management-assets}에 사용자 지정 속성 추가
...
1. 속성 노드(여기지리적 할당)에서 이름이 `items`인 새 노드를 추가합니다. 항목 노드 아래에서 드롭다운에 있는 값에 대해 각 노드를 추가합니다. 드롭다운의 기본값으로 사용할 첫 번째 노드를 공백으로 추가하고 사용자가 필드에 값을 지정하지 않도록 하는 옵션을 지정하는 것이 좋습니다. 여러 옵션/드롭다운 값을 추가하려면 다음 단계를 반복합니다.
   ...
   1. 새로 만든 노드(here item1)에 다음 속성을 추가한 다음 **모두 저장**&#x200B;을 클릭합니다.
      ...
      | 이름 | 유형 | 값 |
      |--- |--- |--- |
      | text | 문자열 | 사용자가 볼 수 있는 드롭다운 옵션의 값입니다. 빈(기본값) 값을 비워 두거나 **International** 또는 **Within US** 등의 값을 입력합니다. |
...
1. commetadataproperties 폴더 아래에서 모든 자산에 대한 사용자 정의 탭을 만들기 위한 노드를 추가합니다(예:[일반] 탭 사용):
   ...
   1. 노드에 다음 속성을 추가합니다(여기 공통 탭).
      ...
      | 이름 | 유형 | 값 |
      |--- |--- |--- |
      | jcr:title | 문자열 | 열을 지정할 이름입니다. (여기:받는 사람) |
...
1. 새로 만든 항목 폴더에서 다음 단계를 사용하여 자산의 사용자 정의 탭에 대한 노드를 추가합니다(여기 mytab - 이 이름은 사용자 인터페이스에 표시되지 않음).
   ...
   1. 노드에 다음 2개의 속성을 추가합니다(여기서 customtab).
      ...
      | sling:resourceType | 문자열 | granal/ui/components/coral/foundation/include |
...
1. (선택 사항) 기본적으로 새 열은 UI의 마지막 열로 표시됩니다. 열을 특정 위치에 표시하려면 열 노드에 다음 속성을 추가하십시오.
   ...
   | 이름 | 유형 | 값 |
   |--- |--- |--- |
   | sling:orderBefore | 문자열 | 사용자 지정 열이 UI에 표시되어야 하는 경로 &quot;`/libs/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns`&quot;의 열 노드의 이름입니다. 여기에서 [버전] 열 앞(왼쪽)에 [지리적 위치] 열을 표시하려면 속성 `sling:orderBefore`을(를) &quot;`/apps/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns/GeoLocation`&quot; 경로의 GeoLocation 노드에 추가하고 속성 값을 버전으로 설정합니다. |
...
1. 노드에 다음 속성을 추가하고 **모두 저장**&#x200B;을 클릭합니다.
   ...
   | 이름 | 유형 | 값 |
   |---|---|---|
   | sling:resourceType | 문자열 | fd/cm/ma/gui/components/admin/assets/properties/custofpropertyconfig |
