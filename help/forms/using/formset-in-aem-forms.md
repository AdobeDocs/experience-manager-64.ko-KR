...
# AEM Forms 양식 세트 {#form-set-in-aem-forms}
...
모든 양식, 양식 세트 및 관련 자산을 [AEM Forms 유저](/help/forms/using/introduction-managing-forms.md) 인터페이스에서 관리할 수 있습니다.
...
1. 양식 구성 페이지에서 다음을 수행할 수 있습니다.
   ...
   * 데이터 루트: 작성자는 양식 세트의 각 양식에 대해 해당 특정 양식의 데이터가 제출된 XML에 배치된 XPATH를 구성할 수 있습니다. 기본값은 / 입니다. 양식 세트의 모든 양식이 스키마 바인딩되어 있고 동일한 XML 스키마를 공유하는 경우 이 값을 변경할 수 있습니다. 양식의 모든 필드에 XDP에 지정된 적절한 데이터 바인딩이 있는 것이 좋습니다. 두 개의 다른 양식의 두 필드가 공통 데이터 바인딩을 공유하는 경우 두 번째 양식의 필드에 첫 번째 양식의 프리플라이트 값이 표시됩니다. 동일한 내부 컨텐츠로 두 개의 하위 양식을 동일한 XML 노드에 바인딩하지 마십시오. 양식 세트의 XML 구조에 대한 자세한 내용은 양식 세트에 [대한 XML 미리 보기를 참조하십시오](/help/forms/using/formset-in-aem-forms.md#p-prefill-xml-for-form-set-p).
...
1. Forms > Forms 및 문서를 선택합니다.
1. 편집할 양식 세트를 찾습니다. 마우스를 위에 놓고 편집( ![편집)을 선택합니다](assets/editicon.png).
...
이전 예에서 데이터 루트를 지정하는 경우 세 양식에 대해 각각 &quot;/form1&quot;, &quot;/form2&quot; 및 &quot;/form3&quot;을 사용하려면 다음 구조의 자동 완성 XML을 사용해야 합니다.
...
### XML 요소 설명 자동 채우기 {#prefill-xml-elements-description}
...
AEM Forms 앱에 대한 자세한 내용은 [AEM Forms 앱을 참조하십시오](/help/forms/using/aem-forms-app.md).

## 알려진 제한 사항 - 양식 세트에서 패턴이 완전히 지원되지 않음 {#known-limitations-patterns-not-fully-supported-in-form-set}
