...
# HTML5 양식용 Form Bridge API {#form-bridge-apis-for-html-forms}
...
* **입력**:
   ...
   * **컨텍스트**: *handler *함수의 컨텍스트(이)가 설정되는 개체입니다.
...
* **입력:**
   ...
   * **옵션:** 다음 속성을 포함하는 JavaScript 개체:
      ...
      * **오류**: 오류 처리기 함수
      * **성공**: 성공 처리기 함수입니다. 이 함수는 *데이터* 속성에 XML이 포함된 개체를 전달합니다.
...
* **입력:**
   ...
   * **configName:** 무시할 구성 이름
      ...
      * **widgetConfig:** 사용자가 사용자 정의 위젯을 사용하여 양식의 기본 위젯을 무시할 수 있습니다. 구성은 다음과 같이 재정의됩니다.
      ...
      * **로깅 구성:** 사용자가 로깅 수준을 재정의하거나, 카테고리에 대한 로깅을 비활성화하거나, 로그 콘솔을 표시할지 또는 서버로 전송할지를 지정할 수 있습니다. 구성은 다음과 같이 재정의할 수 있습니다.
      ...
      * **SubmitServiceProxyConfig:** 사용자는 제출 및 로거 프록시 서비스를 등록할 수 있습니다.
...
* **출력:** 데이터 속성에 있는 구성의 원래 값이 들어 있는 ** 개체입니다.
...
* **입력:** 없음
* **출력:** 데이터 속성에서 현재 양식 상태를 나타내는 JSON이 포함된 ** 개체
...
* **입력:**
   ...
   * **som:** 필드의 Som 표현식을 포함하는 배열입니다. 필드의 값을 설정하는 일부 식입니다.
   * **value:** 일부 배열에 제공된 Som 표현식에 해당하는 값을 포함하는 **** 배열입니다. 값의 데이터 유형이 fieldType과 같지 않으면 값이 수정되지 않습니다.
...
* **입력:**
   ...
   * **som:** 값을 설정해야 하는 필드의 일부 표현식이 포함된 배열
   * **property**: 값을 설정해야 하는 속성
   * **value:** Som 표현식에 지정된 필드의 지정된 속성 값을 포함하는 배열

* **출력:** 없음
* **오류:** 없음

## 양식 브리지 API의 샘플 사용 {#sample-usage-of-form-bridge-api}
