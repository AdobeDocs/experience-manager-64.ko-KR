...
+ AEM Forms 설치 및 구성 {#install-aem-forms}
   ...
   + Install AEM Forms on OSGi {#osgi-installation}
      + [OSGi 기반의 AEM 양식에 대해 지원되는 플랫폼](https://helpx.adobe.com/experience-manager/6-4/sites/deploying/using/technical-requirements.html)
...
+ 업그레이드 AEM Forms {#upgrade-aem-forms}
   ...
   + JEE에서 AEM Forms 업그레이드 {#aem-forms-jee-upgrade}
      ...
      + [AEM Forms 릴리스에서 JBoss용 JEE의 AEM 6.4 Forms으로 업그레이드](http://www.adobe.com/go/learn_aemforms_upgradeJBoss_64)
      + [JBoss용 JEE의 LiveCycle ES4 SP1에서 Adobe Experience Manager Forms으로 업그레이드](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/pdf/upgrade-jboss-livecycle.pdf)
      + [JBoss 턴키용 JEE의 AEM Forms 릴리스에서 AEM 6.4 Forms으로 업그레이드](http://www.adobe.com/go/learn_aemforms_upgradeTurnkey_64)
      + [JBoss 턴키용 JEE의 LiveCycle ES4 SP1에서 AEM Forms으로 업그레이드](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/pdf/upgrade-turnkey-livecycle.pdf)
      + [WebLogic용 JEE의 AEM Forms 릴리스에서 AEM 6.4 Forms으로 업그레이드](http://www.adobe.com/go/learn_aemforms_upgradeWebLogic_64)
      + [WebLogic용 JEE의 LiveCycle ES4 SP1에서 AEM Forms으로 업그레이드](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/pdf/upgrade-weblogic-livecycle.pdf)
      + [WebSphere용 JEE의 AEM Forms 릴리스에서 AEM 6.4 Forms으로 업그레이드](http://www.adobe.com/go/learn_aemforms_upgradeWebSphere_64)
      + [WebSphere용 JEE에서 LiveCycle ES4 SP1에서 AEM Forms으로 업그레이드](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/pdf/upgrade-websphere-livecycle.pdf)
+ AEM Forms 관리 {#manage-administer-aem-forms}
   + [OSGi 그룹 및 특권](using/forms-groups-privileges-tasks.md)
   ...
   + AEM Forms 환경 강화 {#hardening-aem-forms-environment}
      + [OSGi 환경의 AEM 양식 강화 및 보안](using/hardening-securing-aem-forms-environment.md)
...
+ 적응형 Forms - 기본 제작 {#adaptive-forms-basic-authoring}
   ...
   + [AEM 사이트 페이지에 적응형 양식 또는 인터랙티브한 커뮤니케이션 포함](using/embed-adaptive-form-aem-sites.md)
...
+ AEM Forms 작업 공간 {#use-aem-forms-workspace}
   ...
   + [하나의 서버에 두 개의 AEM Forms 작업 영역 인스턴스 호스팅](using/two-html-workspace-instances-one.md)
...
+ 문서 보안 {#use-document-security}
   ...
   + [Portable Protection Library를 사용하여 정책으로 보호된 PDF 문서 확장 Reader](using/reader-extending-policy-protected-pdf-documents-using-ppl.md)
   + [AEM에서 보호된 PDF 및 Microsoft Office 문서를 검색할 수 있도록 설정](using/enable-search-for-drm-protected-pdf-and-office-documents-in-aem.md)
...
+ JEE의 AEM Forms 관리자 도움말 {#administrator-help}
   ...
   + 사용자 관리 구성 {#configure-user-management}
      ...
      + [AEM 양식에서 SSO(Single Sign-On) 사용](/help/forms/using/admin-help/enabling-single-sign-on-aem.md)
   ...
   + Acrobat Reader DC 확장 구성 {#configure-acrobat-reader-dc-extensions}
      ...
      + [Acrobat Reader DC 확장 기능에 사용할 시간 초과 값 설정](/help/forms/using/admin-help/setting-timeout-values-acrobat-reader.md)
...
+ 개발자 참조 {#developer-reference}
   ...
   + [적응형 양식 디버그를 위한 AEM 플러그인](https://adobe-consulting-services.github.io/acs-aem-tools/aem-chrome-plugin/adaptive-form/)
   ...
   + JEE에서 AEM Forms을 사용한 프로그래밍 {#programming-aem-forms-jee}
      ...
      + API를 사용하여 서비스 작업 수행 {#performing-service-operations-using-apis}
         ...
         + [렌더링 Forms](developing/rendering-forms.md)
         ...
         + [제출된 처리 Forms](developing/handling-submitted-forms.md)
         ...
         + [Forms을 플로우 가능한 레이아웃으로 미리 채우기](developing/prepopulating-forms-flowable-layouts.md)
