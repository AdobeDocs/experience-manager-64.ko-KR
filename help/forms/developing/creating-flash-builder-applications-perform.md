...
HTTP 토큰을 사용하여 SSO(Single Sign On) 인증을 수행하는 Flash Builder을 사용하여 클라이언트 응용 프로그램을 만들 수 있습니다. 예를 들어 Flash Builder을 사용하여 웹 기반 응용 프로그램을 만든다고 가정합니다. 애플리케이션에 서로 다른 보기가 있으며 각 보기가 다른 AEM Forms 작업을 불러온다고 가정합니다. 각 Forms 작업에 대해 사용자를 인증하는 대신 사용자가 한 번 인증할 수 있는 로그인 페이지를 만들 수 있습니다. 인증된 사용자는 다시 인증하지 않고도 여러 작업을 호출할 수 있습니다. 예를 들어 사용자가 작업 공간(또는 다른 Forms 애플리케이션)에 로그인한 경우에는 사용자가 다시 인증할 필요가 없습니다.

클라이언트 응용 프로그램에 SSO 인증을 수행하는 데 필요한 응용 프로그램 로직이 포함되어 있지만 AEM Forms 사용자 관리는 실제 사용자 인증을 수행합니다. HTTP 토큰을 사용하여 사용자를 인증하기 위해 클라이언트 응용 프로그램은 Authentication Manager 서비스의 작업을 `authenticateWithHTTPToken` 호출합니다. 사용자 관리는 HTTP 토큰을 사용하여 사용자를 인증할 수 있습니다. 이후 원격 또는 웹 서비스 호출이 AEM Forms에 대해 인증을 위해 자격 증명을 전달할 필요가 없습니다.

>[!NOTE]
>
>이 섹션을 읽기 전에 Remoting을 사용하여 Inferring AEM Forms에 익숙한 것이 좋습니다. (AEM Forms [Remoting을 사용하여 AEM Forms 호출을 참조하십시오](/help/forms/developing/invoking-aem-forms-using-remoting.md#invoking-aem-forms-using-remoting).)

사용자가 SSO를 사용하여 인증된 후 이름이 지정된 다음 AEM Forms `MyApplication/EncryptDocument`의 단기 프로세스가 호출됩니다. 입력 및 출력 값과 같은 이 프로세스에 대한 자세한 내용은 [짧은 기간 프로세스 예를 참조하십시오](/help/forms/developing/aem-forms-processes.md).
...
Flash Builder을 사용하여 작성한 클라이언트 응용 프로그램은 및 에 구성된 사용자 관리자의 보안 서블릿과 상호 `/um/login` 작용합니다 `/um/logout`. 즉, 클라이언트 응용 프로그램은 시작 중에 사용자의 상태를 확인하기 위해 요청을 `/um/login` URL로 보냅니다. 그러면 사용자 관리자가 사용자 상태에 응답합니다. 클라이언트 응용 프로그램 및 User Manager 보안 서블릿은 HTTP를 사용하여 통신합니다.

**요청 형식**

보안 서블릿에는 다음과 같은 입력 변수가 필요합니다.

* `um_no_redirect` - 이 값은 반드시 입력해야 합니다 `true`. 이 변수는 User Manager 보안 서블릿에 수행된 모든 요청을 동반합니다. 또한 보안 서블릿이 flex 클라이언트 또는 기타 웹 응용 프로그램에서 들어오는 요청을 차별화할 수 있습니다.
* `j_username` - 이 값은 로그인 양식에 제공된 사용자의 로그인 식별자 값입니다.
* `j_password` - 이 값은 로그인 양식에 제공된 사용자의 해당 암호입니다.

이 `j_password` 값은 자격 증명 요청에만 필요합니다. 암호 값이 지정되지 않은 경우 보안 서블릿은 사용 중인 계정이 이미 인증되었는지 확인하기 위해 확인합니다. 그렇다면 계속 진행하십시오. 그러나 보안 서블릿은 사용자를 다시 인증하지 않습니다.
...
에 구성된 보안 서블릿은 형식을 사용하여 `/um/login` 응답합니다 `URLVariables` . 이 형식으로 내용 유형의 출력은 text/plain입니다. 출력에는 앰퍼샌드(&amp;) 문자로 구분된 이름 값 쌍이 포함됩니다. 응답에는 다음 변수가 포함됩니다.

* `authenticated` - 값은 `true` 또는 `false`입니다.
* `authstate` - 이 값은 다음 값 중 하나를 포함할 수 있습니다.

   * `CREDENTIAL_CHALLENGE` - 이 상태는 사용자 관리자가 어떤 수단을 통해서도 사용자의 ID를 확인할 수 없음을 나타냅니다. 인증이 수행되려면 사용자의 사용자 이름과 암호가 필요합니다.
   * `SPNEGO_CHALLENGE`- 이 주는 동일한 것으로 처리됩니다 `CREDENTIAL_CHALLENGE`.
...
* `assertionid` - 상태가 `COMPLETE` 그렇다면 사용자의 `assertionId` 값이 포함됩니다. 클라이언트 응용 프로그램은 사용자의 응용 프로그램 `AuthResult` 을 가져올 수 있습니다.
...
1. 이름이 지정된 쿠키를 찾습니다 `lcAuthToken`. 사용자가 이미 다른 Forms 응용 프로그램에 로그인한 경우에는 이 쿠키가 표시됩니다. 쿠키가 발견되면 컨텐츠의 유효성이 확인됩니다.
1. 헤더 기반 SSO가 활성화된 경우 서블릿은 사용자 ID를 결정하기 위해 구성된 헤더를 찾습니다.
1. SPNEGO가 활성화된 경우 서블릿은 SPNEGO를 시작하고 사용자의 ID를 확인하려고 시도합니다.

보안 서블릿이 사용자와 일치하는 유효한 토큰을 찾는 경우 보안 서블릿을 사용하여 작업을 진행하고 응답할 수 있습니다 `authstate=COMPLETE`. 그렇지 않으면 보안 서블릿이 다음으로 응답합니다 `authstate=CREDENTIAL_CHALLENGE`. 다음 목록은 다음 값에 대해 설명합니다.

* `Case authstate=COMPLETE`: 사용자가 인증되고 값에 사용자에 대한 `assertionid` 어설션 식별자가 포함됨을 나타냅니다. 이 단계에서는 클라이언트 응용 프로그램이 AEM Forms에 연결할 수 있습니다. 해당 URL에 대해 구성된 서블릿은 메서드를 호출하여 사용자 `AuthResult` 에 대한 `AuthenticationManager.authenticate(HttpRequestToken)` 서블릿을 가져올 수 있습니다. 인스턴스는 사용자 관리자 컨텍스트를 만들어 세션에 저장할 수 있습니다. `AuthResult`
* `Case authstate=CREDENTIAL_CHALLENGE`: 보안 서블릿에 사용자의 자격 증명이 필요함을 나타냅니다. 응답으로 클라이언트 응용 프로그램은 사용자에게 로그인 화면을 표시하고 가져온 자격 증명을 보안 서블릿으로 전송할 수 있습니다(예: `https://<your_serverhost>:<your_port>/um/login?um_no_redirect=true&j_username=administrator&j_password=password)`.). 인증이 성공하면 보안 서블릿이 응답합니다 `authstate=COMPLETE`.

인증이 여전히 실패할 경우 보안 서블릿이 응답합니다 `authstate=FAILED`. 이 값에 응답하기 위해 클라이언트 응용 프로그램에서 자격 증명을 다시 획득하는 메시지를 표시할 수 있습니다.

>[!NOTE]
>
>그러나 클라이언트 `authstate=CREDENTIAL_CHALLENGE`는 가져온 자격 증명을 POST 형식으로 보안 서블릿으로 보내는 것이 좋습니다.
...
이 요청을 받으면 User Manager 보안 서블릿이 `lcAuthToken` 쿠키를 삭제하고 이에 응답합니다 `authstate=LOGGED_OUT`. 클라이언트 응용 프로그램이 이 값을 수신하면 응용 프로그램에서 정리 작업을 수행할 수 있습니다.
...
SSO 인증을 수행하는 클라이언트 응용 프로그램을 만드는 방법을 보여 주는 예제 클라이언트 응용 프로그램이 만들어집니다. 다음 그림은 클라이언트 응용 프로그램이 SSO를 사용하여 사용자를 인증하기 위해 수행하는 단계를 보여줍니다.
...
1. 전화가 `ISSOManager.singleSignOn` 왔습니다. 클라이언트 응용 프로그램이 User Manager 보안 서블릿으로 요청을 보냅니다.
1. 보안 서블릿이 사용자를 인증하면 `ISSOManager` 전달됩니다 `SSOEvent.AUTHENTICATION_SUCCESS`. 응답으로 클라이언트 응용 프로그램은 기본 페이지를 표시합니다. 이 예제에서 기본 페이지는 MyApplication/EncryptDocument라는 짧은 기간 동안의 AEM Forms 프로세스를 호출합니다.
1. 보안 서블릿이 사용자가 유효한지 여부를 확인할 수 없는 경우 애플리케이션이 사용자 자격 증명을 다시 요청합니다. 이 `ISSOManager` 클래스는 이벤트를 `SSOEvent.AUTHENTICATION_REQUIRED` 전달합니다. 클라이언트 응용 프로그램에 로그인 페이지가 표시됩니다.
...
* `SSOStandalone.mxml`: 클라이언트 응용 프로그램을 나타내는 주 MXML 파일입니다. (SOStandalone.mxml 파일 [만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-ssostandalone-mxml-file).)
* `um/ISSOManager.as`: SSO(Single Sign On)와 관련된 작업 노출 (ISSOManager.as [파일 만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-issomanager-as-file).)
* `um/SSOEvent.as`: SSO 관련 이벤트에 대해 `SSOEvent` 전달됩니다. (SOEvent.as [파일 만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-ssoevent-as-file).)
* `um/SSOManager.as`: SSO 관련 작업을 관리하고 적절한 이벤트를 전달합니다. SOManager.as 파일 [만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-ssomanager-as-file).
* `um/UserManager.as`: WSDL을 사용하여 인증 관리자 서비스를 호출하는 응용 프로그램 논리를 포함합니다. (UserManager.as [파일 만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-usermanager-as-file).)
* `views/login.mxml`: 로그인 화면을 나타냅니다. login.mxml 파일 [만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-login-mxml-file).
* `views/logout.mxml`: 로그아웃 화면을 나타냅니다. 자세한 내용은 [logout.mxml 파일 만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-logout-mxml-file).
* `views/progress.mxml`: 진행 보기를 나타냅니다. progress.mxml 파일 [만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-progress-mxml-file).
* `views/remoting.mxml`: Remoting을 사용하여 MyApplication/EncryptDocument라는 짧은 기간 동안의 프로세스를 호출하는 보기를 나타냅니다. remoting.mxml 파일 [만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-remoting-mxml-file).

다음 그림에서는 클라이언트 응용 프로그램을 시각적으로 보여 줍니다.

![cf_sso_project](assets/cf_cf_sso_project.png)

>[!NOTE]
>
>um과 view라는 두 개의 패키지가 있습니다. 클라이언트 응용 프로그램을 만들 때 해당 파일을 적합한 패키지에 배치해야 합니다. 또한 adobe-remoting-provider.swc 파일을 프로젝트의 클래스 경로에 추가해야 합니다. AEM Forms Flex 라이브러리 파일 [포함을 참조하십시오](/help/forms/developing/invoking-aem-forms-using-remoting.md#including-the-aem-forms-flex-library-file).

### SSOStandalone.mxml 파일 만들기 {#creating-the-ssostandalone-mxml-file}

다음 코드는 SOStandalone.mxml 파일을 나타냅니다.
...
다음 코드는 프로세스를 호출하는 remoting.mxml 파일을 `MyApplication/EncryptDocument` 나타냅니다. 문서가 프로세스에 전달되기 때문에 이 파일에 보안 문서를 AEM Forms으로 전달하는 응용 프로그램 논리가 있습니다. (원격 [을 사용하여 프로세스를 호출하는 보안 문서 전달을 참조하십시오](/help/forms/developing/invoking-aem-forms-using-remoting.md#passing-secure-documents-to-invoke-processes-using-remoting).)
...
다음 섹션에서는 클라이언트 응용 프로그램과 User Manager 보안 서블릿 간의 통신을 설명하는 추가 세부 정보를 제공합니다.

### 새 인증 발생 {#a-new-authentication-occurs}

이 경우 사용자가 클라이언트 응용 프로그램에서 처음으로 AEM Forms으로 로그인을 시도합니다. (사용자와 관련된 이전 세션이 없습니다.) 이 `applicationComplete` 이벤트에서 사용자 `SSOManager.singleSignOn` 관리자에게 요청을 보내는 메서드가 호출됩니다.
...
이 값에 대한 응답으로 값이 `SSOEvent.AUTHENTICATION_REQUIRED` 전달됩니다. 그 결과 클라이언트 응용 프로그램은 사용자에게 로그인 화면을 표시합니다. 자격 증명은 User Manager 보안 서블릿으로 다시 제출됩니다.
...
따라서 `authstate=COMPLETE the SSOEvent.AUTHENTICATION_SUCCESS` 전달됩니다. 클라이언트 응용 프로그램은 필요한 경우 추가 처리를 수행할 수 있습니다. 예를 들어 사용자가 인증된 날짜와 시간을 추적하는 로그를 만들 수 있습니다.

### 사용자가 이미 인증되었습니다. {#the-user-is-already-authenticated}

이 경우 사용자가 이미 AEM Forms에 로그인한 다음 클라이언트 응용 프로그램으로 이동합니다. 클라이언트 응용 프로그램은 시작 중에 User Manager 보안 서블릿에 연결됩니다.
...
사용자가 이미 인증되었기 때문에 사용자 관리자 쿠키가 존재하고 User Manager 보안 서블릿으로 전송됩니다. 그런 다음 서블릿은 값을 `assertionId` 가져와 유효한지 확인합니다. 유효하면 `authstate=COMPLETE` 반환됩니다. 그렇지 않으면 `authstate=CREDENTIAL_CHALLENGE` 반환됩니다. 다음은 일반적인 응답입니다.
