...
책갈피가 포함된 PDF 문서를 취합할 수 있습니다. 예를 들어 책갈피를 포함하지 않는 PDF 문서가 있고 책갈피를 제공하여 수정하려고 한다고 가정합니다. Assembler 서비스를 사용하면 책갈피가 포함되지 않은 PDF 문서를 전달하여 책갈피가 포함된 PDF 문서를 다시 가져올 수 있습니다.
...
이 DDX 문서 내에서 소스 속성에 값이 지정되어 있음을 알 수 있습니다 `Loan.pdf`. 이 DDX 문서는 단일 PDF 문서를 어셈블러 서비스로 전달하도록 지정합니다. 책갈피가 있는 PDF 문서를 취합할 때는 결과 문서의 책갈피를 설명하는 책갈피 XML 문서를 지정해야 합니다. 책갈피 XML 문서를 지정하려면 DCX 문서에 `Bookmarks` 요소가 지정되어 있는지 확인합니다.

이 DDX 문서에서 `Bookmarks` 요소는 값 `doc2` 으로 지정합니다. 이 값은 어셈블러 서비스로 전달된 입력 맵에 이름이 지정된 키가 포함되어 있음을 나타냅니다 `doc2`. 키 값은 `doc2` 책갈피 XML 문서를 나타내는 `com.adobe.idp.Document` 값입니다. 어셈블리 서비스의 &quot;책갈피 [언어&quot;와 DDX 참조를 참조하십시오](https://www.adobe.com/go/learn_aemforms_ddx_63).
...
이 책갈피 XML 문서 내에서 사용자가 책갈피를 클릭할 때 수행되는 작업을 정의하는 작업 요소를 확인합니다. 동작 요소 아래에는 NotePad와 같은 응용 프로그램을 실행하고 PDF 파일과 같은 파일을 여는 론치 요소가 있습니다. PDF 파일을 열려면 열 파일을 지정하는 파일 요소를 사용해야 합니다. 예를 들어 이 섹션에 지정된 책갈피 XML 파일에서 여는 파일의 이름은 LoanDetails.pdf입니다.

>[!NOTE]
>
>지원되는 작업에 대한 자세한 내용은 어셈블리 서비스 및 DDX 참조 `Action` 의 &quot; [요소&quot;를 참조하십시오](https://www.adobe.com/go/learn_aemforms_ddx_63).

이 섹션에 지정된 DCX 문서를 입력하고 XML 파일을 입력으로 책갈피로 지정한 경우 어셈블러 서비스는 다음 책갈피가 포함된 PDF 문서를 어셈블합니다.

![aw_aw_bmark](assets/aw_aw_bmark.png)

사용자가 대출 세부 정보 *책갈피* 열기를 클릭하면 LoanDetails.pdf가 열립니다. 마찬가지로 사용자가 *시작 노트* 패드 책갈피를 클릭하면 노트 패드가 시작됩니다.

>[!NOTE]
>
>이 섹션을 읽기 전에 Assembler 서비스를 사용하여 PDF 문서를 조합하는 것에 익숙해지는 것이 좋습니다. 이 섹션에서는 입력 문서를 포함하는 컬렉션 개체를 만들거나 반환된 컬렉션 개체에서 결과를 추출하는 방법을 배우는 등의 개념을 논하지 않습니다. 자세한 내용은 [프로그래밍 방식으로 PDF 문서 조합을 참조하십시오](/help/forms/developing/programmatically-assembling-pdf-documents.md#programmatically-assembling-pdf-documents).
...
PDF 문서를 조합하려면 DDX 문서를 참조해야 합니다. 이 DDX 문서에는 책갈피가 포함된 PDF를 어셈블하는 데 필요한 `Bookmarks` 요소가 포함되어야 합니다. (예제는 이 섹션 앞부분에 나와 있는 DCX 문서를 참조하십시오.)

**책갈피가 추가되는 PDF 문서 참조**

책갈피가 추가되는 PDF 문서를 참조합니다. 참조된 PDF 문서에 이미 책갈피가 포함되어 있는지 여부는 중요하지 않습니다. 요소가 PDF 소스 `Bookmarks` 요소의 하위 요소인 경우 책갈피는 PDF 소스에 이미 존재하는 요소를 대체합니다. 그러나 기존 책갈피를 유지하려면 PDF 소스 요소 `Bookmarks` 의 동위 항목인지 확인하십시오. 예를 들어 다음 예를 생각해 보십시오.
...
>[!NOTE]
>
>어셈블리 서비스 및 DDC 참조에서 [&quot;책갈피 언어&quot;를 참조하십시오](https://www.adobe.com/go/learn_aemforms_ddx_63).
...
새 책갈피가 포함된 PDF 문서를 어셈블하려면 어셈블러 서비스의 작업을 `invokeDDX` 사용합니다. 이와 같은 다른 어셈블러 서비스 작업과 반대로 작업을 사용해야 하는 이유는 어셈블러 서비스에 맵 수집 개체 내에 전달되는 책갈피 XML 문서가 필요하기 때문입니다 `invokeDDX` `invokeOneDocument` . 이 개체는 작업의 `invokeDDX` 매개 변수입니다.

**책갈피가 포함된 PDF 문서 저장**

반환된 맵 개체에서 결과를 추출하고 해당 PDF 문서를 저장해야 합니다. 자세한 내용은 프로그래밍 방식으로 [PDF 문서 조합](/help/forms/developing/programmatically-assembling-pdf-documents.md)의 &quot;결과 추출&quot;을 참조하십시오.
...
1. PDF 문서와 책갈피 XML 문서를 Map 컬렉션에 추가합니다.
   ...
   * 개체의 메서드를 호출하고 다음 인수를 전달하여 입력 PDF 문서 `java.util.Map` 를 `put` 추가합니다.

      * 키 이름을 나타내는 문자열 값입니다. 이 값은 DCX 문서에 지정된 PDF 소스 요소의 값과 일치해야 합니다.
      * 입력 PDF 문서를 포함하는 `com.adobe.idp.Document` 오브젝트입니다.
   * 개체의 메서드를 호출하고 다음 인수를 전달하여 책갈피 XML 문서를 `java.util.Map` `put` 추가합니다.
...
1. PDF 문서 조합
   ...
   * 입력 PDF 문서와 책갈피 XML 문서를 모두 포함하는 `java.util.Map` 오브젝트입니다.
...
1. 책갈피가 추가되는 PDF 문서를 참조합니다.
   ...
   * 생성자를 사용하여 `BLOB` 개체를 만듭니다. 이 `BLOB` 개체는 입력 PDF를 저장하는 데 사용됩니다.
...
1. 책갈피 XML 문서를 참조합니다.
   ...
   * 생성자를 사용하여 `BLOB` 개체를 만듭니다. 이 `BLOB` 개체는 책갈피 XML 문서를 저장하는 데 사용됩니다.
   * 생성자를 호출하고 입력 PDF 문서의 파일 위치와 파일을 열 모드를 나타내는 문자열 값을 전달하여 `System.IO.FileStream` 개체를 만듭니다.
   * 개체의 내용을 저장하는 바이트 배열을 `System.IO.FileStream` 만듭니다. 개체의 속성을 가져와 바이트 배열의 크기를 결정할 수 `System.IO.FileStream` `Length` 있습니다.
   * 개체의 메서드를 호출하고 바이트 배열, 시작 위치 및 읽을 스트림 길이를 전달하여 바이트 배열 `System.IO.FileStream` `Read` 을 스트림 데이터로 채웁니다.
   * 바이트 배열의 내용으로 해당 `BLOB` `MTOM` 필드를 할당하여 개체를 채웁니다.
...
1. PDF 문서와 책갈피 XML 문서를 Map 컬렉션에 추가합니다.
   ...
   * 개체를 `MyMapOf_xsd_string_To_xsd_anyType` 만듭니다. 이 컬렉션 개체는 입력 PDF 문서와 책갈피 XML 문서를 저장하는 데 사용됩니다.
   ...
   * 개체에 `MyMapOf_xsd_string_To_xsd_anyType_Item` 개체를 `MyMapOf_xsd_string_To_xsd_anyType` 추가합니다. 개체의 `MyMapOf_xsd_string_To_xsd_anyType` 메서드를 호출하고 개체를 `Add` `MyMapOf_xsd_string_To_xsd_anyType` 전달합니다. 각 입력 PDF 문서 및 책갈피 XML 문서에 대해 이 작업을 수행합니다.
...
1. PDF 문서 조합
   ...
   * 런타임 옵션을 지정하는 `AssemblerOptionSpec` 개체
   ...
   이 `invokeDDX` `AssemblerResult` 메서드는 작업 결과와 발생했을 수 있는 예외를 포함하는 개체를 반환합니다.
...
1. 책갈피가 포함된 PDF 문서를 저장합니다.
   ...
   * 개체의 필드에 액세스하여 PDF 문서를 나타내는 바이너리 데이터 `BLOB` 를 `MTOM` 추출합니다. PDF 파일에 기록할 수 있는 바이트 배열을 반환합니다.
