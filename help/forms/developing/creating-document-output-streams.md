...
1. PDF/A 문서를 생성합니다.
   ...
   * 양식 디자인과 병합할 데이터가 포함된 XML 데이터 소스가 포함된 `com.adobe.idp.Document` 객체입니다.
   ...
   >[!NOTE]
   >
   >개체 `OutputResult` 의 `getRecordLevelMetaDataList` 메서드는 `null`*.
...
AEM Forms 저장소에서 출력 서비스로 가져온 문서를 전달하려면 다음 작업을 수행하십시오.
...
>[!NOTE]
>
>양식 디자인 `generatePDFOutput2`이 포함된 `generatePrintedOutput2`개체 `com.adobe.idp.Document`를 사용하여 이름을 지정하고 수락하는 두 가지 새로운 메서드입니다. 또한 인쇄 스트림을 네트워크 프린터로 전송할 때 양식 디자인이 포함된 `com.adobe.idp.Document` 파일을 출력 서비스로 전달할 수 있습니다.
...
1. 출력 및 어셈블러 클라이언트 개체를 만듭니다.
   ...
   * `BasicHttpSecurityMode.TransportCredentialOnly` 필드에 `BasicHttpBindingSecurity.Security.Mode`상수 값을 지정합니다.
   ...
   >[!NOTE]
   >
   >* 개체에 대해 `AssemblerServiceClient`다음 단계를 반복합니다. *
...
1. 파일로 인쇄하는 데 필요한 인쇄 런타임 옵션을 설정합니다.
   ...
   * PrintedOutputOptionsSpec 개체의 메서드를 호출하고 파일의 이름과 위치를 나타내는 문자열 값을 전달하여 파일을 지정합니다. `setFileURI` 예를 들어 출력 서비스가 C:\Adobe에 있는 PostScript 파일* MorgageForm.ps*로 인쇄되도록 하려면 C:\\Adobe\MortgageForm.ps을 지정합니다.
...
1. 인쇄 런타임 옵션 설정
   ...
   >[!NOTE]
   >
   >ZPL 인쇄 스트림을 생성하는 경우 `PrintedOutputOptionsSpec` 개체의 `setPagination` 방법을 사용하여 페이지 매김 값을 설정할 수 없습니다. 마찬가지로 ZPL 인쇄 스트림에 대해 다음 옵션을 설정할 수 없습니다. 출력 조그, 페이지 오프셋 및 스테이플. * `setPagination`메서드는 PostScript 생성에서 유효하지 않습니다. PCL 생성 전용입니다. *
...
1. 인쇄 런타임 옵션을 설정합니다.
   ...
   >[!NOTE]
   >
   >ZPL 인쇄 스트림을 생성하는 경우 `PrintedOutputOptionsSpec` 개체의 `pagination` 데이터 멤버를 사용하여 페이지 매김 값을 설정할 수 없습니다. 마찬가지로 ZPL 인쇄 스트림에 대해 다음 옵션을 설정할 수 없습니다. 출력 조그, 페이지 오프셋 및 스테이플. * `pagination`데이터 멤버는 PostScript 생성에서 유효하지 않습니다. PCL 생성 전용입니다. *
