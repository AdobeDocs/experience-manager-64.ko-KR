...
1. 프로젝트 파일 포함
   ...
   >[!NOTE]
   >
   >AEM Forms를 호스팅하는 서버의 IP 주소로 `localhost`*를 바꿉니다.*
...
1. PDF/A 문서를 생성합니다.
   ...
   * 양식 디자인과 병합할 데이터가 들어 있는 XML 데이터 소스가 들어 있는 `com.adobe.idp.Document` 개체입니다.
   ...
   >[!NOTE]
   >
   >객체의 `OutputResult` 메서드는 `getRecordLevelMetaDataList` `null`*를 반환합니다.*
...
1. 출력 및 어셈블러 클라이언트 개체를 만듭니다.
   ...
   * `BasicHttpSecurityMode.TransportCredentialOnly` 필드에 `BasicHttpBindingSecurity.Security.Mode`상수 값을 지정합니다.
   >[!NOTE]
   >
   >이 단계를 `AssemblerServiceClient`* 개체에 대해 반복합니다.*
...
1. 파일로 인쇄하는 데 필요한 인쇄 런타임 옵션을 설정합니다.
   ...
   * PrintedOutputOptionsSpec 개체의 `setFileURI` 메서드를 호출하고 파일의 이름과 위치를 나타내는 문자열 값을 전달하여 파일을 지정합니다. 예를 들어 출력 서비스가 C:\Adobe에 있는 PostScript 파일* MortgageForm.ps*로 인쇄되도록 하려면 C:\\Adobe\MortgageForm.ps을 지정합니다.
...
1. 인쇄 런타임 옵션 설정
   ...
   >[!NOTE]
   >
   >ZPL 인쇄 스트림을 생성하는 경우 `PrintedOutputOptionsSpec` 개체의 `setPagination` 방법을 사용하여 페이지 매김 값을 설정할 수 없습니다. 마찬가지로 ZPL 인쇄 스트림에 대해 다음 옵션을 설정할 수 없습니다.출력 조그, 페이지 오프셋 및 스테이플. PostScript 생성에는 `setPagination`* 메서드가 유효하지 않습니다. PCL 생성에만 유효합니다.*
...
1. 인쇄 런타임 옵션을 설정합니다.
   ...
   >[!NOTE]
   >
   >ZPL 인쇄 스트림을 생성하는 경우 `PrintedOutputOptionsSpec` 개체의 `pagination` 데이터 멤버를 사용하여 페이지 매김 값을 설정할 수 없습니다. 마찬가지로 ZPL 인쇄 스트림에 대해 다음 옵션을 설정할 수 없습니다.출력 조그, 페이지 오프셋 및 스테이플. PostScript 생성에는 `pagination`* 데이터 멤버가 유효하지 않습니다. PCL 생성에만 유효합니다.*
