...
다음 Java 코드 예제는 /Company Home에서 *MorgageForm.pdf* 라는 PDF 파일을 검색합니다. PDF 파일은 로컬 파일 시스템에 저장되고 이름이 UpdatedModerationForm.pdf *로 지정됩니다*.
...
다음 Java 코드는 XML 데이터 파일과 PDF 양식의 연결을 만듭니다. 이 유형의 연결은 LinkedBy라고 합니다. PDF 문서에는 해당 문서에 적용되는 종횡비 링크가 있어야 합니다.
