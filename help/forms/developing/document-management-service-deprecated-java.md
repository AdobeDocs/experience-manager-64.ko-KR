...
>[!NOTE]
>
>Adobe는 2011년 8월 5일부터 Content Services ES 고객을 Adobe Digital Enterprise Platform Experience Services로 마이그레이션합니다. Content Services를 사용하는 고객을 위한 제품 로드맵은 Adobe의 Day Software 인수 과정에서 취득한 최신 모듈형 CRX 아키텍처를 기반으로 구축된 기본 컨텐츠 저장소를 포함하는 새로운 ADEP Experience Services - Core로 이동하는 것입니다.

[빠른 시작(SOAP 모드):Java API 파섹](document-management-service-deprecated-java.md#quick-start-soap-mode-create-content-services-spaces-using-the-java-api-deprecated)
...
[빠른 시작(SOAP 모드):Java API 파섹](document-management-service-deprecated-java.md#quick-start-soap-mode-retrieve-content-from-content-services-using-the-java-api-deprecated)

[빠른 시작(SOAP 모드):Java API 파섹](document-management-service-deprecated-java.md#quick-start-soap-mode-move-content-services-content-using-the-java-api-deprecated)

[빠른 시작(SOAP 모드):Java API를 사용하여 컨텐츠 서비스 컨텐츠 나열](document-management-service-deprecated-java.md#quick-start-soap-mode-list-content-services-content-using-the-java-api-deprecated)
...
***참고&#x200B;**:AEM 양식을 사용한 프로그래밍에서 빠른 시작은 JBoss 및 Windows 운영 체제에 배포되는 Forms 서버를 기반으로 합니다. 그러나 UNIX와 같은 다른 운영 체제를 사용하는 경우 창 특정 경로를 해당 운영 체제에서 지원되는 경로로 바꿉니다. 마찬가지로 다른 J2EE 응용 프로그램 서버를 사용하는 경우 올바른 연결 속성을 지정해야 합니다. 연결[속성](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)설정을 참조하십시오.*
...
다음 Java 코드 예제에서는 /Company Home에서 MortgageForm.pdf *라는* PDF 파일을 검색합니다. PDF 파일은 로컬 파일 시스템에 저장되고 이름이 UpdatedModerationForm.pdf *로 지정됩니다*.
...
다음 Java 코드 예제에서는 MortgageForm.pdf라는 PDF 파일을 */Company Home/Test Directory* 에서 /Company Home으로 이동합니다. 이동한 컨텐츠의 식별 값이 콘솔에 기록됩니다.
...
다음 Java 코드는 /Company Home에서 MortgageForm 텍스트가 포함된 문서를 검색합니다. 하위 폴더도 검색됩니다.
...
다음 Java 코드 예제에서는 tony blue라는 사용자에 대한 권한을 설정합니다. 지정된 도메인이 기본 도메인입니다. 소비자 권한이 지정되고 노드가 `/Company Home/Test Directory`지정됩니다.
...
다음 Java 코드는 XML 데이터 파일과 PDF 양식의 연결을 만듭니다. 이러한 유형의 연결은 LinkedBy로 명명됩니다. PDF 문서에 적용되는 종횡비 링크가 있어야 합니다.
