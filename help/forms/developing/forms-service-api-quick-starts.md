...
>[!NOTE]
>
>v로 프로그래밍에 있는 빠른 시작은 Unix와 같은 다른 운영 체제를 사용하는 양식 서버를 기반으로 하며 Windows 특정 경로를 해당 운영 체제에서 지원하는 경로로 바꿉니다. 마찬가지로, 다른 J2EE 응용 프로그램 서버를 사용하는 경우 올바른 연결 속성을 지정해야 합니다. 자세한 내용은 [연결 속성 설정](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties).


## 빠른 시작(SOAP 모드): Java API를 사용하여 대화형 PDF 양식 렌더링 {#quick-start-soap-mode-rendering-an-interactive-pdf-form-using-the-java-api}

다음 코드 예제에서는 이름이 인 대화형 PDF 양식을 렌더링합니다. *Loan.xdp* 클라이언트 웹 브라우저에 매핑됩니다. 양식에 파일이 첨부됩니다. 양식 디자인은 응용 프로그램의 일부이며 컨텐츠 루트 URI 값을 사용하여 참조됩니다 `repository:///`. (자세한 내용은 [대화형 PDF forms 렌더링](/help/forms/developing/rendering-forms.md#rendering-interactive-pdf-forms))
...
다음 코드 예제에서는 Forms 서비스 Java API를 사용하여 클라이언트에서 *Loan.xdp *라는 양식을 렌더링합니다. 양식 디자인은 응용 프로그램의 일부이며 컨텐츠 루트 URI 값을 사용하여 참조됩니다 `repository:///`. (자세한 내용은 [클라이언트에서 Forms 렌더링](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client))
...
다음 코드 예는 라는 안내서(더 이상 사용되지 않음)를 렌더링합니다. *TLALifeClaim.xdp* 클라이언트 웹 브라우저에 매핑됩니다.
...
다음 코드 예제에서는 조각을 기반으로 하는 양식을 렌더링합니다. 양식 디자인의 이름은 *PurchaseOrderDynamic.xdp* AEM Forms 리포지토리에 있습니다(XDP 파일은 리포지토리에 있는 FormsFolder라는 폴더에 저장됨). 또한 POFragment 양식 참조가 참조하는 조각도 리포지토리에 있어야 합니다. (자세한 내용은 [조각을 기반으로 Forms 렌더링](/help/forms/developing/rendering-forms.md#rendering-forms-based-on-fragments))
...
다음 코드 예제에서는 권한이 활성화된 양식을 클라이언트 웹 브라우저에 렌더링합니다. 이 코드 예제에서 설정된 사용 권한 을 통해 사용자는 양식에 주석을 추가하고 양식 데이터를 저장할 수 있습니다. (자세한 내용은 [렌더링 권한 사용 Forms](/help/forms/developing/rendering-forms.md#rendering-rights-enabled-forms))
...
다음 코드 예제에서는 Forms 서비스 Java API를 사용하여 HTML 양식을 렌더링합니다. 도구 모음이 HTML 양식과 첨부 파일 두 개에 추가됩니다. 또한 사용자 에이전트 값은 `HttpServletRequest` 개체. (자세한 내용은 [Forms을 HTML으로 렌더링](/help/forms/developing/rendering-forms.md#rendering-forms-as-html))
...
다음 코드 예제에서는 Forms 서비스 클라이언트 API를 사용하여 HTML 양식을 렌더링합니다. 참조되는 사용자 지정 CSS 파일의 이름은 다음과 같습니다 *custom.css*. (자세한 내용은 [사용자 지정 CSS 파일을 사용하여 HTML Forms 렌더링](/help/forms/developing/rendering-forms.md#rendering-html-forms-using-custom-css-files))
...
다음 코드 예제에서는 프랑스어로 표시되는 도구 모음이 있는 HTML 양식을 렌더링합니다. fscmenu.xml의 위치는 C:\Adobe 입니다(이 폴더는 AEM Forms을 호스팅하는 서버에 있어야 함). 로케일 값은 다음과 같습니다. `fr_FR`. 사용자 지정 도구 모음을 사용하여 HTML 양식을 렌더링하는 방법에 대해 설명하는 섹션에서는 이 빠른 시작에 사용되는 fscmenu.xml 파일의 구문을 보여줍니다. (자세한 내용은 [사용자 지정 도구 모음을 사용하여 HTML Forms 렌더링](/help/forms/developing/rendering-forms.md#rendering-html-forms-with-custom-toolbars))
...
다음 코드 예제에서는 XML로 제출된 양식을 처리합니다. 에 전달된 컨텐츠 유형 값 `processFormSubmission` 메서드 `CONTENT_TYPE=text/xml`. 이름이 지정된 필드에 해당하는 값 `mortgageAmount`, `lastName`, 및 `firstName` 표시됩니다. 이름이 지정된 사용자 정의 메서드 `getNodeText` 이 빠른 시작에 사용됩니다. 수락합니다. `org.w3c.dom.Document` 인스턴스 및 노드 이름을 지정하는 문자열 값입니다. 이 메서드는 노드의 값을 나타내는 문자열 값을 반환합니다. (자세한 내용은 [제출된 Forms 처리](/help/forms/developing/rendering-forms.md#handling-submitted-forms))
...
>[!NOTE]
>
>사용 시 `com.adobe.idp.Document` 개체 및 `org.w3c.dom.Document` 동일한 응용 프로그램에서 `org.w3c.dom.Document`.
...
다음 코드 예제에서는 PDF 데이터로 제출된 양식을 처리합니다. 에 전달된 컨텐츠 유형 값 `processFormSubmission` 메서드 `CONTENT_TYPE=application/pdf`. 제출된 양식은 이름이 인 PDF 파일로 저장됩니다. *tempPDF.pdf*. 또한 양식이 PDF으로 제출되므로 첨부 파일을 검색할 수 있습니다. 모든 첨부 파일은 JPEG 파일로 저장됩니다. (자세한 내용은 [제출된 Forms 처리](/help/forms/developing/rendering-forms.md#handling-submitted-forms))
...
다음 코드 예제에서는 XML 데이터로 제출된 HTML 양식을 처리합니다. 에 전달된 컨텐츠 유형 값 `processFormSubmission` 메서드 `CONTENT_TYPE=application/x-www-form-urlencoded`.이름이 지정된 필드에 해당하는 값입니다 `mortgageAmount`, `lastName`, 및 `firstName` 표시됩니다. 이름이 지정된 사용자 정의 메서드 `getNodeText` 이 빠른 시작에 사용됩니다. 수락합니다. `org.w3c.dom.Document` 인스턴스 및 노드 이름을 지정하는 문자열 값입니다. 이 메서드는 노드의 값을 나타내는 문자열 값을 반환합니다. (자세한 내용은 [제출된 Forms 처리](/help/forms/developing/rendering-forms.md#handling-submitted-forms))
...
## 빠른 시작(SOAP 모드): Java API를 사용하여 제출된 XML 데이터를 사용하여 PDF 문서 만들기 {#quick-start-soap-mode-creating-pdf-documents-with-submitted-xml-data-using-the-java-api}

다음 Java 코드 예제에서는 XML로 제출된 양식 데이터를 처리합니다. 양식 데이터는 Forms API를 사용하여 양식 제출에서 검색되고 출력 서비스로 전송됩니다. 양식 데이터와 양식 디자인은 비대화형 PDF 문서를 만드는 데 사용됩니다. 비대화형 PDF 문서는 이름이 인 Content Services(사용 중지) 노드에 저장됩니다. `/Company Home/Test Directory`. 양식 이름이 동적으로 만들어집니다. 즉, PDF 파일의 이름을 지정하는 데 사용자의 이름과 성이 사용됩니다. 새 컨텐츠의 리소스 식별자가 클라이언트 웹 브라우저에 작성됩니다. (자세한 내용은 [제출된 XML 데이터를 사용하여 PDF 문서 만들기](/help/forms/developing/rendering-forms.md#creating-pdf-documents-with-submitted-xml-data))
...
## 빠른 시작(SOAP 모드): Java API를 사용하여 유동형 레이아웃으로 Forms 미리 채우기 {#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api}
...
* `createDataSource`: 만들기 `org.w3c.dom.Document` 양식을 미리 채우는 데 사용되는 데이터 소스를 나타내는 객체입니다. 이 사용자 정의 메서드는 `org.w3c.dom.Document` 개체.
* `convertDataSource`: 변환 `org.w3c.dom.Document` 개체 `com.adobe.idp.Document` 개체. 이 메서드는 `org.w3c.dom.Document` 개체를 입력 매개 변수로 사용하고 `com.adobe.idp.Document` 개체.
* `renderPOForm`: Forms 서비스 Java API를 사용하여 동적 구매 발주 양식을 렌더링합니다. 다음 `com.adobe.idp.Document` 반환되는 개체 `convertDataSource` 메서드를 사용하여 양식을 미리 채웁니다.

   이러한 모든 메서드는 Java 서블릿 내에서 호출됩니다 `doPost` 메서드를 사용합니다. (자세한 내용은 [유동형 레이아웃으로 Forms 미리 채우기](/help/forms/developing/rendering-forms.md#prepopulating-forms-with-flowable-layouts))
...
다음 코드 예제에서는 계산 스크립트가 포함된 양식을 처리하고 결과를 클라이언트 웹 브라우저에 다시 기록합니다. (자세한 내용은 [양식 데이터 계산](/help/forms/developing/rendering-forms.md#calculating-form-data))
...
다음 코드 예제에서는 캐싱, 독립형 및 선형 옵션을 설정하여 성능을 최적화합니다. 선형화된 파일은 웹에서 배달되도록 최적화되었습니다. (자세한 내용은 [Forms 서비스의 성능 최적화](/help/forms/developing/rendering-forms.md#optimizing-the-performance-of-the-forms-service))
...
다음 Java 빠른 시작은 이름이 인 양식 디자인을 기반으로 하는 대화형 PDF 양식을 렌더링합니다. *Loan.xdp* 값 기준. 양식 디자인은 `com.adobe.idp.Document` 개체 이름* inputXDP*. (자세한 내용은 [값별 Forms 렌더링](/help/forms/developing/rendering-forms.md#rendering-forms-by-value))
...
다음 Java 빠른 시작은 Content Services에서 Loan.xdp 파일을 검색합니다(더 이상 사용되지 않음). 이 XDP 파일은 공간에 있습니다 `/Company Home/Form Designs`. XDP 파일은 `com.adobe.idp.Document` 인스턴스. 다음 `com.adobe.idp.Document` 인스턴스가 Forms 서비스에 전달됩니다. 대화형 양식은 클라이언트 웹 브라우저에 작성됩니다. (자세한 내용은 [Forms 서비스에 문서 전달](/help/forms/developing/passing-documents-forms-service.md))
