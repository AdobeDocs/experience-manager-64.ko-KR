...
[빠른 시작(SOAP 모드):Java API 파섹](forms-service-api-quick-starts.md#quick-start-soap-mode-rendering-an-interactive-pdf-form-using-the-java-api)

[빠른 시작(SOAP 모드):클라이언트에서 Java API를 사용하여 양식 렌더링](forms-service-api-quick-starts.md#quick-start-soap-mode-rendering-a-form-at-the-client-using-the-java-api)
...
[빠른 시작(SOAP 모드):Java API 파섹](forms-service-api-quick-starts.md#quick-start-soap-mode-handling-pdf-forms-submitted-as-xml-using-the-java-api)
...
[빠른 시작(SOAP 모드):Java API 파섹](forms-service-api-quick-starts.md#quick-start-soap-mode-handling-html-forms-submitted-as-xml-using-the-java-api)

[빠른 시작(SOAP 모드):Java API 파섹](forms-service-api-quick-starts.md#quick-start-soap-mode-creating-pdf-documents-with-submitted-xml-data-using-the-java-api)

[빠른 시작(SOAP 모드):Java API 파섹](forms-service-api-quick-starts.md#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api)
...
다음 코드 예제에서는 Loan.xdp라는 인터랙티브한 PDF *양식을* 클라이언트 웹 브라우저로 렌더링합니다. 파일이 양식에 첨부됩니다. 양식 디자인은 응용 프로그램의 일부이며 컨텐츠 루트 URI 값을 사용하여 참조됩니다 `repository:///`. (대화형 [PDF 양식 렌더링을 참조하십시오](/help/forms/developing/rendering-forms.md#rendering-interactive-pdf-forms).)
...
## 빠른 시작(SOAP 모드):클라이언트에서 Java API를 사용하여 양식 렌더링 {#quick-start-soap-mode-rendering-a-form-at-the-client-using-the-java-api}

다음 코드 예제에서는 Forms 서비스 Java API를 사용하여 클라이언트에서 *Loan.xdp *라는 양식을 렌더링합니다. 양식 디자인은 응용 프로그램의 일부이며 컨텐츠 루트 URI 값을 사용하여 참조됩니다 `repository:///`. (클라이언트에서 [양식 렌더링을 참조하십시오](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client).)
...
## 빠른 시작(SOAP 모드):Java API를 사용하여 안내서 렌더링(더 이상 사용되지 않음) {#quick-start-soap-mode-rendering-a-guide-deprecated-using-the-java-api}

다음 코드 예제에서는 TLALifeClaim.xdp라는 *안내서를* 클라이언트 웹 브라우저로 렌더링합니다.
...
다음 코드 예제에서는 조각을 기반으로 하는 양식을 렌더링합니다. 양식 디자인의 이름은 PurchaseOrderDynamic. *xdp* 이며 AEM Forms 리포지토리에 있습니다(XDP 파일은 보관소에 있는 FormsFolder라는 폴더에 저장됨). 또한 POFragment 양식 참조가 참조하는 조각도 저장소에 있어야 합니다. (조각을 [기반으로 양식 렌더링을 참조하십시오](/help/forms/developing/rendering-forms.md#rendering-forms-based-on-fragments).)
...
다음 코드 예제에서는 권한이 활성화된 양식을 클라이언트 웹 브라우저에 렌더링합니다. 이 코드 예제의 사용 권한 집합을 사용하면 사용자가 양식에 주석을 추가하고 양식 데이터를 저장할 수 있습니다. (Rendering [Rights-Enabled Forms를 참조하십시오](/help/forms/developing/rendering-forms.md#rendering-rights-enabled-forms).)
...
다음 코드 예제에서는 Forms 서비스 Java API를 사용하여 HTML 양식을 렌더링합니다. 도구 모음은 HTML 양식과 두 개의 첨부 파일에 추가됩니다. 또한 사용자 에이전트 값은 `HttpServletRequest` 객체로부터 얻습니다. (HTML [로 양식 렌더링을 참조하십시오](/help/forms/developing/rendering-forms.md#rendering-forms-as-html).)
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-rendering-an-html-form-that-uses-a-css-file-using-the-java-api}

다음 코드 예제에서는 Forms 서비스 클라이언트 API를 사용하여 HTML 양식을 렌더링합니다. 참조된 사용자 지정 CSS 파일의 이름은 *custom.css*&#x200B;입니다. (사용자 [지정 CSS 파일을 사용하여 HTML 양식 렌더링을 참조하십시오](/help/forms/developing/rendering-forms.md#rendering-html-forms-using-custom-css-files).)
...
다음 코드 예제에서는 툴바가 프랑스어로 표시된 HTML 양식을 렌더링합니다. fscmenu.xml의 위치는 C:\Adobe 입니다(이 폴더는 AEM Forms를 호스팅하는 서버에 있어야 함). 로케일 값은 `fr_FR`입니다. 사용자 정의 도구 모음을 사용하여 HTML 양식을 렌더링하는 방법에 대해 설명하는 섹션은 이 빠른 시작 부분에 사용되는 fscmenu.xml 파일의 구문을 보여줍니다. (사용자 [지정 도구 모음을 사용하여 HTML 양식 렌더링을 참조하십시오](/help/forms/developing/rendering-forms.md#rendering-html-forms-with-custom-toolbars).)
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-handling-pdf-forms-submitted-as-xml-using-the-java-api}

다음 코드 예제에서는 XML로 제출된 양식을 처리합니다. 메서드에 전달된 컨텐츠 유형 값은 `processFormSubmission` 입니다 `CONTENT_TYPE=text/xml`. 이름이 `mortgageAmount`, `lastName`및 인 필드에 해당하는 값이 `firstName` 표시됩니다. 이 빠른 시작에서 이름이 지정된 사용자 정의 메서드가 `getNodeText` 사용됩니다. 노드 이름을 지정하는 `org.w3c.dom.Document` 인스턴스와 문자열 값을 받습니다. 이 메서드는 노드의 값을 나타내는 문자열 값을 반환합니다. (제출된 [양식 처리를 참조하십시오](/help/forms/developing/rendering-forms.md#handling-submitted-forms).)
...
>[!NOTE]
>
>동일한 애플리케이션에서 `com.adobe.idp.Document` 객체와 `org.w3c.dom.Document` 객체를 사용할 경우 자격을 충분히 갖추십시오 `org.w3c.dom.Document`.
...
다음 코드 예제에서는 PDF 데이터로 제출된 양식을 처리합니다. 메서드에 전달된 컨텐츠 유형 값은 `processFormSubmission` 입니다 `CONTENT_TYPE=application/pdf`. 제출된 양식이 tempPDF.pdf라는 PDF 파일로 *저장됩니다*. 또한 양식은 PDF로 제출되므로 첨부 파일을 검색할 수 있습니다. 모든 첨부 파일은 JPEG 파일로 저장됩니다. (제출된 [양식 처리를 참조하십시오](/help/forms/developing/rendering-forms.md#handling-submitted-forms).)
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-handling-html-forms-submitted-as-xml-using-the-java-api}

다음 코드 예제에서는 XML 데이터로 제출된 HTML 양식을 처리합니다. 메서드에 전달된 컨텐츠 유형 값은 `processFormSubmission``CONTENT_TYPE=application/x-www-form-urlencoded`, 및 `mortgageAmount`라는 필드에 해당하는 값이며 `lastName``firstName` 표시됩니다. 이 빠른 시작에서 이름이 지정된 사용자 정의 메서드가 `getNodeText` 사용됩니다. 노드 이름을 지정하는 `org.w3c.dom.Document` 인스턴스와 문자열 값을 받습니다. 이 메서드는 노드의 값을 나타내는 문자열 값을 반환합니다. (제출된 [양식 처리를 참조하십시오](/help/forms/developing/rendering-forms.md#handling-submitted-forms).)
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-creating-pdf-documents-with-submitted-xml-data-using-the-java-api}

다음 Java 코드 예제에서는 XML로 전송된 양식 데이터를 처리합니다. 양식 데이터는 양식 API를 사용하여 양식 제출 시 검색되어 출력 서비스로 전송됩니다. 양식 데이터와 양식 디자인은 비대화형 PDF 문서를 만드는 데 사용됩니다. 비대화형 PDF 문서는 이름이 `/Company Home/Test Directory`지정된 Content Services(더 이상 사용되지 않음) 노드에 저장됩니다. 양식 이름이 동적으로 만들어집니다. 즉, 사용자의 이름과 성은 PDF 파일의 이름을 지정하는 데 사용됩니다. 새 컨텐츠의 리소스 식별자는 클라이언트 웹 브라우저에 작성됩니다. (제출된 [XML 데이터를 사용하여 PDF 문서 만들기를 참조하십시오](/help/forms/developing/rendering-forms.md#creating-pdf-documents-with-submitted-xml-data).)
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api}

다음 코드 예제에서는 동적 데이터 소스로 양식을 미리 채웁니다. 즉, 데이터 소스는 런타임 시 생성되며 XML 파일에 포함되어 있지 않거나 디자인 시간 동안 만들어지지 않습니다. 이 코드 예에는 세 가지 사용자 정의 메서드가 포함되어 있습니다.

* `createDataSource`:양식을 채울 때 사용되는 데이터 소스를 나타내는 `org.w3c.dom.Document` 개체를 만듭니다. 이 사용자 정의 메서드는 `org.w3c.dom.Document` 개체를 반환합니다.
* `convertDataSource`:개체를 `org.w3c.dom.Document` 개체로 `com.adobe.idp.Document` 변환합니다. 이 메서드는 `org.w3c.dom.Document` 개체를 입력 매개 변수로 받아들이고 `com.adobe.idp.Document` 개체를 반환합니다.
* `renderPOForm`:Forms 서비스 Java API를 사용하여 동적 구매 주문 양식을 렌더링합니다. 메서드에서 반환한 `com.adobe.idp.Document` 개체는 `convertDataSource` 양식을 미리 채우는 데 사용됩니다.

   이러한 모든 메서드는 Java 서블릿의 `doPost` 메서드 내에서 호출됩니다. 자세한 내용은 [플로우 가능한 레이아웃으로 양식 미리 채우기를 참조하십시오](/help/forms/developing/rendering-forms.md#prepopulating-forms-with-flowable-layouts).
...
다음 코드 예제에서는 계산 스크립트가 포함된 양식을 처리하고 결과를 클라이언트 웹 브라우저에 다시 기록합니다. (양식 [데이터 계산을 참조하십시오](/help/forms/developing/rendering-forms.md#calculating-form-data).)
...
다음 코드 예제에서는 캐싱, 독립 실행형 및 선형화된 옵션을 설정하여 성능을 최적화합니다. 선형화된 파일은 웹에 전달할 수 있도록 최적화되어 있습니다. (양식 [서비스의 성능 최적화를 참조하십시오](/help/forms/developing/rendering-forms.md#optimizing-the-performance-of-the-forms-service).)
...
다음 Java 빠른 시작은 Loan.xdp라는 양식 디자인을 기반으로 하는 대화형 PDF 양식을 *값별로 렌더링합니다* . 양식 디자인은* inputXDP*라는 `com.adobe.idp.Document` 개체를 채우는 데 사용됩니다. 값별 [양식](/help/forms/developing/rendering-forms.md#rendering-forms-by-value)렌더링을 참조하십시오.
...
다음 Java 빠른 시작은 Content Services(더 이상 사용되지 않음)에서 Loan.xdp 파일을 검색합니다. 이 XDP 파일은 공간에 `/Company Home/Form Designs`있습니다. XDP 파일은 `com.adobe.idp.Document` 인스턴스에서 반환됩니다. 인스턴스는 Forms 서비스로 `com.adobe.idp.Document` 전달됩니다. 대화형 양식은 클라이언트 웹 브라우저에 작성됩니다. (Forms [서비스에 문서 전달을 참조하십시오](/help/forms/developing/passing-documents-forms-service.md).)
