...
서버측 스크립트 실행으로 인한 양식의 변경 사항은 루트 하위 양식에 설정된 `restoreState` 속성이 포함되어 있지 않으면 클라이언트에서 렌더링되는 양식에 반영되지 않습니다 `auto`. 이 속성에 대한 자세한 내용은 양식 [디자이너를 참조하십시오.](https://www.adobe.com/go/learn_aemforms_designer_63)
