...
DCX 문서를 어셈블러 서비스로 전달하려면 먼저 XML을 `org.w3c.dom.Document` 인스턴스의 `com.adobe.idp.Document` 인스턴스로 변환하십시오. 웹 서비스를 사용하는 경우 XML(예: `XmlDocument`)을 만드는 데 사용된 데이터 유형에서 `BLOB` 인스턴스로 XML을 변환합니다.
...
이 DCX 문서는 PDF 문서를 분해합니다. PDF 문서를 분해하는 것에 익숙해지는 것이 좋습니다.
...
동적으로 생성된 DCX 문서를 사용하여 PDF 문서를 분해하려면 다음 작업을 수행합니다.
...
사용 중인 프로그래밍 언어를 사용하여 DCX 문서를 만듭니다. PDF 문서를 분해하는 DCX 문서를 만들려면 해당 문서에 `PDFsFromBookmarks` 요소가 포함되어 있는지 확인합니다. Java API를 사용하는 경우 DCX 문서를 만드는 데 사용되는 데이터 유형을 `com.adobe.idp.Document` 인스턴스로 변환합니다. 웹 서비스를 사용하는 경우 데이터 유형을 `BLOB` 인스턴스로 변환합니다.

**DCX 문서 변환**

`org.w3c.dom` 클래스를 사용하여 만든 DCX 문서는 `com.adobe.idp.Document` 객체로 변환되어야 합니다. Java API를 사용할 때 이 작업을 수행하려면 Java XML 변환 클래스를 사용합니다. 웹 서비스를 사용하는 경우 DCX 문서를 `BLOB` 개체로 변환합니다.
...
작업을 수행하는 동안 어셈블리 서비스의 동작을 제어하는 런타임 옵션을 설정할 수 있습니다. 예를 들어 오류가 발생하면 어셈블리 서비스에 작업을 계속 처리하도록 하는 옵션을 설정할 수 있습니다. 런타임 옵션을 설정하려면 `AssemblerOptionSpec` 개체를 사용합니다.

**PDF 문서 분해**

`invokeDDX` 작업을 호출하여 PDF 문서를 분해합니다. 동적으로 생성된 DCX 문서를 전달합니다. 어셈블러 서비스는 컬렉션 개체 내에서 분해된 PDF 문서를 반환합니다.
...
## Java API {#dynamically-create-a-ddx-document-using-the-java-api}를 사용하여 DCX 문서를 동적으로 만들기
...
1. DCX 문서를 만듭니다.
   ...
   * `org.w3c.dom.Document` 개체의 `createElement` 메서드를 호출하여 DCX 문서의 루트 요소를 만듭니다. 이 메서드는 루트 요소를 나타내는 `Element` 객체를 만듭니다. 요소의 이름을 나타내는 문자열 값을 `createElement` 메서드에 전달합니다. 반환 값을 `Element`으로 캐스팅합니다. 그런 다음 `setAttribute` 메서드를 호출하여 자식 요소의 값을 설정합니다. 마지막으로 머리글 요소의 `appendChild` 메서드를 호출하여 요소를 헤더 요소에 추가하고 자식 요소 객체를 인수로 전달합니다. 다음 코드 행은 이 응용 프로그램 논리를 보여 줍니다.
      ` Element root = (Element)document.createElement("DDX");  root.setAttribute("xmlns","https://ns.adobe.com/DDX/1.0/");  document.appendChild(root);`

   * `Document` 개체의 `createElement` 메서드를 호출하여 `PDFsFromBookmarks` 요소를 만듭니다. 요소의 이름을 나타내는 문자열 값을 `createElement` 메서드에 전달합니다. 반환 값을 `Element`으로 캐스팅합니다. `setAttribute` 메서드를 호출하여 `PDFsFromBookmarks` 요소의 값을 설정합니다. `PDFsFromBookmarks` 요소를 DCX 요소의 `appendChild` 메서드를 호출하여 `DDX` 요소에 추가합니다. `PDFsFromBookmarks` 요소 객체를 인수로 전달합니다. 다음 코드 행은 이 응용 프로그램 논리를 보여 줍니다.

      ` Element PDFsFromBookmarks = (Element)document.createElement("PDFsFromBookmarks");  PDFsFromBookmarks.setAttribute("prefix","stmt");  root.appendChild(PDFsFromBookmarks);`

   * `Document` 개체의 `createElement` 메서드를 호출하여 `PDF` 요소를 만듭니다. 요소의 이름을 나타내는 문자열 값을 전달합니다. 반환 값을 `Element`으로 캐스팅합니다. `setAttribute` 메서드를 호출하여 `PDF` 요소의 값을 설정합니다. `PDFsFromBookmarks` 요소의 `appendChild` 메서드를 호출하여 `PDF` 요소를 `PDFsFromBookmarks` 요소에 추가합니다. `PDF` 요소 객체를 인수로 전달합니다. 다음 코드 행은 이 응용 프로그램 논리를 보여 줍니다.
...
1. DCX 문서를 변환합니다.
   ...
   * `javax.xml.transform.Transformer` 객체의 `transform` 메서드를 호출하여 Java `ByteArrayOutputStream` 객체를 채웁니다. `javax.xml.transform.dom.DOMSource` 및 `javax.xml.transform.stream.StreamResult` 개체를 전달합니다.
...
1. 분해할 PDF 문서를 참조합니다.
   ...
   * `put` 메서드를 호출하고 다음 인수를 전달하여 `java.util.Map` 객체에 항목을 추가합니다.
      ...
      * 키 이름을 나타내는 문자열 값입니다. 이 값은 DCX 문서에 지정된 PDF 소스 요소의 값과 일치해야 합니다. 동적으로 만들어지는 DCX 문서에서 값은 `AssemblerResultPDF.pdf`입니다.
...
## 웹 서비스 API {#dynamically-create-a-ddx-document-using-the-web-service-api}를 사용하여 DCX 문서를 동적으로 생성
...
1. DCX 문서를 만듭니다.
   ...
   * `XmlElement` 개체의 `CreateElement` 메서드를 호출하여 DCX 문서의 루트 요소를 만듭니다. 이 메서드는 루트 요소를 나타내는 `Element` 객체를 만듭니다. 요소의 이름을 나타내는 문자열 값을 `CreateElement` 메서드에 전달합니다. `SetAttribute` 메서드를 호출하여 DCX 요소의 값을 설정합니다. 마지막으로 `XmlElement` 객체의 `AppendChild` 메서드를 호출하여 요소를 DCX 문서에 추가합니다. DDX 객체를 인수로 전달합니다. 다음 코드 행은 이 응용 프로그램 논리를 보여 줍니다.

      ` System.Xml.XmlElement root = ddx.CreateElement("DDX");  root.SetAttribute("xmlns", "https://ns.adobe.com/DDX/1.0/");  ddx.AppendChild(root);`

   * `XmlElement` 개체의 `CreateElement` 메서드를 호출하여 DCX 문서의 `PDFsFromBookmarks` 요소를 만듭니다. 요소의 이름을 나타내는 문자열 값을 `CreateElement` 메서드에 전달합니다. 그런 다음 해당 `SetAttribute` 메서드를 호출하여 요소의 값을 설정합니다. `DDX` 요소의 `AppendChild` 메서드를 호출하여 루트 요소에 `PDFsFromBookmarks` 요소를 추가합니다. `PDFsFromBookmarks` 요소 객체를 인수로 전달합니다. 다음 코드 행은 이 응용 프로그램 논리를 보여 줍니다.

      ` XmlElement PDFsFromBookmarks = ddx.CreateElement("PDFsFromBookmarks");  PDFsFromBookmarks.SetAttribute("prefix", "stmt");  root.AppendChild(PDFsFromBookmarks);`

   * `XmlElement` 개체의 `CreateElement` 메서드를 호출하여 DCX 문서의 `PDF` 요소를 만듭니다. 요소의 이름을 나타내는 문자열 값을 `CreateElement` 메서드에 전달합니다. 그런 다음 `SetAttribute` 메서드를 호출하여 자식 요소의 값을 설정합니다. `PDFsFromBookmarks` 요소의 `AppendChild` 메서드를 호출하여 `PDF` 요소를 `PDFsFromBookmarks` 요소에 추가합니다. `PDF` 요소 객체를 인수로 전달합니다. 다음 코드 행은 이 응용 프로그램 논리를 보여 줍니다.
...
1. DCX 문서를 변환합니다.
   ...
   * DCX 문서를 나타내는 `XmlElement` 개체를 사용하여 DCX 문서로 `MemoryStream` 개체를 채웁니다. `XmlElement` 객체의 `Save` 메서드를 호출하고 `MemoryStream` 객체를 전달합니다.
   * 바이트 배열을 만들어 `MemoryStream` 개체에 있는 데이터로 채웁니다. 다음 코드는 이 응용 프로그램 논리를 보여 줍니다.

      ` int bufLen = Convert.ToInt32(stream.Length);  byte[] byteArray = new byte[bufLen];  stream.Position = 0;  int count = stream.Read(byteArray, 0, bufLen);`

   * `BLOB` 개체를 만듭니다. 바이트 배열을 `BLOB` 객체의 `MTOM` 필드에 할당합니다.
