...
description: 구성 요소 및 서비스 Java API 빠른 시작을 사용하여 구성 요소를 배포하고, 서비스의 실행 컨텍스트를 설정하고, 서비스 보안을 비활성화하고, 서비스를 시작하고, 서비스 구성 값을 수정하고, 구성 요소를 제거할 수 있습니다.
seo-description: 구성 요소 및 서비스 Java API 빠른 시작을 사용하여 구성 요소를 배포하고, 서비스의 실행 컨텍스트를 설정하고, 서비스 보안을 비활성화하고, 서비스를 시작하고, 서비스 구성 값을 수정하고, 구성 요소를 제거할 수 있습니다.
...
>[!NOTE]
>
>AEM 양식을 사용한 프로그래밍에서 빠른 시작은 JBoss 및 Windows 운영 체제에 배포되는 Forms 서버를 기반으로 합니다. 그러나 Unix와 같은 다른 운영 체제를 사용하는 경우에는 Windows 특정 경로를 해당 운영 체제에서 지원되는 경로로 바꿉니다. 마찬가지로 다른 J2EE 응용 프로그램 서버를 사용하는 경우 유효한 연결 속성을 지정해야 합니다. [연결 속성 설정](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)을 참조하십시오.

>[!NOTE]
>
>사용자 지정 구성 요소가 있고 SOAP 또는 EJB 프로토콜을 사용하여 동일한 로컬 서버에서 DSC를 호출하고 해당 호출은 업그레이드 후 작동되지 않는 경우 in-VM([DSC_IN_VM_PASSTHROUGH_STRATEGY](https://help.adobe.com/en_US/AEMForms/6-3/ProgramLC/javadoc/com/adobe/idp/dsc/clientsdk/ServiceClientFactoryProperties.html#DSC_IN_VM_PASSTHROUGH_STRATEGY)) 호출 전략을 사용합니다. 기본 ServiceClientFactory와 함께 VM 내 DSC 호출 메서드를 사용하고 SOAP 또는 EJB 프로토콜을 사용하여 ServiceClientFactory를 만들지 않습니다.
...
## 빠른 시작(SOAP 모드):Java API {#quick-start-soap-mode-setting-the-execution-context-of-a-service-using-the-java-api}을 사용하여 서비스의 실행 컨텍스트 설정
...
## 빠른 시작(SOAP 모드):Java API {#quick-start-soap-mode-disabling-service-security-using-the-java-api}을(를) 사용하여 서비스 보안을 사용하지 않도록 설정

다음 Java 코드 예제에서는 EncryptDocument 서비스의 예와 이 서비스(Set Value and Encryption services) 내에서 호출되는 서비스의 보안을 비활성화합니다.
...
## 빠른 시작(SOAP 모드):Java API {#quick-start-soap-mode-starting-a-service-using-the-java-api}을 사용하여 서비스 시작
...
## 빠른 시작(SOAP 모드):Java API {#quick-start-soap-mode-modifying-a-services-configuration-values-using-the-java-api}을 사용하여 서비스 구성 값 수정

다음 Java 예제에서는 SendEmail 서비스에 속하는 구성 값을 수정합니다.
...
## 빠른 시작(SOAP 모드):Java API {#quick-start-soap-mode-removing-components-using-the-java-api}을(를) 사용하여 구성 요소 제거
