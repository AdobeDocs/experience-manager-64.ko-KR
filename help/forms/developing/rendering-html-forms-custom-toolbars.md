---
title: 사용자 지정 도구 모음을 사용하여 HTML Forms 렌더링
...
# 사용자 지정 도구 모음을 사용하여 HTML Forms 렌더링 {#rendering-html-forms-with-customtoolbars}

>[!CAUTION]
>
>AEM 6.4가 확장 지원이 종료되었으며 이 설명서는 더 이상 업데이트되지 않습니다. 자세한 내용은 [기술 지원 기간](https://helpx.adobe.com/kr/support/programs/eol-matrix.html). 지원되는 버전 찾기 [여기](https://experienceleague.adobe.com/docs/).

## 사용자 지정 도구 모음을 사용하여 HTML Forms 렌더링 {#rendering-html-forms-with-custom-toolbars}
...
fscJS는 각 노드와 연결된 Java 스크립트입니다. 그것을 위해서 하나를 제공해야 한다 `div#fscmenu` 노드 및 선택적으로 `ul#fscmenuItem` 노드 아래에 나열됩니다. JS 파일은 코어 도구 모음 기능을 구현하고 기본 파일이 작동합니다.

fscCSS는 특정 노드와 연결된 스타일 시트입니다. CSS 파일의 스타일은 도구 모음 모양을 지정합니다. *fscVCSS* 는 렌더링된 HTML 양식의 왼쪽에 표시되는 세로 도구 모음의 스타일 시트입니다. *FSCiecss* 는 Internet Explorer에서 렌더링되는 HTML 양식에 사용되는 스타일 시트입니다.

위의 모든 파일이 fscmenu.xml 파일에서 참조되는지 확인합니다. 즉, fscmenu.xml 파일에서 이러한 파일을 찾을 수 있도록 해당 파일을 가리키는 URI 위치를 지정합니다. 기본적으로 이러한 파일은 내부 키워드로 시작하는 URI 위치에서 사용할 수 있습니다 `FSWebRoot` 또는 `ApplicationWebRoot`.

도구 모음을 사용자 지정하려면 외부 키워드를 사용하여 키워드를 바꿉니다 `FSToolBarURI`. 이 키워드는 런타임에 Forms 서비스에 전달되는 URI를 나타냅니다(이 접근 방식은 이 섹션 뒤에 표시).

이러한 JS 및 CSS 파일의 절대 위치(예: https://www.mycompany.com/scripts/misc/fscmenu.js)을 지정할 수도 있습니다. 이 경우 `FSToolBarURI` 키워드.

>[!NOTE]
>
>이러한 파일을 참조하는 방법을 혼합하지 않는 것이 좋습니다. 즉, 모든 URI는 `FSToolBarURI` 키워드 또는 절대 위치입니다.

adobe-forms를 열어 JS 및 CSS 파일을 가져올 수 있습니다.&lt;appserver>.ear 파일. 이 파일 내에서 adobe-forms-res.war을 엽니다. 이 파일들은 모두 WAR 파일에 있습니다. adobe-forms -&lt;appserver>.ear 파일은 AEM forms 설치 폴더(C:\ is the installation directory)에 있습니다. adobe-forms를 열 수 있습니다.&lt;appserver>WinRAR와 같은 파일 추출 도구를 사용하는 .ear
...
* 값 변경 `fscJS`, `fscCSS`, `fscVCSS`, `fscIECSS` 속성(예: `fscJS="FSToolBarURI/scripts/fscmenu.js"`).
...
* 다음과 같은 JS 및 CSS 파일을 `div#fscmenu` 노드에는 도구 모음 기능에 반드시 필요합니다. 개인 `ul#fscmenuItem` 노드에는 JS 또는 CSS 파일을 지원하거나 지원하지 않을 수 있습니다.
...
또한, `HTMLRenderSpec` 개체 `setLocale` 메서드를 사용하여 로케일 값을 지정하는 문자열 값을 전달합니다. 예를 들어 를 전달합니다 `fr_FR` 를 입력합니다. Forms 서비스는 현지화된 도구 모음과 번들로 제공됩니다.

>[!NOTE]
>
>사용자 지정 도구 모음을 사용하는 HTML 양식을 렌더링하기 전에 HTML 양식이 어떻게 렌더링되는지 알고 있어야 합니다. (자세한 내용은 [Forms을 HTML으로 렌더링](/help/forms/developing/rendering-forms-html.md))
...
사용자 지정 도구 모음이 포함된 HTML 양식을 렌더링하려면 도구 모음을 설명하는 fscmenu XML 파일을 참조합니다. 이 섹션에서는 fscmenu XML 파일의 두 가지 예를 제공합니다. 또한 fscmenu.xml 파일이 참조된 모든 파일의 위치를 올바르게 지정하는지 확인합니다. 이 섹션에서 앞에서 언급한 대로, `FSToolBarURI` 키워드 또는 절대 위치.

**HTML 양식 렌더링**

HTML 양식을 렌더링하려면 디자이너에서 만들고 XDP 파일로 저장한 양식 디자인을 지정합니다. HTML 변형 유형도 선택합니다. 예를 들어 Internet Explorer 5.0 이상의 동적 HTML을 렌더링하는 HTML 변형 유형을 지정할 수 있습니다.
...
1. 사용자 정의 fscmenu XML 파일 참조
   ...
   * Fscmenu XML 파일의 위치를 `HTMLRenderSpec` 개체 `setToolbarURI` XML 파일의 URI 위치를 지정하는 문자열 값 전달 및 메서드
   * 해당하는 경우 를 호출하여 로케일 값을 설정합니다 `HTMLRenderSpec` 개체 `setLocale` 메서드를 사용하여 로케일 값을 지정하는 문자열 값을 전달합니다. 기본값은 영문입니다.

   >[!NOTE]
   >
   >이 섹션과 연결된 빠른 시작에서는 이 값을 로 설정합니다. `fr_FR`*.*

1. HTML 양식 렌더링

   를 호출합니다 `FormsServiceClient` 개체 `renderHTMLForm` 메서드를 사용하여 다음 값을 전달합니다.
   ...
   * A `java.util.HashMap` 첨부 파일을 저장하는 객체입니다. 선택적 매개 변수이며 `null` 양식에 파일을 첨부하지 않으려면
   ...
   다음 `renderHTMLForm` 메서드 반환 `FormsResult` 클라이언트 웹 브라우저에 작성해야 하는 양식 데이터 스트림을 포함하는 객체입니다.
...
1. HTML 양식 렌더링
   ...
   를 호출합니다 `FormsService` 개체 `renderHTMLForm` 메서드를 사용하여 다음 값을 전달합니다.
   ...
   * 비어 있음 `com.adobe.idp.services.holders.BLOBHolder` 로 채워지는 객체 `renderHTMLForm` 메서드를 사용합니다. 이 매개 변수 값은 렌더링된 양식을 저장합니다.
   * 비어 있음 `com.adobe.idp.services.holders.BLOBHolder` 로 채워지는 객체 `renderHTMLForm` 메서드를 사용합니다. 이 매개 변수는 출력 XML 데이터를 저장합니다.
   * 비어 있음 `javax.xml.rpc.holders.LongHolder` 로 채워지는 객체 `renderHTMLForm` 메서드를 사용합니다. 이 인수는 양식에 페이지 수를 저장합니다.
   * 비어 있음 `javax.xml.rpc.holders.StringHolder` 로 채워지는 객체 `renderHTMLForm` 메서드를 사용합니다. 이 인수는 로케일 값을 저장합니다.
   * 비어 있음 `javax.xml.rpc.holders.StringHolder` 로 채워지는 객체 `renderHTMLForm` 메서드를 사용합니다. 이 인수는 사용되는 HTML 렌더링 값을 저장합니다.
   * 비어 있음 `com.adobe.idp.services.holders.FormsResultHolder` 이 작업의 결과가 포함될 개체입니다.

   다음 `renderHTMLForm` 메서드 채우기 `com.adobe.idp.services.holders.FormsResultHolder` 클라이언트 웹 브라우저에 작성해야 하는 양식 데이터 스트림과 함께 마지막 인수 값으로 전달되는 객체입니다.
