...
Forms 서비스는 사용자가 양식에 입력하는 값을 계산하고 결과를 표시할 수 있습니다. 양식 데이터를 계산하려면 두 가지 작업을 수행해야 합니다. 먼저 양식 데이터를 계산하는 양식 디자인 스크립트를 만듭니다. 양식 디자인은 세 가지 유형의 스크립트를 지원합니다. 한 스크립트 유형은 클라이언트에서 실행되고, 다른 스크립트 유형은 서버에서 실행되고, 세 번째 유형은 서버와 클라이언트 모두에서 실행됩니다. 이 항목에서 설명한 스크립트 유형은 서버에서 실행됩니다. 서버측 계산은 HTML, PDF 및 양식 안내서(더 이상 사용되지 않음) 변형에 대해 지원됩니다.

양식 디자인 프로세스의 일부로 계산 및 스크립트를 사용하여 보다 풍부한 사용자 경험을 제공할 수 있습니다. 계산 및 스크립트는 대부분의 양식 필드 및 객체에 추가할 수 있습니다. 사용자가 대화형 양식에 입력한 데이터에 대한 계산 작업을 수행하려면 양식 디자인 스크립트를 만들어야 합니다.

사용자가 양식에 값을 입력하고 [계산] 단추를 클릭하여 결과를 봅니다. 다음 프로세스에서는 사용자가 데이터를 계산할 수 있는 예제 응용 프로그램에 대해 설명합니다.

* 사용자는 웹 응용 프로그램의 시작 페이지 역할을 하는 StartLoan.html이라는 HTML 페이지에 액세스합니다. 이 페이지는 `GetLoanForm`이라는 Java 서블릿을 호출합니다.
* `GetLoanForm` 서블릿은 대출 양식을 렌더링합니다. 이 양식에는 스크립트, 대화형 필드, 계산 단추 및 제출 단추가 포함되어 있습니다.
* 사용자가 양식 필드에 값을 입력하고 [계산] 단추를 클릭합니다. 스크립트가 실행되는 `CalculateData` Java 서블릿으로 양식이 전송됩니다. 양식에 계산 결과가 표시된 상태로 양식이 다시 사용자에게 전송됩니다.
* 만족스러운 결과가 표시될 때까지 사용자는 계속 값을 입력하고 계산합니다. 만족스러우면 [제출] 단추를 클릭하여 양식을 처리합니다. 제출된 데이터를 검색하는 데 책임이 있는 `ProcessForm`이라는 다른 Java 서블릿으로 양식이 전송됩니다. ([제출된 Forms 처리](/help/forms/developing/rendering-forms.md#handling-submitted-forms)를 참조하십시오.)
...
<table> 
 ...
 <tbody>
  ...
  <tr> 
   <td><p>5</p></td> 
   <td><p>값이 만족스러우면 [제출] 단추를 클릭합니다. 이 양식은 <code>ProcessForm</code>이라는 다른 Java 서블릿으로 전송됩니다.</p></td> 
  </tr> 
 </tbody> 
</table>

일반적으로 PDF 컨텐츠로 제출된 양식에는 클라이언트에서 실행되는 스크립트가 포함됩니다. 그러나 서버측 계산을 실행할 수도 있습니다. 제출 단추를 사용하여 스크립트를 계산할 수 없습니다. 이러한 경우 Forms 서비스는 상호 작용이 완료된 것으로 간주하므로 계산이 실행되지 않습니다.

양식 디자인 스크립트의 사용을 설명하기 위해 이 섹션에서는 서버에서 실행하도록 구성된 스크립트가 포함된 간단한 대화형 양식을 검사합니다. 다음 다이어그램은 사용자가 처음 두 필드에 입력하는 값을 추가하고 3차 필드에 결과를 표시하는 스크립트를 포함하는 양식 디자인을 보여 줍니다.

![cf_caldata](assets/cf_cf_caldata.png)

**A.** NumericField1  **B.** A 필드명 NumericField2  **C.** A 필드명 NumericField3
...
이 양식 디자인에서 [계산] 단추는 명령 단추이며 스크립트는 이 단추의 `Click` 이벤트에 있습니다. 사용자가 처음 두 필드(NumericField1 및 NumericField2)에 값을 입력하고 [계산] 단추를 클릭하면 Forms 서비스로 양식이 전송되며, 여기에서 스크립트가 실행됩니다. Forms 서비스는 NumericField3 필드에 표시된 계산 결과를 사용하여 양식을 다시 클라이언트 장치로 렌더링합니다.

>[!NOTE]
>
>양식 디자인 스크립트 만들기에 대한 자세한 내용은 [Forms 디자이너](https://www.adobe.com/go/learn_aemforms_designer_63)를 참조하십시오.
...
Forms 서비스 클라이언트 API를 사용하여 서버에서 실행하도록 구성된 스크립트가 포함된 양식을 처리하는 응용 프로그램 논리를 만듭니다. 이 프로세스는 제출된 양식을 처리하는 것과 유사합니다. ([제출된 Forms 처리](/help/forms/developing/handling-submitted-forms.md)를 참조하십시오.)

제출된 양식과 연관된 처리 상태가 `1` `(Calculate)`인지 확인합니다. 즉, Forms 서비스가 양식 데이터에 대해 계산 작업을 수행하고 있으며 결과를 사용자에게 다시 기록해야 합니다. 이 경우 서버에서 실행하도록 구성된 스크립트가 자동으로 실행됩니다.
...
## Java API {#calculate-form-data-using-the-java-api}를 사용하여 양식 데이터를 계산합니다.
...
1. 계산 스크립트가 포함된 양식 검색
   ...
   * 계산 스크립트가 포함된 양식 데이터를 검색하려면 생성자를 사용하고 생성자 내에서 `javax.servlet.http.HttpServletResponse` 객체의 `getInputStream` 메서드를 호출하여 `com.adobe.idp.Document` 객체를 만듭니다.
   * `FormsServiceClient` 객체의 `processFormSubmission` 메서드를 호출하고 다음 값을 전달합니다.

      * 양식 데이터를 포함하는 `com.adobe.idp.Document` 객체입니다.
      * 모든 관련 HTTP 헤더를 포함하는 환경 변수를 지정하는 문자열 값입니다. `CONTENT_TYPE` 환경 변수에 하나 이상의 값을 지정하여 처리할 컨텐트 유형을 지정해야 합니다. 예를 들어 XML 및 PDF 데이터를 처리하려면 이 매개 변수에 다음 문자열 값을 지정합니다.`CONTENT_TYPE=application/xml&CONTENT_TYPE=application/pdf`
   ...
   * 제출된 양식과 연결된 처리 상태가 `FormsResult` 개체의 `getAction` 메서드를 호출하여 `1`인지 확인합니다. 이 메서드가 `1` 값을 반환하면 계산이 수행되었으며 데이터를 클라이언트 웹 브라우저에 다시 쓸 수 있습니다.
...
1. 계산 스크립트가 포함된 양식 검색
   ...
   * `FormsServiceClient` 객체의 `processFormSubmission` 메서드를 호출하고 다음 값을 전달합니다.
      ...
      * 모든 관련 HTTP 헤더를 포함하는 환경 변수를 지정하는 문자열 값입니다. 예를 들어 다음 문자열 값을 지정할 수 있습니다.`HTTP_REFERER=referrer&HTTP_CONNECTION=keep-alive&CONTENT_TYPE=application/xml`
      * `HTTP_USER_AGENT` 헤더 값을 지정하는 문자열 값;예: `Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1; .NET CLR 1.1.4322)`
      * 런타임 옵션을 저장하는 `RenderOptionsSpec` 객체입니다. 추가 정보, .
      ...
      * 제출된 폼으로 메서드에 의해 채워진 빈 `FormsResultHolder` 객체입니다.
      ...
      `processFormSubmission` 메서드는 `FormsResultHolder` 매개 변수를 양식 제출 결과로 채웁니다. `processFormSubmission` 메서드는 양식 제출 결과를 포함하는 `FormsResult` 객체를 반환합니다.
