...
seo-title: 작업 관리자 서비스 Java API 빠른 시작(SOAP)
description: 'null'
...
다음 빠른 시작을 작업 관리자 서비스에 사용할 수 있습니다.
...
[빠른 시작(SOAP 모드): Java API를 사용하여 작업에서 파일 첨부 파일 검색](task-manager-service-java-api.md#quick-start-soap-mode-retrieving-file-attachments-from-tasks-using-the-java-api)
...
>[!NOTE]
>
>웹 서비스 API를 사용하여 사용자에게 할당된 작업을 검색할 수 없습니다. 이유는 이 작업을 수행하기 위해 필요한 메서드 호출인 메서드를 호출할 수 없기 때문입니다. `taskList`
...
다음 Java 코드 예는 Tony Blue라는 사용자에게 작업을 할당합니다.
...
다음 Java 코드 예는 *tony blue라는 사용자에게 할당된 모든 작업을 검색합니다*. 이 사용자는 연결 속성에 지정되어 있습니다. 식별자 값 및 설명과 같이 반환되는 작업에 대한 정보가 표시됩니다.
...
다음 Java 코드 예제에서는 식별자 값이 304인 작업의 양식 데이터를 검색합니다. 양식 데이터는 C:\Adobe에 있는 FormData.xml *이라는* XML 파일에 기록됩니다.
...
다음 Java 코드 예제에서는 FormData.xml *파일에 있는 데이터가 포함된 양식을* 업데이트합니다.
...
## 빠른 시작(SOAP 모드): Java API를 사용하여 작업에서 파일 첨부 파일 검색 {#quick-start-soap-mode-retrieving-file-attachments-from-tasks-using-the-java-api}

다음 Java 코드 예제에서는 파일 첨부 파일을 검색합니다. 각 파일 첨부 파일은 TXT 파일로 저장됩니다.
...
다음 Java 코드 예제에서는 MortgageLoan - Prebuted라는 프로세스를 기반으로 하는 모든 *작업을 검색합니다*. 반환된 각 작업의 상태가 완료된 작업인지 확인합니다. 작업을 완료한 사용자의 이름 및 작업이 완료된 날짜와 같은 정보가 검색되고 표시됩니다.
