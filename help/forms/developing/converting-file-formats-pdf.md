...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>응용 프로그램별 대화 상자 지침에 대한 무시 및 추가 항목을 지정합니다. 이 섹션에는 이러한 정보의 예가 나와 있습니다. </p><p>이 정보가 포함된 파일이 적용됩니다.<i>[appname]</i>.add.<i>[locale]</i>.xml. 예를 들면 appmon.addition.en_US.xml이 있습니다.</p></td> 
...
* appmon을 참조하십시오.*[appname]*.additional.*[locale]*.xml
* appmon을 참조하십시오.*[appname].[locale]*.xml(이 파일은 수정하지 마십시오.)
...
* adobe-livecycle-native-jboss-x86_win32.ear > adobe-Native2PDFSvc.war\WEB-INF\lib > adobe-native.jar > Native2PDFSvc-native.jar\bin > adobe-appmondata.jar\com\adobe\appmon adobe-livecycle-native-jboss-x86_win32.ear 파일은 *[AEM 양식 설치 디렉토리]\*configurationManager의 내보내기 폴더에 있습니다. AEM Forms이 다른 J2EE 응용 프로그램 서버에 배포되는 경우 adobe-livecycle-native-jboss-x86_win32.ear 파일을 J2EE 응용 프로그램 서버에 해당하는 EAR 파일로 바꿉니다.
* adobe-generatepdf-dsc.jar > adobe-appmondata.jar\com\adobe\appmon (adobe-appmondata.jar 파일은 adobe-generatepdf-dsc.jar 파일에 있습니다.) adobe-generatepdf-dsc.jar 파일은 *[AEM 양식 설치 디렉토리]*\deploy 폴더에 있습니다.
...
스크립트에는 기본 응용 프로그램의 윈도우 요소를 탐색하고 해당 요소에 대한 특정 응답을 제공하는 지침이 들어 있습니다. 이 정보가 포함된 파일이 적용됩니다.*[appname]*.script.*[locale]*.xml 예제는 appmon.notepad.script.en_US.xml입니다.
...
추가 dialogXML 파일을 식별하는 파일 이름이 적용됩니다.*[appname]*.addition.*[locale]*.xml 예를 들면 appmon.excel.addition.en_US.xml이 있습니다.

추가 대화 상자 XML 파일의 이름은 appmon 형식을 사용해야 합니다.*[applicationname]*.addition.*[locale]*.xml에서  ** applicationname은 XML 구성 파일 및 스크립트에서 사용되는 응용 프로그램 이름과 정확히 일치해야 합니다.
...
기본 응용 프로그램 실행 파일의 위치를 지정하는 환경 변수를 만듭니다. 이 변수는 *[applicationname]*_PATH 형식을 사용해야 합니다. 여기서 *applicationname*&#x200B;은(는) XML 구성 파일과 스크립트에서 사용되는 응용 프로그램 이름과 정확히 일치해야 하며, 경로에는 실행 파일의 경로가 큰따옴표로 포함되어 있습니다. 이러한 환경 변수의 예는 `Photoshop_PATH`입니다.
...
1. [새 시스템 변수] 대화 상자의 **변수 이름** 상자에 *[응용 프로그램 이름]*_PATH 형식을 사용하는 이름을 입력합니다.
