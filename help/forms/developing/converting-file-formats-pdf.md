...
PDF 생성 서비스는 기본 파일 형식을 PDF로 변환합니다. 또한 PDF를 다른 파일 형식으로 변환하고 PDF 문서의 크기를 최적화합니다.

PDF 생성 서비스는 기본 응용 프로그램을 사용하여 다음 파일 형식을 PDF로 변환합니다. 별도로 지정하지 않는 한 이러한 응용 프로그램의 독일어, 프랑스어, 영어 및 일본어 버전만 지원됩니다. *Windows* 는 Windows Server® 2003 및 Windows Server 2008만 지원합니다.
...
* Adobe Photoshop® CS2에서 PSD 변환(Windows 전용)
...
이 섹션에서는 Generate PDF API를 사용하여 프로그래밍 방식으로 Microsoft Word 문서를 PDF 문서로 변환하는 방법에 대해 설명합니다.
...
PDF 서비스 클라이언트를 만든 후 `createPDF2` 메서드를 호출할 수 있습니다. 이 메서드는 파일 확장자를 포함하여 변환할 문서에 대한 정보를 필요로 합니다.
...
1. 파일을 PDF 문서로 변환합니다.
   ...
   * 파일 확장명을 포함하는 `java.lang.String` 개체입니다.
   * 변환에 사용할 파일 형식 설정을 포함하는 `java.lang.String` 개체입니다. 파일 형식 설정은 .doc 또는 .xls와 같은 다양한 파일 유형에 대한 변환 설정을 제공합니다.
   * 사용할 PDF 설정의 이름을 포함하는 `java.lang.String` 개체입니다. 예를 들어 `Standard` 을 지정할 수 있습니다.
   * 사용할 보안 설정의 이름을 포함하는 `java.lang.String` 개체입니다.
   * PDF 문서를 생성하는 동안 적용할 설정이 포함된 선택적 `com.adobe.idp.Document` 개체입니다.
   * PDF 문서에 적용할 메타데이터 정보가 포함된 선택적 `com.adobe.idp.Document` 개체입니다.

   `createPDF2` 메서드는 새 PDF 문서와 로그 정보가 포함된 `CreatePDFResult` 개체를 반환합니다. 로그 파일은 일반적으로 전환 요청에서 생성된 오류 또는 경고 메시지를 포함합니다.
...
1. 결과를 검색합니다.
   ...
   * `com.adobe.idp.Document` 개체의 `copyToFile` 메서드를 호출하여 이전 단계에서 만든 개체에서 PDF 문서를 추출합니다.

   `createPDF2` 메서드를 사용하여 로그 문서를 가져온 경우(HTML 변환에는 적용할 수 없음) 다음 작업을 수행하십시오.
...
1. PDF 생성 클라이언트를 만듭니다.
   ...
   * `System.ServiceModel.EndpointAddress` 생성자를 사용하여 `GeneratePDFServiceClient.Endpoint.Address` 개체를 만듭니다. WSDL을 지정하는 문자열 값을 AEM Forms 서비스에 전달합니다(예: `http://localhost:8080/soap/services/GeneratePDFService?blob=mtom`). `lc_version` 특성을 사용할 필요는 없습니다. 그러나 `?blob=mtom`을 지정합니다.
   * `GeneratePDFServiceClient.Endpoint.Binding` 필드의 값을 가져와서 `System.ServiceModel.BasicHttpBinding` 개체를 만듭니다. 반환 값을 `BasicHttpBinding`(으)로 캐스팅합니다.
   * `System.ServiceModel.BasicHttpBinding` 개체의 `MessageEncoding` 필드를 `WSMessageEncoding.Mtom`로 설정합니다. 이 값은 MTOM이 사용되도록 합니다.
   * 다음 작업을 수행하여 기본 HTTP 인증을 활성화합니다.

      * AEM Forms 사용자 이름을 `GeneratePDFServiceClient.ClientCredentials.UserName.UserName` 필드에 지정합니다.
      * 해당 암호 값을 `GeneratePDFServiceClient.ClientCredentials.UserName.Password` 필드에 할당합니다.
...
1. 파일을 검색하여 PDF 문서로 변환합니다.
   ...
   * 생성자를 사용하여 `BLOB` 개체를 만듭니다. `BLOB` 개체는 PDF 문서로 변환할 파일을 저장하는 데 사용됩니다.
...
1. 파일을 PDF 문서로 변환합니다.
   ...
   * 파일 확장자를 포함하는 문자열입니다.
   * 변환에 사용할 파일 형식 설정을 포함하는 `java.lang.String` 개체입니다. 파일 형식 설정은 .doc 또는 .xls와 같은 다양한 파일 유형에 대한 변환 설정을 제공합니다.
   * 사용할 PDF 설정을 포함하는 문자열 개체. `Standard`을 지정할 수 있습니다.
   * 사용할 보안 설정을 포함하는 문자열 개체. `No Security`을 지정할 수 있습니다.
   ...
   * `CreatePDF2` 메서드로 채워지는 `BLOB` 유형의 출력 매개 변수입니다. `CreatePDF2` 메서드는 이 개체를 로그 문서로 채웁니다. (이 매개 변수 값은 웹 서비스 호출에만 필요합니다.)
...
이 섹션에서는 PDF 생성 API를 사용하여 HTML 문서를 프로그래밍 방식으로 PDF 문서로 변환하는 방법에 대해 설명합니다.
...
PDF 생성 작업을 프로그래밍 방식으로 수행하려면 먼저 PDF 생성 서비스 클라이언트를 만들어야 합니다. Java API를 사용하는 경우 `GeneratePdfServiceClient` 개체를 만듭니다. 웹 서비스 API를 사용하는 경우 `GeneratePDFServiceService`을 만듭니다.

**HTML 컨텐츠를 검색하여 PDF 문서로 변환**

PDF 문서로 변환할 HTML 컨텐츠를 참조합니다. URL을 사용하여 액세스할 수 있는 HTML 파일 또는 HTML 콘텐츠와 같은 HTML 컨텐츠를 참조할 수 있습니다.

**HTML 컨텐츠를 PDF 문서로 변환**

서비스 클라이언트를 만든 후 적절한 PDF 만들기 작업을 호출할 수 있습니다. 이 작업을 수행하려면 대상 문서의 경로를 포함하여 변환할 문서에 대한 정보가 필요합니다.

**결과 검색**

HTML 내용이 PDF 문서로 변환되면 결과를 검색하고 PDF 문서를 저장할 수 있습니다.
...
1. PDF 생성 클라이언트를 만듭니다.
   ...
   해당 생성자를 사용하여 `GeneratePdfServiceClient` 개체를 만들고 연결 속성이 포함된 `ServiceClientFactory` 개체를 전달합니다.
...
1. HTML 컨텐츠를 PDF 문서로 변환합니다.
   ...
   `GeneratePdfServiceClient` 개체의 `htmlToPDF2` 메서드를 호출하고 다음 값을 전달합니다.

   * 변환할 HTML 파일의 URL을 포함하는 `java.lang.String` 개체입니다.
   * 변환에 사용할 파일 형식 설정을 포함하는 `java.lang.String` 개체입니다. 파일 형식 설정에는 배광 수준이 포함될 수 있습니다.
   * 사용할 보안 설정의 이름을 포함하는 `java.lang.String` 개체입니다.
   * PDF 문서를 생성하는 동안 적용할 설정이 포함된 선택적 `com.adobe.idp.Document` 개체입니다. 이 정보를 제공하지 않으면 설정은 이전 세 매개 변수를 기반으로 자동으로 선택됩니다.
   * PDF 문서에 적용할 메타데이터 정보가 포함된 선택적 `com.adobe.idp.Document` 개체입니다.

1. 결과를 검색합니다.

   `htmlToPDF2` 메서드는 생성된 새 PDF 문서가 포함된 `HtmlToPdfResult` 개체를 반환합니다. 새로 만든 PDF 문서를 가져오려면 다음 작업을 수행하십시오.
...
1. HTML 컨텐츠를 PDF 문서로 변환합니다.
   ...
   `GeneratePDFServiceService` 개체의 `HtmlToPDF2` 메서드를 호출하여 HTML 콘텐츠를 PDF 문서로 변환하고 다음 값을 전달합니다.

   * 변환할 HTML 콘텐츠를 포함하는 문자열입니다.
   * 변환에 사용할 파일 형식 설정을 포함하는 `java.lang.String` 개체입니다.
...
1. 변환할 PDF 문서를 검색합니다.
   ...
   * 해당 생성자를 사용하여 변환할 PDF 문서를 나타내는 `java.io.FileInputStream` 개체를 만듭니다. PDF 문서의 위치를 지정하는 문자열 값을 전달합니다.
   * 생성자를 사용하여 `com.adobe.idp.Document` 개체를 만들고 `java.io.FileInputStream` 개체를 전달합니다.

1. PDF 문서를 변환합니다.

   `GeneratePdfServiceClient` 개체의 `exportPDF2` 메서드를 호출하고 다음 값을 전달합니다.
   ...
   * 변환의 대상 파일 유형을 지정하는 `ConvertPDFFormatType` 개체입니다.
   * PDF 문서를 생성하는 동안 적용할 설정이 포함된 선택적 `com.adobe.idp.Document` 개체입니다.

   `exportPDF2` 메서드는 변환된 파일을 포함하는 `ExportPDFResult` 개체를 반환합니다.
...
1. PDF 문서를 변환합니다.
   ...
   * `com.adobe.idp.Document` 개체의 `copyToFile` 메서드를 호출하여 새 문서를 추출합니다.
...
1. 변환할 PDF 문서를 검색합니다.
   ...
   * 생성자를 사용하여 `BLOB` 개체를 만듭니다. `BLOB` 개체는 변환된 PDF 문서를 저장하는 데 사용됩니다.
...
1. PDF 문서를 변환합니다.
   ...
   `GeneratePDFServiceServiceWse` 개체의 `ExportPDF2` 메서드를 호출하고 다음 값을 전달합니다.
...
1. 변환된 파일을 저장합니다.
   ...
   * `BLOB` 개체의 `MTOM` 필드를 바이트 배열에 할당하여 변환된 RTF 문서를 검색합니다. 바이트 배열은 변환된 RTF 문서를 나타냅니다. `ExportPDF2` 메서드의 출력 매개 변수로 사용되는 `BLOB` 개체를 사용해야 합니다.
...
* Generate PDF 서비스가 이 제품에서 기본 파일 형식을 PDF로 변환하는 데 이미 사용하는 기본 애플리케이션에 제공하는 응답을 수정하는 방법
* PDF 생성 서비스, AppMon(Generate PDF service Application Monitor) 구성 요소와 Microsoft Word와 같은 기본 애플리케이션 간의 상호 작용
...
이 문서에서는 *네이티브 응용 프로그램*&#x200B;이라는 용어를 사용하여 Microsoft Word와 같은 기본 파일 형식을 생성하는 데 사용되는 응용 프로그램을 나타냅니다.
...
AppMon은 표준 Win32 API를 사용하여 키 스트로크 및 마우스 클릭과 같은 UI 이벤트를 전송하기 위해 타사 애플리케이션과 상호 작용하며, 이러한 응용 프로그램을 제어하여 PDF 파일을 생성할 때 유용합니다.
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td><p>이미 지원되는 기본 응용 프로그램에 대한 설정을 변경하려면 시스템 관리자는 관리 콘솔에서 파일 유형 설정을 설정합니다. </p><p>새로운 기본 파일 형식에 대한 지원을 추가하려면 파일을 수동으로 편집해야 합니다. (<a href="converting-file-formats-pdf.md#adding-or-modifying-support-for-a-native-file-format">기본 파일 형식</a>에 대한 지원 추가 또는 수정을 참조하십시오.) </p></td> 
  </tr> 
  <tr> 
   <td><p>스크립트 </p></td> 
   ...
   <td><p>PDF 생성 서비스에는 지원되는 모든 기본 애플리케이션에 대한 스크립트 파일이 포함됩니다. XML 편집 응용 프로그램을 사용하여 이러한 파일을 수정할 수 있습니다.</p><p>새 기본 애플리케이션에 대한 지원을 추가하려면 새 스크립트 파일을 생성해야 합니다. (<a href="converting-file-formats-pdf.md#creating-or-modifying-an-additional-dialog-xml-file-for-a-native-application">기본 응용 프로그램에 대한 추가 대화 상자 XML 파일 만들기 또는 수정 참조</a>). </p></td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>응용 프로그램별 대화 상자에 응답하는 방법을 지정합니다. </p><p>이 정보가 포함된 파일이 적용됩니다.<i>[appname]</i> 대화 상자<i>[locale]</i>.xml(예: appmon.word.en_US.xml)</p></td> 
   <td><p>이 파일은 수정하지 마십시오. </p><p>새 기본 응용 프로그램에 대한 대화 상자 지침을 추가하려면 <a href="converting-file-formats-pdf.md#creating_or_modifying_an_additional_dialog_xml_file_for_a_native_application">기본 응용 프로그램에 대한 추가 대화 상자 XML 파일 만들기 또는 수정</a>을 참조하십시오.</p></td> 
  </tr> 
  <tr> 
...
이 섹션 및 다음 섹션에서 사용자의 관점에서 대화 상자 및 해당 구성 요소를 설명하는 경우 *대화 상자*, *단추*, *필드* 및 *콤보 상자*&#x200B;와 같은 용어가 사용됩니다.

이 섹션 및 다음 섹션에서 내부 표현의 관점에서 대화 상자 및 해당 컴포넌트를 설명하는 경우 *창 요소*&#x200B;라는 용어가 사용됩니다. 창 요소의 내부 표현은 각 창 요소 인스턴스가 레이블로 식별되는 계층입니다. 창 요소 인스턴스는 물리적인 특성과 동작도 설명합니다.
...
이 섹션에서는 AppMon에 지침을 제공하는 XML 요소에 대해 설명합니다. 이러한 요소에는 `dialog` 요소 및 `window` 요소와 같은 이름이 있습니다. 이 문서에서는 단간격 글꼴을 사용하여 XML 요소를 구분합니다. `*dialog*` 요소는 XML 스크립트 파일이 고의 또는 실수로 표시될 수 있는 대화 상자를 식별합니다. `*window*` 요소는 창 요소(대화 상자 또는 대화 상자의 구성 요소)를 식별합니다.
...
*스크립트 XML 파일*&#x200B;은 기본 응용 프로그램이 특정 창 요소로 이동한 다음 해당 요소에 대한 응답을 제공하는 일련의 단계를 지정합니다. 대부분의 응답은 사용자가 해당 대화 상자의 필드, 콤보 상자 또는 단추에 제공하는 입력에 해당하는 텍스트나 키 입력입니다.
...
스크립트 XML 파일의 각 단계는 단계의 지침이 성공적으로 수행된 경우 표시될 것으로 예상되는 창 요소를 식별합니다. 스크립트 단계를 실행하는 동안 예기치 않은 대화 상자가 나타나면 PDF 생성 서비스는 다음 섹션에 설명된 대로 대화 상자 XML 파일을 검색합니다.

#### 대화 상자 XML 파일 {#dialog-xml-files}

기본 응용 프로그램을 실행하면 기본 응용 프로그램이 표시 모드인지 표시 모드인지에 관계없이 나타나는 다른 대화 상자가 표시됩니다. 대화 상자는 운영 체제나 응용 프로그램 자체에 의해 생성할 수 있습니다. 기본 응용 프로그램이 PDF 생성 서비스를 통해 실행 중이면 시스템 및 기본 응용 프로그램 대화 상자가 보이지 않는 창에 표시됩니다.

*대화 상자 XML 파일*&#x200B;은 PDF 생성 서비스가 시스템 또는 기본 응용 프로그램 대화 상자에 응답하는 방식을 지정합니다. 대화 상자 XML 파일을 사용하면 변환 프로세스를 용이하게 하는 방식으로 PDF 생성 서비스가 메시지가 없는 대화 상자에 응답할 수 있습니다.

현재 실행 중인 스크립트 XML 파일에 의해 처리되지 않은 대화 상자를 시스템 또는 기본 응용 프로그램에 표시할 경우 PDF 생성 서비스는 대화 상자 XML 파일을 이 순서로 검색하여 일치 항목을 찾으면 중지됩니다.

* appmon.*[appname]*.additional.*[locale]*.xml
* appmon.*[appname].[locale]*.xml(이 파일은 수정하지 마십시오.)
* appmon.global.*[locale]*.xml(이 파일은 수정하지 마십시오.)

[PDF 생성] 서비스에서 대화 상자에 대한 일치 항목을 찾으면 대화 상자에 지정된 키 또는 기타 작업을 전송하여 제거합니다. 대화 상자에 대한 지침에 abort 메시지가 지정되면 PDF 생성 서비스는 현재 실행 중인 작업을 종료하고 오류 메시지를 생성합니다. 이러한 abort 메시지는 스크립트 XML 문법의 `abortMessage` 요소에 지정됩니다.
...
제목 표시줄에 표시되는 캡션으로 대화 상자를 쉽게 식별할 수 있습니다. 그러나 Microsoft Spy++와 같은 도구를 사용하여 하위 수준 창 요소를 식별해야 합니다. 하위 수준 창 요소는 다양한 속성을 통해 식별할 수 있으며, 이는 분명하지 않습니다. 또한 각 기본 애플리케이션은 창 요소를 다르게 식별할 수 있습니다. 따라서 창 요소를 식별하는 방법에는 여러 가지가 있습니다. 다음은 창 요소 식별을 고려해 볼 수 있는 제안 주문입니다.
...
특성이 캡션을 식별하지 못하면 대신 해당 상위에 대한 인덱스를 사용하여 창 요소를 식별할 수 있습니다. *index*&#x200B;은 해당 동위 창 요소와 상대적인 창 요소의 위치를 지정합니다. 자주 인덱스는 콤보 상자를 식별하는 유일한 방법입니다.

다음 문제를 숙지하십시오.

* Microsoft Spy++에서는 캡션의 핫키를 식별하기 위해 앰퍼샌드(&amp;)를 사용하여 캡션을 표시합니다. 예를 들어 Spy++는 `Pri&nt`(으)로 하나의 [인쇄] 대화 상자에 대한 캡션을 보여 줍니다. 이는 핫키가 *n*&#x200B;임을 나타냅니다. 스크립트 및 대화 상자 XML 파일의 캡션 제목에는 앰퍼샌드를 생략해야 합니다.
* 일부 캡션에는 줄 바꿈이 포함됩니다. pdf 생성 서비스는 줄 바꿈을 식별할 수 없습니다. 캡션에 라인 브레이크가 포함되어 있는 경우 캡션을 충분히 포함시켜 다른 메뉴 항목과 구분한 다음 생략된 부분에 정규 표현식을 사용합니다. 예는 ( `^Long caption title$`)입니다. ([캡션 특성에 정규 표현식 사용](converting-file-formats-pdf.md#using-regular-expressions-in-caption-attributes)을 참조하십시오.)
* 예약된 XML 문자에 문자 엔티티(이스케이프 시퀀스라고도 함)를 사용합니다. 예를 들어, 앰퍼샌드(`&`)는 `<`, `>` 는 기호보다 작음, `&apos;` 는 아포스트로피의 경우, `&quot;`는 따옴표로 묶습니다.
...
1. 대화 상자 및 스크립트 XML 파일을 appmondata.jar 파일에 추가하거나 이 파일에서 기존 XML 파일을 수정합니다. ([기본 응용 프로그램용 스크립트 XML 파일 만들기 또는 수정](converting-file-formats-pdf.md#creating-or-modifying-a-script-xml-file-for-a-native-application)및 [기본 응용 프로그램용 추가 대화 상자 XML 파일 만들기 또는 수정 참조](converting-file-formats-pdf.md#creating-or-modifying-an-additional-dialog-xml-file-for-a-native-application)).
1. WinZip 또는 WinRAR와 같은 도구를 사용하여 adobe-generatepdf-dsc.jar > adobe-appmondata.jar를 엽니다.
1. 대화 상자 및 스크립트 XML 파일을 appmondata.jar 파일에 추가하거나 이 파일에서 기존 XML 파일을 수정합니다. ([기본 응용 프로그램용 스크립트 XML 파일 만들기 또는 수정](converting-file-formats-pdf.md#creating-or-modifying-a-script-xml-file-for-a-native-application)및 [기본 응용 프로그램용 추가 대화 상자 XML 파일 만들기 또는 수정 참조](converting-file-formats-pdf.md#creating-or-modifying-an-additional-dialog-xml-file-for-a-native-application)). XML 파일을 adobe-appmondata.jar 파일에 추가한 후 새 adobe-appmondata.jar 파일을 adobe-generatepdf-dsc.jar 파일에 배치합니다.
1. 추가 기본 파일 형식에 대한 지원을 추가한 경우 응용 프로그램의 경로를 제공하는 시스템 환경 변수를 만드십시오([환경 변수 만들기를 참조하여 기본 응용 프로그램을 찾습니다(](converting-file-formats-pdf.md#creating-an-environment-variable-to-locate-the-native-application)).

**GeneratePDF 구성 요소를 재배포하려면**

1. Workbench에 로그인합니다.
1. **Window** > **보기 표시** > **구성 요소**&#x200B;를 선택합니다. 이 작업은 구성 요소 보기를 Workbench에 추가합니다.
1. GeneratePDF 구성 요소를 마우스 오른쪽 단추로 클릭하고 **구성 요소 중지**&#x200B;를 선택합니다.
...
* `windowList` 또는 `dialog` 요소에서 여러 `window` 요소가 자식으로 표시되는 경우 해당 `window` 요소의 순서를 내림차순으로 정렬하고 해당 위치를 나타내는 `caption` 이름의 길이가 순서대로 지정됩니다.
* `window` 요소에 여러 개의 `windowList` 요소가 나타나면 순서대로 위치를 나타내는 첫 번째 `indexes/` 요소의 `caption` 길이로 이러한 `windowList` 요소를 내림차순으로 정렬합니다.
...
>[!NOTE]
>
>이 컨텍스트에서 추가라는 용어는 애플리케이션의 컨텐츠를 의미합니다.[applicationname].addition.[locale].xml 파일. 이러한 파일은 대화 상자 XML 파일에 대한 무시 및 추가를 지정합니다.
...
추가 dialogXML 파일을 식별하는 파일 이름이 적용됩니다.*[appname]*.additional.*[locale]*.xml 예로는 appmon.excel.addition.en_US.xml이 있습니다.

추가 대화 상자 XML 파일의 이름은 응용 프로그램 형식을 사용해야 합니다.*[applicationname]*.addition.*[locale]*.xml에서 application ** name은 XML 구성 파일 및 스크립트에서 사용되는 응용 프로그램 이름과 정확히 일치해야 합니다.

>[!NOTE]
>
>native2pdfconfig.xml 구성 파일에 지정된 일반 응용 프로그램 중 주 대화 상자 XML 파일이 없습니다. [기본 파일 형식에 대한 지원을 추가 또는 수정하는 섹션](converting-file-formats-pdf.md#adding-or-modifying-support-for-a-native-file-format)에서는 이러한 사양에 대해 설명합니다.
...
1. **홈** > **서비스** > **Adobe PDF Generator** >**구성 파일**&#x200B;을 선택한 다음 **구성 가져오기**&#x200B;를 선택합니다. 구성 데이터를 이전 설정을 대체하는 PDF 생성 서비스로 가져옵니다.

>[!NOTE]
>
>응용 프로그램 이름은 `GenericApp` 요소의 `name` 속성 값으로 지정됩니다. 이 값은 해당 응용 프로그램에 대해 개발하는 스크립트에 지정된 해당 이름과 정확히 일치해야 합니다. 마찬가지로, `GenericApp` 요소의 `displayName` 속성은 해당 스크립트의 `expectedWindow` 창 캡션과 정확히 일치해야 합니다. 이러한 동등한 기능은 `displayName` 또는 `caption` 속성에 표시되는 정규 표현식을 확인한 후 평가됩니다.

이 예에서는 메모장(Microsoft Word가 아님)을 사용하여 파일 이름 확장명이 .txt인 파일을 처리하도록 PDF 생성 서비스와 함께 제공된 기본 구성 데이터가 수정되었습니다. 이 수정 전에 Microsoft Word가 해당 파일을 처리해야 하는 기본 응용 프로그램으로 지정되었습니다.
...
1. **Campaign 컨트롤 패널 > System**&#x200B;을 선택합니다.
...
AEM Forms에는 파일 이름 확장명이 .txt인 파일을 처리하는 데 메모장을 사용하는 PDF 생성 서비스가 사용되는 샘플 XML 파일이 포함되어 있습니다. 이 코드는 이 섹션에 포함되어 있습니다. 또한 이 섹션에 설명된 다른 수정 사항을 수행해야 합니다.
