...
[빠른 시작(SOAP 모드):Java API 파섹](assembler-service-java-api-quick.md#quick-start-soap-mode-assembling-a-pdf-document-using-the-java-api)
...
[빠른 시작(SOAP 모드):Java API 파섹](assembler-service-java-api-quick.md#quick-start-soap-mode-assembling-an-encrypted-pdf-document-using-the-java-api)
...
[빠른 시작(SOAP 모드):Java API 파섹](assembler-service-java-api-quick.md#quick-start-soap-mode-validating-ddx-documents-using-the-java-api)

[빠른 시작(SOAP 모드):Java API 파섹](assembler-service-java-api-quick.md#quick-start-soap-mode-assembling-pdf-documents-with-bookmarks-using-the-java-api)

[빠른 시작(SOAP 모드):동적으로 Java API를 사용하여 DCX 문서 만들기](assembler-service-java-api-quick.md#quick-start-soap-mode-dynamically-creating-a-ddx-document-using-the-java-api)
...
[빠른 시작(SOAP 모드):Java API 파섹](assembler-service-java-api-quick.md#quick-start-soap-mode-assembling-multiple-xdp-fragments-using-the-java-api)
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-assembling-a-pdf-document-using-the-java-api}

다음 Java 코드 예제에서는 이름이* map.pdf*이고 *directions.pdf* 인 두 개의 PDF 소스 문서를 하나의 PDF 문서로 병합합니다. 단일 PDF 문서의 이름은 AssemblerResultPDF. *pdf*&#x200B;입니다. DCX 문서의 이름은 *shell.xml*&#x200B;입니다. 자세한 내용은 [프로그래밍 방식으로 PDF 문서 작성을 참조하십시오](/help/forms/developing/assembling-pdf-documents.md#programmatically-assembling-pdf-documents).
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-assembling-an-encrypted-pdf-document-using-the-java-api}

다음 Java 코드 예제에서는 암호로 암호화된 PDF 문서를 작성합니다. 비보안 PDF 문서의 이름은 *Loan.pdf*. DCX 문서의 이름은 *shell_Encrypt.xml*&#x200B;입니다. 암호화된 PDF 문서의 이름은 AssemblerEncryptedPDF. *pdf*&#x200B;입니다. 자세한 내용은 [암호화된 PDF 문서 작성을 참조하십시오](/help/forms/developing/assembling-pdf-documents.md#assembling-encrypted-pdf-documents).
...
다음 Java 코드 예제에서는 고유한 페이지 식별자(bates 번호 매기기)를 사용하여 PDF 문서를 어셈블합니다. DCX 문서의 이름은 *shell_Bates.xml*&#x200B;입니다. 어셈블러 서비스에서 반환되는 PDF 문서는 AssemblerResultBatesPDF.pdf *라는 PDF 파일로 저장됩니다*. (Bates [번호 매기기를 사용하여 문서 조합 참조](/help/forms/developing/assembling-pdf-documents.md#assembling-documents-using-bates-numbering))
...
다음 Java 코드 예제에서는 비대화형 PDF 문서를 만듭니다. Assembler 서비스로 전달되는 대화형 PDF 문서의 이름은 Loan.pdf *입니다*. DCX 문서의 이름은 *shell_XFA.xml*&#x200B;입니다. 비대화형 PDF 문서는 AssembleNonInteractivePDF.pdf *라는 PDF 파일로 저장됩니다*. ( [비대화형 PDF 문서 조합 참조](/help/forms/developing/assembling-pdf-documents.md#assembling-non-interactive-pdf-documents))
...
다음 Java 코드 예제에서는 입력 PDF 문서가 PDF/A 규격인지 확인합니다. 어셈블러 서비스로 전달되는 입력 PDF 문서의 이름은 Loan.pdf *입니다*. DCX 문서의 이름은 shell_PDFA.xml입니다. 어셈블러 서비스에서 반환되는 XML 문서로서 입력 PDF 문서가 PDF/A 규격인지 여부를 result.xml이라는 XML 파일로 저장할지 여부를 지정합니다. 이 코드 예제에서 사용되는 *shell_PDFA.xml* DDX 문서에 대한 자세한 내용은 [문서가 PDF/A인지 여부를 참조하십시오](/help/forms/developing/assembling-pdf-documents.md#determining-whether-documents-are-pdf-a-compliant).
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-validating-ddx-documents-using-the-java-api}

다음 Java 코드 예제에서는 bookmarkDCX.xml이라는 파일을 기반으로 DCX *문서의 유효성을 검사합니다*. (DCX [문서 유효성 검사를 참조하십시오](/help/forms/developing/assembling-pdf-documents.md#validating-ddx-documents).)
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-assembling-pdf-documents-with-bookmarks-using-the-java-api}

다음 Java 코드 예제에서는 책갈피가 포함된 PDF 문서를 작성합니다. DCX 문서의 이름은 bookmarkDCX. *xml*&#x200B;입니다. PDF 문서에 추가할 책갈피를 설명하는 책갈피 XML 문서의 이름은 bookmarks.xml입니다. 결과 PDF 문서는 AssemblerResultBookmarks.pdf라는 PDF 파일로 저장됩니다. (책갈피를 [사용하여 PDF 문서 조합 참조](/help/forms/developing/assembling-pdf-documents.md#assembling-pdf-documents-with-bookmarks))
...
## 빠른 시작(SOAP 모드):동적으로 Java API를 사용하여 DCX 문서 만들기 {#quick-start-soap-mode-dynamically-creating-a-ddx-document-using-the-java-api}

다음 Java 코드 예제에서는 PDF 문서를 분해하는 DCX 문서를 동적으로 만듭니다. 입력 PDF 문서의 각 레벨 1 책갈피에 대해 새 PDF 문서가 만들어집니다. 이 코드 예에는 두 가지 사용자 정의 메서드가 포함되어 있습니다.

* `createDDX`:Assembler 서비스로 전송된 DCX 문서를 나타내는 개체를 `org.w3c.dom.Document` 만듭니다. 이 사용자 정의 메서드는 `org.w3c.dom.Document` 개체를 반환합니다.
* `convertDDX`:개체를 `org.w3c.dom.Document` 개체로 `com.adobe.idp.Document` 변환합니다. 이 메서드는 `org.w3c.dom.Document` 개체를 입력 매개 변수로 받아들이고 `com.adobe.idp.Document` 개체를 반환합니다.

   이 두 가지 방법 모두 이 빠른 시작 시 호출됩니다. (DCX [문서 동적으로 만들기를](/help/forms/developing/assembling-pdf-documents.md#dynamically-creating-ddx-documents)참조하십시오.)&quot;
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-assembling-multiple-xdp-fragments-using-the-java-api}

다음 Java 코드 예제에서는 다음 XDP 파일을 기반으로 하는 XDP 조각을 어셈블합니다.tac *018_template_crrolled.xdp*, tac018_contact.xdp **&#x200B;및* tuc018_patient.xdp* 모든 조각을 포함하는 결합된 XDP 문서는 AssemblerResultXDP.xdp라는 XDP 파일로 *저장됩니다*. (여러 [XDP 조각 조합 참조](/help/forms/developing/assembling-pdf-documents.md#assembling-multiple-xdp-fragments))
...
다음 코드 예제에서는 를 사용하여 PDF 문서를 교정할 수 `PDFUtility`있습니다.
