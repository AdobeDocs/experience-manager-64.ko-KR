...
다음 빠른 시작을 AEM Forms 리포지토리 서비스에 사용할 수 있습니다.
...
[빠른 시작(SOAP 모드):Java API를 사용하여 리소스 작성](repository-service-api-quick-starts.md#quick-start-soap-mode-writing-a-resource-using-the-java-api)
...
[빠른 시작(SOAP 모드):Java API 파섹](repository-service-api-quick-starts.md#quick-start-soap-mode-creating-relationships-between-resources-using-the-java-api)
...
대부분의 AEM Forms 리포지토리 서비스는 다음 그림과 `Applications/FormsApplication,` 같이 이름이 지정된 응용 프로그램과 신속하게 상호 작용합니다.

FormsFolder 폴더는 AEM Forms 저장소의 위치입니다. 예를 들어 프로그래밍 방식으로 이 폴더를 에 추가할 수 `Applications/FormsApplication`있습니다. (빠른 [시작(SOAP 모드 참조):Java API를 사용하여 폴더 만들기](repository-service-api-quick-starts.md#quick-start-soap-mode-creating-a-folder-using-the-java-api))
...
>[!NOTE]
>
>Applications/FormsApplication은 기본적으로 존재하지 않습니다. 빠른 시작을 따르려면 워크벤치를 사용하여 이 애플리케이션을 만드십시오. 워크벤치를 사용하여 응용 프로그램을 만드는 방법에 대한 자세한 내용은 [프로세스 디자인](https://help.adobe.com/en_US/livecycle/11.0/WorkbenchHelp/WS92d06802c76abadb-1cc35bda128261a20dd-7fc4.html)시작을 참조하십시오.

## 빠른 시작(SOAP 모드):Java API를 사용하여 폴더 만들기 {#quick-start-soap-mode-creating-a-folder-using-the-java-api}

다음 Java 코드 예제에서는 FormsFolder라는 *폴더를* 다음 위치에 만듭니다 `/Applications/FormsApplication/1.0/`. (폴더 [만들기를 참조하십시오](/help/forms/developing/aem-forms-repository.md#creating-folders).)
...
## 빠른 시작(SOAP 모드):Java API를 사용하여 리소스 작성 {#quick-start-soap-mode-writing-a-resource-using-the-java-api}

다음 Java 코드 예는 *loan.xdp라는* 리소스를 저장소에 기록합니다. 리소스가 `/Applications/FormsApplication/1.0/FormsFolder` 위치에 추가됩니다. (리소스 [쓰기를 참조하십시오](/help/forms/developing/aem-forms-repository.md#writing-resources).)
...
## 빠른 시작(SOAP 모드):Java API를 사용하여 리소스 목록 {#quick-start-soap-mode-listing-resources-using-the-java-api}

다음 Java 코드 예는 에 있는 리소스를 `Applications/FormsApplication/1.0/FormsFolder`나열합니다. (리소스 [나열을 참조하십시오](/help/forms/developing/aem-forms-repository.md#listing-resources).)
...
다음 Java 코드 예는 저장소에서 Loan. *xdp* 라는 리소스를 읽습니다. XDP 파일은 에 `/Applications/FormsApplication/1.0/FormsFolder/`있습니다. 자세한 내용은 [리소스 읽기를 참조하십시오](/help/forms/developing/aem-forms-repository.md#reading-resources).
...
다음 Java 코드 예는 설명을 수정하여 `/Applications/FormsApplication/1.0/FormsFolder` 업데이트합니다. (리소스 [업데이트를 참조하십시오](/help/forms/developing/aem-forms-repository.md#updating-resources).)
...
다음 Java 코드 예제에서는 Loan.xdp를 검색합니다 `Applications/FormsApplication/1.0/FormsFolder`. (리소스 [검색을 참조하십시오](/help/forms/developing/aem-forms-repository.md#searching-for-resources).)
...
## 빠른 시작(SOAP 모드):Java API 파섹 {#quick-start-soap-mode-creating-relationships-between-resources-using-the-java-api}

다음 Java 코드 예제에서는 AEM Forms 저장소의 두 리소스 간에 관계를 만듭니다. (리소스 [관계 만들기를 참조하십시오](/help/forms/developing/aem-forms-repository.md#creating-resource-relationships).)
...
다음 Java 코드 예는 /Applications/FormsApplication/1.0/FormsFolder/Loan.xdp을 잠급니다. 자세한 내용은 [리소스 잠금을 참조하십시오](/help/forms/developing/aem-forms-repository.md#locking-resources).
...
다음 Java 코드 예제에서는 보관소에서 ACL(액세스 제어 목록)을 읽고 만듭니다.
...
다음 Java 코드 예제에서는 Loan.xdp를 `Applications/FormsApplication/1.0/FormsFolder`삭제합니다. 이 XDP 파일이 이 폴더에 없으면 예외가 발생합니다. (리소스 [삭제를 참조하십시오](/help/forms/developing/aem-forms-repository.md#deleting-resources).)
