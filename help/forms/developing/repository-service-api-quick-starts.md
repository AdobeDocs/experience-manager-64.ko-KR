...
대부분의 AEM Forms 리포지토리 서비스는 다음 그림과 같이 이름이 지정된 응용 프로그램 `Applications/FormsApplication,` 과 신속하게 상호 작용합니다.

FormsFolder 폴더는 AEM Forms 저장소의 위치입니다. 예를 들어 프로그래밍 방식으로 이 폴더를 `Applications/FormsApplication` (빠른 [시작(SOAP 모드 참조): Java API를 사용하여 폴더 만들기](repository-service-api-quick-starts.md#quick-start-soap-mode-creating-a-folder-using-the-java-api).)
...
>[!NOTE]
>
>웹 브라우저를 사용하여 AEM Forms 저장소를 찾아볼 수 있습니다. 리포지토리를 찾아보려면 다음 URL을 웹 브라우저 https://[서버 이름]:[서버 포트]/리포지토리에 입력합니다. 웹 브라우저를 사용하여 빠른 시작 결과를 확인할 수 있습니다. 예를 들어 AEM Forms 저장소에 컨텐츠를 추가하는 경우 웹 브라우저에서 컨텐츠를 볼 수 있습니다.

>[!NOTE]
>
>응용 프로그램/FormsApplication이 기본적으로 존재하지 않습니다. 빠른 시작을 따르려면 워크벤치를 사용하여 이 응용 프로그램을 만듭니다. 워크벤치를 사용하여 응용 프로그램을 만드는 방법에 대한 자세한 내용은 [프로세스 디자인 시작을 참조하십시오](https://help.adobe.com/en_US/livecycle/11.0/WorkbenchHelp/WS92d06802c76abadb-1cc35bda128261a20dd-7fc4.html).

## 빠른 시작(SOAP 모드): Java API를 사용하여 폴더 만들기 {#quick-start-soap-mode-creating-a-folder-using-the-java-api}

다음 Java 코드 예제에서는 다음 위치에 *FormsFolder라는* 폴더를 만듭니다 `/Applications/FormsApplication/1.0/`. 폴더 [만들기를 참조하십시오](/help/forms/developing/aem-forms-repository.md#creating-folders).
...
다음 Java 코드 예제는 보관소에 *loan.xdp* 라는 리소스를 씁니다. 리소스가 `/Applications/FormsApplication/1.0/FormsFolder` 위치에 추가됩니다. (리소스 [쓰기를 참조하십시오](/help/forms/developing/aem-forms-repository.md#writing-resources).)
...
다음 Java 코드 예는 에 있는 리소스를 나열합니다 `Applications/FormsApplication/1.0/FormsFolder`. (리소스 [나열을 참조하십시오](/help/forms/developing/aem-forms-repository.md#listing-resources).)
...
다음 Java 코드 예는 보관소에서 *Loan.xdp* 라는 리소스를 읽습니다. XDP 파일은 에 있습니다 `/Applications/FormsApplication/1.0/FormsFolder/`. 자세한 내용은 리소스 [읽기를 참조하십시오](/help/forms/developing/aem-forms-repository.md#reading-resources).
...
다음 Java 코드 예는 설명을 수정하여 `/Applications/FormsApplication/1.0/FormsFolder` 로 업데이트합니다. (리소스 [업데이트를 참조하십시오](/help/forms/developing/aem-forms-repository.md#updating-resources).)
...
다음 Java 코드 예는 Loan.xdp를 검색합니다 `Applications/FormsApplication/1.0/FormsFolder`. (리소스 [검색을 참조하십시오](/help/forms/developing/aem-forms-repository.md#searching-for-resources).)
...
다음 Java 코드 예제에서는 Roan.xdp를 삭제합니다 `Applications/FormsApplication/1.0/FormsFolder`. 이 XDP 파일이 이 폴더에 없으면 예외가 발생합니다. (리소스 [삭제를 참조하십시오](/help/forms/developing/aem-forms-repository.md#deleting-resources).)
