...
seo-description: Java API 및 웹 서비스 API를 사용하여 프로그래밍 방식으로 DDX 문서의 유효성을 검사합니다.
...
# DDX 문서 {#validating-ddx-documents} 확인
...
어셈블러 서비스 클라이언트를 만들고 DDX 문서를 참조하며 런타임 옵션을 설정한 후에는 `invokeDDX` 작업을 호출하여 DDX 문서의 유효성을 확인할 수 있습니다. DDX 문서의 유효성을 확인할 때 맵 매개 변수로 `null`을 전달할 수 있습니다. 이 매개 변수는 일반적으로 어셈블러가 DDX 문서에 지정된 작업을 수행하는 데 필요한 PDF 문서를 저장합니다.

유효성 검사가 실패하면 예외가 발생하고 로그 파일에 `OperationException` 인스턴스에서 DDX 문서가 유효하지 않은 이유를 설명하는 세부 정보가 포함됩니다. 기본 XML 구문 분석 및 스키마 검사를 통과하면 DDX 사양에 대한 유효성 검사가 수행됩니다. DDX 문서에 있는 모든 오류는 로그에 지정됩니다.
...
## Java API {#validate-a-ddx-document-using-the-java-api}를 사용하여 DDX 문서의 유효성 검사
...
1. DDX 문서의 유효성을 검사할 런타임 옵션을 설정합니다.
   ...
   * `AssemblerOptionSpec` 개체의 setValidateOnly 메서드를 호출하고 `true`을 전달하여 어셈블러 서비스에서 DDX 문서의 유효성을 검사하도록 지시하는 런타임 옵션을 설정합니다.
   * `AssemblerOptionSpec` 개체의 `getLogLevel` 메서드를 호출하고 문자열 값을 전달하여 어셈블러 서비스가 로그 파일에 쓰는 정보의 양을 설정합니다. DDX 문서의 유효성을 확인할 때 유효성 검사 프로세스에 도움이 될 로그 파일에 자세한 정보를 기록할 수 있습니다. 따라서 값 `FINE` 또는 `FINER`을 전달할 수 있습니다.
...
1. 유효성 검사 결과를 로그 파일에 저장합니다.
   ...
   * `java.io.File` 개체를 만들고 파일 이름 확장명이 .xml인지 확인합니다.
   * `AssemblerResult` 개체의 `getJobLog` 메서드를 호출합니다. 이 메서드는 유효성 검사 정보를 포함하는 `com.adobe.idp.Document` 인스턴스를 반환합니다.
   * `com.adobe.idp.Document` 개체의 `copyToFile` 메서드를 호출하여 `com.adobe.idp.Document` 개체의 내용을 파일에 복사합니다.

   >[!NOTE]
   >
   >DDX 문서가 올바르지 않으면 `OperationException`이(가) 발생합니다. catch 문 내에서 `OperationException` 개체의 `getJobLog` 메서드를 호출할 수 있습니다.

**참고 항목**

[DDX 문서 확인](#validating-ddx-documents)

[빠른 시작(SOAP 모드):Java API를 사용하여 DDX 문서 확인](/help/forms/developing/assembler-service-java-api-quick.md#quick-start-soap-mode-validating-ddx-documents-using-the-java-api) (SOAP 모드)

[AEM Forms Java 라이브러리 파일 포함](/help/forms/developing/invoking-aem-forms-using-java.md#including-aem-forms-java-library-files)

[연결 속성 설정](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)

## 웹 서비스 API {#validate-a-ddx-document-using-the-web-service-api}를 사용하여 DDX 문서의 유효성 검사
