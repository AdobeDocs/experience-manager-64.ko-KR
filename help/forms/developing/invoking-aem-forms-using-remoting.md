...
>[!NOTE]
>
>Remoting을 사용하는 경우 AEM Forms 서비스가 아닌 Workbench에서 만든 프로세스를 호출하는 것이 좋습니다. 그러나 AEM Forms 서비스를 직접 호출할 수도 있습니다. (AEM Forms 개발자 센터에 있는 Remoting을 사용하여 PDF 문서 암호화를 참조하십시오.)

>[!NOTE]
>
>AEM Forms 서비스가 익명 액세스를 허용하도록 구성되지 않은 경우 Flex 클라이언트의 요청이 웹 브라우저 문제에 직면하게 됩니다. 사용자는 사용자 이름과 암호 자격 증명을 입력해야 합니다.

Remoting을 사용하여 이름이 짧은 다음 AEM Forms `MyApplication/EncryptDocument`를 호출할 수 있습니다. 입력 및 출력 값과 같은 이 프로세스에 대한 자세한 내용은 [짧은 기간 프로세스 예를 참조하십시오](/help/forms/developing/aem-forms-processes.md).
...
Flex 그래프 컨트롤에서 프로세스 데이터를 표시하는 방법에 대한 자세한 내용은 Flex 그래프로 [AEM Forms 프로세스 데이터 표시를 참조하십시오](https://www.adobe.com/devnet/livecycle/articles/populating_flexcontrols.html).

>[!NOTE]
>
>*crossdomain.xml 파일을 적절한 위치에 배치해야 합니다. 예를 들어 JBoss에 AEM Forms를 배포한 경우 이 파일을 다음 위치에 배치합니다. &lt;install_directory>\Adobe_Experience_Manager_forms\jboss\server\lc_turnkey\deploy\jboss-web.deployer\ROOT.war*
...
[AEM Forms 호출(AEM Forms에 대해 더 이상 사용되지 않음) AEM Forms Remoting](invoking-aem-forms-using-remoting.md#invoking-aem-forms-using-remoting)
...
Flex로 제작한 클라이언트 응용 프로그램은 문서를 AEM Forms 프로세스에 직접 전달할 수 없습니다. 대신 클라이언트 응용 프로그램은 `mx.rpc.livecycle.DocumentReference` ActionScript 클래스의 인스턴스를 사용하여 인스턴스 `com.adobe.idp.Document` 가 필요한 작업에 입력 매개 변수를 전달합니다. Flex 클라이언트 애플리케이션에는 `DocumentReference` 객체를 설정하는 여러 가지 옵션이 있습니다.
...
* 문서가 서버에 없으면 원격 업로드 서블릿을 사용하여 문서를 AEM Forms에 업로드합니다. AEM Forms의 새로운 기능은 보안 문서를 업로드하는 기능입니다. 보안 문서를 업로드할 때* 문서 업로드 응용 프로그램 사용자 *역할이 있는 사용자를 사용해야 합니다. 이 역할이 없으면 보안 문서를 업로드할 수 없습니다. 단일 사인온을 사용하여 보안 문서를 업로드하는 것이 좋습니다. (원격 [을 사용하여 프로세스를 호출하는 보안 문서 전달을 참조하십시오](invoking-aem-forms-using-remoting.md#passing-secure-documents-to-invoke-processes-using-remoting).)
   ...
   >[!NOTE]
   AEM Forms가 안전하지 않은 문서를 업로드하도록 구성된 경우 문서 업로드 응용 프로그램 사용자 역할이 없는 사용자를 사용하여 문서를 업로드할 수 있습니다. 사용자는 문서 업로드 권한을 가질 수도 있습니다. 그러나 AEM Forms가 보안 문서만 허용하도록 구성된 경우, 사용자에게 문서 업로드 응용 프로그램 사용자 역할 또는 문서 업로드 권한이 있는지 확인하십시오. 보안 [및 비보안 문서를 수락하도록 AEM Forms 구성을 참조하십시오](invoking-aem-forms-using-remoting.md#configuring-aem-forms-to-accept-secure-and-unsecure-documents).*
...
원격 빠른 시작 기능은 원격 업로드 서블릿을 사용하여 PDF 파일을 `MyApplication/EncryptDocument`프로세스에 전달합니다. ( [AEM Forms Remoting을 사용하여 안전하지 않은 문서를 전달하여 단기 프로세스 호출을 참조하십시오](invoking-aem-forms-using-remoting.md#invoking-a-short-lived-process-by-passing-an-unsecure-document-using-remoting).)
...
>[!NOTE]
이 섹션에서는 AEM Forms가 안전하지 않은 문서를 업로드하도록 구성된 경우 AEM Forms 프로세스를 호출하고 문서를 업로드하는 방법에 대해 설명합니다. AEM Forms 프로세스를 호출하고 보안 문서를 업로드하는 방법 및 보안 및 비보안 문서를 허용하도록 AEM Forms를 구성하는 방법에 대한 자세한 내용은 [Remoting을 사용하여 보안 문서 전달을 참조하십시오](invoking-aem-forms-using-remoting.md#passing-secure-documents-to-invoke-processes-using-remoting).

**mx:RemoteObject 인스턴스 만들기**

Workbench에서 만든 AEM Forms 프로세스를 호출하는 인스턴스를 만듭니다. `mx:RemoteObject` 인스턴스를 만들려면 `mx:RemoteObject` 다음 값을 지정합니다.
...
클라이언트 응용 프로그램은 다음 ActionScript 예제와 같이 MXML 또는 ActionScript에서 채널을 지정하여 AEM Forms를 호출할 수 있습니다. 채널은 `AMFChannel`, `SecureAMFChannel``HTTPChannel`또는 `SecureHTTPChannel`여야 합니다.
...
다음 예제와 같이 AEM Forms 프로세스 출력 매개 변수는 클라이언트 응용 프로그램이 이름별로 특정 매개 변수를 추출하는 ActionScript 객체로 디시리얼라이즈(deserialized)됩니다. (프로세스의 출력 값 `MyApplication/EncryptDocument` 이름이 `outDoc`지정됩니다.)
...
1. AEM Forms와 통신할 인스턴스를 `ChannelSet` 설정하고 인스턴스에 `mx:RemoteObject` 연결합니다. AEM 양식에 대한 채널 만들기를 참조하십시오.
...
AEM Forms 사용자 관리자가 중앙 로그인 서비스, 기본 인증 및 사용자 정의 인증을 통해 AEM Forms Single Sign-On을 비롯한 Flex 애플리케이션에서 원격 요청을 인증할 수 있는 방법에는 여러 가지가 있습니다. Single Sign-On 또는 익명 액세스가 활성화되지 않은 경우 Remoting 요청은 기본 인증(기본값)이나 사용자 정의 인증을 초래합니다.
...
AEM Forms 사용자는 여러 AEM Forms 웹 응용 프로그램에 연결하여 작업을 수행할 수 있습니다. 사용자가 웹 애플리케이션 간에 이동할 때 각 웹 애플리케이션에 개별적으로 로그인하도록 하는 것은 효율적이지 않습니다. AEM Forms 단일 사인온 메커니즘을 사용하면 사용자가 한 번 로그인한 다음 모든 AEM Forms 웹 응용 프로그램에 액세스할 수 있습니다. AEM Forms 개발자는 AEM Forms에서 사용할 클라이언트 애플리케이션을 만들 수 있으므로 단일 사인온 메커니즘을 활용할 수도 있어야 합니다.

각 AEM Forms 웹 응용 프로그램은 자체 WAR(Web Archive) 파일로 패키지되어 EAR(Enterprise Archive) 파일의 일부로 패키지됩니다. 응용 프로그램 서버는 다른 웹 응용 프로그램에서 세션 데이터의 공유를 허용하지 않으므로 AEM Forms는 HTTP 쿠키를 사용하여 인증 정보를 저장합니다. 인증 쿠키를 사용하여 사용자는 Forms 응용 프로그램에 로그인한 다음 다른 AEM Forms 웹 응용 프로그램에 연결할 수 있습니다. 이 기술을 단일 사인온이라고 합니다.

AEM Forms 개발자는 클라이언트 애플리케이션을 작성하여 양식 가이드(더 이상 사용되지 않음)의 기능을 확장하고 작업 영역을 사용자 정의합니다. 예를 들어 작업 공간 응용 프로그램에서 프로세스를 시작할 수 있습니다. 그런 다음 클라이언트 응용 프로그램은 원격 끝점을 사용하여 Forms 서비스에서 데이터를 검색합니다.

AEM Forms 서비스가 AEM Forms Remoting을 사용하여 호출되면 클라이언트 응용 프로그램이 인증 쿠키를 요청의 일부로 전달합니다. 사용자가 이미 인증되었으므로 클라이언트 응용 프로그램에서 AEM Forms 서비스에 연결하는 데 추가 로그인이 필요하지 않습니다.
...
AEM Forms Single Sign-On 메커니즘을 사용하는 경우 기본 인증이 아닌 사용자 정의 인증을 사용하도록 Remoting 끝점을 구성합니다. 그렇지 않으면 기본 인증을 사용할 때 인증 오류로 인해 사용자가 보지 않기를 바라는 브라우저 문제가 발생합니다. 대신 응용 프로그램에서 인증 오류를 감지한 다음 사용자가 중앙 로그인 서비스를 사용하여 로그인하도록 알려주는 메시지를 표시합니다.
...
Flex로 제작한 애플리케이션에는 AEM Forms 서비스에 대한 모든 요청과 함께 인증 쿠키가 포함되어 있습니다. 성능상의 이유로 AEM Forms는 모든 요청에서 쿠키의 유효성을 검사하지 않습니다. 그러나 AEM Forms는 인증 쿠키가 다른 인증 쿠키로 바뀌는 경우를 감지합니다.
...
Flex로 구축한 클라이언트 애플리케이션을 시작하고 중앙 로그인 서비스를 사용하여 로그아웃할 수 있습니다. 로그아웃 프로세스의 일부로 인증 쿠키가 삭제됩니다. 쿠키 없이 또는 잘못된 쿠키로 원격 요청이 수행된 경우 사용자 세션이 무효화됩니다. 이 작업은 실제로 로그아웃 상태입니다. 다음 번에 클라이언트 응용 프로그램이 AEM Forms 서비스에 연결하려고 하면 사용자에게 로그인이 요청됩니다.
...
하나 이상의 문서가 필요한 프로세스를 호출할 때 보안 문서를 AEM Forms로 전달할 수 있습니다. 안전한 문서를 전달함으로써 비즈니스 정보와 기밀 문서를 보호합니다. 이 경우 문서는 PDF 문서, XML 문서, Word 문서 등을 참조할 수 있습니다. AEM Forms가 보안 문서를 허용하도록 구성된 경우 Flex로 작성된 클라이언트 애플리케이션에서 AEM Forms로 보안 문서를 전달해야 합니다. (보안 및 비보안 [문서를 수락하도록 AEM Forms 구성을 참조하십시오](invoking-aem-forms-using-remoting.md#configuring-aem-forms-to-accept-secure-and-unsecure-documents).)
...
AEM Forms는 업로드 서블릿에 전달된 토큰을 반환하는 작업 `getFileUploadToken` 을 지원합니다. 이 `DocumentReference.constructRequestForUpload` 메서드에는 메서드에서 반환된 토큰과 함께 AEM Forms에 대한 URL이 `LC.FileUploadAuthenticator.getFileUploadToken` 필요합니다. 이 메서드는 업로드 서블릿으로의 호출에 사용되는 `URLRequest` 객체를 반환합니다. 다음 코드는 이 응용 프로그램 논리를 보여 줍니다.
...
관리 콘솔을 사용하여 Flex 클라이언트 애플리케이션에서 AEM Forms 프로세스로 문서를 전달할 때 문서의 보안 여부를 지정할 수 있습니다. 기본적으로 AEM Forms는 보안 문서를 허용하도록 구성되어 있습니다. 다음 단계를 수행하여 보안 문서를 수락하도록 AEM Forms를 구성할 수 있습니다.
...
>[!NOTE]
AEM Forms가 안전하지 않은 문서를 허용하도록 구성하려면 Flex 애플리케이션에서 비보안 문서 업로드 허용 옵션을 선택합니다. 그런 다음 응용 프로그램 또는 서비스를 다시 시작하여 설정이 적용되도록 합니다.
...
보안 문서만 업로드하도록 AEM Forms가 구성되어 있고 사용자에게* 문서 업로드 응용 프로그램 사용자 *역할이 없는 경우 예외가 발생합니다. 사용자에게 이 역할이 있으면 파일이 업로드되고 프로세스가 호출됩니다.
...
ActionScript 클래스에서 `RemoteClass` 메타데이터 태그를 사용하여 AEM Forms 복잡한 유형에 매핑합니다. 예를 들어 고객 서비스 `createCustomer` 작업을 호출할 때 데이터 유형에 매핑되는 ActionScript 클래스를 `com.adobe.livecycle.sample.customer.Customer` 만듭니다.
...
ActionScript 클래스의 필드는 AEM Forms 복잡한 유형에 속하는 필드와 일치합니다. 고객 ActionScript 클래스에 있는 6개의 필드는 속한 필드와 일치합니다 `com.adobe.livecycle.sample.customer.Customer`.
...
고객 ActionScript 클래스는 customer라는 패키지에 속합니다. 복잡한 AEM Forms 데이터 유형에 매핑되는 모든 ActionScript 클래스를 고유한 패키지에 배치하는 것이 좋습니다. 다음 그림과 같이 Flex 프로젝트의 src 폴더에 폴더를 만들고 폴더에 ActionScript 파일을 배치합니다.
