...
저장소 서비스는 AEM Forms에 리소스 저장 및 관리 서비스를 제공합니다. 개발자는 *AEM Forms* 애플리케이션을 만들 때 파일 시스템 대신 저장소에 자산을 배포할 수 있습니다. 자산은 XML 양식, PDF forms(Acrobat 양식 포함), 양식 조각, 이미지, 프로필, 정책, SWF 파일, DDX 파일, XML 스키마, WSDL 파일, 테스트 데이터 등 모든 유형의 자료를 포함할 수 있습니다.

예를 들어 Applications/FormsApplication이라는 다음 Forms 애플리케이션을 *생각해 보십시오*.
...
>[!NOTE]
>
>웹 브라우저를 사용하여 AEM Forms 저장소를 찾아볼 수 있습니다. 리포지토리를 찾아보려면 다음 URL을 웹 브라우저 https://[서버 이름]:[서버 포트]/리포지토리에 입력합니다. 웹 브라우저를 사용하여 AEM Forms 리포지토리로 작업 섹션에 연결된 빠른 시작 결과를 확인할 수 있습니다. 예를 들어 AEM Forms 저장소에 컨텐츠를 추가하는 경우 웹 브라우저에서 컨텐츠를 볼 수 있습니다. (빠른 [시작(SOAP 모드 참조): Java API를 사용하여 리소스](/help/forms/developing/repository-service-api-quick-starts.md#quick-start-soap-mode-writing-a-resource-using-the-java-api)작성)
...
* 작업 내역, 관련 리소스 및 속성을 비롯한 리소스를 검색합니다. 리소스 [검색을 참조하십시오](aem-forms-repository.md#searching-for-resources).
...
>[!NOTE]
>
>저장소 서비스에 대한 자세한 내용은 AEM Forms [에 대한 서비스 참조를 참조하십시오](https://www.adobe.com/go/learn_aemforms_services_63).
...
1. 폴더 만들기
   ...
   개체의 `repositoryInfomodelFactoryBean` 메서드를 `newResourceCollection` 호출하고 다음 매개 변수를 전달합니다.
   ...
   * 리소스 컬렉션의 이름이 들어 `java.lang.String` 있는 것입니다. 예, `FormsFolder`.
   ...
   메서드를 사용하여 폴더 설명을 설정하고 다음 매개 변수를 전달하십시오. `setDescription`

   * 리소스 컬렉션 `String` 을 설명하는 것입니다. 이 예에서, `"test Folder"` `.`


1. 저장소에 폴더 쓰기

   객체의 메서드를 호출하고 폴더 및 객체의 URI를 `ResourceRepositoryClient` `writeResource` `ResourceCollection` 전달합니다. 예를 들어 폴더에 대한 URI는 다음 값이 될 수 있습니다 `/Applications/FormsApplication/1.0/`.

   이 메서드는 새로 만든 `com.adobe.repository.infomodel.bean.Resource` 개체의 인스턴스를 반환합니다. 예를 들어 개체의 메서드를 호출하여 새 리소스의 식별자 값을 검색할 수 `com.adobe.repository.infomodel.bean.Resource` `getId` 있습니다.
...
1. 서비스 클라이언트 만들기
   ...
   Microsoft .NET 클라이언트 어셈블리를 사용하여 기본 생성자를 호출하여 `RepositoryServiceService` 개체를 만듭니다. 사용자 `Credentials` 이름과 암호를 포함하는 `System.Net.NetworkCredential` 개체를 사용하여 속성을 설정합니다.

1. 폴더 만들기

   클래스에 대한 기본 생성자를 사용하여 폴더를 `ResourceCollection` 만들고 다음 매개 변수를 전달합니다.

   * 클래스 `Id` 의 기본 생성자를 호출하고 개체의 `Id` 필드에 할당하여 만든 `Resource` `id` 개체입니다.
   * 클래스 `Lid` 의 기본 생성자를 호출하고 개체의 `Lid` 필드에 할당하여 만든 `Resource` `lid` 개체입니다.
   * 리소스 컬렉션의 이름이 포함된 문자열( `Resource` 개체의 `name` 필드)입니다. 이 예에서 사용되는 이름은 입니다 `"testfolder"`.
   * 리소스 컬렉션의 설명이 포함된 문자열( `Resource` 개체의 `description` 필드)입니다. 이 예에서 사용된 설명은 입니다 `"test folder"`.

1. 저장소에 폴더 쓰기

   개체의 `RepositoryServiceService` `writeResource` 메서드를 호출하고 다음 매개 변수를 전달합니다.

   * 폴더를 만들 경로입니다.
   * 폴더를 나타내는 `ResourceCollection` 개체입니다.
   * 다른 두 매개 변수 `null` 에 대해 전달합니다.
...
1. 리소스에 대한 대상 폴더의 URI 지정
   ...
   리소스에 대한 대상 폴더의 URI를 지정합니다. 이 경우 이름이 지정된 리소스가 이름이 지정된 폴더에 저장되므로 폴더 `testResource` 의 URI가 저장됩니다 `testFolder``"/testFolder"`. URI는 `java.lang.String` 개체로 저장됩니다.

1. 리소스 만들기

   리소스를 만들려면 먼저 `com.adobe.repository.infomodel.bean.RepositoryInfomodelFactoryBean` 개체를 만들어야 합니다.

   개체를 만드는 `RepositoryInfomodelFactoryBean` 개체의 `newResource` 메서드를 `com.adobe.repository.infomodel.bean.Resource` 호출합니다. 이 예에서 다음 매개 변수가 제공됩니다.

   * 클래스 `com.adobe.repository.infomodel.Id` 의 기본 생성자를 호출하여 만들어지는 `Id` 개체입니다.
   * 클래스 `com.adobe.repository.infomodel.Lid` 의 기본 생성자를 호출하여 만들어지는 `Lid` 개체입니다.
   * 리소스의 파일 이름이 들어 `java.lang.String` 있는 경우

   리소스의 설명을 지정하려면 `Resource` 개체의 `setDescription` 메서드를 호출하고 설명이 포함된 문자열을 전달합니다. 이 예에서 설명은 입니다 `"test resource"`.

1. 리소스 컨텐츠 지정

   리소스에 대한 컨텐츠를 만들려면 `RepositoryInfomodelFactoryBean` 개체의 `newResourceContent` 메서드를 호출하여 `com.adobe.repository.infomodel.bean.ResourceContent` 개체를 반환합니다. 객체에 컨텐츠를 `ResourceContent` 추가합니다. 이 예에서는 다음 작업을 수행하여 수행됩니다.

   * 개체 `ResourceContent` 의 `setDataDocument` 메서드 호출 및 `com.adobe.idp.Document` 개체 전달
   * 개체 `ResourceContent` 의 `setSize` 메서드 호출 및 개체 크기(바이트 단위) `Document`

   개체의 메서드를 호출하고 개체를 전달하여 `Resource` 리소스에 `setContent` 컨텐츠를 `ResourceContent` 추가합니다. 자세한 내용은 [AEM Forms API 참조를 참조하십시오](https://www.adobe.com/go/learn_aemforms_javadocs_63_en).

1. 대상 폴더에 리소스 쓰기

   개체의 `ResourceRepositoryClient` 메서드를 `writeResource` 호출하고 개체와 폴더의 URI를 `Resource` 전달합니다.
...
1. 서비스 클라이언트 만들기
   ...
   Microsoft .NET 클라이언트 어셈블리를 사용하여 기본 생성자를 호출하여 `RepositoryServiceService` 개체를 만듭니다. 사용자 `Credentials` 이름과 암호를 포함하는 `System.Net.NetworkCredential` 개체를 사용하여 해당 속성을 설정합니다.

1. 리소스에 대한 대상 폴더의 URI 지정

   리소스에 대한 대상 폴더의 URI를 지정합니다. 이 경우 이름이 지정된 리소스가 이름이 지정된 폴더에 저장되므로 폴더 `testResource` 의 URI가 저장됩니다 `testFolder``"/testFolder"`. Microsoft .NET Framework(예: C#)와 호환되는 언어를 사용하는 경우 `System.String` 개체에 URI를 저장합니다.

1. 리소스 만들기

   리소스를 만들려면 클래스에 대한 기본 생성자를 `Resource` 호출합니다. 이 예에서 다음 정보는 `Resource` 개체에 저장됩니다.

   * 클래스 `com.adobe.repository.infomodel.Id` 의 기본 생성자를 호출하고 개체의 `Id` 필드에 할당하여 만든 `Resource` `id` 개체입니다.
   * 클래스 `com.adobe.repository.infomodel.Lid` 의 기본 생성자를 호출하고 개체의 `Lid` 필드에 할당하여 만든 `Resource` `lid` 개체입니다.
   * 리소스의 파일 이름이 들어 있는 문자열로서, `Resource` 개체의 `name` 필드에 할당됩니다. 이 예에서 사용되는 이름은 입니다 `"testResource"`.
   * 리소스의 설명을 포함하는 문자열( `Resource` 객체의 `description` 필드에 지정됨). 이 예에서 사용된 설명은 입니다 `"test resource"`.

1. 리소스 컨텐츠 지정

   리소스에 대한 컨텐츠를 만들려면 클래스에 대한 기본 생성자를 `ResourceContent` 호출합니다. 그런 다음 객체에 컨텐츠를 `ResourceContent` 추가합니다. 이 예에서는 다음 작업을 수행하여 수행됩니다.

   * 문서가 `BLOB` 들어 있는 개체를 `ResourceContent` 개체 `dataDocument` 필드에 할당합니다.
   * 개체 필드에 개체 크기(바이트)를 `BLOB` 할당합니다 `ResourceContent` `size` .

   개체 `ResourceContent` `Resource` `content` 필드에 개체를 할당하여 리소스에 컨텐츠를 추가합니다.

1. 대상 폴더에 리소스 쓰기

   개체의 `RepositoryServiceService` 메서드를 `writeResource` 호출하고 개체와 폴더의 URI를 `Resource` 전달합니다. 다른 두 매개 변수 `null` 에 대해 전달합니다.
...
1. 폴더 경로 지정
   ...
   쿼리할 리소스 컬렉션의 URI를 지정합니다. 이 경우 URI가 됩니다 `"/testFolder"`. URI는 `java.lang.String` 개체로 저장됩니다.

1. 리소스 목록 검색

   개체의 `ResourceRepositoryClient` `listMembers` 메서드를 호출하고 폴더의 URI를 전달합니다.

   이 메소드는 유형 `java.util.List` 의 소스인 `com.adobe.repository.infomodel.bean.Resource` 객체 `com.adobe.repository.infomodel.bean.Relation` 를 반환하고 리소스 수집 URI를 대상으로 `Relation.TYPE_MEMBER_OF` 합니다. 이를 통해 각 리소스 `List` 를 검색할 수 있습니다. 이 예에서는 각 리소스의 이름과 설명이 표시됩니다.
...
### 웹 서비스 API를 사용하는 리소스 목록 {#list-resources-using-the-web-service-api}
...
1. 폴더 경로 지정
   ...
   쿼리할 폴더의 URI가 포함된 문자열을 지정합니다. 이 경우 URI가 됩니다 `"/testFolder"`. Microsoft .NET Framework(예: C#)를 준수하는 언어를 사용하는 경우 `System.String` 개체에 URI를 저장합니다.

1. 리소스 목록 검색

   객체의 `RepositoryServiceService` `listMembers` 메서드를 호출하고 폴더의 URI를 첫 번째 매개 변수로 전달합니다. 다른 두 매개 변수 `null` 에 대해 전달합니다.
...
1. 읽을 리소스의 URI 지정
   ...
   검색할 리소스의 URI를 나타내는 문자열 값을 지정합니다. 예를 들어 리소스 이름이 testFolder라는 폴더에 있는 *testResource* 라고 가정할 경우 *testFolder*&#x200B;를 `/testFolder/testResource`지정합니다.
...
### 웹 서비스 API를 사용한 리소스 읽기 {#reading-resources-using-the-web-service-api}

Repository service API(웹 서비스)를 사용하여 리소스를 확인합니다.

1. 프로젝트 파일 포함

   * 저장소 WSDL을 사용하는 Microsoft .NET 클라이언트 어셈블리를 만듭니다. ( [Base64 인코딩을 사용하는 .NET 클라이언트 어셈블리 만들기를 참조하십시오](/help/forms/developing/invoking-aem-forms-using-web.md#creating-a-net-client-assembly-that-uses-base64-encoding).)
...
1. 읽을 리소스의 URI 지정
   ...
   검색할 리소스의 URI가 포함된 문자열을 지정합니다. 이 경우 이름이 지정된 리소스 `testResource` 가 이름이 지정된 폴더에 있으므로 해당 URI가 `testFolder`됩니다 `"/testFolder/testResource"`. Microsoft .NET Framework(예: C#)와 호환되는 언어를 사용하는 경우 `System.String` 개체에 URI를 저장합니다.

1. 리소스 보기

   개체의 `RepositoryServiceService` `readResource` 메서드를 호출하고 리소스의 URI를 첫 번째 매개 변수로 전달합니다. 다른 두 매개 변수 `null` 에 대해 전달합니다.
...
1. 리소스 업데이트
   ...
   * 주 버전 또는 부 버전을 업데이트할지 여부를 나타내는 `boolean` 값입니다. 이 예에서, 의 값 `true` 이 전달되어 주 버전이 증가함을 나타냅니다.
...
1. 업데이트할 리소스 검색
   ...
   검색할 리소스의 URI를 지정하고 리소스를 읽습니다. 이 예에서 리소스의 URI는 입니다 `"/testFolder/testResource"`. 자세한 내용은 리소스 [읽기를 참조하십시오](aem-forms-repository.md#reading-resources).

1. 리소스 업데이트

   개체 정보를 `Resource` 업데이트합니다. 이 예에서 설명을 업데이트하려면 `Resource` 개체 `description` 필드에 새 값을 지정합니다.

1. 개체의 `RepositoryServiceService` 메서드를 `updateResource` 호출하고 다음 매개 변수를 전달합니다.

   * 리소스의 URI가 포함된 `System.String` 개체입니다.
   * 업데이트된 리소스 정보가 들어 있는 `Resource` 개체입니다.
   * 주 버전 또는 부 버전을 업데이트할지 여부를 나타내는 `boolean` 값입니다. 이 예에서, 의 값 `true` 이 전달되어 주 버전이 증가함을 나타냅니다.
   * 나머지 두 매개 변수 `null` 에 대해 전달합니다.
...
쿼리에 ** 포함된 하나 이상의 문은 조건에 논리적으로 연결되어 있습니다. 문은 *왼쪽 피연산자, 연산자 및 오른쪽 피연산자로 구성됩니다* . 또한 검색 결과에 사용할 정렬 순서를 지정할 수 있습니다. 정렬 순서 *는 SQL* `ORDER BY` 절에 상응하는 정보를 포함하며, 검색의 기준이 되는 속성이 포함된 요소와 오름차순 또는 내림차순 사용 여부를 나타내는 값으로 구성됩니다.

저장소 서비스 Java API를 사용하여 프로그래밍 방식으로 리소스를 검색할 수 있습니다. 지금은 웹 서비스 API를 사용하여 리소스를 검색할 수 없습니다.

**정렬 동작**

개체의 메서드를 호출하고 정렬 순서를 지정할 때 정렬 순서가 `ResourceRepositoryClient` `searchProperties` 적용되지 않습니다. 예를 들어, 속성 이름이 `name`, `secondName`및 `asecondName`인 세 개의 사용자 지정 속성이 있는 리소스를 만든다고 가정합니다. 그런 다음 속성 이름에 정렬 순서 요소를 만들고 값을 로 `ascending` 설정합니다 `true`.

그런 다음 `ResourceRepositoryClient` 개체의 `searchProperties` 메서드를 호출하고 정렬 순서를 전달합니다. 검색은 세 개의 속성을 가진 올바른 리소스를 반환합니다. 하지만 속성은 속성 이름별로 정렬되지 않습니다. 추가된 순서대로 반환됩니다. `name`, `secondName`and `asecondName`.
...
1. 검색에 사용되는 속성 지정
   ...
   검색을 수행할 속성의 값을 지정합니다. 속성은 객체 내에 `com.adobe.repository.infomodel.bean.Resource` 있습니다. 이 예에서는 name 속성에 대해 검색이 수행됩니다. 따라서 이 경우, 개체 `java.lang.String` 의 `Resource` 이름이 들어 있는 `testResource` 것이 사용됩니다.

1. 검색에 사용되는 쿼리 만들기

   쿼리를 만들려면 클래스에 대한 기본 생성자를 호출하여 개체를 만들고 쿼리에 문을 `com.adobe.repository.query.Query` `Query` 추가합니다.

   문을 만들려면 클래스에 대한 생성자를 `com.adobe.repository.query.Query.Statement` 호출하고 다음 매개 변수를 전달합니다.

   * 리소스 속성 상수가 들어 있는 왼쪽 피연산자입니다. 이 예에서는 리소스 이름이 검색 기준으로 사용되므로 정적 값이 `Resource.ATTRIBUTE_NAME` 사용됩니다.
   * 속성 검색에 사용되는 조건을 포함하는 연산자입니다. 연산자는 클래스에 있는 정적 상수 중 하나여야 `Query.Statement` 합니다. 이 예에서 정적 값 `Query.Statement.OPERATOR_BEGINS_WITH` 이 사용됩니다.
   * 검색을 수행할 속성 값을 포함하는 오른쪽 피연산자입니다. 이 예에서 값이 들어 있는 name 속성 `String` 이 `"testResource"`사용됩니다.

   객체의 메서드를 호출하고 클래스에 포함된 정적 값 중 하나를 전달하여 왼쪽 피연산자의 네임스페이스를 `Query.Statement` `setNamespace` `com.adobe.repository.infomodel.bean.ResourceProperty` 지정합니다. 이 예에서 `ResourceProperty.RESERVED_NAMESPACE_REPOSITORY` 가 사용됩니다.

   개체의 메서드를 호출하고 개체를 전달하여 각 `Query` 문을 쿼리에 `addStatement` `Query.Statement` 추가합니다.

1. 검색 결과에 대한 정렬 순서 만들기

   검색 결과에 사용되는 정렬 순서를 지정하려면 클래스에 대한 기본 생성자를 호출하여 `com.adobe.repository.query.sort.SortOrder` 개체를 만들고 정렬 `SortOrder` 순서에 요소를 추가합니다.

   정렬 순서에 대한 요소를 만들려면 클래스에 대한 생성자 중 하나를 `com.adobe.repository.query.sort.SortOrder.Element` 호출합니다. 이 예에서 리소스 이름이 검색 기준으로 사용되므로 정적 값 `Resource.ATTRIBUTE_NAME` 은 첫 번째 매개 변수로 사용되고 오름차순(값 `boolean` )은 두 번째 매개 변수로 `true`지정됩니다.

   개체의 메서드를 호출하고 개체를 전달하여 각 요소를 정렬 순서에 `SortOrder` `addSortElement` `SortOrder.Element` 추가합니다.

1. 리소스 검색

   속성 속성을 `resources` 기반으로 검색하려면 `ResourceRepositoryClient` `searchProperties` 객체의 메서드를 호출하고 다음 매개 변수를 전달합니다.

   * 검색을 실행할 기본 경로를 포함하는 `String` 것입니다. 이 경우 `"/testFolder"` 가 사용됩니다.
   * 검색에 사용되는 쿼리입니다.
   * 검색 깊이 이 경우 기본 경로 `com.adobe.repository.infomodel.bean.ResourceCollection.DEPTH_INFINITE` 와 모든 해당 폴더를 사용할 것임을 나타내는 데 사용됩니다.
   * 페이징되지 않은 결과 집합을 선택할 첫 번째 행을 나타내는 `int` 값입니다. 이 예에서, 가 `0` 지정됩니다.
   * 반환할 최대 결과 수를 나타내는 `int` 값입니다. 이 예에서, 가 `10` 지정됩니다.
   * 검색에 사용되는 정렬 순서.

   이 메서드는 지정된 정렬 순서 `java.util.List` 에서 `Resource` 개체를 반환합니다.

1. 검색 결과에서 리소스 검색

   검색 결과에 포함된 리소스를 검색하려면 해당 정보를 추출하기 위해 각 개체를 `List` 반복하여 `Resource` a에 캐스팅합니다. 이 예에서 각 리소스의 이름이 표시됩니다.
...
1. 연결할 리소스의 URI 지정
   ...
   관련된 리소스의 URI를 지정합니다. 이 경우 리소스가 이름 `testResource1` 으로 이름이 지정되고 이름이 지정된 폴더 `testResource2` 에 있으므로 해당 URI `testFolder`는 and `"/testFolder/testResource1"` 입니다 `"/testFolder/testResource2"`. URI는 개체로 `java.lang.String` 저장됩니다. 이 예에서 리소스는 먼저 저장소에 기록되고 해당 URI가 검색됩니다. 리소스 작성에 대한 자세한 내용은 리소스 [쓰기를 참조하십시오](aem-forms-repository.md#writing-resources).

1. 관계 만들기

   개체의 `ResourceRepositoryClient` `createRelationship` 메서드를 호출하고 다음 매개 변수를 전달합니다.

   * 소스 리소스의 URI입니다.
   * 대상 리소스의 URI입니다.
   * 클래스의 정적 상수 중 하나인 관계 `com.adobe.repository.infomodel.bean.Relation` 유형입니다. 이 예에서는 값을 지정하여 종속 관계가 설정됩니다 `Relation.TYPE_DEPENDANT_OF`.
   * 대상 리소스를 새 헤드 리소스의 `boolean` `com.adobe.repository.infomodel.Id`기반 식별자로 자동으로 업데이트할지 여부를 나타내는 값입니다. 이 예에서는 종속 관계로 인해 값이 `true` 지정됩니다.

   객체의 메서드를 호출하고 다음 매개 변수를 전달하여 주어진 리소스에 대한 관련 리소스 목록을 검색할 수도 있습니다. `ResourceRepositoryClient` `getRelated`

   * 관련 리소스를 검색할 리소스의 URI입니다. 이 예에서 소스 리소스( `"/testFolder/testResource1"`)가 지정됩니다.
   * 지정한 리소스가 관계의 소스 리소스인지 여부를 나타내는 `boolean` 값입니다. 이 예에서 값 `true` 은 대/소문자이므로 지정됩니다.
   * 클래스의 정적 상수 중 하나인 관계 `Relation` 유형입니다. 이 예에서는 앞서 사용한 것과 동일한 값을 사용하여 종속 관계를 지정합니다. `Relation.TYPE_DEPENDANT_OF`.

   이 `getRelated` 메서드는 관련된 각 리소스를 검색할 수 있도록 반복할 수 있는 객체 `java.util.List` 를 반환하고 여기에 포함된 객체를 `Resource` 가져오는 방식 `List` 을 `Resource` 반환합니다. 이 예에서 `testResource2` 는 반환된 리소스 목록에 있어야 합니다.
...
1. 연결할 리소스의 URI 지정
   ...
   관련된 리소스의 URI를 지정합니다. 이 경우 리소스가 이름 `testResource1` 으로 이름이 지정되고 이름이 지정된 폴더 `testResource2` 에 있으므로 해당 URI `testFolder`는 and `"/testFolder/testResource1"` 입니다 `"/testFolder/testResource2"`. Microsoft .NET Framework(예: C#)와 호환되는 언어를 사용하는 경우 URI가 `System.String` 개체로 저장됩니다. 이 예에서 리소스는 먼저 저장소에 기록되고 해당 URI가 검색됩니다. 리소스 작성에 대한 자세한 내용은 리소스 [쓰기를 참조하십시오](aem-forms-repository.md#writing-resources).

1. 관계 만들기

   개체의 `RepositoryServiceService` `createRelationship` 메서드를 호출하고 다음 매개 변수를 전달합니다.

   * 소스 리소스의 URI입니다.
   * 대상 리소스의 URI입니다.
   * 관계 유형. 이 예에서는 값을 지정하여 종속 관계가 설정됩니다 `3`.
   * 관계 유형을 지정했는지 여부를 나타내는 `boolean` 값입니다. 이 예에서 값이 `true` 지정됩니다.
   * 대상 리소스를 새 헤드 리소스의 `boolean` `Id`기반 식별자로 자동으로 업데이트할지 여부를 나타내는 값입니다. 이 예에서는 종속 관계로 인해 값이 `true` 지정됩니다.
   * 대상 헤드가 지정되었는지 여부를 나타내는 `boolean` 값입니다. 이 예에서 값이 `true` 지정됩니다.
   * 마지막 매개 변수 `null` 에 대해 전달됩니다.

   객체의 메서드를 호출하고 다음 매개 변수를 전달하여 주어진 리소스에 대한 관련 리소스 목록을 검색할 수도 있습니다. `RepositoryServiceService` `getRelated`

   * 관련 리소스를 검색할 리소스의 URI입니다. 이 예에서 소스 리소스( `"/testFolder/testResource1"`)가 지정됩니다.
   * 지정한 리소스가 관계의 소스 리소스인지 여부를 나타내는 `boolean` 값입니다. 이 예에서 값 `true` 은 대/소문자이므로 지정됩니다.
   * 소스 리소스를 지정했는지 여부를 나타내는 `boolean` 값입니다. 이 예에서 값이 `true` 제공됩니다.
   * 관계 유형을 포함하는 정수 배열. 이 예에서는 앞서 사용된 것과 동일한 값을 배열에 사용하여 종속 관계가 지정됩니다. `3`.
   * 나머지 두 매개 변수 `null` 에 대해 전달합니다.

   이 `getRelated` 메서드는 관련된 각 리소스를 검색할 수 있도록 반복할 수 있는 객체 `Resource` 의 배열을 반환합니다. 이 예에서 `testResource2` 는 반환된 리소스 목록에 있어야 합니다.
...
1. 잠글 리소스의 URI 지정
   ...
   잠글 리소스의 URI를 지정합니다. 이 경우 이름이 지정된 리소스 `testResource` 가 이름이 지정된 폴더에 있으므로 해당 URI가 `testFolder`됩니다 `"/testFolder/testResource"`. URI는 `java.lang.String` 개체로 저장됩니다.

1. 리소스 잠금

   개체의 `ResourceRepositoryClient` `lockResource` 메서드를 호출하고 다음 매개 변수를 전달합니다.

   * 리소스의 URI입니다.
   * 잠금 범위 이 예에서 리소스는 독점적으로 사용할 수 있도록 잠겨 있으므로 잠금 범위가 로 지정됩니다 `com.adobe.repository.infomodel.bean.Lock.SCOPE_EXCLUSIVE`.
   * 잠금 깊이입니다. 이 예에서 잠금은 특정 리소스에만 적용되고 해당 멤버 또는 하위 리소스만 적용되지 않으므로 잠금 깊이가 로 지정됩니다 `Lock.DEPTH_ZERO`.

   >[!NOTE]
   >
   >4개의 매개 변수가 필요한 `lockResource` 메서드의 오버로드된 버전에서는 예외가 발생합니다. 이 연습에서 보듯이 세 개의 매개 변수가 필요한 `lockResource` 메서드를 사용해야 합니다.

1. 리소스에 대한 잠금 검색

   개체의 `ResourceRepositoryClient` `getLocks` 메서드를 호출하고 리소스의 URI를 매개 변수로 전달합니다. 이 메서드는 반복할 수 있는 List of Lock 객체를 반환합니다. 이 예에서 각 Lock 개체의 잠금 소유자, 깊이 및 범위는 각각 각 개체에 대해 `getOwnerUserId``getDepth`및 `getType` 메서드를 호출하여 인쇄됩니다.
...
1. 잠글 리소스의 URI 지정
   ...
   잠글 리소스의 URI가 포함된 문자열을 지정합니다. 이 경우 이름이 지정된 리소스 `testResource` 가 폴더에 있으므로 해당 URI `testFolder`가 됩니다 `"/testFolder/testResource"`. Microsoft .NET Framework(예: C#)와 호환되는 언어를 사용하는 경우 `System.String` 개체에 URI를 저장합니다.

1. 리소스 잠금

   개체의 `RepositoryServiceService` `lockResource` 메서드를 호출하고 다음 매개 변수를 전달합니다.

   * 리소스의 URI입니다.
   * 잠금 범위 이 예에서 리소스는 독점적으로 사용할 수 있도록 잠겨 있으므로 잠금 범위가 로 지정됩니다 `11`.
   * 잠금 깊이입니다. 이 예에서 잠금은 특정 리소스에만 적용되고 해당 멤버 또는 하위 리소스만 적용되지 않으므로 잠금 깊이가 로 지정됩니다 `2`.
   * 잠금이 만료될 때까지 시간(초)을 나타내는 `int` 값입니다. 이 예에서는 값이 `1000` 사용됩니다.
   * 마지막 매개 변수 `null` 에 대해 전달됩니다.

1. 리소스에 대한 잠금 검색

   개체의 `RepositoryServiceService` 메서드를 호출하고 리소스의 URI를 첫 번째 매개 변수로 전달하며 두 번째 매개 변수 `getLocks` `null` 에 대해 전달합니다. 이 메서드는 반복할 수 있는 개체를 포함하는 `object` `Lock` 배열을 반환합니다. 이 예에서는 각 `Lock` 개체의 필드, `ownerUserId`필드 `depth`및 필드를 각각 액세스하여 잠금 소유자, 깊이 및 범위를 각 개체에 대해 `type` 인쇄합니다.

1. 리소스 잠금 해제

   개체의 `RepositoryServiceService` 메서드를 호출하고 리소스의 URI를 첫 번째 매개 변수로 전달하며 두 번째 매개 변수 `unlockResource` `null` 에 대해 전달합니다.
...
>[!NOTE]
>
>ECM 저장소(EMC Documentum Content Server 및 IBM FileNet P8 Content Manager)에서 `com.adobe.repository.bindings.dsc.client.ResourceRepositoryClient.deleteResources()` 메서드를 사용하는 경우, 지정된 리소스 중 하나에 대한 삭제가 실패할 경우 트랜잭션이 롤백되지 않으므로 삭제된 파일은 삭제할 수 없습니다.
...
1. 삭제할 리소스의 URI를 지정합니다.
   ...
   검색할 리소스의 URI를 지정합니다. 이 경우 testResourceToBeDeleted라는 리소스가 testFolder라는 폴더에 있으므로 해당 URI가 됩니다 `/testFolder/testResourceToBeDeleted`. URI는 `java.lang.String` 개체로 저장됩니다. 이 예에서 리소스는 처음으로 저장소에 기록되고 해당 URI가 검색됩니다. 리소스 작성에 대한 자세한 내용은 리소스 [쓰기를 참조하십시오](aem-forms-repository.md#writing-resources).
...
1. 삭제할 리소스의 URI를 지정합니다.
   ...
   검색할 리소스의 URI를 지정합니다. 이 경우 이름이 지정된 리소스 `testResourceToBeDeleted` 가 이름이 지정된 폴더에 있으므로 해당 URI가 `testFolder`됩니다 `"/testFolder/testResourceToBeDeleted"`. 이 예에서 리소스는 처음으로 저장소에 기록되고 해당 URI가 검색됩니다. 리소스 작성에 대한 자세한 내용은 리소스 [쓰기를 참조하십시오](aem-forms-repository.md#writing-resources).

1. 리소스 삭제

   개체의 `RepositoryServiceService` 메서드를 `deleteResources` 호출하고 리소스의 URI가 포함된 `System.String` 배열을 첫 번째 매개 변수로 전달합니다. 두 번째 매개 변수 `null` 에 대해 전달합니다.
