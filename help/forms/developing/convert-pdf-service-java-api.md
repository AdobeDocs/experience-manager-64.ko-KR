...
다음 빠른 시작을 Convert PDF 서비스 API에 사용할 수 있습니다.
...
다음 코드 예는 Loan.pdf라는 PDF 문서 *를 Roan.ps라는* PostScript 문서로 *변환합니다*. (PDF [문서를 PostScript로 변환을 참조하십시오](/help/forms/developing/converting-pdf-postscript-image-files.md#converting-pdf-documents-to-postscript).)
...
다음 Java 코드 예제에서는 Loan.pdf라는 PDF 문서 *를* 일련의 JPEG 파일로 변환하여 C:\Adobe directory폴더에 저장합니다. 각 파일의 이름은 *[tempFileindex].jpg*&#x200B;로 지정됩니다. 여기서 첫 번째 이미지 파일의 이름은 *tempFile0.jpg입니다*. (PDF [문서를 이미지 포맷으로 변환을 참조하십시오](/help/forms/developing/converting-pdf-postscript-image-files.md#converting-pdf-documents-to-image-formats).)
