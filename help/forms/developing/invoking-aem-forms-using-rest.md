...
필수 `ServiceName` 값은 호출할 Forms 서비스의 이름입니다. 선택적 `OperationName` 값은 서비스 작업의 이름입니다. 이 값을 지정하지 않으면 이 이름 `invoke`의 기본값은 프로세스를 시작하는 작업 이름입니다. 선택적 `ServiceVersion` 값은 X.Y 형식으로 인코딩된 버전입니다. 이 값을 지정하지 않으면 최신 버전이 사용됩니다. 값은 `enctype` 될 수도 있습니다 `application/x-www-form-urlencoded`.
...
* XML 데이터 유형(예: `org.w3c.Document` 및 `org.w3c.Element`
* 컬렉션 개체(예: `java.util.List` 및 `java.util.Map`
   ...
   입력 맵이 서비스 전용 입력 매개 변수가 아닌 경우, 매개 변수 이름과 레코드 키의 연결이라는 매개 변수를 사용하여 맵에 속하는 각 키/값 레코드를 전송할 수 있습니다. 예를 들어 다음 키/값 쌍 목록을 사용하여 호출된 입력 맵을 보낼 `attributes` 수 있습니다.
   ...
   이렇게 하면 세 개의 레코드 맵으로 변환됩니다. `Color=red`, `Shape=box`and `Width=5`.

   목록 및 맵 유형의 출력 매개 변수는 결과 XML 메시지의 일부가 됩니다. 출력 목록은 XML에서 목록의 각 항목에 대해 하나의 요소가 있는 일련의 XML 요소로 표현됩니다. 모든 요소에는 출력 목록 매개 변수와 동일한 이름이 지정됩니다. 각 XML 요소의 값은 다음 두 가지 중 하나입니다.

* 목록에 있는 항목의 텍스트 표현(목록이 문자열 유형으로 구성된 경우)
* 문서의 컨텐츠를 가리키는 URL(목록이 `com.adobe.idp.Document` 개체로 구성된 경우)

   다음 예제는 정수 목록인 list라는 단일 출력 매개 변수가 있는 서비스에서 반환되는 XML *메시지입니다*.
   ` <result>   <list>12345</list>   . . .   <list>67890</list>  </result>`출력 맵 매개 변수는 결과 XML 메시지에 맵의 각 레코드에 대해 하나의 요소가 포함된 일련의 XML 요소로 표현됩니다. 모든 요소에는 지도 레코드의 키와 동일한 이름이 지정됩니다. 각 요소의 값은 맵 레코드 값의 텍스트 표현(맵이 문자열 값이 있는 레코드로 구성된 경우) 또는 문서 컨텐츠를 가리키는 URL(맵이 값이 있는 레코드로 구성된 경우)입니다 `com.adobe.idp.Document` . 다음은 이름이 지정된 단일 출력 매개 변수가 있는 서비스에서 반환되는 XML 메시지의 예입니다 `map`. 이 매개 변수 값은 문자와 개체를 연결하는 레코드로 구성된 `com.adobe.idp.Document` 맵입니다.
...
대신 호출 URL을 사용하여 비동기 호출의 상태를 검색할 수 `services` 있습니다 `async_status`. URL에는 이 호출과 연관된 작업의 식별자 값을 지정하는 매개 변수가 포함되어야 합니다. `job_id` 예:
...
작업이 완료되고 결과가 검색되면 호출 URL을 사용하여 작업을 처리할 수 있습니다(대신 `services` 로 대체됨) `async_dispose`. URL에는 작업의 식별자 값을 지정하는 매개 `job_id` 변수도 포함되어야 합니다. 예:
...
서버에서 throw되는 예외로 인해 동기 또는 비동기 호출 요청을 완료할 수 없는 경우 예외가 HTTP 응답 메시지의 일부로 보고됩니다. 호출 URL(또는 비동기 호출의 경우 `async_result` URL)에 .xml 접미사가 없는 경우 REST 제공자는 HTTP 코드를 반환하고 예외 메시지를 `500 Internal Server Error` 반환합니다.

호출 URL(또는 비동기 호출의 경우 `async_result` URL)에 .xml 접미어가 있으면 REST 공급자는 다음 형식의 예외를 설명하는 XML 문서 `200 OK`뒤에 HTTP 코드를 반환합니다.
...
요소는 `DSCError` 선택 사항이며 예외가 의 인스턴스인 경우에만 존재합니다 `com.adobe.idp.dsc.DSCException`.
...
정책을 만들기 위해 REST 요청을 보내는 대신( `PolicyEntry` 개체와 같은 복잡한 데이터 유형 정의 포함) 워크벤치를 사용하여 정책을 만드는 프로세스를 만듭니다. 프로세스 이름을 정의하는 문자열 값 또는 오프라인 임대 기간을 정의하는 정수와 같은 기본 입력 변수를 허용하는 프로세스를 정의합니다.

이렇게 하면 작업에 필요한 복잡한 데이터 형식을 포함하는 REST 호출 요청을 만들 필요가 없습니다. 프로세스는 복잡한 데이터 유형을 정의하며 REST 클라이언트에서 수행하는 모든 작업은 프로세스를 호출하고 원시 데이터 유형을 전달합니다. REST를 사용한 프로세스 호출에 대한 자세한 내용은 REST를 [사용하여 MyApplication/EncryptDocument 프로세스 호출을 참조하십시오](#rest-invocation-examples).
...
다음 HTML 예제에서는 PDF 문서가 필요한 AEM Forms 프로세스 `MyApplication/EncryptDocument` 를 호출합니다. 이 프로세스에 대한 자세한 내용은 MTOM을 [사용하여 AEM Forms 호출을 참조하십시오](/help/forms/developing/invoking-aem-forms-using-web.md#invoking-aem-forms-using-mtom).
...
다음 HTML 예제에서는 열거형 값이 필요한 AEM Forms 프로세스 `SOAPEchoService` 를 호출합니다. HTML Post 메서드가 사용된다는 것을 확인합니다.
...
>[!NOTE]
>
>이 프로세스는 기존 AEM Forms 프로세스를 기반으로 하지 않습니다. 코드 예제를 따라 하려면 workbench라는 프로세스를 `MyApplication/EncryptDocument` 만듭니다. (워크벤치 [사용을 참조하십시오](https://www.adobe.com/go/learn_aemforms_workbench_63).)
