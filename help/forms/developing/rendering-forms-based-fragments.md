...
Forms 서비스는 디자이너를 사용하여 만든 조각을 기반으로 하는 양식을 렌더링할 수 있습니다. A *조각* 는 양식의 재사용 가능한 부분이며 여러 양식 디자인에 삽입할 수 있는 별도의 XDP 파일로 저장됩니다. 예를 들어 조각에는 주소 블록 또는 법적 텍스트가 포함될 수 있습니다.

조각을 사용하면 많은 수의 양식을 간편하게 만들고 유지 관리할 수 있습니다. 새 양식을 만들 때 필요한 조각에 대한 참조를 삽입하면 조각이 양식에 나타납니다. 조각 참조에는 실제 XDP 파일을 가리키는 하위 양식이 포함되어 있습니다. 조각을 기반으로 양식 디자인을 만드는 방법에 대한 자세한 내용은 [Forms 디자이너](https://www.adobe.com/go/learn_aemforms_designer_63_kr)
...
A *스크립트 조각* 에는 날짜 파서 또는 웹 서비스 호출과 같은 특정 개체와 별도로 저장되는 재사용 가능한 JavaScript 함수 또는 값이 포함되어 있습니다. 이러한 조각에는 계층 팔레트에서 변수의 하위로 표시되는 단일 스크립트 개체가 포함되어 있습니다. 유효성 검사, 계산 또는 초기화와 같은 이벤트 스크립트와 같은 다른 개체의 속성인 스크립트에서 조각을 만들 수 없습니다.
...
### 조각을 사용하여 조립된 양식 디자인 조립하기 {#assembling-a-form-design-assembled-using-fragments}

양식 디자인을 어셈블하여 여러 조각을 기반으로 Forms 서비스로 전달할 수 있습니다. 여러 조각을 어셈블하려면 어셈블러 서비스를 사용합니다. 다른 Forms 서비스(출력 서비스)에서 사용하는 양식 디자인을 만들기 위해 조합 서비스를 사용하는 예를 보려면 를 참조하십시오 [조각을 사용하여 PDF 문서 생성](/help/forms/developing/creating-document-output-streams.md#creating-pdf-documents-using-fragments). 출력 서비스를 사용하는 대신 Forms 서비스를 사용하여 동일한 워크플로우를 수행할 수 있습니다.
...
>[!NOTE]
>
>조각을 기반으로 양식을 렌더링하는 웹 기반 응용 프로그램을 만드는 방법에 대한 자세한 내용은 [Forms을 렌더링하는 웹 애플리케이션 만들기](/help/forms/developing/creating-web-applications-renders-forms.md).
...
한 위치에 양식을 배치하고 다른 위치에 조각을 배치하여 양식 및 해당 조각을 구성하거나 모든 XDP 파일을 동일한 위치에 배치할 수 있습니다. 이 섹션을 위해 모든 XDP 파일이 AEM Forms 저장소에 있다고 가정합니다. AEM Forms 저장소에 XDP 파일 배치에 대한 자세한 내용은 [리소스 쓰기](/help/forms/developing/aem-forms-repository.md#writing-resources).
...
조각을 기반으로 하는 양식을 조각화되지 않은 양식과 동일한 방식으로 렌더링할 수 있습니다. 즉, 양식을 PDF, HTML 또는 양식 안내서(더 이상 사용되지 않음)로 렌더링할 수 있습니다. 이 섹션의 예제에서는 조각을 기반으로 하는 양식을 대화형 PDF 양식으로 렌더링합니다. (자세한 내용은 [대화형 PDF forms 렌더링](/help/forms/developing/rendering-interactive-pdf-forms.md))
...
1. URI 값 지정
   ...
   * 를 호출합니다 `URLSpec` 개체 `setContentRootURI` 메서드 및 컨텐츠 루트 URI 값을 지정하는 문자열 값을 전달합니다. 양식 디자인과 조각이 컨텐츠 루트 URI에 있는지 확인합니다. 없는 경우 Forms 서비스에 예외가 발생합니다. 저장소를 참조하려면 `repository://`.
...
1. 양식 렌더링
   ...
   * A `PDFFormRenderSpec` 런타임 옵션을 저장하는 개체입니다. 입력 문서가 PDF 문서인 경우 태그가 지정된 PDF 옵션을 설정할 수 없습니다. 입력 파일이 XDP 파일인 경우 태그가 지정된 PDF 옵션을 설정할 수 있습니다.
   * A `URLSpec` Forms 서비스에 필요한 URI 값을 포함하는 객체입니다.
   * A `java.util.HashMap` 첨부 파일을 저장하는 객체입니다. 선택적 매개 변수이며 `null` 양식에 파일을 첨부하지 않으려면
   * 비어 있음 `com.adobe.idp.services.holders.BLOBHolder` 메서드로 채워지는 개체입니다. 이 매개 변수는 렌더링된 양식을 저장하는 데 사용됩니다.
