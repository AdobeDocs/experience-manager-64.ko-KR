...
* PDF forms으로 데이터 가져오기 자세한 내용은 양식 데이터 [가져오기를 참조하십시오](importing-exporting-data.md#importing-form-data).
* PDF forms에서 데이터 내보내기 자세한 내용은 양식 데이터 [내보내기를 참조하십시오](importing-exporting-data.md#exporting-form-data).

>[!NOTE]
>
>양식 데이터 통합 서비스에 대한 자세한 내용은 AEM Forms에 대한 [서비스 참조를 참조하십시오](https://www.adobe.com/go/learn_aemforms_services_63).
...
프로그래밍 방식으로 데이터를 PDF 양식 클라이언트 API로 가져오기 전에 데이터 통합 서비스 클라이언트를 만들어야 합니다. 서비스 클라이언트를 만들 때 서비스를 호출하는 데 필요한 연결 설정을 정의합니다. 자세한 내용은 연결 [속성 설정을 참조하십시오](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties).
...
1. 양식 데이터 통합 서비스 클라이언트를 만듭니다.
   ...
   * 생성자를 사용하여 개체를 `FormDataIntegrationClient` 만들고 개체를 `ServiceClientFactory` 전달합니다.

1. PDF 양식 참조

   * 생성자를 사용하여 `java.io.FileInputStream` 개체를 만듭니다. PDF 양식의 위치를 지정하는 문자열 값을 전달합니다.
   * 생성자를 사용하여 PDF 양식을 저장하는 `com.adobe.idp.Document` 개체를 `com.adobe.idp.Document` 만듭니다. PDF 양식이 포함된 `java.io.FileInputStream` 개체를 생성자에게 전달합니다.

1. XML 데이터 소스를 참조합니다.

   * 생성자를 사용하여 `java.io.FileInputStream` 개체를 만들고 폼으로 가져올 데이터가 포함된 XML 파일의 위치를 지정하는 문자열 값을 전달합니다.
   * 생성자를 사용하여 양식 데이터를 저장하는 `com.adobe.idp.Document` 개체를 `com.adobe.idp.Document` 만듭니다. 양식 데이터가 포함된 `java.io.FileInputStream` 개체를 생성자에 전달합니다.

1. 데이터를 PDF 양식으로 가져올 수 있습니다.

   개체의 메서드를 호출하고 다음 값을 전달하여 데이터를 PDF 양식 `FormDataIntegrationClient` 으로 `importData` 가져옵니다.

   * PDF 양식을 저장하는 `com.adobe.idp.Document` 개체입니다.
   * 양식 데이터를 저장하는 `com.adobe.idp.Document` 개체입니다.

   이 `importData` 메서드는 XML 데이터 소스에 있는 데이터를 포함하는 PDF 양식을 저장하는 `com.adobe.idp.Document` 개체를 반환합니다.

1. PDF 양식을 PDF 파일로 저장합니다.

   * 개체를 만들고 파일 확장자가 &quot;.PDF&quot;인지 확인합니다. `java.io.File`
   * 객체의 메서드를 호출하여 `Document` 객체의 내용을 파일에 복사합니다(메서드에서 반환된 `copyToFile` 객체를 `Document` `Document` `importData` 사용해야 함).
...
1. 양식 데이터 통합 서비스 클라이언트를 만듭니다.
   ...
   * 생성자를 사용하여 `FormDataIntegrationClient.Endpoint.Address` 개체를 `System.ServiceModel.EndpointAddress` 만듭니다. WSDL을 지정하는 문자열 값을 AEM Forms 서비스에 전달합니다(예: `http://localhost:8080/soap/services/FormDataIntegration?blob=mtom`.) 속성을 사용할 필요는 `lc_version` 없습니다. 이 속성은 서비스 참조를 만들 때 사용됩니다. 하지만 MTOM `?blob=mtom` 을 사용하도록 지정합니다.
   * 필드의 값을 `System.ServiceModel.BasicHttpBinding` 가져와 개체를 `FormDataIntegrationClient.Endpoint.Binding` 만듭니다. 반환 값을 다음으로 캐스팅합니다 `BasicHttpBinding`.
   * 개체 `System.ServiceModel.BasicHttpBinding` 필드를 (으)로 `MessageEncoding` 설정합니다 `WSMessageEncoding.Mtom`. 이 값을 사용하면 MTOM이 사용됩니다.
   * 다음 작업을 수행하여 기본 HTTP 인증을 활성화합니다.

      * 필드에 AEM 양식 사용자 이름을 지정합니다 `FormDataIntegrationClient.ClientCredentials.UserName.UserName`.
      * 해당 암호 값을 필드에 지정합니다 `FormDataIntegrationClient.ClientCredentials.UserName.Password`.
...
1. 데이터를 PDF 양식으로 가져올 수 있습니다.
   ...
   * 양식 데이터를 저장하는 `BLOB` 개체입니다.
   ...
   이 `importData` 메서드는 XML 데이터 소스에 있는 데이터를 포함하는 PDF 양식을 저장하는 `BLOB` 개체를 반환합니다.

1. PDF 양식을 PDF 파일로 저장합니다.

   * 생성자를 호출하고 PDF 파일의 파일 위치를 나타내는 문자열 값을 전달하여 `System.IO.FileStream` 개체를 만듭니다.
   * 메서드에서 반환된 개체의 데이터 내용을 저장하는 바이트 배열을 `BLOB` `importData` 만듭니다. 개체 필드의 값을 가져와 바이트 배열 `BLOB` 을 `MTOM` 채웁니다.
...
프로그래밍 방식으로 데이터를 PDF formClient API로 가져오기 전에 데이터 통합 서비스 클라이언트를 만들어야 합니다. 서비스 클라이언트를 만들 때 서비스를 호출하는 데 필요한 연결 설정을 정의합니다. 자세한 내용은 연결 [속성 설정을 참조하십시오](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties).
...
1. PDF 양식에서 데이터 내보내기
   ...
   개체의 메서드를 호출하여 양식 데이터 `FormDataIntegrationClient` 를 `exportData` 내보내고 PDF 양식을 저장하는 `com.adobe.idp.Document` 개체를 전달합니다. 이 메서드는 양식 데이터를 XML 스키마로 저장하는 `com.adobe.idp.Document` 객체를 반환합니다.

1. PDF 양식을 PDF 파일로 저장합니다.

   * 객체를 만들고 파일 확장자가 XML인지 확인합니다. `java.io.File`
   * 객체의 메서드를 호출하여 `Document` 객체의 내용을 파일에 복사합니다(메서드에서 반환된 `copyToFile` 객체를 `Document` `Document` `exportData` 사용해야 함).
...
1. PDF 양식에서 데이터 내보내기
   ...
   오브젝트의 방법을 불러오고 PDF 양식을 저장하는 `FormDataIntegrationClient` `exportData` `BLOB` 오브젝트를 전달하여 데이터를 PDF 양식으로 가져옵니다. 이 메서드는 양식 데이터를 XML 스키마로 저장하는 `BLOB` 객체를 반환합니다.

1. PDF 양식을 PDF 파일로 저장합니다.

   * 생성자를 호출하고 XML 파일의 위치를 나타내는 문자열 값을 전달하여 `System.IO.FileStream` 개체를 만듭니다.
   * 메서드에서 반환된 개체의 데이터 내용을 저장하는 바이트 배열을 `BLOB` `exportData` 만듭니다. 개체 필드의 값을 가져와 바이트 배열 `BLOB` 을 `MTOM` 채웁니다.
   * 생성자를 호출하고 개체를 전달하여 `System.IO.BinaryWriter` 개체를 `System.IO.FileStream` 만듭니다.
