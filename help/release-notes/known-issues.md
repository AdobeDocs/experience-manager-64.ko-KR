...
이 페이지는 2018년 4월에 릴리스된 Adobe Experience Manager 6.4의 알려진 문제 목록을 유지합니다. 알려진 문제에 대한 자세한 내용은 지원 [센터에 문의하십시오](https://helpx.adobe.com/kr/support/experience-manager.html).
...
* 주소 표시줄 `about:flags` 을 입력하고 Return 키를 누릅니다.
* 시험적 기능으로 스크롤 후 **[!UICONTROL 터치를 누릅니다]**.
...
* **작업 대시보드:** 백업 파일에 .zip 확장명이 누락된 경우 진행률 표시줄이 표시되지 않습니다. (GRANITE-10713)
* **HTL:** 패키지 선언에서 후행 공백이 있는 Java Use 개체가 SightlyJavaCompilerService(GRANITE-20836)를 멈춥니다.
...
* **도구 - CRX DE Lite:** 속성 이름(GRANITE-12351)을 두 번 클릭하지 않으면 &quot;Long&quot; 값 유형에 대한 변경 내용을 저장할 수 없습니다.

* **도구 - CRX DE Lite:** RegExp 검색 시 Ctrl+F 검색 기능이 정지됩니다(GRANITE-5996).

* **도구 - CRX DE Lite:** 노드 이름을 바꾼 후 노드 속성이 표시되지 않음(GRANITE-7160)
* **UI:** 풀다운 &quot;자세히..&quot; IE 및 Firefox의 팝업 요소에서 열 때 일부 요소가 표시되지 않음(GRANITE-16326)
* **UI:** 2개의 나란히 열(GRANITE-16869)을 사용하는 고정 열 레이아웃을 사용할 때 정보 툴팁이 숨겨집니다.
* **UI:** 존재하지 않는 사용자로 가장할 때 처리되지 않은 오류가 발생했습니다(GRANITE-23228). 오류 메시지 [를 사용자 지정하는 오류](/help/sites-developing/customizing-errorhandler-pages.md) 처리기를 구현하여 해결책.

* **Omnisearch:** 백슬래시 원인 예외가 있는 검색(GRANITE-11769)
...
* **검색**: 검색 문자열이 공백 [OAK-4786으로 시작하는 경우 검색은 결과를 반환하지 않습니다.](https://issues.apache.org/jira/browse/OAK-4786)
...
* **보고서**: 예약된 일반 자산 보고서 생성이 일관성 없이 실패함(일부 보고서는 큐에 있음) (CQ-4239089)

* **메타데이터**: 자산 스키마에 다중 값 텍스트 필드를 추가할 때 필요한 필드 계단식 규칙이 작동하지 않습니다(CQ-4239333).

* **BrandPortal**: BrandPortal에 게시가 컬렉션에서 작동하지 않습니다(CQ-4238731).

* **업로드**: 파일 이름에 특수 문자가 포함된 자산을 업로드할 때 각 자산에 대해 허용되지 않는 문자에 대한 유효성 검사 오류 메시지가 표시되지 않습니다. 첫 번째 에셋에만 표시되지만 제공된 에셋의 파일 이름이 허용되지 않는다는 것을 인터페이스가 사용자에게 분명히 표시합니다. (CQ-4256876)
...
* 인쇄 미리 보기의 데이터 표시 형식은 dd-mm-yyyy이고 웹 미리 보기는 `dd-mmm-yy` (CQ-4237045)
...
* 웹 채널이 인쇄 채널과 동기화할 때 다음 문제가 관찰됩니다.
   ...
   * Live Copy를 동기화하는 동안 오류가 발생했습니다. 오류로 인해 동기화가 실패하는 경우가 있습니다. 문제를 해결하려면 페이지를 새로 고칩니다.
   * 표의 첫 번째 열이 인쇄 채널 템플릿의 헤더 열이면 레이아웃 조각의 정적 텍스트는 표 셀 이름으로 바뀝니다.
...
* SOAP 기반 웹 서비스에 대한 인증 구성은 보이지 않으므로 클라우드 서비스에서 구성할 수 없습니다. 문제를 해결하려면 다음을 수행하십시오.
   ...
   1. 값 속성 값을 텍스트 속성 값과 동일하게 업데이트합니다.
