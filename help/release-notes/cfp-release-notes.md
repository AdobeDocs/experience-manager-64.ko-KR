...
AEM Cumulative Fix Pack 6.4.8.3은 2020년 3월 AEM 6.4 서비스 팩 8(6.4.8.0)의 일반 출시 이후 여러 가지 내부 및 고객 픽스를 포함하는 중요한 업데이트입니다.

AEM 6.4.8.3은 AEM 6.4 서비스 팩 8에 따라 제공되는 CFP(Cumulative Fix Pack)입니다. AEM 6.4 서비스 팩 8을 설치한 후 CFP를 설치합니다.

AEM 6.4.8.3에서는 내장된 저장소(Apache Jackrabbit Oak)가 버전 1.8.23으로 업데이트됩니다.
...
Adobe Experience Manager 6.4.8.3에서는 다음 문제에 대한 수정 사항을 제공합니다.

### 사이트 {#sites-6483}

* 컨텐츠 조각 변형의 텍스트를 업데이트할 때 마스터 컨텐츠 조각 컨텐츠가 변형 대신 업데이트됩니다(NPR-35080).

* 구성 요소의 문자열 유형 레이블 속성에 대한 숫자 값을 설정하고, 구성 요소를 삭제하고, 실행 취소 옵션을 사용하여 다시 가져오면, 레이블 속성의 유형은 자동으로 String에서 Long(NPR-34738)으로 변경됩니다.

* 다중 필드에 파일 업로드 구성 요소를 추가하면 이미지 경로가 다중 필드 노드(NPR-34423) 대신 구성 요소 노드에 저장됩니다.

* 페이지 이동 마법사에서, 대상을 선택하지 않아도 다음 단추는 활성화된 상태로 유지됩니다(NPR-34460).

* 상위 구성 요소에 `cq:isContainer` 속성이 포함된 경우 상속된 구성 요소에 속성이 자동으로 포함되지 않습니다(CQ-4308409).

* `calc()` 함수를 사용하여 CSS 미니폴리션을 사용하는 경우 `+` 기호 주위의 빈 공백이 제거됩니다(NPR-34991).

* AEM 인스턴스를 시작하면 `com.adobe.granite.maintenance.impl.MaintenanceTaskManagerImpl` 및 `com.adobe.granite.maintenance.impl.TaskScheduler` 구성 요소가 `Active` 상태(NPR-34952)에 표시되지 않습니다.
...
* 폴더로 자산을 드래그하여 이동할 때 사용자 인터페이스에는 Lightbox[!UICONTROL  및 ]Drop in Collection[!UICONTROL 에 대한 옵션도 표시됩니다. ] 이동 작업이 취소되더라도 사용자 인터페이스는 이후 두 옵션(NPR-34525)을 계속 표시합니다.
* 게시 관리 인터페이스를 열면 게시 옵션을 사용할 수 없으며 게시 취소 옵션을 선택하면 범위 페이지가 비어 있습니다(CQ-4302509).

#### [!DNL Dynamic Media] {#dynamic-media}

* 이미지 사전 설정 설정에서 [!UICONTROL JPG 색차 다운샘플링 활성화] 옵션이 [!DNL Experience Manager]에서 선택 해제되면 변경 내용이 [!DNL Dynamic Media](NPR-34284)과 동기화되지 않습니다.
...
* `emitUseStrict` 플래그가 GCC(Google Closure Compiler) 프로세서 함수 `com.adobe.granite.ui.clientlibs.impl.HtmlLibraryManagerImpl`에 추가됩니다. 이 플래그는 `use strict` 명령의 출력을 억제 합니다(NPR-34830).
...
* [!UICONTROL Create] 단추가 [!UICONTROL Audiences] 페이지에서 [!UICONTROL Audiences] 페이지로 이동합니다(NPR-35152).

### 사용자 인터페이스 {#ui-6483}

* [!UICONTROL Omnisearch] 사용자 인터페이스의 [!UICONTROL 필터] 검색 패널은 검색이 실행되는 위치 외의 위치의 결과도 반환합니다(NPR-34877).
* [!UICONTROL Omnisearch] 사용자 인터페이스에서 [!UICONTROL 필터] 패널을 닫을 때 왼쪽 레일이 [!UICONTROL 콘텐츠] 선택 항목으로 재설정되지 않습니다. 이 경우 [!UICONTROL 필터] 패널(NPR-34483)이 다시 열리지 않습니다.
...
* 제품의 불공정한 용어 인스턴스는 모두 승인된 용어(NPR-34506)로 대체됩니다.
...
* 조각이 캐시 무효화로 인해 응용 양식으로 로드되는 데 시간이 더 오래 걸립니다(NPR-34655).

* 접근성:탭 탐색은 응용 양식의 화면 판독기에 적합하지 않습니다(NPR-34550).

**서신 관리**

* ES3에서 자산을 마이그레이션할 때 자산은 두 가지 비편집 기본 조건(NPR-34971)을 포함합니다.
...
* `RolloutConfigManagerFactoryImpl`이(가) 롤아웃 구성을 로드할 수 없는 경우 누락된 구성을 로드하지 않습니다. 캐시된 구성을 반환합니다(NPR-34091).
...
* [!UICONTROL 라이브 사용] 탭에서 49개의 결과만 표시됩니다. 구성 요소에 대한 모든 사용법은 표시되지 않습니다(NPR-33710).
...
* 페이지 이름과 동일한 이름으로 블루프린트 페이지에 구성 요소를 만들 수 있습니다. 롤아웃 중에 이러한 구성 요소의 이름은 접미사 `_msm_moved`로 변경됩니다. 그러나 구성 요소는 [!UICONTROL 단락 시스템](NPR-33534)의 끝으로 이동합니다.
* 첫 번째 컨텐츠 루트에서 [!UICONTROL 하위 페이지 포함] 속성이 선택되지 않은 경우 시작 판촉 행사가 페이지를 게시하지 않습니다(NPR-33533).
* `PageRedirectServlets`이(가) URL 조각 또는 앵커 뒤에 쿼리 문자열(NPR-34287)을 제시하므로 앵커가 있는 [!DNL Experience Manager] 페이지로 리디렉션할 수 없습니다.
* `PageRedirectServlet` 링크 실패로 연결되는 Sling 매핑  `.html` 후에 추가합니다(NPR-34271).
* 페이지의 [!DNL Live Copy]을 일시 중단할 수 있으며 상속은 편집기 모드에서 보듯이 끊깁니다. 그러나 페이지 속성에서 상속을 나타내는 아이콘은 상속이 존재하며 끊어진 것이 아님을 잘못 나타냅니다(NPR-34096).
...
* 사용자가 자산 세트를 선택한 후 [!UICONTROL 필터] 옵션을 선택한 다음 이동할 다른 자산 세트를 선택한 경우 이전에 선택한 자산도 새 위치로 이동됩니다(NPR-34018).
...
* [!UICONTROL Adobe Experience Manager]의 이미지 사전 설정 변경 사항이 Scene7 게시 시스템과 동기화되지 않습니다(NPR-34284, CQ-429713).

* [!UICONTROL 뷰어 사전 설정 편집기] 페이지의 [!UICONTROL 비헤이비어] 탭에서 [!UICONTROL PANORAMICVIEW_AUTOOTATE] 수정자 레이블이 없습니다(CQ-4302043).

#### 플랫폼 {#platform-6482}

* 기본 에이전트(게시) 구성에 대한 **[!UICONTROL Connect Timeout]** 및 **[!UICONTROL 소켓 Timeout]** 설정에 대한 기본값이 지정되지 않았습니다(NPR-33708).
...
* [!DNL Adobe Dynamic Tag Management]에서 마이그레이션된 라이브러리에 대한 [!DNL Adobe Launch] 라이브러리 URL을 생성할 때 `library_path`의 값이 고려되지 않습니다. 또한 마이그레이션된 라이브러리는 [!DNL Adobe Launch] 라이브러리와 다른 접두어를 사용합니다. (NPR-34238).
...
* **[!UICONTROL Revalidate on Server]** 속성을 선택하고 여러 첨부 파일을 업로드하면 응용 양식이 제출되지 않습니다(NPR-33701).

* **[!UICONTROL 페이지 언어 사용]** 및 **[!UICONTROL 양식이 [!DNL Experience Manager Sites] 페이지의 [!DNL Experience Manager Forms] 구성 요소에서 페이지]** 옵션의 전체 너비를 덮을 때 페이지가 변환되지 않습니다(NPR-33641).

* [!DNL Experience Manager Sites] 페이지에 포함된 분석 지원 응용 양식을 제출하면 분석이 올바르게 작동하지 않습니다(NPR-31359).

* [!DNL Lodash] 및 [!DNL backbone] 라이브러리에 대한 종속성을 제거했습니다(NPR-33458).

* REST 끝점에 제출&#x200B;**[!UICONTROL 제출 동작은 응용 양식에 대해 작동하지 않습니다(NPR-34513).]**
...
* [!DNL Experience Manager] 6.4.8.1을 설치할 때 항목의 [!UICONTROL To Do] 목록이 링크로 표시되지 않습니다. [!UICONTROL To Do] 항목에 대한 텍스트는 HTML 태그를 포함합니다(NPR-34318).

**백엔드 통합**

* AWS에서 호스팅하는 [!DNL Experience Manager Forms Linux] 환경에서 양식 데이터 모델을 구성할 수 없습니다(NPR-33617).
...
* [!DNL Experience Manager] 6.4.8.0(NPR-34309)을 설치한 후 PDF 파일에서 HSM 기반 인증서로 서명 작업을 실행할 수 없습니다.

**업그레이드**

* [!DNL Linux] 환경에서 Document Security를 사용하여 [!DNL Experience Manager Forms]에 대해 [!DNL JBoss] 버전을 7.0.9로 업그레이드하면 오류가 발생합니다(CQ-4300546).
...
* CRXDE Lite에 대한 익명의 액세스는 보안을 강화하도록 허용되지 않습니다. 대신 사용자에게 로그인 화면이 표시됩니다. [CRXDE Lite](/help/sites-developing/developing-with-crxde-lite.md)으로 개발 작업을 참조하십시오.
...
* 추가 폴더는 이름에 점 `.` 문자가 포함된 상위 폴더 내에 있는 자산을 스마트 자르기에 대해 만들어집니다(NPR-32755).
...
* `ScriptProcessor` API를 사용하여 JavaScript 라이브러리를 축소하면 로그 파일에 JavaScript 코드가 엄격한 모드를 준수하지 않음을 나타내는 오류 메시지가 표시됩니다. API는 엄격 모드를 활성화하거나 비활성화할 수 있는 옵션을 제공하지 않습니다. (NPR-32746).
...
>[!NOTE]
>
>AEM 누적 수정 팩에는 AEM Forms에 대한 수정 사항이 포함되어 있지 않습니다. 이러한 수정 사항은 별도의 Forms 추가 기능 패키지를 사용하여 전달됩니다. 또한, JEE의 AEM Forms에 대한 수정 사항이 포함된 누적 설치 프로그램이 릴리스됩니다. 자세한 내용은 [AEM Forms 추가 기능 패키지](#install-aem-forms-add-on-package) 및 [AEM Forms JEE 설치 프로그램](#install-aem-forms-jee-installer) 설치를 참조하십시오.
...
* 적응형 Forms:인스턴스 관리자 클래스에 대한 `AddInstance` 및 `RemoveInstance` 메서드는 [!DNL Internet Explorer 11]에 지연 로드 조각에 대한 동적 인스턴스 수를 추가하지 않습니다(NPR-33201).
...
* 문서 서비스:[!DNL Linux] 서버에서 [!DNL WebLogic]에 대한 깁슨 항아리가 누락되어 PDF 문서를 PostScript로 변환하지 못합니다(NPR-33515, CQ-4292239).
...
* AEM 6.4.8.3에는 AEM 6.4.8.0이 필요합니다. 자세한 내용은 [업그레이드 설명서](../sites-deploying/upgrade.md)를 참조하십시오.
...
1. 제품 정보 페이지(*/system/console/productinfo*)에 설치된 제품 아래에 업데이트된 버전 문자열 &quot;Adobe Experience Manager, 버전 6.4.8.3&quot;이 표시되어야 합니다.
...
이 AEM Sites 및 자산 릴리스에서 실행할 인증된 플랫폼을 결정하려면 [기술 요구 사항](../sites-deploying/technical-requirements.md)을 참조하십시오.

>[!NOTE]
>패키지를 성공적으로 설치할 때 **&quot;Content Package AEM-6.4-Service-Pack-8이 성공적으로 설치되었음을 나타내는 정보 메시지가 나타납니다.&quot;**

### 동적 미디어 뷰어 업데이트(5.10.1) {#update-dynamic-media-viewers}

AEM 6.4.8.3에는 이미지 사전 설정 페이지에서 중복 이름을 확인할 수 있는 새로운 버전의 Dynamic Media 뷰어(5.10.1)가 포함되어 있습니다. Dynamic Media 고객은 박스 뷰어 사전 설정에서 최신 상태로 전환하려면 다음 명령을 실행해야 합니다.
...
1. 운영 체제용 [AEM Forms 릴리스](https://helpx.adobe.com/kr/aem-forms/kb/aem-forms-releases.html)에 나열된 해당 양식 추가 기능 패키지를 다운로드하십시오.
1. [AEM forms Add-on 패키지 설치](https://docs.adobe.com/content/help/en/experience-manager-64/forms/install-aem-forms/osgi-installation/installing-configuring-aem-forms-osgi.html#install-aem-forms-add-on-package)에 설명된 대로 양식 추가 기능 패키지를 설치합니다.
...
AEM Forms JEE용 누적 설치 프로그램 및 배포 후 구성 설치에 대한 자세한 내용은 [AEM Forms JEE 패치 설치 프로그램](jee-patch-installer-64.md)을 참조하십시오.
...
Maven 프로젝트에서 Uber Jar를 사용하려면 [Uber jar](../sites-developing/ht-projects-maven.md)을 사용하는 방법을 참조하고 프로젝트 POM에 다음 종속성을 포함하십시오.
...
>[!NOTE]
>
>UberJar 및 기타 관련 가공물은 Adobe Public Maven 리포지토리(repo.adobe.com) 대신 Maven Central Repository에서 사용할 수 있습니다. 기본 UberJar 파일의 이름이 `uber-jar-<version>.jar`로 변경되었습니다. 따라서 `dependency` 태그의 값으로 `apis`가 있는 `classifier`이 없습니다.
...
| Assets과 Adobe Creative Cloud 통합 | [AEM-Creative Cloud 폴더 ](https://docs.adobe.com/content/help/en/experience-manager-64/assets/administer/aem-cc-folder-sharing-best-practices.html) 공유 기능은 크리에이티브 사용자가 AEM의 에셋에 액세스할 수 있도록 하는 방법으로 AEM 6.2에서 도입되었습니다. Creative Cloud 애플리케이션에서 새롭게 출시된 기능인 Adobe Asset Link는 Photoshop, InDesign 및 Illustrator에서 직접 AEM 자산에 액세스할 수 있는 강력한 권한과 함께 우수한 사용자 경험을 제공합니다. Adobe는 폴더 공유 기능에 대한 추가 개선을 하지 않습니다. 이 기능은 AEM에 포함되어 있지만, 고객은 이 교체 기능을 사용하는 것이 좋습니다. | Adobe 자산 링크 또는 데스크탑 앱. 자세한 내용은 [AEM Creative Cloud 통합](/help/assets/aem-cc-integration-best-practices.md) 문서를 참조하십시오. | AEM 6.4.4.0 |

## 알려진 문제 {#known-issues}

* [!DNL Experience Manager] 6.4에서 [!DNL Experience Manager] 6.5로 업그레이드하는 경우 일부 번들의 상태가 `Active`로 표시되지 않을 수 있습니다. 최신 [!DNL Experience Manager] 6.5 서비스 팩을 설치하여 문제를 해결하십시오.
...
다음 텍스트 문서에는 AEM 6.4.8.3에 포함된 OSGi 번들 및 컨텐츠 패키지가 나와 있습니다.
